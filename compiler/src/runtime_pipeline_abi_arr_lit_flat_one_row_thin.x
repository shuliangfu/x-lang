...
/**
 * wave438: emit one row of ARRAY-of-SLICE flatten.
 * @param arena *u8 — ASTArena*
 * @param elf_ctx *u8 — ELF ctx
 * @param init_ref i32 — outer ARRAY_LIT (unused)
 ...
 * @param inner i32 — SLICE elem type
 * @return i32 — 0 ok / -1 error
...
export function pipeline_asm_emit_array_lit_flat_one_row_elf_c(arena: *u8, elf_ctx: *u8, init_ref: i32, ctx: *u8, ta: i32, stack_slot_off: i32, leaf_esz: i32, flat_i: *i32, dest_elem: i32, elem_ref: i32, row_home: i32, inner: i32): i32 {
  unsafe {
    ...
    if (ko == 46) {
      return pipeline_asm_emit_array_lit_flat_slice_cells_elf_c(
          arena, elf_ctx, elem_ref, ctx, ta, stack_slot_off, leaf_esz, flat_i, dest_elem, elem_ref, row_home, inner);
    }
