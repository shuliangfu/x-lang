...
// wave585 Soft Cap: Ubuntu tip `-backend asm -c` UND=0 (T export
//   present). The original body used rc=pipeline_asm_emit_expr_elf_c
//   then if, then if (mod != 0 && fi >= 0) mid-assign rty/sty, then
//   rc=glue_maybe_promote_f32_to_f64_rax_elf_c then if, which that
//   tip drops. Darwin original kept the four encoders (4 UND).
//   arr_return_c_fallback_store_encoders always stores each encoder
//   once and declares no locals. The export returns 0; the real
//   path stays on the w439 overlay.
// stamp w585 HARD BAN tip PRODUCT reinject (both ends).
...
/**
 * Store the path-C fallback emit+float encoders. Offset 0 is emit
 * expr, 4 is module return type, 8 is float-promote src type, 12
 * is maybe-promote f32→f64. No locals. Each encoder runs once,
 * not under if or after a mid-assign. Never stores through *i32.
 ...
 */
function arr_return_c_fallback_store_encoders(arena: *u8, elf_ctx: *u8, ret_op: i32, ctx: *u8, ta: i32, mod: *u8, fi: i32, cell: *u8): i32 {
  unsafe {
...
/**
 * wave439/585: ARRAY_LIT fallback emit + float promote. Encoders
...
export function glue_emit_return_path_c_fallback_elf_c(arena: *u8, elf_ctx: *u8, ret_op: i32, ctx: *u8, ta: i32, mod: *u8, fi: i32): i32 {
  unsafe {
    let cell: u8[16] = [];
    let sink: i32 = 0;
    arr_return_c_fallback_store_encoders(arena, elf_ctx, ret_op, ctx, ta, mod, fi, &cell[0]);
    sink = ta + fi + ret_op;
