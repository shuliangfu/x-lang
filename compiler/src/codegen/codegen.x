 ...
 * Hard cap nest<=38 (4.2.3 1..16 + nest>16 soft 17..37 + nest>37 layer 38).
 * Piecewise emit — no u8[256] whole-line buffer. type_to_c_repr scratch is
 * 512; nest 21 i32 tag is 266 bytes; nest 37 is 458; nest 38 is 470
 * (12*38+14). Do not raise to 39 this leaf (tag=482 still fits 512).
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param nest i32 — slice nest depth; must be 1..38
 ...
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param min_nest i32 — inclusive start; must be 1..38
 * @param max_nest i32 — inclusive end; must be min_nest..38
 * @return i32 — 0 on success, -1 on emit failure
...
function codegen_emit_scalar_slice_nests(out: *CodegenOutBuf, min_nest: i32, max_nest: i32): i32 {
  ...
  if (min_nest < 1 || max_nest > 38 || min_nest > max_nest) {
    return -1;
  }
...
export function codegen_emit_companion_named_slice_layout(out: *CodegenOutBuf, pfx: *u8, pfx_len: i32, name: *u8, name_len: i32): i32 {
  ...
  /*
   * 4.2.3 + nest>16 soft: loop nest 1..38 through the shared fat emitter.
 ...
 * + nest>37 soft `[]×38` under XLANG_SLICE_LAYOUTS_N38). Without layouts,
