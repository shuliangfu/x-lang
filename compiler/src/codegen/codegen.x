...
export function emit_type(arena: *ASTArena, out: *CodegenOutBuf, type_ref: i32, struct_prefix: *u8, struct_prefix_len: i32, ctx: *PipelineDepCtx): i32 {
  ...
  unsafe {
    ...
    if (tk == TypeKind.TYPE_SLICE as i32 && !ast.ref_is_null(elem_ref)) {
              ...
              if (c_bt > 0 && c_bt != type_ref && g_bt > 0
                ...
                if (g_bt_nl == cur_sl_nl && g_bt_nl > 0) {
                  ...
                  if (eq_bt != 0) {
                     ...
                     */
                    let eb_bt: u8[512] = [];
                    let n_bt: i32 = type_to_c_repr(arena, &eb_bt[0], 512, c_bt, struct_prefix, struct_prefix_len);
                    if (n_bt > 0) {
                      let sp_bt: i32 = 0;
      ...
      let slb: u8[512] = [];
      let nl: i32 = type_to_c_repr(arena, &slb[0], 512, type_ref, pfx_use, pfx_len_use);
      if (nl <= 0) {
        return -1;
      }
      let si: i32 = 0;
 ...
 * Hard cap nest<=41 (4.2.3 1..16 + nest>16 soft 17..40 + nest>40 layer 41).
 * Piecewise emit — no u8[256] whole-line buffer. type_to_c_repr scratch is
 * 512; nest 21 i32 tag is 266 bytes; nest 40 is 494; nest 41 is 506
 * (12*41+14). Do not raise to 42 this leaf (tag=518 overflows 512).
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param nest i32 — slice nest depth; must be 1..41
 ...
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param min_nest i32 — inclusive start; must be 1..41
 * @param max_nest i32 — inclusive end; must be min_nest..41
 * @return i32 — 0 on success, -1 on emit failure
...
function codegen_emit_scalar_slice_nests(out: *CodegenOutBuf, min_nest: i32, max_nest: i32): i32 {
  ...
  if (min_nest < 1 || max_nest > 41 || min_nest > max_nest) {
    return -1;
  }
 ...
 * After `struct TAG { ... };` emit nest 1..41 companion fat layouts
 ...
 * (wave698 unrolled only to 7; nest>16 soft → 17..40 then nest>40 layer 41).
...
export function codegen_emit_companion_named_slice_layout(out: *CodegenOutBuf, pfx: *u8, pfx_len: i32, name: *u8, name_len: i32): i32 {
  ...
  /*
   * 4.2.3 + nest>16 soft: loop nest 1..41 through the shared fat emitter.
    ...
    while (ti <= nt) {
      if (pipeline_type_kind_ord_at(arena, ti) == (TypeKind.TYPE_SLICE as i32)) {
        ...
        if (!ast.ref_is_null(elem) && elem > 0 && elem <= nt
          ...
          let seen: i32 = 0;
          let slb: u8[512] = [];
          let nl: i32 = type_to_c_repr(arena, &slb[0], 512, ti, pfx_use, pfx_len_use);
          if (nl <= 0) {
            return -1;
          }
          let tj: i32 = 1;
          while (tj < ti) {
            if (pipeline_type_kind_ord_at(arena, tj) == (TypeKind.TYPE_SLICE as i32)) {
              ...
              if (!ast.ref_is_null(ej) && ej > 0 && ej <= nt
                ...
                let slj: u8[512] = [];
                let nj: i32 = type_to_c_repr(arena, &slj[0], 512, tj, pfx_use, pfx_len_use);
                if (nj == nl && nj > 0) {
                  let eq: i32 = 1;
                  let ci: i32 = 0;
                  while (ci < nl) {
 ...
 * + nest>40 soft `[]×41` under XLANG_SLICE_LAYOUTS_N41). Without layouts,
