 ...
 * Hard cap nest<=40 (4.2.3 1..16 + nest>16 soft 17..39 + nest>39 layer 40).
 * Piecewise emit — no u8[256] whole-line buffer. type_to_c_repr scratch is
 * 512; nest 21 i32 tag is 266 bytes; nest 39 is 482; nest 40 is 494
 * (12*40+14). Do not raise to 41 this leaf (tag=506 still fits 512).
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param nest i32 — slice nest depth; must be 1..40
 ...
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param min_nest i32 — inclusive start; must be 1..40
 * @param max_nest i32 — inclusive end; must be min_nest..40
 * @return i32 — 0 on success, -1 on emit failure
...
function codegen_emit_scalar_slice_nests(out: *CodegenOutBuf, min_nest: i32, max_nest: i32): i32 {
  ...
  if (min_nest < 1 || max_nest > 40 || min_nest > max_nest) {
    return -1;
  }
 ...
 * After `struct TAG { ... };` emit nest 1..40 companion fat layouts
 ...
 * (wave698 unrolled only to 7; nest>16 soft → 17..39 then nest>39 layer 40).
...
export function codegen_emit_companion_named_slice_layout(out: *CodegenOutBuf, pfx: *u8, pfx_len: i32, name: *u8, name_len: i32): i32 {
  ...
  /*
   * 4.2.3 + nest>16 soft: loop nest 1..40 through the shared fat emitter.
 ...
 * + nest>39 soft `[]×40` under XLANG_SLICE_LAYOUTS_N40). Without layouts,
