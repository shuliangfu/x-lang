 ...
 * Hard cap nest<=39 (4.2.3 1..16 + nest>16 soft 17..38 + nest>38 layer 39).
 * Piecewise emit — no u8[256] whole-line buffer. type_to_c_repr scratch is
 * 512; nest 21 i32 tag is 266 bytes; nest 38 is 470; nest 39 is 482
 * (12*39+14). Do not raise to 40 this leaf (tag=494 still fits 512).
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param nest i32 — slice nest depth; must be 1..39
 ...
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param min_nest i32 — inclusive start; must be 1..39
 * @param max_nest i32 — inclusive end; must be min_nest..39
 * @return i32 — 0 on success, -1 on emit failure
...
function codegen_emit_scalar_slice_nests(out: *CodegenOutBuf, min_nest: i32, max_nest: i32): i32 {
  ...
  if (min_nest < 1 || max_nest > 39 || min_nest > max_nest) {
    return -1;
  }
 ...
 * After `struct TAG { ... };` emit nest 1..38 companion fat layouts
 ...
 * (wave698 unrolled only to 7; nest>16 soft → 17..37 then nest>37 layer 38).
...
export function codegen_emit_companion_named_slice_layout(out: *CodegenOutBuf, pfx: *u8, pfx_len: i32, name: *u8, name_len: i32): i32 {
  ...
  /*
   * 4.2.3 + nest>16 soft: loop nest 1..39 through the shared fat emitter.
 ...
 * + nest>38 soft `[]×39` under XLANG_SLICE_LAYOUTS_N39). Without layouts,
