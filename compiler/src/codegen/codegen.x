...
export function emit_type(arena: *ASTArena, out: *CodegenOutBuf, type_ref: i32, struct_prefix: *u8, struct_prefix_len: i32, ctx: *PipelineDepCtx): i32 {
  ...
  unsafe {
    ...
    if (tk == TypeKind.TYPE_SLICE as i32 && !ast.ref_is_null(elem_ref)) {
              ...
              if (c_bt > 0 && c_bt != type_ref && g_bt > 0
                ...
                if (g_bt_nl == cur_sl_nl && g_bt_nl > 0) {
                  ...
                  if (eq_bt != 0) {
                     ...
                     */
                    let eb_bt: u8[640] = [];
                    let n_bt: i32 = type_to_c_repr(arena, &eb_bt[0], 640, c_bt, struct_prefix, struct_prefix_len);
                    if (n_bt > 0) {
                      let sp_bt: i32 = 0;
      ...
      let slb: u8[640] = [];
      let nl: i32 = type_to_c_repr(arena, &slb[0], 640, type_ref, pfx_use, pfx_len_use);
      if (nl <= 0) {
        return -1;
      }
      let si: i32 = 0;
 ...
 * Hard cap nest<=52 (4.2.3 1..16 + nest>16 soft 17..41 + nest>41 layer 42
 * + nest>42 layer 43 + nest>43 layer 44 + nest>44 layer 45
 * + nest>45 layer 46 + nest>46 layer 47 + nest>47 layer 48
 * + nest>48 layer 49 + nest>49 layer 50 + nest>50 layer 51
 * + nest>51 layer 52).
 * Piecewise emit — no u8[256] whole-line buffer. type_to_c_repr scratch is
 * 640; nest 21 i32 tag is 266 bytes; nest 41 is 506; nest 42 is 518;
 * nest 43 is 530; nest 44 is 542; nest 45 is 554; nest 46 is 566;
 * nest 47 is 578; nest 48 is 590; nest 49 is 602; nest 50 is 614;
 * nest 51 is 626; nest 52 is 638 (12*52+14). Do not raise to 53 this leaf
 * (tag=650 overflows 640).
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param nest i32 — slice nest depth; must be 1..52
 ...
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param min_nest i32 — inclusive start; must be 1..52
 * @param max_nest i32 — inclusive end; must be min_nest..52
 * @return i32 — 0 on success, -1 on emit failure
...
function codegen_emit_scalar_slice_nests(out: *CodegenOutBuf, min_nest: i32, max_nest: i32): i32 {
  ...
  if (min_nest < 1 || max_nest > 52 || min_nest > max_nest) {
    return -1;
  }
 ...
 * After `struct TAG { ... };` emit nest 1..52 companion fat layouts
 ...
 * (wave698 unrolled only to 7; nest>16 soft → 17..41 then nest>41 layer 42
 * then nest>42 layer 43 then nest>43 layer 44 then nest>44 layer 45
 * then nest>45 layer 46 then nest>46 layer 47 then nest>47 layer 48
 * then nest>48 layer 49 then nest>49 layer 50 then nest>50 layer 51
 * then nest>51 layer 52).
...
export function codegen_emit_companion_named_slice_layout(out: *CodegenOutBuf, pfx: *u8, pfx_len: i32, name: *u8, name_len: i32): i32 {
  ...
  /*
   * 4.2.3 + nest>16 soft: loop nest 1..52 through the shared fat emitter.
    ...
    while (ti <= nt) {
      if (pipeline_type_kind_ord_at(arena, ti) == (TypeKind.TYPE_SLICE as i32)) {
        ...
        if (!ast.ref_is_null(elem) && elem > 0 && elem <= nt
          ...
          let seen: i32 = 0;
          let slb: u8[640] = [];
          let nl: i32 = type_to_c_repr(arena, &slb[0], 640, ti, pfx_use, pfx_len_use);
          if (nl <= 0) {
            return -1;
          }
          let tj: i32 = 1;
          while (tj < ti) {
            if (pipeline_type_kind_ord_at(arena, tj) == (TypeKind.TYPE_SLICE as i32)) {
              ...
              if (!ast.ref_is_null(ej) && ej > 0 && ej <= nt
                ...
                let slj: u8[640] = [];
                let nj: i32 = type_to_c_repr(arena, &slj[0], 640, tj, pfx_use, pfx_len_use);
                if (nj == nl && nj > 0) {
                  let eq: i32 = 1;
                  let ci: i32 = 0;
                  while (ci < nl) {
 ...
 * + nest>51 soft `[]×52` under XLANG_SLICE_LAYOUTS_N52). Without layouts,
...
export function codegen_x_ast_emit_header(out: *CodegenOutBuf): i32 {
  ...
  unsafe {
    ...
    if (emit_bytes_from_ptr(out, &g27[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g28[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g29[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g30[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g31[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g32[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g33[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g34[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g35[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g37[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g38[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g39[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g40[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g41[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g42[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g43[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g44[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g45[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g46[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g47[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g48[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g49[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g50[0], 64) != 0) {
      return -1;
    }
    ...
    if (emit_bytes_from_ptr(out, &g51[0], 64) != 0) {
      return -1;
    }
     ...
     * (fixed N=224). Do not grow seed emit_header u8[256]. type_to_c_repr
     * scratch is 640 so nest 52 i32 tag=638 fits. Do not raise to 53 this
     * leaf (tag=650 overflows 640).
     * PLATFORM: SHARED host-C. G.7: emit_header is the deep-nest authority.
     */
    ...
    if (emit_bytes_from_ptr(out, &g52[0], 64) != 0) {
      return -1;
    }
