 ...
 * Hard cap nest<=37 (4.2.3 1..16 + nest>16 soft 17..36 + nest>36 layer 37).
 * Piecewise emit — no u8[256] whole-line buffer. type_to_c_repr scratch is
 * 512; nest 21 i32 tag is 266 bytes; nest 36 is 446; nest 37 is 458
 * (12*37+14). Do not raise to 38 this leaf (tag=470 still fits 512).
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param nest i32 — slice nest depth; must be 1..37
 ...
 * @param out *CodegenOutBuf — C text buffer; null rejected
 * @param min_nest i32 — inclusive start; must be 1..37
 * @param max_nest i32 — inclusive end; must be min_nest..37
 * @return i32 — 0 on success, -1 on emit failure
...
function codegen_emit_scalar_slice_nests(out: *CodegenOutBuf, min_nest: i32, max_nest: i32): i32 {
  ...
  if (min_nest < 1 || max_nest > 37 || min_nest > max_nest) {
    return -1;
  }
 ...
 * After `struct TAG { ... };` emit nest 1..37 companion fat layouts
 ...
 * (wave698 unrolled only to 7; nest>16 soft → 17..36 then nest>36 layer 37).
...
export function codegen_emit_companion_named_slice_layout(out: *CodegenOutBuf, pfx: *u8, pfx_len: i32, name: *u8, name_len: i32): i32 {
  ...
  /*
   * 4.2.3 + nest>16 soft: loop nest 1..37 through the shared fat emitter.
 ...
 * + nest>36 soft `[]×37` under XLANG_SLICE_LAYOUTS_N37). Without layouts,
...
export function codegen_x_ast_emit_header(out: *CodegenOutBuf): i32 {
  ...
  unsafe {
     ...
     * (fixed N=224). Do not grow seed emit_header u8[256]. type_to_c_repr
     * scratch is 512 so nest 37 i32 tag=458 fits. Do not raise to 38 this
     * leaf (tag=470 still fits 512; one layer at a time).
     * PLATFORM: SHARED host-C. G.7: emit_header is the deep-nest authority.
     */
