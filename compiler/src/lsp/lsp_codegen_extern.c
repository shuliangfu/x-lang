/**
 * lsp_codegen_extern.c — LSP -E-extern 跨 TU extern / inline 包装（io.o、heap.o、lsp_io_su.o 桥接）。
 *
 * 自 codegen.c 内嵌块迁出，供 C codegen_library_module_to_c 与 codegen.su（经 emit_to_buf）共用，
...
/** 路径是否 lsp_io.su 的 -E-extern 入口。 */
int lsp_codegen_emit_path_is_lsp_io_su(const char *path) {
  return path != NULL && strstr(path, "lsp_io.su") != NULL;
}

/** 路径是否 lsp/lsp.su 的 -E-extern 入口（排除 lsp_io 等子路径误匹配）。 */
int lsp_codegen_emit_path_is_lsp_main_su(const char *path) {
  if (!path)
    return 0;
  if (strstr(path, "/lsp/lsp.su") != NULL || strstr(path, "\\lsp\\lsp.su") != NULL)
    return 1;
  if (strstr(path, "lsp/lsp.su") != NULL && strstr(path, "lsp_io") == NULL)
    return 1;
  return 0;
}

static const char *lsp_io_extern_block =
    "\n/* lsp_codegen_extern: lsp_io -E-extern stubs (io.o / heap.o / lsp_diag) */\n"
...
static const char *lsp_gen_extern_block =
    "\n/* lsp_codegen_extern: lsp.su -E-extern stubs (lsp_io_su.o 符号桥接) */\n"
