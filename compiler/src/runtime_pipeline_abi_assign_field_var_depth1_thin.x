// Thin pure: FIELD VAR-root depth-1 scalar store (wave441/w475).
// G.7: part of pipeline_asm_emit_assign_elf_c FIELD VAR-root path.
// wave475: no-local (ban `let rc=call()`). Tip U=3/3.
 ...
 * wave475: no-local — rhs→rax then slot ptr then store; no let-bound call.
