 ...
 * means no extra wrap). dim_ix==ndims+1 is extra PTR wrap count
      ...
      if (dyn_trait_nlen > 0) {
        ...
        while (arg_i < num_args) {
          ...
          if (dyn_arg > 0 && dyn_pk == 10) {
             ...
             * because wrap-once dest-stamps `[2][]i32`. Sit-red
             * `[2][]*i32` nested ARRAY_LIT `[[&n, &n], [&m, &m]]`
             * is 139 because wrap-once dest-stamps `[2][]i32` not
             * `[2][]*i32` (impl-match already matches at SLICE;
             * named / UFCS / typed lets dest-stamp via the formal
             * 6/7). Sit-red `[2][][2]i32` nested ARRAY_LIT
             * `[[[2, 3]], [[1, 4]]]` is 139 because wrap-once dest-
             * stamps `[2][]i32` not `[2][][2]i32` (skip-trait after
             * `[N][]` then `[M]` already stores ndims>=1 via the
             * wave437 pending LBRACKET dim collector; named / UFCS
             * / asg / typed lets already 7; impl-match leftover
             * SLICE vs eek=SLICE is not T001). Skip-trait stores
             * elem_kind=SLICE + eek=leaf after `[N][]`; extra inner
             * SLICE uses ndims=-2 with wrap count in dims[0] (0
             * means 1 = `[2][][]T`; 2 = `[2][][][]T`); extra inner
             ...
             * wrap below (no second dest-ARRAY stamp; do not invent
             * -3; do not add impl-match extra ARRAY peels). Twin of
             * dest extras dest-SLICE-of-ARRAY extra PTR wraps.
             * NAMED leaf of `[N][]Pair` is handled below via
             * param_name. dest extras dest-SLICE-of-SLICE extra PTR
             * (`[][]*T`) is handled in the dest-SLICE path (same
             * dims[0] extra PTR encoding; discriminant is SLICE
             * outer vs ARRAY outer). dest extras dest-SLICE-of-
             * SLICE extra ARRAY (`[][][2]T`) stays deferred (T001).
             * PLATFORM: SHARED.
             */
