...
/**
 * 以下为 .sx typeck 入口使用的 AST 查询 API（全面自举：typeck 逻辑迁入 .sx）。
 * 参数均以 void* 传递以与 .sx 的 *u8 一致；内部转为具体 AST 类型后访问。
 */
...
int typeck_sx_ast_type_i32(void);
int typeck_sx_ast_type_i64(void);
int typeck_sx_ast_type_bool(void);

/** 全面自举（重写）：仅填 resolved_type，不做语义检查；.sx 用访问器遍历并做检查。 */
int typeck_sx_fill_resolved_types(void *mod, void *dep_mods, int num_deps);

/** 块/表达式访问器：.sx 中实现遍历与「条件须为 bool」等判断。 */
...
/** 块内表达式语句与最终表达式：.sx 遍历 if/三元 所在表达式。 */
...
/** 表达式节点 kind（ASTExprKind）；供 .sx 判断是否为 IF/TERNARY/BLOCK 并递归。 */
...
int typeck_sx_expr_kind_if(void);
int typeck_sx_expr_kind_ternary(void);
int typeck_sx_expr_kind_block(void);
int typeck_sx_expr_kind_assign(void);
void *typeck_sx_expr_assign_left(const void *expr);
void *typeck_sx_expr_assign_right(const void *expr);
/** 两表达式 resolved_type 是否一致（C 的 type_equal）；供 .sx 做赋值类型一致检查。 */
int typeck_sx_types_equal(const void *expr_a, const void *expr_b);

/** .sx 自实现 typeck 用：块体结果类型是否与函数返回类型一致，返回 1 一致 0 否。 */
...
/** double 的 64 位位模式拆成低/高 32 位，供 .sx typeck 填写 EXPR_FLOAT_LIT 的 float_bits_lo/hi（asm 后端用）。 */
