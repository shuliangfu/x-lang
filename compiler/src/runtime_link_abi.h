...
#ifndef XLANG_RUNTIME_LINK_ABI_H
#define XLANG_RUNTIME_LINK_ABI_H

#include <stddef.h>

#ifndef XLANG_ASM_LD_PATH_BANK_SLOTS
...
#define XLANG_ASM_LD_PATH_BANK_SLOTS 192
#endif
#ifndef XLANG_LINK_ABI_PATH_MAX
#define XLANG_LINK_ABI_PATH_MAX 4096
#endif
#ifndef XLANG_INVOKE_CC_MAX_C_FILES
#define XLANG_INVOKE_CC_MAX_C_FILES 64
#endif
#ifndef XLANG_LD_ARGV_CAP
/** ASM ld/gcc 子进程 argv 槽位上限（须 ≥ std 模块 + glue + on_demand + -l 参数）。 */
#define XLANG_LD_ARGV_CAP 192
#endif

/** ASM 链接子进程内向 argv[] 填入的冗长路径存这里（execvp 前须保持有效）。 */
typedef struct ShuAsmLdPathBank {
    char slots[XLANG_ASM_LD_PATH_BANK_SLOTS][XLANG_LINK_ABI_PATH_MAX];
    int n;
} ShuAsmLdPathBank;
...
/**
 * 解析 xlang 可执行文件所在 compiler/ 目录（/proc/self/exe 或 argv0 realpath）。
...
/**
 * std/io/io.o 路径；与 xlang 所在目录的 ../std/io/ 对齐。
...
/**
 * crt0_user.o 路径；与 runtime_panic.o 同目录（compiler/），XLANG_FREESTANDING 链入。
 * 参数：argv0 可选 xlang 路径。
 * 返回值：.o 路径或空串。
 */
...
/**
 * freestanding_io.o 路径；compiler/ 下，XLANG_FREESTANDING syscall write 链入。
 * 参数：argv0 可选 xlang 路径。
 * 返回值：.o 路径或空串。
 */
...
/**
 * runtime_asm_io_stubs.o 路径；seed asm 用户程序 std.io 桩，与 xlang 同目录。
 * 参数：argv0 可选 xlang 路径。
 * 返回值：.o 路径或空串。
 */
...
/**
 * XLANG_FREESTANDING=1 或 driver -freestanding：Linux 上 nostdlib 静态链是否启用。
 ...
 * 参数：argv0；driver_freestanding 同 xlang_link_freestanding_enabled。
 * 返回值：0 成功，-1 失败。
 */
...
/** 用户 .o 是否引用 xlang_sys_*（按需链 freestanding_io.o）。 */
int xlang_freestanding_user_o_needs_io(const char *user_o);

/** 用户 .o 是否引用 xlang_panic_（按需链 runtime_panic.o）。 */
...
/**
 * ASM -o exe 薄包装：prepare + xlang_asm_invoke_ld_platform；freestanding 取自 driver_freestanding_get。
 ...
 * Why: `xlang -o exe file.x extra.o` drops extra.o because xlang_invoke_cc takes
 *   a fixed variadic std/core .o list. These setters plumb user .o args from
 *   argv through xlang_invoke_cc_impl to the cc link line.
 * Contract: caller must wrap xlang_invoke_cc with set/clear to avoid stale state.
...
/**
 * XLANG_NET_TLS 非 stub 时尝试 make net-o-openssl / net-o-mbedtls。
...
#endif /* XLANG_RUNTIME_LINK_ABI_H */
