...
/**
 * wave438: emit one scalar ARRAY_LIT elem into flat slot.
 * @param arena *u8 — ASTArena*
...
export function pipeline_asm_emit_array_lit_flat_one_scalar_elf_c(arena: *u8, elf_ctx: *u8, init_ref: i32, ctx: *u8, ta: i32, stack_slot_off: i32, leaf_esz: i32, flat_i: *i32, elem_ref: i32, store_sz: i32): i32 {
  unsafe {
    ...
    if (may_clobber != 0) {
      rc = glue_array_lit_flat_repark_rbx_elf_c(arena, elf_ctx, init_ref, ctx, ta, stack_slot_off, leaf_esz, flat_i);
    }
    if (rc != 0) {
      return 0 - 1;
    }
