// Thin pure: INDEX SIMD dispatcher (wave441/w466).
...
// wave466: split like array dispatcher — setup + type peel + body peer;
//   no `let x = call()` / no `slot[0]=call()`. Tip U=5/5. PRODUCT inject:
//   LINUX PREFER (stamp w466); MACOS skip (g05 mega UNDEF peers).
...
export extern function glue_emit_assign_index_simd_body_elf_c(arena: *u8, elf_ctx: *u8, right_ref: i32, ctx: *u8, ta: i32, base_ref: i32, idx_ref: i32, ltr: i32): i32;

/**
 * INDEX SIMD path dispatcher — setup, peel element type, body home+init.
 * wave466: no-local — setup via `if (call()!=0)`; left/base type via
 *   re-call into body args (no mid `let` binds). Monolithic tip otherwise
 *   drops setup U when lit*nbytes body is co-located.
...
export function glue_emit_assign_index_simd_elf_c(arena: *u8, elf_ctx: *u8, expr_ref: i32, left_ref: i32, right_ref: i32, ctx: *u8, ta: i32): i32 {
  unsafe {
    ...
    if (pipeline_expr_resolved_type_ref(arena, left_ref) > 0) {
      return glue_emit_assign_index_simd_body_elf_c(arena, elf_ctx, right_ref, ctx, ta, base_s[0], idx_s[0], pipeline_expr_resolved_type_ref(arena, left_ref));
    }
    ...
    return glue_emit_assign_index_simd_body_elf_c(arena, elf_ctx, right_ref, ctx, ta, base_s[0], idx_s[0], pipeline_type_elem_ref_at(arena, pipeline_expr_resolved_type_ref(arena, base_s[0])));
  }
}
