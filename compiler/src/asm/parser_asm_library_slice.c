...
/** 与 ast.su Block 布局一致（library 形态仅写 labeled return）。 */
...
/** 与 ast.su ExprKind 序一致（library 形态用到的子集；勿与 primary_slice 枚举混用）。 */
enum {
...
/** 与 parser.su ast.expr_init_match_enum 一致：清零 match 占位字段。 */
...
/**
 * 解析库函数单参 bool return 形态并写入 module/arena；与 parser.su parse_one_function_library 一致。
 */
