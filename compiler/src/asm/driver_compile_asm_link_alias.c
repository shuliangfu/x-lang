/**
 * driver_compile_asm_link_alias.c — strict 链 driver SX 符号别名
 *
 * build_asm/driver_compile.o（EMIT_HEAVY 单编 compile.sx）导出 run_compiler_full_sx /
 * compile_dispatch_* 裸名；runtime.c 与 C-gen driver_compile_sx.o 期望 driver_* 链接名。
...
extern int32_t run_compiler_full_sx(int32_t argc, uint8_t *argv);
extern int32_t run_compiler_full_sx_post_parse(struct driver_DriverCompileState *state, int32_t argc,
                                               uint8_t *argv);
...
int32_t driver_run_compiler_full_sx(int32_t argc, uint8_t *argv) {
  return run_compiler_full_sx(argc, argv);
}

/** post_parse 链接名 → asm 裸名（impl_c 经 thin delegate 回调）。 */
int32_t driver_run_compiler_full_sx_post_parse(struct driver_DriverCompileState *state, int32_t argc,
                                               uint8_t *argv) {
  return run_compiler_full_sx_post_parse(state, argc, argv);
}

/** C-gen / gate 期望的 dispatch 符号 → asm SX 真 emit 裸名。 */
