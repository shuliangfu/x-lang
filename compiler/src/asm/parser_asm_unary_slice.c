 ...
 * 与 parser.su parse_unary_into 一致：await/run/spawn/-,!,& 前缀 + primary 回落。
 */
#ifndef PARSER_ASM_UNARY_SLICE_INCLUDED
#define PARSER_ASM_UNARY_SLICE_INCLUDED

/** 与 ast.su ExprKind 序一致。 */
enum {
...
void parser_asm_parse_unary_into_slice_c(void *arena, struct parser_asm_lexer lex, struct parser_asm_slice_u8 *source,
  ...
  if (r.tok.kind == (int32_t)TOKEN_AWAIT) {
    ...
    if (!out->ok)
      return;
    if (parser_asm_unary_wrap_operand_c(arena, out, PARSER_ASM_EXPR_AWAIT, out->expr_ref) == 0)
      out->ok = 0;
    return;
  }
  if (r.tok.kind == (int32_t)TOKEN_RUN) {
    ...
    if (!out->ok)
      return;
    if (parser_asm_unary_wrap_operand_c(arena, out, PARSER_ASM_EXPR_RUN, out->expr_ref) == 0)
      out->ok = 0;
    return;
  }
  if (r.tok.kind == (int32_t)TOKEN_SPAWN) {
    ...
    if (!out->ok)
      return;
    if (parser_asm_unary_wrap_operand_c(arena, out, PARSER_ASM_EXPR_SPAWN, out->expr_ref) == 0)
      out->ok = 0;
    return;
  }
  if (r.tok.kind == (int32_t)TOKEN_MINUS) {
    ...
    if (!out->ok)
      return;
    if (parser_asm_unary_wrap_operand_c(arena, out, PARSER_ASM_EXPR_NEG, out->expr_ref) == 0)
      out->ok = 0;
    return;
  }
  if (r.tok.kind == (int32_t)TOKEN_BANG) {
    ...
    if (!out->ok)
      return;
    if (parser_asm_unary_wrap_operand_c(arena, out, PARSER_ASM_EXPR_LOGNOT, out->expr_ref) == 0)
      out->ok = 0;
    return;
  }
  if (r.tok.kind == (int32_t)TOKEN_AMP) {
    ...
    if (!out->ok)
      return;
    if (parser_asm_unary_wrap_operand_c(arena, out, PARSER_ASM_EXPR_ADDR_OF, out->expr_ref) == 0)
      out->ok = 0;
    return;
  }
