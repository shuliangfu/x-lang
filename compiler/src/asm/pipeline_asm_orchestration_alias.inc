 ...
 * 本 TU 按 pipeline.x 提供 run_x_pipeline_impl、parse entry 与 parse_into_with_init_buf。
 * codegen/load 仍由 pipeline_strict_link_partial.o（build_asm/pipeline.o 子集）X 真 emit。
 */
#include <stddef.h>
#include <stdint.h>
#include <stdio.h>
...
extern int32_t run_x_pipeline_codegen_deps(struct ast_Module *module, struct ast_ASTArena *arena,
                                            struct codegen_CodegenOutBuf *out_buf, struct ast_PipelineDepCtx *ctx,
                                            int32_t skip_asm_dep_codegen);
extern int32_t run_x_pipeline_codegen_entry(struct ast_Module *module, struct ast_ASTArena *arena,
                                             struct codegen_CodegenOutBuf *out_buf, struct ast_PipelineDepCtx *ctx);
...
/** pipeline.x run_x_pipeline_parse_entry_if_needed；勿链 pipeline.o 内 broken parse stub。 */
int32_t run_x_pipeline_parse_entry_if_needed(struct ast_Module *module, struct ast_ASTArena *arena,
                                              uint8_t *source_data, size_t source_len,
                                              struct ast_PipelineDepCtx *ctx) {
  if (!module || !arena || !ctx)
    return -1;
...
/**
 * 完整 .x 流水线：parse → load/sync deps → typeck → codegen（与 pipeline.x run_x_pipeline_impl 一致）。
 */
int32_t run_x_pipeline_impl(struct ast_Module *module, struct ast_ASTArena *arena, uint8_t *source_data,
  ...
  if (run_x_pipeline_parse_entry_if_needed(module, arena, source_data, source_len, ctx) != 0)
    return -2;
  /** 无 import 的单文件 entry：跳过 load/sync X（build_asm partial 在 ndep=0 时仍可能 SIGSEGV）。 */
  ...
  } else if (driver_x_pipeline_skip_typeck_get() != 0 || driver_x_pipeline_skip_codegen_get() != 0 ||
             driver_asm_build_skip_typeck() != 0) {
    /*
     * asm -o 跳过全量 check_block 时：无 import 单文件仍须全量 typeck（field_access_offset）；
     * 有 import 时 runtime 已 C typeck 预检 / dep parse-only，勿再跑 .x typecheck_entry（SIGSEGV）。
     */
    if (driver_x_pipeline_skip_typeck_get() != 0 && parser_get_module_num_imports(module) == 0 &&
        driver_x_pipeline_skip_codegen_get() != 0) {
      tc_rc = run_x_pipeline_typecheck_entry_emit_c(module, arena, ctx);
  ...
  if (run_x_pipeline_codegen_deps(module, arena, out_buf, ctx, skip_asm_dep_codegen) != 0)
    return -6;
