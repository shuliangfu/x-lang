 ...
 *
 * 勿链 pipeline_x.o 仅导出 run_x_pipeline_impl 的 partial（同 TU 内 local 编排/codegen 绑定 → 大模块 SIGSEGV）。
 * build_asm/pipeline.o 自举完成（__text>512B 且 run_x_pipeline_impl 含 bl 真机码）时链真 run_x_pipeline_impl；
 * 否则 strict_core partial 的 pipeline_impl_run_all（由 pipeline_x.o / strict_core partial 导出；-E-extern 瘦 TU 无 pipeline_pipeline_* 前缀）。
 */
#include <stddef.h>
#include <stdint.h>
...
#else
/** strict_core partial（pipeline_x.o ld -r）导出的完整编排入口。 */
