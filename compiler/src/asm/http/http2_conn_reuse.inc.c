/**
 * std/http/conn_reuse.inc.c — HTTP/2 单连接多请求复用 v1（STD-HTTP-H2-v11）
 *
 * 【文件职责】同一 TCP/TLS 连接上 preface+SETTINGS 一次握手，多 stream 顺序/并发请求往返。
 * 由 network.inc.c 末尾 #include。
 */

/** 连接未完成 handshake（须 conn_handshake）。 */
...
typedef struct {
    ...
    int32_t goaway_seen;
    int32_t is_https;
    multistream_client_t ms;
} conn_t;

/** 从 conn 取统一 IO 视图。 */
static io_t conn_io(const conn_t *conn) {
    io_t io;
...
void http2_conn_init_c(conn_t *conn) {
    if (!conn)
        return;
    conn->fd = -1;
    conn->tls_ctx = 0;
...
int32_t http2_conn_attach_h2c_c(int32_t fd, conn_t *conn) {
    if (!conn || fd < 0)
        return -1;
    conn->fd = fd;
    conn->tls_ctx = 0;
...
int32_t http2_conn_attach_tls_c(int64_t tls_ctx, conn_t *conn) {
    if (!conn || tls_ctx == 0)
        return -1;
    conn->fd = -1;
...
int32_t http2_conn_is_ready_c(const conn_t *conn) {
    if (!conn)
        return 0;
 ...
 */
int32_t http2_conn_handshake_with_enable_push_c(conn_t *conn, int32_t client_enable_push) {
    io_t io;
    uint8_t preface[32];
    ...
    if (conn->tls_ctx == 0) {
        ...
        if (preface_len != HTTP2_PREFACE_LEN)
            return -1;
        if (io_write_all(&io, preface, preface_len) != 0)
    ...
    cs_len = http2_build_client_settings_ex_c(100, HTTP2_DEFAULT_INITIAL_WINDOW, client_enable_push,
                                              csettings, 32);
    if (cs_len > 0) {
        if (io_write_all(&io, csettings, cs_len) != 0)
            return HTTP2_ERR_NETWORK;
    }
 ...
 */
int32_t http2_conn_handshake_c(conn_t *conn) {
 ...
 */
int32_t http2_conn_handshake_with_max_frame_c(conn_t *conn, int32_t client_max_frame_size) {
    io_t io;
    uint8_t preface[32];
 ...
 */
int32_t http2_conn_request_c(conn_t *conn, uint8_t method_u8, const uint8_t *authority,
    ...
    has_body = method_has_body(method_u8);
    if (has_body != 0) {
        if (body_len <= 0 || !body)
            return -1;
    } else {
    ...
    if (hdr_frame_len <= 0)
        return -1;
    if (io_write_all(&io, payload, hdr_frame_len) != 0)
        return HTTP2_ERR_NETWORK;
    ...
    if (has_body != 0) {
        ...
        if (http2_flow_state_consume_send_c(&conn->ms.flow, body_len) != 0)
            return HTTP2_FLOW_ERR_BLOCKED;
        if (io_write_all(&io, payload, data_frame_len) != 0)
            return HTTP2_ERR_NETWORK;
    }

    rc = read_response_stream_io(&io, stream_id, out, out_cap, &conn->ms.peer,
                                       &conn->goaway_seen);
...
int32_t http2_conn_goaway_seen_c(const conn_t *conn) {
    if (!conn)
        return 0;
...
void http2_conn_mark_goaway_seen_c(conn_t *conn) {
    if (!conn)
        return;
    conn->goaway_seen = 1;
    conn->ready = 0;
...
int32_t http2_conn_is_pool_reusable_c(const conn_t *conn) {
    if (!conn)
        return 0;
 ...
 */
int32_t http2_conn_reset_stream_c(conn_t *conn, int32_t stream_id, int32_t error_code) {
    io_t io;
    uint8_t frame[32];
    int32_t n;
...
void http2_conn_close_c(conn_t *conn) {
    if (!conn)
        return;
    conn->ready = 0;
    conn->fd = -1;
...
void http2_conn_shutdown_c(conn_t *conn) {
    if (!conn)
        return;
    if (conn->tls_ctx != 0) {
 ...
 */
void http2_conn_shutdown_graceful_c(conn_t *conn, int32_t last_stream_id, int32_t error_code) {
    io_t io;
    uint8_t frame[32];
    int32_t n;
 ...
 */
int32_t http2_conn_read_goaway_c(conn_t *conn, int32_t *out_last_stream,
...
/** h2c server GOAWAY fork 集成烟测（定义于 server.inc.c）；0 通过。 */
...
/** h2c server PING/PONG fork 集成烟测（定义于 server.inc.c）；0 通过。 */
 ...
 */
int32_t http2_conn_ping_c(conn_t *conn, const uint8_t *opaque) {
    io_t io;
    uint8_t frame[32];
    uint8_t hdr[9];
    uint8_t payload[16];
