...
// and calls try_skip_into.
// 7.2.1 P11d B-minus (2026-09-13): 有则补全 this file with
// collect_imports. The walk records `const name = import("path");`
// onto the opaque module (plus `#[cfg]` prune). Language has no
// local u8[N]; dest 128-byte path/bind scratches are C-stack-owned.
...
// Do not duplicate consume_path (this file, P11c).
...
//
// Hybrid P11b/P11c/P11d: g05_try_x_to_o this file;
...
/** Pipeline sidecar: allocate one import slot on the opaque module; <0 on fail. */
export extern "C" function pipeline_module_import_alloc(module: *u8): i32;
/** Pipeline sidecar: write the import path bytes. */
export extern "C" function pipeline_module_import_set_path(module: *u8, idx: i32, bytes: *u8, len: i32): void;
/** Pipeline sidecar: write kind (1 = const-import binding). */
export extern "C" function pipeline_module_import_set_kind(module: *u8, idx: i32, kind: i32): void;
/** Pipeline sidecar: write the binding IDENT spelling. */
export extern "C" function pipeline_module_import_set_binding_name(module: *u8, idx: i32, bytes: *u8, len: i32): void;
/** Pipeline sidecar: write select-count (collect records 0). */
export extern "C" function pipeline_module_import_set_select_count(module: *u8, idx: i32, n: i32): void;
...
/** Dest-buffer capacity for path_buf / bind_buf (C trampoline owns 128 bytes). */
const IMPORT_BIND_CAP: i32 = 128;
...
/**
 * Collect top-level `const name = import("path");` bindings onto the
 * opaque module and leave the lexer at the first non-import token.
 * Entry cursor is the start of the first token. Peek ATTR_CFG: step
 * and set pending from int_val==0. While pending, P18 cfg-skip
 * consumes the next top-level construct. Peek CONST: lookahead
 * IDENT ASSIGN IMPORT without committing; a non-import CONST
 * fail-leaves AT CONST (unlike skip_imports, which steps CONST
 * then try_skip). On a confirmed binding: copy the IDENT into
 * bind_buf (cap 127; C dest is 128 bytes; C twin's ident_len<=255
 * would overflow bind_buf[128] for 128..255 — product names are
 * short), step through IMPORT, P11c consume_path, then
 * pipeline_module_import_* (kind=1, select_count=0). consume_path
 * fail returns 0 with the lexer already mutated (C twin returned
 * without restore). Language has no local u8[N]; dest path/bind
 * scratches are C-stack-owned. Do not call skip_imports. Do not
 * wrap leftover collect walk AUDIT.
 * @param lex_inout *u8 — opaque lexer (advanced past collected
 *   imports, or left at the first non-import / non-import CONST)
 * @param source *u8 — opaque slice
 * @param module *u8 — opaque Module; null rejected
 * @param path_buf *u8 — dest 128-byte path scratch; trampoline owns it
 * @param bind_buf *u8 — dest 128-byte binding scratch; trampoline owns it
 * @param path_len *i32 — out length slot for consume_path
 * @return i32 — 1 on the success / fail-leave path; 0 on null or
 *   consume_path / alloc fail
 * PLATFORM: SHARED — product P11d B-minus. Do not open a new P-lane.
 */
#[no_mangle]
export function parser_asm_collect_imports_into_c(lex_inout: *u8, source: *u8, module: *u8, path_buf: *u8, bind_buf: *u8, path_len: *i32): i32 {
  let pending: i32 = 0;
  let kind: i32 = 0;
  let n: i32 = 0;
  let pl: i32 = 0;
  ...
  let slen_us: usize = 0;
  let slen: i32 = 0;
  if (lex_inout == 0 as *u8 || source == 0 as *u8 || module == 0 as *u8 || path_buf == 0 as *u8 || bind_buf == 0 as *u8 || path_len == 0 as *i32) {
    return 0;
  }
  unsafe {
    while (true) {
      ...
      // Lookahead IDENT ASSIGN IMPORT; restore to CONST on mismatch.
      pos0 = parser_asm_lex_pos_c(lex_inout);
      line0 = parser_asm_lex_line_c(lex_inout);
      col0 = parser_asm_lex_col_c(lex_inout);
      parser_asm_lex_step_kind_c(lex_inout, source);
      ...
      zi = 0;
      while (zi < IMPORT_BIND_CAP) {
        bind_buf[zi as usize] = 0;
        zi = zi + 1;
      }
      if (pl > 0 && data != 0 as *u8) {
        parser_asm_copy_slice_to_name64_buf_c(data, slen, ts, pl, bind_buf);
      }
      parser_asm_lex_step_kind_c(lex_inout, source);
      ok = parser_asm_imports_expect_kind(lex_inout, source, TOKEN_ASSIGN);
      if (ok == 0) {
      ...
      imp_ix = pipeline_module_import_alloc(module);
      if (imp_ix < 0) {
        return 0;
      }
      pipeline_module_import_set_path(module, imp_ix, path_buf, path_len[0]);
      pipeline_module_import_set_kind(module, imp_ix, 1);
      pipeline_module_import_set_binding_name(module, imp_ix, bind_buf, pl);
      pipeline_module_import_set_select_count(module, imp_ix, 0);
    }
  }
  return 1;
}
