/**
 * pipeline_asm_run_all_alias.c — strict 链：pipeline_impl_run_all / run_su_pipeline_impl 的 C 实现。
 *
 * build_asm/pipeline.o 第二遍 emit 的 run_all 在 typecheck 失败后 cbz 分支错误，仍会进入 codegen，
 * 导致 return-value smoke 等 case CPU 100% 挂死；本 TU 按 pipeline.su 语义在 typeck 失败时立即返回。
 */
#include <stddef.h>
#include <stdint.h>
...
/** 完整流水线编排；与 pipeline.su pipeline_impl_run_all 一致。 */
