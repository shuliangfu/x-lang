...
export function asm_pool_live_add(lay: *u8, name: *u8, nlen: i32, sz: i32): void {
  ...
  unsafe {
    ...
    let i: i32 = 0;
    while (i < num) {
      // Cap 4.2.8: LiveVar stride 268 (name[256] + 3×i32).
      let base: i32 = 12 + i * 268;
      let elen: i32 = asm_pool_load_i32_le(lay, base + 256);
      if (elen == nlen) {
        let eq: i32 = 1;
        let j: i32 = 0;
        while (j < nlen) {
    ...
    i = 0;
    while (i < num) {
      let base2: i32 = 12 + i * 268;
      off = off + asm_pool_load_i32_le(lay, base2 + 260);
      i = i + 1;
    }
    let slot: i32 = 12 + num * 268;
    let k: i32 = 0;
    while (k < nlen) {
      lay[slot + k] = name[k];
      k = k + 1;
    ...
    lay[slot + nlen] = 0;
    asm_pool_store_i32_le(lay, slot + 256, nlen);
    let sz2: i32 = sz;
    if (sz2 <= 0) { sz2 = 4; }
    asm_pool_store_i32_le(lay, slot + 260, sz2);
    asm_pool_store_i32_le(lay, slot + 264, off);
 ...
 * @param out layout buffer (must be at least 17168 bytes)
...
export function async_asm_pool_build_layout(arena: *u8, mod: *u8, func_index: i32, out: *u8): i32 {
  ...
  unsafe {
    ...
    asm_pool_store_i32_le(out, 17164, 0 - 1);
    let br: i32 = pipeline_module_func_body_ref_at(mod, func_index);
    let nso: i32 = ast_ast_block_num_stmt_order(arena, br);
          ...
          if (idx < nlets) {
            ...
            if (init > 0) {
              if (asm_pool_expr_has_await(arena, init) != 0) {
                ...
                let cur_await: i32 = asm_pool_load_i32_le(out, 17164);
                if (cur_await < 0) {
                  asm_pool_store_i32_le(out, 17164, si);
                }
