 ...
 */
#[no_mangle]
export function glue_emit_as_i2f64_u64_elf_c(arena: *u8, elf_ctx: *u8, op: i32, ctx: *u8, ta: i32): i32 {
  unsafe {
    if (pipeline_expr_resolved_type_ref(arena, op) <= 0) { return 0 - 2; }
    if (!(pipeline_type_kind_ord_at(arena, pipeline_expr_resolved_type_ref(arena, op)) == 4 || pipeline_type_kind_ord_at(arena, pipeline_expr_resolved_type_ref(arena, op)) == 6)) { return 0 - 2; }
