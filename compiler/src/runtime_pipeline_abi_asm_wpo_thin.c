/*
 * Thin pure: wave274 asm WPO v0 DCE + PGO-Lite Cap domain leave.
 * G.7: bodies match mega runtime_pipeline_abi.x wave274 leave /
 ...
 *   g_asm_wpo / g_asm_wpo_pgo_{hot,depth,emit_*}.
 * Faces: pipeline_asm_wpo_reach_* / should_emit_func / pgo_emit_order_* /
 *   pgo_is_hot_func.
 * ensure injects via weaken leftover T then first-wins ld -r.
 * PLATFORM: SHARED freestanding WPO Cap leave.
 */
...
/* Cap 4.2.8 LP64 layouts (match pipeline_gen / w151_ExprMirror + as_operand).
 * Thin-local complete types so field peels compile; live slots come from
 * pipeline_arena_*_ptr. PLATFORM: SHARED. */
struct ast_Module;
struct ast_ASTArena;
...
/* Cap residual callees (product host / pure elsewhere). */
extern void *pipeline_module_func_ptr(struct ast_Module *m, int32_t fi);
...
extern int32_t pipeline_module_func_name_equal_at(struct ast_Module *m, int32_t fi, const uint8_t *name, int32_t name_len);
...
extern int32_t pipeline_expr_struct_lit_init_ref(struct ast_ASTArena *a, int32_t expr_ref, int32_t i);
extern int32_t pipeline_expr_array_lit_elem_ref(struct ast_ASTArena *a, int32_t expr_ref, int32_t i);
extern int32_t pipeline_typeck_pick_overload_func_index_for_call_c(struct ast_Module *m, struct ast_ASTArena *a, int32_t call_ref);
...
extern int32_t pipeline_block_const_init_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_let_init_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_expr_stmt_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_while_cond_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_while_body_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_for_init_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_for_cond_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_for_step_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_for_body_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_if_cond_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_if_then_body_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_if_else_body_ref(struct ast_ASTArena *a, int32_t br, int32_t i);
extern int32_t pipeline_block_region_body_ref(struct ast_ASTArena *a, int32_t br, int32_t i);


/** Product-face aliases (G.7: match .x leave; seed cold used Cap residual names). */
static struct ast_Func *wpo_module_func_at(struct ast_Module *m, int32_t fi) {
  return (struct ast_Func *)pipeline_module_func_ptr(m, fi);
}

/*
 * Strong shims for Cap residual names still referenced by weakened leftover
 * WPO bodies after first-wins inject (Darwin ld -r keeps U from weak dead).
 * Live thin paths use wpo_module_func_at / pipeline_expr_*_arg_ref.
 * PLATFORM: SHARED single-threaded compile; arg_slot tmp is not reentrant.
 */
struct ast_Func *module_func_at(struct ast_Module *m, int32_t fi) {
  return wpo_module_func_at(m, fi);
}

static int32_t g_wpo_arg_slot_tmp;

int32_t *expr_call_arg_slot(struct ast_ASTArena *a, int32_t expr_ref, int32_t i, int32_t create) {
  (void)create;
  g_wpo_arg_slot_tmp = pipeline_expr_call_arg_ref(a, expr_ref, i);
  return &g_wpo_arg_slot_tmp;
}

int32_t *expr_method_call_arg_slot(struct ast_ASTArena *a, int32_t expr_ref, int32_t i, int32_t create) {
  (void)create;
  g_wpo_arg_slot_tmp = pipeline_expr_method_call_arg_ref(a, expr_ref, i);
  return &g_wpo_arg_slot_tmp;
}
/* ExprKind enum tokens used by residual body (product pool). */
#ifndef ast_ExprKind_EXPR_CALL
enum {
  ast_ExprKind_EXPR_NEG = 22,
  ast_ExprKind_EXPR_BITNOT = 23,
  ast_ExprKind_EXPR_LOGNOT = 24,
  ast_ExprKind_EXPR_IF = 25,
  ast_ExprKind_EXPR_BLOCK = 26,
  ast_ExprKind_EXPR_TERNARY = 27,
  ast_ExprKind_EXPR_RETURN = 41,
  ast_ExprKind_EXPR_PANIC = 42,
  ast_ExprKind_EXPR_FIELD_ACCESS = 44,
  ast_ExprKind_EXPR_STRUCT_LIT = 45,
  ast_ExprKind_EXPR_ARRAY_LIT = 46,
  ast_ExprKind_EXPR_INDEX = 47,
  ast_ExprKind_EXPR_CALL = 48,
  ast_ExprKind_EXPR_METHOD_CALL = 49,
  ast_ExprKind_EXPR_ADDR_OF = 51,
  ast_ExprKind_EXPR_DEREF = 52,
  ast_ExprKind_EXPR_AS = 54,
  ast_ExprKind_EXPR_AWAIT = 55,
  ast_ExprKind_EXPR_RUN = 56,
  ast_ExprKind_EXPR_SPAWN = 57
};
#endif

/* wave274 cold twin body (from pipeline_asm_wpo.c residual) */
/* ============================================================================
...
void pipeline_asm_wpo_reach_clear(void) {
  ...
  /* wave274: root_id = -1 after clear so named-root <0 gates run (pure twin). */
  g_asm_wpo.root_id = -1;
}
...
static void asm_wpo_collect_edges_from_expr(struct ast_ASTArena *a, int32_t expr_ref, int32_t caller_id,
  ...
  if (ex->kind == ast_ExprKind_EXPR_CALL) {
    ...
    for (i = 0; i < ex->call_num_args; i++) {
      { int32_t _ar = pipeline_expr_call_arg_ref(a, expr_ref, i);
      if (_ar > 0)
        asm_wpo_collect_edges_from_expr(a, _ar, caller_id, caller_mod, ctx, depth + 1);
      }
    }
  ...
  if (ex->kind == ast_ExprKind_EXPR_METHOD_CALL ||
    ...
    for (i = 0; i < mnargs; i++) {
      { int32_t _ar = pipeline_expr_method_call_arg_ref(a, expr_ref, i);
      if (_ar > 0)
        asm_wpo_collect_edges_from_expr(a, _ar, caller_id, caller_mod, ctx, depth + 1);
      }
    }
...
static void asm_wpo_scan_func_body_calls(struct ast_ASTArena *a, struct ast_Module *mod, int32_t func_fi,
  ...
  if (!a || !mod || caller_id < 0 || func_fi < 0)
    return;
  f = wpo_module_func_at(mod, func_fi);
  if (!f)
    return;
  if (f->body_ref > 0)
...
static int32_t asm_wpo_user_pgo_force_main_callee_edge(struct ast_Module *entry, struct ast_ASTArena *a) {
  ...
  f = wpo_module_func_at(entry, main_fi);
  if (!f || f->body_ref <= 0)
    return -1;
