...
| 形式 | 行为 |
|------|------|
| `shux build` | 无源文件参数时：读取当前目录 **`build.x`**，编译并运行 **`build_runner`**（项目级构建脚本）。 |
| `shux build file.x` | 仅编译；默认输出 **`a.out`**。 |
| `shux build file.x -o exe` | 仅编译到指定可执行文件。 |
| `shux run file.x` | 编译并运行给定源文件。 |
