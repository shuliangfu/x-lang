 ...
 * return 4 (f32, widened) and return 5 (f64) start at 1 and clear
 * when host `bv > av` (no bare `av >= bv`), matching ucomis/setae
 * for ordered values. Keep 1 when equal. 2.0 >= 1.0 stores 1 and
 * 1.0 >= 2.0 stores 0. 1.0 >= 1.0 stores 1.
...
export function pipe_modlet_array_lit_elem_const_val(arena: *u8, eref: i32, out_val: *i32, out_hi: *i32): i32 {
  ...
  // Float GE: return 4/5 widen to host f64; start at 1 and clear when
  // `bv > av` (no bare `av >= bv`). Keep 1 when equal.
  // 2.0 >= 1.0 stores 1.
  // PLATFORM: MACOS|DARWIN / WINDOWS.
  if (ek == 19) {
    unsafe {
    ...
    ok = pipe_modlet_array_lit_elem_const_val(arena, left, out_val, out_hi);
    // Float greater-or-equal. Widen f32 through glue (same as float GT).
    // Start at 1 and clear on `bv > av` — same Win-safe shape as the
    // integer GE arm. Keep 1 when equal. Do not use a bare `av >= bv`.
    // PLATFORM: MACOS|DARWIN / WINDOWS.
    if (ok == 4 || ok == 5) {
      got = ok;
      if (ok == 4) {
      ...
      // Start-1 then clear on `bv > av` — same Win-safe shape as
      // integer GE. Equal keeps 1. PLATFORM: WINDOWS.
      result = 1;
      if (bv > av) {
        result = 0;
      }
  ...
  if (ek >= 4 && ek <= 13) {
    ...
    if (enter_walk == 0) {
      // Integer operators — scratch arrays live in the iops helper.
      // PLATFORM: WINDOWS — keep recursive compare/AS frames small.
      return pipe_modlet_array_lit_elem_const_iops(arena, eref, ek, out_val, out_hi);
    }
  }
...
/**
 * Integer binop folder for array-literal elements (ek 4..13) when
 * the node is not an f32/f64 ADD/SUB/MUL/DIV walk. Owns stk_lo/stk_hi
 * scratch so the recursive compare/AS folder frame stays smaller on
 * Windows global array bake.
 * @param arena *u8 - AST arena
 * @param eref i32 - binop expr ref
 * @param ek i32 - expr kind 4..13
 * @param out_val *i32 - low half
 * @param out_hi *i32 - high half or null
 * @return i32 - folder success codes (1/2/3) or 0
 * PLATFORM: MACOS|DARWIN / WINDOWS.
 */
function pipe_modlet_array_lit_elem_const_iops(
  arena: *u8, eref: i32, ek: i32, out_val: *i32, out_hi: *i32
): i32 {
  let left: i32 = 0;
  let right: i32 = 0;
  let lv: i32 = 0;
