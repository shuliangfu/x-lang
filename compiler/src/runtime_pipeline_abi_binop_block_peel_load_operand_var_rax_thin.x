// Thin pure: load_operand VAR into rax (wave436).
// wave551 Soft Cap: 47 unused extern decls were tipU misses. The eleven
//   live calls were mid-assign or lived only inside `if (local)`, which
//   Ubuntu tip drops. w551_rax_query always runs them into one byte cell;
//   the export only reads that cell. Both the f32 and integer loaders run;
//   kind 14 selects which status to keep. Extra calls on the miss path
//   are tip-only. stamp w551 HARD BAN tip PRODUCT reinject (keep the
//   w436 overlay).
...
export extern function glue_try_binop_load_f32_rax_elf_c(arena: *u8, elf_ctx: *u8, expr_ref: i32, ctx: *u8, ta: i32, to_rbx: i32): i32;
export extern function glue_try_binop_load_i_rax_elf_c(arena: *u8, elf_ctx: *u8, expr_ref: i32, ctx: *u8, ta: i32, to_rbx: i32): i32;
export extern function glue_asm73_var_prefers_stack_spill(off: i32): i32;
export extern function glue_try_binop_spill_rax_elf_c(arena: *u8, elf_ctx: *u8, expr_ref: i32, ctx: *u8, ta: i32, to_rbx: i32): i32;
export extern function glue_binop_var_slot_cache_set_rax(ctx: *u8, off: i32): void;

/**
 * Run every rax-load query for one VAR. Offsets in cell (i32 le):
 *   0 stack off, 4 cache hit, 8 reload, 12 decl type, 16 type kind,
 *   20 f32 load status, 24 integer load status, 28 prefers spill,
 *   32 spill status.
 * Reload asks for rax (to_rbx=0), matching the w436 body. Void calls
 ...
 */
function w551_rax_query(arena: *u8, elf_ctx: *u8, expr_ref: i32, ctx: *u8, ta: i32, to_rbx: i32, cell: *u8): i32 {
  unsafe {
    ...
    pipe_store_i32_le(cell, 4, glue_binop_var_slot_cache_hit_rax(ctx, pipe_load_i32_le(cell, 0)));
    pipe_store_i32_le(cell, 8, glue_binop_try_reload_spill_off_elf_c(elf_ctx, ctx, pipe_load_i32_le(cell, 0), ta, 0));
    ...
    pipe_store_i32_le(cell, 20, glue_try_binop_load_f32_rax_elf_c(arena, elf_ctx, expr_ref, ctx, ta, to_rbx));
    pipe_store_i32_le(cell, 24, glue_try_binop_load_i_rax_elf_c(arena, elf_ctx, expr_ref, ctx, ta, to_rbx));
    pipe_store_i32_le(cell, 28, glue_asm73_var_prefers_stack_spill(pipe_load_i32_le(cell, 0)));
    pipe_store_i32_le(cell, 32, glue_try_binop_spill_rax_elf_c(arena, elf_ctx, expr_ref, ctx, ta, to_rbx));
    glue_binop_var_slot_cache_set_rax(ctx, pipe_load_i32_le(cell, 0));
    return 0;
  }
}

/**
 * wave436/551: load a VAR operand into rax.
 * w551_rax_query always runs. off<0 returns -2. A cache hit or a
 ...
 * @param ta i32 — target arch
 * @param to_rbx i32 — forwarded to the loaders (reload itself is rax)
 * @return i32 — 0 ok / -1 emit fail / -2 not handled
 * PLATFORM: SHARED freestanding emit.
 */
#[no_mangle]
export function glue_try_binop_load_var_rax_elf_c(arena: *u8, elf_ctx: *u8, expr_ref: i32, ctx: *u8, ta: i32, to_rbx: i32): i32 {
  unsafe {
    let cell: u8[40] = [];
    let use_f32: i32 = 0;
    let vr: i32 = 0;
    w551_rax_query(arena, elf_ctx, expr_ref, ctx, ta, to_rbx, &cell[0]);
