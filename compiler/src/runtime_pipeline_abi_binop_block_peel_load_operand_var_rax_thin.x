// Thin pure: load_operand VAR→rax orchestrator (wave436 flat).
...
export function glue_try_binop_load_var_rax_elf_c(arena: *u8, elf_ctx: *u8, expr_ref: i32, ctx: *u8, ta: i32, to_rbx: i32): i32 {
  unsafe {
    ...
    vr = glue_binop_var_slot_cache_hit_rax(ctx, off);
    if (vr != 0) {
      return 0;
    }
    ...
    if (tk == 14) {
      vr = glue_try_binop_load_f32_rax_elf_c(arena, elf_ctx, expr_ref, ctx, ta, to_rbx);
    }
    if (tk != 14) {
      vr = glue_try_binop_load_i_rax_elf_c(arena, elf_ctx, expr_ref, ctx, ta, to_rbx);
    }
    if (vr != 0) {
      return vr;
    }
    if (glue_asm73_var_prefers_stack_spill(off) != 0) {
      return glue_try_binop_spill_rax_elf_c(arena, elf_ctx, expr_ref, ctx, ta, to_rbx);
    }
