// Thin pure: binop stack-spill try_reload rax arm (wave478).
...
/**
 * Spill reload into rax/x0 — enc LDR then stamp VAR cache.
 ...
 */
#[no_mangle]
export function glue_binop_stack_spill_try_reload_rax_elf_c(elf_ctx: *u8, off: i32): i32 {
  unsafe {
    ...
    if (arch_arm64_enc_enc_ldr_sp_slot_to_xreg(elf_ctx, glue_index_scratch_stack_depth_get() - glue_binop_stack_spill_find_depth(off), 0) != 0) {
      return 0 - 1;
    }
