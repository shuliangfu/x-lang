// Thin pure: wave317 M2 — emit_ctx_bss Cap residual C→.x (was wave220–221 C thin).
// Arm-depth + emit_ctx scalar/pointer BSS + host_is_arm64; 15 exports.
// G.7: bodies match runtime_pipeline_abi.x wave220/221 leave.
// PRODUCT inject: -E+$CC via pipeline_abi_inject_emit_ctx_bss_thin
// (ALLOW_E_REPLACE + stamp). Named scalar BSS OK under -E+$CC (file-local
// static cells; strong get/set faces first-wins). Pure-asm still banned
// (Lxml COMMON dual-home vs mega named .data — f32 SEGV).
