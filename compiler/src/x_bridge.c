/**
 * x_bridge.c — 将 runtime.c 调用的 C 原版符号桥接到 .x 源码生成的实现
 *
 * 策略：
 *   parser.x → -E-extern → parser_gen.c → cc → parser_x.o
 *   typeck.x → -E-extern → typeck_gen.c → cc → typeck_x.o
 *   codegen.x → -E-extern → codegen_gen.c → cc → codegen_x.o
 *
...
/* .x codegen 导出 */
extern int32_t codegen_codegen_x_ast(
