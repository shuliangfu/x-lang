// Thin pure: wave328 M2 — asm_codegen_mega_body Cap residual C→.x (was wave290 C thin).
...
// PRODUCT inject: -E+$CC via pipeline_abi_inject_asm_codegen_mega_body_thin
// (ALLOW_E_REPLACE + stamp). POSIX product path only (WIN leftover ARRAY_LIT
// return stays leftover-PE / WIN macro authority — not this leaf).
  ...
  while (k < emit_n) {
    ...
    if (ta == 0 || ta == 1) {
      ...
      if (fn_ret_sz > 16) {
        ...
        pipeline_asm_emit_ctx_sret_home_off_set(w328_load(bctx, W328_CTX_NEXT_OFFSET) + 256);
    ...
    if (body_ref != 0) {
      ...
      if (pipeline_asm_block_num_stmt_order_at(a, body_ref) > 0) {
        ...
        slot_base = w328_load(bctx, W328_CTX_NUM_LOCALS) - ast_ast_block_num_consts(a, body_ref) - ast_ast_block_num_lets(a, body_ref);
