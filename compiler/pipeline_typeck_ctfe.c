/**
 * pipeline_typeck_ctfe.c — typeck CTFE producer slice (wave965 · BC 8.3.1).
 *
 * Mechanically extracted from pipeline_glue.c (same translation unit via
 * #include). Authority for LANG-006 const fold: writes Expr.const_folded_valid
 * / const_folded_val; emit only *consumes* those fields (mov imm).
 *
 * G.7: single CTFE producer path. twin notes in
 * seeds/pipeline_glue_strict_minimal.from_x.c still mirror whitelist semantics.
 * Do not open a second fold path in emit or typeck.x.
 *
 * Not compiled as a separate .o — #included from pipeline_glue.c after
 * glue_arena_expr_at_ref (static helper shared with later struct_lit accessors).
 *
 * PLATFORM: SHARED — product residual C; host-cc via pipeline_x.o TU.
 * Wave: 965 first 8.3.1 domain thin cut (inventory still present until full .x).
 */
