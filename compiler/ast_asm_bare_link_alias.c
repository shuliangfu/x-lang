/**
 * ast_asm_bare_link_alias.c — ast.su 裸符号 → pipeline_glue 的 ast_ast_* 前缀实现
 *
...
/** ast.su ast_block_final_expr_ref → glue ast_ast_block_final_expr_ref。 */
int32_t ast_block_final_expr_ref(struct ast_ASTArena *a, int32_t br) {
  return ast_ast_block_final_expr_ref(a, br);
}

/** ast.su ast_block_region_body_ref → glue ast_ast_block_region_body_ref。 */
...
/** ast.su ast_block_if_else_body_ref → glue ast_ast_block_if_else_body_ref。 */
...
/** ast.su ast_block_resolve_var_to_type_ref → glue ast_ast_block_resolve_var_to_type_ref。 */
