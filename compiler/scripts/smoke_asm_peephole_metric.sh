#!/bin/sh
# smoke_asm_peephole_metric.sh — 对 peephole.su 的 -backend asm __text 段做一次性字节度量（可作回归基线）。
# 用法：在 compiler 目录执行 SHU=./shu ./scripts/smoke_asm_peephole_metric.sh
...
OUT="$(mktemp -u /tmp/shu_peephole_XXXXXX).o"
trap 'rm -f "$OUT"' EXIT
eval "$SHU -backend asm -o \"$OUT\" $LIBROOT \"src/asm/peephole.su\""
