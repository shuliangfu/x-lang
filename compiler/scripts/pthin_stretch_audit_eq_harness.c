 ...
 * same sibling-TU reason (first-kind ABI cannot be a k_cases row). */
extern int leftover_preamble_eq_run(long *checks, int *fail);
/* v5.60: leftover_helpers simd_builtin_deep_from_at lex_after_ident eq —
 * same sibling-TU reason (twins.h static from_at; first-at_kind ABI
 * cannot be a k_cases row). */
extern int leftover_fromat_eq_run(long *checks, int *fail);
 ...
 * for hyper_mega / ultra_hyper / max_ultra / apex_max — see eq_tok_hits_name).
...
int main(int argc, char **argv) {
   ...
   * / v5.59 leftover_preamble / v5.60 leftover_fromat: sibling TUs so
   * twins.h static C copies do not shadow the .x T from audit_x.o
   * (import_path_post / preamble have no twins.h static, but
   * first-path_buf / first-kind ABI still cannot be a k_cases row;
   * from_at has a twins.h static). Shard 0 only so parallel workers
   * do not double-count. */
  if (g_shard_i == 0) {
    ...
    leftover_preamble_eq_run(&g_checks, &g_fail);
    leftover_fromat_eq_run(&g_checks, &g_fail);
  }
