...
# Darwin arm64 cold body for runtime_random_fill.o.
# The whole TU is src/asm/runtime_random_fill.x. Public names stay strong.
# A missing object after a pure-asm fault falls back to the C seed.
# An object already on disk is left as-is. No gcc -E.
# Linux and Windows do not call this; they host-cc the C seed.
# PLATFORM: MACOS|DARWIN arm64.
ensure_random_fill_darwin_pure() {
  local o="runtime_random_fill.o"
  local xsrc="src/asm/runtime_random_fill.x"
  local seed="seeds/runtime_random_fill.from_x.c"
  if [ ! -f "$xsrc" ]; then
    echo "ensure_host_cc_seed_o random-fill: missing $xsrc" >&2
    return 1
  fi
  if [ "$FORCE" != "1" ] && [ -f "$o" ] && [ ! "$xsrc" -nt "$o" ]; then
    log "skip $o (up-to-date vs $xsrc)"
  ...
  if ! (
    ...
    echo "ensure_host_cc_seed_o random-fill: pure-asm failed for $xsrc" >&2
    if [ ! -s "$o" ]; then
      log "cc -c $seed → $o (pure-asm missing object)"
      # shellcheck disable=SC2086
      $CC ${CFLAGS:-} -I. -Iinclude -Isrc -c "$seed" -o "$o" || return 1
...
# No gcc -E. Linux and Windows do not call this; they host-cc the C seed.
# PLATFORM: MACOS|DARWIN arm64.
# $1 = output object (default runtime_sync_os.o, cwd is compiler/).
ensure_sync_os_darwin_pure() {
  local o="${1:-runtime_sync_os.o}"
  local xsrc="src/asm/runtime_sync_os_darwin.x"
  local seed="seeds/runtime_sync_os.from_x.c"
  local try=0
  if [ ! -f "$xsrc" ]; then
    echo "ensure_host_cc_seed_o sync-os: missing $xsrc" >&2
    return 1
  fi
  if [ "$FORCE" != "1" ] && [ -f "$o" ] && [ ! "$xsrc" -nt "$o" ]; then
    log "skip $o (up-to-date vs $xsrc)"
  ...
  while [ "$try" -lt 3 ]; do
    ...
    echo "ensure_host_cc_seed_o sync-os: pure-asm try $try failed" >&2
    rm -f "$o"
  done
  if [ ! -s "$o" ]; then
    log "cc -c $seed → $o (pure-asm missing object)"
...
# Darwin arm64 cold body for runtime_env_os.o.
# Two translation units, then ld -r: the shared wrappers in
# src/asm/runtime_env_os.x and the bridges in
# src/asm/runtime_env_os_darwin.x. Reads call link_abi_getenv.
# setenv does not call libc setenv. Each half can SIGSEGV on its
...
# PLATFORM: MACOS|DARWIN arm64.
# $1 = output object (default runtime_env_os.o, cwd is compiler/).
ensure_env_os_darwin_pure() {
  local o="${1:-runtime_env_os.o}"
  local x_thin="src/asm/runtime_env_os.x"
  local x_os="src/asm/runtime_env_os_darwin.x"
  local seed="seeds/runtime_env_os.from_x.c"
  local try=0
  local thin_o os_o
  if [ ! -f "$x_thin" ] || [ ! -f "$x_os" ]; then
    echo "ensure_host_cc_seed_o env-os: missing $x_thin or $x_os" >&2
    return 1
  fi
  if [ "$FORCE" != "1" ] && [ -f "$o" ] \
    && [ ! "$x_thin" -nt "$o" ] && [ ! "$x_os" -nt "$o" ]; then
  ...
  while [ "$try" -lt 8 ]; do
    ...
    if [ ! -s "$thin_o" ]; then
      ...
      if (
        ...
        echo "ensure_host_cc_seed_o env-os: thin pure-asm try $try failed" >&2
        rm -f "$thin_o"
      fi
    fi
    if [ -s "$thin_o" ] && [ ! -s "$os_o" ]; then
      ...
      if (
        ...
        echo "ensure_host_cc_seed_o env-os: os pure-asm try $try failed" >&2
        rm -f "$os_o"
      fi
    fi
    if [ -s "$thin_o" ] && [ -s "$os_o" ] \
      && /usr/bin/ld -r -o "$o" "$thin_o" "$os_o"; then
      local _eo_short _eo_ok
      _eo_ok=1
      for _eo_short in \
        _env_getenv_c_impl \
        _env_getenv_ptr_c_impl \
        _env_getenv_z_c_impl \
        _env_getenv_exists_c_impl \
        _env_setenv_c_impl \
        _env_unsetenv_c_impl \
        _env_temp_dir_c_impl \
        _env_iter_count_c_impl \
        _env_iter_at_c_impl \
        _env_build_key \
        _env_getenv_c \
        _env_setenv_c; do
        if ! nm "$o" 2>/dev/null | awk -v s="$_eo_short" '$2=="T" && $3==s { n++ } END { exit (n==1)?0:1 }'; then
          _eo_ok=0
          break
        fi
      done
      if [ "$_eo_ok" != "1" ]; then
        echo "ensure_host_cc_seed_o env-os: required text symbols missing" >&2
        rm -f "$o"
        continue
      fi
      rm -f "$thin_o" "$os_o"
...
ensure_one() {
  ...
  # w1077: Darwin arm64 user-domain CSPRNG fill is the .x, not this seed.
  # Linux and Windows keep host-cc of the C seed (syscall / BCrypt differ).
  # PLATFORM: MACOS|DARWIN arm64.
  if [ "$out" = "runtime_random_fill.o" ]; then
    local rf_s rf_m
    rf_s="$(uname -s 2>/dev/null || echo Unknown)"
    rf_m="$(uname -m 2>/dev/null || echo unknown)"
  ...
  # w1082: Darwin arm64 mutex, rwlock, and condvar are the .x, not this seed.
  # Linux keeps the futex seed. Windows keeps the Win32 seed.
  # PLATFORM: MACOS|DARWIN arm64.
  if [ "$(basename "$seed")" = "runtime_sync_os.from_x.c" ] \
    || [ "$(basename "$out")" = "runtime_sync_os.o" ]; then
    local so_s so_m
    so_s="$(uname -s 2>/dev/null || echo Unknown)"
    so_m="$(uname -m 2>/dev/null || echo unknown)"
  ...
  # w1087: Darwin arm64 environ bridges are the .x, not this seed.
  # Linux keeps the POSIX seed. Windows keeps the Win32 seed.
  # PLATFORM: MACOS|DARWIN arm64.
  if [ "$(basename "$seed")" = "runtime_env_os.from_x.c" ] \
    || [ "$(basename "$out")" = "runtime_env_os.o" ]; then
    local eo_s eo_m
    eo_s="$(uname -s 2>/dev/null || echo Unknown)"
    eo_m="$(uname -m 2>/dev/null || echo unknown)"
...
#   pipeline_expr_binop_left_ref_at mangled face, the w871
...
# into the .x; it stays weak. The unsuffixed body stays in the pipeline object.
# w872 moved pipeline_expr_field_access_name_into_u8_ptr_i32_u8_ptr
# into the .x; it stays weak. The unsuffixed body returns void and stays
# in the pipeline object. This face is void with three arguments.
# w873 moved pipeline_dep_ctx_ndep_u8_ptr_reti32 into the .x; it stays
# weak. The unsuffixed body stays in the pipeline object. This face is
# i32 with one argument, not the void three-argument shape and not the
# i32 two-argument shape.
# w874 moved pipeline_dep_ctx_module_at_u8_ptr_i32_retu8_ptr into the .x;
# it stays weak. The unsuffixed body stays in the pipeline object. This
# face returns *u8 and takes two arguments. It is not the i32 one-argument
# shape, not the void three-argument shape, and not the i32 two-argument
# shape.
# w875 moved pipeline_asm_emit_dep_pipe_c_retu8_ptr into the .x; it stays
...
# arch_riscv64_enc_enc_jalr_reg (thin, w881, stays strong), and
# arch_riscv64_enc_enc_ldr_xreg_xreg_imm (thin, w882, stays strong), and
# arch_x86_64_enc_enc_cdqe_rax_impl (thin, w883, stays strong), and
# backend_enc_append_u8_c_impl (thin, w884, stays strong), and
# backend_enc_append_u32_le_c_impl (thin, w885, stays strong).
# Failure leaves the previous .o in place and returns 1.
# PLATFORM: SHARED.
ensure_enc_dispatch_pure() {
  ...
  if ! ld $ld_flags -o "$merged_o" "$thin_o" "$rest_o" \
    ...
    || ! r3_prefer_nm_has_sym "$merged_o" "arch_riscv64_enc_enc_jalr_reg" \
    || ! r3_prefer_nm_has_sym "$merged_o" "arch_riscv64_enc_enc_ldr_xreg_xreg_imm" \
    || ! r3_prefer_nm_has_sym "$merged_o" "arch_x86_64_enc_enc_cdqe_rax_impl" \
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_append_u8_c_impl" \
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_append_u32_le_c_impl" \
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_arm64_blr_c" \
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_riscv64_jalr_reg_c" \
    ...
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_subsd_rbx_rax_arch" \
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_subsd_rax_rbx_arch" \
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_mulsd_rax_rbx_arch" \
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_divsd_rax_rbx_arch" \
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_ucomisd_rbx_rax_arch" \
  ...
  # w909: four ARM64 immediate encoders live in the thin and stay strong.
  # PLATFORM: SHARED.
  local w909_sym
  for w909_sym in \
  ...
  # w913: ARM64 frame load and store encoders live in the thin.
  # PLATFORM: SHARED.
  local w913_sym
  for w913_sym in \
  ...
  # w914: ARM64 add-immediate encoders live in the thin.
  # PLATFORM: SHARED.
  local w914_sym
  for w914_sym in \
  ...
  # w915: three x86 append helpers. The last name has no continuation backslash.
  local w915_sym
  for w915_sym in \
    x86_enc_u8 \
    x86_enc_u32_le \
  ...
  # w916: x86 prologue and epilogue. The last name has no continuation backslash.
  local w916_sym
  for w916_sym in \
  ...
  # w918: six ARM64 branches. The last name has no continuation backslash.
  local w918_sym
  for w918_sym in \
  ...
  # w919: ARM64 cmp_setcc. The last name has no continuation backslash.
  local w919_sym
  for w919_sym in \
  ...
  # w922: mov_imm32_to_w0 ta forward. The last name has no continuation backslash.
  local w922_sym
  for w922_sym in \
  ...
  # w923: x86 jcc patch and unconditional jmp. The last name has no continuation backslash.
  local w923_sym
  for w923_sym in \
    x86_enc_jcc_rel32 \
  ...
  # w924: ARM64 branch patch. The last name has no continuation backslash.
  local w924_sym
  for w924_sym in \
  ...
  # w925: two ARM64 rbp leas. The last name has no continuation backslash.
  local w925_sym
  for w925_sym in \
  ...
  # w926: ARM64 label. The last name has no continuation backslash.
  local w926_sym
  for w926_sym in \
  ...
  # w927: x86 label. The last name has no continuation backslash.
  local w927_sym
  for w927_sym in \
...
ensure_runtime_os_prefer_one() {
  ...
  # w1077: Darwin arm64 whole TU is the .x. Do not thin+rest host-cc it.
  # PLATFORM: MACOS|DARWIN arm64. Linux and Windows fall through.
  if [ "$o" = "runtime_random_fill.o" ]; then
    local rf_s rf_m
    rf_s="$(uname -s 2>/dev/null || echo Unknown)"
    rf_m="$(uname -m 2>/dev/null || echo unknown)"
    if [ "$rf_s" = "Darwin" ] && [ "$rf_m" = "arm64" ] \
  ...
  # w1082: Darwin arm64 whole TU is the .x. Do not thin+rest host-cc it.
  # PLATFORM: MACOS|DARWIN arm64. Linux and Windows fall through.
  if [ "$o" = "runtime_sync_os.o" ]; then
    local so_s so_m
    so_s="$(uname -s 2>/dev/null || echo Unknown)"
    so_m="$(uname -m 2>/dev/null || echo unknown)"
    if [ "$so_s" = "Darwin" ] && [ "$so_m" = "arm64" ] \
  ...
  # w1087: Darwin arm64 whole object is thin .x plus environ .x. Do not host-cc it.
  # PLATFORM: MACOS|DARWIN arm64. Linux and Windows fall through.
  if [ "$o" = "runtime_env_os.o" ]; then
    local eo_s eo_m
    eo_s="$(uname -s 2>/dev/null || echo Unknown)"
    eo_m="$(uname -m 2>/dev/null || echo unknown)"
    if [ "$eo_s" = "Darwin" ] && [ "$eo_m" = "arm64" ] \
