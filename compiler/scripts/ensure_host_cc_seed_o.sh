...
ensure_enc_dispatch_pure() {
  ...
  if ! ld $ld_flags -o "$merged_o" "$thin_o" "$rest_o" \
    ...
    || ! r3_prefer_nm_has_sym "$merged_o" "backend_enc_arm64_blr_c"; then
  ...
  mv -f "$merged_o" "$o"
  rm -f "$thin_o" "$rest_o"
  log "backend_enc_dispatch.o from $x_src (pure-asm) + f64/Cap tail [w891; marker, arch_arm64_enc_enc_blr, arch_arm64_enc_enc_ldr_xreg_xreg_imm, arch_x86_64_enc_enc_call_reg, arch_x86_64_enc_enc_load_rax_rbx_disp32, arch_riscv64_enc_enc_jalr_reg, arch_riscv64_enc_enc_ldr_xreg_xreg_imm, arch_x86_64_enc_enc_cdqe_rax_impl, backend_enc_append_u8_c_impl, backend_enc_append_u32_le_c_impl, and backend_enc_arm64_blr_c are in the .x; all stay strong]"
  return 0
}
