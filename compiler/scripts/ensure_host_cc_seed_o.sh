...
ensure_catalog_family() {
  ...
  for o in $list; do
    [ -z "$o" ] && continue
    # w847/w863/w864/w865/w866/w867/w868: seed-only cc drops the alias bodies that live in the .x.
    # PLATFORM: SHARED.
...
#   glue_codegen_import_path_to_c_prefix_into mangled face, and the w868
#   pipeline_expr_field_access_name_len mangled face; six weakened)
...
ensure_x_frontend_link_alias_prefer() {
  ...
  local weak_funcs="check_block_impl,check_expr_impl,find_or_alloc_ptr_type_ref,pipeline_typeck_set_active_ctx_c,pipeline_typeck_ptr_for_addr_of_operand_c,pipeline_expr_field_access_name_len_u8_ptr_i32_reti32"
  ...
  if (
    ...
    log "x-frontend-link-alias $o <- pure-asm $xsrc + lexer/mangled rest (w868; type_kind_ord, func_export, import_binding, heap_redirect, import_path, and field_name_len faces are in the .x; field_name_len stays weak)"
    rm -f "$thin" "$rest"
    return 0
  fi
...
try_ensure_r1_one() {
  ...
  # w847/w863/w864/w865/w866/w867/w868: alias bodies that live in the .x are not in the seed.
