...
# wave316 M2: emit_ctx_sret Cap residual C→.x (was wave223 C thin).
# PRODUCT inject: -E+$CC (ALLOW_E_REPLACE + stamp). Named scalar BSS OK
# under -E (file-local static + strong get/set). Pure-asm still banned.
# G.7 match mega wave223 leave. PLATFORM: SHARED.
pipeline_abi_inject_emit_ctx_sret_thin() {
  local o="$1"
  local thin_x="src/runtime_pipeline_abi_emit_ctx_sret_thin.x"
  local stamp="src/.pabi_w316_emit_ctx_sret.stamp"
  local saved_newer="${XLANG_PABI_THIN_INJECT_IF_NEWER-}"
  local saved_prefer="${XLANG_PABI_THIN_PREFER_ASM-}"
  local saved_e_repl="${XLANG_PABI_THIN_ALLOW_E_REPLACE-}"
  local had_newer=0 had_prefer=0 had_e_repl=0
  ...
  export XLANG_PABI_THIN_PREFER_ASM=0
  export XLANG_PABI_THIN_ALLOW_E_REPLACE=1
  pipeline_abi_inject_thin_leaf "$o" "$thin_x" "w316-emit-ctx-sret"
  rc=$?
  if [ "$had_newer" = "1" ]; then
    export XLANG_PABI_THIN_INJECT_IF_NEWER="$saved_newer"
  fi
...
# wave319 M2: typeck_check_expr Cap residual C→.x (was wave286 C thin).
# PRODUCT inject: -E+$CC (ALLOW_E_REPLACE + stamp). Match BSS OK under -E.
# Cold WEAK check_expr_impl{,_mega} left to typeck_x / seed (not in .x thin).
# G.7 WAVE286_TYPECK_CHECK_EXPR_ALWAYS. PLATFORM: SHARED.
pipeline_abi_inject_typeck_check_expr_thin() {
  local o="$1"
  local thin_x="src/runtime_pipeline_abi_typeck_check_expr_thin.x"
  local stamp="src/.pabi_w319_typeck_check_expr.stamp"
  local saved_newer="${XLANG_PABI_THIN_INJECT_IF_NEWER-}"
  local saved_prefer="${XLANG_PABI_THIN_PREFER_ASM-}"
  local saved_e_repl="${XLANG_PABI_THIN_ALLOW_E_REPLACE-}"
  local had_newer=0 had_prefer=0 had_e_repl=0
  ...
  export XLANG_PABI_THIN_PREFER_ASM=0
  export XLANG_PABI_THIN_ALLOW_E_REPLACE=1
  pipeline_abi_inject_thin_leaf "$o" "$thin_x" "w319-typeck-check-expr"
  rc=$?
  if [ "$had_newer" = "1" ]; then
    export XLANG_PABI_THIN_INJECT_IF_NEWER="$saved_newer"
  fi
...
case "$MODE" in
  ...
  inject-elf-ctx|inject_elf_ctx)
    # wave312: C→.x elf_ctx via -E+$CC (stamp + ALLOW_E_REPLACE).
    # Excludes macho_write (macho_write_thin). PLATFORM: SHARED shell.
    if [ "$#" -lt 1 ]; then
  ...
  inject-emit-ctx-sret|inject_emit_ctx_sret)
    # wave316: C→.x emit_ctx_sret via -E+$CC (stamp + ALLOW_E_REPLACE).
    # PLATFORM: SHARED shell · MACOS ingest · LINUX gold co-path.
    if [ "$#" -lt 1 ]; then
  ...
  inject-typeck-check-expr|inject_typeck_check_expr)
    # wave319: C→.x typeck_check_expr via -E+$CC (stamp + ALLOW_E_REPLACE).
    # PLATFORM: SHARED shell · MACOS ingest · LINUX gold co-path.
    if [ "$#" -lt 1 ]; then
