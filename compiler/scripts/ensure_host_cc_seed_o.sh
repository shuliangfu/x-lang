...
# arch_riscv64_enc_enc_ldr_xreg_xreg_imm (thin, w882, stays strong).
# Failure leaves the previous .o in place and returns 1.
# PLATFORM: SHARED.
ensure_enc_dispatch_pure() {
  ...
  if [ "$FORCE" != "1" ] && [ -f "$o" ]; then
    ...
    if [ "$stale" = "0" ]; then
      log "skip up-to-date $o (enc dispatch pure-asm w882)"
      return 0
    fi
  fi
  ...
  if ! ld $ld_flags -o "$merged_o" "$thin_o" "$rest_o" \
    ...
    || ! r3_prefer_nm_has_sym "$merged_o" "arch_riscv64_enc_enc_ldr_xreg_xreg_imm"; then
  ...
  mv -f "$merged_o" "$o"
  rm -f "$thin_o" "$rest_o"
  log "backend_enc_dispatch.o from $x_src (pure-asm) + f64/Cap tail [w882; marker, arch_arm64_enc_enc_blr, arch_arm64_enc_enc_ldr_xreg_xreg_imm, arch_x86_64_enc_enc_call_reg, arch_x86_64_enc_enc_load_rax_rbx_disp32, arch_riscv64_enc_enc_jalr_reg, and arch_riscv64_enc_enc_ldr_xreg_xreg_imm are in the .x; all stay strong]"
  return 0
}
