...
ensure_pipeline_abi_prefer_one() {
  ...
  if [ "$FORCE" != "1" ] && [ -f "$o" ]; then
    ...
    if [ -f src/runtime_pipeline_abi_glue_statics_thin.c ] \
      && [ src/runtime_pipeline_abi_glue_statics_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_type_alias_thin.c ] \
      && [ src/runtime_pipeline_abi_type_alias_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_module_import_thin.c ] \
      && [ src/runtime_pipeline_abi_module_import_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_module_enum_thin.c ] \
    ...
    if [ -f src/runtime_pipeline_abi_struct_layout_thin.c ] \
      && [ src/runtime_pipeline_abi_struct_layout_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_asm_locals_thin.c ] \
      && [ src/runtime_pipeline_abi_asm_locals_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_block_tree_thin.c ] \
    ...
    if [ -f src/runtime_pipeline_abi_grow_vec_thin.c ] \
      && [ src/runtime_pipeline_abi_grow_vec_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_dep_ctx_thin.c ] \
      && [ src/runtime_pipeline_abi_dep_ctx_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_elf_ctx_thin.c ] \
    ...
    if [ -f src/runtime_pipeline_abi_sidecar_pool_thin.c ] \
      && [ src/runtime_pipeline_abi_sidecar_pool_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_value_abi_thin.c ] \
      && [ src/runtime_pipeline_abi_value_abi_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_block_domain_thin.c ] \
    ...
    if [ -f src/runtime_pipeline_abi_module_func_thin.c ] \
      && [ src/runtime_pipeline_abi_module_func_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_onefunc_thin.c ] \
      && [ src/runtime_pipeline_abi_onefunc_thin.c -nt "$o" ]; then
      stale=1
    fi
    if [ -f src/runtime_pipeline_abi_bootstrap_glue_thin.c ] \
    ...
    if [ -f src/runtime_pipeline_abi_parse_orch_thin.c ] \
...
# wave261 glue_statics Cap bridges (C thin; function bodies only).
# Separate leaf: Darwin additive ingest. G.7: match mega leave. PLATFORM: SHARED.
pipeline_abi_inject_glue_statics_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_glue_statics_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w261-glue-statics inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_glue_st.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_glue_st_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_glue_st_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w261-glue-statics inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w261-glue-statics inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w261-glue-statics inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w261-glue-statics inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w261-glue-statics inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}

# wave262 type_alias Cap domain (C thin; file-local BSS map + faces).
# Separate leaf: Darwin additive ingest. G.7: match mega/seed leave. PLATFORM: SHARED.
pipeline_abi_inject_type_alias_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_type_alias_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w262-type-alias inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_ta.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_ta_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_ta_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w262-type-alias inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w262-type-alias inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w262-type-alias inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w262-type-alias inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w262-type-alias inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}

# wave263 module_import Cap domain (C thin; file-local BSS map + faces).
# Separate leaf: Darwin additive ingest. G.7: match mega/seed leave. PLATFORM: SHARED.
pipeline_abi_inject_module_import_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_module_import_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w263-module-import inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_imp.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_imp_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_imp_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w263-module-import inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w263-module-import inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w263-module-import inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w263-module-import inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w263-module-import inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}

# wave264 module_enum Cap domain (C thin; file-local BSS map + faces).
# Separate leaf: Darwin additive ingest. G.7: match mega/seed leave. PLATFORM: SHARED.
pipeline_abi_inject_module_enum_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_module_enum_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w264-module-enum inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_en.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_en_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_en_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w264-module-enum inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w264-module-enum inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w264-module-enum inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w264-module-enum inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
...
# wave266 struct_layout Cap domain (C thin; file-local BSS map + faces).
# Separate leaf: Darwin additive ingest. G.7: match mega/seed leave. PLATFORM: SHARED.
pipeline_abi_inject_struct_layout_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_struct_layout_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w266-struct-layout inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_sl.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_sl_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_sl_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w266-struct-layout inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w266-struct-layout inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w266-struct-layout inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w266-struct-layout inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w266-struct-layout inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}

# wave267 asm_locals Cap domain (C thin; file-local BSS map + faces).
# Separate leaf: Darwin additive ingest. G.7: match mega/seed leave. PLATFORM: SHARED.
pipeline_abi_inject_asm_locals_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_asm_locals_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w267-asm-locals inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_al.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_al_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_al_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w267-asm-locals inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w267-asm-locals inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w267-asm-locals inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w267-asm-locals inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w267-asm-locals inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}

# wave269 block_tree Cap domain (C thin; file-local walk stack + faces).
# Separate leaf: Darwin additive ingest. G.7: match mega/seed leave. PLATFORM: SHARED.
# Note: wave268 sizing already via slot_bytes_thin.x + NL-04 seed (no C redo).
pipeline_abi_inject_block_tree_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_block_tree_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w269-block-tree inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_bt.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_bt_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_bt_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w269-block-tree inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w269-block-tree inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w269-block-tree inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w269-block-tree inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w269-block-tree inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}
...
# wave271 GrowVec Cap domain (C thin; init/free/ensure/at/push/copy_append).
# Separate leaf: Darwin additive ingest. G.7: match mega/seed leave. PLATFORM: SHARED.
pipeline_abi_inject_grow_vec_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_grow_vec_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w271-grow-vec inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_gv.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_gv_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_gv_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w271-grow-vec inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w271-grow-vec inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w271-grow-vec inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w271-grow-vec inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w271-grow-vec inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}

# wave272 PipelineDepCtx Cap domain (C thin; sidecar table + faces).
# Separate leaf: Darwin additive ingest. G.7: match mega/seed leave. PLATFORM: SHARED.
pipeline_abi_inject_dep_ctx_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_dep_ctx_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w272-dep-ctx inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_dc.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_dc_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_dc_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w272-dep-ctx inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w272-dep-ctx inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w272-dep-ctx inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w272-dep-ctx inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w272-dep-ctx inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}

# wave273 ELF/Mach-O ctx + write_o Cap domain (C thin; file-local sidecars).
# Excludes macho_write_o (owned by macho_write_thin). PLATFORM: SHARED.
pipeline_abi_inject_elf_ctx_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_elf_ctx_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w273-elf-ctx inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_elf.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_elf_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_elf_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -Wno-unused-function -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w273-elf-ctx inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w273-elf-ctx inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w273-elf-ctx inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w273-elf-ctx inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w273-elf-ctx inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}
...
# wave275 sidecar pool Cap domain (C thin; arena/module/onefunc get+free).
# Separate leaf: Darwin additive ingest. G.7: match mega/seed leave. PLATFORM: SHARED.
pipeline_abi_inject_sidecar_pool_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_sidecar_pool_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w275-sidecar-pool inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_sc.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_sc_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_sc_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -Wno-unused-function -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w275-sidecar-pool inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w275-sidecar-pool inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w275-sidecar-pool inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w275-sidecar-pool inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w275-sidecar-pool inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}
...
# wave276 value-ABI Cap residual (C thin; by-value get/set_copy + float IEEE).
# Separate leaf: Darwin additive ingest. ALWAYS residual (not FROM_X-gated).
# G.7: match seed WAVE276_ARENA_VALUE_ABI_ALWAYS. PLATFORM: SHARED.
pipeline_abi_inject_value_abi_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_value_abi_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w276-value-abi inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_vabi.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_vabi_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_vabi_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -Wno-unused-function -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w276-value-abi inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w276-value-abi inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w276-value-abi inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w276-value-abi inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w276-value-abi inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}
...
# wave277 block domain Cap residual (C thin; append/getters/patch/stmt_order).
# Separate leaf: Darwin additive ingest. ALWAYS residual (not FROM_X-gated).
# G.7: match seed WAVE277_BLOCK_DOMAIN_ALWAYS. PLATFORM: SHARED.
pipeline_abi_inject_block_domain_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_block_domain_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w277-block-domain inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_blk.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_blk_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_blk_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -Wno-unused-function -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w277-block-domain inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w277-block-domain inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w277-block-domain inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w277-block-domain inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
...
# wave280 module_func Cap residual (C thin; Func accessors + param + pmfo BSS).
# Separate leaf: Darwin additive ingest. ALWAYS residual (not FROM_X-gated).
# G.7: match seed WAVE280_MODULE_FUNC_DOMAIN_ALWAYS. PLATFORM: SHARED.
pipeline_abi_inject_module_func_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_module_func_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w280-module-func inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_mfn.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_mfn_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_mfn_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -Wno-unused-function -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w280-module-func inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w280-module-func inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w280-module-func inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w280-module-func inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w280-module-func inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}
...
# wave281 onefunc Cap residual (C thin; onefunc_* + fill_from_onefunc).
# Separate leaf: Darwin additive ingest. ALWAYS residual (not FROM_X-gated).
# G.7: match seed WAVE281_ONEFUNC_DOMAIN_ALWAYS. PLATFORM: SHARED.
pipeline_abi_inject_onefunc_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_onefunc_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w281-onefunc inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_ofn.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_ofn_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_ofn_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -Wno-unused-function -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w281-onefunc inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w281-onefunc inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w281-onefunc inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w281-onefunc inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w281-onefunc inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}
...
# wave282 bootstrap_glue Cap residual (C thin; typeck/asm scope/dep_skip).
# Separate leaf: Darwin additive ingest. ALWAYS residual (not FROM_X-gated).
# G.7: match seed WAVE282_BOOTSTRAP_GLUE_ALWAYS. PLATFORM: SHARED.
pipeline_abi_inject_bootstrap_glue_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_bootstrap_glue_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w282-bootstrap-glue inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_bglue.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_bglue_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_bglue_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -Wno-unused-function -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w282-bootstrap-glue inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w282-bootstrap-glue inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w282-bootstrap-glue inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w282-bootstrap-glue inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
...
# wave284 parse_orch Cap residual (C thin; parse/load/typeck orch).
# Separate leaf: Darwin additive ingest. ALWAYS residual (not FROM_X-gated).
# G.7: match seed WAVE284_PARSE_ORCH_ALWAYS. PLATFORM: SHARED.
pipeline_abi_inject_parse_orch_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_parse_orch_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w284-parse-orch inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_porch.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_porch_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_porch_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -Wno-unused-function -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w284-parse-orch inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w284-parse-orch inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w284-parse-orch inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w284-parse-orch inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
  log "pipeline_abi w284-parse-orch inject: merge failed; restored base"
  rm -f "$thin_o" "$base_o" "$restore_o"
  return 1
}
...
# wave289 codegen_outbuf Cap residual (C thin; float_lit / try_propagate emit).
# Separate leaf: Darwin additive ingest. ALWAYS residual (not FROM_X-gated).
# G.7: match seed WAVE289_CODEGEN_OUTBUF_ALWAYS. PLATFORM: SHARED.
pipeline_abi_inject_codegen_outbuf_thin() {
  local o="$1"
  local src="src/runtime_pipeline_abi_codegen_outbuf_thin.c"
  local thin_o base_o restore_o
  [ -s "$o" ] && [ -f "$src" ] || return 0
  if pipeline_abi_o_is_libtool_archive "$o"; then
    log "pipeline_abi w289-codegen-outbuf inject skip: $o is libtool archive"
    return 1
  fi
  thin_o="$(mktemp "${TMPDIR:-/tmp}/pabi_cobuf.XXXXXX.o")"
  base_o="$(mktemp "${TMPDIR:-/tmp}/pabi_cobuf_base.XXXXXX.o")"
  restore_o="$(mktemp "${TMPDIR:-/tmp}/pabi_cobuf_restore.XXXXXX.o")"
  # shellcheck disable=SC2086
  if ! ${CC:-cc} ${BASE_CFLAGS:--I. -Iinclude -Isrc} -I. -Iinclude -Isrc -Wno-unused-function -c -o "$thin_o" "$src" 2>/dev/null; then
    log "pipeline_abi w289-codegen-outbuf inject: cc thin failed"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 1
  fi
  cp -f "$o" "$base_o"
  cp -f "$o" "$restore_o"
  if ! pipeline_abi_weaken_thin_syms_in_obj "$base_o" "$thin_o"; then
    log "pipeline_abi w289-codegen-outbuf inject skip: cannot weaken leftover T"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  if pure_ld_partial_merge "$o" "$thin_o" "$base_o" 2>/dev/null; then
    if pipeline_abi_o_is_libtool_archive "$o"; then
      cp -f "$restore_o" "$o"
      log "pipeline_abi w289-codegen-outbuf inject: libtool archive; restored base"
      rm -f "$thin_o" "$base_o" "$restore_o"
      return 1
    fi
    log "pipeline_abi w289-codegen-outbuf inject OK (first-wins over leftover)"
    rm -f "$thin_o" "$base_o" "$restore_o"
    return 0
  fi
  cp -f "$restore_o" "$o"
