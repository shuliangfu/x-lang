...
case "$UNAME_S" in
  MINGW*|MSYS*|CYGWIN*|Windows_NT*)
    for _wov in src/win_assign_var_override.o src/win_assign_field_override.o src/win_assign_index_override.o src/win_assign_deref_override.o src/win_struct_let_init_override.o; do
      if [ -s "$_wov" ]; then
