...
#   ./scripts/preflight_g06_coldstart.sh --asm-e   # 额外跑 asm.sx -E 烟测（默认 timeout 300s）
#
# 环境：
...
# lsp_gen 不得与 sx_seed_bridge / lsp_state 重复定义
...
# --- 阶段 C：可选 asm.sx -E 烟测（分钟级，带 timeout，失败即停）---
if [ "$ASM_E" -eq 0 ]; then
...
if [ "$rc" -eq 124 ]; then
  fail "asm.sx -E timeout after ${TIMEOUT_SEC}s (increase SHUX_G06_E_TIMEOUT or fix hang)"
fi
if [ "$rc" -ne 0 ]; then
  echo "preflight_g06: asm.sx -E exit=$rc" >&2
  if [ -s "$ERR" ]; then
    echo "--- stderr tail ---" >&2
    tail -30 "$ERR" >&2
  else
  ...
  fail "asm.sx -E failed"
fi
if [ ! -s "$TMP" ]; then
  fail "asm.sx -E produced empty output"
fi
ok "asm.sx -E smoke ($(wc -c <"$TMP" | tr -d ' ') bytes C)"
