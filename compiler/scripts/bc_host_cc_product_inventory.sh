...
# 8.3.1 domain thin cuts: ctfe + assign + coerce_init + method_call (#include TU).
...
PRODUCT_RESIDUAL_ROWS=(
  ...
  "compiler/pipeline_glue.c|8.3.1|product mega glue (typeck/codegen/asm/match)|25000|present"
...
run_check() {
  ...
  echo "bc_host_cc_product_inventory: CHECK OK (BC open · 8.3.1 ctfe+assign+coerce_init+method_call slices present · 8.3.9 absent · host-cc residual still required)" >&2
}
