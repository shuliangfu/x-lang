...
# 用法：在 compiler 目录执行 XLANG=./xlang ./scripts/check_asm_o_quality.sh
# 依赖：build_asm/*.o 已由 build_xlang_asm 或等价 asm 编译产生；需 objdump（LLVM/GNU）。
# Mach-O 段名为 __text；ELF（Linux GNU ld 产出）为 .text —— 二者皆检测，避免 Linux 上误判「全部 EMPTY」。
# 退出码：默认 0（仅报告）；XLANG_ASM_QUALITY_STRICT=1 时若有代码段为空或缺失 .o 则退出 1。
# 写入：build_asm/.asm_text_quality — 1 全部非空，0 否则（供 build_xlang_asm 拓扑自动选择）。
...
BUILD_DIR="build_asm"
STRICT="${XLANG_ASM_QUALITY_STRICT:-0}"
TAB=$(printf '\t')
...
while IFS= read -r line; do
  ...
  if [ ! -f "$path" ]; then
    ...
    # token.x 仅 enum+struct+单函数；若仍空则计 bad。允许显式跳过：XLANG_ASM_ALLOW_EMPTY_TEXT=token.o,...
    allow_empty=0
    case ",${XLANG_ASM_ALLOW_EMPTY_TEXT:-}," in
      *",$out,"*) allow_empty=1 ;;
    esac
