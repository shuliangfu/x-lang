#!/bin/sh
# shux_compile_std_fs_formal.sh — formal std/fs/fs.o for -backend asm product link
#
...
#
# Usage: shux_compile_std_fs_formal.sh <out.o>
# Env: SHUX=compiler binary (default ./shux_asm → ./shux → ./shux-c)
set -e

out_o="$1"
if [ -z "$out_o" ]; then
  echo "usage: shux_compile_std_fs_formal.sh <out.o>" >&2
  exit 1
fi

SHUX_BIN="${SHUX:-./shux_asm}"
[ -x "$SHUX_BIN" ] || SHUX_BIN="./shux"
[ -x "$SHUX_BIN" ] || SHUX_BIN="./shux_asm"
[ -x "$SHUX_BIN" ] || SHUX_BIN="./shux-c"
[ -x "$SHUX_BIN" ] || { echo "shux_compile_std_fs_formal.sh: no shux binary" >&2; exit 1; }
...
tmp_dir=$(mktemp -d 2>/dev/null || mktemp -d -t shuxfs)
...
EOF
log="$tmp_dir/shux.log"
# -o fails at final link sometimes; SHUX_KEEP_C keeps full product C.
SHUX_KEEP_C=1 "$SHUX_BIN" -backend c -L .. -o "$tmp_dir/try.out" "$vehicle" \
  >"$log" 2>&1 || true

kept=$(grep -E 'kept generated C:|keeping generated C:' "$log" 2>/dev/null \
  | sed -n 's/.*generated C: //p' | tail -1)
if [ -z "$kept" ] || [ ! -f "$kept" ] || [ ! -s "$kept" ]; then
  echo "shux_compile_std_fs_formal.sh: no KEEP_C from vehicle (see $log)" >&2
  tail -30 "$log" >&2 || true
  exit 1
fi
...
if ! cc -c $CFLAGS "$gen_c" -o "$raw_o" 2>"$tmp_dir/cc.err"; then
  echo "shux_compile_std_fs_formal.sh: cc -c failed for KEEP_C" >&2
...
if ! nm "$out_o" 2>/dev/null | grep -q 'std_fs_invalid'; then
  echo "shux_compile_std_fs_formal.sh: missing std_fs_invalid in $out_o" >&2
  exit 1
fi

echo "shux_compile_std_fs_formal.sh: OK -> $out_o"
