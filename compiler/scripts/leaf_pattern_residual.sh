...
# paths (40; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=49.
# 8.3.1+8.3.2: +29 #include slices (ctfe/.../soa + asm_emit_assign + asm_emit_index +
#   asm_emit_match +
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=49
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=49' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=49 (8.3.1 match/expr_if)"
fi
...
# Fixed multi-token authority COUNT=49 (8.3.1: +1 asm_emit_match slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 40
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 49 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 49 in mk, got ${_xsd_n:-0}"
fi
