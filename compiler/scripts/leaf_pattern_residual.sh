...
# paths (15; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=24.
# 8.3.1: +5 #include slices (ctfe/assign/coerce_init/field_access/soa) into PIPELINE_X_DEPS STALE.
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=24
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=24' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=24 (8.3.1 slices)"
fi
...
# Fixed multi-token authority COUNT=24 (8.3.1: was 23; +1 coerce_init slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 15
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 24 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 24 in mk, got ${_xsd_n:-0}"
fi
