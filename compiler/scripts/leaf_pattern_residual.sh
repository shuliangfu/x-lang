...
# paths (51; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=60.
# 8.3.1+8.3.2: +40 #include slices (ctfe/.../soa + asm_emit_assign + asm_emit_index +
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=60' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=60 (8.3.1 spill emit face)"
fi
...
# Fixed multi-token authority COUNT=60 (8.3.1: +1 asm_emit_spill slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 51
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 60 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 60 in mk, got ${_xsd_n:-0}"
fi
