...
# paths (26; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=35.
# 8.3.1+8.3.2: +16 #include slices (ctfe/.../block_inits/field_access/soa + ast_pool_module_import) into PIPELINE_X_DEPS STALE.
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=35
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=35' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=35 (8.3.1 slices)"
fi
...
# Fixed multi-token authority COUNT=35 (8.3.2: was 34; +1 ast_pool_module_import slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 26
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 35 ]; then
  bad "8.3.2 expected SOURCE_DEPS fixed multi-token count 35 in mk, got ${_xsd_n:-0}"
fi
