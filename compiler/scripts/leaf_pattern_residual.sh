...
# paths (14; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=23.
# 8.3.1: +4 #include slices (ctfe/assign/field_access/soa) into PIPELINE_X_DEPS STALE.
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=23
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=23' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=23 (8.3.1 slices)"
fi
...
# Fixed multi-token authority COUNT=23 (8.3.1: was 22; +1 assign slice):
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 23 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 23 in mk, got ${_xsd_n:-0}"
fi
