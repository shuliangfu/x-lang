...
# paths (45; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=54.
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=54
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=54' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=54 (8.3.1 call_args emit face)"
fi
...
# Fixed multi-token authority COUNT=54 (8.3.1: +1 asm_emit_call_args slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 45
