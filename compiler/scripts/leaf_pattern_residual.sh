...
# paths (46; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=55.
# 8.3.1+8.3.2: +35 #include slices (ctfe/.../soa + asm_emit_assign + asm_emit_index +
#   asm_emit_match + asm_emit_panic + asm_emit_field_access + asm_emit_binop + asm_emit_cmp +
#   asm_emit_call_args + asm_emit_struct_lit +
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=55
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=55' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=55 (8.3.1 struct_lit emit face)"
fi
...
# Fixed multi-token authority COUNT=55 (8.3.1: +1 asm_emit_struct_lit slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 46
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 55 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 55 in mk, got ${_xsd_n:-0}"
fi
