...
# paths (18; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=27.
# 8.3.1: +8 #include slices (ctfe/assign/coerce_init/method_call/check_block/region_assign/field_access/soa) into PIPELINE_X_DEPS STALE.
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=27
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=27' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=27 (8.3.1 slices)"
fi
...
# Fixed multi-token authority COUNT=27 (8.3.1: was 26; +1 region_assign slice):
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 27 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 27 in mk, got ${_xsd_n:-0}"
fi
