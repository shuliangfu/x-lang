...
# paths (43; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=52.
# 8.3.1+8.3.2: +32 #include slices (ctfe/.../soa + asm_emit_assign + asm_emit_index +
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=52
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=52' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=52 (8.3.1 binop emit face)"
fi
...
# Fixed multi-token authority COUNT=52 (8.3.1: +1 asm_emit_binop slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 43
