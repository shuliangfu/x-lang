...
# paths (44; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=53.
# 8.3.1+8.3.2: +34 #include slices (ctfe/.../soa + asm_emit_assign + asm_emit_index +
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=53
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=53' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=53 (8.3.1 cmp emit face)"
fi
...
# Fixed multi-token authority COUNT=53 (8.3.1: +1 asm_emit_cmp slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 44
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 53 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 53 in mk, got ${_xsd_n:-0}"
fi
