...
# paths (42; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=51.
# 8.3.1+8.3.2: +31 #include slices (ctfe/.../soa + asm_emit_assign + asm_emit_index +
#   asm_emit_match + asm_emit_panic + asm_emit_field_access +
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=51
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=51' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=51 (8.3.1 field_access emit face)"
fi
...
# Fixed multi-token authority COUNT=51 (8.3.1: +1 asm_emit_field_access slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 42
