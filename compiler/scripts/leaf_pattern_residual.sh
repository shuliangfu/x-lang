...
# paths (22; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=31.
# 8.3.1: +12 #include slices (ctfe/assign/coerce_init/method_call/check_block/region_assign/asm_emit_unary/asm_emit_as/asm_emit_return/asm_emit_logand/field_access/soa) into PIPELINE_X_DEPS STALE.
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=31
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=31' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=31 (8.3.1 slices)"
fi
...
# Fixed multi-token authority COUNT=31 (8.3.1: was 30; +1 asm_emit_logand slice):
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 31 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 31 in mk, got ${_xsd_n:-0}"
fi
