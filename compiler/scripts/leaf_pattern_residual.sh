...
# paths (39; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=48.
# 8.3.1+8.3.2: +28 #include slices (ctfe/.../soa + asm_emit_assign + asm_emit_index +
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=48
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=48' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=48 (8.3.1 index/addr_of/deref)"
fi
...
# Fixed multi-token authority COUNT=48 (8.3.1: +1 asm_emit_index slice):
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 48 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 48 in mk, got ${_xsd_n:-0}"
fi
