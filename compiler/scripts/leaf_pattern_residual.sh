...
# Wave: 746–791 Track MG · 11.3.1 path (B7A heat dep-edge thin wave791 · thin-unify wave790 · try-heat wave789 · B7B shell catalog · B7A cold 0-make · Makefile dep edges residual · not physical delete · Windows gate + dual-end).
...
PHYS_DEL_BUCKET_B7_WAVE=wave791
...
PHYS_DEL_BUCKET_B7A_HEAT_DEP_THIN_SCOPE=runtime_star_seed_x_pure_force_ensure
PHYS_DEL_BUCKET_B7A_HEAT_DEP_THIN_COUNT=28
SWALLOWED_B7A_HEAT_DEP_THIN=1
B7A_HEAT_DEP_THIN_SWALLOWED=1
B7A_HEAT_DEP_THIN_VIA=makefile_force_plus_try_heat_shell_mtime
B7A_HEAT_DEP_THIN_WAVE=wave791
B7A_HEAT_DEP_THIN_NOTE=28_runtime_star_leaves_source_prereqs_removed_shell_owns_freshness
...
if ! printf '%s\n' "$_out" | grep -q 'PHYS_DEL_PREP_NEXT=B7_physical_delete_makefile_after_windows_not_this_wave'; then
  bad "dump PHYS_DEL_PREP_NEXT must stay physical-delete-after-windows (wave791)"
fi
# wave791: pure runtime_* dep-edge thin (FORCE + shell mtime)
if ! printf '%s\n' "$_out" | grep -q 'SWALLOWED_B7A_HEAT_DEP_THIN=1'; then
  bad "dump must set SWALLOWED_B7A_HEAT_DEP_THIN=1 (wave791)"
fi
if ! printf '%s\n' "$_out" | grep -q 'B7A_HEAT_DEP_THIN_SWALLOWED=1'; then
  bad "dump B7A_HEAT_DEP_THIN_SWALLOWED must be 1 (wave791)"
fi
if ! printf '%s\n' "$_out" | grep -q 'PHYS_DEL_BUCKET_B7A_HEAT_DEP_THIN=1'; then
  bad "dump must set PHYS_DEL_BUCKET_B7A_HEAT_DEP_THIN=1 (wave791)"
fi
if ! printf '%s\n' "$_out" | grep -q 'PHYS_DEL_BUCKET_B7A_HEAT_RESIDUAL=1'; then
  bad "dump must keep PHYS_DEL_BUCKET_B7A_HEAT_RESIDUAL=1 (wave791 residual edges)"
else
  note "residual class inventory dump OK (wave747–791 + B7A heat dep-thin + thin-unify + try-heat + B7B shell catalog + Windows + dual-end)"
fi
...
# wave791: pure runtime_* leaves use FORCE + ensure script (source prereqs removed)
if [ -f "$MF" ]; then
  ...
  if [ "${_force_n:-0}" -lt 28 ]; then
    bad "Makefile wave791 FORCE dep-thin runtime leaves expected >=28 (n=${_force_n})"
  else
    note "Makefile heat dep-edge FORCE thin (n=${_force_n}; wave791)"
  fi
