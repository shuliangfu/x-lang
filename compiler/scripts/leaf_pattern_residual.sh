...
# paths (41; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=50.
# 8.3.1+8.3.2: +30 #include slices (ctfe/.../soa + asm_emit_assign + asm_emit_index +
#   asm_emit_match + asm_emit_panic +
...
PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=50
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=50' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=50 (8.3.1 panic face)"
fi
...
# Fixed multi-token authority COUNT=50 (8.3.1: +1 asm_emit_panic slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 41
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 50 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 50 in mk, got ${_xsd_n:-0}"
fi
