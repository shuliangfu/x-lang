...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=61' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=61 (8.3.1 index_eff_addr emit face)"
fi
...
# Fixed multi-token authority COUNT=61 (8.3.1: +1 asm_emit_index_eff_addr slice):
#   SRCS 4 + MAIN_X_DEPS 4 + PREPROCESS_X_DEPS 1 + PIPELINE_X_DEPS fixed paths 52
...
_xsd_n=$(awk '
  ...
  END { print t+0 }
' "$_XSD_MK")
if [ "${_xsd_n:-0}" -ne 61 ]; then
  bad "8.3.1 expected SOURCE_DEPS fixed multi-token count 61 in mk, got ${_xsd_n:-0}"
fi
