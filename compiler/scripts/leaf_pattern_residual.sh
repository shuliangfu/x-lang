...
# paths (50; excludes $(PIPELINE_ASM_X_DEPS) wildcard token) = COUNT=59.
...
#   asm_emit_struct_let + asm_emit_index_helpers +
...
if ! grep -q 'PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=59' <<<"$_out"; then
  bad "dump must set PHYS_DEL_B7B_SOURCE_DEPS_LIST_COUNT=59 (8.3.1 index_helpers emit face)"
fi
...
# Fixed multi-token authority COUNT=59 (8.3.1: +1 asm_emit_index_helpers slice):
