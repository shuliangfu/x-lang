...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # G-02f-13 / G-02f-267～274：runtime_link_abi.o ← seed（G05 hot）
  # PREFER_X_O=1：L0..L5 + L8/L8b hybrid → rest（SHUX_LABI_*_FROM_X）
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_labi_l0_seed" ]; then
        ...
        if [ "$_labi_l0_ok" = "1" ] && [ -n "$_labi_rest_o" ] \
          ...
          if $CC -r -nostdlib -o "$_labi_o" $_labi_link "$_labi_rest_o" 2>/dev/null; then
            echo "g05_ensure: $_labi_o ← L0/L1/L2/L3/L4/L5/L8/L8b + link_abi rest (G-02f-274 hybrid)"
            _labi_done=1
          fi
        fi
        if [ "$_labi_done" = "0" ]; then
          echo "g05_ensure: L0/L1/L2/L3/L4/L5/L8/L8b link_abi hybrid failed; fallback full seed" >&2
        fi
        rm -f "$_labi_l0_o" "$_labi_l1_o" "$_labi_l2_o" "$_labi_l3_o" "$_labi_l4_o" "$_labi_l5_o" "$_labi_l8_o" "$_labi_l8b_o" "$_labi_rest_o"
      fi
      if [ "$_labi_done" = "0" ]; then
