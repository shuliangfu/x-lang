...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # 默认整 seed；PREFER_X_O=1 时 simd_enc_thin.x（64：+ select/hadd/movss/soa）+ seed-rest ld -r
    ...
    if [ ! -f "$_simd_enc_o" ] || [ "$_simd_enc" -nt "$_simd_enc_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_simd_enc_thin_x" ]; then
        ...
        if [ -n "$_simd_enc_thin_o" ] && [ -n "$_simd_enc_rest_o" ] \
          ...
          echo "g05_ensure: $_simd_enc_o ← $_simd_enc_thin_x + seed-rest (G-02f-348/417 L2 hybrid simd_enc thin)"
          _simd_enc_done=1
        else
