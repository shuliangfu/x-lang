...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # G-02f-9 / G-02f-352/354：backend_enc_dispatch.o
  # 默认整 seed；PREFER 时 thin 13 arm64+ta shells + rest ld -r
    ...
    if [ ! -f "$_bed_o" ] || [ "$_bed" -nt "$_bed_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_bed_thin_x" ]; then
        ...
        if [ -n "$_bed_thin_o" ] && [ -n "$_bed_rest_o" ] \
          ...
          echo "g05_ensure: $_bed_o ← $_bed_thin_x + seed-rest (G-02f-352/354 L2 hybrid enc_dispatch thin)"
          _bed_done=1
        else
  ...
  for _disp in backend_arch_emit_dispatch backend_try_inline_dispatch backend_call_dispatch; do
