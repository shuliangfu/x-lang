...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # 默认整 seed；PREFER_X_O=1 时 abi_thin.x（45：+ dep_path/entry_store/target_os）+ rest ld -r
    ...
    if [ ! -f "$_rdabi_o" ] || [ "$_rdabi" -nt "$_rdabi_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_rdabi_thin_x" ]; then
        ...
        if [ -n "$_rdabi_thin_o" ] && [ -n "$_rdabi_rest_o" ] \
          ...
          echo "g05_ensure: $_rdabi_o ← $_rdabi_thin_x + seed-rest (G-02f-343/401 L2 hybrid driver_abi thin)"
          _rdabi_done=1
        else
