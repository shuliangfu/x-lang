...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # 默认整 seed；PREFER_X_O=1 时 abi_thin.x（38：+ module_only/parse_metric/entry_len）+ rest ld -r
    ...
    if [ ! -f "$_rdabi_o" ] || [ "$_rdabi" -nt "$_rdabi_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_rdabi_thin_x" ]; then
        ...
        if [ -n "$_rdabi_thin_o" ] && [ -n "$_rdabi_rest_o" ] \
          ...
          echo "g05_ensure: $_rdabi_o ← $_rdabi_thin_x + seed-rest (G-02f-343/388 L2 hybrid driver_abi thin)"
          _rdabi_done=1
        else
  ...
  # 默认整 seed；PREFER_X_O=1 时 simd_enc_thin.x（11：pure+insn forward）+ seed-rest ld -r
    ...
    if [ ! -f "$_simd_enc_o" ] || [ "$_simd_enc" -nt "$_simd_enc_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_simd_enc_thin_x" ]; then
        ...
        if [ -n "$_simd_enc_thin_o" ] && [ -n "$_simd_enc_rest_o" ] \
          ...
          echo "g05_ensure: $_simd_enc_o ← $_simd_enc_thin_x + seed-rest (G-02f-348/390 L2 hybrid simd_enc thin)"
          _simd_enc_done=1
        else
    ...
    if [ ! -f "$_simd_loop_o" ] || [ "$_simd_loop" -nt "$_simd_loop_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_simd_loop_thin_x" ]; then
        ...
        if [ -n "$_simd_loop_thin_o" ] && [ -n "$_simd_loop_rest_o" ] \
          ...
          echo "g05_ensure: $_simd_loop_o ← $_simd_loop_thin_x + seed-rest (G-02f-349/384 L2 hybrid simd_loop thin)"
          _simd_loop_done=1
        else
