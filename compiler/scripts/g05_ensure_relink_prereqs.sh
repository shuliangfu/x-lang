...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # 默认整 seed；PREFER_X_O=1 时 abi_thin.x（59：+ import scan/large-stack）+ rest ld -r
    ...
    if [ ! -f "$_rdabi_o" ] || [ "$_rdabi" -nt "$_rdabi_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_rdabi_thin_x" ]; then
        ...
        if [ -n "$_rdabi_thin_o" ] && [ -n "$_rdabi_rest_o" ] \
          ...
          echo "g05_ensure: $_rdabi_o ← $_rdabi_thin_x + seed-rest (G-02f-343/414 L2 hybrid driver_abi thin)"
          _rdabi_done=1
        else
  ...
  # 默认整 seed；PREFER_X_O=1 时 diagnostic_thin.x（76 门闩）+ seed-rest ld -r
    ...
    if [ ! -f "$_rdd_o" ] || [ "$_rdd" -nt "$_rdd_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_rdd_thin_x" ]; then
        ...
        if [ -n "$_rdd_thin_o" ] && [ -n "$_rdd_rest_o" ] \
          ...
          echo "g05_ensure: $_rdd_o ← $_rdd_thin_x + seed-rest (G-02f-341/409 L2 hybrid diagnostic thin)"
          _rdd_done=1
        else
  ...
  # 默认整 seed；PREFER_X_O=1 时 simd_enc_thin.x（56：pure+insn forward）+ seed-rest ld -r
    ...
    if [ ! -f "$_simd_enc_o" ] || [ "$_simd_enc" -nt "$_simd_enc_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_simd_enc_thin_x" ]; then
        ...
        if [ -n "$_simd_enc_thin_o" ] && [ -n "$_simd_enc_rest_o" ] \
          ...
          echo "g05_ensure: $_simd_enc_o ← $_simd_enc_thin_x + seed-rest (G-02f-348/399 L2 hybrid simd_enc thin)"
          _simd_enc_done=1
        else
  ...
  # 默认整 seed；PREFER_X_O=1 时 simd_loop_thin.x（13 pure/forward glue）+ seed-rest ld -r
    ...
    if [ ! -f "$_simd_loop_o" ] || [ "$_simd_loop" -nt "$_simd_loop_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_simd_loop_thin_x" ]; then
        ...
        if [ -n "$_simd_loop_thin_o" ] && [ -n "$_simd_loop_rest_o" ] \
          ...
          echo "g05_ensure: $_simd_loop_o ← $_simd_loop_thin_x + seed-rest (G-02f-349/412 L2 hybrid simd_loop thin)"
          _simd_loop_done=1
        else
  ...
  # 默认整 seed；PREFER_X_O=1 时 diag_thin.x（62 门闩：+ io fprint/table_len）+ seed-rest ld -r
    ...
    if [ ! -f "$_diag_o" ] || [ "$_diag" -nt "$_diag_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_diag_thin_x" ]; then
        ...
        if [ -n "$_diag_thin_o" ] && [ -n "$_diag_rest_o" ] \
          ...
          echo "g05_ensure: $_diag_o ← $_diag_thin_x + seed-rest (G-02f-347/415 L2 hybrid diag thin)"
          _diag_done=1
        else
