...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # G-02f-11 / fmt_check R2 thin + Cap residual pure 深迁（续 try_walk pure）：fmt_check_cmd_driver.o
  # PREFER_X_O=1：thin.x（lit/entry+pure 含 try_walk/cwd_fallback）+ seed-rest（FROM_X）ld -r
