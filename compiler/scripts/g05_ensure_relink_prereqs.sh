      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_rt_content_seed" ]; then
        ...
        if [ -n "$_rt_run_o" ] && [ -f "$_rt_run_seed" ]; then
          ...
          if $CC $BASE_CFLAGS -I. -Iinclude -Isrc -c -o "$_rt_run_o" "$_rt_run_seed"; then
            _rt_run_ok=1
            echo "g05_ensure: R7 run/exec ← $_rt_run_seed (G-02f-297~299 seed slice)"
          fi
        fi
        ...
        if [ "$_rt_content_ok" = "1" ] && [ -n "$_rt_rest_o" ] \
          ...
          if $CC -r -nostdlib -o "$_rt_o" $_rt_link_objs "$_rt_rest_o" 2>/dev/null; then
            echo "g05_ensure: $_rt_o ← R2/R0/R1/R5-lite/R3/R6/R7/R9/R10/diag/emit_st/elf_diag/lib_root/parse_diag/fs_open + rest (G-02f-308 hybrid)"
            _rt_done=1
          fi
        fi
        if [ "$_rt_done" = "0" ]; then
        ...
        rm -f "$_rt_c_o" "$_rt_u_o" "$_rt_a_o" "$_rt_e_o" "$_rt_p_o" "$_rt_cmp_o" "$_rt_run_o" "$_rt_asm_o" "$_rt_ent_o" "$_rt_diag_o" "$_rt_est_o" "$_rt_elfd_o" "$_rt_lr_o" "$_rt_pd_o" "$_rt_fs_o" "$_rt_rest_o"
      fi
      if [ "$_rt_done" = "0" ]; then
