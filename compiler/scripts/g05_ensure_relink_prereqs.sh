...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # 8.3.1: #include slices (ctfe/assign/coerce_init/method_call/check_block/region_assign/field_access/soa/ast_pool*) also force STALE —
  ...
  if [ -f pipeline_glue.c ] && [ -f pipeline_gen.c ]; then
    _glue_slice_stale=0
    for _gs in pipeline_typeck_ctfe.c pipeline_typeck_assign.c pipeline_typeck_coerce_init.c pipeline_typeck_method_call.c pipeline_typeck_check_block.c pipeline_typeck_region_assign.c pipeline_typeck_field_access.c pipeline_typeck_soa.c \
