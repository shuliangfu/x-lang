...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # 默认整 seed；PREFER_X_O=1 时 simd_loop_thin.x（10 pure/forward glue）+ seed-rest ld -r
    ...
    if [ ! -f "$_simd_loop_o" ] || [ "$_simd_loop" -nt "$_simd_loop_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_simd_loop_thin_x" ]; then
        ...
        if [ -n "$_simd_loop_thin_o" ] && [ -n "$_simd_loop_rest_o" ] \
          ...
          echo "g05_ensure: $_simd_loop_o ← $_simd_loop_thin_x + seed-rest (G-02f-349/403 L2 hybrid simd_loop thin)"
          _simd_loop_done=1
        else
    ...
    if [ ! -f "$_fcc_o" ] || [ "$_fcc" -nt "$_fcc_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_fcc_thin_x" ]; then
        ...
        if [ -n "$_fcc_thin_o" ] && [ -n "$_fcc_rest_o" ] \
          ...
          echo "g05_ensure: $_fcc_o ← $_fcc_thin_x + seed-rest (G-02f-350/389 L2 hybrid fmt_check thin)"
          _fcc_done=1
        else
