...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # G-02f-12 / G-02f-343/344：runtime_driver_abi.o
  # 默认整 seed；PREFER_X_O=1 时 abi_thin.x（28：flag pure + check_ok/now_sec/large_stack + timing）+ rest ld -r
    ...
    if [ ! -f "$_rdabi_o" ] || [ "$_rdabi" -nt "$_rdabi_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_rdabi_thin_x" ]; then
        ...
        if [ -n "$_rdabi_thin_o" ] && [ -n "$_rdabi_rest_o" ] \
          ...
          echo "g05_ensure: $_rdabi_o ← $_rdabi_thin_x + seed-rest (G-02f-343/344 L2 hybrid driver_abi thin)"
          _rdabi_done=1
        else
  ...
  # 默认整 seed；PREFER_X_O=1 时 diag_thin.x（32 门闩）+ seed-rest（_impl）ld -r
    ...
    if [ ! -f "$_diag_o" ] || [ "$_diag" -nt "$_diag_o" ] \
      ...
      if [ "${SHUX_G05_PREFER_X_O:-0}" = "1" ] && [ -f "$_diag_thin_x" ]; then
        ...
        if [ -n "$_diag_thin_o" ] && [ -n "$_diag_rest_o" ] \
          ...
          echo "g05_ensure: $_diag_o ← $_diag_thin_x + seed-rest (G-02f-342 L2 hybrid diag thin)"
          _diag_done=1
        else
