...
if [ "${G05_SKIP_HOT_REBUILD:-}" != "1" ]; then
  ...
  # 7.2.1 P3b Route C: type_ref .x bodies (kind / dyn / builtin TypeKind / vector ident)
      ...
      if [ "${XLANG_G05_PREFER_X_O:-1}" = "1" ] && { [ -f "$_pthin_p1_seed" ] || [ -f "$_pthin_p2_seed" ] || [ -f "$_pthin_p3_seed" ] || [ -f "$_pthin_p4p_seed" ] || [ -f "$_pthin_p4u_seed" ] || [ -f "$_pthin_p4b_seed" ] || [ -f "$_pthin_p4as_seed" ] || [ -f "$_pthin_p4t_seed" ] || [ -f "$_pthin_p5_seed" ] || [ -f "$_pthin_p6_seed" ] || [ -f "$_pthin_p7_seed" ] || [ -f "$_pthin_p9_seed" ] || [ -f "$_pthin_p10_seed" ] || [ -f "$_pthin_p11_seed" ] || [ -f "$_pthin_p12_seed" ] || [ -f "$_pthin_p13_seed" ] || [ -f "$_pthin_p14_seed" ] || [ -f "$_pthin_p15_seed" ] || [ -f "$_pthin_p16_seed" ] || [ -f "$_pthin_p17_seed" ] || [ -f "$_pthin_p18_seed" ] || [ -f "$_pthin_p19_seed" ] || [ -f "$_pthin_p20_seed" ]; }; then
        ...
        if [ -n "$_pthin_p3b_thin_o" ] && [ -f "$_pthin_p3b_x" ]; then
          if G05_X_O_WEAK=1 g05_try_x_to_o "$_pthin_p3b_x" "$_pthin_p3b_thin_o"; then
            ...
            echo "g05_ensure: P3b type_ref bodies ← $_pthin_p3b_x (7.2.1 Route C)"
          else
        ...
        if [ -n "$_pthin_p4p_o" ] && [ -f "$_pthin_p4p_seed" ]; then
          # shellcheck disable=SC2086
          if $CC $BASE_CFLAGS -I. -Iinclude -Isrc -Isrc/lexer -Isrc/asm -Iseeds/parser_asm \
               $_pthin_p4p_extra -c -o "$_pthin_p4p_o" "$_pthin_p4p_seed"; then
            _pthin_p4p_ok=1
        ...
        # PLATFORM: SHARED — 7.2.1 P5b Route C (2026-09-13).
        # pthin_ctrl.x holds comment-aware brace skip / kw_at_pos. Runs
        # before P5 C so BODIES_FROM_X skips the portable .inc region.
        # No lexer-step bridge. Cold: no define, full .inc.
        _pthin_p5_extra=""
        if [ -n "$_pthin_p5b_thin_o" ] && [ -f "$_pthin_p5b_x" ]; then
        ...
        # pthin_lex_skip.x holds kind predicates, buf copies, and B-minus
        # skip_balanced/generic_into (peek+step via P9a bridge). Runs after
        # P9a so BODIES_FROM_X is only set when the bridge will be linked
        # (otherwise skip_balanced would UNDEF). Cold: no define, full .inc.
        _pthin_p1_extra=""
        if [ "$_pthin_p9a_ok" = "1" ] && [ -n "$_pthin_p1b_thin_o" ] && [ -f "$_pthin_p1b_x" ]; then
          if G05_X_O_WEAK=1 g05_try_x_to_o "$_pthin_p1b_x" "$_pthin_p1b_thin_o"; then
            ...
            echo "g05_ensure: P1b lex_skip bodies ← $_pthin_p1b_x (7.2.1 Route C + B-minus)"
          else
        ...
        # pthin_skip_tl.x holds skip_one_struct/enum/extern walks.
        # Requires P9a lexer-step bridge AND P1b skip_balanced /
        # skip_generic_angle (otherwise those would UNDEF). Runs before
        # P12 C so BODIES_FROM_X skips the portable .inc region. Cold: no
        # define, full .inc. Do not reuse XLANG_PTHIN_SKIP_TL_FROM_X.
        _pthin_p12_extra=""
        if [ "$_pthin_p9a_ok" = "1" ] && [ "$_pthin_p1b_ok" = "1" ] \
          ...
          if G05_X_O_WEAK=1 g05_try_x_to_o "$_pthin_p12b_x" "$_pthin_p12b_thin_o"; then
            ...
            echo "g05_ensure: P12b skip_tl bodies ← $_pthin_p12b_x (7.2.1 B-minus)"
          else
        ...
        # skip_one_top_level let/const. Requires P9a lexer-step bridge AND
        # P14b skip_one_if_statement (otherwise that would UNDEF). Runs
        # before P18 C so BODIES_FROM_X skips the portable .inc region.
        # Cold: no define, full .inc. Do not reuse XLANG_PTHIN_BODY_TL_FROM_X.
        _pthin_p18_extra=""
        if [ "$_pthin_p9a_ok" = "1" ] && [ "$_pthin_p14b_ok" = "1" ] \
          ...
          if G05_X_O_WEAK=1 g05_try_x_to_o "$_pthin_p18b_x" "$_pthin_p18b_thin_o"; then
            ...
            echo "g05_ensure: P18b body_tl bodies ← $_pthin_p18b_x (7.2.1 Route C + B-minus)"
          else
        ...
        rm -f "$_pthin_p1_o" "$_pthin_p1b_thin_o" "$_pthin_p2_o" "$_pthin_p3_o" "$_pthin_p3b_thin_o" "$_pthin_p4p_o" "$_pthin_p4pb_thin_o" "$_pthin_p4u_o" "$_pthin_p4ub_thin_o" "$_pthin_p4b_o" "$_pthin_p4bb_thin_o" "$_pthin_p4as_o" "$_pthin_p4t_o" "$_pthin_p5_o" "$_pthin_p5b_thin_o" "$_pthin_p6_o" "$_pthin_p7_o" "$_pthin_p7b_thin_o" "$_pthin_p9_o" "$_pthin_p9a_thin_o" "$_pthin_p9a_o" "$_pthin_p9b_thin_o" "$_pthin_p10_o" "$_pthin_p10b_thin_o" "$_pthin_p11_o" "$_pthin_p11b_thin_o" "$_pthin_p12_o" "$_pthin_p12b_thin_o" "$_pthin_p13_o" "$_pthin_p13b_thin_o" "$_pthin_p14_o" "$_pthin_p14b_thin_o" "$_pthin_p15_o" "$_pthin_p16_o" "$_pthin_p17_o" "$_pthin_p18_o" "$_pthin_p18b_thin_o" "$_pthin_p19_o" "$_pthin_p19b_thin_o" "$_pthin_p20_o" "$_pthin_rest_o"
      fi
      if [ "$_pthin_done" = "0" ]; then
