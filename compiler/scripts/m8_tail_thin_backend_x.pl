#!/usr/bin/env perl
# M8-tail：将 backend.x 大 emit 函数体替换为单行 C/partial 薄包装（恢复自举 second pass）。
use strict;
use warnings;
use utf8;

my $path = shift @ARGV // 'src/asm/backend.x';
...
replace_func('compute_frame_size', <<'X');
...
replace_func('fill_param_slots', <<'X');
...
replace_func('fill_local_slots', <<'X');
...
replace_func('asm_emit_call_args_elf', <<'X');
...
replace_func('emit_if_then_block_body_text', <<'X');
...
replace_func('emit_if_then_block_body_elf', <<'X');
...
replace_func('emit_expr_method_call', <<'X');
...
replace_func('emit_expr_elf_method_call', <<'X');
...
replace_func('emit_expr_elf_call', <<'X');
...
replace_func('emit_block_inits_elf', <<'X');
...
replace_func('emit_while_loop_elf', <<'X');
...
replace_func('emit_for_loop_elf', <<'X');
...
replace_func('emit_block_inits', <<'X');
...
replace_func('emit_next_label', <<'X');
...
replace_func('format_label_id', <<'X');
...
replace_func('emit_while_loop', <<'X');
...
replace_func('emit_block_body', <<'X');
...
print {$out} $src;
close $out;
print "m8_tail_thin_backend_x.pl: OK $path\n";
