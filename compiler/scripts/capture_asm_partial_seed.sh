...
#   2) bootstrap_xlangc / xlang-seed-phase1 + build_seed_asm_host（须 Linux x86_64，大内存）
#
# 用法（compiler 目录）：
#   ./scripts/capture_asm_partial_seed.sh
#
# 环境：XLANG_E 默认 ./bootstrap_xlangc；Docker 建议 XLANG_DOCKER_MEMORY=16g
...
if [ -x ./xlang-seed-phase1 ]; then
  XLANG_E=./xlang-seed-phase1
elif [ ! -x "$XLANG_E" ] && [ -x ./xlang-c ]; then
  XLANG_E=./xlang-c
fi

if [ ! -x "$XLANG_E" ]; then
  cp -f seeds/bootstrap_xlangc.linux.x86_64 bootstrap_xlangc 2>/dev/null || true
  chmod +x bootstrap_xlangc 2>/dev/null || true
  XLANG_E=./bootstrap_xlangc
fi

if [ ! -x "$XLANG_E" ]; then
  echo "capture_asm_partial: FAIL: no XLANG_E" >&2
  exit 1
fi
...
XLANG_E="$XLANG_E" ./scripts/build_seed_asm_host.sh

if [ ! -s "$PARTIAL" ]; then
  echo "capture_asm_partial: FAIL: no $PARTIAL (try XLANG_DOCKER_MEMORY=16g)" >&2
  exit 1
fi
