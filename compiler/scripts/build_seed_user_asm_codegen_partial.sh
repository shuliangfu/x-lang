#!/bin/sh
# build_seed_user_asm_codegen_partial.sh — 从 gen_driver/pipeline_x.o 抽出 asm 四入口，供 seed xlang 链入（无需 cp xlang_asm）
# 用法：在 compiler 目录执行；须先有 build_asm/gen_driver/pipeline_x.o（含 backend_asm_codegen_ast T 符号）。
# 通常由 XLANG=./xlang ./scripts/build_xlang_asm.sh 一次产出；或 xlang-c -E pipeline.x 编出大 pipeline_x.o 后复制到 gen_driver/。
...
if [ ! -f "$GEN_SUO" ]; then
  echo "build_seed_user_asm_codegen_partial: 缺少 $GEN_SUO（先运行 build_xlang_asm 或 make gen-x-driver-objs 并复制大 pipeline_x.o）" >&2
  exit 1
fi

if ! nm "$GEN_SUO" 2>/dev/null | grep -q ' T _backend_asm_codegen_ast'; then
  echo "build_seed_user_asm_codegen_partial: $GEN_SUO 无 T _backend_asm_codegen_ast（当前为 -E-extern 瘦 pipeline，须用 xlang-c -E 全量 pipeline 或 build_xlang_asm）" >&2
  exit 1
fi
