#!/usr/bin/env python3
"""
审计 k_asm_parser_thin_delegate 表：sx/c 名长度、parser.sx 是否存在对应 function。
...
def main() -> int:
    ...
    bad = 0
    for sx, sl, cg, cl in entries:
        if int(sl) != len(sx):
            print(f"BAD sx_len: {sx} table={sl} actual={len(sx)}")
            bad += 1
        if int(cl) != len(cg):
            print(f"BAD c_len: {cg} table={cl} actual={len(cg)}")
            bad += 1
        if sx not in funcs:
            print(f"MISSING func: {sx}")
            bad += 1
