...
ensure_asm_driver_seed_c_objs() {
  ...
  if [ ! -f "$SEED_O/preprocess.o" ] || [ "src/preprocess.c" -nt "$SEED_O/preprocess.o" ]; then
    ...
    "$CC" $CFLAGS -DSHU_USE_SU_PREPROCESS -c -o "$SEED_O/preprocess.o" src/preprocess.c
  fi
  ...
  if [ ! -f "$SEED_O/ast_seed.o" ] || [ "src/ast/ast.c" -nt "$SEED_O/ast_seed.o" ]; then
    ...
    "$CC" $CFLAGS -DSHU_USE_SU_AST -c -o "$SEED_O/ast_seed.o" src/ast/ast.c
  fi
...
# ast_pool.c / pipeline_glue.c 变更后须重编 pipeline_su.o（glue 在 pipeline_gen.c #include 内）。
ensure_pipeline_su_fresh_for_ast_pool() {
  if [ -f ast_pool.c ] && { [ ! -f pipeline_su.o ] || [ ast_pool.c -nt pipeline_su.o ]; }; then
    echo "relink_experimental_bootstrap: ast_pool.c newer than pipeline_su.o — rebuild ..."
    if command -v make >/dev/null 2>&1 && [ -f Makefile ]; then
      make -s pipeline_su.o PIPELINE_SU_FORCE_COMPILE=1 || {
        echo "relink_experimental_bootstrap: warn: pipeline_su.o rebuild failed" >&2
        return 1
      }
    fi
  fi
  if [ -f pipeline_glue.c ] && { [ ! -f pipeline_su.o ] || [ pipeline_glue.c -nt pipeline_su.o ]; }; then
    echo "relink_experimental_bootstrap: pipeline_glue.c newer than pipeline_su.o — rebuild ..."
    if command -v make >/dev/null 2>&1 && [ -f Makefile ]; then
      make -s pipeline_su.o PIPELINE_SU_FORCE_COMPILE=1 || {
        echo "relink_experimental_bootstrap: warn: pipeline_su.o rebuild failed" >&2
        return 1
...
# build_asm 伴生 .o（experimental 链与 build_shu_asm ensure_asm_bootstrap_su_companion_objs 对齐）。
ensure_experimental_companion_objs() {
  ...
  if [ ! -f "$BUILD_DIR/su_seed_bridge.o" ] || [ "src/su_seed_bridge.c" -nt "$BUILD_DIR/su_seed_bridge.o" ]; then
    echo "relink_experimental_bootstrap: cc su_seed_bridge.o"
...
# parser_su.o：Makefile 要求 parser_copy_module_import_path64 在 gen 内；experimental 链由 link_alias+thin_glue 提供。
ensure_parser_su_obj() {
  if [ -f parser_su.o ]; then
    return 0
  fi
  if command -v make >/dev/null 2>&1 && [ -f Makefile ]; then
    make -s parser_su.o 2>/dev/null || true
  fi
  if [ ! -f parser_su.o ] && [ -f parser_gen.c ]; then
    echo "relink_experimental_bootstrap: cc parser_gen.c -> parser_su.o (link_alias supplies parser_* pipeline symbols)"
    "$CC" $CFLAGS $(make -s -f Makefile print-PIPELINE_GEN_CFLAGS 2>/dev/null || echo "") \
      ...
      -c parser_gen.c -o parser_su.o
  fi
  if [ ! -f parser_su.o ]; then
    echo "relink_experimental_bootstrap: missing parser_su.o (make parser_su.o or restore parser_gen.c)" >&2
    exit 1
  fi
}
ensure_parser_su_obj

# 瘦 parser_su.o 无 parse_into_buf：默认 cc seed slice TU；SU PARSE_BOOTSTRAP_EMIT 仍 139，仅 opt-in 探测。
ensure_parser_parse_bootstrap_asm_obj() {
  ...
  try_parser_parse_bootstrap_su_emit() {
    ...
    if [ -x ./shu_asm2_refreshed ] && file ./shu_asm2_refreshed 2>/dev/null | grep -q "ELF.*x86-64"; then
      _min="/tmp/shu_seed_parse_probe.$$.su"
      printf 'function main(): i32 { return 0; }\n' > "$_min"
      if ! env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 ./shu -backend asm -o /tmp/shu_seed_probe.$$.o \
          $PBOOT_LIBROOT "$_min" 2>/dev/null || [ ! -s /tmp/shu_seed_probe.$$.o ]; then
        SHU_SEED="./shu_asm2_refreshed"
        echo "relink_experimental_bootstrap: ./shu parse probe failed — bootstrap seed=$SHU_SEED" >&2
      fi
      rm -f "$_min" /tmp/shu_seed_probe.$$.o 2>/dev/null || true
    fi
    if [ ! -x "$SHU_SEED" ]; then
      return 1
    fi
    if [ -f ast_pool.c ] && { [ ! -f pipeline_su.o ] || [ ast_pool.c -nt pipeline_su.o ]; }; then
      echo "relink_experimental_bootstrap: ast_pool.c newer — rebuild pipeline_su.o for parse bootstrap ..."
      if command -v make >/dev/null 2>&1 && [ -f Makefile ]; then
        make -s pipeline_su.o PIPELINE_SU_FORCE_COMPILE=1 || true
      fi
    fi
    if [ -f pipeline_su.o ] && [ pipeline_su.o -nt "$SHU_SEED" ] 2>/dev/null; then
      echo "relink_experimental_bootstrap: refresh ./shu (pipeline_su.o for parse bootstrap emit) ..."
      if command -v make >/dev/null 2>&1 && [ -f Makefile ]; then
        make -s relink-shu 2>/dev/null || true
      fi
    fi
    echo "relink_experimental_bootstrap: $SHU_SEED asm parser parse bootstrap (SHU_ASM_PARSER_PARSE_BOOTSTRAP_EMIT opt-in) ..."
    ulimit -s 65532 2>/dev/null || ulimit -s hard 2>/dev/null || true
    if ! env -u SHU_ASM_START_FUNC SHU_ASM_PARSER_PARSE_BOOTSTRAP_EMIT=1 \
        SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_WPO_DCE=0 \
        "$SHU_SEED" -backend asm -o "$PARSER_PARSE_BOOT_O" $PBOOT_LIBROOT src/parser/parser.su \
  ...
  if [ "$need_rebuild" = "1" ]; then
    if [ "${SHU_ASM_PARSER_PARSE_BOOTSTRAP_SU_EMIT:-0}" = "1" ]; then
  ...
  # parser_su.o 已真 emit parse_into* 时勿再链 bootstrap .o（否则 ld multiple definition）。
  if [ -f parser_su.o ] && nm parser_su.o 2>/dev/null | grep -qE '[[:space:]]T parser_parse_into_buf$'; then
    PARSER_PARSE_BOOT_O=""
  fi
}
ensure_parser_parse_bootstrap_asm_obj

for o in pipeline_su.o pipeline_bootstrap_orchestration.o preprocess_su.o lexer_su.o typeck_su.o codegen_su.o \
  lexer_su_link_alias.o typeck_su_link_alias.o codegen_su_link_alias.o \
...
# seed parser/typeck/codegen/lexer 供 runtime/lsp_diag 链接；parser_su/typeck_su 须链在末尾压过重复 glue（build_shu_asm ST_PARSER_SU_TAIL）。
...
ensure_experimental_lsp_objs() {
  ...
  mkdir -p "$GEN_DIR"
  if [ ! -f Makefile ] || ! command -v make >/dev/null 2>&1; then
    echo "relink_experimental_bootstrap: warn: cannot make lsp_su.o" >&2
    return 1
  fi
  make -s lsp_io_gen.c lsp_gen.c lsp_diag_gen.c lsp_io_std_heap_gen.c \
    lsp_su.o lsp_io_su.o lsp_diag_su.o lsp_io_std_heap_su.o
  cp -f lsp_su.o lsp_io_su.o lsp_diag_su.o lsp_io_std_heap_su.o "$GEN_DIR/"
}
ensure_experimental_lsp_objs || true
ST_LSP_SU=""
if [ -f "$BUILD_DIR/gen_driver/lsp_su.o" ]; then
  ST_LSP_SU="$BUILD_DIR/gen_driver/lsp_su.o $BUILD_DIR/gen_driver/lsp_io_su.o $BUILD_DIR/gen_driver/lsp_io_std_heap_su.o $BUILD_DIR/gen_driver/lsp_diag_su.o"
fi
