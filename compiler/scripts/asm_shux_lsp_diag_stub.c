 ...
 *
 * lsp_sx.o 引用若干 LSP 响应构建符号；整份 lsp_diag_sx.o 与 pipeline_sx.o 重复 ast 符号，不能并列链入。
 * 本文件提供最小桩，使 shux_asm 可链接；完整 LSP 仍用 bootstrap-driver-seed 的 shux / shux-sx。
 */
#include <stdint.h>

/** 空 Diagnostic[] JSON 片段，供 lsp.sx 走通链接。 */
...
/** semanticTokens/full：返回空数组占位，避免依赖 lsp_diag_sx.o 的 typeck_* 实现。 */
...
/** lsp.sx 经 lsp_io 模块名 mangling 后的 invalidate；转发到已链入的 lsp_diag.o。 */
