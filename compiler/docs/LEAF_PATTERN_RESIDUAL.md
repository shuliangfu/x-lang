...
> - **wave922** · Windows MSYS2 min-gate reproof @ `39baf14b4` (tip drift from wave804 `bb8f07263` → reproof green; B-hybrid + win32 write/read gates OK; proof stamp RC=0; leaf apply idempotent `ALREADY=1` — STATUS stays `reproven_green`; **not physical delete**; **not code change wave**; non-leaf edit)
> - **open** · thin edges + mk lists hybrid · ✅ Windows reproof done (wave922) · → dual L4 (mac + Ubuntu true cold + 129 bstrict) → explicit auth ship delete  # post-wave922 Windows reproof
