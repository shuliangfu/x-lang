...
> - **wave907** · B7B ASYNC_THREE product edges multi-target FORCE thin try-heat (3 leaves → `$(ASYNC_THREE_SEED_OBJS)` in `mk/driver_seed_r_lists.mk`; G.7 有则补全 no second list)
...
> - **wave920** · B7B cp-alias family multi-target FORCE thin (3 leaves → 2 multi-target rules split by platform scope: `$(CP_ALIAS_SHARED_OBJS)` = `ast_x.o` (1 leaf; SHARED G-02a C ABI alias) + `$(CP_ALIAS_LINUX_X86_64_OBJS)` = `crt0_user.o` + `freestanding_io.o` (2 leaves; Linux x86_64 freestanding wrappers inside ifeq guard) — 2 new lists in `mk/driver_seed_r_lists.mk`; G.7 `@bash scripts/ensure_cp_alias_o.sh ensure $@` — script CATALOG owns OUT|SRC map; migrated from 3 per-leaf recipes; **cp-alias family closed**)
> - **wave921** · B7B class-G filter family multi-target FORCE thin (4 leaves → 2 multi-target rules split by script: `$(FILTER_AGAINST_PARTIAL_OBJS)` = 3 leaves (backend_x86_64_enc_c / user_asm_seed_bridge / asm_backend_compat_stubs) + `$(FILTER_PIPELINE_OBJS)` = 1 leaf (bootstrap_seed_pipeline) — 2 new lists in `mk/driver_seed_r_lists.mk`; G.7 `@bash scripts/filter_bootstrap_seed_{against_partial,pipeline}_o.sh ensure $@` — script CATALOG owns OUT|SRC map; migrated from 4 per-leaf recipes; **class-G filter family closed**)
> - **wave923** · dual-end L4 true cold green @ `81285129e` + async rule parse-order fix (wave907 `$(ASYNC_THREE_SEED_OBJS)` multi-target FORCE rule was BEFORE `include mk/driver_seed_r_lists.mk` → make silently dropped rule at parse time (empty var) → 3 async .o became "Not a target" → "No rule to make target" on true cold L4; fix: move rule block 17 lines to after include, alongside other 7 multi-target FORCE rules; mac L4 699s + Ubuntu L4 477s + 129 bstrict all green; **not physical delete**; **not pin bump**; code change wave)
> - **open** · thin edges + mk lists hybrid · ✅ Windows reproof done (wave922) · ✅ dual L4 done (wave923 @ `81285129e`) · → explicit auth ship delete (`phys-del-gate --delete-body`)  # post-wave923 dual L4 green
