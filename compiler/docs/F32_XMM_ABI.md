...
> **状态**：**默认开启**（`xlang_asm` x86_64 SysV）；显式 **`XLANG_ABI_F32_XMM=0`** 回落 legacy（f64 widen + `cvtsd2ss`）。  
...
| `XLANG_ABI_F32_XMM` | 行为 |
|-------------------|------|
| **unset**（默认） | **xmm ABI** |
| `1` | xmm ABI（与默认相同） |
| `0` | legacy：gp + f64 widen + callee `cvtsd2ss` |

**CLI**（与 `XLANG_ABI_F32_XMM=0` 等价，无需记环境变量）：
...
```bash
# 默认 xmm（无需 export）
XLANG=./compiler/xlang_asm ./compiler/xlang_asm -backend asm -L . app.x -o app

# 显式 legacy 回归（环境变量或 CLI）
XLANG_ABI_F32_XMM=0 XLANG=./compiler/xlang_asm ./compiler/xlang_asm -backend asm -L . app.x -o app
XLANG=./compiler/xlang_asm ./compiler/xlang_asm -backend asm -L . -legacy-f32-abi app.x -o app
```

`pipeline_asm_abi_f32_xmm_enabled_c()`（`compiler/src/asm/pipeline_abi_f32_xmm.c`）读 `getenv("XLANG_ABI_F32_XMM")`：**仅 `"0"` 关闭**。

### 2.2 构建 `xlang_asm`（链接 dispatch TU）
...
./scripts/relink_xlang_asm_experimental_bootstrap.sh && cp xlang_asm.experimental xlang_asm
```
...
| 角色 | legacy（`XLANG_ABI_F32_XMM=0`） | xmm ABI（**默认**） |
|------|-------------------------------|---------------------|
...
**P5 / Docker / GHA**：`run-f32-xmm-gates.sh` 统一覆盖 xmm + CLI legacy；**不再**单独跑 `XLANG_ABI_F32_XMM=0` 全量 dod-s2。
...
| 阶段 | 目标日期 | 行为 |
|------|----------|------|
| **当前** | 2026 Q2 | release **`xlang_asm -backend asm -O2`**；**f32 xmm ABI 默认 ON** |
| **警告期** | 2026 Q4 | 文档与 `--help` 标明 legacy 仅 `-legacy-f32-abi` / `XLANG_ABI_F32_XMM=0`；CI 仅 CLI 烟测 |
| **移除计划** | 2027 Q1 | 删除 legacy f64 widen + `cvtsd2ss` 路径（breaking） |

**Release 推荐命令**：

```bash
xlang_asm -backend asm -O2 -L . app.x -o app
```
