...
> **状态**：**默认开启**（`shu_asm` x86_64 SysV）；显式 **`SHU_ABI_F32_XMM=0`** 回落 legacy（f64 widen + `cvtsd2ss`）。  
...
**CLI**（与 `SHU_ABI_F32_XMM=0` 等价，无需记环境变量）：
...
```bash
# 默认 xmm（无需 export）
SHU=./compiler/shu_asm ./compiler/shu_asm -backend asm -L . app.su -o app

# 显式 legacy 回归（环境变量或 CLI）
SHU_ABI_F32_XMM=0 SHU=./compiler/shu_asm ./compiler/shu_asm -backend asm -L . app.su -o app
SHU=./compiler/shu_asm ./compiler/shu_asm -backend asm -L . -legacy-f32-abi app.su -o app
```

`pipeline_asm_abi_f32_xmm_enabled_c()`（`compiler/src/asm/pipeline_abi_f32_xmm.c`）读 `getenv("SHU_ABI_F32_XMM")`：**仅 `"0"` 关闭**。

### 2.2 构建 `shu_asm`（链接 dispatch TU）
...
| `pipeline_glue.c`（在 `pipeline_su.o` 内） | callee xmm homing、f32 load、struct layout |

重建示例（Linux Docker / amd64）：

```bash
cd compiler
make pipeline_su.o PIPELINE_SU_FORCE_COMPILE=1   # pipeline_glue 变更后
cc -c -o src/asm/pipeline_abi_f32_xmm.o src/asm/pipeline_abi_f32_xmm.c   # 开关变更后
./scripts/relink_shu_asm_experimental_bootstrap.sh && cp shu_asm.experimental shu_asm
```

**勿**在已链 `pipeline_su.o`（含 glue）的 experimental 链上再单独链 `pipeline_abi_f32_xmm.o`（duplicate symbol）。

---

## 3. 语义摘要

| 角色 | legacy（`SHU_ABI_F32_XMM=0`） | xmm ABI（**默认**） |
|------|-------------------------------|---------------------|
...
**P5 / Docker / GHA**：`run-f32-xmm-gates.sh` 统一覆盖 xmm + CLI legacy；**不再**单独跑 `SHU_ABI_F32_XMM=0` 全量 dod-s2。
...
| 阶段 | 目标日期 | 行为 |
|------|----------|------|
| **当前** | 2026 Q2 | release **`shu_asm -backend asm -O2`**；**f32 xmm ABI 默认 ON** |
| **警告期** | 2026 Q4 | 文档与 `--help` 标明 legacy 仅 `-legacy-f32-abi` / `SHU_ABI_F32_XMM=0`；CI 仅 CLI 烟测 |
| **移除计划** | 2027 Q1 | 删除 legacy f64 widen + `cvtsd2ss` 路径（breaking） |

**Release 推荐命令**：

```bash
shu_asm -backend asm -O2 -L . app.su -o app
```
