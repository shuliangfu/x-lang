...
run-hello 要成功
  ...
  → 入口必须在 pipeline 内用 .x 解析（否则 C 的 AST 与 .x 的 Module 布局不一致，.x 读到的 num_funcs 也是 0）
...
- hello.x 是 `function main() { let msg = ...; return if (n>=0) 0 else 1; }`，不是单表达式 return。
...
**已确认**：在 driver_run_compiler_full 中，C 侧先调 parser_parse_into 再进 pipeline；诊断显示「after C parse num_funcs=0」，即 **C 解析器对 hello.x 同样未把 main 加入 module**（与 .x 解析器同源、同 bug）。因此「复用 C 解析、entry_already_parsed=1」无法解决 run-hello，必须修解析/codegen 根因。
...
| 修改 | 作用 | 位置 |
|------|------|------|
| 入口在 pipeline 内解析 | 保证 module 布局一致，避免 C 解析 + .x 使用混用 | runtime: memset(arena,0); memset(module,0); 不设 entry_already_parsed=1 |
| parse_into_with_init_buf(data, len) | 用 (data,len) 进 parser，避免 C 与 .x 的 u8[] 布局问题 | pipeline.x |
| codegen 跳过 is_extern 函数 | 不生成 io_read(void) 等定义，避免与 preamble 的 extern 冲突 | codegen.x: if (f.is_extern != 0) continue |
...
2. **parser.x**：`return if ( cond ) { int } else { int }` 解析中，当条件非 true/false 走 `skip_balanced_parens` 后，下一 token 已是 `{` 不再消费 `)`，改为「仅当 r.tok == TOKEN_RPAREN 时才消费 )」，避免 hello.x 的 `return if (n>=0){0}else{1}` 失败。
3. **parser.x**：**parse_into_buf** 在从 onefunc 加入函数时，与 parse_into 一致：先分配 block、用 res 填充 const/let（含 **let 名称**）、loops/for、stmt_order、final_expr_ref，再 `ast_arena_block_set`，并设置 `f.body_ref = block_ref`、`f.body_expr_ref = 0`，保证 codegen 使用带 let 名的 block。
...
- 不再在 parser 里加 `parser_slice_from_buf_into`、不在 .x 里拼 slice 字面量、不再改 C 与 .x 的 Module 布局、不再为 run-hello 加更多特殊分支。
