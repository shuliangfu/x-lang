...
#define ast_pipeline_run_su_pipeline_impl pipeline_run_su_pipeline_impl
...
int32_t main_driver_argv_parse_su_path(int32_t argc, uint8_t * argv, struct main_DriverSuEmitState * state) {
  ...
  (void)(({ int32_t __tmp = 0; if (vlen >= 0 && vlen == 1 && (arg_buf)[0] == 99) {   (void)(((state)->use_asm_backend = 0));
 } else (__tmp = 0) ; __tmp; }));
...
int32_t main_driver_run_su_emit_su(struct main_DriverSuEmitState * state) {
  ...
  int32_t out_len = preprocess_su_buf(loaded_buf, n, preprocess_buf, 4194304);
 ...
 } else {   ptrdiff_t written = std_fs_fs_write(fd, (out).data, ((size_t)(len)));
  (void)(std_fs_fs_close(fd));
 ...
 } else {   ptrdiff_t written = std_fs_fs_write(1, (out).data, ((size_t)(len)));
  __tmp = ({ int32_t __tmp = 0; if (written < 0 || ((int32_t)(written)) != len) {   return 1;
 } else (__tmp = 0) ; __tmp; });
 } ; __tmp; });
 } ; __tmp; }));
...
int32_t main_cmd_run(int32_t argc, uint8_t * argv) {
  ...
  int32_t rc = main_run_compiler_su_path_impl(argc, argv);
...
int32_t main_entry(int32_t argc, uint8_t * argv) {
  ...
  while (k < n_roots) {
    ...
    (void)(driver_run_su_emit_c_set_lib(k, (&((lib_tmp)[0])), llen));
    (void)((k = k + 1));
  }
  (void)(driver_run_su_emit_c_set_n_lib_roots(n_roots));
