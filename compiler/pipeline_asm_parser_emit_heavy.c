...
/* --- relocated from pipeline_asm_thin_delegate.c (8.3.2 thin_delegate pure-owned leave) ---
 * AsmBackendThinDelegateRow + k_asm_parser_thin_delegate co-located with their only 2 consumers
 * (asm_parser_func_is_thin_delegate / asm_parser_m8_tail_thin_delegate_c_name, below).
 * Same host-cc mega-TU (ast_pool.c includes both); static visibility unchanged. PLATFORM: SHARED. */
typedef struct {
  const char *x_name;
  int32_t x_len;
  const char *c_name;
