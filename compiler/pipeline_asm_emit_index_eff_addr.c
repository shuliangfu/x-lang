 ...
 * bounds-guard or rax+rbx*esz path. try_index forest stays in
 * pipeline_asm_emit_index_helpers.c; face emitters stay in
 * pipeline_asm_emit_index.c / assign. Older text/ELF base twins
 * (glue_emit_index_eff_addr_base_*) remain later in glue until a later wave.
 *
 * Callers: pipeline_asm_emit_index.c; assign INDEX lhs; index_helpers try_index;
 * spill index_assign residual (via forward decl); pipeline_asm_emit_index_eff_addr_elf_c
 * (add_index_to_base).
 *
 * Not compiled as a separate .o — #included from pipeline_glue.c in place of
 * the former residual body (after binop load helpers; before lit_i32 + expr_rec).
 *
 ...
 * - pipeline_asm_emit_expr_elf_rec / pipeline_asm_expr_lit_i32_at_c (later glue)
