/* PLATFORM: WINDOWS cold archaeology (restored post-wave309 for seeds/pipeline_glue_standalone.from_x.c). */
/**
 ...
 * spill.c (glue_loop_break_exit_push/pop + glue_asm_cache_invalidate_
 * at_cfg_merge_selective + glue_asm_loop_phi (wave159 pure) +
 * glue_asm_loop_merge_live_union (wave160 pure) +
 ...
 * glue_asm73_evict_cache_if_live_pressure* (wave166 pure));
 ...
 * sits after typeck_fwd (assign #include retired wave256); region_assign
 * #include retired wave257.
 * PLATFORM: SHARED. */
 ...
 * live on typeck_x.o only (#[no_mangle] thin → typeck_check_*).
