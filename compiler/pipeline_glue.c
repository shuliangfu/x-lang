 ...
 *
 * parser.sx 聚合 -o 可执行时，runtime 在含入前 #define SHUX_PARSER_EXE_PIPELINE_GLUE：省略依赖
 * platform_elf_ElfCodegenCtx 完整定义与 codegen.o 转发符号的片段，避免单文件 TU 编译失败。
 *
 * ── extern 消费者索引（10.4.2 自举；删函数前 grep 本表）────────────────────────────
 * | 符号 | 消费者 |
 * |------|--------|
 * | parser_slice_from_buf / lexer_parser_slice_from_buf / pipeline_source_slice | parser.sx, lexer.sx, pipeline.sx |
 * | parser_lex_from_lexer_result_ptr_into | parser.sx（*LexerResult → *Lexer，避 typeck 链式 FIELD_ACCESS） |
 * | pipeline_run_sx_pipeline | runtime.c（C 包装 buf+len） |
 * | pipeline_sizeof_* / pipeline_arena_offset_num_types | parser.sx, lsp_diag.sx；**PipelineDepCtx 增字段时须同步** runtime.c / lsp_diag_pipeline_sizes.c / ast.sx |
 * | pipeline_expr_ref_is_assign_lvalue | parser.sx |
 * | compound_assign_token_to_expr_kind_from_glue | parser.sx |
 * | pipeline_expr_* / ast_pipeline_expr_* / implicit_tail_expr_disallowed_by_glue | ast.sx, typeck.sx |
 * | pipeline_type_* / pipeline_module_struct_layout_* | typeck.sx, codegen.sx |
 * | pipeline_module_func_* / pipeline_arena_func_* | parser.sx |
 * | pipeline_asm_array_lit_elem_type_ref | asm/backend.sx |
 * | pipeline_asm_cmp_cc_for_expr_kind_ord / pipeline_asm_arm64_cset_cond_enc_from_cc | asm/backend.sx, arch/arm64_enc.sx |
 * | pipeline_module_func_is_extern_at / pipeline_module_func_body_ref_at / pipeline_module_func_name_len_at | asm/backend.sx, arch/arm64.sx |
 * | pipeline_backend_get_return_expr_ref_at / pipeline_arm64_get_return_lit_ref_at | asm/backend.sx, arch/arm64.sx |
 ...
 * | driver_diagnostic_entry_* | pipeline.sx（日常 no-op） |
 * 原因：SU/asm 对大 struct 按值读写字段或 enum 比较易撕裂/ typeck 失败，暂保留 C 指针读池。
 */

/* struct shux_slice_uint8_t 已由 -E 产出的 pipeline_gen.c 上方定义，不在此重复。 */
#include <stddef.h>
#include <string.h>
#include <stdlib.h>
#include <stdint.h>
...
/** 当前 asm emit 函数下标；供形参 *T 槽 load/leа 判定（driver compile.sx state 等）。 */
...
extern int32_t typeck_sx_type_align_from_layout_glue(struct ast_Module *module, struct ast_ASTArena *arena,
                                                     int32_t li, int32_t depth);
...
/** pipeline.sx：PipelineDepCtx / CodegenOutBuf 字段 glue（ast_pool.c）。 */
...
/** parser.sx：usize 起点回溯；asm 自 typecheck 对 usize/i32 混算失败，保留薄 C sidecar。 */
...
/**
 * lexer.sx 内的 extern：Codegen 会带 lexer_ 模块前缀，否则会链接到不存在的 lexer_parser_slice_from_buf。
...
/**
 * parser.sx：从 *LexerResult 拷贝 next_lex 三字段到 *Lexer。
...
/** 与 parser.sx ExternParseResult 布局一致；glue TU 内局部 typedef，勿与 -E 生成符号混用。 */
typedef struct {
...
/** *ExternParseResult.next_lex → *Lexer；parse_one_extern_and_add_into SU emit 勿写 lex_out.pos=…（kind=28）。 */
...
/** 将 extern 解析失败快照写入 out（name_len=-1）；SU `out.name[ni]=` INDEX ASSIGN 会 asm fail。 */
...
/** *LibraryParseResult 从栈上临时结果拷贝；避免 SU out.name[nli]=res.name[nli] INDEX ASSIGN。 */
...
/** *TrySkipAllowResult 从栈上临时结果拷贝；避免 SU out.lex.pos= / out._pad[i]= INDEX ASSIGN。 */
...
/** TrySkipAllowResult.lex → *Lexer（按值参）；lex_from_try_skip_into SU emit 勿 out.pos=。 */
...
/** LibraryParseResult.next_lex → *Lexer（按值参）；lex_from_library_into SU emit 勿 out.pos=。 */
...
/** 与 parser.sx OneFuncResult（allow(padding)）布局一致；仅 glue 内使用。 */
typedef struct {
  int32_t ok;
  struct lexer_Lexer next_lex;
  uint8_t name[64];
...
/** 写失败 OneFuncResult（等价 set_onefunc_fail；force_stub 勿 SU emit 本函数体）。 */
...
/** parse_one_function_buf_into 成功路径：写 ok/next_lex/name/return_val，勿 SU name[] INDEX ASSIGN。 */
...
/** 供 pipeline.sx run_sx_pipeline_impl 使用：与 parser_slice_from_buf 相同，避免 -E 对 parser_* 符号双重前缀。 */
...
extern int32_t pipeline_run_sx_pipeline_impl(struct ast_Module *module, struct ast_ASTArena *arena, uint8_t *source_data, size_t source_len, struct codegen_CodegenOutBuf *out_buf, struct ast_PipelineDepCtx *ctx);

int32_t pipeline_run_sx_pipeline(struct ast_Module *module, struct ast_ASTArena *arena, const uint8_t *source_data, size_t source_len, struct codegen_CodegenOutBuf *out_buf, struct ast_PipelineDepCtx *ctx) {
  return pipeline_run_sx_pipeline_impl(module, arena, (uint8_t *)source_data, source_len, out_buf, ctx);
}
...
/** LSP / lsp_diag.sx：PipelineDepCtx 体量大（含 4MiB×2 缓冲），一次性 calloc 后 memset 复用。 */
...
/** 供 runtime.c 烟测摘要：解析后 module.main_func_index（无 main 为 -1，与 pipeline.sx parse_into_set_main_index 一致）。 */
int driver_get_module_main_func_index(void *m) {
  return m ? (int)((struct ast_Module *)m)->main_func_index : -2;
}

/** 诊断：解析 entry 后打印 main 的 body_ref；日常构建关闭，排查 main 空体时取消下方注释。由 pipeline.sx 在 parse 后 / codegen 前调用。 */
...
/** pipeline.sx -E 产出 typeck_ 前缀；实现于 runtime.c（pipeline_sx.o 编译期须可见声明）。 */
...
/** pipeline.sx 经 typeck_ 前缀调用；实现于 runtime.c。 */
...
/** typeck.sx / pipeline.sx：入口源码长度（runtime.c）。 */
...
/** build_shux_asm：SHUX_ASM_BUILD_SKIP_TYPECK=1 时 pipeline 跳过 .sx typeck。 */
...
/** asm -o：C typeck 预检已通过时跳过重复 .sx typeck（见 driver_run_asm_backend）。 */
int32_t pipeline_driver_su_pipeline_skip_typeck(void) {
  return driver_su_pipeline_skip_typeck_get();
}
...
/**
 * parser.sx expr_ref_is_assign_lvalue：与 shux-c 生成码对 struct 直读一致。
 * SU 中对「let e: Expr = ast_arena_expr_get(...)」再写 e.kind == … 会在 .sx typeck 路径失败（见 ast.sx 注释）；由 C 从池读 kind / is_enum_variant。
 ...
 */
/** ast.sx：池槽 CALL 解析占位 -1（避免 SU 对 Expr 尾字段 FIELD_ACCESS 缺 layout）。 */
...
/** ast.sx / typeck.sx：typeck 命中后写入 call_resolved_*。 */
...
/** parser.sx expr_set_common_zeros：对堆/池上 Expr 指针写 call_resolved 占位。 */
...
/**
 * M-3：从 arena.types[ref-1] 拷贝 region_label[64] 到 out64；仅 TYPE_SLICE 有效。
 * 返回 region_label_len；无域标签或 ref 无效时返回 0。
 */
...
/** M-3：读取 slice 类型域标签长度；ref 无效或无标签时返回 0。 */
...
/**
 * M-3：为 TYPE_SLICE 槽写入域标签（name_len 须为 0）；label_len 须 1..63。
 * 返回 1 成功，0 失败。
 */
int32_t pipeline_type_set_region_label_at(struct ast_ASTArena *arena, int32_t ref, uint8_t *label,
                                          int32_t label_len) {
  struct ast_Type *t;
  if (!arena || ref <= 0 || ref > arena->num_types || !label || label_len <= 0 || label_len > 63)
    return 0;
  t = pipeline_arena_type_ptr(arena, ref);
...
/**
 * M-3：查找或分配 []T<label>（elem_ref + region）；region_len==0 表示未标注域 []T。
 */
...
/**
 * asm/backend.sx asm_expr_array_elem_store_sz_bytes：从数组字面量 expr 池 ref 取 TYPE_ARRAY 的 elem_type_ref。
 * 无效、非数组或 elem 缺失时返回 0。在 C 内读池，避免 .sx 中 `let e: Expr = ast_arena_expr_get` 后字段访问触发 typeck 失败，
 ...
 * backend.sx 多路 `if (kind==…) cc=N` 经 shux-c -E 后可能全部执行，须在 C 内 switch 一次性映射。
 */
...
extern int32_t asm_driver_m8_tail_thin_delegate_c_name(struct ast_Module *m, int32_t func_index, uint8_t *out,
                                                        int32_t out_cap, int32_t *out_len);
/** typeck EMIT_HEAVY 第二遍：桩路径 bl→typeck_su.o 同名符号（扩 __text，避免 mega SU emit Abort）。 */
...
/** EXPR_FLOAT_LIT 64 位立即数入 rax/x0（backend.sx backend_enc_mov_imm64_to_rax_arch）。 */
...
extern int32_t backend_ctx_push_loop_labels(struct backend_AsmFuncCtx *ctx, uint8_t *exit_buf, int32_t exit_len,
                                            uint8_t *loop_buf, int32_t loop_len);
extern void backend_ctx_pop_loop_labels(struct backend_AsmFuncCtx *ctx);
...
static int32_t glue_emit_index_eff_addr_scaled_elf_c(struct ast_ASTArena *arena,
                                                      struct platform_elf_ElfCodegenCtx *elf_ctx, int32_t base_ref,
                                                      int32_t idx_ref, struct backend_AsmFuncCtx *ctx, int32_t ta,
                                                      int32_t esz);
...
/**
 * 与 backend.sx AsmFuncCtx 前缀字段布局一致，供 C glue 读取 break/continue/tail_join 标签。
 * 勿在 SU 侧按值拷贝 AsmFuncCtx；仅通过指针访问。
 */
typedef struct {
  int32_t frame_size;
  int32_t next_offset;
...
/** 将 backend_AsmFuncCtx* 转为布局视图（与 SU 结构体前缀同偏移）。 */
...
/** C ASTExprKind 与 SU ast_ExprKind 在序 54 碰撞：C AWAIT=54，SU EXPR_AS=54；SU EXPR_AWAIT=55。 */
#define GLUE_EXPR_KIND_ORD54 54
#define GLUE_EXPR_KIND_SU_AWAIT 55

/**
 * pool/C parser 上的 await 表达式。
 * SU pool：kind==EXPR_AWAIT(55) 且 unary 操作数有效。
...
/** kind 序 54 且为 SU pool EXPR_AS（as_operand 有效；非 C await）。 */
static int32_t glue_expr_is_su_as_cast_at_c(struct ast_ASTArena *arena, int32_t expr_ref) {
  int32_t ko;
  if (!arena || expr_ref <= 0)
    return 0;
...
/** WPO-S3 asm CPS emit 上下文（单函数 emit 期间有效）。 */
typedef struct {
  int32_t active;
  int32_t ta;
  uint32_t fn_id;
...
/** 调 frame store/load helper：arg0=fn_id, arg1=data_off, arg2=ptr, arg3=nbytes。 */
...
/** 保存 layout 中全部 live 变量到帧 data 区。 */
...
/** await 边界：save live → suspend → 挂 resume 标签或 return SUSPENDED。 */
static int32_t glue_async_cps_emit_after_await(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
  ...
  if (backend_enc_call_arch(elf_ctx, (uint8_t *)suspend_nm, (int32_t)(sizeof(suspend_nm) - 1), ta) != 0)
    return -1;
  /** suspend 返回 0 → resume；x86 须 test eax，arm64 cbz 直读 w0（glue_enc_jz_after_bool_in_eax）。 */
...
/**
 * prologue/param_home 之后：async+await 函数 emit phase 分发（x86_64 / arm64 / riscv64）。
 * phase==1 → 跳到 resume 标签并 restore live；否则从 phase0 继续。
 * 无 await 时 async_asm_pool_build_layout 返回非 0，本函数 no-op。
 */
...
int32_t pipeline_asm_emit_expr_if_arm_elf_c(struct ast_ASTArena *arena,
  ...
  if (ko == 26) {
    ...
    sv_locs = ctx->num_locals;
    sv_next = ctx->next_offset;
...
/**
 * 表达式级 if/三元 ELF 发射（C 实现）：避免 seed partial 中 backend.sx EXPR_IF 经 -E 后漏打 else 标签（.L_else_N offset=-1）。
 * backend.sx::emit_expr_elf 在 ek 25/27 时 extern 调用本函数。
 */
/**
...
/** SU/asm partial 可调用的 STRUCT_LIT 字段 store 宽度（勿在 backend.sx 直链 pipeline_type_kind_ord_at）。 */
int32_t pipeline_expr_struct_lit_field_store_sz(struct ast_ASTArena *a, struct ast_Module *m, int32_t expr_ref,
                                                int32_t field_ix) {
...
/** TYPE_ARRAY 在 TypeKind 序数表中的值（与 ast.sx / pipeline_type_kind_ord_at 一致）。 */
...
static int32_t glue_emit_vector_select_lane_scalar_elf_c(struct ast_ASTArena *arena,
  ...
  int32_t esz;
  int32_t li;
  uint8_t pick_b_lbl[64];
  uint8_t lane_done_lbl[64];
  int32_t pick_b_len;
  int32_t done_len;
...
/** AsmFuncCtx.module_ref 在 SU/C 布局中的字节偏移（与 backend.sx fill_param_slots 一致）。 */
...
/**
 * 形参为定长 [N]T 且 CALL 侧 lea 传址时，槽内为 8B 指针（非内联数组 blob）。
 */
...
int32_t pipeline_asm_emit_func_param_is_indirect_struct_slot_c(struct ast_ASTArena *arena, struct ast_Module *mod,
  ...
  /** layout 未命中时 glue_type_size_simple 回落 4；named struct 形参仍按 hidden pointer（勿 lea 槽址当 struct 基址）。 */
  ...
  /** ≤8B POD struct 按值直存 home 槽；更大 struct 与 CALL 侧一致走 hidden pointer。 */
  return (sz > 8) ? 1 : 0;
}

/**
 * 局部 VAR 槽是否须 load 槽内指针（*T 形参 / [N]T 形参传址 / 块内 let *T）；resolved 缺失时按 stack_off 回落形参表。
 */
...
/**
 * 局部 VAR 槽地址：*T 为 load 指针；定长 [N]T / struct / 向量等为 lea 栈槽（按值存放）。
 */
        ...
        if (nlen > 0 && nlen <= 63) {
          ...
          for (k = 0; k < (int32_t)mod->num_struct_layouts; k++) {
            ...
            if (ln != nlen)
              continue;
            for (j = 0; j < nlen; j++) {
              if (pipeline_module_struct_layout_name_byte_at(mod, k, j) != struct_name[j]) {
                eq = 0;
                break;
              }
            }
...
/** 由类型 ref 推断 INDEX 元素字节宽（*f32→4，*u8→1；TYPE_F32=14 须与 ast.sx TypeKind 一致）。 */
...
int32_t pipeline_asm_emit_lvalue_eff_addr_elf_c(struct ast_ASTArena *arena,
  ...
  if (ko == 3) {
    ...
    if (off < 0)
      return -1;
    /** 左值 VAR 默认 lea 栈槽；*T/[N]T 才 load 指针（见 holds_indirect + let 声明类型）。 */
  ...
  if (ko == 47) {
    ...
    return glue_emit_index_eff_addr_scaled_elf_c(arena, elf_ctx, base_ref, idx_ref, ctx, ta, esz);
  }
  return -1;
}
...
static int32_t glue_expr_kind_is_assign_like_ord(int32_t ko) {
 ...
 */
static void glue_binop_kill_assign_lhs_slots_elf_c(struct ast_ASTArena *arena, struct backend_AsmFuncCtx *ctx,
...
static void glue_live_fwd_clear(GlueBlockLiveFwd *live) {
  if (live)
    live->n = 0;
}
...
static void glue_live_fwd_add(GlueBlockLiveFwd *live, int32_t off) {
  if (!live || off < 0)
    return;
 ...
 */
static void glue_live_fwd_collect_expr_uses(struct ast_ASTArena *arena, struct backend_AsmFuncCtx *ctx,
...
static void glue_binop_cache_intersect_live_fwd(void) {
  int32_t i;
  if (!glue_block_live_fwd_active)
    return;
...
static void glue_block_compute_cfg_peak_live_and_color(struct ast_ASTArena *arena, struct backend_AsmFuncCtx *ctx,
...
static void glue_block_compute_linear_live_in(struct ast_ASTArena *arena, struct backend_AsmFuncCtx *ctx,
 ...
 */
static void glue_block_compute_live_end_linear(struct ast_ASTArena *arena, struct backend_AsmFuncCtx *ctx,
 ...
 */
static void glue_live_fwd_forward_after_def(struct ast_ASTArena *arena, struct backend_AsmFuncCtx *ctx,
 ...
 * 第十一步（已接）：peephole_elf 消除 x10–x14↔x0/x1 连续往返 mov（8 字节对，见 peephole.sx）。
...
/** 清空本块 spill 着色表。 */
static void glue_asm73_clear_spill_color_map(void) {
...
static void glue_asm73_compute_spill_color_pins(void) {
  int32_t peak;
  int32_t peak_i;
  int32_t i;
  glue_asm73_interf_clear();
...
static void glue_block_live_fwd_apply_top_stmt(struct ast_ASTArena *arena, struct backend_AsmFuncCtx *ctx,
                                                int32_t block_ref, int32_t slot_base, int32_t nconst, int32_t nlet,
                                                int32_t stmt_i) {
  GlueBlockLiveFwd gen;
  GlueBlockLiveFwd kill;
...
static void glue_block_live_fwd_before_stmt(int32_t stmt_i, int32_t ta,
                                              struct platform_elf_ElfCodegenCtx *elf_ctx) {
  if (!glue_block_live_fwd_active)
    return;
 ...
 * 返回 1=已发射，0=不适用，-1=错误。
 */
static int32_t glue_try_block_let_index_init_from_assign_cache_elf_c(struct ast_ASTArena *arena,
                                                                      struct platform_elf_ElfCodegenCtx *elf_ctx,
                                                                      struct backend_AsmFuncCtx *ctx,
...
static void glue_index_subadd3_sum_cache_clear(void) {
...
static void glue_index_minus_pair_cache_clear(void) {
...
int32_t pipeline_asm_emit_assign_elf_c(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
  ...
  if (lko == 47) {
    ...
    if (glue_try_index_var_minus_add3_mul_lit_idx_addr_to_rbx_elf_c(arena, elf_ctx, base_ref, idx_ref, ctx, ta, esz) == 0)
      return glue_index_assign_finish_store_elf_c(arena, elf_ctx, ctx, base_ref, idx_ref, esz, ta);
    glue_index_assign_addr_cache_clear();
    if (glue_emit_index_eff_addr_scaled_elf_c(arena, elf_ctx, base_ref, idx_ref, ctx, ta, esz) != 0)
      return -1;
    if (backend_enc_mov_rax_to_rbx_arch(elf_ctx, ta) != 0)
      return -1;
    return glue_index_assign_finish_store_elf_c(arena, elf_ctx, ctx, base_ref, idx_ref, esz, ta);
...
/**
 * ARRAY_LIT 元素标量字节宽（[N]T 的 T）；与 backend.sx asm_array_lit_elem_byte_sz 一致。
 */
static int32_t pipeline_asm_array_lit_elem_byte_sz_c(struct ast_ASTArena *arena, int32_t expr_ref) {
  int32_t elem_ty;
  int32_t kind_ord;
...
/**
 * let/const 初值为 `[]` 的空 ARRAY_LIT：与 backend.sx asm_init_is_empty_array_lit 一致，stmt_order 路径须跳过 rec emit。
 */
...
int32_t pipeline_asm_emit_array_lit_elf_c(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
  ...
  if (n_arr == 0) {
    temp_base = ctx->next_offset;
    return backend_enc_lea_rbp_to_rax_arch(elf_ctx, temp_base, ta);
  }
  /** [64]u8 字面量可达 64 元；上限 256 与 OneFuncResult 侧车一致。 */
  if (n_arr <= 0 || n_arr > 256)
    return -1;
...
/**
 * INDEX 元素字节宽：*u8/u8→1，i32/u32/f32→4，[N]Struct→layout 宽，默认 8。
 * glue_type_size_simple 定义见本文件后部，须前向声明（避免 C99 隐式非 static）。
 */
static int32_t glue_type_size_simple(struct ast_Module *m, struct ast_ASTArena *a, int32_t ty_ref, int32_t depth);
...
static int32_t pipeline_asm_index_elem_byte_sz_c(struct ast_ASTArena *arena, int32_t expr_ref) {
  ...
  if (kind_ord == 10 || kind_ord == 11) {
    ...
    if (pointee > 0) {
      ...
      if (kind_ord == 0 || kind_ord == 3 || kind_ord == 13 || kind_ord == 14)
        return 4;
      /** [N]Struct AoS：步长为 layout 槽宽（勿误落默认 8 → 错位/栈破坏）。 */
...
int32_t pipeline_asm_emit_addr_of_elf_c(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
  ...
  if (ok == 3) {
    ...
    if (off < 0)
      return PIPELINE_ASM_ELF_EXPR_FAST_UNHANDLED;
    /** 向量/struct 按值槽 lea；*T/[N]T 指针槽 load（与 glue_enc_local_slot_ptr_or_addr 一致）。 */
...
static int32_t glue_lazy_append_block_let_local(struct ast_ASTArena *arena, struct backend_AsmFuncCtx *ctx,
  ...
  if (asm_ctx_local_append((uint8_t *)ctx, name, name_len, slot_off) < 0)
    return -1;
  ctx->num_locals = asm_ctx_local_count((uint8_t *)ctx);
  return 0;
}
...
/** 前向声明：f32 xmm ABI 形参 load 判定用（定义见本文件后部）。 */
...
/** 前向声明：7.3 binop 操作数 loader 用（定义见本文件后部）。 */
static int32_t pipeline_asm_expr_lit_i32_at_c(struct ast_ASTArena *arena, int32_t expr_ref, int32_t *out_imm);
static int32_t pipeline_asm_emit_var_field_access_elf_c(struct ast_ASTArena *arena,
                                                        struct platform_elf_ElfCodegenCtx *elf_ctx, int32_t expr_ref,
                                                        struct backend_AsmFuncCtx *ctx, int32_t ta);
/** binop 操作数 FIELD_ACCESS 须走枚举 TokenKind/ExprKind 回落（定义见本文件后部）。 */
...
static int32_t pipeline_asm_emit_expr_elf_rec(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
  ...
  /** FIELD_ACCESS 已在 fast 尝试过；enum namespace 最后回落（skip typeck 无 is_enum_variant）。 */
  if (ko == 44) {
...
/**
 * text 路径：局部 VAR 槽地址（*T/[N]T load 指针，其余 lea 栈槽）。
 */
...
/**
 * INDEX 左值 base 有效地址入 rax/x0（ELF）；与 backend.sx 原 emit_index_eff_addr_elf 语义一致。
 */
...
/** 与 typeck.sx typeck_sx_type_size 一致（前向声明，定义见本文件后部）。 */
static int32_t glue_type_size_simple(struct ast_Module *m, struct ast_ASTArena *a, int32_t ty_ref, int32_t depth);

/**
 * CALL 实参 VAR：块内 let struct / 定长数组 [N]T 须 lea 传地址；标量 / *T / 形参 struct 指针槽须 load。
 */
static int32_t glue_call_arg_var_use_lea_not_load_elf_c(struct ast_ASTArena *arena, int32_t expr_ref,
  ...
  /** main 等无 scope sidecar：用 skip-typeck 回填的 VAR resolved_type_ref（hello msg: [12]u8）。 */
  if (decl_ty <= 0)
...
static int32_t pipeline_asm_emit_binop_mul_elf_c(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
   ...
   */
  if (pipeline_expr_kind_ord_at(arena, left_ref) == 6 &&
      pipeline_expr_kind_ord_at(arena, right_ref) == GLUE_EXPR_KIND_VAR) {
    if (pipeline_asm_emit_expr_elf_rec(arena, elf_ctx, left_ref, ctx, ta) != 0)
      return -1;
    glue_binop_var_slot_cache_invalidate_rax();
...
static int32_t pipeline_asm_emit_binop_and_elf_c(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
  ...
  /** 7.3：左结合 & 链 ((…)&VAR) — 与 add/mul 同序。 */
  if (pipeline_expr_kind_ord_at(arena, left_ref) == 11 &&
      pipeline_expr_kind_ord_at(arena, right_ref) == GLUE_EXPR_KIND_VAR) {
    if (pipeline_asm_emit_expr_elf_rec(arena, elf_ctx, left_ref, ctx, ta) != 0)
      return -1;
    glue_binop_var_slot_cache_invalidate_rax();
...
static int32_t pipeline_asm_emit_binop_bitwise_elf_c(struct ast_ASTArena *arena,
  ...
  if (pipeline_asm_expr_lit_i32_at_c(arena, left_ref, &lit_imm)) {
    ...
    if (backend_enc_mov_imm32_to_rbx_arch(elf_ctx, lit_imm, ta) != 0)
      return -1;
    return is_xor ? backend_enc_xor_rbx_rax_arch(elf_ctx, ta) : backend_enc_or_rbx_rax_arch(elf_ctx, ta);
  }
  if (pipeline_asm_expr_lit_i32_at_c(arena, right_ref, &lit_imm)) {
    if (pipeline_asm_emit_expr_elf_rec(arena, elf_ctx, left_ref, ctx, ta) != 0)
      return -1;
  ...
  /** 7.3：左结合 | / ^ 链 ((…){op}VAR) — 与 add/mul 同序。 */
  if ((!is_xor && pipeline_expr_kind_ord_at(arena, left_ref) == 12) ||
      (is_xor && pipeline_expr_kind_ord_at(arena, left_ref) == 13)) {
    if (pipeline_expr_kind_ord_at(arena, right_ref) == GLUE_EXPR_KIND_VAR) {
      if (pipeline_asm_emit_expr_elf_rec(arena, elf_ctx, left_ref, ctx, ta) != 0)
        return -1;
      glue_binop_var_slot_cache_invalidate_rax();
...
static int32_t pipeline_asm_emit_field_access_elf_fast_c(struct ast_ASTArena *arena,
  ...
  if (fa_base2 > 0 && pipeline_expr_kind_ord_at(arena, fa_base2) == 47) {
    int32_t load_sz;
    /** emit 时 stride 仍缺：再跑 SoA typeck（skip .sx typeck / 形参 [N]T 回填遗漏）。 */
...
/**
 * emit_expr_elf 统一 C 入口（fast + rec）；供日后 SU 薄包装，勿在 rec 内再回调本符号。
 */
...
/**
 * emit_expr_elf 快速路径：LIT/const_fold/RETURN/算术二元；未命中返回 -99 由 .sx 尾 return 结束函数。
 */
int32_t pipeline_asm_emit_expr_elf_fast(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
                                        int32_t expr_ref, struct backend_AsmFuncCtx *ctx, int32_t ta) {
  struct ast_Expr *e;
 ...
 * parse_one_function_library SU 真 emit 时勿对局部 Type/Expr 数组逐元素 ASSIGN（asm INDEX 发射失败）。
 */
void pipeline_module_fill_u8_64_from_src_c(uint8_t *dst, const uint8_t *src, int32_t n, int32_t src_cap) {
  int32_t i;
  if (!dst)
...
/** parse_one_function_library SU emit：在 arena 写 TYPE_BOOL，避免 SU 局部 Type 大结构体 get/set。 */
...
/** parse_one_function_library SU emit：在 arena 写 TYPE_NAMED + 名字节。 */
...
/** parse_one_function_library SU emit：VAR Expr + param 名。 */
...
/** parse_one_function_library SU emit：FIELD_ACCESS Expr。 */
...
/** parse_one_function_library SU emit：ENUM_VARIANT 占位 Expr。 */
...
/** parse_one_function_library SU emit：EQ binop Expr。 */
...
/** parse_one_function_library SU emit：library 形态 labeled return Block。 */
...
/**
 * parse_one_extern_and_add_into SU emit：写 arena Func 槽并登记 module，避免 SU `f.name[i]` INDEX ASSIGN。
 * 返回 module func 下标；池满或参数非法时返回 -1。
 */
...
/** 自举 codegen：将 module->funcs[fi].name[64] 拷入 dst，避免 .sx 对嵌套大数组下标 typeck/asm GEP 失败。 */
void pipeline_module_func_name_copy64(struct ast_Module *m, int32_t func_index, uint8_t *dst) {
  struct ast_Func *f;
...
/**
 * C 版 struct_layout_metrics：与 typeck.sx typeck_struct_layout_metrics 语义一致。
 * asm 栈槽宽 / frame_size 热路径走此实现，避免 gen2 自举 SU metrics 在 Stage2 上极慢挂死。
 */
 ...
 * 典型场景：atomic head 与普通 data 字段伪共享。
 */
...
/** 与 typeck.sx typeck_sx_type_align 一致（§11.1 布局步进）。 */
static int32_t glue_type_align_simple(struct ast_Module *m, struct ast_ASTArena *a, int32_t ty_ref, int32_t depth) {
  int32_t kind_ord;
  ...
  if (kind_ord == 11)
    return 8;
  /** ARRAY / LINEAR / VECTOR：对齐取 elem（与 typeck.sx ko==10/12/13 一致）。 */
...
/** DOD-S1：[N]SoAStruct 列主序总字节数；非 SoA 时返回 0 由调用方回落 AoS。 */
...
/** 与 typeck.sx typeck_sx_type_size 一致。 */
static int32_t glue_type_size_simple(struct ast_Module *m, struct ast_ASTArena *a, int32_t ty_ref, int32_t depth) {
  int32_t kind_ord;
  if (!a || ty_ref <= 0 || ty_ref > a->num_types || depth > 64)
    return 0;
  ...
  if (kind_ord == 8) {
    ...
    for (k = 0; m && k < (int32_t)m->num_struct_layouts; k++) {
      ...
      if (!eq)
        continue;
      return typeck_sx_type_size_from_layout_glue(m, a, k, depth + 1);
    }
    return 4;
  }
  return 0;
 ...
 * skip .sx typeck 的 asm emit 前调用，使 align(64) 等字段偏移落盘（DOD-CL-S1）。
 */
...
static int32_t glue_struct_layout_index_by_type_name_c(struct ast_Module *m, uint8_t *struct_name, int32_t nlen) {
  int32_t k;
  int32_t j;
  if (!m || !struct_name || nlen <= 0 || nlen > 63)
    return -1;
 ...
 * typeck.sx check_block_impl 块末 `return;` 判定须用此 glue，勿 `let fin_ex: Expr = ast_arena_expr_get` 再读字段（自举 asm FIELD_ACCESS 失败）。
 */
int32_t pipeline_expr_unary_operand_ref_at(struct ast_ASTArena *a, int32_t expr_ref) {
  struct ast_Expr *ex = glue_arena_expr_at_ref(a, expr_ref);
...
/** EXPR_VAR 变量名 memcpy 到 out64（最多 63 字节 + 清零）。 */
...
/**
 * 比较表达式 ELF 发射（EQ/NE/LT/LE/GT/GE 全在 C 内，避免 backend.sx 经 shux-c -E 后
...
/** 右操作数为 TypeKind/ExprKind 枚举 FIELD_ACCESS 时返回 tag，否则 -1（不查 module 字段，避免 typeck_sx_ast 中 module.x 误触）。 */
...
static int32_t glue_fixed_array_temp_bytes(struct ast_ASTArena *arena, int32_t type_ref) {
  ...
  if (!t || t->array_size <= 0)
    return 0;
  /** [N]Struct：与 typeck / asm_local_slot_bytes 同宽（SoA 列主序或 AoS N×layout）。 */
...
void pipeline_asm_bump_next_offset_after_let_init(struct ast_ASTArena *arena, int32_t block_ref, int32_t let_idx,
  ...
  if (!ctx)
    return;
  /** EXPR_ARRAY_LIT 已在 emit_expr_elf_slow 末尾 bump；STRUCT_LIT let 直写栈槽，勿再占 temp 区。 */
  if (init_ref > 0) {
...
int32_t pipeline_asm_emit_block_inits_elf_c(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
  ...
  nlet = ast_ast_block_num_lets(arena, block_ref);
  idx = 0;
  for (i = 0; i < nconst && (slot_base + idx) < ctx->num_locals; i++) {
  ...
  for (i = 0; i < nlet && (slot_base + idx) < ctx->num_locals; i++) {
    ...
    if (glue_block_let_is_simd_vector_type(arena, block_ref, i)) {
      ...
      /** [N]T = [..]：逐元素直写内联栈槽，勿 temp 指针 + 后续 let 重叠。 */
...
/**
 * 按 stmt_order 同步发射块体（C for 循环）：避免 shux-c -E 使 backend.sx 内 while(i<nso) 只跑一轮，
      ...
      if (idx >= 0 && idx < nlet) {
        ...
        if (init_ref != 0) {
          if (glue_init_is_empty_array_lit(arena, init_ref)) {
            ...
            /** [N]T = [] 内联 blob；prologue 已清零栈，勿 pointer+temp（会错位 offset 0）。 */
            ...
            /** [N]u8 = [..] 非空 ARRAY_LIT：走 rec（pipeline_asm_emit_array_lit_elf_c）。 */
            glue_index_assign_addr_cache_clear();
            if (pipeline_asm_emit_expr_elf_rec(arena, elf_ctx, init_ref, ctx, ta) != 0) {
              if (getenv("SHUX_ASM_DEBUG"))
                fprintf(stderr,
            ...
            if (ix_init == 0) {
              ...
              if (let_ty > 0 && pipeline_type_kind_ord_at(arena, let_ty) == GLUE_TYPE_KIND_F32_ORD && init_ko == 1) {
                ...
                if (getenv("SHUX_ASM_DEBUG"))
                  fprintf(stderr,
                          "shux: block let init emit fail block_ref=%d let_idx=%d init_ref=%d kind_ord=%d nlet=%d\n",
                          (int)block_ref, (int)idx, (int)init_ref, (int)pipeline_expr_kind_ord_at(arena, init_ref),
      ...
      if (idx >= 0 && idx < ast_ast_block_num_expr_stmts(arena, block_ref)) {
        ...
        if (expr_ref != 0) {
          ...
          if (pipeline_asm_emit_expr_elf_rec(arena, elf_ctx, expr_ref, ctx, ta) != 0) {
            if (getenv("SHUX_ASM_DEBUG"))
              fprintf(stderr, "shux: block expr_stmt emit fail block_ref=%d ex_i=%d expr_ref=%d kind=%d\n",
                      (int)block_ref, (int)idx, (int)expr_ref, (int)pipeline_expr_kind_ord_at(arena, expr_ref));
            return -1;
          }
          glue_block_live_fwd_apply_top_stmt(arena, ctx, block_ref, slot_base, nconst, nlet, i);
        }
...
/**
 * backend.sx emit_block_body_elf 入口：转发 C 同步块体发射（避免 SU while 在自举 asm 下只跑一轮）。
 */
int32_t backend_emit_block_body_sync_elf(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
                                         int32_t block_ref, struct backend_AsmFuncCtx *ctx, int32_t ta) {
  pipeline_glue_AsmFuncCtxLayout *ly = pipeline_asm_ctx_layout(ctx);
  /** 供 FIELD_ACCESS layout 查表；与 backend.sx ctx.module_ref / dep_pipe 对齐。 */
  if (ly && ly->module_ref)
    g_pipeline_asm_emit_module = ly->module_ref;
  if (ly && ly->dep_pipe)
    g_pipeline_asm_emit_dep_pipe = (struct ast_PipelineDepCtx *)ly->dep_pipe;
...
/** backend.sx：读 Block.final_expr_ref（勿 ast_arena_block_get 按值拷贝）。 */
int32_t pipeline_asm_block_final_expr_ref_at(struct ast_ASTArena *a, int32_t br) {
  struct ast_Block *blk = pipeline_arena_block_ptr(a, br);
  return blk ? blk->final_expr_ref : 0;
}

/** backend.sx：读 Block.num_stmt_order（与 ast_ast_block_num_stmt_order 一致）。 */
...
/**
 * 块级 if ELF 发射（then-first + jz）：避免 backend.sx 经 shux-c -E 后 jnz/else/then 顺序错乱，
...
/** EXPR_AS 目标类型池 ref；无效 ref 返回 0（typeck return 分支 SU emit 用）。 */
int32_t pipeline_expr_as_target_type_ref_at(struct ast_ASTArena *a, int32_t expr_ref) {
  struct ast_Expr *ex = glue_arena_expr_at_ref(a, expr_ref);
...
/**
 * SU typeck_ensure_primitive_by_kind_ord：写新分配 primitive 槽（memset+kind）；
 * 查找去重循环在 typeck.sx 真 emit，避免 ensure_* 单行 wrapper bl→pipeline_type_ensure_by_kind_ord。
 */
...
/**
 * SU find_or_alloc_named_type_ref：写新分配 TYPE_NAMED 槽（memset+kind+name memcpy）；
 * 按名扫描去重在 typeck.sx 真 emit。
 */
int32_t pipeline_type_init_named_at(struct ast_ASTArena *a, int32_t ref, uint8_t *name, int32_t name_len) {
  struct ast_Type *t;
  if (!a || ref <= 0 || ref > a->num_types || !name || name_len <= 0 || name_len > 63)
    return 0;
  t = pipeline_arena_type_ptr(a, ref);
  if (!t)
    return 0;
...
/**
 * SU typeck_find_or_alloc_compound_type_ref：写新分配复合 type 槽；
 * 查找 loop（kind+elem+array_size+name_len==0）在 typeck.sx 真 emit。
 */
...
/**
 * backend.sx get_return_expr_ref：在 C 内读 Block/LabeledStmt/Expr 池，避免 SU 自举 asm 对
...
/** backend.sx get_return_expr_ref：按 module 下标在 C 内读 Func.body_ref，勿传 SU 侧按值拷贝的 *Func。 */
...
/**
 * arm64.sx get_return_lit_ref：同上，最小集返回值 ref 查找。
 */
...
/** arm64.sx get_return_lit_ref：按 module 下标在 C 内读 Func.body_ref。 */
...
/** INDEX 元素字节宽；避免 backend.sx asm_index_elem_byte_sz 内 Type 局部副本自举 asm 失败。 */
...
/**
 * 写 arena.exprs[expr_ref].resolved_type_ref；供 typeck.sx EMIT_HEAVY 真 emit，避免 Expr 按值 get/set 撕裂。
 */
...
/** 从 float_val 重算并写入 float_bits_lo/hi；typeck EXPR_FLOAT_LIT SU emit 用（勿 Expr 按值 set）。 */
...
/** 读 expr.line；break/continue 诊断 SU emit 用。 */
int32_t pipeline_expr_line_at(struct ast_ASTArena *a, int32_t expr_ref) {
  struct ast_Expr *ex = glue_arena_expr_at_ref(a, expr_ref);
  return ex ? ex->line : 0;
}

/** 读 expr.col；break/continue 诊断 SU emit 用。 */
int32_t pipeline_expr_col_at(struct ast_ASTArena *a, int32_t expr_ref) {
  struct ast_Expr *ex = glue_arena_expr_at_ref(a, expr_ref);
  return ex ? ex->col : 0;
}

/** 读 ctx.typeck_loop_depth；break/continue SU emit 勿直接读 PipelineDepCtx 字段（自举 asm SIGSEGV）。 */
int32_t pipeline_dep_ctx_typeck_loop_depth_at(struct ast_PipelineDepCtx *ctx) {
  return ctx ? ctx->typeck_loop_depth : 0;
}

/**
 * 写 expr.field_access_is_enum_variant 与 enum_variant_tag；供 typeck_coerce_init_expr_to_decl SU emit。
 */
...
/** typeck.sx get_field_offset_from_layout_deps；asm emit 跨模块 struct 字段偏移回落。 */
...
static int32_t glue_field_layout_offset_for_var_base_field(struct ast_ASTArena *a, struct ast_Module *m,
  ...
  int32_t k;
  int32_t j;
  if (!a || !m || base_var_ref <= 0 || !field_name || flen <= 0 || flen > 63)
    return -1;
  base_ty = 0;
...
static int32_t glue_field_layout_offset_for_base_field(struct ast_ASTArena *a, struct ast_Module *m, int32_t base_ref,
  ...
  int32_t nlen;
  int32_t k;
  if (!a || !m || base_ref <= 0 || !field_name || flen <= 0 || flen > 63)
    return -1;
 ...
 * 与 backend.sx asm_field_access_load_byte_sz 中 TypeKind 分支一致。
 */
 ...
 */
static int32_t pipeline_token_kind_variant_tag(const uint8_t *variant_name, int32_t variant_len) {
  /** 与 include/token.h TokenKind 枚举序一致；import token.sx 未登记 sidecar 时 asm 快路径回落。 */
...
int32_t pipeline_expr_enum_namespace_field_tag(struct ast_ASTArena *a, int32_t expr_ref) {
  ...
  /** import token.sx 的 TokenKind：快路径 pipeline_token_kind_variant_tag 处理；此处仅 module sidecar 回落。 */
...
/** 绑定 emit 期 arena；backend.sx 主循环每函数 emit 前调用。 */
 ...
 * backend.sx SU 路径在 resolved_type 缺失时调用。
 */
int32_t pipeline_asm_emit_func_param_is_ptr_by_name_c(struct ast_ASTArena *arena, struct ast_Module *mod,
  ...
  int32_t param_ty;
  int32_t kind;
  if (!arena || !mod || !vname || vlen <= 0 || vlen > 63)
    return 0;
  fi = g_pipeline_asm_emit_func_index;
...
/**
 * backend.sx asm 主循环 / fill_param_slots：仅 backend 模块声明 extern，转发至无前缀 pipeline_module_func_*，
...
/**
 * 将函数形参填入 asm 局部 sidecar（偏移 8,16,…）；与 backend.sx fill_param_slots 一致。
 */
void pipeline_asm_fill_param_slots(struct backend_AsmFuncCtx *ctx, struct ast_Module *mod, int32_t func_index) {
  ...
  for (i = 0; i < np; i++) {
    ...
    if (asm_ctx_local_append((uint8_t *)ctx, pname_buf, plen, off) < 0)
      return;
    ctx->num_locals = asm_ctx_local_count((uint8_t *)ctx);
    off += 8;
  }
...
/**
 * SysV x86 + SHUX_ABI_F32_XMM=1：gp/xmm 分轨形参 homing（f32 从 xmm0–7 movd 落 32-bit 栈槽）。
 */
...
/**
 * 将块内 const/let 填入 asm 局部 sidecar；与 backend.sx fill_local_slots 一致（+8 槽 + lit temp 预留）。
 */
...
/**
 * 将无符号十进制写入 buf[off..]，最多 max 字节；返回写入长度，失败 -1。
 * 供 pipeline_asm_emit_next_label_c 格式化 ".Lf<scope>_<id>" 后缀。
 */
...
/**
 * 将有符号 i32 十进制写入 buf[off..]；供 format_label_id 等 match 标签格式化。
 */
...
/**
 * 将 label 序号格式化为 ".L_<id>" 写入 buf；与 backend.sx format_label_id 一致（不推进 label_counter）。
 */
...
/**
 * if 语句 then 块 ELF 发射：暂存/恢复 locals，fill + sync block body；与 backend.sx emit_if_then_block_body_elf 一致。
 */
...
int32_t backend_emit_loop_body_content_elf_sync(struct ast_ASTArena *arena,
  ...
  if (!arena || !elf_ctx || !ctx || body_ref <= 0)
    return -1;
  {
    pipeline_glue_AsmFuncCtxLayout *ly = pipeline_asm_ctx_layout(ctx);
    if (ly && ly->module_ref)
  ...
  /** 嵌套 while 体等可能仅有 loops[]/expr_stmts 而无 stmt_order（parser 子块未 push）。 */
  if (nso == 0) {
    int32_t wi;
    int32_t ei;
    int32_t slot_base_fb = backend_block_slot_base_for(ctx, arena, body_ref);
    if (ast_ast_block_num_consts(arena, body_ref) > 0 || ast_ast_block_num_lets(arena, body_ref) > 0) {
      if (pipeline_asm_emit_block_inits_elf_c(arena, elf_ctx, body_ref, ctx, ta, slot_base_fb) != 0)
        return -1;
    }
    for (wi = 0; wi < ast_ast_block_num_loops(arena, body_ref); wi++) {
      if (backend_emit_while_loop_elf_sync(arena, elf_ctx, body_ref, wi, ctx, ta) != 0)
        return -1;
    }
    for (ei = 0; ei < ast_ast_block_num_expr_stmts(arena, body_ref); ei++) {
      int32_t expr_ref = ast_pipeline_block_expr_stmt_ref(arena, body_ref, ei);
      if (expr_ref != 0 && pipeline_asm_emit_expr_elf_rec(arena, elf_ctx, expr_ref, ctx, ta) != 0)
        return -1;
    }
    return glue_emit_block_final_expr_elf(arena, elf_ctx, body_ref, ctx, ta);
  }
  for (i = 0; i < nso; i++) {
    uint8_t item_kind = ast_ast_block_stmt_order_kind(arena, body_ref, i);
    int32_t idx = ast_ast_block_stmt_order_idx(arena, body_ref, i);
    int32_t slot_base = backend_block_slot_base_for(ctx, arena, body_ref);
    int32_t nconst = ast_ast_block_num_consts(arena, body_ref);
    int32_t nlet = ast_ast_block_num_lets(arena, body_ref);
    if (item_kind == 0 || item_kind == 1) {
      if (glue_emit_block_stmt_order_let_const_elf(arena, elf_ctx, body_ref, ctx, ta, item_kind, idx, slot_base,
                                                   nconst, nlet) != 0)
        return -1;
    } else if (item_kind == 2) {
      if (idx >= 0 && idx < ast_ast_block_num_expr_stmts(arena, body_ref)) {
        int32_t expr_ref = ast_pipeline_block_expr_stmt_ref(arena, body_ref, idx);
        if (expr_ref != 0 && pipeline_asm_emit_expr_elf_rec(arena, elf_ctx, expr_ref, ctx, ta) != 0)
          return -1;
      }
    } else if (item_kind == 3) {
      if (idx >= 0 && idx < ast_ast_block_num_loops(arena, body_ref)) {
        if (backend_emit_while_loop_elf_sync(arena, elf_ctx, body_ref, idx, ctx, ta) != 0)
          return -1;
      }
    } else if (item_kind == 4) {
      if (idx >= 0 && idx < ast_ast_block_num_for_loops(arena, body_ref)) {
        if (backend_emit_for_loop_elf_sync(arena, elf_ctx, body_ref, idx, ctx, ta) != 0)
          return -1;
      }
    } else if (item_kind == 5) {
      /** 与块体主路径一致：须 emit else（while 内 `if (i>0) { … } else { i=i+1 }` 等）。 */
      if (idx >= 0 && idx < ast_ast_block_num_if_stmts(arena, body_ref)) {
        if (pipeline_asm_emit_block_if_stmt_elf(arena, elf_ctx, body_ref, idx, ctx, ta, i) != 0)
          return -1;
        glue_asm_ctx_set_scope_block((uint8_t *)ctx, body_ref);
      }
    }
...
/** 两 VAR expr ref 是否同名（B-CMP LCG xor while 模式匹配）。 */
static int32_t glue_fold_expr_var_refs_same_c(struct ast_ASTArena *arena, int32_t a_ref, int32_t b_ref) {
  ...
  if (alen <= 0 || blen <= 0 || alen != blen || alen > 63)
    return 0;
...
/** 解析 `while (i < n)`：左 VAR i，右 LIT 或 VAR n。 */
...
/** 同块 let 绑定整型字面量初值（`let n: i32 = 100000000`）。 */
...
/** 解析 `i * c1 + c2` 或 `c1 * i + c2`（loop_i32 LCG 混合项）。 */
...
static int32_t glue_is_assign_u8_index_store_cast_i_c(struct ast_ASTArena *arena, int32_t er, int32_t *out_buf_ref,
  ...
  if (!glue_fold_expr_var_refs_same_c(arena, pipeline_expr_index_index_ref(arena, left_ref), i_ref))
    return 0;
  if (!glue_expr_is_su_as_cast_at_c(arena, right_ref))
    return 0;
...
static int32_t glue_is_assign_sum_plus_u8_index_cast_c(struct ast_ASTArena *arena, int32_t er, int32_t *out_sum_ref,
  ...
  if (!glue_fold_expr_var_refs_same_c(arena, add_l, left_ref))
    return 0;
  if (!glue_expr_is_su_as_cast_at_c(arena, as_op))
    return 0;
...
/** x86：addl $imm8/imm32, -off(%rbp)（i++ 快路径；大 disp 用 disp32）。 */
...
/** x86：movl -off(%rbp), %ecx（loop_i32 LCG 寄存器化）。 */
...
/** x86：imull $imm32, %edx, %ecx（t = i*C1，i 驻留 edx）。 */
...
/** x86：cmpl $imm32, %edx（LCG bottom-tested i 与 n-1 比较）。 */
...
/** 发射单次 LCG body：t=i*C1+C2; s^=t; i++（edx=i, eax=s, ecx=scratch）。 */
...
/** x86：imull $imm32, %ecx, %eax（t = i*C1，i 保持 ecx）。 */
...
/** x86：xorl %eax, %edx（LCG s ^= t，s 驻留 edx）。 */
...
/** x86：xorl %eax, -off(%rbp)（s ^= t 原位，免 t 栈槽）。 */
...
/** VAR expr 名是否与 body 内第 let_idx 条 let 同名。 */
static int32_t glue_expr_var_name_eq_let_idx_c(struct ast_ASTArena *arena, int32_t var_expr_ref,
  ...
  if (vlen <= 0 || llen <= 0 || vlen != llen || vlen > 63)
    return 0;
  pipeline_expr_var_name_into(arena, var_expr_ref, vname);
...
static int32_t glue_try_fold_struct_pair_n2_while_elf_c(struct ast_ASTArena *arena,
  ...
  if (!glue_match_struct_pair_n2_body_pattern_c(arena, mod, body_ref, i_ref, &s_ref, &pair_ref))
    return 0;
  /** s 在 while 外层块；须切 scope 到 block_ref 再查栈槽。 */
  glue_asm_ctx_set_scope_block((uint8_t *)ctx, block_ref);
  off_s = glue_asm_local_var_stack_off_scoped(arena, ctx, s_ref);
  if (off_s < 0)
    return 0;
  /** 编译期 n²（i32 wrap），避免 1e8 次循环。 */
  {
...
/** 编译期计算 mem_copy 外层 rounds×内层 u8 累加的最终 i32 结果（含 wrap）。 */
...
/**
 * 只匹配 u8 fill 循环 AST 形态（不查栈槽；供外层 fold 在 let 尚未 emit 时调用）。
 */
...
/**
 * 只匹配 `while (i<N) { buf[i]=(i as u8); i++ }`，不写码。
 * 返回 1=匹配；out_buf/out_i/out_n 填关键 ref/常量。
 */
...
/**
 * 只匹配 `while (j<N) { sum+=(buf[j] as i32); j++ }`，不写码。
 */
...
/**
 * mem_copy 外层：`while (r<R) { fill; sum; r++ }` → 编译期写入 sum 常量，跳过整棵循环。
 */
static int32_t glue_try_fold_mem_copy_outer_while_elf_c(struct ast_ASTArena *arena,
  ...
  if (!rounds_ok)
    return 0;
  /** B-CMP mem_copy 快捷路径：r<8192 + 双内层 4096 u8 fill/sum（绕过外层 stmt_order 形态差异）。 */
  ...
  fill_li = -1;
  sum_li = -1;
  /** 子块可能仅有 loops[]/expr_stmts 而无 stmt_order（parser 未 push）。 */
  if (nso == 0) {
    int32_t nloops;
    int32_t nexpr;
    int32_t last_er;
...
/**
 * 尝试优化 `while (i < n) { let t = i*C1+C2; s ^= t; i++ }`（loop_i32 LCG）。
 * x86_64 专用 tight loop；返回 1=已处理，0=未匹配，-1=错误。
 */
static int32_t glue_try_fold_lcg_xor_while_elf_c(struct ast_ASTArena *arena,
  ...
  if (loop_len <= 0)
    return -1;
  n_cmp = n_const - 1;
  /** n 为 4/2 的倍数时 4×/2× 展开，减少回跳分支（stretch 0.95× loop_i32）。 */
  unroll = 1;
  if ((n_const & 3) == 0)
    unroll = 4;
  else if ((n_const & 1) == 0)
...
/**
 * ELF while 循环；C glue 真实现（与 backend.sx emit_while_loop_elf 语义一致）。
 */
int32_t backend_emit_while_loop_elf_sync(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
                                         int32_t block_ref, int32_t loop_idx, struct backend_AsmFuncCtx *ctx,
                                         int32_t ta) {
...
/**
 * ELF for 循环；C glue 真实现（与 backend.sx emit_for_loop_elf 语义一致）。
 */
int32_t backend_emit_for_loop_elf_sync(struct ast_ASTArena *arena, struct platform_elf_ElfCodegenCtx *elf_ctx,
                                       int32_t block_ref, int32_t for_idx, struct backend_AsmFuncCtx *ctx, int32_t ta) {
  int32_t init_ref;
  ...
  if (dbg_env && dbg_env[0] != '\0' && dbg_env[0] != '0' && mod != NULL) {
    ...
    if (dbg_stub_n <= 8 || (clen > 0 && dbg_delegate_hit <= 5)) {
      ...
      fprintf(stderr, "parser_delegate_stub #%d fi=%d fn=%.*s clen=%d hit_total=%d\n", (int)dbg_stub_n,
              (int)func_index, (int)(fl > 63 ? 63 : fl), fn, (int)clen, (int)dbg_delegate_hit);
      if (clen > 0)
        fprintf(stderr, "  -> cname=%.*s\n", (int)(clen > 71 ? 71 : clen), cname);
      fflush(stderr);
    }
    if (dbg_stub_n == 1) {
      int32_t fi;
...
/** ast.sx 池 init/alloc：须在 #include ast_pool.c 之前（ast_pool 内 strict parse 调 ast_ast_arena_init）。 */
void ast_expr_layout_prime_call_resolved(void) {
  /* ast.sx expr_layout_prime_call_resolved；C glue 侧无额外状态。 */
}
...
/** WPO-S2 mono thunk 槽（glue 收集，避免 codegen ASTModule 与 ast_Module 布局差异）。 */
...
/** 当前 ELF codegen 已匹配的全常量 call→mono 注册表（emit 末尾写 thunk 体）。 */
...
/** 标量双参 mono 登记（scale 等）；委托 glue_wpo_mono_register_thunk_n。 */
...
#include "ast_pool.c"
#include "src/async/async_asm_pool.c"
...
extern int32_t parser_copy_module_import_path64(struct ast_Module *module, int32_t idx, uint8_t out[64]);
...
/**
 * M8-tail strict 回退（pipeline.o 未 EMIT_HEAVY 时由 ast_pool/pipeline_glue 提供）：
 * - parse_into_buf / load_import / sync_dep_slots / resolve_path_su / read_file_su / parse_into_with_init_buf：SU 真 emit；_c 经 weak→强符号 dispatch（build_asm 覆盖）。
...
/** M8-tail：优先 dispatch 至 pipeline_parse_into_buf（SU 或 weak impl_c）。 */
int32_t pipeline_parse_into_buf_c(struct ast_ASTArena *arena, struct ast_Module *module, uint8_t *buf,
                                   int32_t buf_len) {
  return pipeline_parse_into_buf(arena, module, buf, buf_len);
}

/**
 * strict 回退：单 import resolve/read/preprocess/parse（C 路径，与 SU 版语义对齐）。
...
int32_t pipeline_load_import_from_disk_impl_c(struct ast_Module *module, struct ast_ASTArena *arena,
  ...
  if (pipeline_resolve_path_su(ctx, path_buf, path_len) != 0)
    return -7;
  if (pipeline_read_file_su(ctx) != 0)
    return -8;
...
/**
 * strict 回退：各 dep 槽与 driver seed 对齐；build_asm 链入时由 SU 强符号覆盖 weak 默认。
 */
...
/**
 * strict 回退：strict reset 后 parser_parse_into_buf；勿 ast_ast_arena_init+parser_parse_into_init 叠加。
 */
...
/** weak 默认：standalone glue 链；build_asm pipeline.o 链入时被 SU 强符号覆盖。 */
...
__attribute__((weak)) int32_t pipeline_resolve_path_su(struct ast_PipelineDepCtx *ctx, uint8_t import_path[64],
                                                        int32_t path_len) {
  return pipeline_resolve_path_su_impl_c(ctx, import_path, path_len);
}

__attribute__((weak)) int32_t pipeline_read_file_su(struct ast_PipelineDepCtx *ctx) {
  return pipeline_read_file_su_impl_c(ctx);
}

/** weak 默认：parse_into_with_init / lsp / typeck 薄编排（standalone；build_asm pipeline.o SU 强符号覆盖）。 */
...
/** M8-tail：优先 dispatch 至 pipeline_load_import_from_disk（SU 或 weak impl_c）。 */
...
/**
 * M8-tail：优先 dispatch 至 pipeline_parse_into_with_init_buf（SU 或 weak impl_c）。
 */
...
/** M8-tail：优先 dispatch 至 pipeline_sync_dep_slots_from_driver（SU 或 weak impl_c）。 */
...
/**
 * strict 回退：LSP parse + load deps + typeck（无 codegen）；parse 走 set_main_c，typeck 走 pipeline_typeck_parsed_module。
 */
...
/**
 * LSP parse 半路径：pipeline_parse_set_main_from_buf_c；失败时 parse 诊断已在 C glue 内。
 */
...
/** SU 或 weak impl_c；_c dispatch 前向声明。 */
...
/** M8-tail：优先 dispatch 至 pipeline_lsp_diag_parse_typeck_buf（SU 或 weak impl_c）。 */
...
/**
 * strict 回退：slice parse_into_with_init + typeck；SU 薄编排调用 _c（勿 _c→裸名 dispatch，避免 SU 薄包装递归）。
 */
...
/**
 * strict 回退：slice parse + typeck；parse_into mega 经 pipeline_parse_into_with_init_c（见上，无 dispatch 环）。
 */
...
/** shux check 后 WPO-S3 SU 栈逃逸 gate（strict parse + post-scan；C typeck 已通过）。 */
int32_t pipeline_typeck_sx_stack_escape_gate_from_src_c(uint8_t *src, int32_t src_len) {
  size_t asz;
  size_t msz;
  void *arena_heap;
  void *module_heap;
...
/** M8-tail：优先 dispatch 至 pipeline_typeck_after_parse_ok（SU 或 weak impl_c）。 */
...
/** weak 默认：lsp / typeck 诊断路径（standalone；build_asm pipeline.o SU 强符号覆盖）。 */
#ifdef SHUX_PIPELINE_GLUE_STANDALONE_TU
...
/**
 * 为函数形参同名 EXPR_VAR 回填 resolved_type_ref（skip .sx typeck 时 SoA `arr[i].field` 须 [N]T）。
...
/** 为块内 let 同名 EXPR_VAR 回填 resolved_type_ref（跳过 .sx typeck 时 arr[i] 等 INDEX 需 base 类型）。 */
...
/** 遍历块内 let，为跳过 .sx typeck 的 asm 路径补齐 ARRAY_LIT 类型（hello [12]u8 等）。 */
...
/**
 * C 预检后跳过 .sx typeck 时：为各函数体块内 `let buf: [N]u8 = [..]` 回填 ARRAY_LIT 的 resolved_type_ref，
...
/** pipeline.sx 经 import codegen 解析 extern 时带 codegen_ 前缀。 */
...
/** asm.sx 经 import codegen 解析 extern 时带 codegen_ 前缀；转发 ast_pool scratch。 */
...
/*
 * ast.sx 中 extern pipeline_* 经 codegen 调用时带 ast_ 模块前缀（如 ast_pipeline_block_append_if），
...
/** ast.sx extern pipeline_dep_ctx_*；typeck 经 ast_pipeline_* 前缀调用（ast_pool.c 实现 pipeline_*）。 */
...
/**
 * 将 lib_root[lib_idx] 拷入 dst[0..255]（先清零）；返回 lib_root 字节长度。
 * 避免 resolve_path_su 内 `let buf: [256]u8 = []` 触发 parse_into_buf 跳过大函数体。
 */
...
/** M-3：parser.sx import ast 前缀 → ast_pool pipeline_block_append_region。 */
...
/**
 * ast.sx 中 ast_arena_*_get/set 仅作 extern 声明；实现在此，避免 shux-c -E 将 get 错误 lowering 为
...
/** M-3：typeck/codegen.sx 经 ast_ 前缀读 Block.num_regions。 */
...
/**
 * typeck.sx::func_body_tail_expr_ref_for_implicit_rule 的 C 委托：读块 final_expr / stmt_order 尾表达式。
 * SU 真 emit 在 asm 后端失败；EMIT_HEAVY 桩 bl 本符号，避免依赖 typeck_su.o 同名实现。
 */
...
/**
 * typeck.sx::func_body_has_implicit_return_tail 的 C 委托：判定块末是否存在不允许的隐式尾返回。
 */
...
/**
 * typeck.sx::typeck_patch_all_body_parent_links 的 C 委托：为 module 各函数体块 patch parent 链。
 */
...
/** 比较两个 NAMED 类型名（与 typeck.sx::type_refs_equal_named 一致）。 */
...
/** type_refs_equal_named 薄委托（与 typeck.sx 拆分 helper 一致）。 */
...
/**
 * kind 已相等时的复合类型比较（与 typeck.sx::type_refs_equal_same_kind 一致；EMIT_HEAVY 薄委托）。
 */
...
/** 内部：a/b 已非 null 且 a!=b；读 kind 后委托 same_kind（与 typeck.sx::type_refs_equal_impl 一致）。 */
...
/** 判断 type_ref 是否为 bool（内部；与 typeck.sx::type_ref_is_bool_impl 一致）。 */
...
/** 判断 type_ref 是否为 bool（与 typeck.sx::type_ref_is_bool 一致；EMIT_HEAVY 薄委托）。 */
...
/**
 * typeck.sx::expr_type_ref_impl 的 C 委托：读 expr.resolved_type_ref（glue 指针读，避免 Expr 按值撕裂）。
 */
...
/**
 * typeck.sx::expr_type_ref 的 C 委托：带 null/边界检查的 resolved_type_ref 读取。
 */
...
/** 诊断缓冲追加字面量（与 typeck.sx::typeck_diag_append_lit 一致）。 */
...
/** 诊断缓冲追加十进制 u32（与 typeck.sx::typeck_diag_append_u32_dec 一致）。 */
...
/**
 * typeck.sx::typeck_diag_fmt_type_at 的 C 委托：将类型 ref 格式化为可读 ASCII（不写 NUL）。
 */
...
/** typeck.sx::typeck_diag_fmt_type_into 的 C 委托。 */
...
/** typeck.sx::typeck_diag_fmt_type_or_question 的 C 委托。 */
...
/**
 * typeck.sx::typeck_ret_coerce_integral_to_expect_i32 的 C 委托：i32 返回时对 u8/usize 操作数写回 resolved_type_ref。
 */
...
/** typeck.o / typeck_su：按元素类型分配或复用 *T Type ref。 */
extern int32_t find_or_alloc_ptr_type_ref(struct ast_ASTArena *arena, int32_t elem_ref);

/**
 * typeck.sx::typeck_check_expr_unary 的 C 委托：NEG/BITNOT/LOGNOT 检查操作数并写 resolved_type。
 */
...
/**
 * typeck.sx::typeck_check_expr_addr_of 的 C 委托：操作数类型 T，表达式类型 *T。
 */
...
/** skip typeck 时从 STRUCT_LIT 补登记 module.struct_layouts（typeck.sx ensure_struct_layout_from_struct_lit）。 */
extern int32_t typeck_ensure_struct_layout_from_struct_lit(struct ast_Module *module, struct ast_ASTArena *arena,
                                                           int32_t expr_ref);

/**
 * asm emit 前：为 `arr[i].field` SoA 读/写补 col_base + stride（C/SU typeck 遗漏或 skip 时）。
 * 须在 glue_fill_var_types_from_lets 之后（INDEX 基址须 resolved [N]T）。
 */
...
/**
 * typeck.sx::typeck_check_expr_index 的 C 委托：检查 base/index 下标，写元素类型与 slice/bounds 标记。
 */
...
/** typeck_sx_no_layout_partial：顶层 let 名比较。 */
...
/**
 * typeck.sx::typeck_check_expr_var 的 C 委托：块 symtab / 顶层 let / 形参 / TokenKind·TypeKind 限定名。
 */
...
/**
 * typeck.sx::typeck_import_segment_at 的 C 委托：取 import 路径第 want_seg 段偏移与长度。
 */
int32_t pipeline_typeck_import_segment_at_c(struct ast_Module *module, int32_t imp_ix, int32_t want_seg,
  ...
  if (!module || imp_ix < 0 || imp_ix >= module->num_imports || !ostr || !olen)
    return 0;
...
/**
 * typeck.sx::dep_return_type_to_caller_arena 的 C 实现：dep 侧 return_type_ref 递归映到 caller arena。
 */
...
/** typeck.sx::dep_return_type_to_caller_arena 的 C 委托（EMIT_HEAVY 薄包装入口）。 */
...
/** typeck.sx::get_dep_return_type_in_caller_arena 的 C 委托。 */
...
/** typeck.sx::expr_var_name_equal_func 的 C 委托：VAR callee 名与 module.funcs[j] 名逐字节相等。 */
...
/** typeck.sx::find_func_return_type_in_module_by_name 的 C 委托。 */
...
/** typeck.sx::find_func_return_type_in_module 的 C 委托。 */
...
/** typeck.sx::typeck_expr_apply_call_resolve 的 C 委托：写入 CALL 结点的 resolved dep/func 下标。 */
...
/** typeck ImportKind.IMPORT_BINDING / IMPORT_SELECT（与 ast.sx 序数一致）。 */
enum {
  GLUE_TYPECK_IMPORT_BINDING = 1,
  GLUE_TYPECK_IMPORT_SELECT = 2,
};

/** typeck.sx::typeck_import_path_segment_count 的静态辅助：统计 import 路径 `.` 分段数。 */
...
/** typeck.sx::typeck_import_path_slice_equal 的静态辅助。 */
...
/** typeck.sx::typeck_import_binding_name_equal 的静态辅助。 */
...
/** typeck.sx::typeck_import_select_name_equal 的静态辅助。 */
...
/**
 * typeck.sx::resolve_whole_import_qualified_call_return_type 的 C 委托：
    ...
    return 0;
  {
    ...
    while (dep_j < module->num_imports) {
      ...
      }
      {
        ...
        dm = pipeline_dep_ctx_module_at(ctx, dep_j);
        if (!dm) {
          dep_j = dep_j + 1;
          continue;
        }
        ...
        ret_fn = pipeline_typeck_find_func_return_type_in_module_by_name_c(
            dm, arena, layer_buf, asm_qual_sym_layer_len(0), dep_j, ctx, func_index_out);
        if (ret_fn != 0 && dep_index_out)
          dep_index_out[0] = dep_j;
        return ret_fn;
      }
    }
  }
...
/**
 * typeck.sx::resolve_call_callee_return_type 的 C 委托：
        ...
        while (ii < module->num_imports) {
          if (pipeline_module_import_kind_at(module, ii) == GLUE_TYPECK_IMPORT_BINDING &&
            ...
            if (dm) {
              ...
              if (want_resolve)
                p_bind_fn = &bind_fn;
              ret_b = pipeline_typeck_find_func_return_type_in_module_by_name_c(dm, arena, field_nm, field_len, ii, ctx,
                                                                              p_bind_fn);
              if (ret_b != 0) {
                if (want_resolve)
                  pipeline_typeck_expr_apply_call_resolve_c(arena, call_expr_ref, ii, bind_fn);
                return ret_b;
              }
            }
            break;
...
/** typeck.o 简单 kind helper；pipeline_typeck_check_expr_impl_c 与 check_expr_impl SU 共用。 */
...
/** typeck.sx::typeck_validate_struct_layouts_zero_padding 的 C 委托。 */
...
/**
 * M-3：.sx typeck 统一 slice 域 assign/let/实参检查；与 typeck.c typeck_check_slice_region_assign 措辞一致。
...
int32_t pipeline_typeck_linear_use_var_c(struct ast_ASTArena *arena, int32_t type_ref, int32_t expr_ref,
  ...
  int32_t line;
  int32_t col;
  if (!arena || name_len <= 0 || name_len > 63 || !name)
    return 0;
...
int32_t pipeline_typeck_read_ptr_slice_return_ref_c(struct ast_ASTArena *arena) {
  ...
  if (u8_ref <= 0)
    return 0;
  return pipeline_type_find_or_alloc_slice(arena, u8_ref, (uint8_t *)lbl, 10);
}

/**
 * M-3：.sx typeck return 路径 slice 域逃逸 / 不一致；ret_site_ref 用于 line/col。
 */
...
static uint8_t g_typeck_region_saved_label[TYPECK_REGION_SCOPE_MAX][64];
...
int32_t pipeline_dep_ctx_scope_region_push_c(struct ast_PipelineDepCtx *ctx, uint8_t *label, int32_t label_len) {
  int32_t slot;
  if (!ctx || !label || label_len <= 0 || label_len > 63)
    return -1;
  ...
  if (ctx->typeck_scope_region_len > 0 && ctx->typeck_scope_region_len <= 63)
    memcpy(g_typeck_region_saved_label[slot], ctx->typeck_scope_region_label, 64);
...
void pipeline_dep_ctx_scope_region_pop_c(struct ast_PipelineDepCtx *ctx) {
  ...
  memset(ctx->typeck_scope_region_label, 0, sizeof(ctx->typeck_scope_region_label));
  if (g_typeck_region_saved_len[slot] > 0 && g_typeck_region_saved_len[slot] <= 63)
    memcpy(ctx->typeck_scope_region_label, g_typeck_region_saved_label[slot], 64);
}
...
/** M-3：typeck 单条 region 块：push 域标签 → check 体块 → pop。 */
...
/**
 * typeck.sx::typeck_coerce_init_lit_to_decl 的 C 委托（整型字面量分支）。
 */
...
/** typeck.sx::typeck_coerce_init_float_lit_to_decl 的 C 委托。 */
...
/** typeck.sx::typeck_coerce_init_enum_field_to_decl 的 C 委托（枚举 variant 字段访问）。 */
...
/** typeck.sx::typeck_coerce_init_named_call_to_decl 的 C 委托。 */
...
/** typeck.sx::typeck_coerce_init_array_vector_lit_to_decl 的 C 委托。 */
...
/** typeck.sx::typeck_coerce_init_vector_binop_to_decl 的 C 委托。 */
...
/**
 * typeck.sx::typeck_coerce_init_expr_to_decl 的 C 委托（薄分发，对齐 SU 子 helper 调用序）：
...
/** 判定 ExprKind 是否为赋值/复合赋值（与 typeck.sx check_block 块尾推断一致）。 */
 ...
 * strict+pipeline 链不链整颗 typeck.o 时子 helper 来自 typeck_sx_no_layout。
 */
int32_t pipeline_typeck_check_expr_impl_mega_c(struct ast_Module *module, struct ast_ASTArena *arena,
  ...
  if (pipeline_typeck_expr_is_any_assign_kind_c(kind)) {
    int32_t left_ref;
    int32_t right_ref;
    int32_t rc = typeck_check_expr_assign(module, arena, expr_ref, return_type_ref, ctx);
    if (rc != 0)
      return rc;
    left_ref = pipeline_expr_binop_left_ref_at(arena, expr_ref);
    right_ref = pipeline_expr_binop_right_ref_at(arena, expr_ref);
    return pipeline_typeck_check_struct_stack_escape_assign_c(module, arena, expr_ref, left_ref, right_ref, ctx);
  }
...
/**
 * typeck.sx::check_expr_impl 的 C 委托：简单 kind→typeck_check_expr_*；mega→check_expr_impl_mega。
 * EMIT_HEAVY 第二遍 check_expr_impl 仍 bl 本符号（勿 SU 真 emit if+递归 check_expr）。
 */
...
/** typeck.sx::check_expr 的 C 委托：边界检查后委托 pipeline_typeck_check_expr_impl_c（简单 kind C 处理 + mega 回退）。 */
int32_t pipeline_typeck_check_expr_c(struct ast_Module *module, struct ast_ASTArena *arena, int32_t expr_ref,
                                     int32_t return_type_ref, struct ast_PipelineDepCtx *ctx) {
  if (ast_ref_is_null(expr_ref))
    return 0;
...
/** 写 ctx.typeck_loop_depth；typeck_loop_depth_push/pop SU emit 用。 */
...
/**
 * typeck.sx::check_block_impl 的 C 委托：stmt_order/legacy 编排（C while）+ typeck.o one_* / final。
 * strict+pipeline 链不链整颗 typeck.o 时须自包含，不可 extern SU 递归 walker。
 */
...
/** typeck.sx::check_block 的 C 委托：边界检查后委托 pipeline_typeck_check_block_impl_c。 */
int32_t pipeline_typeck_check_block_c(struct ast_Module *module, struct ast_ASTArena *arena, int32_t block_ref,
                                      int32_t return_type_ref, struct ast_PipelineDepCtx *ctx) {
...
/** typeck.sx::check_block_as_loop_body 的 C 委托：loop_depth push/pop + check_block（无 typeck.o 时自包含）。 */
...
/**
 * typeck.sx::typeck_sx_ast_check_one_func 的 C 委托：有 typeck.o 时由 SU 强符号覆盖；否则 glue 调 pipeline_typeck_check_block_c。
 */
int32_t pipeline_typeck_sx_ast_check_one_func_c(struct ast_Module *module, struct ast_ASTArena *arena,
...
/**
 * typeck.sx::typeck_sx_ast_impl 的 C 委托：遍历有体函数（无 typeck.o 时 glue while 自包含）。
 */
int32_t pipeline_typeck_sx_ast_impl_c(struct ast_Module *module, struct ast_ASTArena *arena,
                                      struct ast_PipelineDepCtx *ctx) {
  int32_t mi;
  int32_t ret_kind;
  int32_t i;
...
/** typeck.sx::typeck_sx_ast_library 的 C 委托：库模块无 main（无 typeck.o 时 glue while 自包含）。 */
int32_t pipeline_typeck_sx_ast_library_c(struct ast_Module *module, struct ast_ASTArena *arena,
                                         struct ast_PipelineDepCtx *ctx) {
  int32_t i;
...
/** typeck.sx::typeck_sx_ast 的 C 委托：校验 main 下标后委托 impl_c。 */
int32_t pipeline_typeck_sx_ast_c(struct ast_Module *module, struct ast_ASTArena *arena,
                                 struct ast_PipelineDepCtx *ctx) {
  int32_t mi;
  ...
  if (mi >= module->num_funcs)
    return -11;
  return pipeline_typeck_sx_ast_impl_c(module, arena, ctx);
}
...
/** ast.sx / typeck / codegen / backend / parser 经 import 前缀调用的 Expr 侧车池符号转发。 */
...
/**
 * typeck.sx / asm glue 统一入口：委托 C metrics，勿再调 gen2 自举 SU typeck_struct_layout_metrics。
 */
...
/** TYPE_NAMED 且命中 struct_layout 时算 size；C 栈 out 参数，.sx check_block 勿 &local。 */
int32_t typeck_sx_type_size_from_layout_glue(struct ast_Module *module, struct ast_ASTArena *arena, int32_t li,
    int32_t depth) {
  int32_t z2 = 0;
  int32_t al2 = 1;
  if (li < 0)
...
/** TYPE_NAMED 且命中 struct_layout 时算 align；与 typeck.sx typeck_sx_type_align ko==8 分支一致。 */
int32_t typeck_sx_type_align_from_layout_glue(struct ast_Module *module, struct ast_ASTArena *arena, int32_t li,
    int32_t depth) {
  int32_t z2 = 0;
  int32_t al2 = 1;
  if (li < 0)
