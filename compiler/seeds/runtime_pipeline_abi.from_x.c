...
/*
 * wave137: pipeline_asm_emit_cmp pure-owned leave cold twins.
 * PREFER pure; cold path when PREFER!=1 / hybrid fail.
 * Faces: cmp_cc + arm64_cset + typekind_variant_tag + emit_cmp_elf.
 * Cap residual symbols remain host-cc (try_binop_cmp, enc_*, pool).
 */
...
extern int32_t glue_try_binop_cmp_rbx_rax_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta);
...
static int32_t glue_type_kind_is_64bit_int_seed(int32_t kind) {
  return kind == 4 || kind == 5 || kind == 6 || kind == 7 || kind == 9;
}

static int32_t glue_emit_rex_w_if_64bit_seed(void *elf_ctx, int32_t is_64bit, int32_t ta) {
  if (is_64bit && ta == 0) {
    uint8_t rex_w[1] = { 0x48 };
    return pipeline_elf_ctx_append_bytes(elf_ctx, rex_w, 1);
  }
  return 0;
}
...
static int32_t glue_emit_cmp_finish_seed(void *arena, void *ctx, void *elf_ctx, int32_t left_ref,
...
int32_t pipeline_asm_emit_cmp_elf(void *arena, void *elf_ctx, int32_t cmp_expr_ref, void *ctx, int32_t ta) {
  int32_t right_ref, left_ref, lit_imm, cc, enum_tag, cmp_ko, is_cmp_64bit, lt_ref, lt_kind, rt_ref, rt_kind, rk, var_off, left_ref2, vr;
  if (!arena || cmp_expr_ref <= 0)
    return -1;
  if (pipeline_expr_const_folded_valid_at(arena, cmp_expr_ref) != 0)
    return backend_enc_mov_imm32_to_w0_arch(elf_ctx, pipeline_expr_const_folded_val_at(arena, cmp_expr_ref), ta);
  ...
  if (left_ref > 0 && right_ref > 0 && pipeline_expr_kind_ord_at(arena, left_ref) == 48 &&
      cmp_expr_lit_i32_seed(arena, right_ref, &lit_imm) && lit_imm == 0 && (cmp_ko == 14 || cmp_ko == 15)) {
    if (pipeline_asm_emit_expr_elf_c(arena, elf_ctx, left_ref, ctx, ta) != 0)
      return -1;
  ...
  if (right_ref > 0 && (enum_tag = pipeline_asm_cmp_enum_rhs_tag_seed(arena, right_ref)) >= 0) {
    if (pipeline_asm_emit_expr_elf_c(arena, elf_ctx, pipeline_expr_binop_left_ref_at(arena, cmp_expr_ref), ctx, ta) != 0)
      return -1;
    if (backend_enc_mov_rax_to_rbx_arch(elf_ctx, ta) != 0)
      return -1;
  ...
  if (left_ref > 0 && right_ref > 0 && pipeline_expr_kind_ord_at(arena, left_ref) == 3 &&
      !glue_binop_operand_is_scalar_f64_elf_c(arena, ctx, left_ref) &&
      !glue_binop_operand_is_scalar_f32_elf_c(arena, ctx, left_ref) &&
      cmp_expr_lit_i32_seed(arena, right_ref, &lit_imm)) {
  ...
  if (left_ref > 0 && right_ref > 0 &&
      ...
      !glue_binop_operand_is_scalar_f64_elf_c(arena, ctx, left_ref) &&
      !glue_binop_operand_is_scalar_f32_elf_c(arena, ctx, left_ref) &&
      cmp_expr_lit_i32_seed(arena, left_ref, &lit_imm)) {
    var_off = -1;
  ...
  if (vr == -1)
    return -1;
  if (vr == -2) {
    if (pipeline_asm_emit_expr_elf_c(arena, elf_ctx, left_ref2, ctx, ta) != 0)
      return -1;
    if (backend_enc_push_rax_arch(elf_ctx, ta) != 0)
      return -1;
    if (pipeline_asm_emit_expr_elf_c(arena, elf_ctx, right_ref, ctx, ta) != 0)
 ...
 * PREFER pure; cold path when PREFER!=1 / hybrid fail.
 * Faces: lhs_f32 + assign_rhs + assign_rhs_to_rax + assign_elf +
 *   field_assign_pair_base + body_expr_stmt_at.
 * Cap residual: binop/try_index/spill/slice dual-gp/float promote/fixed_array/
 *   field offset/enc store-div-shift/array_lit/expr faces.
...
extern int32_t glue_var_decl_type_ref_elf_c(void *arena, void *ctx, int32_t var_expr_ref);
extern int32_t glue_field_access_field_type_ref_c(void *arena, void *mod, int32_t fa_ref);
extern int32_t glue_emit_float_lit_to_rax_elf_c(void *arena, void *elf_ctx, int32_t expr_ref, int32_t ta,
                                                 int32_t force_ty_ref, int32_t call_abi_widen_f64);
extern int32_t pipeline_asm_emit_expr_elf_c(void *arena, void *elf_ctx, int32_t expr_ref, void *ctx, int32_t ta);
extern int32_t glue_try_binop_left_rax_right_rbx_elf_c(void *arena, void *elf_ctx, int32_t left_ref,
                                                        int32_t right_ref, void *ctx, int32_t ta);
extern int32_t glue_emit_binop_add_rax_rbx_elf_c(void *arena, void *elf_ctx, void *ctx, int32_t left_ref,
                                                   int32_t right_ref, int32_t ta);
extern int32_t glue_emit_binop_sub_rax_minus_rbx_elf_c(void *arena, void *elf_ctx, void *ctx, int32_t left_ref,
                                                         int32_t right_ref, int32_t ta);
extern int32_t glue_emit_binop_mul_rax_rbx_elf_c(void *arena, void *elf_ctx, void *ctx, int32_t left_ref,
                                                   int32_t right_ref, int32_t ta);
extern int32_t glue_binop_operand_is_scalar_f32_elf_c(void *arena, void *ctx, int32_t expr_ref);
extern int32_t glue_binop_operand_is_scalar_f64_elf_c(void *arena, void *ctx, int32_t expr_ref);
extern int32_t glue_binop_operand_is_unsigned_elf_c(void *arena, void *ctx, int32_t left_ref, int32_t right_ref);
extern int32_t glue_binop_operand_is_64bit_elf_c(void *arena, void *ctx, int32_t left_ref, int32_t right_ref);
extern int32_t pipeline_asm_emit_divisor_zero_check_rbx_elf_c(void *elf_ctx, void *ctx, int32_t ta);
extern int32_t backend_enc_push_rax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_pop_rbx_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_pop_rax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_divsd_rax_rbx_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_divss_rax_rbx_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_idiv_rbx_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_rem_mod_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_and_rbx_rax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_or_rbx_rax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_xor_rbx_rax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_mov_rbx_to_ecx_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_shl_cl_rax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_shl_cl_eax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_shr_cl_rax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_shr_cl_eax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_sar_cl_rax_arch(void *elf_ctx, int32_t ta);
extern int32_t backend_enc_sar_cl_eax_arch(void *elf_ctx, int32_t ta);
...
extern int32_t pipeline_expr_field_access_base_ref(void *arena, int32_t expr_ref);
extern int32_t glue_enc_local_slot_ptr_or_addr_rbx_elf_c(void *arena, void *elf_ctx, int32_t var_ref,
...
extern int32_t pipeline_asm_emit_struct_lit_fields_elf_c(void *arena, void *elf_ctx, int32_t expr_ref, void *ctx,
                                                          int32_t ta, int32_t stack_slot_off);
extern int32_t glue_var_expr_stack_off_elf_c(void *arena, void *ctx, int32_t var_expr_ref);
extern int32_t glue_emit_index_eff_addr_scaled_elf_c(void *arena, void *elf_ctx, int32_t ix_ref, int32_t base_ref,
...
extern int32_t backend_enc_store_eax_to_rbp_arch(void *elf_ctx, int32_t offset, int32_t ta);
extern int32_t backend_enc_mov_imm64_to_rax_arch(void *elf_ctx, int32_t lo, int32_t hi, int32_t ta);
...
extern int32_t glue_index_assign_addr_cache_hit(void *arena, void *ctx, int32_t base_ref, int32_t idx_ref,
                                                 int32_t esz);
extern void glue_index_assign_addr_cache_clear(void);
extern int32_t glue_index_assign_finish_store_elf_c(void *arena, void *elf_ctx, void *ctx, int32_t base_ref,
                                                     int32_t idx_ref, int32_t esz, int32_t ta);
extern int32_t glue_try_index_var_lit_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref, int32_t idx_ref,
                                                         void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref, int32_t idx_ref,
                                                         void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_plus_lit_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                  int32_t idx_ref, void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_plus_var_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                  int32_t idx_ref, void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_minus_lit_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                   int32_t idx_ref, void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_minus_var_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                   int32_t idx_ref, void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_mul_lit_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                 int32_t idx_ref, void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_mul_var_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                 int32_t idx_ref, void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_plus_var_plus_var_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                           int32_t idx_ref, void *ctx, int32_t ta,
                                                                           int32_t esz);
extern int32_t glue_try_index_var_minus_var_plus_var_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                            int32_t idx_ref, void *ctx, int32_t ta,
                                                                            int32_t esz);
extern int32_t glue_try_index_var_minus_var_minus_var_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                             int32_t idx_ref, void *ctx, int32_t ta,
                                                                             int32_t esz);
extern int32_t glue_try_index_var_minus_add3_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                    int32_t idx_ref, void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_plus_var_mul_lit_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                          int32_t idx_ref, void *ctx, int32_t ta,
                                                                          int32_t esz);
extern int32_t glue_try_index_var_minus_var_mul_lit_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                           int32_t idx_ref, void *ctx, int32_t ta,
                                                                           int32_t esz);
extern int32_t glue_try_index_var_add3_mul_lit_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                      int32_t idx_ref, void *ctx, int32_t ta, int32_t esz);
extern int32_t glue_try_index_var_subadd3_mul_lit_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                         int32_t idx_ref, void *ctx, int32_t ta,
                                                                         int32_t esz);
extern int32_t glue_try_index_var_subsub3_mul_lit_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                         int32_t idx_ref, void *ctx, int32_t ta,
                                                                         int32_t esz);
extern int32_t glue_try_index_var_minus_add3_mul_lit_idx_addr_to_rbx_elf_c(void *arena, void *elf_ctx, int32_t base_ref,
                                                                            int32_t idx_ref, void *ctx, int32_t ta,
                                                                            int32_t esz);
extern int32_t pipeline_expr_var_name_len(void *arena, int32_t expr_ref);
extern void pipeline_expr_var_name_into(void *arena, int32_t expr_ref, uint8_t *out);
extern int32_t asm_ctx_local_find_offset_scoped(void *ctx, void *arena, uint8_t *name, int32_t name_len);
extern int32_t asm_ctx_local_find_offset(void *ctx, uint8_t *name, int32_t name_len);
extern void glue_index_scratch_spill_invalidate_var(void *arena, void *elf_ctx, void *ctx, int32_t var_ref, int32_t ta);
...
extern void glue_slice_dual_gp_bump_past_home_c(void *ctx, int32_t data_home, int32_t ta);
...
static int32_t seed_assign_expr_lit_i32_at(void *arena, int32_t expr_ref, int32_t *out_imm) {
  int32_t ko;
  if (!expr_ref)
    return 0;
  ko = pipeline_expr_kind_ord_at(arena, expr_ref);
...
int32_t glue_assign_lhs_f32_type_ref_elf_c(void *arena, void *ctx, int32_t left_ref) {
  int32_t lko;
  int32_t tr;
  void *mod;
  if (!arena || left_ref <= 0)
...
int32_t glue_emit_assign_rhs_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx,
                                    int32_t ta) {
  int32_t lhs_f32;
...
int32_t glue_emit_assign_rhs_to_rax_elf_c(void *arena, void *elf_ctx, int32_t assign_expr_ref, int32_t left_ref,
                                           int32_t right_ref, void *ctx, int32_t ta) {
  int32_t ako;
  int32_t vr;
  int32_t is_64bit;
...
int32_t pipeline_asm_emit_assign_elf_c(void *arena, void *elf_ctx, int32_t expr_ref, void *ctx, int32_t ta) {
  int32_t left_ref;
  int32_t right_ref;
  int32_t lko;
  int32_t load_sz;
  ...
  if (lko == 44 && pipeline_expr_field_access_is_enum_variant(arena, left_ref) == 0) {
    ...
    if (base_ref > 0 && pipeline_expr_kind_ord_at(arena, base_ref) == 3) {
      ...
      if (var_off < 0)
        var_off = asm_ctx_local_find_offset(ctx, vname, vlen);
      if (var_off < 0)
        return -1;
      mod = pipeline_asm_emit_module_ref_c();
  ...
  if (lko == 47) {
    ...
    if (pipeline_expr_kind_ord_at(arena, expr_ref) == 28 && rko_idx == 45 && esz > 0 &&
        pipeline_expr_kind_ord_at(arena, base_ref) == 3) {
      int32_t lit_imm;
      int32_t base_off;
      int32_t elem_home;
      int32_t base_tr;
      ...
      if (base_tk == 10) {
        if (seed_assign_expr_lit_i32_at(arena, idx_ref, &lit_imm)) {
          base_off = glue_var_expr_stack_off_elf_c(arena, ctx, base_ref);
          if (base_off >= 0) {
            elem_home = (ta == 1) ? (base_off + lit_imm * esz) : (base_off - lit_imm * esz);
            if (elem_home >= 0 &&
                pipeline_asm_emit_struct_lit_fields_elf_c(arena, elf_ctx, right_ref, ctx, ta, elem_home) == 0) {
              glue_index_assign_addr_cache_clear();
              return 0;
            }
          }
      ...
      if (rko_bulk == 3 || rko_bulk == 44 || rko_bulk == 47 || rko_bulk == 48 || rko_bulk == 49) {
        ...
        *(int32_t *)((uint8_t *)ctx + 4) = next_off;
        ...
        if (rko_bulk == 3 || rko_bulk == 44 || rko_bulk == 47) {
          ...
          temp_home = next_off;
          *(int32_t *)((uint8_t *)ctx + 4) = next_off;
          if (glue_emit_struct_type_let_init_elf_c(arena, elf_ctx, right_ref, ctx, ta, 0, temp_home) != 0) {
            glue_index_assign_addr_cache_clear();
            return -1;
          }
  ...
  if (lko == 3) {
    ...
    if (is_modlet == 0 && off >= 0 && ltk_pre == 11 && rko_pre == 46 &&
        pipeline_expr_kind_ord_at(arena, expr_ref) == 28) {
    ...
    if (is_modlet == 0 && off >= 0 && ltk_pre == 10 && pipeline_expr_kind_ord_at(arena, expr_ref) == 28) {
      int32_t arr_st =
...
int32_t glue_field_assign_pair_base_ref_c(void *arena, int32_t er) {
  int32_t left_ref;
  if (!arena || er <= 0 || pipeline_expr_kind_ord_at(arena, er) != 28)
    return 0;
  left_ref = pipeline_expr_binop_left_ref_at(arena, er);
...
int32_t glue_body_expr_stmt_at_c(void *arena, int32_t body_ref, int32_t si, int32_t nso, int32_t *out_er) {
  int32_t er;
...
int32_t glue_emit_vector_operand_lane_elf_c(void *arena,
                                                   ...
                                                   int32_t lane, int32_t esz, void *ctx,
                                                   int32_t ta);
 ...
 */
int32_t glue_try_vector_lane_binop_operands_elf_c(void *arena,
                                                            void *elf_ctx,
...
int32_t glue_apply_vector_lane_scalar_binop_elf_c(void *elf_ctx, int32_t binop_ko,
                                                          void *ctx, int32_t ta) {
  switch (binop_ko) {
  case 4:
    return backend_enc_add_rax_rbx_arch(elf_ctx, ta);
...
int32_t glue_emit_vector_lane_scalar_binop_elf_c(void *arena,
                                                         void *elf_ctx, int32_t binop_ko,
...
int32_t pipeline_asm_emit_vector_var_copy_elf_c(void *arena,
                                                       void *elf_ctx, int32_t src_expr_ref,
                                                       void *ctx, int32_t ta, int32_t dst_off,
                                                       int32_t type_ref) {
  int32_t lanes;
  int32_t esz;
  int32_t src_off;
...
uint32_t glue_simd_emit_cpu_features_c(void) {
  void *p;
  int32_t feats;
  p = pipeline_asm_emit_dep_pipe_c();
  if (p) {
    /* LP64 offsetof PipelineDepCtx.target_cpu_features */
    feats = *(int32_t *)((uint8_t *)p + 8389668);
    if (feats != 0)
      return (uint32_t)feats;
  }
  return driver_get_pending_target_cpu_features();
}

/** SIMD-S3：local VAR+VAR 向量 add/sub/mul 尝试硬件指令（失败则回退 lane-scalar）。 */
int32_t glue_try_hw_vector_add_binop_elf_c(void *arena,
                                                  void *elf_ctx, int32_t binop_ko,
                                                  int32_t left_ref, int32_t right_ref, int32_t dst_off,
                                                  int32_t type_ref, void *ctx, int32_t ta) {
  int32_t lanes;
  int32_t esz;
  int32_t off_a;
  int32_t off_b;
 ...
 */
int32_t glue_call_callee_func_name_into_c(void *arena, int32_t callee_ref, uint8_t *out,
                                                 int32_t out_cap) {
  int32_t ko;
  int32_t clen;
...
int32_t glue_shuffle_mask_elem_is_int_lit_c(void *arena, int32_t elem_ref) {
  int32_t ko;
  if (elem_ref <= 0)
    return 0;
 ...
 * 成功写 *out_imm；失败返回 -1。
 */
int32_t glue_shuffle_pshufd_imm8_from_mask_c(void *arena, int32_t mask_ref, int32_t lanes,
                                                    int32_t *out_imm) {
  int32_t ne;
  int32_t i;
  int32_t m0;
...
int32_t glue_emit_vector_shuffle_lane_scalar_elf_c(void *arena,
                                                          void *elf_ctx,
...
int32_t glue_emit_vector_select_lane_scalar_elf_c(void *arena,
                                                         void *elf_ctx,
...
int32_t pipeline_asm_emit_vector_binop_let_init_elf_c(void *arena,
                                                             void *elf_ctx,
                                                             int32_t init_ref, void *ctx,
...
int32_t glue_fold_func_return_operand_ref_c(void *arena, void *mod,
                                                   int32_t func_idx) {
  int32_t body_ref;
  int32_t fin;
  int32_t nes;
 ...
 */
int32_t glue_try_array_lit_lane_const_i32_c(void *arena, int32_t arr_ref, int32_t lane,
                                                  int32_t *out) {
  int32_t ne;
  int32_t elem_ref;
  int32_t eko;
 ...
 */
int32_t glue_fold_func_returns_param01_vector_binop_dst_c(void *arena, void *mod,
                                                           int32_t func_idx, int32_t *out_binop_ko,
...
int32_t glue_callee_is_vec4f_fma3_c(uint8_t *cname, int32_t clen) {
  if (!cname || clen <= 0)
    return 0;
 ...
 */
/* wave145 pure Cap residual: static→extern. */
int32_t glue_emit_vector_type_let_init_elf_c(void *arena,
                                                    void *elf_ctx, int32_t init_ref,
                                                    void *ctx, int32_t ta, int32_t stack_slot_off,
                                                    int32_t type_ref) {
  int32_t ko;
  int32_t inl;
 ...
 * precedes #include, so a fwd decl is kept at glue.c L2068.
 ...
 */
/* glue_vector_type_lanes_esz_c: see wave148 accessor twin below */

/* glue_is_vector_lane_scalar_binop_ko: backend_try_inline authority (extern). */
extern int32_t glue_is_vector_lane_scalar_binop_ko(int32_t ko);
 ...
 */
/* wave145 pure Cap residual: static→extern. */
int32_t glue_vector_let_init_uses_direct_slot(void *arena, int32_t type_ref, int32_t init_ref) {
  int32_t ko;
...
int32_t glue_vector_elem_is_f32_c(void *arena, int32_t type_ref) {
  int32_t tk;
  int32_t nlen;
  int32_t er;
  int32_t nt;
  ...
  if (tk == 8) {
    ...
    if (nlen == 5 && memcmp(nm, "f32x4", 5) == 0)
      return 1;
    if (nlen == 5 && memcmp(nm, "Vec4f", 5) == 0)
      return 1;
    return 0;
  }
  if (tk != 13)
...
#ifndef XLANG_RUNTIME_PIPELINE_ABI_FROM_X
/* wave149: pipeline_asm_emit_binop pure-owned leave cold twins (void* style).
 * PLATFORM: SHARED freestanding cold twin.
 */

/* wave149 cold twin glue_binop_as_needs_full_emit_elf_c. PLATFORM: SHARED. */
int32_t glue_binop_as_needs_full_emit_elf_c(void *arena, int32_t expr_ref) {
  int32_t op_ref;
  int32_t tgt;
  int32_t tgt_k;
  int32_t src_tr;
...
/* wave149 cold twin glue_binop_operand_index_addr_clobbers_rbx_elf_c. PLATFORM: SHARED. */
int32_t glue_binop_operand_index_addr_clobbers_rbx_elf_c(void *arena, int32_t expr_ref) {
  int32_t ko;
  int32_t op_ref;
  if (!arena || expr_ref <= 0)
    return 0;
...
/* wave149 cold twin glue_binop_operand_load_to_rbx_clobbers_rax_elf_c. PLATFORM: SHARED. */
int32_t glue_binop_operand_load_to_rbx_clobbers_rax_elf_c(void *arena, int32_t expr_ref) {
  int32_t ko;
  int32_t op_ref;
  if (!arena || expr_ref <= 0)
    return 1;
...
/* wave149 cold twin glue_expr_emit_may_clobber_rbx_elf_c. PLATFORM: SHARED. */
int32_t glue_expr_emit_may_clobber_rbx_elf_c(void *arena, int32_t expr_ref) {
  int32_t ko;
  int32_t op_ref;
  if (!arena || expr_ref <= 0)
    return 1;
...
/* wave149 cold twin glue_binop_restore_rbx_after_index_elf_c. PLATFORM: SHARED. */
int32_t glue_binop_restore_rbx_after_index_elf_c(void *elf_ctx, int32_t ta) {
  if (ta == 1)
    return arch_arm64_enc_enc_mov_x2_to_rbx(elf_ctx);
  return backend_enc_pop_rbx_arch(elf_ctx, ta);
}

/* wave149 cold twin glue_binop_preserve_rax_for_rbx_load_elf_c. PLATFORM: SHARED. */
int32_t glue_binop_preserve_rax_for_rbx_load_elf_c(void *elf_ctx, int32_t ta, void *ctx) {
  if (ta != 1)
    return backend_enc_push_rax_arch(elf_ctx, ta);
  {
    ...
    base_off = pipe_load_i32_le(ly, 4);
    if ((base_off % 8) != 0)
      base_off = (base_off + 7) / 8 * 8;
    home = base_off;
    pipe_store_i32_le(ly, 4, base_off + 8);
    glue_align_next_offset(ctx);
...
/* wave149 cold twin glue_binop_restore_rax_after_rbx_load_elf_c. PLATFORM: SHARED. */
int32_t glue_binop_restore_rax_after_rbx_load_elf_c(void *elf_ctx, int32_t ta) {
  if (ta != 1)
    return backend_enc_pop_rax_arch(elf_ctx, ta);
  {
    int32_t home;
    if (!elf_ctx || glue_binop_rax_frame_spill_depth() <= 0)
      return -1;
...
/* wave149 cold twin glue_try_binop_commutative_var_var_slot_cached_elf_c. PLATFORM: SHARED. */
int32_t glue_try_binop_commutative_var_var_slot_cached_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  int32_t loff;
  int32_t roff;
  int32_t rax_hit_l;
  int32_t rax_hit_r;
  ...
  if (glue_binop_var_slot_cache_ctx_matches(ctx) == 0)
    return -2;
  rax_hit_l = (glue_binop_var_slot_cache_valid_rax_get() != 0 && glue_binop_var_slot_cache_rax_off_get() == loff);
  rax_hit_r = (glue_binop_var_slot_cache_valid_rax_get() != 0 && glue_binop_var_slot_cache_rax_off_get() == roff);
  rbx_hit_l = (glue_binop_var_slot_cache_valid_rbx_get() != 0 && glue_binop_var_slot_cache_rbx_off_get() == loff);
  rbx_hit_r = (glue_binop_var_slot_cache_valid_rbx_get() != 0 && glue_binop_var_slot_cache_rbx_off_get() == roff);
...
/* wave149 cold twin glue_try_binop_commutative_rax_rbx_elf_c. PLATFORM: SHARED. */
int32_t glue_try_binop_commutative_rax_rbx_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  int32_t r;
  int32_t cr;
  if (!arena || !elf_ctx || !ctx)
    return -2;
...
/* wave149 cold twin glue_finish_binop_commutative_slow_rax_rbx_elf_c. PLATFORM: SHARED. */
int32_t glue_finish_binop_commutative_slow_rax_rbx_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  glue_binop_var_slot_cache_clear();
  if (pipeline_asm_emit_expr_elf_c(arena, elf_ctx, left_ref, ctx, ta) != 0)
    return -1;
  if (backend_enc_push_rax_arch(elf_ctx, ta) != 0)
...
/* wave149 cold twin glue_try_binop_left_rax_right_rbx_elf_c. PLATFORM: SHARED. */
int32_t glue_try_binop_left_rax_right_rbx_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  int32_t r;
  if (!arena || !elf_ctx || !ctx)
    return -2;
...
/* wave149 cold twin glue_try_binop_cmp_rbx_rax_elf_c. PLATFORM: SHARED. */
int32_t glue_try_binop_cmp_rbx_rax_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  int32_t r;
  if (!arena || !elf_ctx || !ctx)
    return -2;
  r = glue_try_binop_load_operand_elf_c(arena, elf_ctx, left_ref, ctx, ta, 1);
...
/* wave149 cold twin glue_type_ref_is_scalar_f32_c. PLATFORM: SHARED. */
...
/* wave149 cold twin glue_expr_type_is_ptr_c. PLATFORM: SHARED. */
int32_t glue_expr_type_is_ptr_c(void *arena, int32_t expr_ref) {
  int32_t tr;
  if (!arena || expr_ref <= 0)
    return 0;
  tr = pipeline_expr_resolved_type_ref(arena, expr_ref);
  if (tr <= 0)
    return 0;
  return pipeline_type_kind_ord_at(arena, tr) == 9 ? 1 : 0;
}

/* wave149 cold twin glue_ptr_expr_pointee_byte_sz_c. PLATFORM: SHARED. */
int32_t glue_ptr_expr_pointee_byte_sz_c(void *arena, int32_t ptr_expr_ref) {
  int32_t tr;
  int32_t esz;
...
/* wave149 cold twin glue_ptr_arith_scale_rbx_offset_if_left_ptr_c. PLATFORM: SHARED. */
int32_t glue_ptr_arith_scale_rbx_offset_if_left_ptr_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, int32_t ta) {
  int32_t esz;
...
/* wave149 cold twin glue_try_emit_ptr_arith_scaled_elf_c. PLATFORM: SHARED. */
int32_t glue_try_emit_ptr_arith_scaled_elf_c(void *arena, void *elf_ctx, void *ctx, int32_t left_ref, int32_t right_ref, int32_t ta, int32_t is_sub) {
  int32_t lp;
  int32_t rp;
  int32_t ptr_ref;
  int32_t off_ref;
...
/* wave149 cold twin pipeline_asm_emit_binop_add_elf_c. PLATFORM: SHARED. */
int32_t pipeline_asm_emit_binop_add_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  int32_t lit_imm;
  int32_t vr;
  int32_t inl;
  int32_t mixed_rc;
...
/* wave149 cold twin pipeline_asm_emit_binop_sub_elf_c. PLATFORM: SHARED. */
int32_t pipeline_asm_emit_binop_sub_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  int32_t lit_imm;
  int32_t mixed_rc;
  int32_t ptr_rc;
  /**
...
/* wave149 cold twin pipeline_asm_emit_binop_mul_elf_c. PLATFORM: SHARED. */
...
/* wave149 cold twin glue_binop_operand_is_unsigned_elf_c. PLATFORM: SHARED. */
int32_t glue_binop_operand_is_unsigned_elf_c(void *arena, void *ctx, int32_t left_ref, int32_t right_ref) {
  int32_t tr = 0;
  int32_t kind_ord;
  if (!arena)
    return 0;
...
/* wave149 cold twin pipeline_asm_emit_binop_div_elf_c. PLATFORM: SHARED. */
int32_t pipeline_asm_emit_binop_div_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  int32_t lit_imm;
  int32_t vr;
  int32_t is_unsigned;
  int32_t mixed_rc;
...
int32_t pipeline_asm_emit_binop_and_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  int32_t lit_imm;
  int32_t vr;
  if (pipeline_asm_expr_lit_i32_at_c(arena, left_ref, &lit_imm)) {
    if (pipeline_asm_emit_expr_elf_c(arena, elf_ctx, right_ref, ctx, ta) != 0)
...
/* wave149 cold twin pipeline_asm_emit_binop_bitwise_elf_c. PLATFORM: SHARED. */
int32_t pipeline_asm_emit_binop_bitwise_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta, int32_t is_xor) {
  int32_t lit_imm;
  int32_t vr;
  if (pipeline_asm_expr_lit_i32_at_c(arena, left_ref, &lit_imm)) {
    if (pipeline_asm_emit_expr_elf_c(arena, elf_ctx, right_ref, ctx, ta) != 0)
...
/* wave149 cold twin pipeline_asm_emit_binop_shift_elf_c. PLATFORM: SHARED. */
int32_t pipeline_asm_emit_binop_shift_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta, int32_t op) {
  int32_t lit_imm;
  int32_t vr;
  int32_t is_64bit;
  int32_t is_unsigned;
...
/* wave149 cold twin pipeline_asm_emit_binop_mod_elf_c. PLATFORM: SHARED. */
int32_t pipeline_asm_emit_binop_mod_elf_c(void *arena, void *elf_ctx, int32_t left_ref, int32_t right_ref, void *ctx, int32_t ta) {
  int32_t lit_imm;
  int32_t vr;
  int32_t is_unsigned;
  is_unsigned = glue_binop_operand_is_unsigned_elf_c(arena, ctx, left_ref, right_ref);
...
/* ========================================================================
 * wave151 cold twins: pipeline_asm_emit_field_access pure-owned leave.
...
/* Cap residual BSS / helpers (same product link as residual mega-TU faces). */
extern void *g_pipeline_asm_emit_module;
extern int32_t g_pipeline_asm_emit_call_param_ty_ref;
extern int32_t g_pipeline_asm_emit_func_index;
extern int32_t g_pipeline_asm_emit_scope_block;
extern void *g_pipeline_asm_emit_dep_pipe;

/* Cap residual callees (non-static product faces). */
...
extern int32_t pipeline_asm_emit_expr_elf_c(void *arena, void *elf_ctx, int32_t expr_ref, void *ctx, int32_t ta);
extern int32_t glue_store_retval_pair_to_rbp_elf_c(void *m, void *arena, void *elf_ctx, int32_t ty_ref, int32_t slot_off, int32_t ta, int32_t init_ref, void *ctx);
extern int32_t try_inline_struct_lit_return_call_to_slot_elf(void *arena, void *elf_ctx, int32_t call_ref, void *ctx, int32_t ta, int32_t stack_slot_off);
extern int32_t try_inline_const_struct_lit_return_call_to_slot_elf(void *arena, void *elf_ctx, int32_t call_ref, void *ctx, int32_t ta, int32_t stack_slot_off);
extern int32_t pipeline_asm_emit_struct_let_init_elf_c(void *arena, void *elf_ctx, int32_t init_ref, void *ctx, int32_t ta, int32_t stack_slot_off);
extern int32_t glue_index_deref_ptr_field_slot_rax_elf_c(void *arena, void *elf_ctx, int32_t fa_ref, int32_t ta);
extern int32_t pipeline_expr_kind_ord_at(void *arena, int32_t expr_ref);
extern int32_t pipeline_expr_field_access_base_ref(void *arena, int32_t expr_ref);
extern int32_t pipeline_expr_field_access_name_len(void *arena, int32_t expr_ref);
...
extern int32_t pipeline_expr_resolved_type_ref(void *arena, int32_t expr_ref);
extern int32_t glue_call_return_byte_size_c(void *arena, int32_t call_expr_ref);
extern int32_t pipeline_expr_struct_lit_value_bytes(void *arena, void *mod, int32_t expr_ref);
extern int32_t glue_sysv_dual_gp_byte_size_c(void *arena, int32_t ty_ref);
extern void *pipeline_asm_ctx_layout(void *ctx);
extern void glue_align_next_offset(void *ctx);
extern int32_t pipeline_expr_var_name_len(void *arena, int32_t expr_ref);
extern void pipeline_expr_var_name_into(void *arena, int32_t expr_ref, uint8_t *out);
...
extern int32_t glue_var_expr_type_ref_with_decl_fallback_c(void *arena, int32_t var_ref);
extern int32_t glue_local_var_slot_needs_ptr_load_elf_c(void *arena, int32_t var_expr_ref, int32_t stack_off, void *ctx);
extern int32_t glue_slice_dual_gp_length_off_c(int32_t var_off, int32_t ta);
extern int32_t glue_enc_local_slot_ptr_or_addr_elf_c(void *arena, void *elf_ctx, int32_t var_ref, int32_t var_off, void *ctx, int32_t ta);
...
extern int32_t typeck_soa_field_soa_index(void *module, void *arena, int32_t expr_ref, int32_t base_ref);
extern int32_t glue_emit_soa_index_field_addr_elf_c(void *arena, void *elf_ctx, int32_t base_ref, int32_t expr_ref, void *ctx, int32_t ta);
extern int32_t pipeline_asm_emit_lvalue_eff_addr_elf_c(void *arena, void *elf_ctx, int32_t expr_ref, void *ctx, int32_t ta);
extern int32_t pipeline_asm_typekind_variant_tag(uint8_t *field_buf, int32_t flen);
extern int32_t pipeline_module_struct_layout_num_fields(void *module, int32_t layout_idx);
extern int32_t pipeline_module_struct_layout_field_name_len(void *module, int32_t layout_idx, int32_t fi);
extern void pipeline_module_struct_layout_field_name_into(void *module, int32_t layout_idx, int32_t fi, uint8_t *out64);
...
extern int32_t pipeline_module_func_body_ref_at(void *module, int32_t fi);
extern int32_t pipeline_module_num_funcs(void *module);
extern void *pipeline_arena_type_ptr(void *a, int32_t ty_ref);
extern void *glue_arena_expr_at_ref(void *a, int32_t expr_ref);
extern void *pipeline_arena_expr_ptr(void *a, int32_t expr_ref);
extern int32_t typeck_get_field_offset_from_layout_deps(void *module, void *ctx, uint8_t *type_name, int32_t type_name_len, uint8_t *field_name, int32_t field_name_len);
extern int32_t pipeline_dep_ctx_ndep(void *ctx);
extern void *pipeline_dep_ctx_module_at(void *ctx, int32_t idx);
extern int32_t glue_struct_layout_compute_field_offset_c(void *m, void *a, int32_t li, int32_t j);
extern int32_t glue_struct_layout_index_by_type_name_c(void *m, uint8_t *struct_name, int32_t nlen);
extern int32_t pipeline_type_named_name_into(void *arena, int32_t ref, uint8_t *out64);
extern int32_t pipeline_module_enum_variant_tag_for_names(void *m, uint8_t *enum_name, int32_t enum_len, uint8_t *variant_name, int32_t variant_len);
extern char *link_abi_getenv(const char *name);

#ifndef PIPELINE_ASM_ELF_EXPR_FAST_UNHANDLED
#define PIPELINE_ASM_ELF_EXPR_FAST_UNHANDLED (-99)
#endif
#ifndef GLUE_TYPE_NAMED
...
#define GLUE_TYPE_KIND_ARRAY 10
#endif
/* ast TypeKind enums used via numeric kind field when peels Type* */
#ifndef ast_TypeKind_TYPE_PTR
#define ast_TypeKind_TYPE_PTR 9
#endif
#ifndef ast_TypeKind_TYPE_NAMED
#define ast_TypeKind_TYPE_NAMED 8
#endif
...
/* Minimal LP64 mirrors for cold twin field peels (wave151). PLATFORM: SHARED. */
typedef struct w151_ExprMirror {
  int32_t kind;
 ...
 */
int32_t glue_field_access_layout_field_type_ref_by_name_c(void *arena,
  ...
  if (!arena || !mod || fa_ref <= 0)
    return 0;
  ex = (w151_ExprMirror *)pipeline_arena_expr_ptr(arena, fa_ref);
  if (!ex)
    return 0;
 ...
 */
int32_t glue_field_access_call_arg_struct_by_addr_elf_c(void *arena, int32_t fa_ref) {
  int32_t fty;
 ...
 */
int32_t glue_field_call_arg_try_load_agg_from_rax_elf_c(void *arena,
                                                               void *elf_ctx,
 ...
 */
int32_t glue_field_access_call_base_rvalue_elf_c(void *arena,
                                                        void *elf_ctx, int32_t expr_ref,
                                                        void *ctx, int32_t ta, int32_t leave_addr) {
  int32_t base_ref;
  int32_t base_ko;
  int32_t base_ty;
  int32_t ret_sz;
  ...
  ly = (w151_LyMirror *)pipeline_asm_ctx_layout(ctx);
  if (!ly)
    return -1;
 ...
 */
int32_t pipeline_asm_emit_var_field_access_elf_c(void *arena,
                                                        void *elf_ctx, int32_t expr_ref,
                                                        void *ctx, int32_t ta) {
  int32_t base_ref;
  int32_t vlen;
...
int32_t pipeline_asm_emit_field_access_elf_fast_c(void *arena,
                                                           void *elf_ctx,
                                                           int32_t expr_ref, void *ctx,
                                                           int32_t ta) {
  int32_t fa_base2;
  int32_t blen2;
  if (pipeline_expr_field_access_is_enum_variant(arena, expr_ref) != 0)
  ...
  if (fa_base2 > 0 && pipeline_expr_kind_ord_at(arena, fa_base2) == 47) {
    ...
    if (pipeline_expr_field_access_soa_stride(arena, expr_ref) <= 0 && g_pipeline_asm_emit_module != 0)
...
void *glue_arena_expr_at_ref(void *a, int32_t expr_ref);
 ...
 */
int32_t glue_dep_layout_field_offset_by_name_c(void *ctx, uint8_t *field_name,
                                                        int32_t flen) {
  int32_t nd;
  int32_t di;
  int32_t k;
...
int32_t glue_field_layout_offset_for_var_base_field(void *a, void *m,
  ...
  if (pipeline_expr_kind_ord_at(a, base_var_ref) == 3) {
    ...
    if (fi >= 0 && fi < pipeline_module_num_funcs(m) && vlen > 0 && vlen <= 63) {
  ...
  if (base_ty <= 0 && pipeline_expr_kind_ord_at(a, base_var_ref) == 3 && g_pipeline_asm_emit_func_index >= 0 &&
      g_pipeline_asm_emit_func_index < pipeline_module_num_funcs(m)) {
    vlen = pipeline_expr_var_name_len(a, base_var_ref);
    if (vlen > 0 && vlen <= 63) {
  ...
  if (base_ty <= 0 && pipeline_module_num_struct_layouts_at(m) == 1) {
  ...
  if (base_ty <= 0)
    return -1;
  tp = (w151_TypeMirror *)pipeline_arena_type_ptr(a, base_ty);
  if (!tp)
    return -1;
 ...
 */
int32_t glue_field_access_effective_offset_c(void *arena, void *mod,
                                                     int32_t fa_ref) {
...
int32_t pipeline_expr_field_access_layout_offset(void *a, void *m, int32_t expr_ref) {
  ...
  if (!a || expr_ref <= 0)
    return 0;
  ex = (w151_ExprMirror *)glue_arena_expr_at_ref(a, expr_ref);
  if (!ex)
    return 0;
  ...
  if (m && pipeline_module_num_struct_layouts_at(m) == 0 && g_pipeline_asm_emit_dep_pipe) {
...
int32_t glue_field_access_load_bytes_for_type_ref(void *a, int32_t ty_ref) {
  int32_t kind_ord;
  if (!a || ty_ref <= 0 || ty_ref > ((int32_t *)a)[0])
    return 8;
 ...
 */
int32_t pipeline_expr_field_access_load_byte_sz(void *a, void *m, int32_t expr_ref) {
  w151_ExprMirror *ex;
  int32_t tr;
  int32_t base_tr;
  uint8_t struct_name[128];
 ...
 */
int glue_enum_field_name_equal(const uint8_t *field_buf, int32_t flen, const char *expect) {
  int32_t elen;
 ...
 */
void pipeline_expr_set_field_access_enum_variant(void *a, int32_t expr_ref, int32_t tag) {
  w151_ExprMirror *ex;

  if (!a || expr_ref <= 0 || expr_ref > ((int32_t *)a)[1])
    return;
  ex = (w151_ExprMirror *)glue_arena_expr_at_ref(a, expr_ref);
  if (!ex)
    return;
...
void pipeline_expr_set_field_access_soa_stride(void *a, int32_t expr_ref, int32_t stride) {
  void *ex = (w151_ExprMirror *)glue_arena_expr_at_ref(a, expr_ref);
  if (ex)
    ex->field_access_soa_stride = stride;
}

/** Read expr.field_access_is_enum_variant flag (1 if set, 0 otherwise). */
int32_t pipeline_expr_field_access_is_enum_variant(void *a, int32_t expr_ref) {
  void *ex = (w151_ExprMirror *)glue_arena_expr_at_ref(a, expr_ref);
