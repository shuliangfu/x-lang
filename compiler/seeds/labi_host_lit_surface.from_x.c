 ...
 * Regen: ./shux -E ... src/runtime/labi_host_lit.x | filter DBG + polish prologue
 */
#include <stdint.h>
#include <stddef.h>
#include <sys/types.h>
extern int32_t shux_host_is_linux(void);
extern int32_t shux_host_is_apple_aarch64(void);
extern int32_t labi_host_lit_count(void);
extern int32_t shux_host_is_linux_impl(void);
extern int32_t shux_host_is_apple_aarch64_impl(void);
int32_t shux_host_is_linux(void) {
  {
    return shux_host_is_linux_impl();
  }
  return 0;
}
int32_t shux_host_is_apple_aarch64(void) {
  {
