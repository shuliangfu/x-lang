/* seeds/backend_call_dispatch_thin_surface.from_x.c
 * G-02f backend_call_dispatch_thin R2 mixed surface - isomorphic with src/asm/backend_call_dispatch_thin.x
 * Product PREFER_X_O: xlang-c -E(.x) -> thin.o + ld -r with rest (seeds/backend_call_dispatch_thin.from_x.c)
 * Prove: full.x vs this surface -> nm IDENTICAL (44 symbols)
 * Mode: mixed - 6 DIRECT compute + 1 thin forward (pipeline_asm_call_param_type_ref_at_c)
 *       + 37 thin+rest forwards to _impl
 * Cap residual: 40 extern bridges (36 *_impl + 4 pipeline_*)
 * doc_anchor: not present in this module (no doc_anchor in 44 surface symbols).
 * Logic: 44 functions = 6 DIRECT compute
 *   (glue_asm_call_reg_max, glue_asm_call_stack_cleanup_bytes,
 *    glue_asm_append_export_c_suffix, glue_asm_string_lit_len,
 *    glue_call_param_is_f32_c, glue_asm_c_prefix_redundant_with_name)
 *   + 1 thin forward to pipeline_asm_call_param_type_ref_at_c
 *     (glue_call_param_type_ref_at)
 *   + 37 thin+rest forwards to _impl (glue_asm_import_path_segment_count_impl,
 *     glue_asm_import_path_slice_equal_impl, glue_asm_import_binding_name_equal_impl,
 *     glue_f32_xmm_flag_get_body_impl, glue_module_func_overload_count_c_impl,
 *     glue_asm_fill_c_prefix_from_module_import_impl,
 *     glue_asm_std_c_wrapper_fname_needs_export_c_suffix_impl,
 *     glue_asm_prefix_is_fmt_or_debug_impl, glue_asm_import_segment_at_impl,
 *     glue_asm_build_import_binding_call_sym_impl,
 *     glue_try_std_string_xlang_redirect_sym_local_impl,
 *     glue_try_std_encoding_redirect_sym_local_impl, glue_type_kind_to_suffix_c_impl,
 *     glue_asm_emit_string_lit_ptr_rax_elf_c_impl, glue_sysv_x86_call_n_stack_c_impl,
 *     glue_asm_build_dep_export_sym_c_impl, glue_asm_enc_call_redirected_impl,
 *     glue_try_std_heap_redirect_sym_local_impl, pipeline_asm_abi_f32_xmm_enabled_c_impl,
 *     glue_asm_build_func_export_sym_c_impl, glue_asm_emit_jmp_skip_string_then_lea_impl,
 *     glue_spill_struct16_call_arg_to_lea_elf_c_impl,
 *     pipeline_asm_resolve_whole_import_qualified_symbol_c_impl,
 *     pipeline_asm_emit_call_elf_c_impl, pipeline_asm_emit_method_call_elf_c_impl,
 *     pipeline_asm_emit_call_args_text_c_impl, pipeline_asm_emit_call_args_elf_c_impl,
 *     glue_emit_call_args_elf_sysv_f32_xmm_c_impl, glue_asm_emit_call_with_cleanup_impl,
 *     glue_emit_one_call_arg_elf_c_impl, glue_asm_build_call_export_sym_c_impl,
 *     glue_asm_try_emit_fmt_string_lit_import_call_elf_c_impl,
 *     pipeline_asm_emit_set_call_f32_xmm_impl,
 *     glue_codegen_import_path_to_c_prefix_into_impl, glue_asm_string_lit_into_impl,
 *     glue_sysv_x86_call_arg_slot_c_impl, glue_f32_xmm_flag_get_body_impl).
 *     Note: glue_f32_xmm_flag_get_body_impl is shared by 2 forwards
 *     (pipeline_asm_emit_get_call_f32_xmm_c + glue_f32_xmm_flag_get_body),
 *     so 37 forwards map to 36 unique _impl externs.
 * Regen: xlang_asm -E src/asm/backend_call_dispatch_thin.x | filter DBG + polish prologue
 */
#include <stdint.h>
#include <stddef.h>
/* Forward declarations for all 44 surface functions (nm IDENTICAL targets). */
...
/* Cap residual: 4 pipeline_* cross-module externs (DIRECT/thin forward targets). */
...
/* Cap residual: 36 _impl bridges (thin+rest forwards target these). */
...
int32_t glue_call_param_is_f32_c(uint8_t * arena, int32_t type_ref) {
  ...
  {
    int32_t k = pipeline_type_kind_ord_at(arena, type_ref);
    if ((k ==14)) {
      return 1;
    }
    if ((k ==15)) {
