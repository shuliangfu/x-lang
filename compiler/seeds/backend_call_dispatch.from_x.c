...
int32_t glue_asm_emit_jmp_skip_string_then_lea_impl(uint8_t *ctx_bytes, int32_t ta, int32_t reg_k,
   ...
   * Runtime link_abi_host_is_windows (G.7 with .x): leftover-PE must not
   * bake SysV from a non-_WIN32 compile of this seed. */
  {
    extern int link_abi_host_is_windows(void);
    int is_win = link_abi_host_is_windows();
    if (reg_k == 0)
...
static int32_t try_emit_va_cap_ov_select_elf_c(struct platform_elf_ElfCodegenCtx *elf_ctx,
  ...
  /* Class BK: Cap residual body retired (always miss). */
  return 0;
}
