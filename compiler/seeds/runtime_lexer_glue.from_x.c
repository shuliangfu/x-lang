 ...
 * lexer_next（XLANG ABI：Lexer, u8[] -> LexerResult）签名不兼容，无法由 .x 替代。
 * no_c 模式不链 lexer.o，lexer_next 由 lexer_x.o 提供；LEGACY 模式（XLANG_LEGACY_C_FRONTEND=1）
 * 链 lexer.o 但缺 lexer_next 会链接失败 — 这是 G-02a 预期行为（LEGACY 模式已废弃，
 * 见 Makefile "G-02a：C 前端已删；XLANG_LEGACY_C_FRONTEND=1 不再支持"）。
