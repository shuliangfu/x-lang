 ...
 * G-02f backend_arch_emit_dispatch_thin R2 mixed surface - isomorphic with src/asm/backend_arch_emit_dispatch_thin.x
 * Product PREFER_X_O: xlang_asm -E(.x) -> thin.o + ld -r with rest (seeds/backend_arch_emit_dispatch_thin.from_x.c)
 * Prove: full.x vs this surface -> nm IDENTICAL (47 symbols)
 * Mode: mixed - 47 thin dispatch forwards to arch_<arch>_emit_* by target arch (ta: 1=arm64, 2=riscv64, else=x86_64)
 * Cap residual: 135 extern bridges (45 arch_arm64_emit_* + 45 arch_riscv64_emit_* + 45 arch_x86_64_emit_*)
 * doc_anchor: none (not present in .x)
 * Logic: 47 dispatch functions = 43 pure dispatch (all 3 arches have impl)
 *   + 4 dispatch with DIRECT return 0 fallback (arch doesn't support op):
 *     mov_rax_to_arg_reg (arm64 returns 0), mov_rbx_to_ecx (arm64 returns 0),
 *     ldr_sp_offset_to_wi (non-arm64 returns 0), add_sp_imm (non-arm64 returns 0).
 * Regen: xlang_asm -E src/asm/backend_arch_emit_dispatch_thin.x | filter DBG + polish prologue
 */
#include <stdint.h>
#include <stddef.h>
/* Cap residual: 45 arch_arm64_emit_* bridges (defined in arch_arm64 rest C). */
...
/* Cap residual: 45 arch_riscv64_emit_* bridges (defined in arch_riscv64 rest C). */
...
/* Cap residual: 45 arch_x86_64_emit_* bridges (defined in arch_x86_64 rest C). */
