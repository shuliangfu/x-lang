 ...
 *
 * 提供 runtime_driver.o 调用的 pipeline_run_x_pipeline 包装；实现体为 pipeline_run_x_pipeline_impl
 *（pipeline_x.o）或 build_asm/pipeline.o 的 run_x_pipeline_impl（经 pipeline_run_impl_alias.o 别名）。
 */
#include <stddef.h>
#include <stdint.h>
...
/** runtime.c 经 buf+len 调 pipeline；转发至 .x pipeline 实现符号。 */
