...
void parser_parse_block_into(struct ast_ASTArena * restrict arena, struct lexer_Lexer lex_after_lbrace, struct shux_slice_uint8_t * source, int32_t type_ref, struct parser_ParseBlockResult * restrict out) {
  ...
  while (1) {
    ...
    (stmt_tok_ready = (0));
    (void)(({ int32_t __tmp = 0; if (((r).tok).kind == token_TokenKind_TOKEN_RBRACE) {   break;
 } else (__tmp = 0) ; __tmp; }));
    (void)(({ int32_t __tmp = 0; if (((r).tok).kind == token_TokenKind_TOKEN_EOF) {   ((out)->ok = (0));
 return;
 } else (__tmp = 0) ; __tmp; }));
...
void parser_parse_one_function_impl(struct parser_OneFuncResult * restrict out, struct ast_ASTArena * restrict arena, struct lexer_Lexer lex, struct shux_slice_uint8_t * source) {
  ...
  (lex = (ret_kw_lex));
    ...
    (void)(({ int32_t __tmp = 0; if (((r).tok).kind == token_TokenKind_TOKEN_RETURN || ((r).tok).kind == token_TokenKind_TOKEN_RBRACE || ((r).tok).kind == token_TokenKind_TOKEN_MATCH || ((r).tok).kind == token_TokenKind_TOKEN_EOF) {   break;
 } else (__tmp = 0) ; __tmp; }));
