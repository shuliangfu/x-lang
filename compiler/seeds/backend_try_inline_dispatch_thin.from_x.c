 ...
 * G-02f backend_try_inline_dispatch L2 thin surface — isomorphic with src/asm/backend_try_inline_dispatch_thin.x
 * Product PREFER_X_O: g05_try_x_to_o(thin.x) + full seed rest (-DSHUX_L2_TRY_INLINE_THIN_FROM_X) ld -r
 * Cold full seed: seeds/backend_try_inline_dispatch.from_x.c (unchanged)
 * Prove: thin.x vs this seed → nm IDENTICAL (public surface; try_inline_* heavy C tail are U)
 * Regen: ./shux -E ... src/asm/backend_try_inline_dispatch_thin.x | filter DBG + polish externs
 */
#include <stddef.h>
#include <stdint.h>
#include <sys/types.h>
