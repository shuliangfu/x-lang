...
void pipeline_codegen_try_mark_enum_field_access(struct ast_Module * m, struct ast_ASTArena * a, int32_t expr_ref, struct ast_PipelineDepCtx * dep_ctx);
...
SHUX_LIB_WEAK int32_t codegen_emit_prefix_len_from_ctx(struct ast_PipelineDepCtx * ctx, uint8_t * buf, int32_t buf_cap) {
  ...
  if ((ctx)->current_codegen_dep_index < 0 && (ctx)->entry_module_import_path_len > 0) {
    ...
    while (pi < (ctx)->entry_module_import_path_len && pi < buf_cap - 1) {
      ((buf)[pi] = ((pi < 0 || (pi) >= 64 ? (shux_panic_(1, 0), ((ctx)->entry_module_import_path_mirror)[0]) : ((ctx)->entry_module_import_path_mirror)[pi])));
      ++pi;
    }
    ((buf)[pi] = (((uint8_t)(0))));
    return pi;
...
SHUX_LIB_WEAK int32_t codegen_emit_block_final_expr(struct ast_ASTArena * arena, struct codegen_CodegenOutBuf * out, int32_t block_ref, int32_t final_ref, int32_t indent, struct ast_PipelineDepCtx * ctx, int32_t fn_ret_void) {
  ...
  /* Nested final: emit `expr;` (not return, not (void)expr). (void) breaks GNU stmt-expr value for unsafe{}. */
  if (parent_br > 0) {
    uint8_t endb[4] = { 59, 10, 0, 0 };
    ...
    return codegen_emit_bytes_from_ptr(out, (&((endb)[0])), 2);
  }
...
SHUX_LIB_WEAK int32_t codegen_emit_type(struct ast_ASTArena * arena, struct codegen_CodegenOutBuf * out, int32_t type_ref, uint8_t * struct_prefix, int32_t struct_prefix_len, struct ast_PipelineDepCtx * ctx) {
  ...
  /* i16 */
  if (name_len == 3 && (nm)[0] == 105 && (nm)[1] == 49 && (nm)[2] == 54) {   uint8_t i16_t[8] = { 105, 110, 116, 49, 54, 95, 116, 0 };
  return codegen_emit_bytes_8(out, i16_t, 7);
 }
  /* i8 */
  if (name_len == 2 && (nm)[0] == 105 && (nm)[1] == 56) {   uint8_t i8_t[7] = { 105, 110, 116, 56, 95, 116, 0 };
  return codegen_emit_bytes_8(out, i8_t, 6);
 }
  /* vector spellings as TYPE_NAMED → i32x4_t etc (not struct ast_i32x4) */
  if (name_len == 5 && (nm)[0] == 105 && (nm)[1] == 51 && (nm)[2] == 50 && (nm)[3] == 120 && (nm)[4] == 52) {
    return codegen_emit_vector_c_type_out(out, ((int32_t)(ast_TypeKind_TYPE_I32)), 4);
  }
  if (name_len == 5 && (nm)[0] == 105 && (nm)[1] == 51 && (nm)[2] == 50 && (nm)[3] == 120 && (nm)[4] == 56) {
    return codegen_emit_vector_c_type_out(out, ((int32_t)(ast_TypeKind_TYPE_I32)), 8);
  }
  if (name_len == 5 && (nm)[0] == 117 && (nm)[1] == 51 && (nm)[2] == 50 && (nm)[3] == 120 && (nm)[4] == 52) {
    return codegen_emit_vector_c_type_out(out, ((int32_t)(ast_TypeKind_TYPE_U32)), 4);
  }
  if (name_len == 5 && (nm)[0] == 117 && (nm)[1] == 51 && (nm)[2] == 50 && (nm)[3] == 120 && (nm)[4] == 56) {
    return codegen_emit_vector_c_type_out(out, ((int32_t)(ast_TypeKind_TYPE_U32)), 8);
  }
  if (name_len == 6 && (nm)[0] == 105 && (nm)[1] == 51 && (nm)[2] == 50 && (nm)[3] == 120 && (nm)[4] == 49 && (nm)[5] == 54) {
    return codegen_emit_vector_c_type_out(out, ((int32_t)(ast_TypeKind_TYPE_I32)), 16);
  }
  if (name_len == 6 && (nm)[0] == 117 && (nm)[1] == 51 && (nm)[2] == 50 && (nm)[3] == 120 && (nm)[4] == 49 && (nm)[5] == 54) {
    return codegen_emit_vector_c_type_out(out, ((int32_t)(ast_TypeKind_TYPE_U32)), 16);
  }
  ...
  /* dep module user enum: emit "enum <prefix>_<name>" */
  if (ctx != ((struct ast_PipelineDepCtx *)(0))) {   uint8_t dep_enum_prefix[128] = { 0 };
  int32_t dep_enum_prefix_len = codegen_type_dep_enum_prefix_into(ctx, arena, type_ref, (&((dep_enum_prefix)[0])), 128);
  if (dep_enum_prefix_len > 0) {   uint8_t e[8] = { 101, 110, 117, 109, 32, 0, 0, 0 };
  if (codegen_emit_bytes_8(out, e, 5) != 0) {   return (-1);
 }
  if (codegen_emit_bytes_from_ptr(out, (&((dep_enum_prefix)[0])), dep_enum_prefix_len) != 0) {   return (-1);
 }
  { int32_t bare_off2 = 0; int32_t bi2 = 0;
    while (bi2 < name_len && bi2 < 64) { if (nm[bi2] == 46) bare_off2 = bi2 + 1; ++bi2; }
    int32_t ci2 = bare_off2;
    while (ci2 < name_len && ci2 < 64) { if (codegen_append_byte_u8(out, nm[ci2]) != 0) { return (-1); } ++ci2; }
  }
  return 0;
 }
 }
  ...
  /* qualified-name fallback: if dep_prefix_len == 0 and name has '.', extract module path prefix */
  if (dep_prefix_len == 0) {   int32_t qmod_end = 0; int32_t qhas_dot = 0; int32_t qi = 0;
    while (qi < name_len && qi < 64) { if (nm[qi] == 46) { qhas_dot = 1; qmod_end = qi; } ++qi; }
    if (qhas_dot && qmod_end > 0 && qmod_end < 64) {   uint8_t mod_path[64] = { 0 }; int32_t mi = 0;
      while (mi < qmod_end) { mod_path[mi] = nm[mi]; ++mi; }
      mod_path[mi] = 0;
      codegen_import_path_to_c_prefix_into((&((mod_path)[0])), (&((dep_prefix_buf)[0])), 128);
      dep_prefix_len = 0;
      while (dep_prefix_len < 128 && (dep_prefix_buf)[dep_prefix_len] != ((uint8_t)(0))) { ++dep_prefix_len; }
    }
  }
  /* dep_prefix (defining module) takes priority over struct_prefix (current module) */
  if (dep_prefix_len > 0) {   if (codegen_emit_bytes_from_ptr(out, (&((dep_prefix_buf)[0])), dep_prefix_len) != 0) {   return (-1);
 }
 } else   if (struct_prefix != ((uint8_t *)(0)) && struct_prefix_len > 0) {   if (codegen_emit_bytes_from_ptr(out, struct_prefix, struct_prefix_len) != 0) {   return (-1);
 }
 } else   if (ctx != ((struct ast_PipelineDepCtx *)(0)) && (ctx)->current_codegen_module != ((struct ast_Module *)(0)) && codegen_type_is_module_user_struct((ctx)->current_codegen_module, arena, type_ref) != 0) {   uint8_t cur_pre[128] = { 0 };
  ...
  /* emit bare name (after last '.') */
  { int32_t bare_off2 = 0; int32_t bi2 = 0;
    while (bi2 < name_len && bi2 < 64) { if (nm[bi2] == 46) bare_off2 = bi2 + 1; ++bi2; }
    return codegen_emit_bytes_64(out, (&((nm)[0])) + bare_off2, name_len - bare_off2);
  }
 }
...
SHUX_LIB_WEAK int32_t codegen_should_skip_emit_struct_layout_for_abi_dup(uint8_t * name, int32_t name_len) {
  ...
  /* rt_preamble: struct std_string_String / StrView — skip when entry is std.string. */
...
SHUX_LIB_WEAK int32_t codegen_emit_expr(struct ast_ASTArena * arena, struct codegen_CodegenOutBuf * out, int32_t expr_ref, struct ast_PipelineDepCtx * ctx) {
 ...
 } else if (ctx != ((struct ast_PipelineDepCtx *)(0)) && (ctx)->current_codegen_prefix_len > 0) {
    /* Entry module: use lib_name prefix from ctx (set by runtime). */
    int32_t _cpl = (ctx)->current_codegen_prefix_len;
    if (_cpl > 0 && _cpl < 128) { memcpy((&((cur_pre)[0])), (ctx)->current_codegen_prefix_mirror, (size_t)_cpl); (cur_pre)[_cpl] = 0; }
  ...
  /* 末路 CALL：import 绑定 FIELD_ACCESS 须走 emit_call_func_name 重载 mangle。
   * 旧逻辑 emit_expr_as_callee → import_module_field_symbol 只发 prefix+原名（std_fmt_print），
   * 与定义端 print_u8_ptr_i32 不一致（hello fmt.print(msg,12) cc 失败）。 */
  if (ctx != ((struct ast_PipelineDepCtx *)(0)) && (ctx)->current_codegen_module != ((struct ast_Module *)(0))
      && (!ast_ref_is_null((e).call_callee_ref)) && (e).call_callee_ref > 0 && (e).call_callee_ref <= (arena)->num_exprs) {
    struct ast_Expr callee_fb = ast_arena_expr_get(arena, (e).call_callee_ref);
    if ((callee_fb).kind == ast_ExprKind_EXPR_FIELD_ACCESS && (callee_fb).field_access_field_len > 0
        && (callee_fb).field_access_base_ref > 0 && (callee_fb).field_access_base_ref <= (arena)->num_exprs) {
      struct ast_Expr base_fb = ast_arena_expr_get(arena, (callee_fb).field_access_base_ref);
      if ((base_fb).kind == ast_ExprKind_EXPR_VAR && (base_fb).var_name_len > 0) {
        struct ast_Module * cur_fb = (ctx)->current_codegen_module;
        int32_t j_fb = 0;
        int32_t nd_fb = pipeline_dep_ctx_ndep(ctx);
        int32_t n_imp_fb = codegen_module_num_imports(cur_fb);
        while (j_fb < n_imp_fb && j_fb < nd_fb) {
          if (pipeline_module_import_kind_at(cur_fb, j_fb) == 1) {
            int32_t bl = pipeline_module_import_binding_name_len(cur_fb, j_fb);
            if (bl == (base_fb).var_name_len) {
              int eqb = 1;
              int32_t kk = 0;
              while (kk < bl && kk < 64) {
                if ((base_fb).var_name[kk] != pipeline_module_import_binding_name_byte_at(cur_fb, j_fb, kk)) { eqb = 0; break; }
                kk = kk + 1;
              }
              if (eqb) {
                uint8_t dpb[64] = { 0 };
                int32_t dpl = codegen_module_import_path_len_at(cur_fb, j_fb, (&((dpb)[0])));
                if (dpl > 0) {
                  uint8_t pref[128] = { 0 };
                  (void)(codegen_import_path_to_c_prefix_into((&((dpb)[0])), (&((pref)[0])), 128));
                  int32_t pl = 0;
                  while (pl < 128 && pref[pl] != 0) { pl = pl + 1; }
                  if (pl > 0 && codegen_c_prefix_redundant_with_name((&((pref)[0])), pl, (&(((callee_fb).field_access_field_name)[0])), (callee_fb).field_access_field_len) == 0
                      && codegen_emit_bytes_from_ptr(out, (&((pref)[0])), pl) != 0) { return (-1); }
                  int32_t bix = codegen_find_dep_index_by_path(ctx, (&((dpb)[0])), dpl);
                  struct ast_Module * dmod = cur_fb;
                  if (bix >= 0 && bix < nd_fb) dmod = pipeline_dep_ctx_module_at(ctx, bix);
                  if (codegen_emit_call_func_name(out, arena, ctx, expr_ref, dmod, (&(((callee_fb).field_access_field_name)[0])), (callee_fb).field_access_field_len) != 0) { return (-1); }
                  if (codegen_append_byte(out, 40) != 0) { return (-1); }
                  int32_t na = (e).call_num_args;
                  int32_t ai = 0;
                  while (ai < na) {
                    if (ai > 0) { uint8_t cm[3] = { 44, 32, 0 }; if (codegen_emit_bytes_3(out, cm, 2) != 0) return (-1); }
                    if (ast_ref_is_null(pipeline_expr_call_arg_ref(arena, expr_ref, ai))) {
                      if (codegen_append_byte(out, 48) != 0) return (-1);
                    } else if (codegen_emit_expr(arena, out, pipeline_expr_call_arg_ref(arena, expr_ref, ai), ctx) != 0) {
                      return (-1);
                    }
                    ai = ai + 1;
                  }
  ...
  /* 【Why】错 call_resolved → 盲信会把 driver.submit_* 发成 std_fmt_append_*；须名+arity 校验。 */
  ...
  if (fn_len > 0 && fn_len == (e).method_call_name_len && (e).method_call_name_len > 0) {
    int32_t mi = 0; mc_name_ok = 1;
    while (mi < fn_len) {
      if (fn_name[mi] != ((e).method_call_name)[mi]) { mc_name_ok = 0; break; }
      mi++;
    }
  }
  if (mc_name_ok != 0 && pipeline_module_func_num_params_at(dep_mod, func_ix) == (e).method_call_num_args) {
  uint8_t dep_path[64] = { 0 };
  (void)(pipeline_dep_ctx_import_path_copy64(ctx, dep_ix, (&((dep_path)[0]))));
  uint8_t pre_buf[128] = { 0 };
 ...
 } else   if ((ty).kind == ast_TypeKind_TYPE_VECTOR) {   (is_vector = (1));
 } else   if ((ty).kind == ast_TypeKind_TYPE_NAMED && (ty).name_len >= 5) {
  int32_t ni = 0; while (ni < (ty).name_len) { if ((ty).name[ni] == 120) { is_vector = 1; ni = (ty).name_len; } else ++ni; }
 }

 }
  if (is_vector != 0) {
    /* (vec_ty){ e0, e1, ... } — not (uint8_t[]){...} into vector */
    if (codegen_append_byte(out, 40) != 0) { return (-1); }
    if (codegen_emit_type(arena, out, (e).resolved_type_ref, ((uint8_t *)(0)), 0, ctx) != 0) { return (-1); }
    if (codegen_append_byte(out, 41) != 0) { return (-1); }
    if (codegen_append_byte(out, 123) != 0) { return (-1); }
    int32_t vai = 0;
    while (vai < n) {
      if (vai > 0) { uint8_t comma[3] = { 44, 32, 0 }; if (codegen_emit_bytes_3(out, comma, 2) != 0) { return (-1); } }
      if ((!ast_ref_is_null(pipeline_expr_array_lit_elem_ref(arena, expr_ref, vai))) && codegen_emit_expr(arena, out, pipeline_expr_array_lit_elem_ref(arena, expr_ref, vai), ctx) != 0) { return (-1); }
      ++vai;
    }
...
SHUX_LIB_WEAK int32_t codegen_emit_block(struct ast_ASTArena * arena, struct codegen_CodegenOutBuf * out, int32_t block_ref, int32_t indent, struct ast_PipelineDepCtx * ctx) {
  ...
  if ((callee_let).kind == ast_ExprKind_EXPR_VAR) {   if (codegen_callee_var_is_string_new(callee_let) != 0) {   uint8_t str_ty[7] = { 83, 116, 114, 105, 110, 103, 0 };
  if (codegen_emit_bytes_from_ptr(out, (&((str_ty)[0])), 6) != 0) {   return (-1);
 }
  if (codegen_append_byte(out, 32) != 0) {   return (-1);
 }
  ...
  /* vector TYPE_NAMED / TYPE_VECTOR: lit 0 → { 0 }; ARRAY_LIT → braced init */
  int32_t use_vec_z = 0;
  int32_t use_vec_braced = 0;
  if ((!ast_ref_is_null(linit_ref)) && linit_ref > 0 && linit_ref <= (arena)->num_exprs && (!ast_ref_is_null(let_type_ref))) {
    struct ast_Expr init_ez = ast_arena_expr_get(arena, linit_ref);
    int32_t tk_z = pipeline_type_kind_ord_at(arena, let_type_ref);
    int32_t is_vec_ty = 0;
    if (tk_z == ((int32_t)(ast_TypeKind_TYPE_VECTOR))) is_vec_ty = 1;
    else if (tk_z == ((int32_t)(ast_TypeKind_TYPE_NAMED))) {
      uint8_t vzn[64] = {0}; int32_t vzn_l = pipeline_type_named_name_into(arena, let_type_ref, vzn);
      int32_t vi = 0; while (vi < vzn_l) { if (vzn[vi] == 120) { is_vec_ty = 1; vi = vzn_l; } else ++vi; }
    }
    if (is_vec_ty != 0) {
      if ((init_ez).kind == ast_ExprKind_EXPR_LIT && (init_ez).int_val == 0) use_vec_z = 1;
      else if ((init_ez).kind == ast_ExprKind_EXPR_ARRAY_LIT) use_vec_braced = 1;
    }
  }
  if (use_vec_z != 0) {
    uint8_t vz[6] = { 123, 32, 48, 32, 125, 0 };
    if (codegen_emit_bytes_6(out, vz, 5) != 0) { return (-1); }
  } else if (use_vec_braced != 0) {
    if (codegen_emit_braced_array_lit_init(arena, out, linit_ref, ctx) != 0) { return (-1); }
  } else if (codegen_emit_expr(arena, out, linit_ref, ctx) != 0) { return (-1); }
 }
  ...
  /* No braces when region has no lets/consts — keeps GNU stmt-expr value (unsafe{call}). */
  int32_t need_scope = 0;
  if ((!ast_ref_is_null(reg_body)) && reg_body > 0 && reg_body <= (arena)->num_blocks) {
    if (ast_block_num_lets(arena, reg_body) > 0 || ast_block_num_consts(arena, reg_body) > 0) need_scope = 1;
  }
  if (need_scope != 0) {
  if (codegen_emit_indent(out, indent) != 0) {   return (-1);
 }
  ...
  if (codegen_emit_block(arena, out, reg_body, indent, ctx) != 0) {   return (-1);
 }
  }
 }
 }
  ...
  if ((n02 < 0 || (n02) >= 64 ? (shux_panic_(1, 0), ((rt2).name)[0]) : ((rt2).name)[n02]) == 83 && (n02 + 1 < 0 || (n02 + 1) >= 64 ? (shux_panic_(1, 0), ((rt2).name)[0]) : ((rt2).name)[n02 + 1]) == 116 && (n02 + 2 < 0 || (n02 + 2) >= 64 ? (shux_panic_(1, 0), ((rt2).name)[0]) : ((rt2).name)[n02 + 2]) == 114 && (n02 + 3 < 0 || (n02 + 3) >= 64 ? (shux_panic_(1, 0), ((rt2).name)[0]) : ((rt2).name)[n02 + 3]) == 105 && (n02 + 4 < 0 || (n02 + 4) >= 64 ? (shux_panic_(1, 0), ((rt2).name)[0]) : ((rt2).name)[n02 + 4]) == 110 && (n02 + 5 < 0 || (n02 + 5) >= 64 ? (shux_panic_(1, 0), ((rt2).name)[0]) : ((rt2).name)[n02 + 5]) == 103) {   uint8_t str_ty2a[7] = { 83, 116, 114, 105, 110, 103, 0 };
  ...
  if ((callee_let2).kind == ast_ExprKind_EXPR_VAR) {   if (codegen_callee_var_is_string_new(callee_let2) != 0) {   uint8_t str_ty2[7] = { 83, 116, 114, 105, 110, 103, 0 };
...
/* 【Why 根源】CALL 端：resolved 仅在名+arity 一致时用；link_name 必须用目标模块 arena
 * （错 arena → print_u8 而非 print_u8_ptr_usize）。与 codegen.x 同源。 */
static struct ast_ASTArena *codegen_arena_for_module_c(struct ast_PipelineDepCtx *ctx,
    struct ast_Module *module, struct ast_ASTArena *fallback) {
  int32_t di, nd;
  if (!ctx || !module) return fallback;
  nd = pipeline_dep_ctx_ndep(ctx);
  for (di = 0; di < nd; di++) {
    if (pipeline_dep_ctx_module_at(ctx, di) == module) {
      struct ast_ASTArena *da = pipeline_dep_ctx_arena_at(ctx, di);
      return da ? da : fallback;
    }
  }
  return fallback;
}
...
int32_t codegen_emit_call_func_name(struct codegen_CodegenOutBuf * out, struct ast_ASTArena * arena, struct ast_PipelineDepCtx * ctx, int32_t expr_ref, struct ast_Module * current_module, uint8_t * fallback_name, int32_t fallback_len) {
  if (ctx != 0 && arena != 0) {
    ...
    /* 实参个数 + 名：resolved 指向错函数时勿盲信（uring_ok→print_u8_ptr_usize）。 */
    {
      ...
      if (func_ix >= 0) {
        ...
        if (res_mod != 0 && func_ix < (res_mod)->num_funcs) {
          ...
          if (pipeline_module_func_num_params_at(res_mod, func_ix) != nargs0)
            ok_res = 0;
          if (ok_res && fallback_len > 0) {
            int32_t rlen = pipeline_module_func_name_len_at(res_mod, func_ix);
            if (rlen != fallback_len) ok_res = 0;
            else {
              uint8_t rnm[64] = {0};
              int32_t ri;
              pipeline_module_func_name_copy64(res_mod, func_ix, rnm);
              for (ri = 0; ri < rlen; ri++) {
                if (rnm[ri] != fallback_name[ri]) { ok_res = 0; break; }
              }
            }
          }
          /* overload: name+arity only is insufficient; re-search by arg types */
          if (ok_res && fallback_len > 0
              && codegen_module_func_overload_count(res_mod, fallback_name, fallback_len) > 1)
            ok_res = 0;
          if (ok_res) {
            struct ast_ASTArena *res_arena = codegen_arena_for_module_c(ctx, res_mod, arena);
...
SHUX_LIB_WEAK int32_t codegen_type_dep_enum_prefix_into(struct ast_PipelineDepCtx * ctx, struct ast_ASTArena * arena, int32_t type_ref, uint8_t * dst, int32_t dst_cap) {
  int32_t name_len = 0;
  uint8_t ty_nm[64] = { 0 };
  int32_t di = 0;
  int32_t bare_off_e = 0;
  int32_t bare_len_e = 0;
  int32_t bi_e = 0;
  ...
  bare_off_e = 0;
  bi_e = 0;
  while (bi_e < name_len && bi_e < 64) { if (ty_nm[bi_e] == 46) bare_off_e = bi_e + 1; ++bi_e; }
  bare_len_e = name_len - bare_off_e;
  di = 0;
  while (di < pipeline_dep_ctx_ndep(ctx)) {
    struct ast_Module * dep_mod = pipeline_dep_ctx_module_at(ctx, di);
    if (dep_mod != ((struct ast_Module *)(0))) {   int32_t ei = 0;
  while (ei < (dep_mod)->num_module_enums) {
    int32_t dep_name_len = pipeline_module_enum_name_len(dep_mod, ei);
    if (dep_name_len == bare_len_e) {   int32_t eq = 1; int32_t j = 0;
  while (j < bare_len_e && j < 64) {
    if (pipeline_module_enum_name_byte_at(dep_mod, ei, j) != ty_nm[bare_off_e + j]) {   (eq = (0));
  break;
 }
    ++j;
  }
  ...
  int32_t out_len = 0;
  while (out_len < dst_cap && (dst)[out_len] != ((uint8_t)(0))) { ++out_len; }
  return out_len;
 }
 }
 }
...
SHUX_LIB_WEAK int32_t codegen_emit_module_struct_forward_declarations(struct ast_Module * module, struct codegen_CodegenOutBuf * out, uint8_t * struct_prefix, int32_t struct_prefix_len) {
  int32_t k = 0;
  while (k < (module)->num_struct_layouts) {
    int32_t nf = pipeline_module_struct_layout_num_fields(module, k);
    int32_t nl = pipeline_module_struct_layout_name_len(module, k);
  ...
  if (codegen_emit_bytes_from_ptr(out, (&((hdr)[0])), 7) != 0) {   return (-1);
 }
  if (struct_prefix != ((uint8_t *)(0)) && struct_prefix_len > 0) {   if (codegen_emit_bytes_from_ptr(out, struct_prefix, struct_prefix_len) != 0) {   return (-1);
 }
 }
  ...
  if (codegen_emit_bytes_from_ptr(out, (&((semi_nl)[0])), 2) != 0) {   return (-1);
 }
    ++k;
  }
  return 0;
}
SHUX_LIB_WEAK int32_t codegen_emit_dep_struct_forward_declarations(struct ast_PipelineDepCtx * ctx, struct codegen_CodegenOutBuf * out) {
  if (ctx == ((struct ast_PipelineDepCtx *)(0)) || out == ((struct codegen_CodegenOutBuf *)(0))) {   return 0;
 }
  int32_t nd = pipeline_dep_ctx_ndep(ctx);
  int32_t di = 0;
  while (di < nd) {
    struct ast_Module * dep_mod = pipeline_dep_ctx_module_at(ctx, di);
    if (dep_mod != ((struct ast_Module *)(0))) {   uint8_t dep_path[64] = { 0 };
  int32_t dep_path_len = codegen_dep_import_path_len_at(ctx, di, (&((dep_path)[0])));
  uint8_t prefix_buf[128] = { 0 };
  int32_t prefix_len = 0;
  if (dep_path_len > 0 && codegen_path_is_std_io_core_bytes((&((dep_path)[0]))) == 0) {   (void)(codegen_import_path_to_c_prefix_into((&((dep_path)[0])), (&((prefix_buf)[0])), 128));
  while (prefix_len < 128 && (prefix_len < 0 || (prefix_len) >= 128 ? (shux_panic_(1, 0), (prefix_buf)[0]) : (prefix_buf)[prefix_len]) != 0) { ++prefix_len; }
 }
  if (codegen_emit_module_struct_forward_declarations(dep_mod, out, (&((prefix_buf)[0])), prefix_len) != 0) {   return (-1);
 }
 }
    ++di;
  }
...
SHUX_LIB_WEAK int32_t codegen_x_ast(struct ast_Module * module, struct ast_ASTArena * arena, struct codegen_CodegenOutBuf * out, struct ast_PipelineDepCtx * ctx, int32_t dep_index) {
  ...
  /* Entry uses entry_module_import_path (C prefix pin); not current_* (dep-polluted). */
  if (prefix_len == 0 && dep_index < 0 && ctx != ((struct ast_PipelineDepCtx *)(0))) {
    if ((ctx)->entry_module_import_path_len > 0) {
      ...
      while (pi < (ctx)->entry_module_import_path_len && pi < 127) {
        ((prefix_buf)[pi] = ((pi < 0 || (pi) >= 64 ? (shux_panic_(1, 0), ((ctx)->entry_module_import_path_mirror)[0]) : ((ctx)->entry_module_import_path_mirror)[pi])));
        ++pi;
      }
      ((prefix_buf)[pi] = ((uint8_t)(0)));
      (prefix_len = (pi));
    }
  }
  ...
  if (any_let != 0 && dep_index < 0) {   uint8_t init_globals_def[32] = { 115, 116, 97, 116, 105, 99, 32, 118, 111, 105, 100, 32, 105, 110, 105, 116, 95, 103, 108, 111, 98, 97, 108, 115, 40, 118, 111, 105, 100, 41, 32, 0 };
  ...
  continue;
          ...
          if (dnlen > 0 && dnlen <= 63) {
            uint8_t dtl_name[64] = { 0 };
            int32_t dtni = 0;
            while (dtni < dnlen && dtni < 64) {
              ((dtni < 0 || (dtni) >= 64 ? (shux_panic_(1, 0), 0) : ((dtl_name)[dtni] = pipeline_module_top_level_let_name_byte_at(dep_mod, dti, dtni), 0)));
              ++dtni;
            }
            if (codegen_emit_bytes_from_ptr(out, (&((dtl_name)[0])), dnlen) != 0) { return (-1); }
          }
          uint8_t deq[4] = { 32, 61, 32, 0 };
          if (codegen_emit_bytes_4(out, deq, 3) != 0) { return (-1); }
          if ((!ast_ref_is_null(pipeline_module_top_level_let_init_ref(dep_mod, dti))) && codegen_emit_expr(dep_arena, out, pipeline_module_top_level_let_init_ref(dep_mod, dti), ctx) != 0) { return (-1); }
          uint8_t dsc[3] = { 59, 10, 0 };
          if (codegen_emit_bytes_3(out, dsc, 2) != 0) { return (-1); }
        }
        ++dti;
      }
    }
  ...
  if (dep_index < 0 && (module)->num_top_level_lets == 0) {
    ...
    if (dep_any) {
      uint8_t igd[32] = { 115, 116, 97, 116, 105, 99, 32, 118, 111, 105, 100, 32, 105, 110, 105, 116, 95, 103, 108, 111, 98, 97, 108, 115, 40, 118, 111, 105, 100, 41, 32, 0 };
      if (codegen_emit_bytes_from_ptr(out, (&((igd)[0])), 31) != 0) { return (-1); }
      uint8_t br[3] = { 123, 10, 0 };
  ...
  continue;
              ...
              if (dnl2 > 0 && dnl2 <= 63) {
                uint8_t dn2[64] = { 0 };
                int32_t dn_i2 = 0;
                while (dn_i2 < dnl2 && dn_i2 < 64) {
                  ((dn_i2 < 0 || (dn_i2) >= 64 ? (shux_panic_(1, 0), 0) : ((dn2)[dn_i2] = pipeline_module_top_level_let_name_byte_at(dm2, dt2, dn_i2), 0)));
                  ++dn_i2;
                }
                if (codegen_emit_bytes_from_ptr(out, (&((dn2)[0])), dnl2) != 0) { return (-1); }
              }
              uint8_t deq2[4] = { 32, 61, 32, 0 };
              if (codegen_emit_bytes_4(out, deq2, 3) != 0) { return (-1); }
              if ((!ast_ref_is_null(pipeline_module_top_level_let_init_ref(dm2, dt2))) && codegen_emit_expr(da2, out, pipeline_module_top_level_let_init_ref(dm2, dt2), ctx) != 0) { return (-1); }
              uint8_t dsc2[3] = { 59, 10, 0 };
              if (codegen_emit_bytes_3(out, dsc2, 2) != 0) { return (-1); }
            }
            ++dt2;
          }
        }
      ...
      uint8_t cb2[3] = { 125, 10, 0 };
      if (codegen_emit_bytes_3(out, cb2, 2) != 0) { return (-1); }
      call_init_globals = 1;
    }
  }
 }
