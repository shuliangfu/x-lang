 ...
 * Product PREFER_X_O: g05_try_x_to_o(full.x) + rest (-DXLANG_BACKEND_ARCH_EMIT_DISPATCH_FROM_X) ld -r
 ...
 * L2 thin hybrid（XLANG_L2_ARCH_EMIT_THIN_FROM_X）仍作 full.x 失败时的回退路径。
 ...
 */
#ifndef XLANG_BACKEND_ARCH_EMIT_DISPATCH_FROM_X
/**
...
#ifdef XLANG_L2_ARCH_EMIT_THIN_FROM_X
...
#ifndef XLANG_L2_ARCH_EMIT_THIN_FROM_X
int32_t backend_arch_emit_ret_imm32(struct codegen_CodegenOutBuf *out, int32_t imm, int32_t ta) {
  if (ta == 1)
...
#else /* XLANG_BACKEND_ARCH_EMIT_DISPATCH_FROM_X：产品 rest 业务 H=0 */
int backend_arch_emit_dispatch_slice_marker(void) {
  return 0;
}
#endif /* XLANG_BACKEND_ARCH_EMIT_DISPATCH_FROM_X */
