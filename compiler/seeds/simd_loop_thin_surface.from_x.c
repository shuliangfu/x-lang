/* seeds/simd_loop_thin_surface.from_x.c
 * G-02f simd_loop_thin R2 mixed surface - isomorphic with src/asm/simd_loop_thin.x
 * Product PREFER_X_O: xlang-c -E(.x) -> thin.o + ld -r with rest (seeds/simd_loop_thin.from_x.c)
 * Prove: full.x vs this surface -> nm IDENTICAL (22 #[no_mangle])
 * Mode: mixed - 5 DIRECT compute + 17 thin+rest forwards
 * Cap residual: 22 extern bridges (5 pipeline_* + 17 *_impl)
 * No doc_anchor (simd_loop_thin.x has none).
 * Note: glue_ prefix not trigger ast_ (confirmed wave545+).
 * Logic: 22 functions = 5 DIRECT (glue_f32_slot_rbp_disp32 + glue_soa_f32_col_rbp_disp32
 *   + glue_simd_loop_pick_lanes_c + glue_var_array_i32_size_c + glue_var_is_array_i32_n_c)
 *   + 17 thin+rest forwards to _impl.
 * Regen: ./xlang-c -E ... simd_loop_thin.x | filter DBG + polish prologue
 */
#include <stdint.h>
#include <stddef.h>
