 ...
 *
 * pipeline_wpo.o 导出 run_x_pipeline_impl；glue/driver 仍引用 pipeline_run_x_pipeline_impl。
 * X typecheck_entry thin bl 引用 run_x_pipeline_typecheck_entry_emit → ast_pool emit_c。
 * strict 链不链 C orchestration partial 时由本 TU 补齐，避免 duplicate run_x_pipeline_impl。
 */
#include <stdint.h>
#include <stddef.h>
...
/** glue / driver 统一入口名 → pipeline_wpo.o run_x_pipeline_impl。 */
...
/** X typecheck_entry thin bl → C emit（pipeline_wpo.o U 此符号）。 */
