...
/** 与 compile.x DriverCompileState 布局兼容；别名层仅透传指针。 */
struct driver_DriverCompileState;

extern int32_t run_compiler_full_x(int32_t argc, uint8_t *argv);
extern int32_t run_compiler_full_x_post_parse(struct driver_DriverCompileState *state, int32_t argc,
                                               uint8_t *argv);
...
int32_t driver_run_compiler_full_x_post_parse(struct driver_DriverCompileState *state, int32_t argc,
                                               uint8_t *argv) {
  return run_compiler_full_x_post_parse(state, argc, argv);
}

/** C-gen / gate 期望的 dispatch 符号 → asm X 真 emit 裸名。 */
