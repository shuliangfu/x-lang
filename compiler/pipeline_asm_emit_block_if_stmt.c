...
/* wave1206: pipeline_asm_fill_local_slots defined in pipeline_asm_emit_context.c
 * (#include at glue.c L2564, AFTER this file's #include at L2421). Extern fwd
 * decl needed for wave1206-migrated pipeline_asm_emit_if_then_block_body_elf_c
 * at EOF (calls fill_local_slots before backend_emit_block_body_sync_elf). */
...
/* ========================================================================== *
 * wave1206 G.7: pipeline_asm_emit_if_then_block_body_elf_c migrated from
 * pipeline_glue.c L2663-2683. Colocated with if-statement emit domain
 * (block_if_stmt.c #include at glue.c L2421).
 *
 * Members (1 fn):
 *  - pipeline_asm_emit_if_then_block_body_elf_c (if-then block ELF emit)
 *
 * Deps (all visible at #include point L2421):
 *  - pipeline_asm_ctx_layout (static at glue.c L86, before L2421)
 *  - pipeline_glue_AsmFuncCtxLayout (struct decl at top of glue.c)
 *  - pipeline_asm_fill_local_slots (extern, context.c L724 — #include L2564
 *    AFTER L2421, so needs extern fwd decl; but caller is seeds/backend.x
 *    via extern, no TU-internal callsite, so the late #include doesn't
 *    matter for this migrated def)
 *  - backend_emit_block_body_sync_elf (extern fwd decl at glue.c L1068)
 *
 * Callers: no TU-internal callsites. Sole callers are seeds
 * (backend.x L1282) via extern + ast_pool.c symbol table L9796.
 *
 * PLATFORM: SHARED — pure block body emit orchestration, no arch branch.
 * ========================================================================== */

/**
 * Emit ELF asm for an if-statement's then-block.
 *
 * Why: the then-block may introduce new locals (let/const inside the
 *      branch); these must be slot-allocated via fill_local_slots BEFORE
 *      emitting the block body, then the slot cursor (num_locals /
 *      next_offset) must be RESTORED after emit so the enclosing scope's
 *      slot layout is unaffected. Mirrors backend.x emit_if_then_block_body_elf.
 * Contract: NULL ctx/arena or then_block_ref<=0 -> -1.
 *           Otherwise returns backend_emit_block_body_sync_elf result.
 * Invariant: ly->num_locals and ly->next_offset are saved before
 *            fill_local_slots and restored after emit (branch-local slots
 *            do not leak to enclosing scope).
 * Asm/Perf: O(block_size) — dominated by backend_emit_block_body_sync_elf.
 * PLATFORM: SHARED.
 */
