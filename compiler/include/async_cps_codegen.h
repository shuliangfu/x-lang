 ...
 *
 * 文件职责：在协程帧 __shux_frame 上 emit switch(__phase) 分段；await 边界保存/恢复 live 字段。
...
/**
 * 在 __shux_frame 初始化后调用：hoist 块内 let 声明 + 打开 switch(default/case 0)。
 */
...
/** await 操作数是否为 IO 调用（read、write、submit_read、shux_io_ 前缀等）。 */
