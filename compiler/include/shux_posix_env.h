/* shux_posix_env.h — POSIX setenv/unsetenv shims for MinGW.
 ...
 *      setenv("SHUX_RUN_QUIET", "1", 1). Multiple seed files (runtime_env_os,
 ...
 * Guard SHUX_POSIX_ENV_SHIM_DEFINED prevents double static inline definitions
 ...
 *            for SHUX usage which only checks getenv != NULL).
 * PLATFORM: WINDOWS | MSYS | MINGW (only effective when _WIN32 defined). */
#ifndef SHUX_POSIX_ENV_H
#define SHUX_POSIX_ENV_H

#if defined(_WIN32) || defined(_WIN64)

#ifndef SHUX_POSIX_ENV_SHIM_DEFINED
#define SHUX_POSIX_ENV_SHIM_DEFINED

/* Forward-declare getenv / _putenv_s (MinGW <stdlib.h>) instead of pulling
 * in <stdlib.h>, which would conflict with SHUX-generated
...
#ifndef SHUX_POSIX_ENV_GETENV_DECLARED
#define SHUX_POSIX_ENV_GETENV_DECLARED
 ...
 *            removing the variable entirely; acceptable for SHUX usage. */
static inline int unsetenv(const char *name) {
    return _putenv_s(name, "") == 0 ? 0 : -1;
}

#endif /* SHUX_POSIX_ENV_SHIM_DEFINED */

#endif /* _WIN32 */

#endif /* SHUX_POSIX_ENV_H */
