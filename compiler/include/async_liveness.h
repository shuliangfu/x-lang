 ...
 * 所属模块：compiler 前端；供 codegen 输出帧类型与 XLANG_ASYNC_FRAME 注释。
 */
...
/** 在函数定义之前 emit `typedef struct __xlang_async_frame_* { ... }`（无 await 时不输出）。 */
void async_liveness_emit_frame_typedef(const struct ASTFunc *f,
    const AsyncFrameLayout *layout, FILE *out);

/** 函数体开头 emit 帧局部变量占位（sync stub；CPS 后续读写 __xlang_frame）。 */
