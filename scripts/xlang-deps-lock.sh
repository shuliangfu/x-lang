#!/usr/bin/env bash
# xlang-deps-lock.sh — TOOL-008 由 xlang.pkg.tsv 生成 xlang.pkg.lock.tsv
#
# 用法：./scripts/xlang-deps-lock.sh <xlang.pkg.tsv> [out.lock.tsv]
set -e
...
if [ -z "$MANIFEST" ] || [ ! -f "$MANIFEST" ]; then
  echo "xlang-deps-lock FAIL: missing manifest" >&2
  exit 1
fi
if [ -z "$OUT" ]; then
  OUT="$(dirname "$MANIFEST")/xlang.pkg.lock.tsv"
fi

tool_deps_read_manifest "$MANIFEST"
TMP="$(mktemp)"
{
  echo "# xlang.pkg.lock v1 — generated from $(basename "$MANIFEST")"
  echo "# lock_version	1"
  ...
  for req in "${REQUIRES[@]}"; do
    abs="$(tool_deps_resolve_abs "$req")" || {
      echo "xlang-deps-lock FAIL: unresolved $req" >&2
      rm -f "$TMP"
      exit 1
    }
...
mv "$TMP" "$OUT"
echo "xlang-deps-lock OK: wrote $OUT"
