...
run_one() {
  local img="$1"
  echo "===== Docker CI: $img ====="
  docker run --rm -e CI=1 -e SHUX_CI_DOCKER=1 -e SHUX_CC_EXTRA="-std=gnu11" \
    -v "$SRC:/src" -w /src \
    "$img" \
    sh -c '
...
run_ubuntu_gates() {
  ...
  docker run --rm --platform linux/amd64 -e CI=1 -e SHUX_CC_EXTRA="-std=gnu11" \
    ...
    sh -c '
      ...
      cd .. &&
      chmod +x tests/run-pre-push-p5.sh tests/run-refresh-shux-asm-gate.sh tests/run-f32-xmm-gates.sh tests/lib/dod-native-exe.sh &&
      ./tests/run-pre-push-p5.sh
    '
}

# ZC-1/2/3/4/5 统一门禁（Linux shux_asm 运行时实锤；比 ubuntu-gates 轻）
run_ubuntu_zc_gates() {
  echo "===== Docker CI: ubuntu-zc-gates (ZC-1/2/3/4/5 + shux_asm, linux/amd64) ====="
  docker run --rm --platform linux/amd64 -e CI=1 -e SHUX_CC_EXTRA="-std=gnu11" \
    -v "$SRC:/src" -w /src \
    ubuntu:22.04 \
    sh -c '
      ...
      cd compiler && SHUX=./shux ./scripts/build_shux_asm.sh &&
      cd .. &&
      test -x ./compiler/shux_asm || { echo "ubuntu-zc-gates: shux_asm missing after build" >&2; exit 1; } &&
...
run_ubuntu_wpo_s2() {
  echo "===== Docker CI: ubuntu-wpo-s2 (bootstrap + build_shux_asm + wpo-s2 smoke/perf, linux/amd64) ====="
  docker run --rm --platform linux/amd64 -e CI=1 -e SHUX_CC_EXTRA="-std=gnu11" \
    -v "$SRC:/src" -w /src \
    ubuntu:22.04 \
    sh -c '
      ...
      ./tests/run-wpo-s2.sh &&
      SHUX=./compiler/shux_asm SHUX_WPO_S2_COMPILE_ONLY=1 ./tests/run-perf-wpo-s2.sh --bench &&
      SHUX=./compiler/shux_asm ./tests/run-wpo-s3-gate.sh &&
      SHUX=./compiler/shux_asm ./tests/run-wpo-s3-cross-bisect.sh
    '
}

# WPO-S3 asm 门禁（bootstrap + build_shux_asm + await/yield 实锤；比 ubuntu-wpo-s2 轻，无 perf）
run_ubuntu_wpo_s3_full() {
  echo "===== Docker CI: ubuntu-wpo-s3-full (build_shux_asm + WPO-S3 await/yield, linux/amd64) ====="
  docker run --rm --platform linux/amd64 -e CI=1 -e SHUX_CC_EXTRA="-std=gnu11" \
    -v "$SRC:/src" -w /src \
    ubuntu:22.04 \
    sh -c '
...
# WPO-S3 asm disasm 快速门禁（需已有 Linux shux_asm；不重建 bootstrap）
run_ubuntu_wpo_s3() {
  ...
  docker run --rm --platform linux/amd64 -e CI=1 \
    ...
    sh -c '
      ...
      test -x ./compiler/shux_asm || test -x ./compiler/shux_asm.experimental || { echo "missing shux_asm; run ubuntu-wpo-s3-full first" >&2; exit 1; } &&
      if [ ! -x ./compiler/shux_asm ] && [ -x ./compiler/shux_asm.experimental ]; then cp ./compiler/shux_asm.experimental ./compiler/shux_asm; fi &&
...
# WPO-S4 PGO-Lite 门禁（bootstrap + build_shux_asm + .text.hot/unlikely + call-depth emit）
run_ubuntu_wpo_s4_full() {
  echo "===== Docker CI: ubuntu-wpo-s4-full (build_shux_asm + WPO-S4 PGO-Lite, linux/amd64) ====="
  docker run --rm --platform linux/amd64 -e CI=1 -e SHUX_CC_EXTRA="-std=gnu11" \
    -v "$SRC:/src" -w /src \
    ubuntu:22.04 \
    sh -c '
      ...
      make -C compiler OPT=1 all &&
      make -C compiler bootstrap-driver-seed &&
      cd compiler && SHUX_ASM_SKIP_STRICT_SMOKE=1 SHUX=./shux ./scripts/build_shux_asm.sh &&
      cd .. &&
      chmod +x tests/run-wpo-s4-gate.sh tests/run-perf-wpo-dce-shux-asm-text.sh tests/lib/wpo-ab-proxy.sh &&
      SHUX_WPO_PGO_HOT=1 SHUX=./compiler/shux_asm ./tests/run-wpo-s4-gate.sh
    '
}

# WPO-S4 快速门禁（需已有 Linux shux_asm；不重建 bootstrap）
run_ubuntu_wpo_s4() {
  ...
  docker run --rm --platform linux/amd64 -e CI=1 \
    ...
    sh -c '
      ...
      test -x ./compiler/shux_asm || { echo "missing shux_asm; run ubuntu-wpo-s4-full first" >&2; exit 1; } &&
      chmod +x tests/run-wpo-s4-gate.sh tests/run-perf-wpo-dce-shux-asm-text.sh tests/lib/wpo-ab-proxy.sh &&
      SHUX_WPO_PGO_HOT=1 SHUX=./compiler/shux_asm ./tests/run-wpo-s4-gate.sh
    '
}
...
run_ubuntu_wpo_stretch() {
  ...
  docker run --rm --platform linux/amd64 -e CI=1 -e SHUX_CI_DOCKER=1 -e SHUX_CC_EXTRA="-std=gnu11" \
    -v "$SRC:/src" -w /src \
    ubuntu:22.04 \
    sh -c '
      ...
      make -C compiler OPT=1 all &&
      test -x compiler/shux_asm || make -C compiler bootstrap-driver-bstrict &&
      ...
      SHUX_PERF_BCMP_ASM=1 ./tests/run-bcmp-gate.sh || true
    '
}
...
run_ubuntu_net_zc1() {
  ...
  docker run --rm --platform linux/amd64 -e CI=1 -e SHUX_CC_EXTRA="-std=gnu11" \
    ...
    sh -c '
      ...
      cd compiler && SHUX=./shux ./scripts/build_shux_asm.sh &&
      cd .. &&
      test -x ./compiler/shux_asm || { echo "ubuntu-net-zc1: shux_asm missing after build" >&2; exit 1; } &&
      ...
      chmod +x tests/run-zc-gates.sh &&
      SHUX=./compiler/shux_asm ./tests/run-zc-gates.sh --perf
    '
}
