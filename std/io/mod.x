...
function stdin(): usize { return 0; }
function stdout(): usize { return 1; }
function stderr(): usize { return 2; }
...
const IO_CTX_MS_CANCELLED: i32 = -1;
/** timeout_ms_for_context 在 Context 已过期时的 sentinel。 */
const IO_CTX_MS_EXPIRED: i32 = -2;
...
function read_ctx(handle: usize, ptr: *u8, len: usize, ctx: Context): i32 {
...
function write_ctx(handle: usize, ptr: *u8, len: usize, ctx: Context): i32 {
  let tm: i32 = timeout_from_ctx(ctx);
  if (tm == IO_CTX_MS_CANCELLED) {
    return err.io_err_cancelled();
  }
 ...
 */
function ptr_view(handle: usize, timeout_ms: u32): ReadPtrView {
...
function ptr_view_valid(v: ReadPtrView): i32 {
  if (v.ptr == 0) {
    return 0;
  }
...
function stdin_ptr_view(): ReadPtrView {
 ...
 */
function ptr_slice(handle: usize, timeout_ms: u32): u8[]<io_read_ptr> {
  return driver.driver_read_ptr_slice(handle, timeout_ms);
}
/** 零拷贝读 stdin slice；等价 ptr_slice(stdin(), 0)。 */
function stdin_ptr_slice(): u8[]<io_read_ptr> {
...
function register_buffers(p0: *u8, l0: usize, p1: *u8, l1: usize, p2: *u8, l2: usize, p3: *u8, l3: usize, nr: u32): i32 {
...
function register_buffers(bufs: *Buffer, nr: u32): i32 {
...
function read_batch_provided_fd(fd: i32, n: i32, timeout_ms: u32, out_bids: *u32, out_lens: *u32): i32 {
...
function print(x: i32): i32 { return 0; }
/** print 无符号 32 位重载。 */
function print(x: u32): i32 { return 0; }
/** print 64 位整数重载。 */
function print(x: i64): i32 { return 0; }
/** 模块尾占位：transitive import 解析时末位 function 会丢失，须保留非 API 锚点。 */
function io_module_anchor(): i32 { return 0; }
