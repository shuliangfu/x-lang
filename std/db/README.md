...
| **arrow** | `import("std.db.arrow")` | 零拷贝列式内存（`arrow.x` + SIMD 胶层，F-05 v1） |

## 迁移说明

- SQLite 实现位于 **`std/db/sqlite/`**（`mod.x` + `sqlite.x` + `sqlite_glue.c`）。
...
make -C compiler ../std/db/arrow/arrow.o     # Arrow 列式（F-05 v1：arrow.x + 胶层）
# F-02 v1：std.sys.mmap 已纯 .x，无需单独 make mmap.o
```

## F-05 闭合（v4）

- 三后端业务逻辑均在 **`.x`**；仅 **3 个胶层 `.c`**（SIMD / mmap / libsqlite3）
