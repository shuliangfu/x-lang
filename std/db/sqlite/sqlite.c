/**
 * std/db/sqlite/sqlite.c — SQLite3 后端与 stub 回退（STD-057）
 *
 * 【文件职责】
 * db_open_c / db_close_c / db_exec_c / 事务 API；默认 SHUX_DB_USE_SQLITE3（SQLite3 后端）。
 * 无 SHUX_DB_USE_SQLITE3（make sqlite-o-stub）时返回 DB_NOT_IMPL。
 ...
 * STD-137：db_row_col_blob_len_c / db_row_col_blob_read_c 大 BLOB 分块读；db_sqlite_blob_stream_smoke_c 烟测。
 ...
 * STD-139：db_sqlite_stub_smoke_c stub 后端语义烟测（sqlite-o-stub 构建）。
 * 【所属模块】标准库 std.db.sqlite；与 std/db/sqlite/mod.sx 同属一模块。
...
int32_t db_row_col_blob_len_c(int64_t cursor, int32_t col) {
  sqlite3_stmt *stmt = db_cursor_stmt(cursor);
  int32_t n;
  if (!stmt) {
    db_set_err(DB_ERR_NULL, "null cursor");
...
int32_t db_row_col_blob_read_c(int64_t cursor, int32_t col, int32_t offset,
  ...
  if (offset < 0) {
    db_set_err(DB_ERR_EXEC, "bad offset");
    return DB_ERR_EXEC;
  }
  if (!out_buf || out_cap <= 0) {
    db_set_err(DB_ERR_NULL, "null buf");
  ...
  memcpy(out_buf, (const uint8_t *)blob + (size_t)offset, (size_t)to_copy);
  db_clear_err();
  return to_copy;
}
...
/** C 烟测：大 BLOB 分块读（96 字节，32×3 块；校验 row_col_blob_len/read）。 */
int32_t db_sqlite_blob_stream_smoke_c(void) {
  int64_t h;
  int64_t cur;
  int32_t st;
  int32_t total;
  ...
  h = db_open_c((const uint8_t *)":memory:");
  if (!h) return 2;
  if (db_exec_c(h, (const uint8_t *)"CREATE TABLE t(data BLOB);") != DB_OK) {
    db_close_c(h);
    return 3;
  }
  sql[0] = '\0';
  strncat(sql, "INSERT INTO t(data) VALUES (x'", sizeof(sql) - 1);
  for (i = 0; i < 96; i++) {
    char hx[3];
    snprintf(hx, sizeof(hx), "%02x", (unsigned)i);
    strncat(sql, hx, sizeof(sql) - strlen(sql) - 1);
  }
  strncat(sql, "');", sizeof(sql) - strlen(sql) - 1);
  if (db_exec_c(h, (const uint8_t *)sql) != DB_OK) {
    db_close_c(h);
    return 4;
  }
  cur = db_query_begin_c(h, (const uint8_t *)"SELECT data FROM t;");
  if (!cur) {
    db_close_c(h);
    return 5;
  }
  ...
  acc_len = 0;
  while (off < total) {
    got = db_row_col_blob_read_c(cur, 0, off, buf, (int32_t)sizeof(buf));
    if (got <= 0) {
      db_query_end_c(cur);
      db_close_c(h);
      return 8;
...
int32_t db_sqlite_stub_smoke_c(void) {
  return 0;
}
...
/** C 烟测：stub 后端语义（open=0、last_error=DB_NOT_IMPL）；仅 stub 构建执行断言。 */
int32_t db_sqlite_stub_smoke_c(void) {
  int64_t h;
  int32_t ec;
  if (strcmp((const char *)db_backend_name_c(), "stub") != 0) {
    return 0;
  }
  h = db_open_c((const uint8_t *)":memory:");
  if (h != 0) {
  ...
  ec = db_last_error_code_c();
  if (ec != DB_NOT_IMPL) {
    return 2;
  }
  if (db_pool_open_c((const uint8_t *)":memory:", 1) != 0) {
    return 3;
  }
  if (db_last_error_code_c() != DB_NOT_IMPL) {
    return 4;
  }
  return 0;
}
