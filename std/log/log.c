/**
 * std/log/log.c — 日志写入 stderr / 文件，带级别前缀（零分配，对标 Zig std.log / Rust log）
 *
 * 【文件职责】
 *   log_write_c：人类可读 "[LEVEL] msg" 行；log_write_structured_kv_c：OBS-003 结构化行。
 *   多 sink（STD-053）：SINK_STDERR | SINK_FILE；SHUX_LOG_MIN_LEVEL 惰性应用。
 *   轮转 + 异步缓冲（STD-106）：set_rotate_limit / set_async_enabled / async_flush。
 */
...
static const char *log_level_name(int32_t level) {
  static const char *names[] = { "debug", "info", "warn", "error" };
  if (level < 0 || level > 3) return "info";
  return names[level];
}

int32_t log_write_c(int32_t level, const uint8_t *ptr, int32_t len) {
  log_apply_env_once();
  char line[LOG_ASYNC_SLOT_SIZE];
  int pl;
  int total;
  if (level < 0 || level > 3 || ptr == 0) return -1;
  if (level < s_min_level) return 0;
  pl = level_prefix_len[level];
  total = pl + len + 1;
  if (total >= LOG_ASYNC_SLOT_SIZE) return -1;
  memcpy(line, level_prefix[level], (size_t)pl);
  if (len > 0) memcpy(line + pl, ptr, (size_t)len);
  line[pl + len] = '\n';
  return log_emit_bytes(line, (size_t)total);
}

/** OBS-003 结构化行：shux: level=… component=… kv…；comp/kv 为 NUL 结尾 C 串。 */
int32_t log_write_structured_kv_c(const uint8_t *component, int32_t level, const uint8_t *kv_body) {
  log_apply_env_once();
  if (!component || !kv_body || level < 0 || level > 3) return -1;
  if (level < s_min_level) return 0;
  char line[1024];
  int n = snprintf(line, sizeof(line), "shux: level=%s component=%s %s\n", log_level_name(level),
                   (const char *)component, (const char *)kv_body);
  if (n <= 0 || (size_t)n >= sizeof(line)) return -1;
  return log_emit_bytes(line, (size_t)n);
}
