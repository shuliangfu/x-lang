...
function elf_read_fixture_c(path: *u8, out_buf: *u8, out_cap: i32): i32 {
  ...
  if (fd < 0) { return -4; }
  unsafe { n = fs_posix_read_c(fd, out_buf, cap); }
  unsafe { close(fd); }
  if (n < 0) { return -4; }
  return n as i32;
}
...
function elf64_write_min_reloc_c(buf: *u8, cap: i32, text: *u8, text_len: i32, out_len: *i32): i32 {
  ...
  unsafe { memcpy(buf + shstr_off, &shstr[0], ELF64_WRITE_SHSTR); }
  if (text_len > 0) {
    unsafe { memcpy(buf + text_off, text, text_len); }
  }
  /* section[1] .text */
  sh = buf + 64 + 64;
