...
/** SHA-512 摘要；委托 ed25519 同单元 sha512()（FIPS 180-4 金样正确）。 */
CRYPTO_HOT
...
CRYPTO_HOT
void crypto_hmac_sha512_c(const uint8_t * restrict key, int32_t key_len, const uint8_t * restrict msg, int32_t msg_len, uint8_t * restrict out) {
  uint8_t kbuf[128];
  if (key_len > 128) {
  ...
  uint8_t ko[128];
  memcpy(ko, key, (size_t)key_len);
  if (key_len < 128) memset(ko + key_len, 0, (size_t)(128 - key_len));
  uint8_t inner[128 + 4096];
  if (128 + msg_len > (int32_t)sizeof(inner)) { memset(out, 0, 64); return; }
  for (int i = 0; i < 128; i++) inner[i] = ko[i] ^ IPAD;
  memcpy(inner + 128, msg, (size_t)msg_len);
  crypto_sha512_c(inner, 128 + msg_len, out);
  uint8_t outer[128 + 64];
  for (int i = 0; i < 128; i++) outer[i] = ko[i] ^ OPAD;
