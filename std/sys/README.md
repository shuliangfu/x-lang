...
自举 **关卡三** 的第一步：把 `_stubs.c` / 汇编桩上的 **write** 能力以 **纯 `.x` 门面** 暴露给用户与上层 std。

| 平台 | v0 | v1 | v2（规划） |
|------|-----|-----|------------|
| Linux x86_64 freestanding | `os_write` → `xlang_sys_write` | `std.sys.linux` syscall 号表 | 内联 asm invoke |
...
| Windows | — | — | `win32_write` → kernel32 WriteFile（F-02 v2 纯 `.x`） |
...
```bash
# Linux x86_64 freestanding 烟测（须 xlang -freestanding -backend asm）
./tests/run-std-sys-gate.sh
```

## 测试

- `tests/sys/sys_write_freestanding.x`
- `tests/sys/linux_syscall_nr_smoke.x`
- `tests/sys/macos_posix_write_smoke.x`
- `tests/sys/freebsd_posix_write_smoke.x`
