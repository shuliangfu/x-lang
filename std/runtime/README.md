...
| `panic_hook_collect(has_msg, msg_val)` | STD-028：panic 前登记崩溃证据（弱符号 `xlang_crash_evidence_collect_c`）。 |

## 实现

- panic/abort 经 `std/runtime/runtime.c` 调用 `xlang_panic_(0, 0)`，该符号由编译器链接时提供的 runtime_panic.o（或 libc abort）提供。
