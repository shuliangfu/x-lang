...
std/compress/
  ...
  zlib/               # RFC 1950 zlib（deflate/inflate）
    libz.sx      # F-04 v4：libz FFI
    mod.sx            # import("std.compress.zlib")
  gzip/               # .gz 块 + 流式 API
    libz.sx      # F-04 v5：libz FFI
    mod.sx            # import("std.compress.gzip")
  brotli/             # .br 块 + 流式 API
    lib.sx     # F-04 v6：libbrotli FFI
    mod.sx            # import("std.compress.brotli")
  zstd/               # zstd 块 + 流式 API
    lib.sx       # F-04 v7：libzstd FFI
...
- **zlib / gzip**：`libz.sx` / `libz.sx`（F-04 v4/v5）；链接 `-lz`（runtime 扫描 marker 或 deflate/inflate 符号）。
- **Brotli**：`lib.sx`（F-04 v6）；链接 `-lbrotlienc -lbrotlidec`（runtime 扫描 marker）。
- **zstd**：`lib.sx`（F-04 v7）；链接 `-lzstd`（runtime 扫描 marker）。
