...
`await read(...)` / `await write(...)` / **`await read_fd(...)`** / **`await write_fd(...)`** emit submit/suspend_io/complete 三阶段；帧内 `__io_rd_slot` / `__io_wr_slot` 支持多协程并行 in-flight（`SHUX_IO_ASYNC_SLOTS`，默认 8）。双协程 read e2e：`async_scheduler_io_read_multi_e2e.c`；write 对称：`async_scheduler_io_write_multi_e2e.c`。`.sx` 双 run：`async_await_io_dual.sx`。Linux 全链路：`async_run_io_stdin.sx`（pipe stdin + `SHUX_ASYNC_YIELD=1`）；双 pipe：`async_run_io_dual_pipe.sx` + wrapper（fd 3/4）；`shux_async_run_i32` 不清 seed（`run` 先 push 再 sched）。
...
IO-A4 multishot accept：`run-io-multishot.sh`（Linux + liburing）；`.sx` 双 pipe 全链路：`async_run_io_dual_pipe.sx` + wrapper。IO-A5 v3：`shux_io_poll_async_completions` + `run_i32` 自动 poll/wake。IO-A5 v4：**`spawn async_fn(args)`** 非阻塞 submit + **`shux_async_run_drain_until_idle()`**（`.sx` 双协程并行：`async_run_io_dual_parallel.sx`，**`import("std.async")`**）。IO-A5 v5：**`shux_async_spawn_i32`** C 侧 spawn 等价 + **`async_run_io_spawn_parallel.c`** / **`async_run_io_spawn_workers.c`**（`SHUX_ASYNC_WORKERS=2`）。
