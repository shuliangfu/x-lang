 ...
 *   4) A3 CPS suspend/resume 钩子（shux_async_cps_suspend / shux_async_run_i32）；
 *   5) A4 就绪队列（MPSC ring v1；per-worker 环 + 多 consumer drain）；
 *   6) IO-A5 v0–v3：IO 等待队列 + io_uring completion poll/wake hook；
 *   7) run v4：`shux_async_run_seed_*` 实参队列（i32/u32/i64/usize，最多 8 个）；
 *   8) IO-A5 v5：`shux_async_spawn_i32` + `shux_async_run_drain_until_idle` 并行 spawn 语义。
 *
 * 链接：-o exe ... std/async/scheduler.o（按需）；MPSC 烟测需 -pthread。
 * 环境：SHUX_ASYNC_YIELD=1 await 边界 yield；SHUX_ASYNC_IO_WAIT=1 suspend 进 IO 等待队列；
 *       SHUX_ASYNC_WORKERS=N 启用 N 个 per-worker 就绪环（1..8，默认 1）；
 *       SHUX_ASYNC_AFFINITY=1 且链 std.thread 时 worker_drain 绑核 worker_id。
 */
#include <stddef.h>
#include <stdint.h>
#include <stdio.h>
