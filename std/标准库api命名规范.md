...
> **硬性规则**：函数名**不得出现类型名**（`i32` `u32` `i64` `usize` `u8` `f32` `f64` `bool` 等）。
...
```su
const io = import("std.io");     io.read(...);      // not io.io_read
const socket = import("std.socketio"); socket.connect(...); // not socket.socket_connect
const vec = import("std.vec");    vec.push(&v, x);   // not vec.vec_i32_push / vec.push_i32
```
...
| `shux_async_spawn_i32`            | IO-A5 v5：push i32 seed 并 submit 协程（spawn C 侧等价）。                                                                                             | `spawn`                     | 去模块前缀+去类型名     | `async.spawn(...)`                     |
| `shux_async_run_drain_until_idle` | IO-A5 v4：poll + drain 直至就绪环与 IO 等待队列皆空；返回已完成任务结果之和。                                                                                          | `run_drain_until_idle`      | 去模块前缀+去类型名     | `async.run_drain_until_idle(...)`      |
...
| `shux_async_task_submit`          | ─── A4 就绪队列（MPSC ring；per-worker 环 + worker_drain）─── DOD-CL-S2：C 侧 shux_async_task_queue_t 已 align(64) head/tail/slots（见 scheduler_glue.c）。 | `task_submit`               | 去模块前缀+去类型名     | `async.task_submit(...)`               |
...
| `shux_async_worker_count`         | extern C/平台                                                                                                                                  | `worker_count`              | 去模块前缀+去类型名     | `async.worker_count(...)`              |
| `shux_async_worker_pending`       | extern C/平台                                                                                                                                  | `worker_pending`            | 去模块前缀+去类型名     | `async.worker_pending(...)`            |
| `shux_async_queue_reset`          | extern C/平台                                                                                                                                  | `queue_reset`               | 去模块前缀+去类型名     | `async.queue_reset(...)`               |
| `shux_async_scheduler_pending`    | extern C/平台                                                                                                                                  | `scheduler_pending`         | 去模块前缀+去类型名     | `async.scheduler_pending(...)`         |
...
| `shux_io_poll_async_completions`  | IO-A5 v3：poll io_uring CQE（未链 io.o 时为 weak 桩）。                                                                                               | `poll_async_completions`    | 去模块前缀+去类型名     | `async.poll_async_completions(...)`    |
| `shux_async_bind_context_c`       | ─── STD-090/093：Context 绑定与取消传播（scheduler_glue.c ctx_slots）───                                                                               | `bind_context`              | 去模块前缀+去类型名（C层） | `async.bind_context(...)`              |
| `shux_async_unbind_context_c`     | extern C/平台                                                                                                                                  | `unbind_context`            | 去模块前缀+去类型名（C层） | `async.unbind_context(...)`            |
| `shux_async_current_context_c`    | extern C/平台                                                                                                                                  | `current_context`           | 去模块前缀+去类型名（C层） | `async.current_context(...)`           |
| `shux_async_task_submit_with_ctx` | extern C/平台                                                                                                                                  | `task_submit_with_ctx`      | 去模块前缀+去类型名     | `async.task_submit_with_ctx(...)`      |
| `shux_async_spawn_ctx_smoke_c`    | extern C/平台                                                                                                                                  | `spawn_ctx_smoke`           | 去模块前缀+去类型名（C层） | `async.spawn_ctx_smoke(...)`           |
| `async_err_ctx_abort`             | 返回 Context 取消时 drain 的错误码。                                                                                                                   | `err_ctx_abort`             | 去模块前缀+去类型名     | `async.err_ctx_abort(...)`             |
| `bind_context`                    | 绑定 Context 到当前 async 运行时（spawn/drain 取消传播）。                                                                                                  | `bind_context`              | 已符合命名          | `async.bind_context(...)`              |
| `unbind_context`                  | 弹出最近一次 bind_context。                                                                                                                         | `unbind_context`            | 已符合命名          | `async.unbind_context(...)`            |
| `current_context`                 | 返回当前绑定的 Context 句柄；未绑定返回 0。                                                                                                                  | `current_context`           | 已符合命名          | `async.current_context(...)`           |
| `runtime_new`                     | 创建带默认 Context 的 AsyncRuntime。                                                                                                                | `runtime_new`               | 已符合命名          | `async.runtime_new(...)`               |
| `runtime_reset`                   | 重置就绪环/seed 并重新绑定 runtime 的 Context。                                                                                                          | `runtime_reset`             | 已符合命名          | `async.runtime_reset(...)`             |
| `runtime_drain`                   | drain 直至空闲；Context 已取消时返回 async_err_ctx_abort()。                                                                                             | `runtime_drain`             | 已符合命名          | `async.runtime_drain(...)`             |
| `task_submit`                     | 提交 async 任务到就绪环；0 成功。                                                                                                                        | `task_submit`               | 已符合命名          | `async.task_submit(...)`               |
| `task_submit_with_context`        | 提交任务并绑定 Context；ctx 已取消时返回 -2。                                                                                                               | `task_submit_with_context`  | 已符合命名          | `async.task_submit_with_context(...)`  |
| `spawn_context_smoke`             | STD-093：spawn 自动继承 bind_context 烟测；0 通过。                                                                                                     | `spawn_context_smoke`       | 已符合命名          | `async.spawn_context_smoke(...)`       |
| `scheduler_reset`                 | 清空就绪环与 IO 等待队列（保留 run seed 时见 shux_async_queue_reset_impl）。                                                                                  | `scheduler_reset`           | 已符合命名          | `async.scheduler_reset(...)`           |
| `drain_until_idle`                | poll + drain 直至就绪环与 IO 等待队列皆空。                                                                                                               | `drain_until_idle`          | 已符合命名          | `async.drain_until_idle(...)`          |
| `cps_suspend_io`                  | await IO 边界 suspend 门面。                                                                                                                      | `cps_suspend_io`            | 已符合命名          | `async.cps_suspend_io(...)`            |
| `poll_io_completions`             | poll io_uring completion 并唤醒 IO 等待任务。                                                                                                        | `poll_io_completions`       | 已符合命名          | `async.poll_io_completions(...)`       |
| `io_uring_is_available`           | 当前线程 io_uring 是否可用；1 是，0 否（非 Linux 恒 0）。                                                                                                     | `uring_available`           | 语义重命名          | `async.uring_available(...)`           |
...
| `complete_read_async_slot`        | 收割指定 slot 的 read_async；未就绪返回 IO_ASYNC_NOT_READY。                                                                                             | `complete_read_async_slot`  | 已符合命名          | `async.complete_read_async_slot(...)`  |
| `complete_write_async_slot`       | 收割指定 slot 的 write_async；未就绪返回 IO_ASYNC_NOT_READY。                                                                                            | `complete_write_async_slot` | 已符合命名          | `async.complete_write_async_slot(...)` |
| `io_pump_once`                    | 单轮 IO pump：poll CQE + drain 就绪环/IO 等待队列；返回 drain 结果。                                                                                         | `pump_once`                 | 语义重命名          | `async.pump_once(...)`                 |
| `io_timeout_ms_from_ctx`          | —                                                                                                                                            | `io_timeout_ms_from_ctx`    | 已符合命名          | `async.io_timeout_ms_from_ctx(...)`    |
| `io_ctx_check`                    | IO pump 前检查 Context；0 可继续，否则 net_err_cancelled/timeout。                                                                                      | `io_ctx_check`              | 已符合命名          | `async.io_ctx_check(...)`              |
...
| `shux_async_net_fs_smoke_c`       | extern C/平台                                                                                                                                  | `net_fs_smoke`              | 去模块前缀+去类型名（C层） | `async.net_fs_smoke(...)`              |
| `net_fs_async_smoke`              | async net/fs fd 路径 C 烟测（pipe 模拟）；0 通过。                                                                                                       | `net_fs_async_smoke`        | 已符合命名          | `async.net_fs_async_smoke(...)`        |
| `io_waiters_pending`              | IO 等待队列深度。                                                                                                                                   | `waiters_pending`           | 语义重命名          | `async.waiters_pending(...)`           |
...
| `shux_async_future_smoke_c`       | extern C/平台                                                                                                                                  | `future_smoke`              | 去模块前缀+去类型名（C层） | `async.future_smoke(...)`              |
...
| `future_new`                      | 创建 pending Future；池满时 handle=0。                                                                                                              | `future_new`                | 已符合命名          | `async.future_new(...)`                |
...
| `future_reset`                    | 重置 Future 池（烟测/同进程二次 poll）。                                                                                                                  | `future_reset`              | 已符合命名          | `async.future_reset(...)`              |
| `future_smoke`                    | C 侧 Future 烟测；0 通过。                                                                                                                          | `future_smoke`              | 已符合命名          | `async.future_smoke(...)`              |
| `future_wait`                     | —                                                                                                                                            | `future_wait`               | 已符合命名          | `async.future_wait(...)`               |
| `runtime_wait_future`             | —                                                                                                                                            | `runtime_wait_future`       | 已符合命名          | `async.runtime_wait_future(...)`       |
| `poll_loop`                       | —                                                                                                                                            | `poll_loop`                 | 已符合命名          | `async.poll_loop(...)`                 |
| `poll_loop_ctx`                   | —                                                                                                                                            | `poll_loop_ctx`             | 已符合命名          | `async.poll_loop_ctx(...)`             |
...
| `placeholder`                     | 模块可 import 锚点（Future/Poll 见 future_* / poll_*）。                                                                                              | `placeholder`               | 已符合命名          | `async.placeholder(...)`               |
| `shu_async_bind_context_c`        | 绑定 Context（C 别名）。                                                                                                                            | `bind_context`              | 去模块前缀+去类型名（C层） | `async.bind_context(...)`              |
...
| `shu_async_spawn_i32`             | spawn i32 seed（C 别名）。                                                                                                                        | `spawn`                     | 去模块前缀+去类型名     | `async.spawn(...)`                     |
...
| `shu_async_task_submit`           | 提交任务（C 别名）。                                                                                                                                  | `task_submit`               | 去模块前缀+去类型名     | `async.task_submit(...)`               |
| `shu_async_task_submit_with_ctx`  | 带 Context 提交（C 别名）。                                                                                                                          | `task_submit_with_ctx`      | 去模块前缀+去类型名     | `async.task_submit_with_ctx(...)`      |
| `shu_async_spawn_ctx_smoke_c`     | spawn context 烟测（C 别名）。                                                                                                                      | `spawn_ctx_smoke`           | 去模块前缀+去类型名（C层） | `async.spawn_ctx_smoke(...)`           |
...
| `bytes_append_slice`          | 追加切片；0 成功，-1 失败。                               | `extend_from_slice`           | 去模块前缀+去类型名 | `bytes.extend_from_slice(...)`           |
...
| `brotli_smoke`                    | Brotli 往返烟测；未启用时返回 0（skip）。                  | `brotli_smoke`                  | 已符合命名      | `compress.brotli_smoke(...)`                  |
...
| `zstd_smoke`                      | zstd 往返烟测；未启用时返回 0（skip）。                    | `zstd_smoke`                    | 已符合命名      | `compress.zstd_smoke(...)`                    |
...
| `brotli_smoke`                  | Brotli 往返烟测；未启用时返回 0（skip）。         | `smoke`           | 去模块前缀+去类型名 | `brotli.smoke(...)`           |
...
| `zstd_smoke`                  | zstd 块 + 流往返烟测；未启用时返回 0（skip）。     | `smoke`           | 去模块前缀+去类型名 | `zstd.smoke(...)`           |
...
| `config_yaml_smoke_c`      | extern C/平台                                    | `yaml_smoke`      | 去模块前缀+去类型名（C层） | `config.yaml_smoke(...)`      |
...
| `yaml_smoke`               | STD-119：YAML 后端 C 烟测；0 通过。                     | `yaml_smoke`      | 已符合命名          | `config.yaml_smoke(...)`      |
...
| `ctx_smoke_c`         | extern C/平台                       | `smoke`         | 去模块前缀+去类型名（C层） | `context.smoke(...)`         |
...
| `crypto_ed25519_smoke_c`            | extern C/平台                                                                                                                                      | `ed25519_smoke`             | 去模块前缀+去类型名（C层）    | `crypto.ed25519_smoke(...)`             |
...
| `crypto_chacha20_poly1305_smoke_c`  | extern C/平台                                                                                                                                      | `chacha20_poly1305_smoke`   | 去模块前缀+去类型名（C层）    | `crypto.chacha20_poly1305_smoke(...)`   |
...
| `ed25519_smoke`                     | Ed25519 C 层烟测（RFC 8032 TEST 1）；0 通过。                                                                                                             | `ed25519_smoke`             | 已符合命名             | `crypto.ed25519_smoke(...)`             |
...
| `chacha20_poly1305_smoke`           | ChaCha20-Poly1305 C 层烟测；0 通过。                                                                                                                    | `chacha20_poly1305_smoke`   | 已符合命名             | `crypto.chacha20_poly1305_smoke(...)`   |
...
| `csv_stream_smoke_c`             | extern C/平台                                                                                            | `smoke`              | 去模块前缀+去类型名（C层） | `csv.smoke(...)`              |
...
| `csv_stream_smoke`               | STD-128：C 层多行流式烟测；0 通过。                                                                                | `smoke`              | 去模块前缀+去类型名     | `csv.smoke(...)`              |
...
| `datetime_iana_dst_smoke_c`         | extern C/平台                                      | `iana_dst_smoke`          | 去模块前缀+去类型名（C层） | `dt.iana_dst_smoke(...)`          |
| `datetime_timezone_smoke_c`         | extern C/平台                                      | `timezone_smoke`          | 去模块前缀+去类型名（C层） | `dt.timezone_smoke(...)`          |
...
| `iana_dst_smoke`                    | IANA DST 烟测；0 通过。                                | `iana_dst_smoke`          | 已符合命名          | `dt.iana_dst_smoke(...)`          |
| `timezone_smoke`                    | STD-135 C 金样。                                    | `timezone_smoke`          | 已符合命名          | `dt.timezone_smoke(...)`          |
...
| `arrow_smoke_c`                  | extern C/平台                               | `smoke`                  | 去模块前缀+去类型名（C层） | `arrow.smoke(...)`                  |
...
| `db_kv_smoke_c`           | extern C/平台                                              | `smoke`              | 二次精简：去对象前缀     | `kv.smoke(...)`              |
...
| `elf64_write_smoke_c`          | extern C/平台            | `elf64_write_smoke`          | 已符合命名      | `elf.elf64_write_smoke(...)`          |
| `elf64_parse_smoke_c`          | extern C/平台            | `elf64_parse_smoke`          | 已符合命名      | `elf.elf64_parse_smoke(...)`          |
...
| `write_smoke`                  | —                      | `write_smoke`                | 已符合命名      | `elf.write_smoke(...)`                |
| `parse_smoke`                  | —                      | `parse_smoke`                | 已符合命名      | `elf.parse_smoke(...)`                |
...
| `encoding_extra_smoke_c`      | extern C/平台                                           | `extra_smoke`              | 去模块前缀+去类型名（C层） | `enc.extra_smoke(...)`              |
...
| `encoding_extra_smoke`        | STD-127 C 烟测门面；成功 0。                                  | `extra_smoke`              | 去模块前缀+去类型名     | `enc.extra_smoke(...)`              |
...
| 当前名称                    | 功能说明                                                      | 简化名称              | 说明               | 绑定调用                       |
| ----------------------- | --------------------------------------------------------- | ----------------- | ---------------- | -------------------------- |
| `placeholder`           | Tier-S 烟测钩子。                                              | `placeholder`     | 已符合命名            | `fmt.placeholder(...)`     |
...
| 当前名称                          | 功能说明                                                                                                                    | 简化名称                          | 说明                | 绑定调用                                  |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ----------------- | ------------------------------------- |
| `open`                        | 与 POSIX open/read/write/close 一致；链接时需 -lc。fs_*_c 由 fs_posix/fs_win32 提供。 fs_readv_buf/fs_writev_buf 的 bufs 与 Buffer ABI | `open`                        | 已符合命名             | `fs.open(...)`                        |
...
| `fs_posix_read_c`             | —                                                                                                                       | `posix_read`                  | 去模块前缀+去类型名（C层）    | `fs.posix_read(...)`                  |
| `fs_posix_write_c`            | —                                                                                                                       | `posix_write`                 | 去模块前缀+去类型名（C层）    | `fs.posix_write(...)`                 |
| `fs_mmap_ro_c`                | —                                                                                                                       | `mmap_readonly`               | 去模块前缀+去类型名（C层）    | `fs.mmap_readonly(...)`               |
| `fs_mmap_rw_c`                | —                                                                                                                       | `mmap_readwrite`              | 去模块前缀+去类型名（C层）    | `fs.mmap_readwrite(...)`              |
...
| `fs_last_error_c`             | —                                                                                                                       | `last_os_error`               | 去模块前缀+去类型名（C层）    | `fs.last_os_error(...)`               |
...
| `fs_invalid_handle`           | —                                                                                                                       | `invalid_handle`              | 去模块前缀+去类型名        | `fs.invalid_handle(...)`              |
| `fs_read_chunk`               | 推荐大块读单次字节数（1MiB），极致压榨时用此大小循环 fs_read。                                                                                   | `default_read_size`           | 去模块前缀+去类型名        | `fs.default_read_size(...)`           |
...
| `fs_last_error`               | —                                                                                                                       | `last_os_error`               | 去模块前缀+去类型名        | `fs.last_os_error(...)`               |
...
| `fs_mmap_ro`                  | —                                                                                                                       | `mmap_readonly`               | 去模块前缀+去类型名        | `fs.mmap_readonly(...)`               |
| `fs_mmap_rw`                  | —                                                                                                                       | `mmap_readwrite`              | 去模块前缀+去类型名        | `fs.mmap_readwrite(...)`              |
...
| `freestanding_read_file_into` | —                                                                                                                       | `freestanding_read_file_into` | 已符合命名             | `fs.freestanding_read_file_into(...)` |
...
| 当前名称                               | 功能说明                                                          | 简化名称                               | 说明             | 绑定调用                                         |
| ---------------------------------- | ------------------------------------------------------------- | ---------------------------------- | -------------- | -------------------------------------------- |
| `heap_mem_set_c`                   | F-03 v1：纯 .sx 实现（heap_ops.sx）；保留名供 std.mem 链接。                | `mem_set`                          | 去模块前缀+去类型名（C层） | `heap.mem_set(...)`                          |
| `heap_mem_compare_c`               | F-03 v1：纯 .sx 实现（heap_ops.sx）。                                | `mem_compare`                      | 去模块前缀+去类型名（C层） | `heap.mem_compare(...)`                      |
| `map_i32_i32_find_c`               | 线性探测查找 key；F-03 v1 纯 .sx（heap_ops.sx）。                        | `map_find`                         | 去模块前缀+去类型名（C层） | `heap.map_find(...)`                         |
| `heap_trace_enabled`               | trace 是否启用（1/0）。                                              | `trace_enabled`                    | 去模块前缀+去类型名     | `heap.trace_enabled(...)`                    |
...
| `copy_u64_at`                      | 块拷贝 u64；供 std.vec append_slice/from_slice 快路径。                | `copy_at`                          | 去模块前缀+去类型名     | `heap.copy_at(...)`                          |
...
| `copy_f64_at`                      | 块拷贝 f64；供 std.vec append_slice/from_slice 快路径。                | `copy_at`                          | 去模块前缀+去类型名     | `heap.copy_at(...)`                          |
| `allocator_kind_heap`              | 堆分配器 kind 常量（0）。                                              | `allocator_kind_heap`              | 已符合命名          | `heap.allocator_kind_heap(...)`              |
| `allocator_kind_arena`             | Arena bump 分配器 kind 常量（1）。                                    | `allocator_kind_arena`             | 已符合命名          | `heap.allocator_kind_arena(...)`             |
| `allocator_heap`                   | 默认 libc 堆分配器。                                                 | `allocator_heap`                   | 已符合命名          | `heap.allocator_heap(...)`                   |
| `allocator_from_arena`             | 将 Arena64 包装为 bump 分配器（单独 free 为 no-op）。                      | `allocator_from_arena`             | 已符合命名          | `heap.allocator_from_arena(...)`             |
| `scope_allocator`                  | —                                                             | `scope_allocator`                  | 已符合命名          | `heap.scope_allocator(...)`                  |
| `default_allocator`                | —                                                             | `default_allocator`                | 已符合命名          | `heap.default_allocator(...)`                |
| `allocator_alloc`                  | 按 Allocator 分配 size 字节；失败 null。                               | `allocator_alloc`                  | 已符合命名          | `heap.allocator_alloc(...)`                  |
| `allocator_free`                   | 释放 ptr；arena 分配器为 no-op。                                      | `allocator_free`                   | 已符合命名          | `heap.allocator_free(...)`                   |
| `allocator_realloc`                | 调整块大小；仅堆分配器支持，arena 返回 null。                                  | `allocator_realloc`                | 已符合命名          | `heap.allocator_realloc(...)`                |
| `bump_alloc`                       | —                                                             | `bump_alloc`                       | 已符合命名          | `heap.bump_alloc(...)`                       |
| `arena64_empty`                    | 未初始化 Arena64 占位；调用方须随后 arena64_init。                          | `arena64_empty`                    | 已符合命名          | `heap.arena64_empty(...)`                    |
| `arena64_init`                     | —                                                             | `arena64_init`                     | 已符合命名          | `heap.arena64_init(...)`                     |
| `arena64_alloc`                    | —                                                             | `arena64_alloc`                    | 已符合命名          | `heap.arena64_alloc(...)`                    |
| `arena64_deinit`                   | 释放 arena chunk 并重置 off/cap。                                   | `arena64_deinit`                   | 已符合命名          | `heap.arena64_deinit(...)`                   |
| `ptr_mod`                          | 指针对 mod 取模；mod=64 验证 cache line 对齐。                           | `ptr_mod`                          | 已符合命名          | `heap.ptr_mod(...)`                          |
| `alloc_aligned`                    | posix_memalign 薄封装；供 ring slot 等全局 64B 分配。                    | `alloc_aligned`                    | 已符合命名          | `heap.alloc_aligned(...)`                    |
...
| `alloc_zeroed`                     | —                                                             | `alloc_zeroed`                     | 已符合命名          | `heap.alloc_zeroed(...)`                     |
...
| `copy_i32_at`                      | —                                                             | `copy_at`                          | 去模块前缀+去类型名     | `heap.copy_at(...)`                          |
| `copy_u8_at`                       | 块拷贝：dst[dst_offset..] = src[0..count-1]（u8 元素）；供 std.vec 快路径。 | `copy_at`                          | 去模块前缀+去类型名     | `heap.copy_at(...)`                          |
...
| `copy_f32_at`                      | 块拷贝 f32 列；供 std.vec SoA/AoS 扩容与 append。                       | `copy_at`                          | 去模块前缀+去类型名     | `heap.copy_at(...)`                          |
| `alloc_size_zero`                  | 表示“分配大小 0”占位；保留兼容（测试用）。                                       | `alloc_size_zero`                  | 已符合命名          | `heap.alloc_size_zero(...)`                  |
| `freestanding_page_heap_available` | —                                                             | `freestanding_page_heap_available` | 已符合命名          | `heap.freestanding_page_heap_available(...)` |
...
| 当前名称                                            | 功能说明                                                                      | 简化名称                                    | 说明                        | 绑定调用                                              |
| ----------------------------------------------- | ------------------------------------------------------------------------- | --------------------------------------- | ------------------------- | ------------------------------------------------- |
| `method_as_u8`                                  | Method → u8 判别值（GET=0 … OPTIONS=6）。                                       | `method_tag`                            | 语义重命名                     | `http.method_tag(...)`                            |
| `method_from_u8`                                | u8 判别值 → Method；非法值回退 GET。                                                | `method_from_tag`                       | 语义重命名                     | `http.method_from_tag(...)`                       |
...
| `http_https_smoke_c`                            | extern C/平台                                                               | `https_smoke`                           | 去模块前缀+去类型名（C层）            | `http.https_smoke(...)`                           |
...
| `https_smoke`                                   | HTTPS C 烟测钩子（STD-HTTP-HTTPS）。                                             | `https_smoke`                           | 已符合命名                     | `http.https_smoke(...)`（已简洁）                      |
...
| `http_response_parse_smoke_c`                   | extern C/平台                                                               | `response_parse_smoke`                  | 去模块前缀+去类型名（C层）            | `http.response_parse_smoke(...)`                  |
...
| `http_response_body_owned_smoke_c`              | extern C/平台                                                               | `response_body_owned_smoke`             | 去模块前缀+去类型名（C层）            | `http.response_body_owned_smoke(...)`             |
...
| `http_url_owned_smoke_c`                        | extern C/平台                                                               | `url_owned_smoke`                       | 去模块前缀+去类型名（C层）            | `http.url_owned_smoke(...)`                       |
| `http_request_owned_smoke_c`                    | extern C/平台                                                               | `request_owned_smoke`                   | 去模块前缀+去类型名（C层）            | `http.request_owned_smoke(...)`                   |
...
| `response_parse_smoke`                          | Request/Response 解析 C 烟测；0 通过。                                            | `response_parse_smoke`                  | 已符合命名                     | `http.response_parse_smoke(...)`（已简洁）             |
...
| `url_owned_smoke`                               | 堆 URL 复制 C 烟测；0 通过。                                                       | `url_owned_smoke`                       | 已符合命名                     | `http.url_owned_smoke(...)`（已简洁）                  |
...
| `request_owned_smoke`                           | HttpRequestOwned C 烟测；0 通过。                                               | `request_owned_smoke`                   | 已符合命名                     | `http.request_owned_smoke(...)`（已简洁）              |
| `response_body_owned_smoke`                     | 堆 body 复制 C 烟测；0 通过。                                                      | `response_body_owned_smoke`             | 已符合命名                     | `http.response_body_owned_smoke(...)`（已简洁）        |
...
| `http2_smoke_c`                                 | extern C/平台                                                               | `smoke`                                 | 去模块前缀+去类型名（C层）            | `http.smoke(...)`                                 |
...
| `http2_hpack_smoke_c`                           | extern C/平台                                                               | `hpack_smoke`                           | 去模块前缀+去类型名（C层）            | `http.hpack_smoke(...)`                           |
...
| `http2_hpack_dyn_smoke_c`                       | extern C/平台                                                               | `hpack_dyn_smoke`                       | 去模块前缀+去类型名（C层）            | `http.hpack_dyn_smoke(...)`                       |
...
| `http2_hpack_server_dyn_smoke_c`                | extern C/平台                                                               | `hpack_server_dyn_smoke`                | 去模块前缀+去类型名（C层）            | `http.hpack_server_dyn_smoke(...)`                |
...
| `http2_frame_capped_smoke_c`                    | extern C/平台                                                               | `frame_capped_smoke`                    | 去模块前缀+去类型名（C层）            | `http.frame_capped_smoke(...)`                    |
...
| `http2_goaway_smoke_c`                          | extern C/平台                                                               | `goaway_smoke`                          | 去模块前缀+去类型名（C层）            | `http.goaway_smoke(...)`                          |
...
| `http2_ping_smoke_c`                            | extern C/平台                                                               | `ping_smoke`                            | 去模块前缀+去类型名（C层）            | `http.ping_smoke(...)`                            |
...
| `http2_rst_stream_smoke_c`                      | extern C/平台                                                               | `rst_stream_smoke`                      | 去模块前缀+去类型名（C层）            | `http.rst_stream_smoke(...)`                      |
| `http2_conn_reset_stream_c`                     | extern C/平台                                                               | `conn_reset_stream`                     | 去模块前缀+去类型名（C层）            | `http.conn_reset_stream(...)`                     |
| `http2_http2_complete_smoke_c`                  | extern C/平台                                                               | `complete_smoke`                        | 去模块前缀+去类型名（C层）            | `http.complete_smoke(...)`                        |
...
| `http2_client_smoke_c`                          | extern C/平台                                                               | `client_smoke`                          | 去模块前缀+去类型名（C层）            | `http.client_smoke(...)`                          |
| `http2_network_is_available_c`                  | extern C/平台                                                               | `network_is_available`                  | 去模块前缀+去类型名（C层）            | `http.network_is_available(...)`                  |
| `http2_network_smoke_c`                         | extern C/平台                                                               | `network_smoke`                         | 去模块前缀+去类型名（C层）            | `http.network_smoke(...)`                         |
...
| `http2_hpack_huffman_smoke_c`                   | extern C/平台                                                               | `hpack_huffman_smoke`                   | 去模块前缀+去类型名（C层）            | `http.hpack_huffman_smoke(...)`                   |
...
| `http2_flow_control_smoke_c`                    | extern C/平台                                                               | `flow_control_smoke`                    | 去模块前缀+去类型名（C层）            | `http.flow_control_smoke(...)`                    |
...
| `http2_flow_state_smoke_c`                      | extern C/平台                                                               | `flow_state_smoke`                      | 去模块前缀+去类型名（C层）            | `http.flow_state_smoke(...)`                      |
...
| `http2_flow_recv_smoke_c`                       | extern C/平台                                                               | `flow_recv_smoke`                       | 去模块前缀+去类型名（C层）            | `http.flow_recv_smoke(...)`                       |
...
| `http2_push_h2c_smoke_c`                        | extern C/平台                                                               | `push_h2c_smoke`                        | 去模块前缀+去类型名（C层）            | `http.push_h2c_smoke(...)`                        |
...
| `http2_push_fetch_smoke_c`                      | extern C/平台                                                               | `push_fetch_smoke`                      | 去模块前缀+去类型名（C层）            | `http.push_fetch_smoke(...)`                      |
...
| `http2_push_network_smoke_c`                    | extern C/平台                                                               | `push_network_smoke`                    | 去模块前缀+去类型名（C层）            | `http.push_network_smoke(...)`                    |
| `http2_session_request_h2c_c`                   | extern C/平台                                                               | `session_request_h2c`                   | 去模块前缀+去类型名（C层）            | `http.session_request_h2c(...)`                   |
| `http2_h2c_network_smoke_c`                     | extern C/平台                                                               | `h2c_network_smoke`                     | 去模块前缀+去类型名（C层）            | `http.h2c_network_smoke(...)`                     |
...
| `http_h2c_client_smoke_c`                       | extern C/平台                                                               | `h2c_client_smoke`                      | 去模块前缀+去类型名（C层）            | `http.h2c_client_smoke(...)`                      |
...
| `http2_stream_registry_smoke_c`                 | extern C/平台                                                               | `smoke`                                 | 二次精简：去对象前缀                | `http.smoke(...)`                                 |
...
| `http2_settings_smoke_c`                        | extern C/平台                                                               | `settings_smoke`                        | 去模块前缀+去类型名（C层）            | `http.settings_smoke(...)`                        |
| `http2_multistream_client_init_c`               | extern C/平台                                                               | `multistream_client_init`               | 去模块前缀+去类型名（C层）            | `http.multistream_client_init(...)`               |
| `http2_multistream_client_on_settings_c`        | extern C/平台                                                               | `multistream_client_on_settings`        | 去模块前缀+去类型名（C层）            | `http.multistream_client_on_settings(...)`        |
| `http2_multistream_client_open_stream_c`        | extern C/平台                                                               | `multistream_client_open_stream`        | 去模块前缀+去类型名（C层）            | `http.multistream_client_open_stream(...)`        |
| `http2_multistream_client_close_stream_c`       | extern C/平台                                                               | `multistream_client_close_stream`       | 去模块前缀+去类型名（C层）            | `http.multistream_client_close_stream(...)`       |
| `http2_multistream_client_build_get_c`          | extern C/平台                                                               | `multistream_client_build_get`          | 去模块前缀+去类型名（C层）            | `http.multistream_client_build_get(...)`          |
| `http2_multistream_client_build_parallel_get_c` | extern C/平台                                                               | `multistream_client_build_parallel_get` | 去模块前缀+去类型名（C层）            | `http.multistream_client_build_parallel_get(...)` |
| `http2_multistream_client_smoke_c`              | extern C/平台                                                               | `multistream_client_smoke`              | 去模块前缀+去类型名（C层）            | `http.multistream_client_smoke(...)`              |
...
| `http2_conn_goaway_smoke_c`                     | extern C/平台                                                               | `conn_goaway_smoke`                     | 去模块前缀+去类型名（C层）            | `http.conn_goaway_smoke(...)`                     |
| `http2_conn_ping_smoke_c`                       | extern C/平台                                                               | `conn_ping_smoke`                       | 去模块前缀+去类型名（C层）            | `http.conn_ping_smoke(...)`                       |
| `http2_conn_reuse_smoke_c`                      | extern C/平台                                                               | `conn_reuse_smoke`                      | 去模块前缀+去类型名（C层）            | `http.conn_reuse_smoke(...)`                      |
...
| `http2_conn_pool_smoke_c`                       | extern C/平台                                                               | `conn_pool_smoke`                       | 去模块前缀+去类型名（C层）            | `http.conn_pool_smoke(...)`                       |
| `http2_conn_pool_goaway_smoke_c`                | extern C/平台                                                               | `conn_pool_goaway_smoke`                | 去模块前缀+去类型名（C层）            | `http.conn_pool_goaway_smoke(...)`                |
...
| `http2_global_pool_smoke_c`                     | extern C/平台                                                               | `global_pool_smoke`                     | 去模块前缀+去类型名（C层）            | `http.global_pool_smoke(...)`                     |
...
| `http2_server_push_smoke_c`                     | extern C/平台                                                               | `server_push_smoke`                     | 去模块前缀+去类型名（C层）            | `http.server_push_smoke(...)`                     |
| `http2_server_push_tls_smoke_c`                 | extern C/平台                                                               | `server_push_tls_smoke`                 | 去模块前缀+去类型名（C层）            | `http.server_push_tls_smoke(...)`                 |
| `http2_server_push_settings_smoke_c`            | extern C/平台                                                               | `server_push_settings_smoke`            | 去模块前缀+去类型名（C层）            | `http.server_push_settings_smoke(...)`            |
| `http2_server_settings_full_smoke_c`            | extern C/平台                                                               | `server_settings_full_smoke`            | 去模块前缀+去类型名（C层）            | `http.server_settings_full_smoke(...)`            |
| `http2_server_hpack_dyn_smoke_c`                | extern C/平台                                                               | `server_hpack_dyn_smoke`                | 去模块前缀+去类型名（C层）            | `http.server_hpack_dyn_smoke(...)`                |
| `http2_server_max_frame_smoke_c`                | extern C/平台                                                               | `server_max_frame_smoke`                | 去模块前缀+去类型名（C层）            | `http.server_max_frame_smoke(...)`                |
...
| `http2_server_multistream_push_smoke_c`         | extern C/平台                                                               | `server_multistream_push_smoke`         | 去模块前缀+去类型名（C层）            | `http.server_multistream_push_smoke(...)`         |
...
| `http2_server_multistream_smoke_c`              | extern C/平台                                                               | `server_multistream_smoke`              | 去模块前缀+去类型名（C层）            | `http.server_multistream_smoke(...)`              |
...
| `http2_server_smoke_c`                          | extern C/平台                                                               | `server_smoke`                          | 去模块前缀+去类型名（C层）            | `http.server_smoke(...)`                          |
...
| `http2_err_push_not_impl`                       | HTTP/2 server push 未实现（旧 API；读路径已可收集）。                                    | `err_push_not_impl`                     | 去模块前缀+去类型名                | `http.err_push_not_impl(...)`                     |
| `http2_err_h2c_not_impl`                        | HTTP/2 h2c 升级路径未实现（直连 preface 见 http2_h2c_session_begin）。                 | `err_h2c_not_impl`                      | 去模块前缀+去类型名                | `http.err_h2c_not_impl(...)`                      |
| `http2_err_h2c_scheme`                          | h2c 仅支持 http://；https:// URL 返回此错误（-1235）。                                | `err_h2c_scheme`                        | 去模块前缀+去类型名                | `http.err_h2c_scheme(...)`                        |
| `http2_err_max_streams`                         | 并发 stream 已达对端 SETTINGS 上限（-1236）。                                        | `err_max_streams`                       | 去模块前缀+去类型名                | `http.err_max_streams(...)`                       |
| `http2_err_conn_not_ready`                      | 连接未完成 handshake（-1237）。                                                   | `err_conn_not_ready`                    | 去模块前缀+去类型名                | `http.err_conn_not_ready(...)`                    |
| `http2_err_pool_scheme`                         | URL scheme 与 H2 连接池类型不匹配（-1238）。                                          | `err_pool_scheme`                       | 去模块前缀+去类型名                | `http.err_pool_scheme(...)`                       |
| `http2_err_global_pool_full`                    | 全局 H2 连接池 host:port 条目已满（-1239）。                                          | `err_global_pool_full`                  | 去模块前缀+去类型名                | `http.err_global_pool_full(...)`                  |
| `http2_err_server`                              | HTTP/2 h2c server 协议/请求错误（-1240）。                                         | `err_server`                            | 去模块前缀+去类型名                | `http.err_server(...)`                            |
| `http2_err_server_push`                         | HTTP/2 server push 发送侧参数非法（-1242）。                                        | `err_server_push`                       | 去模块前缀+去类型名                | `http.err_server_push(...)`                       |
| `http2_err_server_push_disabled`                | client SETTINGS ENABLE_PUSH=0，server 软拒绝 push（-1243）。                     | `err_server_push_disabled`              | 去模块前缀+去类型名                | `http.err_server_push_disabled(...)`              |
| `http2_err_goaway`                              | 对端发送 GOAWAY / 期望 GOAWAY 时语义码（-1244）。                                      | `err_goaway`                            | 去模块前缀+去类型名                | `http.err_goaway(...)`                            |
| `http2_err_ping`                                | PING 往返失败 / 未收到匹配 PONG（-1245）。                                            | `err_ping`                              | 去模块前缀+去类型名                | `http.err_ping(...)`                              |
| `http2_err_rst_stream`                          | 对端 RST_STREAM / stream 被取消（-1246）。                                        | `err_rst_stream`                        | 去模块前缀+去类型名                | `http.err_rst_stream(...)`                        |
| `http2_err_server_tls`                          | HTTP/2 over TLS server 不可用或 ALPN 非 h2（-1241）。                             | `err_server_tls`                        | 去模块前缀+去类型名                | `http.err_server_tls(...)`                        |
| `http2_err_flow_blocked`                        | HTTP/2 发送窗口耗尽（须 WINDOW_UPDATE；v5 流控状态机）。                                  | `err_flow_blocked`                      | 去模块前缀+去类型名                | `http.err_flow_blocked(...)`                      |
| `http2_err_network_h2`                          | HTTP/2 网络 h2 over TLS 不可用（须 std.net ALPN；v2）。                             | `err_network_h2`                        | 去模块前缀+去类型名                | `http.err_network_h2(...)`                        |
| `http2_err_not_impl`                            | HTTP/2 完整编解码层不可用（v1 前占位；现与 client_is_available 对齐）。                       | `err_not_impl`                          | 去模块前缀+去类型名                | `http.err_not_impl(...)`                          |
| `http2_frame_data`                              | DATA 帧类型。                                                                 | `frame_data`                            | 去模块前缀+去类型名                | `http.frame_data(...)`                            |
| `http2_frame_headers`                           | HEADERS 帧类型。                                                              | `frame_headers`                         | 去模块前缀+去类型名                | `http.frame_headers(...)`                         |
| `http2_frame_settings`                          | SETTINGS 帧类型（RFC 7540）。                                                   | `frame_settings`                        | 去模块前缀+去类型名                | `http.frame_settings(...)`                        |
| `http2_flag_end_stream`                         | END_STREAM 标志。                                                            | `flag_end_stream`                       | 去模块前缀+去类型名                | `http.flag_end_stream(...)`                       |
| `http2_flag_end_headers`                        | END_HEADERS 标志。                                                           | `flag_end_headers`                      | 去模块前缀+去类型名                | `http.flag_end_headers(...)`                      |
| `http2_flag_ack`                                | SETTINGS ACK 标志。                                                          | `flag_ack`                              | 去模块前缀+去类型名                | `http.flag_ack(...)`                              |
| `http2_setting_max_concurrent_streams`          | SETTINGS MAX_CONCURRENT_STREAMS id（0x0003）。                               | `setting_max_concurrent_streams`        | 去模块前缀+去类型名                | `http.setting_max_concurrent_streams(...)`        |
| `http2_setting_initial_window_size`             | SETTINGS INITIAL_WINDOW_SIZE id（0x0004）。                                  | `setting_initial_window_size`           | 去模块前缀+去类型名                | `http.setting_initial_window_size(...)`           |
| `http2_setting_enable_push`                     | SETTINGS ENABLE_PUSH id（0x0002；RFC 7540 §6.5.2）。                          | `setting_enable_push`                   | 去模块前缀+去类型名                | `http.setting_enable_push(...)`                   |
| `http2_setting_header_table_size`               | SETTINGS HEADER_TABLE_SIZE id（0x0001）。                                    | `setting_header_table_size`             | 去模块前缀+去类型名                | `http.setting_header_table_size(...)`             |
| `http2_setting_max_frame_size`                  | SETTINGS MAX_FRAME_SIZE id（0x0005）。                                       | `setting_max_frame_size`                | 去模块前缀+去类型名                | `http.setting_max_frame_size(...)`                |
| `http2_setting_max_header_list_size`            | SETTINGS MAX_HEADER_LIST_SIZE id（0x0006）。                                 | `setting_max_header_list_size`          | 去模块前缀+去类型名                | `http.setting_max_header_list_size(...)`          |
| `http2_preface_len`                             | 连接 preface 长度（24）。                                                        | `preface_len`                           | 去模块前缀+去类型名                | `http.preface_len(...)`                           |
| `http2_is_connection_preface`                   | 检测 buf 是否为 HTTP/2 连接 preface；1 是，0 否。                                     | `is_connection_preface`                 | 去模块前缀+去类型名                | `http.is_connection_preface(...)`                 |
| `http2_parse_frame_header`                      | 解析 9 字节帧头；成功 0，失败 -1。                                                     | `parse_frame_header`                    | 去模块前缀+去类型名                | `http.parse_frame_header(...)`                    |
| `http2_build_settings_ack`                      | 构建 SETTINGS ACK 帧（9 字节）；成功返回写入长度。                                         | `build_settings_ack`                    | 去模块前缀+去类型名                | `http.build_settings_ack(...)`                    |
| `http2_build_settings_one`                      | 构建含一条 SETTINGS 的帧（15 字节）。                                                 | `build_settings_one`                    | 去模块前缀+去类型名                | `http.build_settings_one(...)`                    |
| `http2_wire_is_available`                       | 线格式层（preface/帧头）是否可用。                                                     | `wire_is_available`                     | 去模块前缀+去类型名                | `http.wire_is_available(...)`                     |
| `http2_client_is_available`                     | 完整 HTTP/2 客户端编解码是否可用（v1+ HPACK+帧；v3 动态表；网络 ALPN v2）。                      | `client_is_available`                   | 去模块前缀+去类型名                | `http.client_is_available(...)`                   |
| `http2_hpack_encode_indexed`                    | HPACK indexed 头字段编码；成功返回写入字节数。                                            | `hpack_encode_indexed`                  | 去模块前缀+去类型名                | `http.hpack_encode_indexed(...)`                  |
| `http2_hpack_encode_literal`                    | HPACK literal 头字段（不索引，indexed name）编码。                                    | `hpack_encode_literal`                  | 去模块前缀+去类型名                | `http.hpack_encode_literal(...)`                  |
| `http2_hpack_encode_get_request`                | 编码 GET 请求 HPACK 块（:method/:scheme/:path/:authority）。                      | `hpack_encode_get_request`              | 去模块前缀+去类型名                | `http.hpack_encode_get_request(...)`              |
| `http2_hpack_dyn_reset`                         | 清空 HPACK 动态表（v3 FIFO 16 条目）。                                              | `hpack_dyn_reset`                       | 去模块前缀+去类型名                | `http.hpack_dyn_reset(...)`                       |
| `http2_hpack_dyn_count`                         | 返回 HPACK 动态表当前条目数。                                                        | `hpack_dyn_count`                       | 去模块前缀+去类型名                | `http.hpack_dyn_count(...)`                       |
| `http2_hpack_encode_literal_incremental`        | HPACK incremental indexing + indexed name 编码（并写入动态表）。                     | `hpack_encode_literal_incremental`      | 去模块前缀+去类型名                | `http.hpack_encode_literal_incremental(...)`      |
| `http2_hpack_encode_indexed_any`                | HPACK indexed 头（静态或动态 index≥62）。                                          | `hpack_encode_indexed_any`              | 去模块前缀+去类型名                | `http.hpack_encode_indexed_any(...)`              |
| `http2_hpack_encode_request`                    | 编码多 method 请求 HPACK 块（复用 authority/path 动态表）。method_u8 与 Method 一致。       | `hpack_encode_request`                  | 去模块前缀+去类型名                | `http.hpack_encode_request(...)`                  |
| `http2_hpack_dyn_smoke`                         | HPACK 动态表 C 烟测；0 通过。                                                      | `hpack_dyn_smoke`                       | 去模块前缀+去类型名                | `http.hpack_dyn_smoke(...)`                       |
| `http2_hpack_server_dyn_create`                 | 由对端 SETTINGS 创建 server 响应 HPACK 动态表句柄；失败 0。                               | `hpack_server_dyn_create`               | 去模块前缀+去类型名                | `http.hpack_server_dyn_create(...)`               |
| `http2_hpack_server_dyn_destroy`                | 释放 server HPACK 动态表句柄。                                                    | `hpack_server_dyn_destroy`              | 去模块前缀+去类型名                | `http.hpack_server_dyn_destroy(...)`              |
| `http2_hpack_server_dyn_set_table_size`         | 更新 server 动态表上限（HEADER_TABLE_SIZE 联动）。                                    | `hpack_server_dyn_set_table_size`       | 去模块前缀+去类型名                | `http.hpack_server_dyn_set_table_size(...)`       |
| `http2_hpack_server_dyn_max_size`               | 返回 server 动态表 HEADER_TABLE_SIZE 上限。                                       | `hpack_server_dyn_max_size`             | 去模块前缀+去类型名                | `http.hpack_server_dyn_max_size(...)`             |
| `http2_hpack_server_dyn_count`                  | 返回 server 动态表条目数。                                                         | `hpack_server_dyn_count`                | 去模块前缀+去类型名                | `http.hpack_server_dyn_count(...)`                |
| `http2_hpack_server_encode_status`              | 使用 server 动态表编码 :status 响应 HPACK 块。                                       | `hpack_server_encode_status`            | 去模块前缀+去类型名                | `http.hpack_server_encode_status(...)`            |
| `http2_hpack_server_dyn_smoke`                  | server HPACK 动态表离线烟测；0 通过。                                                | `hpack_server_dyn_smoke`                | 去模块前缀+去类型名                | `http.hpack_server_dyn_smoke(...)`                |
| `http2_frame_payload_limit`                     | 返回有效单帧 payload 上限（RFC 7540 MAX_FRAME_SIZE）。                               | `frame_payload_limit`                   | 去模块前缀+去类型名                | `http.frame_payload_limit(...)`                   |
| `http2_frame_check_payload`                     | 校验 payload 是否不超过 max_frame_size；合法 0，超限 -1。                               | `frame_check_payload`                   | 去模块前缀+去类型名                | `http.frame_check_payload(...)`                   |
| `http2_frame_count_data_chunks`                 | 计算 DATA 按 max_frame_size 分片后的帧数。                                          | `frame_count_data_chunks`               | 去模块前缀+去类型名                | `http.frame_count_data_chunks(...)`               |
| `http2_frame_capped_smoke`                      | MAX_FRAME_SIZE 分片计算烟测；0 通过。                                               | `frame_capped_smoke`                    | 去模块前缀+去类型名                | `http.frame_capped_smoke(...)`                    |
| `http2_frame_goaway`                            | GOAWAY 帧类型（RFC 7540 §6.8，0x07）。                                           | `frame_goaway`                          | 去模块前缀+去类型名                | `http.frame_goaway(...)`                          |
| `http2_goaway_error_no_error`                   | GOAWAY NO_ERROR 错误码（0）。                                                   | `goaway_error_no_error`                 | 去模块前缀+去类型名                | `http.goaway_error_no_error(...)`                 |
| `http2_build_goaway`                            | 构建 GOAWAY 帧；成功返回 17。                                                      | `build_goaway`                          | 去模块前缀+去类型名                | `http.build_goaway(...)`                          |
| `http2_parse_goaway`                            | 解析 GOAWAY payload（8 字节）；成功 0。                                             | `parse_goaway`                          | 去模块前缀+去类型名                | `http.parse_goaway(...)`                          |
| `http2_goaway_smoke`                            | GOAWAY 编解码离线烟测；0 通过。                                                      | `goaway_smoke`                          | 去模块前缀+去类型名                | `http.goaway_smoke(...)`                          |
| `http2_frame_ping`                              | PING 帧类型（RFC 7540 §6.7，0x06）。                                             | `frame_ping`                            | 去模块前缀+去类型名                | `http.frame_ping(...)`                            |
| `http2_build_ping`                              | 构建 PING 帧（8 字节 opaque）；成功返回 17。                                           | `build_ping`                            | 去模块前缀+去类型名                | `http.build_ping(...)`                            |
| `http2_build_ping_ack`                          | 构建 PONG（PING+ACK）帧；成功返回 17。                                               | `build_ping_ack`                        | 去模块前缀+去类型名                | `http.build_ping_ack(...)`                        |
| `http2_parse_ping`                              | 解析 PING payload（8 字节）；成功 0。                                               | `parse_ping`                            | 去模块前缀+去类型名                | `http.parse_ping(...)`                            |
| `http2_ping_smoke`                              | PING/PONG 编解码离线烟测；0 通过。                                                   | `ping_smoke`                            | 去模块前缀+去类型名                | `http.ping_smoke(...)`                            |
| `http2_frame_rst_stream`                        | RST_STREAM 帧类型（RFC 7540 §6.4，0x03）。                                       | `frame_rst_stream`                      | 去模块前缀+去类型名                | `http.frame_rst_stream(...)`                      |
| `http2_rst_error_cancel`                        | RST_STREAM CANCEL 错误码（8）。                                                 | `rst_error_cancel`                      | 去模块前缀+去类型名                | `http.rst_error_cancel(...)`                      |
| `http2_build_rst_stream`                        | 构建 RST_STREAM 帧；成功返回 13。                                                  | `build_rst_stream`                      | 去模块前缀+去类型名                | `http.build_rst_stream(...)`                      |
| `http2_parse_rst_stream`                        | 解析 RST_STREAM payload（4 字节）；成功 0。                                         | `parse_rst_stream`                      | 去模块前缀+去类型名                | `http.parse_rst_stream(...)`                      |
| `http2_rst_stream_smoke`                        | RST_STREAM 编解码离线烟测；0 通过。                                                  | `rst_stream_smoke`                      | 去模块前缀+去类型名                | `http.rst_stream_smoke(...)`                      |
| `http2_hpack_decode_status`                     | 从 HPACK 头块解析 :status；成功 0，未找到 -2，失败 -1。                                   | `hpack_decode_status`                   | 去模块前缀+去类型名                | `http.hpack_decode_status(...)`                   |
| `http2_hpack_decode_get_request`                | 从 HPACK 请求头块解析 GET 与 :path；非 GET 返回 1。                                    | `hpack_decode_get_request`              | 去模块前缀+去类型名                | `http.hpack_decode_get_request(...)`              |
| `http2_hpack_encode_status`                     | 编码 :status 响应 HPACK 块（200 用 indexed）。                                     | `hpack_encode_status`                   | 去模块前缀+去类型名                | `http.hpack_encode_status(...)`                   |
| `http2_build_headers_frame`                     | 构建 HEADERS 帧（9 字节头 + HPACK payload）。                                      | `build_headers_frame`                   | 去模块前缀+去类型名                | `http.build_headers_frame(...)`                   |
| `http2_build_data_frame`                        | 构建 DATA 帧。                                                                | `build_data_frame`                      | 去模块前缀+去类型名                | `http.build_data_frame(...)`                      |
| `http2_build_get_headers_frame`                 | 构建 stream 1 GET HEADERS 帧（END_STREAM|END_HEADERS）。                        | `build_get_headers_frame`               | `http.END_HEADERS）。(...)` | `http.build_get_headers_frame(...)`               |
| `http2_build_request_headers_frame`             | —                                                                         | `build_request_headers_frame`           | 去模块前缀+去类型名                | `http.build_request_headers_frame(...)`           |
| `http2_hpack_smoke`                             | HPACK 编解码 C 烟测；0 通过。                                                      | `hpack_smoke`                           | 去模块前缀+去类型名                | `http.hpack_smoke(...)`                           |
| `http2_client_smoke`                            | HTTP/2 客户端多路复用 C 烟测；0 通过。                                                 | `client_smoke`                          | 去模块前缀+去类型名                | `http.client_smoke(...)`                          |
| `http2_network_is_available`                    | h2 over TLS 网络栈是否可用（须链入 std.net TLS）。                                     | `network_is_available`                  | 去模块前缀+去类型名                | `http.network_is_available(...)`                  |
| `http2_network_smoke`                           | 网络层烟测（ALPN wire + HPACK；不建连）；0 通过。                                        | `network_smoke`                         | 去模块前缀+去类型名                | `http.network_smoke(...)`                         |
...
| `http2_conn_pool_create_h2c`                    | —                                                                         | `conn_pool_create_h2c`                  | 去模块前缀+去类型名                | `http.conn_pool_create_h2c(...)`                  |
| `http2_conn_pool_create_h2`                     | —                                                                         | `conn_pool_create_h2`                   | 去模块前缀+去类型名                | `http.conn_pool_create_h2(...)`                   |
| `http2_conn_pool_destroy`                       | 销毁 H2 连接池并关闭 idle 连接。                                                     | `conn_pool_destroy`                     | 去模块前缀+去类型名                | `http.conn_pool_destroy(...)`                     |
| `http2_conn_pool_request`                       | —                                                                         | `conn_pool_request`                     | 去模块前缀+去类型名                | `http.conn_pool_request(...)`                     |
...
| `http2_conn_pool_is_available`                  | H2 连接池 API 是否可用。                                                          | `conn_pool_is_available`                | 去模块前缀+去类型名                | `http.conn_pool_is_available(...)`                |
| `http2_conn_pool_smoke`                         | H2 连接池 C 烟测；0 通过。                                                         | `conn_pool_smoke`                       | 去模块前缀+去类型名                | `http.conn_pool_smoke(...)`                       |
| `http2_conn_pool_goaway_smoke`                  | 连接池 GOAWAY 感知烟测（离线 + h2c fork 双连）；0 通过。                                   | `conn_pool_goaway_smoke`                | 去模块前缀+去类型名                | `http.conn_pool_goaway_smoke(...)`                |
| `http2_conn_pool_connect_count`                 | 池累计新建 TCP/TLS 连接次数（烟测/诊断）。                                                | `conn_pool_connect_count`               | 去模块前缀+去类型名                | `http.conn_pool_connect_count(...)`               |
| `http2_global_pool_create`                      | —                                                                         | `global_pool_create`                    | 去模块前缀+去类型名                | `http.global_pool_create(...)`                    |
| `http2_global_pool_destroy`                     | 销毁全局池及全部子连接池。                                                             | `global_pool_destroy`                   | 去模块前缀+去类型名                | `http.global_pool_destroy(...)`                   |
| `http2_global_pool_get`                         | —                                                                         | `global_pool_get`                       | 去模块前缀+去类型名                | `http.global_pool_get(...)`                       |
| `http2_global_pool_request`                     | —                                                                         | `global_pool_request`                   | 去模块前缀+去类型名                | `http.global_pool_request(...)`                   |
| `http2_global_pool_entry_count`                 | 全局池已登记的不同 host:port 子池数量。                                                 | `global_pool_entry_count`               | 去模块前缀+去类型名                | `http.global_pool_entry_count(...)`               |
| `http2_global_pool_connect_count`               | 全局池内全部子池累计新建连接次数。                                                         | `global_pool_connect_count`             | 去模块前缀+去类型名                | `http.global_pool_connect_count(...)`             |
| `http2_global_pool_is_available`                | 全局 H2 连接池 API 是否可用。                                                       | `global_pool_is_available`              | 去模块前缀+去类型名                | `http.global_pool_is_available(...)`              |
| `http2_global_pool_smoke`                       | 全局 H2 连接池 C 烟测；0 通过。                                                      | `global_pool_smoke`                     | 去模块前缀+去类型名                | `http.global_pool_smoke(...)`                     |
| `http2_http2_complete_smoke`                    | HTTP/2 v1 收口烟测（RST + 全局池 GOAWAY）；0 通过。                                    | `complete_smoke`                        | 去模块前缀+去类型名                | `http.complete_smoke(...)`                        |
...
| `http2_server_serve_h2c`                        | 对已 accept 的 h2c client fd 处理单次 GET 并回 200+body。                           | `server_serve_h2c`                      | 去模块前缀+去类型名                | `http.server_serve_h2c(...)`                      |
| `http2_server_serve_h2c_with_goaway`            | 对已连接 h2c fd：单次 GET 200+body 后发 GOAWAY（NO_ERROR）。                          | `server_serve_h2c_with_goaway`          | 去模块前缀+去类型名                | `http.server_serve_h2c_with_goaway(...)`          |
...
| `http2_server_serve_h2c_ping_echo`              | 对已连接 h2c fd：handshake 后读 PING 并回 PONG。                                    | `server_serve_h2c_ping_echo`            | 去模块前缀+去类型名                | `http.server_serve_h2c_ping_echo(...)`            |
| `serve_h2c_one_with_push`                       | —                                                                         | `serve_h2c_one_with_push`               | 已符合命名                     | `http.serve_h2c_one_with_push(...)`（已简洁）          |
| `http2_server_serve_h2c_with_push`              | 对已连接 h2c fd：PUSH_PROMISE + push body + 主 GET 200+body。                    | `server_serve_h2c_with_push`            | 去模块前缀+去类型名                | `http.server_serve_h2c_with_push(...)`            |
| `http2_server_push_is_available`                | HTTP/2 server push 发送侧 API 是否可用（v17）。                                     | `server_push_is_available`              | 去模块前缀+去类型名                | `http.server_push_is_available(...)`              |
| `http2_server_push_smoke`                       | server push 烟测（离线 PUSH_PROMISE + fork h2c/h2 集成）；0 通过。                    | `server_push_smoke`                     | 去模块前缀+去类型名                | `http.server_push_smoke(...)`                     |
| `http2_server_push_tls_smoke`                   | TLS h2 server push fork 烟测；TLS 不可用时 C 层跳过返回 0。                            | `server_push_tls_smoke`                 | 去模块前缀+去类型名                | `http.server_push_tls_smoke(...)`                 |
| `http2_server_push_settings_smoke`              | server push ENABLE_PUSH 协商烟测（client=0 时软拒绝）；0 通过。                         | `server_push_settings_smoke`            | 去模块前缀+去类型名                | `http.server_push_settings_smoke(...)`            |
| `http2_server_settings_full_smoke`              | server 全量 SETTINGS 协商烟测（fork 读 server SETTINGS）；0 通过。                     | `server_settings_full_smoke`            | 去模块前缀+去类型名                | `http.server_settings_full_smoke(...)`            |
| `http2_server_hpack_dyn_smoke`                  | server HPACK 动态表 + 多 stream 集成烟测；0 通过。                                    | `server_hpack_dyn_smoke`                | 去模块前缀+去类型名                | `http.server_hpack_dyn_smoke(...)`                |
| `http2_server_max_frame_smoke`                  | server MAX_FRAME_SIZE 分片 DATA 烟测；0 通过。                                    | `server_max_frame_smoke`                | 去模块前缀+去类型名                | `http.server_max_frame_smoke(...)`                |
| `http2_conn_goaway_smoke`                       | GOAWAY 编解码 + h2c fork 集成烟测；0 通过。                                          | `conn_goaway_smoke`                     | 去模块前缀+去类型名                | `http.conn_goaway_smoke(...)`                     |
| `http2_conn_ping_smoke`                         | PING/PONG 编解码 + h2c fork 集成烟测；0 通过。                                       | `conn_ping_smoke`                       | 去模块前缀+去类型名                | `http.conn_ping_smoke(...)`                       |
| `serve_h2c_multi_one`                           | —                                                                         | `serve_h2c_multi_one`                   | 已符合命名                     | `http.serve_h2c_multi_one(...)`（已简洁）              |
| `http2_server_serve_h2c_multi`                  | 对已连接 h2c fd 顺序 serve 至多 max_requests 个 GET。                               | `server_serve_h2c_multi`                | 去模块前缀+去类型名                | `http.server_serve_h2c_multi(...)`                |
| `serve_h2c_multi_one_with_push`                 | —                                                                         | `serve_h2c_multi_one_with_push`         | 已符合命名                     | `http.serve_h2c_multi_one_with_push(...)`（已简洁）    |
| `http2_server_serve_h2c_multi_with_push`        | 对已连接 h2c fd 顺序 serve 至多 max_requests 个带 push 的 GET。                       | `server_serve_h2c_multi_with_push`      | 去模块前缀+去类型名                | `http.server_serve_h2c_multi_with_push(...)`      |
| `http2_server_is_available`                     | HTTP/2 h2c server API 是否可用。                                               | `server_is_available`                   | 去模块前缀+去类型名                | `http.server_is_available(...)`                   |
| `http2_server_smoke`                            | HTTP/2 h2c server C 烟测（含 fork 集成）；0 通过。                                   | `server_smoke`                          | 去模块前缀+去类型名                | `http.server_smoke(...)`                          |
| `http2_tls_server_ctx_create`                   | —                                                                         | `tls_server_ctx_create`                 | 去模块前缀+去类型名                | `http.tls_server_ctx_create(...)`                 |
| `http2_tls_server_ctx_destroy`                  | 销毁 h2 TLS 服务端上下文。                                                         | `tls_server_ctx_destroy`                | 去模块前缀+去类型名                | `http.tls_server_ctx_destroy(...)`                |
...
| `http2_server_serve_h2`                         | 对已 TLS+ALPN h2 会话处理单次 GET 并回 200+body。                                    | `server_serve_h2`                       | 去模块前缀+去类型名                | `http.server_serve_h2(...)`                       |
| `http2_server_serve_h2_with_push`               | 对已 TLS+h2 会话：PUSH_PROMISE + push body + 主 GET 200+body。                   | `server_serve_h2_with_push`             | 去模块前缀+去类型名                | `http.server_serve_h2_with_push(...)`             |
| `serve_h2_multi_one`                            | —                                                                         | `serve_h2_multi_one`                    | 已符合命名                     | `http.serve_h2_multi_one(...)`（已简洁）               |
| `http2_server_serve_h2_multi`                   | 对已 TLS+h2 会话顺序 serve 至多 max_requests 个 GET。                               | `server_serve_h2_multi`                 | 去模块前缀+去类型名                | `http.server_serve_h2_multi(...)`                 |
| `serve_h2_multi_one_with_push`                  | —                                                                         | `serve_h2_multi_one_with_push`          | 已符合命名                     | `http.serve_h2_multi_one_with_push(...)`（已简洁）     |
| `http2_server_serve_h2_multi_with_push`         | 对已 TLS+h2 会话顺序 serve 至多 max_requests 个带 push 的 GET。                       | `server_serve_h2_multi_with_push`       | 去模块前缀+去类型名                | `http.server_serve_h2_multi_with_push(...)`       |
| `http2_server_multistream_is_available`         | server 多 stream API 是否可用。                                                 | `server_multistream_is_available`       | 去模块前缀+去类型名                | `http.server_multistream_is_available(...)`       |
| `http2_server_multistream_smoke`                | server 多 stream C 烟测；0 通过。                                                | `server_multistream_smoke`              | 去模块前缀+去类型名                | `http.server_multistream_smoke(...)`              |
| `http2_server_multistream_push_is_available`    | HTTP/2 server 多 stream + push API 是否可用（v19）。                              | `server_multistream_push_is_available`  | 去模块前缀+去类型名                | `http.server_multistream_push_is_available(...)`  |
| `http2_server_multistream_push_smoke`           | server 多 stream + push 烟测（h2c + TLS fork 集成）；0 通过。                        | `server_multistream_push_smoke`         | 去模块前缀+去类型名                | `http.server_multistream_push_smoke(...)`         |
| `http2_h2c_client_smoke`                        | h2c URL 客户端 C 烟测（https 须 SCHEME 错误）；0 通过。                                 | `h2c_client_smoke`                      | 去模块前缀+去类型名                | `http.h2c_client_smoke(...)`                      |
| `http2_stream_registry_max`                     | 注册表最大 stream 槽位数（8）。                                                      | `max`                                   | 二次精简：去对象前缀                | `http.max(...)`                                   |
| `http2_stream_registry_init`                    | 初始化 stream 注册表；下一 id 从 1（奇数）开始。                                           | `init`                                  | 二次精简：去对象前缀                | `http.init(...)`                                  |
| `http2_stream_registry_open`                    | 分配并登记新 stream id；表满 -1。                                                   | `open`                                  | 二次精简：去对象前缀                | `http.open(...)`                                  |
| `http2_stream_registry_close`                   | 关闭 stream（保留槽位）。                                                          | `close`                                 | 二次精简：去对象前缀                | `http.close(...)`                                 |
| `http2_stream_registry_is_open`                 | stream 是否 open；1 是，0 否。                                                   | `is_open`                               | 二次精简：去对象前缀                | `http.is_open(...)`                               |
| `http2_stream_registry_smoke`                   | 多 stream 注册表 C 烟测；0 通过。                                                   | `smoke`                                 | 二次精简：去对象前缀                | `http.smoke(...)`                                 |
| `http2_peer_settings_init`                      | 初始化对端 SETTINGS 视图。                                                        | `peer_settings_init`                    | 去模块前缀+去类型名                | `http.peer_settings_init(...)`                    |
| `http2_settings_entry_count`                    | SETTINGS payload 6 字节条目数；plen 非 6 倍数 -1。                                  | `settings_entry_count`                  | 去模块前缀+去类型名                | `http.settings_entry_count(...)`                  |
| `http2_parse_settings_entry`                    | 读取 SETTINGS payload 第 entry_index 条；无条目 -2。                               | `parse_settings_entry`                  | 去模块前缀+去类型名                | `http.parse_settings_entry(...)`                  |
| `http2_peer_settings_apply_entry`               | 应用单条 SETTINGS 到对端视图。                                                      | `peer_settings_apply_entry`             | 去模块前缀+去类型名                | `http.peer_settings_apply_entry(...)`             |
| `http2_peer_settings_consume_payload`           | 解析完整 SETTINGS payload 并累加。                                                | `peer_settings_consume_payload`         | 去模块前缀+去类型名                | `http.peer_settings_consume_payload(...)`         |
| `http2_build_client_settings`                   | 构建客户端 SETTINGS（含 ENABLE_PUSH=1）；成功 27。                                    | `build_client_settings`                 | 去模块前缀+去类型名                | `http.build_client_settings(...)`                 |
| `http2_build_client_settings_ex`                | 构建含 ENABLE_PUSH 的客户端 SETTINGS 帧；enable_push 0=拒绝 server push。             | `build_client_settings_ex`              | 去模块前缀+去类型名                | `http.build_client_settings_ex(...)`              |
| `http2_build_client_settings_with_max_frame`    | 构建含 MAX_FRAME_SIZE 的 client SETTINGS 帧；成功 33。                             | `build_client_settings_with_max_frame`  | 去模块前缀+去类型名                | `http.build_client_settings_with_max_frame(...)`  |
| `http2_build_server_settings`                   | 构建 server 全量 SETTINGS 帧（含 HEADER_TABLE_SIZE/MAX_FRAME_SIZE）；成功 39。        | `build_server_settings`                 | 去模块前缀+去类型名                | `http.build_server_settings(...)`                 |
| `http2_peer_settings_enable_push`               | 对端是否允许 server push（ENABLE_PUSH=0 时 false）。                                | `peer_settings_enable_push`             | 去模块前缀+去类型名                | `http.peer_settings_enable_push(...)`             |
| `http2_peer_settings_header_table_size`         | 对端 HEADER_TABLE_SIZE（未设置时 4096）。                                          | `peer_settings_header_table_size`       | 去模块前缀+去类型名                | `http.peer_settings_header_table_size(...)`       |
| `http2_peer_settings_max_frame_size`            | 对端 MAX_FRAME_SIZE（未设置时 16384）。                                            | `peer_settings_max_frame_size`          | 去模块前缀+去类型名                | `http.peer_settings_max_frame_size(...)`          |
| `http2_peer_settings_max_header_list_size`      | 对端 MAX_HEADER_LIST_SIZE（未设置时 0=无限制）。                                      | `peer_settings_max_header_list_size`    | 去模块前缀+去类型名                | `http.peer_settings_max_header_list_size(...)`    |
| `http2_peer_settings_max_streams`               | 有效并发 stream 上限（未设置时默认 100）。                                               | `peer_settings_max_streams`             | 去模块前缀+去类型名                | `http.peer_settings_max_streams(...)`             |
| `http2_peer_settings_initial_window`            | 有效初始窗口（未设置时 65535）。                                                       | `peer_settings_initial_window`          | 去模块前缀+去类型名                | `http.peer_settings_initial_window(...)`          |
| `http2_settings_smoke`                          | SETTINGS 解析 C 烟测；0 通过。                                                    | `settings_smoke`                        | 去模块前缀+去类型名                | `http.settings_smoke(...)`                        |
| `http2_multistream_client_init`                 | 初始化多 stream 并发客户端。                                                        | `multistream_client_init`               | 去模块前缀+去类型名                | `http.multistream_client_init(...)`               |
| `http2_multistream_client_on_settings`          | 应用对端 SETTINGS payload。                                                    | `multistream_client_on_settings`        | 去模块前缀+去类型名                | `http.multistream_client_on_settings(...)`        |
| `http2_multistream_client_open_stream`          | 在 SETTINGS 限制内 open stream；超上限 http2_err_max_streams(-1236)。              | `multistream_client_open_stream`        | 去模块前缀+去类型名                | `http.multistream_client_open_stream(...)`        |
| `http2_multistream_client_close_stream`         | 关闭 stream。                                                                | `multistream_client_close_stream`       | 去模块前缀+去类型名                | `http.multistream_client_close_stream(...)`       |
| `http2_multistream_client_build_get`            | 在指定 stream 构建 GET HEADERS 帧。                                              | `multistream_client_build_get`          | 去模块前缀+去类型名                | `http.multistream_client_build_get(...)`          |
| `http2_multistream_client_build_parallel_get`   | 并发构建 n 个 GET HEADERS 帧（依次 open stream）。                                   | `multistream_client_build_parallel_get` | 去模块前缀+去类型名                | `http.multistream_client_build_parallel_get(...)` |
| `http2_multistream_client_smoke`                | 多 stream 并发客户端 C 烟测；0 通过。                                                 | `multistream_client_smoke`              | 去模块前缀+去类型名                | `http.multistream_client_smoke(...)`              |
| `http2_conn_reuse_is_available`                 | 单连接复用 API 是否可用。                                                           | `conn_reuse_is_available`               | 去模块前缀+去类型名                | `http.conn_reuse_is_available(...)`               |
| `http2_conn_init`                               | 初始化可复用 HTTP/2 连接。                                                         | `conn_init`                             | 去模块前缀+去类型名                | `http.conn_init(...)`                             |
| `http2_conn_attach_h2c`                         | 绑定 cleartext h2c fd（须已 connect）。                                          | `conn_attach_h2c`                       | 去模块前缀+去类型名                | `http.conn_attach_h2c(...)`                       |
| `http2_conn_attach_tls`                         | 绑定已协商 h2 的 TLS ctx。                                                       | `conn_attach_tls`                       | 去模块前缀+去类型名                | `http.conn_attach_tls(...)`                       |
| `http2_conn_is_ready`                           | 连接是否已完成 handshake。                                                        | `conn_is_ready`                         | 去模块前缀+去类型名                | `http.conn_is_ready(...)`                         |
| `http2_conn_handshake`                          | 发送 preface + SETTINGS 并完成协商；成功 0。                                         | `conn_handshake`                        | 去模块前缀+去类型名                | `http.conn_handshake(...)`                        |
| `http2_conn_handshake_with_enable_push`         | handshake 并显式设置 client ENABLE_PUSH（0=拒绝 server push）。                     | `conn_handshake_with_enable_push`       | 去模块前缀+去类型名                | `http.conn_handshake_with_enable_push(...)`       |
| `http2_conn_handshake_with_max_frame`           | handshake 并声明 client MAX_FRAME_SIZE（server 分片 DATA 联动）。                   | `conn_handshake_with_max_frame`         | 去模块前缀+去类型名                | `http.conn_handshake_with_max_frame(...)`         |
| `http2_conn_request`                            | —                                                                         | `conn_request`                          | 去模块前缀+去类型名                | `http.conn_request(...)`                          |
| `http2_conn_close`                              | 标记连接关闭（不 shutdown 底层 fd/TLS）。                                             | `conn_close`                            | 去模块前缀+去类型名                | `http.conn_close(...)`                            |
| `http2_conn_shutdown_graceful`                  | —                                                                         | `conn_shutdown_graceful`                | 去模块前缀+去类型名                | `http.conn_shutdown_graceful(...)`                |
| `http2_conn_read_goaway`                        | 读下一 GOAWAY 帧；非 GOAWAY 返回 http2_err_goaway(-1244)。                         | `conn_read_goaway`                      | 去模块前缀+去类型名                | `http.conn_read_goaway(...)`                      |
| `http2_conn_ping`                               | 在已 ready 连接上发 PING 并等待匹配 PONG；失败 http2_err_ping(-1245)。                   | `conn_ping`                             | 去模块前缀+去类型名                | `http.conn_ping(...)`                             |
| `http2_conn_goaway_seen`                        | 连接是否已收到对端 GOAWAY（不可再复用）。                                                  | `conn_goaway_seen`                      | 去模块前缀+去类型名                | `http.conn_goaway_seen(...)`                      |
| `http2_conn_is_pool_reusable`                   | 连接是否可归还 H2 连接池 idle 栈。                                                    | `conn_is_pool_reusable`                 | 去模块前缀+去类型名                | `http.conn_is_pool_reusable(...)`                 |
| `http2_conn_reset_stream`                       | 向对端发送 RST_STREAM 取消 stream；失败 http2_err_rst_stream(-1246) 读路径。            | `conn_reset_stream`                     | 去模块前缀+去类型名                | `http.conn_reset_stream(...)`                     |
| `http2_conn_reuse_smoke`                        | 单连接复用 C 烟测；0 通过。                                                          | `conn_reuse_smoke`                      | 去模块前缀+去类型名                | `http.conn_reuse_smoke(...)`                      |
| `http2_alpn_h2_len`                             | ALPN 协议名 "h2" 长度（2）。                                                      | `alpn_h2_len`                           | 去模块前缀+去类型名                | `http.alpn_h2_len(...)`                           |
| `http2_write_alpn_h2`                           | 将 ALPN "h2" 写入 out；成功返回 2。                                                | `write_alpn_h2`                         | 去模块前缀+去类型名                | `http.write_alpn_h2(...)`                         |
| `http2_smoke`                                   | HTTP/2 线格式 C 烟测；0 通过。                                                     | `smoke`                                 | 去模块前缀+去类型名                | `http.smoke(...)`                                 |
| `http2_frame_window_update`                     | WINDOW_UPDATE 帧类型（RFC 7540 §6.9）。                                         | `frame_window_update`                   | 去模块前缀+去类型名                | `http.frame_window_update(...)`                   |
| `http2_default_initial_window`                  | 默认初始流控窗口（65535）。                                                          | `default_initial_window`                | 去模块前缀+去类型名                | `http.default_initial_window(...)`                |
| `http2_build_window_update`                     | 构建 WINDOW_UPDATE 帧；成功返回 13。                                               | `build_window_update`                   | 去模块前缀+去类型名                | `http.build_window_update(...)`                   |
| `http2_hpack_huffman_decode`                    | HPACK Huffman 解码（RFC 7541）；成功返回解码字节数。                                     | `hpack_huffman_decode`                  | 去模块前缀+去类型名                | `http.hpack_huffman_decode(...)`                  |
| `http2_hpack_huffman_is_available`              | HPACK Huffman 是否可用。                                                       | `hpack_huffman_is_available`            | 去模块前缀+去类型名                | `http.hpack_huffman_is_available(...)`            |
| `http2_hpack_huffman_smoke`                     | Huffman + 流控 C 烟测（含于 http2_network_smoke）。                                | `hpack_huffman_smoke`                   | 去模块前缀+去类型名                | `http.hpack_huffman_smoke(...)`                   |
| `http2_flow_control_smoke`                      | 流控 WINDOW_UPDATE C 烟测；0 通过。                                               | `flow_control_smoke`                    | 去模块前缀+去类型名                | `http.flow_control_smoke(...)`                    |
| `http2_flow_state_init`                         | 初始化连接/流窗口为默认 65535。                                                       | `flow_state_init`                       | 去模块前缀+去类型名                | `http.flow_state_init(...)`                       |
| `http2_flow_state_reset_stream`                 | 重置流窗口（保留连接窗口）；用于新 stream。                                                 | `flow_state_reset_stream`               | 去模块前缀+去类型名                | `http.flow_state_reset_stream(...)`               |
| `http2_flow_state_apply_initial_window`         | 应用 SETTINGS INITIAL_WINDOW_SIZE 到新 stream 基准。                             | `flow_state_apply_initial_window`       | 去模块前缀+去类型名                | `http.flow_state_apply_initial_window(...)`       |
| `http2_flow_state_apply_window_update`          | —                                                                         | `flow_state_apply_window_update`        | 去模块前缀+去类型名                | `http.flow_state_apply_window_update(...)`        |
| `http2_flow_state_max_send`                     | 返回当前可发送字节数 min(conn, stream, want)；0 表示背压阻塞。                              | `flow_state_max_send`                   | 去模块前缀+去类型名                | `http.flow_state_max_send(...)`                   |
| `http2_flow_state_can_send`                     | want 字节是否可发送（true 可发，false 背压）。                                           | `flow_state_can_send`                   | 去模块前缀+去类型名                | `http.flow_state_can_send(...)`                   |
| `http2_flow_state_consume_send`                 | —                                                                         | `flow_state_consume_send`               | 去模块前缀+去类型名                | `http.flow_state_consume_send(...)`               |
| `http2_parse_window_update_payload`             | 解析 WINDOW_UPDATE payload（4 字节）；成功 0，失败 -1。                                | `parse_window_update_payload`           | 去模块前缀+去类型名                | `http.parse_window_update_payload(...)`           |
| `http2_flow_state_smoke`                        | 流控状态机 C 烟测；0 通过。                                                          | `flow_state_smoke`                      | 去模块前缀+去类型名                | `http.flow_state_smoke(...)`                      |
| `http2_frame_push_promise`                      | PUSH_PROMISE 帧类型（RFC 7540 §6.6）。                                          | `frame_push_promise`                    | 去模块前缀+去类型名                | `http.frame_push_promise(...)`                    |
| `http2_is_push_promise_frame`                   | ftype 是否为 PUSH_PROMISE。                                                   | `is_push_promise_frame`                 | 去模块前缀+去类型名                | `http.is_push_promise_frame(...)`                 |
| `http2_parse_push_promise_stream`               | 解析 PUSH_PROMISE promised stream id；成功 0。                                  | `parse_push_promise_stream`             | 去模块前缀+去类型名                | `http.parse_push_promise_stream(...)`             |
| `http2_is_h2c_upgrade_response`                 | 检测 HTTP/1.1 101 + Upgrade: h2c 响应。                                        | `is_h2c_upgrade_response`               | 去模块前缀+去类型名                | `http.is_h2c_upgrade_response(...)`               |
| `http2_h2c_is_available`                        | cleartext h2c TCP 会话 API 是否可用（v8）。                                        | `h2c_is_available`                      | 去模块前缀+去类型名                | `http.h2c_is_available(...)`                      |
| `http2_push_h2c_smoke`                          | push + h2c wire C 烟测；0 通过。                                                | `push_h2c_smoke`                        | 去模块前缀+去类型名                | `http.push_h2c_smoke(...)`                        |
| `http2_h2c_wire_is_available`                   | cleartext h2c 起始 preface 是否可用（v7 wire）。                                   | `h2c_wire_is_available`                 | 去模块前缀+去类型名                | `http.h2c_wire_is_available(...)`                 |
| `http2_h2c_session_begin`                       | 构建 h2c 直连起始 preface（24 字节）；成功返回 24。                                       | `h2c_session_begin`                     | 去模块前缀+去类型名                | `http.h2c_session_begin(...)`                     |
| `http2_push_fetch_smoke`                        | push 资源离线收集 C 烟测；0 通过。                                                    | `push_fetch_smoke`                      | 去模块前缀+去类型名                | `http.push_fetch_smoke(...)`                      |
| `http2_push_last_reset`                         | 重置读路径 push 收集状态。                                                          | `push_last_reset`                       | 去模块前缀+去类型名                | `http.push_last_reset(...)`                       |
| `http2_push_last_copy`                          | 复制最近一次 push body；成功返回 body_len，无 push 返回 0。                               | `push_last_copy`                        | 去模块前缀+去类型名                | `http.push_last_copy(...)`                        |
| `push_last_body_owned`                          | —                                                                         | `push_last_body_owned`                  | 已符合命名                     | `http.push_last_body_owned(...)`（已简洁）             |
| `http2_push_network_smoke`                      | push 网络拉取 C 烟测；0 通过。                                                      | `push_network_smoke`                    | 去模块前缀+去类型名                | `http.push_network_smoke(...)`                    |
| `http2_h2c_network_smoke`                       | h2c 明文 TCP 会话 C 烟测；0 通过。                                                  | `h2c_network_smoke`                     | 去模块前缀+去类型名                | `http.h2c_network_smoke(...)`                     |
| `http2_flow_recv_init`                          | 初始化 recv 窗口为默认 65535。                                                     | `flow_recv_init`                        | 去模块前缀+去类型名                | `http.flow_recv_init(...)`                        |
| `http2_flow_recv_reset_stream`                  | 重置流 recv 窗口。                                                              | `flow_recv_reset_stream`                | 去模块前缀+去类型名                | `http.flow_recv_reset_stream(...)`                |
| `http2_flow_recv_on_data`                       | 收到 DATA 后扣减 recv 窗口；成功 0。                                                 | `flow_recv_on_data`                     | 去模块前缀+去类型名                | `http.flow_recv_on_data(...)`                     |
| `http2_flow_recv_release`                       | 释放已消费字节并构建 WINDOW_UPDATE；成功返回帧长度 13。                                      | `flow_recv_release`                     | 去模块前缀+去类型名                | `http.flow_recv_release(...)`                     |
| `http2_flow_recv_smoke`                         | 接收侧流控 C 烟测；0 通过。                                                          | `flow_recv_smoke`                       | 去模块前缀+去类型名                | `http.flow_recv_smoke(...)`                       |
...
| `timeout_ms_for_context`      | 从 Context 推导 read/write 超时毫秒；无 deadline 返回 0；取消/过期返回 IO_CTX_MS_*。                                                                 | `timeout_ms_for_context`      | 已符合命名      | `io.timeout_ms_for_context(...)`      |
...
| `read_ptr_len`                | 返回最近一次 read_ptr/read_stdin_ptr 成功读入的字节数；与 read_ptr/read_stdin_ptr 配套使用。                                                           | `read_ptr_len`                | 已符合命名      | `io.read_ptr_len(...)`                |
| `read_ptr_gen`                | —                                                                                                                                 | `read_ptr_gen`                | 已符合命名      | `io.read_ptr_gen(...)`                |
| `read_ptr_gen_valid`          | ZC-2：saved 与当前 generation 相等返回 1（视图仍有效），否则 0。                                                                                     | `read_ptr_gen_valid`          | 已符合命名      | `io.read_ptr_gen_valid(...)`          |
| `read_ptr_backend`            | ZC-2：上次 read_ptr 后端：0=TLS，1=mmap(Linux)，2=dispatch_data(macOS)。                                                                   | `read_ptr_backend`            | 已符合命名      | `io.read_ptr_backend(...)`            |
| `read_ptr_view`               | —                                                                                                                                 | `read_ptr_view`               | 已符合命名      | `io.read_ptr_view(...)`               |
| `read_ptr_view_valid`         | ZC-2：ReadPtrView 是否仍对应当前有效 read_ptr 视图。                                                                                           | `read_ptr_view_valid`         | 已符合命名      | `io.read_ptr_view_valid(...)`         |
| `read_stdin_ptr_view`         | ZC-2：stdin 版 read_ptr_view。                                                                                                       | `read_stdin_ptr_view`         | 已符合命名      | `io.read_stdin_ptr_view(...)`         |
| `read_ptr_slice`              | —                                                                                                                                 | `read_ptr_slice`              | 已符合命名      | `io.read_ptr_slice(...)`              |
| `read_stdin_ptr_slice`        | 零拷贝读 stdin slice；等价 read_ptr_slice(handle_stdin(), 0)。                                                                            | `read_stdin_ptr_slice`        | 已符合命名      | `io.read_stdin_ptr_slice(...)`        |
...
| `read_with_timeout`           | —                                                                                                                                 | `read_with_timeout`           | 已符合命名      | `io.read_with_timeout(...)`           |
| `write_with_timeout`          | —                                                                                                                                 | `write_with_timeout`          | 已符合命名      | `io.write_with_timeout(...)`          |
| `write_stderr_with_timeout`   | 向 stderr 写入 ptr[0..len]，超时 timeout_ms（0=无超时）。返回写入字节数，-1=错误。                                                                       | `write_stderr_with_timeout`   | 已符合命名      | `io.write_stderr_with_timeout(...)`   |
...
| `read_batch_fd_buf`           | 与 Zig/Rust 对齐：从 fd 按「指针+段数」批量读；buffers 指向至少 n 个 Buffer（ptr/len 有效），n 为 1..16。timeout=0。返回总字节数，-1=错误。                              | `read_batch_fd_buf`           | 已符合命名      | `io.read_batch_fd_buf(...)`           |
| `write_batch_fd_buf`          | 与 Zig/Rust 对齐：向 fd 按「指针+段数」批量写；buffers 同上，n 为 1..16。返回总字节数，-1=错误。                                                                 | `write_batch_fd_buf`          | 已符合命名      | `io.write_batch_fd_buf(...)`          |
| `print_str`                   | —                                                                                                                                 | `print_str`                   | 已符合命名      | `io.print_str(...)`                   |
| `register_fixed_buffers`      | 与 std.io.driver 中曾重复的同名实现已移到仅保留在 mod；asm 合并 dep 时仍依赖 runtime merge 对同 import 路径去重。                                                | `register_fixed_buffers`      | 已符合命名      | `io.register_fixed_buffers(...)`      |
| `register_fixed_buffers_buf`  | 与 Zig/Rust 对齐：按「指针+块数」注册固定 buffer 池；bufs 指向至少 nr 个 Buffer（ptr/len 有效），nr 为 1..8。返回 1 成功，0 失败。                                     | `register_fixed_buffers_buf`  | 已符合命名      | `io.register_fixed_buffers_buf(...)`  |
...
| `register_provided_buffers`   | 注册 provided 池；nr 1..32，bufsz 建议 4096。成功 1，失败/不支持 0。                                                                               | `register_provided_buffers`   | 已符合命名      | `io.register_provided_buffers(...)`   |
| `unregister_provided_buffers` | 注销 provided 池。                                                                                                                    | `unregister_provided_buffers` | 已符合命名      | `io.unregister_provided_buffers(...)` |
...
| `complete_read_async`         | 收割在途 read_async（单 in-flight 兼容）；见 IO_ASYNC_NOT_READY。                                                                             | `complete_read_async`         | 已符合命名      | `io.complete_read_async(...)`         |
| `complete_read_async_slot`    | 收割指定 slot 的 read_async。                                                                                                           | `complete_read_async_slot`    | 已符合命名      | `io.complete_read_async_slot(...)`    |
| `write_async`                 | 提交非阻塞 write；成功返回 slot>=0，失败 -1。                                                                                                   | `write_async`                 | 已符合命名      | `io.write_async(...)`                 |
| `complete_write_async`        | 收割在途 write_async（单 in-flight 兼容）；见 IO_ASYNC_NOT_READY。                                                                            | `complete_write_async`        | 已符合命名      | `io.complete_write_async(...)`        |
| `complete_write_async_slot`   | 收割指定 slot 的 write_async。                                                                                                          | `complete_write_async_slot`   | 已符合命名      | `io.complete_write_async_slot(...)`   |
| `poll_async_completions`      | poll io_uring async CQE 并唤醒 scheduler；返回本次收割的 async 完成数。                                                                          | `poll_async_completions`      | 已符合命名      | `io.poll_async_completions(...)`      |
| `io_uring_is_available`       | 当前线程 io_uring 是否可用（Linux 且 ring 初始化成功）；1 是，0 否。                                                                                   | `uring_available`             | 语义重命名      | `io.uring_available(...)`             |
| `read_into_slice`             | 从 stdin 读入至 buf[0..buf.length]，timeout_ms 毫秒（0=无超时）。返回读入字节数，0=EOF，-1=错误。                                                          | `read_into_slice`             | 已符合命名      | `io.read_into_slice(...)`             |
| `write_from_slice`            | 将 buf[0..buf.length] 写入 stdout，timeout_ms 毫秒（0=无超时）。返回写入字节数，-1=错误。                                                                | `write_from_slice`            | 已符合命名      | `io.write_from_slice(...)`            |
...
| `json_typed_decode_smoke_c`          | extern C/平台                                                        | `typed_decode_smoke`          | 去模块前缀+去类型名（C层） | `json.typed_decode_smoke(...)`          |
...
| `escape_string`                      | —                                                                  | `escape_string`               | 已符合命名          | `json.escape_string(...)`               |
...
| `typed_decode_smoke`                 | 类型化 decode C 烟测钩子（STD-116）。                                        | `typed_decode_smoke`          | 已符合命名          | `json.typed_decode_smoke(...)`          |
...
| 当前名称                        | 功能说明                                                                                                                      | 简化名称                  | 说明                                             | 绑定调用                           |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------- | --------------------- | ---------------------------------------------- | ------------------------------ |
| `log_set_min_level_c`       | std.log — 日志级别与 stderr/文件输出（对标 Zig std.log、Rust log，零分配） STD-053：多 sink（SINK_STDERR | SINK_FILE）+ 级别过滤 + 结构化 KV（OBS-003）。 | `set_min_level`       | `log.SINK_FILE）+ 级别过滤 + 结构化 KV（OBS-003）。(...)` | `log.set_min_level(...)`       |
...
| `map_i32_i32_find`            | 查找 key 所在槽位；若存在返回槽位下标，否则返回 -1。F-03 v1：heap_ops.sx 线性探测。 | `find`              | 去模块前缀+去类型名 | `map.find(...)`              |
...
| `math_special_smoke_c`  | extern C/平台                                                                                                                  | `special_smoke`    | 去模块前缀+去类型名（C层） | `math.special_smoke(...)`    |
...
| `special_smoke`         | C 金样烟测；0=通过。                                                                                                                 | `special_smoke`    | 已符合命名          | `math.special_smoke(...)`    |
...
| `registry_register_counter`      | 注册 Counter 到 Registry；返回索引或负错误码。                               | `registry_register_counter`      | 已符合命名      | `metrics.registry_register_counter(...)`      |
| `registry_register_gauge`        | 注册 Gauge。                                                      | `registry_register_gauge`        | 已符合命名      | `metrics.registry_register_gauge(...)`        |
| `registry_register_histogram`    | 注册 Histogram。                                                  | `registry_register_histogram`    | 已符合命名      | `metrics.registry_register_histogram(...)`    |
| `metrics_append_slice`           | 追加 slice 到 out；返回新 offset，失败 -1。                               | `extend_from_slice`              | 去模块前缀+去类型名 | `metrics.extend_from_slice(...)`              |
...
| `net_tls_alpn_h2_http1_wire_c`     | ALPN 线格式（h2 + http/1.1）；由 net_alpn.sx 提供（F-04 v10）。                                                                                                                          | `tls_alpn_h2_http1_wire`     | 去模块前缀+去类型名（C层） | `net.tls_alpn_h2_http1_wire(...)`     |
...
| `tcp_pool_smoke`                   | TCP 连接池烟测；0 通过。                                                                                                                                                              | `tcp_pool_smoke`             | 已符合命名          | `net.tcp_pool_smoke(...)`             |
...
| `sync_queue_contention_smoke_c` | C 层双线程竞争 push 烟测（queue.sx + runtime_queue_contention.c）；0 通过，-1 失败。 | `sync_queue_contention_smoke` | 去模块前缀+去类型名（C层） | `queue.sync_queue_contention_smoke(...)` |
| `sync_queue_i32_new`            | 创建空 SyncQueue_i32；mutex 分配失败时 lock 为 null。                          | `sync_queue_new`              | 去模块前缀+去类型名     | `queue.sync_queue_new(...)`              |
| `sync_queue_i32_deinit`         | 销毁队列并释放 mutex。                                                      | `sync_queue_deinit`           | 去模块前缀+去类型名     | `queue.sync_queue_deinit(...)`           |
| `sync_queue_i32_push_back`      | 加锁队尾插入；失败 -1。                                                       | `sync_queue_push_back`        | 去模块前缀+去类型名     | `queue.sync_queue_push_back(...)`        |
| `sync_queue_i32_try_pop_front`  | —                                                                   | `sync_queue_try_pop_front`    | 去模块前缀+去类型名     | `queue.sync_queue_try_pop_front(...)`    |
...
| `sync_queue_contention_smoke`   | 门面：C 层 sync_queue_contention_smoke_c 烟测。                            | `sync_queue_contention_smoke` | 已符合命名          | `queue.sync_queue_contention_smoke(...)` |
...
| `random_rng_smoke_c`  | extern C/平台                                               | `rng_smoke`      | 去模块前缀+去类型名（C层） | `random.rng_smoke(...)`      |
| `rng_next_u64`        | SplitMix64 单步；更新 *r 并返回下一个 u64。                           | `rng_next`       | 去模块前缀+去类型名     | `random.rng_next(...)`       |
| `rng_init`            | 用 seed 初始化 PRNG；同 seed 产生同序列。                             | `rng_init`       | 已符合命名          | `random.rng_init(...)`       |
| `rng_from_seed`       | rng_init 别名；烟测与文档沿用 from_seed 命名。                         | `rng_from_seed`  | 已符合命名          | `random.rng_from_seed(...)`  |
| `rng_fill_bytes`      | 用 PRNG 字节填充 buf[0..len)。                                  | `rng_fill_bytes` | 已符合命名          | `random.rng_fill_bytes(...)` |
| `rng_range_u32`       | [lo, hi] 闭区间均匀 u32；lo > hi 时返回 lo。                        | `rng_range`      | 去模块前缀+去类型名     | `random.rng_range(...)`      |
| `rng_smoke`           | C 层 PRNG 烟测；0 成功，非 0 失败。                                  | `rng_smoke`      | 已符合命名          | `random.rng_smoke(...)`      |
...
| `sio_packet_smoke_c`                     | extern C/平台                                                                                                                                                                          | `packet_smoke`                       | 去模块前缀+去类型名（C层） | `socket.packet_smoke(...)`                       |
...
| `sio_polling_smoke_c`                    | extern C/平台                                                                                                                                                                          | `polling_smoke`                      | 去模块前缀+去类型名（C层） | `socket.polling_smoke(...)`                      |
...
| `sio_ns_ack_smoke_c`                     | extern C/平台                                                                                                                                                                          | `ns_ack_smoke`                       | 去模块前缀+去类型名（C层） | `socket.ns_ack_smoke(...)`                       |
...
| `sio_ns_router_smoke_c`                  | extern C/平台                                                                                                                                                                          | `ns_router_smoke`                    | 去模块前缀+去类型名（C层） | `socket.ns_router_smoke(...)`                    |
...
| `sio_ns_sessions_smoke_c`                | extern C/平台                                                                                                                                                                          | `ns_sessions_smoke`                  | 去模块前缀+去类型名（C层） | `socket.ns_sessions_smoke(...)`                  |
...
| `sio_ws_hub_smoke_c`                     | extern C/平台                                                                                                                                                                          | `ws_hub_smoke`                       | 去模块前缀+去类型名（C层） | `socket.ws_hub_smoke(...)`                       |
...
| `sio_room_smoke_c`                       | extern C/平台                                                                                                                                                                          | `room_smoke`                         | 去模块前缀+去类型名（C层） | `socket.room_smoke(...)`                         |
...
| `sio_hub_sync_smoke_c`                   | extern C/平台                                                                                                                                                                          | `hub_sync_smoke`                     | 去模块前缀+去类型名（C层） | `socket.hub_sync_smoke(...)`                     |
...
| `sio_session_sync_smoke_c`               | extern C/平台                                                                                                                                                                          | `session_sync_smoke`                 | 去模块前缀+去类型名（C层） | `socket.session_sync_smoke(...)`                 |
...
| `sio_cluster_sync_smoke_c`               | extern C/平台                                                                                                                                                                          | `cluster_sync_smoke`                 | 去模块前缀+去类型名（C层） | `socket.cluster_sync_smoke(...)`                 |
...
| `sio_cluster_adapter_smoke_c`            | extern C/平台                                                                                                                                                                          | `cluster_adapter_smoke`              | 去模块前缀+去类型名（C层） | `socket.cluster_adapter_smoke(...)`              |
...
| `sio_cluster_ring_sync_smoke_c`          | extern C/平台                                                                                                                                                                          | `cluster_ring_sync_smoke`            | 去模块前缀+去类型名（C层） | `socket.cluster_ring_sync_smoke(...)`            |
| `sio_p3_complete_smoke_c`                | extern C/平台                                                                                                                                                                          | `p3_complete_smoke`                  | 去模块前缀+去类型名（C层） | `socket.p3_complete_smoke(...)`                  |
...
| `sio_connect_smoke_c`                    | extern C/平台                                                                                                                                                                          | `connect_smoke`                      | 去模块前缀+去类型名（C层） | `socket.connect_smoke(...)`                      |
| `sio_node_interop_smoke_c`               | extern C/平台                                                                                                                                                                          | `node_interop_smoke`                 | 去模块前缀+去类型名（C层） | `socket.node_interop_smoke(...)`                 |
...
| `sio_server_smoke_c`                     | extern C/平台                                                                                                                                                                          | `server_smoke`                       | 去模块前缀+去类型名（C层） | `socket.server_smoke(...)`                       |
...
| `sio_server_emit_smoke_c`                | extern C/平台                                                                                                                                                                          | `server_emit_smoke`                  | 去模块前缀+去类型名（C层） | `socket.server_emit_smoke(...)`                  |
...
| `polling_smoke`                          | polling 传输层烟测（URL + canned HTTP；不依赖 live server）。                                                                                                                                    | `polling_smoke`                      | 已符合命名          | `socket.polling_smoke(...)`                      |
...
| `connect_smoke`                          | connect v2 烟测（无 live server）。                                                                                                                                                        | `connect_smoke`                      | 已符合命名          | `socket.connect_smoke(...)`                      |
...
| `node_golden_smoke`                      | Node socket.io-client v4 金样烟测（无 live Node）。                                                                                                                                          | `node_golden_smoke`                  | 已符合命名          | `socket.node_golden_smoke(...)`                  |
...
| `server_smoke`                           | server v2 烟测。                                                                                                                                                                        | `server_smoke`                       | 已符合命名          | `socket.server_smoke(...)`                       |
...
| `server_emit_smoke`                      | server emit 烟测。                                                                                                                                                                      | `server_emit_smoke`                  | 已符合命名          | `socket.server_emit_smoke(...)`                  |
...
| `ns_ack_smoke`                           | namespace / ACK / 带 id EVENT 烟测。                                                                                                                                                     | `ns_ack_smoke`                       | 已符合命名          | `socket.ns_ack_smoke(...)`                       |
...
| `ns_router_smoke`                        | 多 namespace 路由烟测。                                                                                                                                                                    | `ns_router_smoke`                    | 已符合命名          | `socket.ns_router_smoke(...)`                    |
...
| `ns_sessions_smoke`                      | 多 namespace 并发会话烟测。                                                                                                                                                                  | `ns_sessions_smoke`                  | 已符合命名          | `socket.ns_sessions_smoke(...)`                  |
...
| `ws_hub_smoke`                           | WS hub + CONNECT ACK 烟测。                                                                                                                                                             | `ws_hub_smoke`                       | 已符合命名          | `socket.ws_hub_smoke(...)`                       |
...
| `room_smoke`                             | room + hub 快照烟测。                                                                                                                                                                     | `room_smoke`                         | 已符合命名          | `socket.room_smoke(...)`                         |
...
| `hub_sync_smoke`                         | hub + room 跨进程同步烟测。                                                                                                                                                                  | `hub_sync_smoke`                     | 已符合命名          | `socket.hub_sync_smoke(...)`                     |
...
| `session_sync_smoke`                     | session 一体快照 + register_or_rebind 烟测。                                                                                                                                                | `session_sync_smoke`                 | 已符合命名          | `socket.session_sync_smoke(...)`                 |
...
| `cluster_sync_smoke`                     | 集群 session 合并烟测。                                                                                                                                                                     | `cluster_sync_smoke`                 | 已符合命名          | `socket.cluster_sync_smoke(...)`                 |
...
| `cluster_adapter_smoke`                  | cluster adapter 烟测。                                                                                                                                                                  | `cluster_adapter_smoke`              | 已符合命名          | `socket.cluster_adapter_smoke(...)`              |
...
| `cluster_ring_sync_smoke`                | cluster ring 烟测（export → import_merge → drain）。                                                                                                                                      | `cluster_ring_sync_smoke`            | 已符合命名          | `socket.cluster_ring_sync_smoke(...)`            |
| `p3_complete_smoke`                      | P3 收口烟测（v9–v15 server/hub/room/cluster 金样串联）。                                                                                                                                        | `p3_complete_smoke`                  | 已符合命名          | `socket.p3_complete_smoke(...)`                  |
...
| `socketio_packet_smoke`                  | C 层编解码烟测；0 通过。                                                                                                                                                                       | `packet_smoke`                       | 去模块前缀+去类型名     | `socket.packet_smoke(...)`                       |
...
| `string_append_slice`       | —                                                              | `extend_from_slice` | 去模块前缀+去类型名     | `str.extend_from_slice(...)` |
...
| 当前名称                              | 功能说明                                                                  | 简化名称                       | 说明             | 绑定调用                                 |
| --------------------------------- | --------------------------------------------------------------------- | -------------------------- | -------------- | ------------------------------------ |
| `sync_mutex_new_c`                | std.sync — 互斥锁与同步原语（对标 Rust std::sync::Mutex、Zig Thread.Mutex） 【文件职责】 | `mutex_new`                | 去模块前缀+去类型名（C层） | `sync.mutex_new(...)`                |
| `sync_mutex_lock_c`               | extern C/平台                                                           | `mutex_lock`               | 去模块前缀+去类型名（C层） | `sync.mutex_lock(...)`               |
| `sync_mutex_try_lock_c`           | extern C/平台                                                           | `mutex_try_lock`           | 去模块前缀+去类型名（C层） | `sync.mutex_try_lock(...)`           |
| `sync_mutex_unlock_c`             | extern C/平台                                                           | `mutex_unlock`             | 去模块前缀+去类型名（C层） | `sync.mutex_unlock(...)`             |
| `sync_mutex_free_c`               | extern C/平台                                                           | `mutex_free`               | 去模块前缀+去类型名（C层） | `sync.mutex_free(...)`               |
| `mutex_new`                       | —                                                                     | `mutex_new`                | 已符合命名          | `sync.mutex_new(...)`                |
| `mutex_lock`                      | —                                                                     | `mutex_lock`               | 已符合命名          | `sync.mutex_lock(...)`               |
| `mutex_try_lock`                  | —                                                                     | `mutex_try_lock`           | 已符合命名          | `sync.mutex_try_lock(...)`           |
| `mutex_unlock`                    | 解锁；必须在已持有锁的线程内调用。返回 0 成功，-1 失败。                                       | `mutex_unlock`             | 已符合命名          | `sync.mutex_unlock(...)`             |
| `mutex_free`                      | —                                                                     | `mutex_free`               | 已符合命名          | `sync.mutex_free(...)`               |
| `sync_rwlock_new_c`               | extern C/平台                                                           | `rwlock_new`               | 去模块前缀+去类型名（C层） | `sync.rwlock_new(...)`               |
| `sync_rwlock_read_lock_c`         | extern C/平台                                                           | `rwlock_read_lock`         | 去模块前缀+去类型名（C层） | `sync.rwlock_read_lock(...)`         |
| `sync_rwlock_write_lock_c`        | extern C/平台                                                           | `rwlock_write_lock`        | 去模块前缀+去类型名（C层） | `sync.rwlock_write_lock(...)`        |
| `sync_rwlock_read_unlock_c`       | extern C/平台                                                           | `rwlock_read_unlock`       | 去模块前缀+去类型名（C层） | `sync.rwlock_read_unlock(...)`       |
| `sync_rwlock_write_unlock_c`      | extern C/平台                                                           | `rwlock_write_unlock`      | 去模块前缀+去类型名（C层） | `sync.rwlock_write_unlock(...)`      |
| `sync_rwlock_free_c`              | extern C/平台                                                           | `rwlock_free`              | 去模块前缀+去类型名（C层） | `sync.rwlock_free(...)`              |
| `sync_condvar_new_c`              | extern C/平台                                                           | `condvar_new`              | 去模块前缀+去类型名（C层） | `sync.condvar_new(...)`              |
| `sync_condvar_wait_c`             | extern C/平台                                                           | `condvar_wait`             | 去模块前缀+去类型名（C层） | `sync.condvar_wait(...)`             |
...
| `sync_condvar_free_c`             | extern C/平台                                                           | `condvar_free`             | 去模块前缀+去类型名（C层） | `sync.condvar_free(...)`             |
| `sync_rwlock_contention_smoke_c`  | extern C/平台                                                           | `rwlock_contention_smoke`  | 去模块前缀+去类型名（C层） | `sync.rwlock_contention_smoke(...)`  |
| `sync_condvar_contention_smoke_c` | extern C/平台                                                           | `condvar_contention_smoke` | 去模块前缀+去类型名（C层） | `sync.condvar_contention_smoke(...)` |
| `rwlock_new`                      | 创建 RwLock；失败 0。                                                       | `rwlock_new`               | 已符合命名          | `sync.rwlock_new(...)`               |
| `rwlock_free`                     | 销毁 RwLock。                                                            | `rwlock_free`              | 已符合命名          | `sync.rwlock_free(...)`              |
| `rwlock_read_lock`                | 获取读锁；成功 0。                                                            | `rwlock_read_lock`         | 已符合命名          | `sync.rwlock_read_lock(...)`         |
| `rwlock_write_lock`               | 获取写锁；成功 0。                                                            | `rwlock_write_lock`        | 已符合命名          | `sync.rwlock_write_lock(...)`        |
| `rwlock_read_unlock`              | 释放读锁；成功 0。                                                            | `rwlock_read_unlock`       | 已符合命名          | `sync.rwlock_read_unlock(...)`       |
| `rwlock_write_unlock`             | 释放写锁；成功 0。                                                            | `rwlock_write_unlock`      | 已符合命名          | `sync.rwlock_write_unlock(...)`      |
| `condvar_new`                     | 创建 Condvar；失败 0。                                                      | `condvar_new`              | 已符合命名          | `sync.condvar_new(...)`              |
| `condvar_wait`                    | 在已持有 mutex 时等待 condvar；唤醒后重新持有 mutex。                                 | `condvar_wait`             | 已符合命名          | `sync.condvar_wait(...)`             |
| `condvar_signal`                  | 唤醒一个等待线程。                                                             | `notify_one`               | 语义重命名          | `sync.notify_one(...)`               |
| `condvar_broadcast`               | 唤醒全部等待线程。                                                             | `notify_all`               | 语义重命名          | `sync.notify_all(...)`               |
| `condvar_free`                    | 销毁 Condvar。                                                           | `condvar_free`             | 已符合命名          | `sync.condvar_free(...)`             |
| `rwlock_contention_smoke`         | RwLock 竞争烟测；成功 0。                                                     | `rwlock_contention_smoke`  | 已符合命名          | `sync.rwlock_contention_smoke(...)`  |
| `condvar_contention_smoke`        | Condvar 竞争烟测；成功 0。                                                    | `condvar_contention_smoke` | 已符合命名          | `sync.condvar_contention_smoke(...)` |
| `sync_lock_diag_set_enabled_c`    | extern C/平台                                                           | `lock_diag_set_enabled`    | 去模块前缀+去类型名（C层） | `sync.lock_diag_set_enabled(...)`    |
| `sync_lock_diag_is_enabled_c`     | extern C/平台                                                           | `lock_diag_is_enabled`     | 去模块前缀+去类型名（C层） | `sync.lock_diag_is_enabled(...)`     |
| `sync_lock_diag_mutex_set_id_c`   | extern C/平台                                                           | `lock_diag_mutex_set_id`   | 去模块前缀+去类型名（C层） | `sync.lock_diag_mutex_set_id(...)`   |
| `sync_lock_diag_last_err_c`       | extern C/平台                                                           | `lock_diag_last_err`       | 去模块前缀+去类型名（C层） | `sync.lock_diag_last_err(...)`       |
| `sync_lock_diag_clear_c`          | extern C/平台                                                           | `lock_diag_clear`          | 去模块前缀+去类型名（C层） | `sync.lock_diag_clear(...)`          |
| `sync_lock_diag_snapshot_c`       | extern C/平台                                                           | `lock_diag_snapshot`       | 去模块前缀+去类型名（C层） | `sync.lock_diag_snapshot(...)`       |
| `sync_lock_diag_smoke_c`          | extern C/平台                                                           | `lock_diag_smoke`          | 去模块前缀+去类型名（C层） | `sync.lock_diag_smoke(...)`          |
| `lock_diag_err_recursive`         | 递归加锁错误码 -1。                                                           | `lock_diag_err_recursive`  | 已符合命名          | `sync.lock_diag_err_recursive(...)`  |
| `lock_diag_err_order`             | 锁序违规错误码 -2。                                                           | `lock_diag_err_order`      | 已符合命名          | `sync.lock_diag_err_order(...)`      |
| `lock_diag_err_unlock`            | 解锁顺序错误码 -3。                                                           | `lock_diag_err_unlock`     | 已符合命名          | `sync.lock_diag_err_unlock(...)`     |
| `lock_diag_set_enabled`           | 开启/关闭锁诊断。                                                             | `lock_diag_set_enabled`    | 已符合命名          | `sync.lock_diag_set_enabled(...)`    |
| `lock_diag_is_enabled`            | 查询锁诊断是否开启。                                                            | `lock_diag_is_enabled`     | 已符合命名          | `sync.lock_diag_is_enabled(...)`     |
| `lock_diag_mutex_set_id`          | 为 mutex 绑定锁序 id。                                                      | `lock_diag_mutex_set_id`   | 已符合命名          | `sync.lock_diag_mutex_set_id(...)`   |
| `lock_diag_last_err`              | 最近诊断错误码。                                                              | `lock_diag_last_err`       | 已符合命名          | `sync.lock_diag_last_err(...)`       |
| `lock_diag_clear`                 | 清空诊断状态。                                                               | `lock_diag_clear`          | 已符合命名          | `sync.lock_diag_clear(...)`          |
| `lock_diag_snapshot`              | 写入文本快照；返回写入字节数。                                                       | `lock_diag_snapshot`       | 已符合命名          | `sync.lock_diag_snapshot(...)`       |
| `lock_diag_smoke`                 | C 烟测门面。                                                               | `lock_diag_smoke`          | 已符合命名          | `sync.lock_diag_smoke(...)`          |
...
| `time_format_timezone_smoke_c` | extern C/平台                              | `format_timezone_smoke` | 去模块前缀+去类型名（C层） | `time.format_timezone_smoke(...)` |
...
| `format_timezone_smoke`        | 时区/格式化烟测；0 成功。                           | `format_timezone_smoke` | 已符合命名          | `time.format_timezone_smoke(...)` |
...
| `trace_hooks_smoke_c`      | extern C/平台                                     | `hooks_smoke`         | 去模块前缀+去类型名（C层） | `trace.hooks_smoke(...)`         |
| `hooks_smoke`              | STD-118：挂钩路径 C 烟测；0 通过。                         | `hooks_smoke`         | 已符合命名          | `trace.hooks_smoke(...)`         |
...
| `unicode_grapheme_case_smoke_c` | extern C/平台                                                                                                                 | `grapheme_case_smoke` | 去模块前缀+去类型名（C层） | `unicode.grapheme_case_smoke(...)` |
...
| `grapheme_case_smoke`           | C 烟测入口（STD-114）；0 表示通过。                                                                                                     | `grapheme_case_smoke` | 已符合命名          | `unicode.grapheme_case_smoke(...)` |
...
| `url_ipv6_host_smoke_c`  | extern C/平台                               | `ipv6_host_smoke`  | 去模块前缀+去类型名（C层） | `url.ipv6_host_smoke(...)`  |
...
| `ipv6_host_smoke`        | STD-134 C 金样：IPv6 bracket 与 host 字节互转。    | `ipv6_host_smoke`  | 已符合命名          | `url.ipv6_host_smoke(...)`  |
...
| 当前名称                                | 功能说明                                                                 | 简化名称                         | 说明         | 绑定调用                                  |
| ----------------------------------- | -------------------------------------------------------------------- | ---------------------------- | ---------- | ------------------------------------- |
| `vec_default_capacity`              | —                                                                    | `default_capacity`           | 去模块前缀+去类型名 | `vec.default_capacity(...)`           |
...
| `vec_i32_reserve_one`               | —                                                                    | `reserve_one`                | 去模块前缀+去类型名 | `vec.reserve_one(...)`                |
| `vec_i32_push`                      | 追加元素 x；成功返回 0，分配失败返回 -1。                                             | `push`                       | 去模块前缀+去类型名 | `vec.push(...)`                       |
| `vec_i32_push_arena`                | MEM-C1 单态化：with_arena 热路径直接 arena64_alloc，无 allocator_alloc kind 分支。 | `push_arena`                 | 去模块前缀+去类型名 | `vec.push_arena(...)`                 |
...
| `vec_i32_append_slice`              | —                                                                    | `extend_from_slice`          | 去模块前缀+去类型名 | `vec.extend_from_slice(...)`          |
...
| `vec_u8_reserve_one`                | 按 v.al 扩容预留 1 个元素；arena 已有 ptr 时不可 realloc（AL-05）。                   | `reserve_one`                | 去模块前缀+去类型名 | `vec.reserve_one(...)`                |
| `vec_u8_push`                       | —                                                                    | `push`                       | 去模块前缀+去类型名 | `vec.push(...)`                       |
| `vec_u8_push_arena`                 | MEM-C1 单态化：with_arena 热路径直接 arena64_alloc，无 allocator_alloc kind 分支。 | `push_arena`                 | 去模块前缀+去类型名 | `vec.push_arena(...)`                 |
...
| `vec_u8_append_slice`               | —                                                                    | `extend_from_slice`          | 去模块前缀+去类型名 | `vec.extend_from_slice(...)`          |
...
| `vec_u8_with_allocator`             | STD-112：按 Allocator 预分配 capacity；写入 v.al 并分配；失败 -1。                  | `with_allocator`             | 去模块前缀+去类型名 | `vec.with_allocator(...)`             |
| `vec_u8_reserve_one_with_allocator` | STD-112：按 Allocator 扩容预留 1 个元素；临时覆盖 v.al 后委托 reserve_one。            | `reserve_one_with_allocator` | 去模块前缀+去类型名 | `vec.reserve_one_with_allocator(...)` |
| `vec_u8_push_with_allocator`        | STD-112：按 Allocator 追加元素；设置 v.al 后委托 push。                           | `push_with_allocator`        | 去模块前缀+去类型名 | `vec.push_with_allocator(...)`        |
| `vec_u8_deinit_with_allocator`      | STD-112：按 Allocator 释放；arena 路径为 no-op。                              | `deinit_with_allocator`      | 去模块前缀+去类型名 | `vec.deinit_with_allocator(...)`      |
| `vec_u16_new`                       | 新建空 Vec_u16。                                                         | `new`                        | 去模块前缀+去类型名 | `vec.new(...)`                        |
| `vec_u16_reserve_one`               | Vec_u16 扩容预留 1 个元素；失败 -1。                                            | `reserve_one`                | 去模块前缀+去类型名 | `vec.reserve_one(...)`                |
...
| `vec_u64_append_slice`              | 追加 u64 切片；失败 -1。                                                     | `extend_from_slice`          | 去模块前缀+去类型名 | `vec.extend_from_slice(...)`          |
...
| `vec_f64_append_slice`              | 追加 f64 切片；失败 -1。                                                     | `extend_from_slice`          | 去模块前缀+去类型名 | `vec.extend_from_slice(...)`          |
...
| `ws_client_handshake_smoke`      | C 层握手 socketpair 烟测；0 通过。                                                    | `handshake_smoke`             | 去模块前缀+去类型名     | `ws.handshake_smoke(...)`             |
| `ws_parse_url_smoke`             | ws(s):// URL 解析烟测；0 通过。                                                      | `parse_url_smoke`             | 去模块前缀+去类型名     | `ws.parse_url_smoke(...)`             |
| `ws_server_accept_smoke`         | 服务端 accept 烟测（请求解析 + socketpair 101）；0 通过。                                   | `accept_smoke`                | 去模块前缀+去类型名     | `ws.accept_smoke(...)`                |
...
> 简化名审计：**已全部符合**「函数名不含类型 token」规则（含 `bool`）。

> 二次精简已执行：批量去除 `view`_ 等对象前缀，目标是方法名最短可读。
> 深化精简已执行：`_2/_3`、`_v4/_v7`、`_512`、`readv2/readv4` 等后缀统一并入语义参数。

## 五、后续实施

1. 编译器：联合类型 + 函数重载。
2. `mod.sx` 导出简化名；**禁止新增**含类型 token 的公开 API。
3. 旧 `_*_i32` 等名保留一版兼容。
