...
| `xlang_async_bind_context_c` | 绑定 Context（C 别名）。 | `bind_ctx` | 去模块前缀+去类型名（C层） | `async.bind_ctx(...)` |
| `xlang_async_current_context_c` | 当前 Context 句柄（C 别名）。 | `current_context` | 去模块前缀+去类型名（C层） | `async.current_context(...)` |
| `xlang_async_unbind_context_c` | 解绑 Context（C 别名）。 | `unbind_context` | 去模块前缀+去类型名（C层） | `async.unbind_context(...)` |
| `xlang_async_coop_pingpong`         | A1 ping-pong（C 别名）。                                                                                                                          | `coop_pingpong`             | 去模块前缀+去类型名     | `async.coop_pingpong(...)`             |
| `xlang_async_coop_pingpong_jmp`     | A2 jmp ping-pong（C 别名）。                                                                                                                      | `coop_pingpong_jmp`         | 去模块前缀+去类型名     | `async.coop_pingpong_jmp(...)`         |
| `xlang_async_cps_suspend`           | CPS suspend（C 别名）。                                                                                                                           | `cps_suspend`               | 去模块前缀+去类型名     | `async.cps_suspend(...)`               |
| `xlang_async_cps_suspend_io`        | CPS suspend IO（C 别名）。                                                                                                                        | `cps_suspend_io`            | 去模块前缀+去类型名     | `async.cps_suspend_io(...)`            |
| `xlang_async_run_i32`               | run i32 协程（C 别名）。                                                                                                                            | `run`                       | 去模块前缀+去类型名     | `async.run(...)`                       |
| `xlang_async_spawn_i32`             | spawn seed 并 submit（C 别名）。                                                                                                                        | `submit`                    | 与 task_submit 合并为 submit 重载（fn, seed）     | `async.submit(fn, seed)`                     |
| `xlang_async_run_drain_until_idle`  | drain 直至空闲（C 别名）。                                                                                                                            | `run_drain_until_idle`      | 去模块前缀+去类型名     | `async.run_drain_until_idle(...)`      |
| `xlang_async_run_seed_reset`        | 重置 run seed 队列（C 别名）。                                                                                                                        | `run_seed_reset`            | 去模块前缀+去类型名     | `async.run_seed_reset(...)`            |
| `xlang_async_run_seed_push_i32`     | —                                                                                                                                            | `run_seed_push`             | 去模块前缀+去类型名     | `async.run_seed_push(...)`             |
| `xlang_async_run_seed_push_u32`     | —                                                                                                                                            | `run_seed_push`             | 去模块前缀+去类型名     | `async.run_seed_push(...)`             |
| `xlang_async_run_seed_push_i64`     | —                                                                                                                                            | `run_seed_push`             | 去模块前缀+去类型名     | `async.run_seed_push(...)`             |
| `xlang_async_run_seed_push_usize`   | —                                                                                                                                            | `run_seed_push`             | 去模块前缀+去类型名     | `async.run_seed_push(...)`             |
| `xlang_async_run_seed_set_i32`      | —                                                                                                                                            | `run_seed_set`              | 去模块前缀+去类型名     | `async.run_seed_set(...)`              |
| `xlang_async_run_seed_valid`        | —                                                                                                                                            | `run_seed_valid`            | 去模块前缀+去类型名     | `async.run_seed_valid(...)`            |
| `xlang_async_run_seed_take_i32`     | —                                                                                                                                            | `run_seed_take`             | 去模块前缀+去类型名     | `async.run_seed_take(...)`             |
| `xlang_async_run_seed_take_u32`     | —                                                                                                                                            | `run_seed_take`             | 去模块前缀+去类型名     | `async.run_seed_take(...)`             |
| `xlang_async_run_seed_take_i64`     | —                                                                                                                                            | `run_seed_take`             | 去模块前缀+去类型名     | `async.run_seed_take(...)`             |
| `xlang_async_run_seed_take_usize`   | —                                                                                                                                            | `run_seed_take`             | 去模块前缀+去类型名     | `async.run_seed_take(...)`             |
| `xlang_async_task_submit` | 提交任务（C 别名）。 | `submit` | 去模块前缀+去类型名；三轮精简 | `async.submit(...)` |
| `xlang_async_task_submit_with_ctx`  | 带 Context 提交（C 别名）。                                                                                                                          | `task_submit_with_ctx`      | 去模块前缀+去类型名     | `async.task_submit_with_ctx(...)`      |
| `xlang_async_spawn_ctx_smoke_c` | spawn context 烟测（C 别名）。 | `spawn_ctx_smoke` | 去模块前缀+去类型名（C层）；Tier-X 不 export | `async.spawn_ctx_smoke(...)` |
| `xlang_async_task_submit_to`        | 提交到指定 worker（C 别名）。                                                                                                                          | `task_submit_to`            | 去模块前缀+去类型名     | `async.task_submit_to(...)`            |
| `xlang_async_scheduler_drain`       | —                                                                                                                                            | `scheduler_drain`           | 去模块前缀+去类型名     | `async.scheduler_drain(...)`           |
| `xlang_async_worker_drain`          | —                                                                                                                                            | `worker_drain`              | 去模块前缀+去类型名     | `async.worker_drain(...)`              |
| `xlang_async_worker_count`          | —                                                                                                                                            | `worker_count`              | 去模块前缀+去类型名     | `async.worker_count(...)`              |
| `xlang_async_worker_pending`        | —                                                                                                                                            | `worker_pending`            | 去模块前缀+去类型名     | `async.worker_pending(...)`            |
| `xlang_async_queue_reset`           | —                                                                                                                                            | `queue_reset`               | 去模块前缀+去类型名     | `async.queue_reset(...)`               |
| `xlang_async_scheduler_pending`     | —                                                                                                                                            | `scheduler_pending`         | 去模块前缀+去类型名     | `async.scheduler_pending(...)`         |
| `xlang_async_io_wake_all`           | —                                                                                                                                            | `io_wake_all`               | 去模块前缀+去类型名     | `async.io_wake_all(...)`               |
| `xlang_async_io_waiters_pending`    | —                                                                                                                                            | `waiters_pending`           | 去模块前缀+去类型名     | `async.waiters_pending(...)`           |
...
| 当前名称                          | 功能说明                                                                                                                              | 简化名称                          | 说明         | 绑定调用                                  |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ---------- | ------------------------------------- |
| `handle_stdin`                | X 流水线 return 字面量可由 typeck 隐式升格为 usize（与 std 内 read(数字,…) 的常见写法等价）。                                                               | `stdin`                       | 语义重命名      | `io.stdin(...)`                       |
...
| `recommend_path`        | STD-153：推荐向量化路径（0=scalar，1=hw）。产品 export；**无** `recommend_simd_path` 第二套。                                                                                | `recommend_path`      | 已符合命名          | `simd.recommend_path(...)`      |
| `add`                   | Vec4f／Vec8i 逐 lane 加法（overload）。**无** `vec4f_add`／`vec8i_add` 第二套。                                                                                         | `add`                 | 去类型名前缀         | `simd.add(...)`                 |
| `sub`                   | Vec4f／Vec8i 逐 lane 减法（SIMD-S3：subps／psubd）。**无** `vec4f_sub` 第二套。                                                                                          | `sub`                 | 去类型名前缀         | `simd.sub(...)`                 |
| `mul`                   | Vec4f／Vec8i 逐 lane 乘法（SIMD-S3：mulps／pmulld）。**无** `vec4f_mul`／`simd_mul` 第二套。                                                                               | `mul`                 | 去类型名前缀         | `simd.mul(...)`                 |
| `splat`                 | 广播单值到全部 lane（Vec4f／Vec8i overload）。**无** `vec4f_splat` 第二套。                                                                                               | `splat`               | 去类型名前缀         | `simd.splat(...)`               |
| `hsum`                  | Vec4f 四 lane 水平求和。**无** `vec4f_hsum` 第二套。                                                                                                                   | `hsum`                | 去类型名前缀         | `simd.hsum(...)`                |
| `dot`                   | Vec4f 点积：sum(a[i]*b[i])。**无** `vec4f_dot` 第二套。                                                                                                                | `dot`                 | 去类型名前缀         | `simd.dot(...)`                 |
| `fma`                   | Vec4f 融合乘加 a+b*c（vfmadd231ps）。**无** `vec4f_fma` 第二套。                                                                                                       | `fma`                 | 去类型名前缀         | `simd.fma(...)`                 |
| `madd`                  | `fma` 同语义别名。**无** `vec4f_madd` 第二套。                                                                                                                        | `madd`                | 去类型名前缀         | `simd.madd(...)`                |
| `shuffle`               | Vec4f／Vec8i permute（STD-047）。**无** `vec4f_shuffle` 第二套。                                                                                                      | `shuffle`             | 去类型名前缀         | `simd.shuffle(...)`             |
| `select`                | Vec4f／Vec8i blend（STD-047）。**无** `vec8i_select` 第二套。                                                                                                         | `select`              | 去类型名前缀         | `simd.select(...)`              |
| `select_lane`           | 单 lane select：mask 非 0 取 a。**无** `vec8i_select_lane` 第二套。                                                                                                    | `select_lane`         | 去类型名前缀         | `simd.select_lane(...)`         |
