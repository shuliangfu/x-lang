...
// 约定：Socket 默认非阻塞；net_* .x + 最小胶层合并 net.o（F-04 v14 无 net.c）。
// 阶段 3 性能压榨：read_batch/write_batch 走 driver 的 submit_read_batch/submit_write_batch（io_uring/readv）。
...
/** F-04 v1：TLS 桩后端；F-04 v8：OpenSSL 见 std/net/tls_openssl.x（按需链 -lssl）。 */
