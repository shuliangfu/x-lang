...
| ----- | --------------------------------------------- | ------------ | ------------ | ------- |
| **一** | 编译器前端全 `.sx`（lexer/parser/ast/typeck/codegen） | ✅ 默认 `*_sx.o` | ✅ B-strict | 🟡 B-hybrid |
...
| **五** | **全仓库 std 无 C**（`.sx` + FFI/asm，按需链入）    | ✅ F-ZC | ✅ F-ZC | ✅ F-ZC |
...
| B-06 | **显式 AST Arena / 对象池**（.sx 写编译器时用）           | ✅  | `run-b06-ast-pool-gate.sh`；parser.sx 依赖 ast_pool glue |
...
| B-13 | **std.sys v3** — mmap（kv 等用）                                        | ✅   | `std/sys/mmap.sx` 纯 `.sx`（F-02 v1 已删 `mmap.inc.c`） |
| B-14 | **Linux .sx 真实 syscall**：`read/write/openat/close/mmap` 内联 asm      | ✅  | `run-linux-*-gate.sh`（x86_64 freestanding）；aarch64 桩表在 linux.sx |
| B-15 | **Linux io_uring** 最小子集（setup/enter/register）                       | ✅  | **v1 门面**：`std/sys/linux_io_uring.sx` + `run-b15-io-uring-sys-gate.sh`；完整 ring 在 std/io |
| B-16 | **macOS** 扩展：`open/read/mmap` 经 libSystem FFI                       | ✅  | `run-macos-mmap-*` + `run-macos-read-file-gate.sh` |
| B-17 | **std/sys/win32.sx** — `ReadFile/WriteFile/CreateFileW/ExitProcess` | ✅  | v1～v3 Write/Read/ExitProcess + `run-b17-exit-process-gate.sh` |
| B-18 | **std/sys/win32 网络** — `WSA*` + **IOCP** 最小 async                   | ✅  | **v1 门面**：`std/sys/win32_net.sx` + `run-b18-win32-net-gate.sh`；IOCP 在 std.io |
| B-19 | **std/sys/mod.sx** 统一门面 + `#[cfg]` 分流                               | ✅  | os_write/read/close/exit/mmap + `run-b19-sys-mod-facade-gate.sh` |
...
| B-31 | freestanding_io 保留为 **极薄 .s** 或迁入 linux.sx asm | ✅  | `run-b31-freestanding-io-gate.sh` |
...
| C-05 | **LSP**：`lsp_diag.sx` 或 C 兼容 parse API                                   | ✅  | **v1 方案 B 子集**：`lsp_diag.sx` + `run-c05-lsp-sx-gate.sh`；`lsp_diag.c` glue ⏭ E-02 |
| C-06 | Makefile **默认** `parser_sx.o` / `typeck_sx.o` / `ast_sx.o`，不链 `parser.o` | ✅  | `DRIVER_SEED_SX_FRONTEND_OBJS` + `run-c06-sx-frontend-default-gate.sh`；`SHUX_LEGACY_C_FRONTEND=1` 回退 |
| C-07 | **像素级**验证：`.sx` 编译器 ≡ C 编译器（同一输入 AST/产物）                                 | ✅  | **v1 行为 parity**：`run-c07-frontend-parity-gate.sh` + matrix；AST/-E 字节 diff ⏭ v2 |
| C-08 | `main.c` 收成一行入口；`runtime.c` 业务迁 `driver.sx` / `build.sx`                 | ✅  | **v1**：薄 main.c + driver 七模块 + build.sx；`run-c08-runtime-driver-gate.sh`；runtime 大块仍 C ⏭ E |
| C-09 | mega7 / force_stub 清理（parser 无 C 回退）                                     | ✅  | **v1**：默认 `parser_sx.o` + strict 覆盖 + force_stub/mega7 manifest；`run-c09-parser-no-c-fallback-gate.sh`；mega7 真 emit ⏭ v2 |
...
| D-04 | Stage2 扩展：`make test_sx` / `run-portable-suite` 两代 diff | ✅  | **v1 子集**：10 用例 stage1/stage2 outcome 一致；`run-d04-stage2-portable-diff-gate.sh`；全量 portable ⏭ v2 |
...
| F-02 | **std.sys** 去 C：`mmap.inc.c` 等 → `.sx` asm/FFI                           | ✅ | std/sys 零 `.c` |
| F-03 | **std.io / std.fs / std.mem / std.heap** 核心路径 .sx 化                      | ✅ | **v1+v2+v3 ✅** 删 `heap.c`/`fs.c`/`io.c`；见 `phase-f-f03-closure.md` |
| F-04 | **std.net / std.compress / std.crypto** 等 `.inc.c` → `.sx` 或分平台 FFI      | ✅   | **std.net v15 ✅**；**std.crypto v21 ✅** 闭合；**F-05 v4 ✅**；见 `phase-f-f04-v21-closure.md` |
| F-05 | **std.db**（sqlite/kv/arrow）`.c` → `.sx` 或 OS 纯 FFI                       | ✅ v4 | **v1～v3 ✅** + **v4 ✅ 闭合**（3 胶层 C）；见 `phase-f-f05-v4-closure.md` |
| F-06 | **runtime.c** 中 std 按需链路径改为链 **`.sx` 产物 .o**                             | ✅ v2 | v1+v2：`run-f06-runtime-std-o-cleanup-gate.sh`；stage2 无 legacy fs/io/heap `.o` |
| F-07 | **Makefile / build.sx**：构建 std 仅 `-backend asm` 或自举 shux，**禁止 cc 编 std** | ✅ v2 | v1+v2：`run-f07-no-cc-std-migrated-gate.sh`；migrated 硬禁 |
...
| F-10 | `**make test_sx` + portable suite** 在「无 std C」构建下全绿                      | ✅ v1 | v1：`run-f10-test-sx-portable-gate.sh`（test_sx + D-04 子集；无 shux SKIP） |
| F-11 | Tag `**vX.Y.Z-selfhost`**：发布物 = 单二进制 + 全 `.sx` 源码树                       | ✅ v1 | v1 checklist：`run-f11-selfhost-release-prep-gate.sh`（tag 发布时人工打） |
...
| NL-03 | **堆**：mmap page 分配器（替代 `heap.c`/malloc） | ✅  | `std/heap/page_mmap.sx` + `run-no-libc-heap-gate.sh` |
| NL-04 | **fs/io** 改调 `std.sys` only | ✅  | `std/fs/freestanding_linux.sx` + `run-no-libc-fs-gate.sh` |
...
> **前提**：阶段 **D + E + F v1 已闭合**（§0.1）。G 不是重复 D/E/F，而是把「软退役 + std 零 C」推进到**仓库内零手写 C/H、Linux bootstrap 零 libc、构建仅 build.sx**。  
...
| # | 任务 | 状态 | 说明 / 验收 |
|---|------|------|-------------|
| G-01 | **core/** 5→0 `.c` | ⬜ | builtin/mem/slice/debug/assert → 编译器内建或 `.sx`；`run-std-c-inventory-gate.sh` total→0 |
...
| G-05 | **Makefile 退役** | ⬜ | 根目录 + `compiler/Makefile` → 仅 `build.sx` / `build_tool`；见完全自举-无C无Makefile |
