...
| **compiler 无 C（运行时）**     | ✅ E-soft v2           | 默认 bootstrap 不链 C 前端；**~208 `.c` 文件仍保留**（考古用 `XLANG_LEGACY_C_FRONTEND=1`）                  |
| **全仓库零 C（F-09 STRICT）**  | ⬜                     | 跟踪模式：**std 0 + core 5 + compiler ~208**；`XLANG_NO_HANDWRITTEN_C_STRICT=1` 当前必红 → **阶段 G**   |
| **语义自举烟测**                | ✅                     | `make -C compiler bootstrap-verify`                                                           |
| **B-strict 构建**           | ✅ macOS arm64 / Linux | `full_asm` + `XLANG_ASM_EXPERIMENTAL_SKIP_GEN=1`；24/24 `__text` 非空                              |
...
| 2028-Q3 | xlang_asm2 跨平台发布深化 |
| 2028-Q4 | xlang_asm2 CI 矩阵与 bench tier 收官 |
...
| A-08 | **Windows** B-hybrid 可构建 `xlang_asm`                   | ⏭  | `run-bootstrap-bstrict-windows-gate.sh`；MSYS experimental 链                                          | **自举闭环后**再补；本地 macOS+Docker 不阻塞 |
| A-09 | **Stage2 SHA256 金标准**门禁                               | ✅  | `run-stage2-hash-gate.sh` + `XLANG_STAGE2_HASH_STRICT=1` 硬失败通过                                  | stage1/stage2 **SHA256 一致**（Docker Linux 2026-06-20） |
| A-10 | **L5 run-all**：bootstrap `xlang` 真 parity（缩小 seed 白名单） | ✅  | `run-l5-run-all-parity-gate.sh`；bstrict **123** 项 ⊆ whitelist                                      | **Docker Linux x86_64**：123 项全绿 |
...
| A-12 | 跨模块符号统一（`arch_arm64_*` 等）                             | ✅  | `run-a12-cross-module-symbols-gate.sh` + baseline TSV                                                | **Docker Linux x86_64**：`XLANG_A12_CROSS_MODULE_FAIL=1` 无新 U |
...
| C-06 | Makefile **默认** `parser_x.o` / `typeck_x.o` / `ast_x.o`，不链 `parser.o` | ✅  | `DRIVER_SEED_X_FRONTEND_OBJS` + `run-c06-x-frontend-default-gate.sh`；`XLANG_LEGACY_C_FRONTEND=1` 回退 |
...
| #    | 任务                                                      | 状态  | 说明 / 验收                                      |
| ---- | ------------------------------------------------------- | --- | -------------------------------------------- |
| D-01 | **Stage 0**：C/seed 编译出 **Stage 1** 纯 xlang 编译器           | ✅  | **v1**：`bootstrap-driver-bstrict` → `xlang_asm`；`run-d01-stage0-to-stage1-gate.sh`；仍含少量 C seed ⏭ v2 |
| D-02 | **Stage 1** 编译同一 tree → **Stage 2**                     | ✅  | **v1**：`verify-selfhost-stage2-bstrict.sh`；`run-d02-stage1-to-stage2-gate.sh`（委托 stage2-bstrict） |
| D-03 | **Stage 1 与 Stage 2 二进制哈希一致**                           | ✅  | **v1 金标准**：`run-d03-stage2-hash-gate.sh` 委托 A-09；verify Step 4c 默认 `XLANG_STAGE2_HASH_STRICT=1` |
...
| D-05 | 发布 `**xlang` 单二进制** 不依赖 `xlang-c` 冷启动                     | ✅  | **v1**：`bootstrap-driver-bstrict` → `xlang=xlang_asm`；`run-d05-single-xlang-release-gate.sh`；冷启动仍须 seed |
...
> **策略（E-soft）**：C/H **文件保留**；默认 B-strict / bootstrap **不链、不调用**已登记模块；考古用 `XLANG_LEGACY_C_FRONTEND=1` / `xlang-c`。  
...
| F-07 | **Makefile / build.x**：构建 std 仅 `-backend asm` 或自举 xlang，**禁止 cc 编 std** | ✅ v2 | v1+v2：`run-f07-no-cc-std-migrated-gate.sh`；migrated 硬禁 |
...
| F-10 | `**make test_x` + portable suite** 在「无 std C」构建下全绿                      | ✅ v1 | v1：`run-f10-test-x-portable-gate.sh`（test_x + D-04 子集；无 xlang SKIP） |
...
| **F-ZC** | **std 零 C/H 终局**（`std/` 下 0 个 `.c`/`.h`）                              | ✅ v1 | **`.c` 0 + `.h` 0**；ABI 头 → `compiler/include/xlang_std_abi/` + runtime 内联；[`phase-f-std-zero-c-v1.md`](analysis/phase-f-std-zero-c-v1.md) |
...
XLANG_F_STD_ZERO_C_STRICT=1 ./tests/run-f-std-zero-c-track-gate.sh   # 终局硬绿（F-ZC ✅ 已 0/0）
XLANG_F_STD_ZERO_C_UPDATE=1 ./tests/run-f-std-zero-c-track-gate.sh   # 迁移后刷新 manifest
```
...
```bash
XLANG_NOLIBC_N01_FAIL=1 ./tests/run-nolibc-n01-preparation-gate.sh   # NL-01 专 gate
XLANG_NOLIBC_N06_FAIL=1 ./tests/run-nolibc-n06-std-track-gate.sh     # NL-06 freestanding std track
XLANG_NOLIBC_N07_FAIL=1 ./tests/run-nolibc-n07-bootstrap-prep-gate.sh # NL-07 bootstrap prep
XLANG_NOLIBC_FAIL=1 ./tests/run-no-libc-gate.sh                      # 全链聚合
```
...
| **E-soft → E-hard** | 默认路径已 no_c；删文件后移除 `XLANG_LEGACY_C_FRONTEND` 考古链 |
| **F-09 STRICT 为终局哨兵** | `XLANG_NO_HANDWRITTEN_C_STRICT=1` 硬绿 = G 完成 |
...
| G-03 | **F-no-libc bootstrap 硬绿** | 🟡 | NL-07 v4 track → v5：`XLANG_BOOTSTRAP_NOSTDLIB=1` 全链 + `ldd` 无 libc |
| G-04 | **F-09 STRICT 白名单为空** | ⬜ | `XLANG_NO_HANDWRITTEN_C_STRICT=1 ./tests/run-no-handwritten-c-gate.sh` |
...
XLANG_NO_HANDWRITTEN_C_STRICT=1 ./tests/run-no-handwritten-c-gate.sh   # 终局（当前必红）

# F-no-libc bootstrap（G-03）
XLANG_NOLIBC_N07_V4_FAIL=1 ./tests/run-nolibc-n07-v4-build-gate.sh
...
| 序 | 条 | 动作 | 变绿命令 / 备注 |
| --- | --- | --- | --- |
| **0** | **G-03** | NL-07 bootstrap `-nostdlib` 全链试跑 | `XLANG_NOLIBC_N07_V4_TRY_BUILD=1 XLANG_NOLIBC_N07_V4_FAIL=1 ./tests/run-nolibc-n07-v4-build-gate.sh` |
...
| **3** | **G-04** | F-09 STRICT 硬绿 | `XLANG_NO_HANDWRITTEN_C_STRICT=1 ./tests/run-no-handwritten-c-gate.sh` |
...
| E-soft v2 | ✅ | `XLANG_E_SOFT_FAIL=1 ./tests/run-e-soft-retire-gate.sh` |
| F-01～F-12 + F-ZC | ✅ | `XLANG_F_PHASE_F_92_FAIL=1 ./tests/run-f-phase-f-92-batch-gate.sh` |
...
- **W3 A-09 hash**：✅ 同次 bootstrap gen1→gen2 后 `sha256=7547d08232200bbb6412a00771f4fabd85f0c9ad095942ae8db5c007bf1237e8`（2816304 B）；`XLANG_W3_RESUME_FROM=ensure` 仅当 stage1/2 与当前 xlang 同次构建时可用。
...
# C-03 Windows track-only（B-hybrid 日志 cc -c pipeline_gen.c 计数；严格：XLANG_WIN_C03_PIPELINE_GEN_FAIL=1）
...
XLANG_TYPECK_PARSE_COUNT_FAIL=1 ./tests/run-typeck-parse-count-gate.sh
...
# NL-07 v2 bootstrap nostdlib 首试（桩编译；Linux 全链：XLANG_BOOTSTRAP_NOSTDLIB=1 build_xlang_asm）
XLANG_NOLIBC_N07_V2_FAIL=1 ./tests/run-nolibc-n07-v2-prep-gate.sh
# NL-07 v3 bootstrap nostdlib 链入烟测（Linux x86_64 硬绿）
XLANG_NOLIBC_N07_V3_FAIL=1 ./tests/run-nolibc-n07-v3-link-gate.sh
# NL-07 v4 bootstrap nostdlib 全链试跑（track；Linux+xlang_asm：XLANG_NOLIBC_N07_V4_TRY_BUILD=1）
XLANG_NOLIBC_N07_V4_FAIL=1 ./tests/run-nolibc-n07-v4-build-gate.sh
XLANG_NOLIBC_N07_V2_FAIL=1 ./tests/run-nolibc-n07-v2-prep-gate.sh
...
XLANG_NOLIBC_HEAP_FAIL=1 ./tests/run-no-libc-heap-gate.sh

# NL-04 freestanding 读文件（Linux x86_64）
XLANG_NOLIBC_FS_FAIL=1 ./tests/run-no-libc-fs-gate.sh

# NL-05 freestanding 链接策略（runtime 无 -lc + smoke）
XLANG_NOLIBC_LINK_FAIL=1 ./tests/run-no-libc-link-gate.sh

# NL-02 freestanding socket syscall（Linux x86_64）
XLANG_NOLIBC_SOCKET_FAIL=1 ./tests/run-no-libc-socket-gate.sh

# B-01：#[cfg(...)] 词法跳过（xlang-c / 任意平台）
...
# B-20：std.sys os_read_file_into（任意平台 xlang-c）
...
# C-04：-E-extern 自动生成 import extern（xlang-c）
XLANG_E_EXTERN_IMPORT_FAIL=1 ./tests/run-e-extern-import-gate.sh
XLANG_C04_NO_PERL_FAIL=1 ./tests/run-c04-no-perl-fallback-gate.sh
XLANG_LEXER_E_EXTERN_FAIL=1 ./tests/run-lexer-e-extern-gate.sh
XLANG_PIPELINE_E_EXTERN_FAIL=1 ./tests/run-pipeline-e-extern-gate.sh
XLANG_PARSER_E_EXTERN_FAIL=1 ./tests/run-parser-e-extern-gate.sh
...
# B-03：#[repr(C)] 词法跳过 + C ABI layout 烟测（xlang-c / 任意平台）
...
# B-19：std.sys 统一 os_write（任意平台 xlang-c）
...
> F-09 门禁：`tests/run-no-handwritten-c-gate.sh`（手写 C 清零审计；**G-04 终局：`XLANG_NO_HANDWRITTEN_C_STRICT=1`**）  
> **F §9.2 聚合**：`XLANG_F_PHASE_F_92_FAIL=1 ./tests/run-f-phase-f-92-batch-gate.sh`（F-06～F-12 + F-01 + F-ZC）  
> **F-ZC 零 C/H 跟踪**：`XLANG_F_STD_ZERO_C_FAIL=1 ./tests/run-f-std-zero-c-track-gate.sh`  
