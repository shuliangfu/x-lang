...
function main(): i32 {
  ...
  if (fd_w < 0) { return 1; }
  if (fs.write(fd_w, &payload[0], 4 as usize) != 4) { fs.close(fd_w); return 2; }
  ...
  if (p == 0 as *u8) { fs.close(fd_r); return 5; }
  if (io.ptr_len() != 4) { fs.close(fd_r); return 6; }
  if (p[0] != 9 || p[3] != 6) { fs.close(fd_r); return 7; }
