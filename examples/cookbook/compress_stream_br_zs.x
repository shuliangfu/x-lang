/**
 * Cookbook CMP-02：brotli/zstd 统一流式压缩（STD-081 #81）。
 * 未链 lib 时 compress_init 失败则 exit 0（skip）。
 */
const compress = import("std.compress");

/** 单格式流式往返；init 失败返回 0（skip）。 */
function roundtrip(format: i32): i32 {
  let raw: u8[6] = [99, 111, 111, 107, 33, 0];
  let sb: i32 = compress.compress_state_bytes_for(format);
  if (sb <= 0 || sb > 512) {
    return 99;
  }
  let st_c: u8[512] = [];
  let st_d: u8[512] = [];
  let sc: StreamCompress = StreamCompress { format: 0, mode: 0, state: 0 as *u8, state_cap: 0 };
  let sd: StreamCompress = StreamCompress { format: 0, mode: 0, state: 0 as *u8, state_cap: 0 };
  if (compress.compress_init(&sc, &st_c[0], sb, format, compress.mode_compress()) != 0) {
    return 0;
  }
  let out: u8[64] = [];
  let scratch: u8[32] = [];
  let consumed: i32 = 0;
  let n: i32 = compress.compress_process(sc, &raw[0], 5, &scratch[0], 32, 1, &consumed);
  if (n < 0) {
    compress.compress_end(sc);
    return 1;
  }
  let i: i32 = 0;
  while (i < n) {
    out[i] = scratch[i];
    i = i + 1;
  }
  compress.compress_end(sc);
  if (n <= 0) {
    return 2;
  }
  if (compress.compress_init(&sd, &st_d[0], sb, format, compress.mode_decompress()) != 0) {
    return 3;
  }
  let dec: u8[32] = [];
  let dconsumed: i32 = 0;
  let dn: i32 = compress.compress_process(sd, &out[0], n, &scratch[0], 32, 0, &dconsumed);
  compress.compress_end(sd);
  if (dn != 5) {
    return 4;
  }
  return 0;
}

function main(): i32 {
  let br: i32 = roundtrip(compress.format_brotli());
  if (br != 0) {
    return 10 + br;
  }
  let zs: i32 = roundtrip(compress.format_zstd());
  if (zs != 0) {
    return 20 + zs;
  }
  return 0;
}
