/**
 * Cookbook CTX-01：Context 取消、超时与父子传播（STD-156）。
 *
 * 演示 background → with_cancel / with_timeout → cancel / is_cancelled /
 * remaining_ns / 键值 bag 的最小可运行路径。
 */
...
function main(): i32 {
  ...
  // 相对超时：remaining_ns 应在 (0, timeout] 内
  ...
  // 键值 bag：trace id 等元数据随 Context 传播
