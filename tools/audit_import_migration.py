#!/usr/bin/env python3
"""
Audit import syntax migration completeness in .x source (non-comment code lines).
...
def main() -> int:
    all_issues: list[str] = []
    for path in sorted(ROOT.rglob("*.x")):
    ...
    if all_issues:
        ...
        for x in all_issues:
            print(f"  {x}")
        return 1
    print(f"import migration audit OK ({len(list(ROOT.rglob('*.x')))} .x files scanned)")
    return 0
