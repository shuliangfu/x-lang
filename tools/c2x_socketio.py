...
SRC = "/home/shuliangfu/worker/shu/shux/std/socketio/socketio_glue.c"
DST = "/home/shuliangfu/worker/shu/shux/std/socketio/socketio.x"
...
def convert_function(fn: str) -> str:
    ...
    body = fn[m.end() : -1]
    x_ret = "void" if ret == "void" else "i32"
    lines = [convert_line(ln) for ln in body.splitlines()]
    lines = [ln for ln in lines if ln is not None]
    body_x = "\n".join(lines)
    return f"function {name}({convert_params(params)}): {x_ret} {{\n{body_x}\n}}\n"
...
def main():
    ...
    parts = [HEADER, CONST_DEFINES, STRUCTS_X, EXTERNS, MARKERS, HELPERS]
    count = 0
