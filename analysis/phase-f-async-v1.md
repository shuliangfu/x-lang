...
> **F-async v1**：删除 **`scheduler.c`** / **`future.c`**；锚点 **`scheduler.sx`** / **`future.sx`**；调度与 Future 胶层在 **`scheduler_glue.c`** / **`future_glue.c`**；`async_net_fs.inc.c` 仍由胶层 `#include`。

## 变更

| 项 | 前 | 后 |
|----|----|-----|
| 调度 | `scheduler.c`（~1000 行） | `scheduler.sx` + `scheduler_glue.c` |
| Future | `future.c`（~162 行） | `future.sx` + `future_glue.c` |
