...
> **F-04 v14 v1**：从 **`net.c`** 迁出 **`net_run_accept_workers_c*`** → **`workers.sx`** + **`runtime_net_workers.c`**（F-ZC）；**删除 `std/net/net.c`**。

## v14 v1 完成（✅ manifest）

| 项 | 说明 |
|----|------|
| `workers.sx` | `net_run_accept_workers_c` / `net_run_accept_workers_c_real`（thread_create/join） |
