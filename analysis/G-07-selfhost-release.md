...
> 打 tag 前须绿：`XLANG_F11_SELFHOST_RELEASE_PREP_FAIL=1 ./tests/run-f11-selfhost-release-prep-gate.sh`
...
| 二进制 | `compiler/xlang`（= relink / B-strict 定稿；与 `xlang_asm` 同步） |
| 构建入口 | `./xlang-build.sh build` 或 `cd compiler && ./build_tool ./xlang` |
...
./compiler/xlang build -o /tmp/let42 - <<'EOF'  # 或已有 let 用例
# …
EOF
```
...
XLANG_NO_HANDWRITTEN_C_STRICT=1 XLANG_NO_HANDWRITTEN_C_FAIL=1 \
  XLANG_NO_HANDWRITTEN_C_MANIFEST_ONLY=1 ./tests/run-no-handwritten-c-gate.sh
```
...
| 手写 C STRICT | `XLANG_NO_HANDWRITTEN_C_STRICT=1` manifest |
...
- macOS：`make xlang_asm` 偶发 `parser_gen` seed 问题；开发机以 Linux 验收为准  
- al06-gate 诊断文案  
