...
- **std.io.core**：自举后可在 .sx 内实现高性能 IO 逻辑，或通过 extern 调用 C 侧「高性能 IO 内核/驱动」接口；当前 C 侧 `io_read`/`io_write`（io.c）在阶段 2 可改为仅调用自研实现（不再走 io_uring/kqueue/IOCP）。
...
- **无 C 的完全自举**：用 shux 编全部 .sx 得到新 shux，**不再依赖项目内 .c/.h**；构建由 build.sx/build_tool + bootstrap_shuxc 完成，无需 Makefile。
