...
| `StreamCodec` + `stream_codec_*` | gzip 流式块处理 |
...
- `encode_into_bytes` 复用 `Bytes` 容量；大块前可 `bytes_grow`（见 std.bytes reserve 策略）。
...
|-----------|----------------|
| `bytes_clear` | L2 编解码前清空 len、保留 cap |
| `bytes_grow` / `bytes_with_capacity` | 按 `encode_upper_bound` 预分配 |
...
```
shux: [SHUX_STD_CODEC] status=ok su=1 skip=0
std-codec gate OK
```
