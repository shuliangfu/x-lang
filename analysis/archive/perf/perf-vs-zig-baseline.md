# 性能基线：Xlang asm vs C -O2 vs Zig -O2
...
```bash
make -C compiler bootstrap-driver-seed   # 或已有 ./compiler/xlang
./tests/run-perf-baseline.sh --bench
```

可选 B-strict 列：先 `make -C compiler bootstrap-driver-bstrict`，脚本会自动尝试 `./compiler/xlang_asm`。
...
| 用例 | Shu (default asm) | Shu (-backend c) | xlang_asm | C -O2 | Zig -O2 |
|------|-------------------|------------------|---------|-------|---------|
| loop_i32 | | | | | |
| mem_copy | | | | | |
| struct_param | | | | | |
...
- CI 可选：`XLANG_PERF_FAIL_ON_ZIG=1 ./tests/run-perf-baseline.sh --bench`。
...
XLANG_PERF_UPDATE_BASELINE=1 ./tests/run-perf-compile-dogfood.sh   # 有意优化后刷新基线
```
...
- **Linux CI**（`.github/workflows/ci.yml`）在 runner 有 `zig` 时跑 `XLANG_PERF_FAIL_ON_ZIG=1` / `XLANG_PERF_FAIL_ON_IO_ZIG=1`；无 zig 时跳过 Zig 列（与 P2 同模式）。本地 median 填入上表。
