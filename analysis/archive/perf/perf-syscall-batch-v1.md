...
| **可 grep** | `xlang: [XLANG_SYSCALL_BATCH]` 报告行 |
...
| 变量 | 默认 | 说明 |
|------|------|------|
| `XLANG_SYSCALL_BATCH_FAIL` | `0` | `1` 时超 cap 或 batch≥ref 硬失败 |
| `XLANG_SYSCALL_BATCH_REQUIRE_STRACE` | `0` | CI Linux 可设 `1` 禁止 SKIP |
| `XLANG_SYSCALL_BATCH_PREFIX` | `xlang: [XLANG_SYSCALL_BATCH]` | 报告前缀 |
| `XLANG_SYSCALL_BATCH_BASELINE` | `tests/baseline/syscall-batch-perf.tsv` | cap 表 |
| `XLANG_IO_BENCH_MB` | `16` | bench 文件大小（MiB） |
...
```text
xlang: [XLANG_SYSCALL_BATCH] case=io_batch_readv read=2 write=0 readv=1024 writev=0 splice=0 sendfile=0 io_total=1026 ref_case=io_read_chunked ref_io_total=4098 ok=1
```
