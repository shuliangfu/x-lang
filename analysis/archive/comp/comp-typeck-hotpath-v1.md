...
| 手段 | 用途 |
|------|------|
| **模块 dogfood** | `xlang check compiler/src/typeck/typeck.x` 中位数（PERF-004 `check_typeck`） |
| **源码体量 + 调用树** | `typeck_x_ast` → `check_block` / `check_expr` → `check_expr_impl_mega` |
...
| `typeck_x_ast` | per-function loop + library 快路径 |
...
| `typeck_soa_array_storage_size_glue` | SoA 数组存储尺寸 C glue，减少 X 递归 depth |
| `hot_reorder_layout` | `XLANG_HOT_REORDER=1` 热字段置前 diagnostic（DOD-CL-S2） |
...
4. hook：`run-typeck-region.sh`、`run-typeck-linear.sh`（有 xlang 时）；WPO smoke（Linux + xlang_asm）  
...
| typeck 源码 | `compiler/src/typeck/typeck.x` |
