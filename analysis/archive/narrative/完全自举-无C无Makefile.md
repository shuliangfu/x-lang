...
- **完全自举**：用 xlang 编译自身全部 .x，得到新的 xlang，且不依赖任何项目内的 .c/.h。
...
**能实现**，但需要完成下面几类工作，且要接受一个前提：**至少有一个「种子」xlang 从哪里来**。

- **方案 A（推荐）**：仓库里**纳入预编译的 bootstrap xlang 二进制**（如 `compiler/bootstrap_xlangc`，按平台放不同目录或单二进制多架构）。之后构建 = 用该二进制 + 纯脚本，不再需要任何 .c/.h/Makefile。
- **方案 B**：保留**唯一一个最小 C 文件 + 一个最小「不用 Makefile」的构建方式**（例如单条 `cc -o bootstrap_xlangc bootstrap.c`），只用于在干净环境里生成**第一个** xlang，之后全部用该 xlang 编 .x，不再用 C。这样「日常构建」不再需要 .c/.h/Makefile，只有「冷启动」需要一次 C。
...
| 当前 | 替代方式 |
|------|----------|
| 这些模块同时存在 .c 与 .x | 构建时**只使用 .x**：用 xlang 的 **-backend asm** 把 compiler 下所有 .x（含 lexer、parser、typeck、codegen、ast、preprocess、pipeline、main、asm 子树等）编成 .o，不再编译任何 .c。 |
...
1. **种子编译器**  
   - 使用预置的 `compiler/bootstrap_xlangc`（或按平台选的二进制）。  
   - 不依赖 Makefile、不依赖任何 .c。

2. **一步：用 bootstrap xlang 编出所有 .x 的 .o**  
   - 按依赖顺序对每个模块执行：  
     `./bootstrap_xlangc -backend asm -o build_asm/<module>.o -L .. -L src/... <path>.x`  
   - 模块列表与顺序与当前 `scripts/build_xlang_asm.sh` 一致（token → ast → codegen → … → pipeline → main 等）。
...
- **build.x 的角色**：与 Zig 的 **build.zig** 一致——**仅用于配置构建**，用项目语言描述「怎么编、编什么、步骤顺序」，是唯一的构建配置入口，用于代替 Makefile。  
  ...
  - Xlang：**build.x 只做构建配置**（步骤 ID、顺序、默认/asm 路径）；xlang 将其编成 **build_tool**，执行 `./build_tool [xlang路径] [asm]` 时按该配置运行；具体命令由 build_runtime 提供，配置与执行分离。

- **根目录 Makefile**：当前仅做 `make -C compiler` 委托，**可以完全由 build.x 替代**：  
  - 约定「构建入口」为：在 compiler 目录下执行 `xlang ../build.x -o build_tool && ./build_tool ./xlangc`（或由脚本/文档提供一条等价命令）。  
  - 不再需要根目录 Makefile；需要时也可保留一个极简脚本（如 `./build`）仅负责调用上述命令。

- **compiler/Makefile**：在「完全自举、无 C」方案中，由 **build.x 描述的步骤** 替代（包括编 C、生成 pipeline_gen.c/driver_gen.c、编 .x、链接）；无 C 时则由 **build.x + bootstrap_xlangc** 直接编所有 .x 并链接，不再需要 compiler/Makefile。  
- 因此：**根目录与 compiler 的 Makefile 都可以被「通过 build.x 构建」取代**，build.x 即我们的 build.zig。

### 3.3 为何当前 build_tool 仍依赖 build_runtime.c？如何彻底去掉？

- **当前状态**：build_tool 由「build.x → xlang -E → build_gen.c」与 **build_runtime.c** 链接而成。build.x 只提供**配置**（步骤顺序、getter）；**执行逻辑**（main、拼命令、system()、对 pipeline_gen.c/driver_gen.c/preprocess_gen.c 的 patch）全部在 **build_runtime.c** 里。
- **为何还没去掉**：实现步骤第四、五步是「bootstrap_xlangc + build.sh」和「删除 .c/.h、Makefile」；第 5 步暂不执行。把「执行」从 C 迁到 .x 需要先让 build_tool 能单独用 .x 编出来并链 crt0 + libc，再在 .x 里实现 run_step（拼命令、调 system）和 patch（读文件、字符串替换、写回），工作量较大，故先保留 build_runtime.c。
...
1. ✅ **提供 asm 入口与 argv**  
   ...
   - **已完成**：`crt0_x86_64.s` 已存在，Makefile 已增加 `crt0_x86_64.o` 目标（仅 Linux）；链接 asm 版 xlang 时可用该 .o。
...
3. ✅ **在 main.x 中消除对 C 的 extern**  
   ...
   - **已完成**：`run_compiler_x_path` 在 main.x 内实现为转调 `run_compiler_x_path_impl`；`run_compiler_c_impl` 在 main.x 内桩为直接返回 1。C 端在 `XLANG_USE_X_DRIVER` 下不再定义二者，避免重复符号。

4. ✅ **引入 bootstrap xlang 与 build.sh**  
   - 在仓库中加入预编译的 **bootstrap_xlangc**（例如 `compiler/bootstrap_xlangc`，或按 `uname -m` 选不同文件）。  
   - 实现 **build.sh**：仅用 bootstrap_xlangc + asm 后端编所有 .x → build_asm/*.o，再链接 crt0 + build_asm/*.o + -lc，产出 xlang。  
   - **部分完成**：根目录已实现 `build.sh`（以 build.x 为入口，无 xlang 时优先用 `compiler/bootstrap_xlangc`）。**本地生成 bootstrap_xlangc**：在 compiler 目录执行 `./scripts/bootstrap_xlangc_create.sh`（需先有 `./xlangc`），得到 `./bootstrap_xlangc` 后即可用 build.sh 不再依赖 make；预编二进制入库仍可选。

5. **删除 .c、.h、Makefile**（暂不执行，仅作文档保留）  
   - 确认上述构建能稳定产出可用的 xlang 后，再删除：  
     - 所有 compiler 下的 .c、.h；  
     - Makefile。  
   - 保留：.x、.s（crt0）、std、脚本、bootstrap_xlangc。  
...
- **做法**：  
  1. **构建**：用 asm 路径产出 `xlang_asm`（`./build_tool ./xlangc asm`，即用 .x → asm → .o + 最少 C 桩 `runtime_asm_build.o`/`runtime_driver.o` 链接）。  
  2. **测试**：用 `xlang_asm` 跑全量测试：`XLANG=compiler/xlangc_asm ./tests/run-all.sh`。  
  3. **一键脚本**：`./tests/run-without-c.sh` 会先构建 xlang_asm，再以 XLANG=xlang_asm 执行 run-all.sh；全部通过即视为「脱离 C 依赖测试」通过。  
...
- **今后开发是否遇阻（简要分析）**：  
  ...
  - **build_tool**：若进一步去掉 build_runtime.c，需在 .x 中实现命令拼接、system()、patch 逻辑，并保证 build_tool 仍能稳定产出 xlang。  
...
- **种子二进制**：方案 A 依赖预置的 bootstrap_xlangc；需为每个目标平台准备一个（或一个多架构包）。方案 B 则保留一次性的最小 C 冷启动。  
...
- **能不能实现？** 能：用 **.x + asm（crt0 + driver_get_argv_i）+ 脚本 + bootstrap xlang** 可以做到**完全自举、不再需要 .c、.h、Makefile**。  
- **要怎么实现？** 按第四节顺序：  
  1）asm crt0 与 driver_get_argv_i；  
  2）pipeline.x 实现 pipeline_parse_into_buf；  
  3）main.x 去掉对 run_compiler_* 的 C 依赖；  
  4）引入 bootstrap_xlangc + build.sh；  
...
| **步骤 3：main.x 去 C extern** | `run_compiler_x_path` / `run_compiler_c_impl` 在 main.x 实现或桩；C 在 `XLANG_USE_X_DRIVER` 下不定义二者。 |
| **步骤 4（部分）** | `build.sh` 已实现（build_tool + xlang；无 xlang 时可用 `bootstrap_xlangc`）；**bootstrap_xlangc 二进制尚未纳入仓库**。 |
| **build.x 配置化** | build.x 仅做步骤顺序配置；build_tool 支持默认构建与 `asm` 路径（`./build_tool ./xlangc asm`）。 |
| **脱离 C 依赖测试** | `tests/run-without-c.sh` 已提供：构建 xlang_asm 后用其跑 run-all.sh。 |

### 7.2 未完成 / 部分完成项（已实现部分见下）

| 项目 | 说明 | 当前状态 |
|------|------|----------|
| **步骤 4（缺口）** | 预置 bootstrap_xlangc 未入库 | **已提供** `compiler/scripts/bootstrap_xlangc_create.sh`：首次 `make` 或 `build_tool` 后执行该脚本即可生成 `compiler/bootstrap_xlangc`，供 build.sh 无 xlang 时使用；预编二进制入库仍可选。 |
| **步骤 5** | 未执行 | 仍保留；按设计测试通过后再删 .c/.h/Makefile。 |
| **asm 链接仍用 C 桩** | 改为 crt0 + build_asm/*.o + -lc | **已实现**：Linux 下 `build_xlang_asm.sh` 优先尝试 **crt0_x86_64.o + typeck_f64_bits.o + runtime_panic.o + build_asm/*.o + -lc -lm**，成功则 xlang_asm 不再链 runtime_driver.o。**typeck_f64_bits 零 C**：新增 `src/typeck/typeck_f64_bits_x86_64.s`，Linux 下 Makefile 由该 .s 生成 typeck_f64_bits.o，不再用 typeck_f64_bits.c。 |
...
- **若「完全自举」仅指**：用现有 xlang 编译自身 .x 得到新的 xlang  
  → **已实现**：`./build_tool ./xlangc`（默认 C 展开路径）或 `./build_tool ./xlangc asm`（asm 路径）均可产出新 xlang；但**仍依赖**由 C 编出的「当前 xlang」和（默认路径下）C 生成的 pipeline_gen.c/driver_gen.c 等。

- **若「完全自举」指**：**不再使用任何项目内 .c**（无 C 源码、无 C 编译、无 C 链接）  
  → **尚未实现**。当前进展与缺口：  
  1）**bootstrap_xlangc**：脚本已提供，入库或等效「种子」仍可选。  
...
2. ~~**（可选）asm 链接改用 crt0**~~：**已做**——build_xlang_asm.sh 在 Linux 下优先用 crt0 + typeck_f64_bits.o（来自 typeck_f64_bits_x86_64.s）+ runtime_panic.o；typeck 符号已零 C。  
3. **（可选）bootstrap_xlangc 入库**：本地已可用 `compiler/scripts/bootstrap_xlangc_create.sh` 生成；为发行/CI 可纳入预编二进制。  
...
| 项 | 说明 |
|----|------|
| bootstrap_xlangc | `compiler/scripts/bootstrap_xlangc_create.sh` 可在首次 make/build_tool 后生成 `compiler/bootstrap_xlangc`，供 build.sh 无 xlang 时使用。 |
...
| 3 | **bootstrap_xlangc 入库（可选）** | 将预编 bootstrap_xlangc 纳入仓库或发布包，使新 clone 无需先有 C 编出的 xlang。 |
| 4 | **第 5 步：删 .c/.h/Makefile** | 在 run-without-c.sh 及自举测试通过、且 build_tool 无 C 后，删除项目内 .c/.h 与 Makefile，仅保留 .x + asm(.s) + 脚本。 |

**检查清单（完全无 C 达成时）**

- [ ] `./tests/run-without-c.sh` 全绿（asm 编全 + 链接成功，用 xlang_asm 跑测试）。  
- [ ] build_tool 仅由 crt0 + .x 生成 .o + -lc 链接，不链任何 .c 生成 .o。  
- [ ] 构建 xlang_asm 时不再链接 runtime_driver.o / typeck_f64_bits.c；Linux 下 typeck_f64_bits 来自 .s。  
- [ ] 仓库或 CI 具备「无 C 种子」：bootstrap_xlangc 或等效，使从零可构建出 xlang。
...
| 项 | 说明 |
|----|------|
| build_runner.x | entry、asm 路径纯 .x（XLANG=path + system）、默认路径调 build_run_step。 |
