# VSCode 插件分析与实现指南（.x / Xlang）

> 本文档分析如何为 Xlang（.x）编写 VSCode 扩展，实现代码高亮、语言配置，以及可选的语法/类型检测与代码导航。与项目宗旨「开发简单易上手」一致，优先提升编辑体验。
...
| **P1** | 语检测（问题面板显示 xlang 报错） | 中 | 可调用 xlang 或通过 LSP 对接编译器 |
...
- **contributes.languages**：注册语言 id（如 `su`）、扩展名 `.x`、别名 `xlang`。
...
- **方式 A**：**任务**（Task）— 用户手动运行「用 xlang 检查当前文件」，输出解析到「问题」面板（需解析 xlang 的 stderr）。
- **方式 B**：**Language Server（LSP）** — 后台跑 xlang（或封装为 LSP），对保存/打开的文件发 `textDocument/didOpen` 等，返回 `Diagnostic[]`；VSCode 通过 `vscode-languageclient` 连接。
- **方式 C**：**Code Action / 命令** — 提供「检查当前 .x 文件」命令，调用 xlang 并把错误以 `vscode.Diagnostic` 写入 `vscode.diagnostics`。
...
- **方案 1**：**LSP** — 自写或复用 LSP 实现，内部调用 xlang 的 parser（或 C 库）得到 AST，实现 `textDocument/definition`、`textDocument/references`。
- **方案 2**：**xlang 提供 LSP 模式** — 在 compiler 中增加「LSP 前端」：读 LSP 请求、调现有 parse/typeck、返回符号与位置；VSCode 扩展只起客户端作用。
...
6. **（可选）P1**：增加命令或任务，调用 xlang 并展示诊断；或接入 LSP。
