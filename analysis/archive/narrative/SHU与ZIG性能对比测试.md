...
搜索：二分查找、哈希表插入/查找/删除。
常见容器：动态数组（Vector）、链表、树、哈希表（如果 Xlang 有对应 stdlib 或手动实现）。
...
分配器对比：Xlang 默认分配器 vs Zig 的 GeneralPurposeAllocator / ArenaAllocator / FixedBufferAllocator。
内存占用：峰值 RSS、总分配量、碎片情况（用 valgrind/massif 或 OS 工具）。
GC vs 手动：Xlang 如果是手动内存或简单 RAII，与 Zig 的手动+comptime 方式对比。
缓存局部性与 prefetch 影响。
...
异步/事件循环（如果适用）。
Zig 在并发上设计很现代，Xlang 需要看是否支持类似能力。
