...
| 类别 | 表现 | 影响函数 |
|---|---|---|
| 深循环 + 大栈帧 | X emit code_len 爆炸或 Segfault | parse_into* / parse / parse_one_function_impl |
...
| A3 | 复杂返回值 lowering（LibraryParseResult 等） | 3 个按值返回包装可 X emit |
...
| 函数 | 策略 | 备注 |
|---|---|---|
| `wrap_block_ref_as_expr` | 保持 C glue | X safe 失败 |
