...
将 **parser / typeck 前端子集** 纳入 Stage2 自举 dogfood：两代编译器（seed `shu_stage1/2` 或 B-strict `xlang_asm_stage1/2`）均须对六条烟测 `check` 通过；链接可用时额外 `link+run`。
...
Runner：`tests/run-bootstrap-stage2-dogfood-parser-typeck.sh`（`XLANG=` 指定代际）。  
...
```
xlang: [XLANG_BOOT019] status=ok check_ok=6 link_ok=N skip=M
```
