...
对每个模块生成临时 `import <mod>; function main(): i32 { return 0; }` 并执行 `xlang check -L .`。
...
**工具链**：优先 `compiler/xlang-c`（跨平台 typeck）；无则 native `xlang`；皆无则 gate **SKIP** runnable。
...
```
xlang: [XLANG_STDLIB_CHECK] status=ok core=11 std=58 fail=0 skip=0
```
