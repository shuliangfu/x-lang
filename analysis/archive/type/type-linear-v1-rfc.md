...
- **不**跨分支做可达性分析（v1 保守语义）：任一分支内 move 即视为 binding 已 move，合并点后不可再用（见 `return_branch.x`）。
...
X/WPO 路径：`pipeline_typeck_linear_*_c`（`pipeline_glue.c`）与 C 侧措辞一致。
...
| 文件 | 预期 |
|------|------|
| `tests/typeck/linear/move_ok.x` | ✅ typeck + emit |
| `tests/typeck/linear/double_move.x` | ❌ `linear value used after move` |
| `tests/typeck/linear/call_double.x` | ❌ 同上（call 消耗实参） |
| `tests/typeck/linear/addr_of.x` | ❌ `cannot take address of linear value` |
| `tests/typeck/linear/return_branch.x` | ❌ 分支 return 后合并点二次使用 |
