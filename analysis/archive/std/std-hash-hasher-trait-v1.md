...
Shux 尚无语言级 `trait`，v1 以**统一 C 状态机 + `.x` 工厂 API** 表达 Hasher：
...
manifest 校验 API / 常量 / C 符号；**C 烟测** `hasher_switch_ok.c` 校验 aHash 金样；有 native `shux-c` 时跑 `hasher_switch.x`（仅 SipHash≠aHash 与默认算法）。
