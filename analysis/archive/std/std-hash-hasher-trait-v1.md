...
Xlang 尚无语言级 `trait`，v1 以**统一 C 状态机 + `.x` 工厂 API** 表达 Hasher：
...
| `finish` | `finish_algo` |
| `drop` | `free_algo` |
| 默认算法 | `default_hasher()` / `XLANG_HASH_ALGO` |
...
**结论**：生产默认保持 SipHash；需要吞吐的内存表可在预研阶段显式选 `HASHER_AHASH`，待真 aHash 落地后替换 `hash_glue.c` 内实现而不改 `.x` API。
...
manifest 校验 API / 常量 / C 符号；**C 烟测** `hasher_switch_ok.c` 校验 aHash 金样；有 native `xlang-c` 时跑 `hasher_switch.x`（仅 SipHash≠aHash 与默认算法）。
