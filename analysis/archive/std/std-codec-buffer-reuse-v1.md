...
| L2 Bytes 桥 | `encode_into_bytes` / `decode_from_bytes` | `Bytes` 堆（`grow`） | 复用 `Bytes.cap`，不清空容量 |
| L3 流 | `codec_process` | 调用方 `state` + `out` | 分块增量，无整包拷贝 |
...
1. 内部先 `clear`（`len=0`，**保留 cap**）。
2. `encode_upper_bound` 估算上界 → `grow(b, need)`。
...
大块或固定大小协议帧：先 `with_capacity` 或 `grow`，再 L1 直接 `encoder_encode` 到 `b.ptr`（跳过 L2 的 clear+grow 组合亦可）。

### 3.3 流式

`StreamCodec` 的 `state` 由调用方提供（`codec_state_bytes()` 取最小尺寸）；`out` 为每块输出缓冲。gzip/base64 stream 均不持有堆指针。

### 3.4 与 StrView / IO

- 解码结果在 `Bytes` 内：用 `as_view` 零拷贝读（生命周期 ≤ `Bytes`）。
- 写 socket/file：用 `as_buffer` 桥接 `std.mem.Buffer` → `io.write`。
...
| 避免 | 推荐 |
|------|------|
| 每帧 `new` + `deinit` | 循环外建 `Bytes`，循环内 L2 或 L1 |
| 未知上界反复小步 `append` | `encode_upper_bound` + 一次 `grow` |
