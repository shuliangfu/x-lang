...
> 更新时间：2026-06-17  
> 状态：**定版（v1）**  
> 关联：P4 最小正则、`std/regex/regex.x`（F-regex v2 纯 .x）
...
| 4 | 烟测：`tests/regex/*.x`、`regex_min_ok.c` |
...
| `match(re, str, len): i32` | 子串匹配；成功 `0`，失败 `-1` |
| `free(re): void` | 释放 |

**v1 纯引擎子集**（`regex.x`，三平台同一实现）：
...
| 平台 | 策略 |
|------|------|
| Linux | manifest `must`：C 烟测 + `.x` compile/match |
| macOS | 同上 |
| Windows (MSYS) | 同上（无 `regex.h`） |

C 烟测 `regex_min_smoke_c` 覆盖 literal/dot/class/star/question；`.x` 烟测验证 `import("std.regex")` 链入。

---

## 4. 可选链入

引用 `compile` / `match` 时链接 `std/regex/regex.o`（`get_std_regex_o_path`）。
...
烟测：`tests/regex/possessive_match.x`（`.++!` 与贪婪 `.+!` 对照）。
