...
- `shux_string_portable_memmem_c`
- `shux_string_memmem_c`

### 3.2 当前仍由 seed/rest 提供
- `shux_string_ptr_at_c`（ptr + off，null 检查）
- `shux_string_memcmp_c`（libc memcmp 包装）
- `shux_string_memcmp_at_c`（带偏移 memcmp）
- `shux_string_copy_c`（libc memcpy 包装）

## 4. ABI Manifest
- _impl 残余列表：无（DIRECT 模式，.x 直接实现）
- thin+rest 宏边界：`SHUX_RUNTIME_STRING_FAST_FROM_X`
...
- thin 函数内部调用链：`shux_string_memmem_c` → `shux_string_memchr_c` + `shux_string_portable_memmem_c`（thin 调 thin，OK）
