...
### 3.1 必须由 thin/.x 提供
- IMPL wrapper（1）：`shux_process_argv_bind_from_crt`

### 3.2 当前仍由 seed/rest 提供
- _impl（1）：`shux_process_argv_bind_from_crt_impl`（含 `#if defined(__APPLE__)`/`#elif defined(__linux__)` 平台条件）
- rest（4）：`process_shux_argc_get`、`process_shux_argv_get`、`process_args_count_c`（weak）、`process_arg_c`（weak）

## 4. ABI Manifest
- _impl 残余列表：1 个
- DIRECT 符号列表：无
- thin+rest 宏边界：`SHUX_RUNTIME_PROCESS_ARGV_FROM_X`
- 前向声明：1 个（`shux_process_argv_bind_from_crt`）
...
- 签名修复：.x 中 `shux_process_argv_bind_from_crt` 原为 `(argc: i32, argv: *u8)`，seed 为 `(void)`，已修复 .x 为无参数（_impl 从 CRT 读取 argc/argv，不需调用方传入）
...
- `shux_process_argv_ctor_bind`（constructor）调用 thin wrapper，需前向声明
