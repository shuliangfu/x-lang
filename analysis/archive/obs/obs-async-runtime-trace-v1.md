...
| **零默认开销** | 未设 `XLANG_ASYNC_RUNTIME_TRACE` 时不计时、不打印 |
...
| 变量 | 默认 | 行为 |
|------|------|------|
| `XLANG_ASYNC_RUNTIME_TRACE` | 关 | 非空且非 `0` 启用采样 |
| `XLANG_ASYNC_RUNTIME_TRACE_TOPN` | `20` | flush 时输出的慢事件行数（1..64） |
| `XLANG_ASYNC_RUNTIME_TRACE_SAMPLE` | `1` | 每 N 次事件采 1 次（低于慢阈值的普通事件） |
| `XLANG_ASYNC_RUNTIME_TRACE_SLOW_US` | `500` | ≥ 该微秒数的事件**必**记录 |

与 `XLANG_ASYNC_YIELD` / `XLANG_ASYNC_IO_WAIT` 独立；可同时开启。
...
| `suspend` | 任务返回 `XLANG_ASYNC_SUSPENDED` | 0 |
...
```text
xlang: [XLANG_ASYNC_RUNTIME_TRACE] summary events=42 slow_top=5
```

明细行（按 `us` 降序）：

```text
xlang: [XLANG_ASYNC_RUNTIME_TRACE] rank=1 kind=task_run worker=0 us=1234 extra=0 fn=0x...
```
...
2. 烟测：`async_runtime_trace_smoke.c` + `XLANG_ASYNC_RUNTIME_TRACE=1` grep summary/rank  
...
```bash
# 调度队列烟测 + trace
XLANG_ASYNC_RUNTIME_TRACE=1 cc -std=gnu11 -o /tmp/fg_trace \
  tests/bench/async_runtime_trace_smoke.c std/async/scheduler.o
XLANG_ASYNC_RUNTIME_TRACE=1 /tmp/fg_trace 2>&1 | grep XLANG_ASYNC_RUNTIME_TRACE

# IO spawn 长尾（与 run-async.sh harness 联动）
XLANG_ASYNC_RUNTIME_TRACE=1 XLANG_ASYNC_YIELD=1 ./tests/bench/async_run_io_spawn_parallel
```
