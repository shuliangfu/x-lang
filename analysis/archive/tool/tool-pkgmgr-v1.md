...
| 4 | 原型：`./scripts/xlang-deps-resolve.sh tests/fixtures/pkgmgr/xlang.pkg.tsv` |
...
`xlang-deps-resolve.sh` 读取清单，对每个 `require` 在 `lib_root` 列表中探测文件；全命中则 exit 0。
...
| case_id | 路径 | 期望 |
|---------|------|------|
| `case_demo` | `tests/fixtures/pkgmgr/` | resolve OK + `main.x` 编译（native xlang） |
...
- 无 central registry / `xlang add` 网络下载。
