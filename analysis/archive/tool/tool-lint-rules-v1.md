...
v1 **不新增** `xlang lint` 子命令；分层通过 **`xlang check`** + 环境开关 + CI profile 表达。`xlang fmt --check` 归属 **info** 层（TOOL-001）。
...
| 环境变量 | 说明 |
|----------|------|
| `XLANG_LINT_PROFILE` | 选用 profile 名（默认 `ci-default`）；CI **configurable** |
| `XLANG_LINT_CI_FAIL_ON` | `error`（默认）或 `warn`：warn 层也令 `check` 非零退出 |
| `XLANG_PAD_FIELDS` | `1` 启用 L3 |
| `XLANG_HOT_REORDER` | `1` 启用 L4 |
| `XLANG_UNUSED_HINT` | `1` 启用 L6（info，默认不阻断） |
| `XLANG_UNUSED_PRIVATE` | `0` 关闭 L7；`1` 强制开；默认在 `xlang check` 与 LSP 诊断会话中开启 |
...
| `case_warn_pad` | `tests/lint/lint_warn_pad.x` | `XLANG_PAD_FIELDS=1 check` 含 `warning: -pad-fields`，exit 0 |
| `case_warn_reorder` | `tests/lint/lint_warn_reorder.x` | `XLANG_HOT_REORDER=1 check` 含 `warning: -hot-reorder`，exit 0 |
| `case_info_unused` | `tests/lint/lint_unused_hint.x` | `XLANG_UNUSED_HINT=1 check` 含 `unused binding`，exit 0 |

---

## 6. 非目标（v1）

- 不实现独立 `xlang lint` CLI（v1.1 可收敛 check+profile）。
- 不新增 LSP Information 级别诊断（info 层暂由 fmt 承担）。
- 不把 warn 默认升为 CI 失败（须显式 `XLANG_LINT_CI_FAIL_ON=warn`）。
