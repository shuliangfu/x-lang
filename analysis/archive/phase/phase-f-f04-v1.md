...
> **F-04 v1**：**`tls_stub.inc.c`** → **`std/net/tls_stub.x`**；`mod.x` import 桩后端；OpenSSL/mbedTLS 仍留 `tls_*.inc.c`（v2+）。

## v1 完成（✅ manifest）

| 项 | 说明 |
|----|------|
| `tls_stub.x` | 全部 `net_tls_*_c` 桩 + `net_tls_last_error_c` |
| `mod.x` | `import("std.net.tls_stub")`；公开 API 经 `tls_plat` 转发 |
