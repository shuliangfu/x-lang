...
> **E-04 v32**：自 `runtime.c` 将 **driver/bootstrap 预处理** 迁入 **`runtime_pipeline_abi.c/h`**；xlang-c 仍经 `XLANG_RUNTIME_PREPROCESS` 宏链 `preprocess.o`。

## v32 完成（✅）

| 符号 | 说明 |
|------|------|
| `xlang_preprocess` | X preprocess.x 路径（默认）或 `preprocess_c_fallback`（LEGACY） |
| `XLANG_RUNTIME_PREPROCESS` | runtime.c 宏：driver 用 `xlang_preprocess`，xlang-c 用 `preprocess()` |
| `RUNTIME_PIPELINE_ABI_CFLAGS` | Makefile 为 pipeline_abi.o 注入 `-DXLANG_USE_X_PIPELINE` |
