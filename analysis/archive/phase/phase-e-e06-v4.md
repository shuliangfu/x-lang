...
> **E-06 v4**：在 E-06 v2/v3 基础上，**gen_driver hybrid / fallback** 链路与 `ensure_asm_driver_seed_c_objs` 在 X 前端 `*.o` 就绪时同样跳过 C 前端 seed `cc -c` 与链接；**不要求** `XLANG_ASM_EXPERIMENTAL_SKIP_GEN`（B-strict 仍禁 gen_driver 回退）。
...
| 路径 | 说明 |
|------|------|
| ~~Windows B-strict~~ | ✅ E-06 v5：`XLANG_WIN_BSTRICT=1` + `phase-e-e06-v5.md` |
