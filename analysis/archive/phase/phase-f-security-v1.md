...
**2026-08-30 leftover XLANG fallthrough 已收**（f-security-v1：`for cand in "${XLANG:-}"` 退役；prefer asm＋`XLANG_LINK_XLANG`；显式坏 XLANG 先硬 die；缺 native 硬 die；leftover nested xlang_compiler_make／std-security 不重写）。
