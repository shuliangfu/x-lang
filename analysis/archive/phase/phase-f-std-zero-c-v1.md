...
**v2 已闭合（纯 .x 或逻辑下沉）**：cache / config / context / datetime / dynlib / elf / ffi / hash / json / queue / regex / schema / security / **simd（F-ZC）** / socketio / sync-lock-diag / tar / task / test / trace / unicode / async-future / backtrace / url 等 **21** 项。

## 3. 推荐推进顺序

1. **Z8 小胶层批量**（env/log/time/random/simd…）— 模式已验证：逻辑 `.x` + `*_os_glue.c` → 再 os 桩 `.s`/syscall  
...
- [ ] `compiler/Makefile`：`ld -r` 或纯 `.x` 规则  
...
| 项 | 说明 |
|----|------|
| `compress_common.h` | → **`std/compress/common.x`**（已删 .h） |
