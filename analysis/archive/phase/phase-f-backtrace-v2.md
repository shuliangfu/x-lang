...
> **F-backtrace v2**：帧 read/write、符号名辅助、gold 烟测编排从 **`backtrace_glue.c`** 迁入 **`backtrace.x`**；平台 capture/symbolicate/crash evidence 在 **`compiler/src/asm/runtime_backtrace_platform.c`**（F-ZC）。
