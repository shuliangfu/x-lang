...
> **NL-07 v4**：在 v3 烟测硬绿之上，**可选** 在 `xlang_asm` 已构建时以 `XLANG_BOOTSTRAP_NOSTDLIB=1` 重跑 `build_xlang_asm` 并记录链接结果。
...
| xlang-c 冷链修复 | runtime_pipeline_abi X 符号弱桩 | `runtime_pipeline_abi_xlang_c_stubs.c` |
...
```bash
# 前置：bootstrap-driver-bstrict 已产出 xlang_asm
...
# v4 gate（Linux x86_64 + xlang_asm 存在时硬试跑）
XLANG_NOLIBC_N07_V4_TRY_BUILD=1 XLANG_NOLIBC_N07_V4_FAIL=1 ./tests/run-nolibc-n07-v4-build-gate.sh
```
...
| 项 | 结果 |
|----|------|
| 命令 | `XLANG_NOLIBC_N07_V4_TRY_BUILD=1 XLANG_NOLIBC_N07_V4_FAIL=1 ./tests/run-nolibc-n07-v4-build-gate.sh` |
| 日志 | `/tmp/ubuntu_n07_v4_try_a0a0fc0e.log` · `compiler/build_asm/.bootstrap_nostdlib_link_err` |
| 机械根修 | `build_xlang_asm.sh`：`bootstrap_link_tail_crt0/driver` 内 `ensure_*` 进度改 **stderr**（commit **`a0a0fc0e`**）。此前 `$(tail)` 吞进 `echo " cc -c ..."` → 链行假失败，**掩盖**真 UNDEF |
| gate | **OK**（track：`build_xlang_asm` exit 0 + **libc fallback**） |
| nostdlib crt0 | **红** — 无 `bootstrap nostdlib crt0 link OK` |
| 产物 | `ldd xlang_asm` 仍 **`libc.so.6`**（≠ 零 libc 硬绿） |

### 修后真 residual（crt0 nostdlib 链 · 分层地图）

> **禁止** 一波搅多债。硬绿按层推进；权威仍 `build_xlang_asm.sh` + freestanding/stubs + 产品对象拓扑。
...
| 备注 | 构建期 postlink smoke 偶发 WARN→experimental 回退；链后复测 `xlang_asm_postlink_smoke` **OK**（-c + asm -o exit=42）。纯 static 产品路径 hello 曾见 f64 发射 `g.0`（g05 绿；**运行时/发射质量 residual，≠ 链接 UNDEF**） |
...
## 2026-07-17 L8（Ubuntu · tip `45670027` · pure static hello -o co-emit）

| 项 | 结果 |
|----|------|
| 根修 | ① `11cfeba0` realign：**ndep > entry imports 时 keep 闭包**（禁清零 BFS）② `b90cc752` pre-parse 后 **re-seed pctx** ③ `45670027` keep 分支 **从 driver_dep 槽 rebind** module/path（本 TU `set_module`，修 pure static 双权威布局漂移） |
| 根因 | realign 清闭包 → 只 co-emit std.fmt；keep 后 pure static module_at 仍 NULL（pctx_seed 布局漂移）→ 无 `std_io_print` 体 → host-cc UNDEF |
| 日志 | `/tmp/ubuntu_n07_l8_45670027.log` · multi/global_T_multi=**0** · crt0 OK · smoke OK |
| pure static | **hello -o 绿** run=0 · `Hello World` · C≈164k · std_io defs=99；rv=42 · opt=102 |
| pure static residual | **si -o** host-cc dep emit 序 / `core_result_*` → **L8+ 已关** |
| **双端 g05** | mac rv/opt/hello/si **绿** |

## 2026-07-17 L8+（Ubuntu · tip `a5e23ae7` · pure static si -o dual-depctx）

| 项 | 结果 |
|----|------|
| 根修 | ① `72110054` weak process-wide `g_xlang_depctx_sc`（禁 per-TU static）② `a5e23ae7` **`ld_partial_export` 注入 keep `g_xlang_depctx_sc`**（禁 objcopy/exported_symbols_list 局部化） |
| 根因 | crt0 L5 `ld -r` + keep-global 把表降为 local BSS；static `depctx_sidecar_get` 双副本绑两表 → Cap module_at vs path 分叉 → types/result 前缀互换 → host-cc 隐式声明 |
| 证据 | 修前 nm `b`+`V` 双表、两 get LEA 不同址；修后仅 **`V`**、两 get 同址 `0x6cf4c0` |
| 日志 | `/tmp/ubuntu_si_depctx_rebuild_a5e23ae7.log` · crt0 OK · smoke OK |
| pure static | **si -o 绿** run=0；rv=42 · opt · hello 绿 |
| **双端 g05** | mac rv/opt/hello/si **绿** |
...
- pure static import typeck — ✅ **L7+ 已达**
- pure static hello -o co-emit — ✅ **L8 已达**
