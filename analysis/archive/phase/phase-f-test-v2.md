...
**2026-08-30 leftover XLANG fallthrough 已收**（f-test-v2：`for cand in "${XLANG:-}"` 退役；prefer asm＋`XLANG_LINK_XLANG`；显式坏 XLANG 先硬 die；缺 native 硬 die；leftover nested xlang_compiler_make／std-test-runner 不重写）。
