...
| strict 链覆盖 | `build_xlang_asm.sh` 调用 `ensure_parser_x_o_for_strict_link` | `run-c09-parser-no-c-fallback-gate.sh` |
...
| 路径 | 说明 |
|------|------|
| `OBJS_CORE` → `src/parser/parser.o` | `xlang-c` 冷启动 seed 仍用 C parser |
| `boot-mega7-gap.md` | mega7 七函数生产仍 **C glue + ret0**（bisect delta=0） |
| `XLANG_LEGACY_C_FRONTEND=1` | 显式 opt-in 恢复 C 双轨前端 |
