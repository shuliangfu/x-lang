...
> **E-05 v2**：默认 `runtime_driver_no_c.o`（`-DXLANG_NO_C_FRONTEND`）编译 `runtime.c` 时**不再** `#include` lexer/parser/typeck/codegen/ast C 前端头；仍保留 `preprocess.h`、`target_cpu.h`、`lsp/lsp_diag.h`。

## 完成（✅）

| 项 | 说明 |
|----|------|
| 条件 include | `#if !defined(XLANG_NO_C_FRONTEND)` 包裹 C 前端头 |
