...
> **NL-07 v2**：在 v1 track 之上，提供 **可选** `XLANG_BOOTSTRAP_NOSTDLIB=1` 全 bootstrap 链 nostdlib 路径 + 扩展 libc/libm 桩；**默认仍 -lc/-lm**，失败自动回退。
...
| 构建开关 | `XLANG_BOOTSTRAP_NOSTDLIB=1` 尝试 nostdlib **全部 6 链** | `compiler/scripts/build_xlang_asm.sh` NL-07 块 |
...
- 默认（未设 `XLANG_BOOTSTRAP_NOSTDLIB`）行为与 v1 相同：`-lc -lm`。

## `-lc` 基线变化

| 指标 | v1 | v2 |
|------|----|----|
| `lc_link_cmds`（build_xlang_asm 非注释含 `-lc` 行） | 6 | **2**（仅 helper 回退 echo） |

## 复现

```bash
# v2 prep（任意平台）
XLANG_NOLIBC_N07_V2_FAIL=1 ./tests/run-nolibc-n07-v2-prep-gate.sh
...
- **NL-07 v4**：`build_xlang_asm` driver/experimental/strict 全链 nostdlib 硬绿
