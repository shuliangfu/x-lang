...
> **E-04 v18**：自 `main.c` / `runtime_asm_build.c` 收拢 **`xlang_forward_main_to_main_entry`** 至 `runtime_abi.c`；C `main()` 仅保留一行转发；Linux crt0 仍直接调 `main_entry`（不经本符号）。

## v18 完成（✅）

| 符号 | 说明 |
|------|------|
| `xlang_forward_main_to_main_entry` | C main() 统一转发至 main.x 的 main_entry |
