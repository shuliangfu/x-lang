...
> **C-05 v1**：采用前置清单 **方案 B 子集** — `lsp_diag.x` 经 `-E-extern` 链入 `lsp_diag_x.o`，诊断路径走 **parse_into_buf + typeck_x_ast**；`lsp_diag.c` 仍提供收集器 / definition / hover / formatting glue。
...
- **lsp_diag_x_alias.c**：强符号覆盖 C weak 实现
...
- definition/hover/references 全迁 `.x`
