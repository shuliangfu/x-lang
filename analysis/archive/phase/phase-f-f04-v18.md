# 阶段 F-04 v18（std.crypto chacha20_poly1305.inc.c → chacha20_poly1305.x）

> **F-04 v18 v1**：**`std/crypto/chacha20_poly1305.inc.c`** → **`chacha20_poly1305.x`**（ChaCha20-Poly1305 AEAD 纯 .x 实现）。

## 迁移范围

| 文件 | 说明 |
|------|------|
| `std/crypto/chacha20_poly1305.x` | ChaCha20、Poly1305、seal/open/smoke C ABI |
| `std/crypto/core.x` | mem_eq、SHA-256、HMAC-SHA256（v16） |
| `std/crypto/aes_gcm.x` | AES-128-GCM（v17） |
...
**2026-08-29 leftover XLANG fallthrough 已收**（f04-crypto-v18：`for cand in "${XLANG:-}"` 退役；prefer asm＋`XLANG_LINK_XLANG`；显式坏 XLANG 先硬 die；缺 native 硬 die；leftover nested inventory／STD-006 crypto／host-c chacha observational／xlang_compiler_make 不重写）。
...
- **F-04 v19 ✅**：`ed25519.x` + `ed25519_ref10_glue.c`；见 `phase-f-f04-v19.md`
