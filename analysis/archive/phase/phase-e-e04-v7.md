...
> **E-04 v7**：自 `runtime.c` 迁入 **`xlang_append_linux_link_harden`**（PIE + NX + partial RELRO）；`invoke_cc` 与 `asm_invoke_ld` 共用同一实现，消除重复 `-pie` 块。

## v7 完成（✅）

| 符号 | 说明 |
|------|------|
| `xlang_append_linux_link_harden` | Linux release 链 `-pie -fpie -Wl,-z,noexecstack -Wl,-z,relro` |
