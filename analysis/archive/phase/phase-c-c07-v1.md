...
| REF | `./compiler/xlang-c`（C parser/typeck/codegen） |
| CAND | `./compiler/xlang_asm` 或 `./compiler/xlang`（C-06 `*_x.o` 前端） |
| 对齐策略 | CAND 编译加 `-backend c`，与 xlang-c 同走 C codegen |
| 断言 | compile 成败一致；typeck_ok 时双方 `typeck OK`；可选 `XLANG_C07_TRY_RUN=1` 做 -o 运行 |
...
# Report: ref=/pass=/obs=/parity=/skip=
# Soft XLANG_C07_FAIL + soft SKIP-when-missing-compiler + soft die on missing DOC retired.
# Hard: archive DOC + ## Gate + both native REF/CAND + REF typeck_ok / compile_fail.
