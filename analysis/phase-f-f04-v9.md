...
> **F-04 v9**：**`tls_mbedtls.inc.c`** → **`tls_mbedtls.x`** + 最小 **`tls_mbedtls_bio.c`**（BIO fn ptr 胶层）。
...
| 项 | 说明 |
|----|------|
| `tls_mbedtls_bio.c` | .x 暂不支持 fn ptr 取址；待语言支持后并入 `.x` |
| `mod.x` | 仍 `import("std.net.tls_stub")`；TLS 符号在独立 `.o` |
