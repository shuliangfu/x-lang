...
>
> **最后更新**：2026-07-21（精简版，聚焦 Xlang 生态核心）。

---

## 0. 一句话结论

**Xlang 生态只需 5 个核心网站：www（官网）+ docs（文档）+ blog（博客）+ registry（包管理）+ play（Playground）。全部前端用自研 dweb 框架一个项目搞定；registry 后端用 XLANG 自身写（dogfooding）。**
...
|--------|------|--------|---------|-----------|
| `www.xlang.dev` | 官方介绍官网 + 下载入口 | P0 | 自举期（0.3） | 必需（门面） |
| `docs.xlang.dev` | 文档 + 教程 + std API | P0 | 自举期（0.3） | 必需（学习入口） |
| `blog.xlang.dev` | 开发动态 + 版本发布 | P0 | 自举期（0.3） | 必需（生态动态） |
| `registry.xlang.dev` | 依赖包管理 | P0 | 0.7 | 必需（生态核心） |
| `play.xlang.dev` | 在线 Playground | P1 | 0.3-0.4 | 必需（降低试用门槛） |

### 1.2 域名体系原则

1. **主域重定向**：`xlang.dev` → `www.xlang.dev`
...
**目标**：建立官方门面 + 文档 + 动态，让外界能找到 Xlang。

| 网站 | 核心功能 | 内容来源 |
|------|---------|---------|
| `www.xlang.dev` | 首页 + 特性 + 安装 + 快速上手 + 下载（含 nightly） | 手写 |
| `docs.xlang.dev` | 语言手册 + std API + 教程 + 学习路径 | `docs/` + `std/` 自动生成 |
| `blog.xlang.dev` | 开发日志 + 版本发布 + 技术深度 | Markdown |
...
| 网站 | 上线时机 | 核心价值 |
|------|---------|---------|
| `play.xlang.dev` | 0.3-0.4 | **最高 ROI**：零安装体验 Xlang |
| `registry.xlang.dev` | 0.7 | 生态核心：包搜索/发布/版本管理 |
...
**项目结构示意**：
```
xlanglang-web/                       # dweb 项目（一个仓库搞定全部网站）
├── apps/
│   ├── www/                        # www.xlang.dev（官网 + 下载页）
│   ├── docs/                       # docs.xlang.dev（文档 + 教程 + 学习路径）
│   ├── blog/                       # blog.xlang.dev（博客）
│   ├── play/                       # play.xlang.dev（Playground）
│   └── registry-frontend/          # registry.xlang.dev（前端，调后端 API）
├── shared/                         # 共享组件 / 布局 / 主题
...
**与 Registry 后端的关系**：
- `registry.xlang.dev` 的**前端**（搜索 UI / 包详情页 / 用户主页）是 dweb 项目的一个 app
- Registry **后端 API** 独立于 dweb 项目，用 XLANG 自身写（dogfooding）
- 前端通过 HTTP 调用后端 API，前后端解耦

### 3.1 `www.xlang.dev` — 官网
...
- 编译器手册：CLI / xlang.toml / 自举流程 / 贡献指南
...
**自动生成**：`xlang doc` 命令从 `std/*.x` 的 docblock 生成 API 参考

### 3.3 `blog.xlang.dev` — 博客
...
### 3.4 `registry.xlang.dev` — 包管理网站

**详见**：[xlang-registry-架构规划.md](xlang-registry-架构规划.md)

**摘要**：
- **前端**：dweb 项目的一个 app（`apps/registry-frontend/`）
- **后端 API**：用 XLANG 自身写（dogfooding），独立部署
- 数据库：PostgreSQL + 对象存储
- 0.7 正式上线
- 包搜索 / 发布 / 版本管理 / 文档 / 下载统计
...
- 用户主页（发布的包 / 组织）
- 发布流程（`xlang publish` CLI）
- 文档自动生成（从包内 docblock）

### 3.5 `play.xlang.dev` — 在线 Playground

**定位**：零安装体验 Xlang，最高 ROI 的获客工具。

**核心功能**：
- 代码编辑器（Monaco Editor，VS Code 同款）
- 语法高亮（复用 `editors/tree-sitter-xlang`）
- 一键运行 + 输出面板
- 代码分享（短链 + 服务端存储）
- 示例预加载（新手友好）
...
- **宿主框架**：dweb（play app）
- **编辑器**：Monaco Editor
- **运行方案 A（推荐）**：xlang 编译为 WASM，浏览器内全栈运行
  - 优势：零后端成本、全球低延迟、隐私友好
...
```
[Xlang Logo]  [特性] [文档] [包] [Playground] [博客] [GitHub]
```
...
```
© 2026 Xlang · AGPL-3.0 · GitHub · Discord · 联系 · 状态 · 隐私
```

### 4.4 代码块样式

- 全站统一代码高亮主题（与 VS Code 插件一致）
- 复制按钮 + 行号 + 语言标识（`xlang` / `bash` / `toml`）
...
| registry 后端 | 自建 VPS（XLANG 后端） | 需要数据库 + 对象存储 |
...
| nightly 构建 | 每日定时 | 编译 xlang → 上传对象存储 → www 下载页更新 |
...
| 时间 | 里程碑 | 上线网站 |
|------|--------|---------|
| 2026-07 | DNS 占位 | `xlang.dev` + 5 个子域 placeholder |
...
`xlang.dev` 主域已注册，所有子域名无需单独注册，只需 DNS 配置。

| 子域名 | 优先级 | 建议配置时机 |
|--------|--------|-------------|
| `www.xlang.dev` | P0 | 立即 |
| `docs.xlang.dev` | P0 | 立即 |
| `blog.xlang.dev` | P0 | 立即 |
| `play.xlang.dev` | P1 | 立即（防配置遗漏） |
| `registry.xlang.dev` | P0 | **已配置** |

### 7.2 立即可做（零成本）

| 项 | 动作 | 耗时 |
|----|------|------|
| 主域 DNS | 确认 `xlang.dev` DNS 指向 Cloudflare | 5 分钟 |
| 子域占位 | 5 个子域名设置 placeholder A 记录 | 15 分钟 |
| TLS 证书 | 主域 + www 申请 Let's Encrypt | 30 分钟 |
| 续费提醒 | 设置 `xlang.dev` 续费日历提醒 | 5 分钟 |
| Placeholder 页 | `xlang.dev` 放 "Xlang · Coming 2026 Q4" 页面 | 1 小时 |
...
**风险**：0.7 时 XLANG 的 HTTP server / PostgreSQL driver 不成熟。

**缓解**：见 [registry 架构文档](xlang-registry-架构规划.md) §10.1（过渡期用 Go/Rust，XLANG 逐步替换）。
...
| 决策 | 选择 | 理由 |
|------|------|------|
| 核心网站数量 | 5 个（www/docs/blog/registry/play） | 聚焦 Xlang 生态必需 |
| 主域名 | `xlang.dev` | 已注册；简洁；体现语言品牌 |
| 子域名策略 | `<功能>.xlang.dev` | 清晰、可扩展、符合行业惯例 |
| **网站前端技术栈** | **自研 dweb 框架（多应用架构）** | 一个项目搞定 5 个网站；统一设计/组件/路由；自研可控 |
| Registry 后端语言 | XLANG 自身 | dogfooding + 验证语言能力（独立于 dweb 项目） |
...
| Placeholder | `xlang.dev` 放 "Coming 2026 Q4" 页 | 1 小时 |
| 续费提醒 | 设置 `xlang.dev` 续费日历提醒 | 5 分钟 |
