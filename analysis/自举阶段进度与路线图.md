...
| 复杂语法（泛型/trait/多文件/LSP） | **部分完成** | `run-all.sh` 体量大，默认 `SHUX=shux-c`；`.x` 全路径见 `run-all-sx.sh`。 |
...
| P0-2 | **bootstrap gate 扩展项** | `run-import.sh` / `run-compound-assign.sh` 在 seed 下绿 | **OK**（`run-bootstrap-shux-gate.sh`）；勿用失败 `shux-c -E pipeline.x` 覆盖 `pipeline_gen.c`；`pipeline_sx.o` 默认取自 `build_asm/gen_driver/` |
...
| L2 语言特性 | `SHUX=./compiler/shux tests/run-binary-expr.sh` | **OK**（bool 算术→i32：`typeck.x`/`typeck.c` + 重链 `typeck_sx.o`） |
