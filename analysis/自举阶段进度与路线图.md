...
8. **seed 重链（2026-05-27）**：`make bootstrap-driver-seed` 成功；`typeck_su_link_alias.o` / `codegen_sx_link_alias.o` / `lexer_sx_link_alias.o` + `fix_slim_arena_gen_c.pl` 等补丁；`CI=1 SHUX=./compiler/shux ./tests/run-all.sh` 全绿。
...
10. **B-strict 链（2026-05-27）**：`build_shux_asm.sh` 实验/strict 链补 `*_su_link_alias.o`；`make bootstrap-driver-bstrict` OK；`run-all-bstrict.sh` 24 项绿（第二遍 asm 自编译仍 warning，用 seed 做 `-backend asm`）。
...
| P0-3 | **多 dep / std.io import codegen** | hello、import 三类方式稳定 | SELFHOST §4.1：bootstrap shux 与 shu_su 的 preamble 差异 |
| P0-4 | **parser/typeck 边角** | `run-typeck.sh` 负例在 `-su` 下与 C 对齐 | 如 `return_operand_type_mismatch` 与 bool→i32 收窄策略 |
...
| P2-4 | LSP 全 .sx pipeline 与 C 路径一致 | `analysis/LSP开发与shuc集成分析.md` |
...
| L2 语言特性 | `SHUX=./compiler/shux tests/run-binary-expr.sh` | **OK**（bool 算术→i32：`typeck.sx`/`typeck.c` + 重链 `typeck_su.o`） |
| L2b csv/import 链接 | `SHUX=./compiler/shux tests/run-csv.sh` | **OK**（`codegen` import 前缀 + `std/csv/csv.c` 别名 + 重链 `codegen_su.o`） |
