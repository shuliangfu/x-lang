...
Shux **不是从零开始自举**：语义自举（两代编译器行为一致）、C 冷启动种子、`bootstrap-driver-seed` 日常可用、asm 后端 B-strict 实验链、以及 **大量 std 模块（C 实现 + mod.x 封装）** 已经存在。
...
**含义**：`shux -backend asm -o …` 对用户 `.x` 直接出机器码；链接可仍用系统 `ld`/`clang`。

| 子项 | 状态 | 说明 |
|------|------|------|
| asm 后端覆盖算术/比较/控制流 | **大部分完成** | 近期 seed 链已修 `return 1+2`、`7%3`、`1<<4` 等；`tests/run-binary-expr.sh` 在 `SHUX=./compiler/shux` 下 **已全绿**。 |
| 复杂语法（泛型/trait/多文件/LSP） | **部分完成** | `run-all.sh` 体量大，默认 `SHUX=shux-c`；`.x` 全路径见 `run-all-x.sh`。 |
...
2. **bootstrap-driver-seed**：日常 `./compiler/shux` 可链 `.x` pipeline + asm backend partial。
...
8. **seed 重链（2026-05-27）**：`make bootstrap-driver-seed` 成功；`typeck_x_link_alias.o` / `codegen_x_link_alias.o` / `lexer_x_link_alias.o` + `fix_slim_arena_gen_c.pl` 等补丁；`CI=1 SHUX=./compiler/shux ./tests/run-all.sh` 全绿。
...
10. **B-strict 链（2026-05-27）**：`build_shux_asm.sh` 实验/strict 链补 `*_x_link_alias.o`；`make bootstrap-driver-bstrict` OK；`run-all-bstrict.sh` 24 项绿（第二遍 asm 自编译仍 warning，用 seed 做 `-backend asm`）。
...
| 编号 | 缺口 | 验收建议 | 备注 |
|------|------|----------|------|
| P0-1 | **`.x` 路径与 `shux-c` 全量 parity** | `SHUX=./compiler/shux ./tests/run-all.sh` 与 `SHUX=./compiler/shux-c` 失败集一致 | **OK**（L5：`CI=1 SHUX=./compiler/shux SHUX_LINK_SHUX=./compiler/shux ./tests/run-all.sh` ~87s；`shux-c` 基线 ~51s） |
| P0-2 | **bootstrap gate 扩展项** | `run-import.sh` / `run-compound-assign.sh` 在 seed 下绿 | **OK**（`run-bootstrap-shux-gate.sh`）；勿用失败 `shux-c -E pipeline.x` 覆盖 `pipeline_gen.c`；`pipeline_x.o` 默认取自 `build_asm/gen_driver/` |
| P0-3 | **多 dep / std.io import codegen** | hello、import 三类方式稳定 | SELFHOST §4.1：bootstrap shux 与 shux_x 的 preamble 差异 |
...
| 编号 | 缺口 | 参考 |
|------|------|------|
| P2-1 | `runtime.c` 驱动逻辑迁 `.x`（~5k 行级） | [shux-CLI重构与完全自举方案](./shux-CLI重构与完全自举方案.md) |
| P2-2 | CLI 子命令（run/build/test）全面 .x 驱动 | 同上 §二 |
| P2-3 | `build_tool` / Makefile 默认可无 Makefile 构建 | `build.x` |
| P2-4 | LSP 全 .x pipeline 与 C 路径一致 | `analysis/LSP开发与shux集成分析.md` |
...
| L2 语言特性 | `SHUX=./compiler/shux tests/run-binary-expr.sh` | **OK**（bool 算术→i32：`typeck.x`/`typeck.c` + 重链 `typeck_x.o`） |
| L2b csv/import 链接 | `SHUX=./compiler/shux tests/run-csv.sh` | **OK**（`codegen` import 前缀 + `std/csv/csv.c` 别名 + 重链 `codegen_x.o`） |
| L3 全量 C 对标 | `SHUX=./compiler/shux-c tests/run-all.sh` | 基线（应全绿） |
| L4 全量 .x（M1） | `CI=1 SHUX=./compiler/shux SHUX_RUN_ALL_BOOTSTRAP_SHUX=1 ./tests/run-all.sh` | **OK**（2026-05-27，~100s）；多数 `-o` 仍经 `SHUX_LINK_SHUX=shux-c` 分流 |
...
- 统一 bootstrap 与 release 默认产物（`shux` vs `shux-x` vs `shux_asm` 用户文档）。
