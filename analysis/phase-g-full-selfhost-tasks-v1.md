...
> **创建**：2026-06-21  
> **目标**：仓库内 **零手写 `.c`/`.h`**（compiler + core + std）；Linux bootstrap **`-nostdlib` 硬绿**；`build.sx` 替代 Makefile。  
...
| **4** | G-02b | runtime ABI 七件套 → `.sx` | 1～2 周 | ⬜ |
...
| # | 任务 | 文件 | 动作 | Gate |
|---|------|------|------|------|
| G-01-01 | 删 assert 重复 C | `core/assert/assert.c` | 逻辑已在 `core/assert/mod.sx` | `run-core-debug-assert-extend-gate.sh` |
| G-01-02 | 删 debug 重复 C | `core/debug/debug.c` | alias 已在 `core/debug/mod.sx` | 同上 |
| G-01-03 | slice 纯 `.sx` | `core/slice/slice.c` | `mod.sx` 用字段赋值构造 slice | `run-core-slice-api-gate.sh` |
| G-01-04 | mem 纯 `.sx` | `core/mem/mem.c` | volatile 读写 + fence（`.sx`/极薄 `.s`） | `run-core-mem-volatile-fence-gate.sh` |
| G-01-05 | builtin 纯 `.sx` | `core/builtin/builtin.c` | 已有 `mod.sx` 实现；更新 codegen 映射 | `run-core-builtin-bitops-gate.sh` |
...
> **阻塞**：须 G-06 预编译 seed 或 `.sx` 完全替代 `shux-c -E` 冷启动。

| # | 文件 | 替代 |
|---|------|------|
| G-02a-01 | `lexer/lexer.c` + `lexer.h` | `lexer_sx.o` |
| G-02a-02 | `ast/ast.c` + `ast.h` | `ast_sx.o` |
| G-02a-03 | `parser/parser.c` + `parser.h` | `parser_sx.o` |
| G-02a-04 | `typeck/typeck.c` + `typeck.h` | `typeck_sx.o` |
| G-02a-05 | `codegen/codegen.c` + `codegen.h` | `codegen_sx.o` |
| G-02a-06 | `codegen/autovec.c` | `autovec.sx` 或内联 |
| G-02a-07 | `preprocess.c` + `preprocess.h` | `preprocess_sx.o` |
| G-02a-08 | `lsp/lsp_diag.c` | `lsp_diag.sx` + stubs |
...
| # | 文件 | 迁往 |
|---|------|------|
| G-02b-01 | `runtime.c`（主体） | `driver.sx` + 薄 ABI |
| G-02b-02 | `runtime_abi.c/h` | `.sx` |
| G-02b-03 | `runtime_io_abi.c/h` | `.sx` |
| G-02b-04 | `runtime_proc_abi.c/h` | `.sx` |
| G-02b-05 | `runtime_link_abi.c/h` | `.sx`（最后：invoke_cc/ld） |
| G-02b-06 | `runtime_driver_abi.c/h` | `.sx` |
| G-02b-07 | `runtime_pipeline_abi.c/h` | `.sx` |
| G-02b-08 | `runtime_driver_diagnostic.c/h` | `.sx` |
| G-02b-09 | `runtime_c_import.c/h` | 删或 `.sx` |
...
| 子批 | 代表文件 | 说明 |
|------|----------|------|
| G-02c-01 | `parser_asm_thin_c.c` + 20× `*_slice.c` | 收拢进 `parser.sx` emit |
| G-02c-02 | `pipeline_*_alias.c`, `pipeline_glue_*.c` | strict 链别名 → `.sx` |
| G-02c-03 | `backend_*_dispatch.c`, `asm_backend_*.c` | asm 后端 `.sx` |
| G-02c-04 | `lsp_*.c`（6 文件） | LSP 全 `.sx` |
| G-02c-05 | `_stubs_driver.c`, `fmt_check_cmd.c`, `target_cpu.c` | driver `.sx` |
| G-02c-06 | `sx_seed_bridge.c`, `std_*_shim.c`, `ast_pool_l5_bridge.c` | bridge → `.sx` |
| G-02c-07 | `async/async_*.c` | async `.sx` |
| G-02c-08 | `runtime_panic*.c` | `.s` 或 `.sx` syscall exit |
...
| G-02d-03 | `asm/crypto/ed25519/*.c` | 纯 `.sx` 或单对象 |
| G-02d-04 | `build_runtime.c`, `build_obj_stubs.c` | G-05 build.sx |

Gate：`run-all-sx.sh` 子集；各 `run-f-*-gate.sh`
...
| `./shux min.sx -o out` | ✅ | `function main(): i32 { return 0; }` → `num_funcs=1` |
| `./shux -L .. hello.sx` | ✅ | `return if` + `fmt.println("…")`；须重建 parser_asm_thin_glue + parser_sx |
| `./shux-c -L .. hello.sx` | ✅ | C 前端路径，输出 `Hello World` |
...
1. **`-E -E-extern` 薄 TU**：`ensure_asm_pipeline_glue_standalone_obj` / `ensure_asm_gen_driver_sx_objs` 对 `pipeline.sx` 改用 `-E-extern`（全量 `-E` 内联 std.io 截断 ~142KB、`exit=1`）。
2. **`asm_strict_pipeline_selfhosted`**：`__text` 门槛 8192→6144；`resolve_path_.*su` 匹配 `resolve_path_probe_dot_sx_and_mod`（实测 __text≈6843B）。
3. **strict 重链符号**：`pipeline_sx_glue_support` 强制导出 `pipeline_load_and_sync_*`；新增 `pipeline_fill_dep_strict_alias.c`；跳过 text 桩 `*_enc.o`。
4. **`driver_compile_link`**：从 `driver_compile_sx.o` 部分导出 `parse_argv_loop`；`driver_compile_link` 补全后 **re-filter** 避免与 `driver_compile_sx.o` 重复定义。

**gen1 实测（Docker Linux x86_64）**：`B-strict OK — LINK_MODE=asm_only_strict` ✅；`run-linux-a09-a11-gate` 全链重跑中。

**本轮 parse 修复（根因）**：

1. **`token.sx` 与 `token.h` 枚举漂移**：缺 `TRY/CATCH/TYPE/ATTR_REPR_COMPATIBLE/DOTDOT`，lexer 发 `TOKEN_IDENT=37` 被 C 薄层读成 `TOKEN_ASYNC` → buf/impl 均失败。
...
4. **`TOKEN_STRING` 未实现（hello 阻塞）**：`parse_primary` 双次 `lexer_next` 跳过 `TOKEN_STRING`，`foo("…")` 实参 parse 失败 → impl 失败、buf 回退只认 `return int`；`return if` 整函数 skip。修复：`parser_asm_primary_slice.c` 第一次 `lexer_next` 后处理 `TOKEN_STRING`；`parser.sx` `parse_body_lets` 增 `TOKEN_STRING` 分支；`parser_asm_one_function_buf_slice.c` buf 回退支持 `return if`。
