...
> **F-random v1 / F-ZC**：删除 **`random.c`**；锚点 **`random.sx`**（纯 .sx → **`random.o`**）；CSPRNG OS fill 在 **`runtime_random_fill.c`**（compiler runtime）。

## 变更

| 项 | 旧 | 现 |
|----|----|-----|
| PRNG / u32/u64 | `random.c` | **`random.sx`** |
| OS CSPRNG | `random_os_glue.c` | **`runtime_random_fill.c`**（已迁出 std/） |
| `random.o` | ld -r glue + .sx | **纯 `.sx`** |

## 符号

- `random_fill_bytes_c` — compiler runtime（getrandom / getentropy / BCryptGenRandom）
- `random_u32_c` / `random_u64_c` / `random_rng_smoke_c` — `random.sx`
