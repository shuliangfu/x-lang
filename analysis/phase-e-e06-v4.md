...
> **E-06 v4**：在 E-06 v2/v3 基础上，**gen_driver hybrid / fallback** 链路与 `ensure_asm_driver_seed_c_objs` 在 SX 前端 `*.o` 就绪时同样跳过 C 前端 seed `cc -c` 与链接；**不要求** `SHUX_ASM_EXPERIMENTAL_SKIP_GEN`（B-strict 仍禁 gen_driver 回退）。

## v4 完成（✅）

| 机制 | 说明 |
|------|------|
| `asm_seed_sx_frontend_o_ready` | 共用 SX 就绪检测 |
| `asm_seed_omit_c_frontend_seed` | SX 就绪即 omit（无 SKIP_GEN 要求） |
| `asm_seed_use_sx_frontend` | B-strict：SKIP_GEN + SX 就绪 |
