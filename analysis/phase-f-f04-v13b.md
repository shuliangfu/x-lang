...
> **F-04 v13b v1**：从 **`net.c`** 迁出 **UDP 批量 recv/send** → **`udp_batch.sx`** + **`compiler/src/asm/runtime_net_udp_batch.c`**（F-ZC）。

## v13b v1 完成（✅ manifest）

| 项 | 说明 |
|----|------|
| `udp_batch.sx` | `net_udp_recv_many_c` / `net_udp_send_many_c` / `net_udp_recv_many_buf_c` / `net_udp_send_many_buf_c` |
...
| 项 | 说明 |
|----|------|
| accept workers | **F-04 v14 ✅** 见 `workers.sx` |
