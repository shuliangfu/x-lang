...
> **目标**：全面实现自举，**编译器业务逻辑一律不用 C 实现**；若自举过程中因 shux 能力不足而受阻，**允许且仅允许通过修改 C 代码、重新编译 shux 来补齐能力**，再继续在 .x 中实现自举。
...
### 1.2 遇阻时：改 C、重编 shux、再继续 .x

- 自举过程中若出现：  
  - **语言/语法不支持**（例如 .x 缺少某种语法或类型），或  
  - **shux 行为不符合预期**（bug、缺失特性、无法编译某段 .x 代码），  
  则允许**修改 shux 的 C 源码**（如 `parser.c`、`lexer.c`、`typeck.c`、`codegen.c`、`ast.c`、`main.c`、`preprocess.c` 等），**重新编译得到新的 shux**，用新 shux 继续自举。
- **原则**：  
  - 修改 C 的**目的**是「让 shux 支持自举所需的语言能力或修复阻碍自举的缺陷」，而不是在 C 里实现新的编译器业务逻辑。  
...
当出现以下情况时，**允许**修改 shux 的 C 代码并重新编译 shux，以继续自举：

| 情况 | 可修改的 C 位置 | 目的 |
|------|------------------|------|
| **.x 语法/语义 shux 不支持** | parser.c, lexer.c, include/ast.h, ast.c | 增加新关键字、新 AST 节点、新语法规则，使 shux 能解析并生成对应 AST |
...
- **遇阻时改 C**：若某语法/类型 shux 当前不支持（如新关键字、新 AST 节点），在 parser.c/lexer.c/ast.h 等中扩展，重编 shux，再在 .x 中使用。
- **验收**：compiler 下所有 .x 源码能由当前 shux 编译通过（类型与语法无缺漏）。

### 阶段 3：.x 覆盖完整代码生成

- **目标**：codegen.x 支持所有已用到的 ExprKind、块内语句、函数/模块输出；不调用 C 的 codegen 业务接口。
- **遇阻时改 C**：若 shux 生成的 C 某处错误或缺失（如某 AST 节点未在 codegen.c 中处理），在 codegen.c 中补最小输出或修 bug，重编 shux；逻辑扩展仍以 .x 为主。
- **验收**：用 .x 流水线生成 compiler 自身所需 C，再经 cc 得到可执行 shux。

### 阶段 4：驱动与可选预处理迁入 .x

- **目标**：命令行解析、读文件、调用 pipeline、调用 cc 等尽可能在 .x 中实现；若保留 C 的 main，则只做「调 .x 入口 + 最小 I/O」。
- **遇阻时改 C**：若 .x 侧需要新的运行时接口（如读文件、执行子进程），在 C 或 core/std 中增加最小实现，重编 shux。
- **验收**：从「命令行 + 输入文件」到「生成可执行文件」的流程由 .x 主导；C 仅提供约定好的最小接口。

### 阶段 5：自举验证与收尾

- **目标**：用宿主编出 shux₁，用 shux₁ 编出 shux₂；两代对同一测试套件行为一致；文档/CI 记录自举通过。
- **遇阻时改 C**：若某测试暴露的是 shux 的 bug 或缺失特性，按 §三 修改 C 并重编，再验证。
...
- **本文档**：侧重**原则与边界**——  
  ...
  - **遇问题时 = 可改 C 重编 shux**，仅用于扩展能力或修 bug，不把业务逻辑写回 C。  
...
- **职责**：用 .x 实现的**构建脚本/程序**，负责  
  ...
  - 调用当前已有的 shux（或上一代 shux）编译这些 .x 为 C；  
  - 调用系统 cc 链接（并链接最小 C 运行时，若需要）；  
  - 产出新的 shux 可执行文件。
- **使用方式**：在已有 shux 的前提下，执行「shux 编译 build.x → 得到构建工具 → 运行该工具」即可完成一次编译器构建，无需执行 Makefile。
- **过渡**：在 build.x 未就绪前，仍用 Makefile 构建；build.x 就绪后，可逐步弃用 Makefile，或保留 Makefile 仅作「首次用系统 cc 编出初始 shux」的兜底。
