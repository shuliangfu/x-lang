...
> **目标**：全面实现自举，**编译器业务逻辑一律不用 C 实现**；若自举过程中因 shux 能力不足而受阻，**允许且仅允许通过修改 C 代码、重新编译 shux 来补齐能力**，再继续在 .sx 中实现自举。
...
### 1.2 遇阻时：改 C、重编 shux、再继续 .sx

- 自举过程中若出现：  
  - **语言/语法不支持**（例如 .sx 缺少某种语法或类型），或  
  - **shux 行为不符合预期**（bug、缺失特性、无法编译某段 .sx 代码），  
...
| 情况 | 可修改的 C 位置 | 目的 |
|------|------------------|------|
| **.sx 语法/语义 shux 不支持** | parser.c, lexer.c, include/ast.h, ast.c | 增加新关键字、新 AST 节点、新语法规则，使 shux 能解析并生成对应 AST |
| **类型或 IR 表示缺失** | ast.h, ast.c, typeck.c | 增加类型种类、表达式种类、块/函数表示，使 .sx 前端能表达自举所需结构 |
| **代码生成缺某种形式** | codegen.c | 增加对新 AST/类型的 C 输出（仅作「能正确输出」的占位或最小实现），使 .sx 生成的 AST 能被当前 shux 正确编成 C |
...
- **遇阻时改 C**：若某语法/类型 shux 当前不支持（如新关键字、新 AST 节点），在 parser.c/lexer.c/ast.h 等中扩展，重编 shux，再在 .sx 中使用。
- **验收**：compiler 下所有 .sx 源码能由当前 shux 编译通过（类型与语法无缺漏）。

### 阶段 3：.sx 覆盖完整代码生成

- **目标**：codegen.sx 支持所有已用到的 ExprKind、块内语句、函数/模块输出；不调用 C 的 codegen 业务接口。
- **遇阻时改 C**：若 shux 生成的 C 某处错误或缺失（如某 AST 节点未在 codegen.c 中处理），在 codegen.c 中补最小输出或修 bug，重编 shux；逻辑扩展仍以 .sx 为主。
- **验收**：用 .sx 流水线生成 compiler 自身所需 C，再经 cc 得到可执行 shux。

### 阶段 4：驱动与可选预处理迁入 .sx

- **目标**：命令行解析、读文件、调用 pipeline、调用 cc 等尽可能在 .sx 中实现；若保留 C 的 main，则只做「调 .sx 入口 + 最小 I/O」。
- **遇阻时改 C**：若 .sx 侧需要新的运行时接口（如读文件、执行子进程），在 C 或 core/std 中增加最小实现，重编 shux。
...
- **职责**：用 .sx 实现的**构建脚本/程序**，负责  
  ...
  - 调用当前已有的 shux（或上一代 shux）编译这些 .sx 为 C；  
  - 调用系统 cc 链接（并链接最小 C 运行时，若需要）；  
  - 产出新的 shux 可执行文件。
- **使用方式**：在已有 shux 的前提下，执行「shux 编译 build.sx → 得到构建工具 → 运行该工具」即可完成一次编译器构建，无需执行 Makefile。
- **过渡**：在 build.sx 未就绪前，仍用 Makefile 构建；build.sx 就绪后，可逐步弃用 Makefile，或保留 Makefile 仅作「首次用系统 cc 编出初始 shux」的兜底。

### 7.2 不要 main.c（入口在 .sx）

- **含义**：**不再用 main.c 实现**「命令行解析、读文件、调 pipeline、调 cc」等驱动逻辑；这些逻辑全部放在 .sx（例如 **driver.sx** 的 `function main(): i32`）。
- **入口从何而来**：由 shux 编译「含 main 的 .sx 模块」（如 driver.sx）时，生成的 C 中会包含 `main()`，该 `main()` 即程序入口；链接时只需再链接提供 `codegen_sx_emit_byte`、`read_file`、`invoke_cc` 等 **extern** 的最小 C 运行时（如 **runtime.c**），无需 main.c。
...
3. 实现 **build.sx**：用 .sx 描述「如何编 compiler 下所有 .sx、如何调 cc、产出 shux」；用现有 shux 编译并运行 build.sx，替代 Makefile。  
