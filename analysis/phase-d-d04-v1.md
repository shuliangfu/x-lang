...
> **D-04 v1**：同一 `.sx` 用例在 **Stage1**（`shux_asm_stage1`）与 **Stage2**（`shux_asm2`）上 **check / link+run 结果一致**——扩面 portable 子集，非全量 `run-portable-suite`。
...
- **Tier P 入口**：`examples/hello.sx`（check）
...
- 全量 `run-portable-suite.sh` / `make test_sx` 两代 diff
