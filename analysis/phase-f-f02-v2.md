...
> **F-02 v2**：Windows I/O / WSA 改走 **`std/sys/win32.sx` / `win32_net.sx` 直接 kernel32/ws2_32 FFI**（对齐 macOS `macos.sx`）；**删除** `win32.inc.c`、`win32_net.inc.c`。

## v2 完成（✅ manifest / Windows 烟测 N/A 于非 Windows 宿主）

| 项 | 说明 |
|----|------|
| `GetStdHandle` / `WriteFile` / `CreateFileA` / `ReadFile` / `ExitProcess` | `std/sys/win32.sx` extern kernel32 |
| `WSAStartup` / `WSACleanup` | `std/sys/win32_net.sx` extern ws2_32 |
