...
本文档说明编译器 SX 自举路径上 **「动态上限」** 的实现方式（**侧车 grow 池**），并列出：**已完成**、**不宜池化**、**可选 C 镜像同步** 三类项，供逐项修改与验收。
...
| **侧车 grow 池（本文简称池化）** | 把原先嵌在 struct 里的**定长数组**挪到 C 侧 `GrowVec`，用 **父对象指针作键**（如 `PipelineDepCtx*`、`DriverSxEmitState*`）挂接 sidecar；SX 通过 `pipeline_*` / `driver_emit_*` / `asm_qual_sym_*` 读写。 |
...
| P-DONE-11 ✅ | `DriverSxEmitState.lib_root_bufs[16]` | DriverEmitSidecar | `main.sx` + `driver_emit_*` |
...
| — | 标识符/字段名 | `ast.sx` Expr/Func/Type `name: u8[64]` | 64 | 语言与 C ABI 约定 |
...
| — | codegen 符号拼接 | `codegen.sx` `buf: u8[96]` | 96 | 单符号 C 名 scratch |
...
**parser.sx 中大量 `while (i < 64)`**：多数是 **拷贝到 Expr 的 u8[64] 字段** 的边界，属于语义长度，不是「第 65 个 func」问题。

---

## 五、C 路径 / 镜像同步 ✅

SX 自举与 C driver 镜像已对齐 **ast.sx 瘦布局 + ast_pool.c sidecar**。
...
**原则**：以 **ast.sx 瘦布局 + ast_pool.c** 为真源；C 镜像与 SX 共用 sidecar API（`ast_pipeline_*` / grow 池）。
...
| `DriverSxEmitState*`（`*u8`） | `-L` 库根 | `driver_emit_*` |
