...
| P-DONE-11 ✅ | `DriverSxEmitState.lib_root_bufs[16]` | DriverEmitSidecar | `main.x` + `driver_emit_*` |
...
SX 自举与 C driver 镜像已对齐 **ast.x 瘦布局 + ast_pool.c sidecar**。
...
**原则**：以 **ast.x 瘦布局 + ast_pool.c** 为真源；C 镜像与 SX 共用 sidecar API（`ast_pipeline_*` / grow 池）。
