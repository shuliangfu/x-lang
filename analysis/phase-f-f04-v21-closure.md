...
> **F-04 v21**：**std.crypto** 对外 API 全在 **`.x`**；**`crypto.c` 及全部 `.inc.c` 已删除**；仅保留文档化最小胶层 C。

## v21 闭合清单（✅ manifest）

| 模块 | 主逻辑 (.x) | 阶段 |
|------|--------------|------|
| 核心 | `core.x`（mem_eq / SHA-256 / HMAC-SHA256） | v16 |
| AES-GCM | `aes_gcm.x` | v17 |
| ChaCha20-Poly1305 | `chacha20_poly1305.x` | v18 |
| Ed25519 API | `ed25519.x` | v19 |
...
| `std/crypto/ed25519/*.c` | glue 内部实现源（v20 可迁 .x） |
...
- **F-04 v20**（可选深化）：ref10 fe/ge/sc 迁 `.x`，清零 `ed25519/*.c`
