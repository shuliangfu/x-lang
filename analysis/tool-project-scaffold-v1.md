...
| 4 | 创建项目：`./scripts/shux-new.sh myapp && cd myapp && shux run main.x` |
...
cd myapp
shux run main.x              # 编译并运行
shux build main.x -o myapp   # 仅编译
./myapp; echo $?             # 期望 42
```
...
`tests/run-tool-scaffold.sh`：复制模板到临时目录 → `shux -L <repo> main.x -o app` → 运行断言 exit 42。
