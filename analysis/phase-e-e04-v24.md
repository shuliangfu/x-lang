...
> **E-04 v24**：自 `runtime.c` 拆出 **`runtime_pipeline_abi.c/h`**，承载 import 路径到 `.sx` 文件路径的解析；`load_one_import` / dep 预读仍留 runtime.c。

## v24 完成（✅）

| 符号 | 说明 |
|------|------|
| `shux_import_path_to_file_path` | 逻辑 import → `lib_root/.../*.sx` |
| `shux_get_entry_dir` | 入口 `.sx` 所在目录 |
| `shux_import_path_is_file_path` | 是否为文件路径 import |
| `shux_resolve_file_import_path` | 相对/绝对路径 → 可读 `.sx` |
