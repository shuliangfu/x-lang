...
| 目标 | 说明 |
|------|------|
| **语法一致** | 四种 import 形式在 C 与 .x 流水线语义对齐 |
| **路径一致** | `a.b.c` → 文件路径规则全平台相同（`-L` 驱动） |
| **行为一致** | 同一 `.x` 在 Linux / macOS / Windows 输出相同 |
...
实现：`parser.x` / `parser.c` `collect_imports`；typeck/codegen 见 `X与C流水线同步状态.md`。
...
1. **扁平文件**：`<root>/<a>/<b>/<c>.x`（`.` → `/`）
2. **单段 fallback**：`<root>/<name>/<name>.x`（如 `preprocess/preprocess.x`）
3. **模块目录**：`<root>/<a>/<b>/mod.x`
4. **入口同目录**（单段）：`<entry_dir>/<name>.x`
5. **入口相对**（带点）：`<entry_dir>/<...>.x` 或 `mod.x`（含包前缀去重）
...
| 能力 | C (`parser.c`) | .x (`parser.x`) | 测试 |
|------|----------------|-------------------|------|
| 整模块 import | ✅ | ✅ | `tests/import/main.x` |
| const 绑定 | ✅ | ✅ | `const_binding.x` |
| const 解构 | ✅ | ✅ | `const_select.x`（整模块等价 body；解构 codegen 见文档） |
| 关键字路径段 | ✅ | ✅ | `tests/parser/import_std_async.x` |
| 缺失模块 | ✅ 报错 | ✅ 报错 | `missing_module.x` |
...
| `async_keyword_path` | `parser/import_std_async.x` | `async` 作路径段 |
...
2. 新增 import 语法须 C/.x 双 pipeline 同步（见 `X与C流水线同步状态.md`）
