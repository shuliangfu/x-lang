...
权威：`tests/baseline/safe-ffi-contract.tsv`（**8** 条 `case_*` + 聚合 `contract_smoke.x`）。

| case_id | 规则 | 源文件 |
|---------|------|--------|
| `case_null_cstr` | C1-null | `tests/ffi/contract_null_cstr.x` |
| `case_cstr_empty` | C2-empty | `tests/ffi/contract_cstr_empty.x` |
| `case_cstring_null0` | C3-owned | `tests/ffi/contract_cstring_null_len0.x` |
| `case_roundtrip` | C4-copy | `tests/ffi/contract_cstring_roundtrip.x` |
| `case_free_null` | C5-free | `tests/ffi/contract_cstring_free_null.x` |
| `case_owned` | C6-lifetime | `tests/ffi/contract_owned_lifetime.x` |
| `case_putchar` | C7-extern | `tests/ffi/contract_extern_putchar.x` |
| `case_smoke` | agg | `tests/ffi/contract_smoke.x` |

另：`tests/ffi/main.x` 由 `run-ffi.sh` 覆盖基础路径。
