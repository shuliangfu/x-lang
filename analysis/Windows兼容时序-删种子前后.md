...
| 模式 | 条件 | 测什么（量级） |
|------|------|----------------|
| **B-hybrid（默认）** | `SHUX_WIN_BSTRICT` 未设或 `0` | `bootstrap-driver-hybrid` → `shux_asm` + return-value **42** + win32 WriteFile/read 等 |
| **B-strict** | `SHUX_WIN_BSTRICT=1` | `bootstrap-driver-bstrict` + 同上；**仍不是** 125 全量 |
...
# SHUX_BSTRICT_SKIP_BUILD=1 ./tests/run-all-bstrict.sh
```
...
| 桶 | 示例能力 | 优先级 |
|----|----------|--------|
| **已有最小门（P0 已覆盖）** | hybrid 起 `shux_asm`、return-value 42、win32 write/read | **必达** |
| **驱动 / 输出** | `hello`、`void main`、简单 `shux run` / `-o` | P0 扩展 |
