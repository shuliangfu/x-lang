...
| 16 | `src/asm/backend_try_inline_dispatch_thin.x` | 并入 `backend_try_inline_dispatch.o` | hybrid：thin.o（**weak** **10**：6 pure + 4 forward：local_slot_off / fold_return / enc·arch local_slot）+ seed-rest（`-DSHUX_L2_TRY_INLINE_THIN_FROM_X`）`cc -r` | f-363～365 |
| 17 | `src/asm/backend_call_dispatch_thin.x` | 并入 `backend_call_dispatch.o` | hybrid：thin.o（**weak** **4** pure：reg_max / cleanup / export_c_suffix / string_lit_len）+ seed-rest（`-DSHUX_L2_CALL_DISPATCH_THIN_FROM_X`）`cc -r` | f-364～365 |
...
33. **L2**：再扩 pure / mega / 默认 PREFER 评估  
34. **asm 直出 .o**：修 CG002 后去掉 `-E` 中间步  
35. 冷启动可稳定 `-E` 后再议默认 `PREFER_X_O`
