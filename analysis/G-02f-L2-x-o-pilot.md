...
| 16 | `src/asm/backend_try_inline_dispatch_thin.x` | 并入 `backend_try_inline_dispatch.o` | hybrid：thin.o（**weak** **6** pure：align8 / vector_ko / const_binop / index_esz / struct_reserve×2）+ seed-rest（`-DSHUX_L2_TRY_INLINE_THIN_FROM_X`）`cc -r` | f-363 |
| 17 | `src/asm/backend_call_dispatch_thin.x` | 并入 `backend_call_dispatch.o` | hybrid：thin.o（**weak** **3** pure：`call_reg_max` / `stack_cleanup_bytes` / `append_export_c_suffix`）+ seed-rest（`-DSHUX_L2_CALL_DISPATCH_THIN_FROM_X`）`cc -r` | f-364 |
...
32. **L2**：try_inline/call 再扩 / mega / 默认 PREFER 评估  
33. **asm 直出 .o**：修 CG002 后去掉 `-E` 中间步  
34. 冷启动可稳定 `-E` 后再议默认 `PREFER_X_O`
