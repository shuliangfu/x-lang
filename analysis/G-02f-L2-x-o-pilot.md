...
| 8 | `src/diag_thin.x` | 并入 `diag.o` | hybrid：thin.o（**weak** **40** 门闩：…+ f-342 json + f-346 snap pure）+ seed-rest（`-DSHUX_L2_DIAG_THIN_FROM_X`）`cc -r` | f-335～346 |
...
13. ~~**diag snap pure**~~ ✅ f-346（**40**：store/load ptr/usize/i32 + le；push/restore 仍 `_impl`）  
14. **L2**：fmt / simd thin；push 改 snap orch；其它 TU 再扩  
15. **asm 直出 .o**：修 CG002 后去掉 `-E` 中间步  
16. 冷启动可稳定 `-E` 后再议默认 `PREFER_X_O`
