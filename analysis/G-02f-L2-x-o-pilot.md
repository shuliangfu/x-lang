...
| 14 | `src/asm/backend_enc_dispatch_thin.x` | 并入 `backend_enc_dispatch.o` | hybrid：thin.o（**weak** **102**：arm64 encode 5 + ta 壳 97）+ seed-rest（`-DSHUX_L2_ENC_DISPATCH_THIN_FROM_X`）`cc -r` | f-352～360 |
...
28. **L2**：更多 ta 壳 / 其它 dispatch / mega  
29. **asm 直出 .o**：修 CG002 后去掉 `-E` 中间步  
30. 冷启动可稳定 `-E` 后再议默认 `PREFER_X_O`
