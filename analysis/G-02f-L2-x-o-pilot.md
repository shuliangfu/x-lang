...
| 16 | `src/asm/backend_try_inline_dispatch_thin.x` | 并入 `backend_try_inline_dispatch.o` | hybrid：thin.o（**weak** **13**：+ `local_var_slot_holds_indirect_ptr`）+ seed-rest（`-DSHUX_L2_TRY_INLINE_THIN_FROM_X`）`cc -r` | f-363～368 |
| 17 | `src/asm/backend_call_dispatch_thin.x` | 并入 `backend_call_dispatch.o` | hybrid：thin.o（**weak** **8**：+ path 段数 forward→seed impl）+ seed-rest（`-DSHUX_L2_CALL_DISPATCH_THIN_FROM_X`）`cc -r` | f-364～368 |
...
35. **L2**：再扩 pure / mega / 默认 PREFER 评估  
36. **asm 直出 .o**：修 CG002 后去掉 `-E` 中间步  
37. 冷启动可稳定 `-E` 后再议默认 `PREFER_X_O`
