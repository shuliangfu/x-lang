...
| 8 | `src/diag_thin.x` | 并入 `diag.o` | hybrid：thin.o（**weak** **15** 门闩：…+ f-337 `push_file`/`restore`→`_impl`）+ seed-rest（`-DSHUX_L2_DIAG_THIN_FROM_X`）`cc -r` | f-335～337 |
...
rm -f src/lsp/lsp_diag_pipeline_sizes_nostub.o src/driver/target_cpu.o \
      ...
      src/runtime_io_abi.o src/diag.o
...
8. ~~**L2 第 8 TU**~~ ✅ f-335 `diag_thin` hybrid（4 pure）→ ✅ f-336 扩至 **13** 门闩  
9. **L2**：整 `diag.x` / 其它字符串重 TU；继续扩 thin（json_enabled / push_file / …）  
10. **asm 直出 .o**：修 CG002 后去掉 `-E` 中间步  
11. 冷启动可稳定 `-E` 后再议默认 `PREFER_X_O`
