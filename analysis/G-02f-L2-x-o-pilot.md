...
| 14 | `src/asm/backend_enc_dispatch_thin.x` | 并入 `backend_enc_dispatch.o` | hybrid：thin.o（**weak** **3** arm64 SP/imm encode→append）+ seed-rest（`-DSHUX_L2_ENC_DISPATCH_THIN_FROM_X`）`cc -r` | f-352 |
...
20. **L2**：enc 再扩 / 其它 dispatch / mega  
21. **asm 直出 .o**：修 CG002 后去掉 `-E` 中间步  
22. 冷启动可稳定 `-E` 后再议默认 `PREFER_X_O`
