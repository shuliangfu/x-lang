...
| 18 | `src/lsp/lsp_fmt_pure_thin.x` | 并入 `lsp_diag.o` | hybrid：thin.o（**weak** **19**：`is_atom_tail`/`is_atom_head`/`unary_lhs`/`last_out`/`prev_src`/`src_ws_before`/`src_ws_after`/`space_before`/`space_after`/`json_escape_ident`/`col_in_ident_span`/`lsp_find_key_after`/`lsp_parse_bool_after`/`lsp_line_has_block_comment_end`/`lsp_line_is_block_comment`/`lsp_load_i32_at`/`lsp_load_ptr_at`/`func_name_covers`/`lsp_hash_source`）+ seed-rest（`-DSHUX_L2_LSP_FMT_THIN_FROM_X`）`cc -r` | f-423/424/425/426/427 |
...
- **f-433 rt_asm_stub.x thin+rest PREFER_X_O 接入**：seed C `rt_asm_stub.from_x.c` 加 `#ifndef SHUX_RT_ASM_STUB_FROM_X` 包裹 `driver_asm_output_want_exe`（.x 已真迁 1/3 函数，转调 `shux_output_want_exe`）；`asm_codegen_ast` 保留 seed C（字符串字面量数组 + memcpy，.x 不支持）；`g05_ensure_relink_prereqs.sh` rt_asm 编译块改 thin+rest 模式；macOS `shux -E` 1 函数 / thin 1 符号 + rest 2 符号 / merged 3 符号全 T 无重复
- **f-434 rt_run_exec.x .x真迁 + thin+rest PREFER_X_O 接入**：重写 `driver_exec_path_is_non_exe` 为 flat early-return + 3 helper 函数（`rt_exec_suffix2_non_exe`/`suffix4_non_exe`/`check_non_exe`），消除 `&&`/`||`/嵌套 if/unsafe 内 let+if → 修复 `shux -E` 丢弃函数体（原版只剩 `init_globals`）；补全 `.obj` 4 字节后缀检查（原 .x 遗漏）；seed C 加 `#ifndef SHUX_RT_RUN_EXEC_FROM_X` 包裹；`g05_ensure_relink_prereqs.sh` rt_run 编译块改 thin+rest 模式；macOS `shux -E` 4 函数 / thin 4 符号 + rest 8 符号 / merged 12 符号全 T 无重复
- **f-435 rt_util.x thin+rest PREFER_X_O 接入**：移除 .x 中被 `"" as *u8` 字符串字面量阻塞的 `driver_argv0_basename_is`（暂留 seed C）；保留 `driver_unlink_failed_output`（1/2 函数真迁，`out_path[0 as usize] == 0 as u8` + `unsafe { unlink(out_path); }`）；seed C `rt_util.from_x.c` 加 `#ifndef SHUX_RT_UTIL_FROM_X` 包裹；`g05_ensure_relink_prereqs.sh` rt_util 编译块改 thin+rest 模式；macOS `shux -E` 1 函数 / thin 1 符号 + rest 1 符号 / merged 2 符号全 T 无重复
- **f-436 rt_content.x -E 超时根因修复 + thin+rest PREFER_X_O 接入**：定位 `n > 2147483647`（usize 与大字面量比较）导致 `shux -E` 指数级分析（60s→0.02s）；改用 `ni = n as i32; if (ni < 0)` 溢出回绕检测；消除 4 处 `&&` → 4 个 flat helper（`rt_is_block_comment_end`/`string_escape`/`line_comment_start`/`block_comment_start`）；state 变量替代 3 flag 降嵌套 7→4 层；移除 .x 中 `driver_source_has_*` stub（需 `driver_peek_source_file` + 65KB 栈缓冲）；seed C 加 `#ifndef SHUX_RT_CONTENT_FROM_X` 包裹 2 函数；`g05_ensure_relink_prereqs.sh` rt_content 编译块改 thin+rest 模式；macOS `shux -E` 0.02s 12 函数 / thin 12 符号 + rest 3 符号 / merged 15 符号全 T 无重复
- **f-437 PREFER_X_O 默认化**：`g05_ensure_relink_prereqs.sh` 中 29 处 `SHUX_G05_PREFER_X_O:-0` → `:-1`（默认优先 .x→-E→.o，失败回退 seed C）；修复 `g05_try_x_to_o` sed 列表遗漏 `unlink`/`strlen` extern（与 `<unistd.h>`/`<string.h>` 冲突导致 rt_util.x/rt_run_exec.x 回退 seed）；23/24 .x 文件成功产出 .o（仅 lsp_fmt_pure_thin.x 因预存变量作用域问题回退 seed）；macOS 默认构建 rv42=42 ✅；全量 PREFER 构建 4.98s（24 .x 文件）
