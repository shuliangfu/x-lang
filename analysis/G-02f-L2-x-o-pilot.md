...
| 10 | `src/runtime_driver_abi_thin.x` | 并入 `runtime_driver_abi.o` | hybrid：thin.o（**weak** **28** flag pure + check_ok/now_sec/large_stack + timing 全套）+ seed-rest（`-DSHUX_L2_RDABI_THIN_FROM_X`）`cc -r` | f-343～344 |
...
12. **L2**：diag snap / fmt / simd thin；其它 TU 再扩  
13. **asm 直出 .o**：修 CG002 后去掉 `-E` 中间步  
14. 冷启动可稳定 `-E` 后再议默认 `PREFER_X_O`
