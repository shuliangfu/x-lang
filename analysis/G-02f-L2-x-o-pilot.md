...
| 9 | `src/runtime_driver_diagnostic_thin.x` | 并入 `runtime_driver_diagnostic.o` | hybrid：thin.o（**weak** **28** `_impl` 门闩）+ seed-rest（`-DSHUX_L2_RDD_THIN_FROM_X`）`cc -r` | f-339 |
...
8. ~~**L2 第 8 TU**~~ ✅ f-335～338 `diag_thin` hybrid（**26** 门闩；重路径 C 尾 `_impl`）  
9. ~~**L2 第 9 TU**~~ ✅ f-339 `runtime_driver_diagnostic_thin` hybrid（**28** 门闩）  
10. **L2**：整 diag/rdd .x 收口 / 其它字符串重 TU / 新 L2 TU  
