...
| 18 | `src/lsp/lsp_fmt_pure_thin.x` | 并入 `lsp_diag.o` | hybrid：thin.o（**weak** **18**：`is_atom_tail`/`is_atom_head`/`unary_lhs`/`last_out`/`prev_src`/`src_ws_before`/`src_ws_after`/`space_before`/`space_after`/`json_escape_ident`/`col_in_ident_span`/`lsp_find_key_after`/`lsp_parse_bool_after`/`lsp_line_has_block_comment_end`/`lsp_line_is_block_comment`/`lsp_load_i32_at`/`lsp_load_ptr_at`/`func_name_covers`）+ seed-rest（`-DSHUX_L2_LSP_FMT_THIN_FROM_X`）`cc -r` | f-423/424/425/426 |
