...
| 13 | `src/driver/fmt_check_cmd_thin.x` | 并入 `fmt_check_cmd_driver.o` | hybrid：thin.o（**weak** **3** pure：quiet_ok / skip_dot / fallback_diag）+ seed-rest（`-DSHUX_L2_FMT_CHECK_THIN_FROM_X` + `SHUX_USE_X_PIPELINE`）`cc -r` | f-350 |
...
18. **L2**：fmt 再扩 / 其它 TU 再扩  
19. **asm 直出 .o**：修 CG002 后去掉 `-E` 中间步  
20. 冷启动可稳定 `-E` 后再议默认 `PREFER_X_O`
