...
本文档说明编译器 SU 自举路径上 **「动态上限」** 的实现方式（**侧车 grow 池**），并列出：**已完成**、**不宜池化**、**可选 C 镜像同步** 三类项，供逐项修改与验收。

**关联实现**：`compiler/ast_pool.c`（grow 向量 + sidecar 键）、`compiler/pipeline_glue.c`（`#include "ast_pool.c"` 参与链接）、`compiler/src/ast/ast.sx`（瘦 struct + `pipeline_*` extern）。
...
| **侧车 grow 池（本文简称池化）** | 把原先嵌在 struct 里的**定长数组**挪到 C 侧 `GrowVec`，用 **父对象指针作键**（如 `PipelineDepCtx*`、`DriverSuEmitState*`）挂接 sidecar；SU 通过 `pipeline_*` / `driver_emit_*` / `asm_qual_sym_*` 读写。 |
...
| 标识符 [64]u8 等 | 池化几乎无收益，还会让每次读名更慢 → **不建议**。 |
...
### 3.1 AST / Module 持久数据（ast_pool + ast.sx）
...
| ID | 原限制 | 现实现 | 主要文件 |
|----|--------|--------|----------|
| P-DONE-07 ✅ | OneFuncResult 内 param_names/type_refs/call_args | OneFuncSidecar | `parser.sx` + `pipeline_onefunc_*` |
| P-DONE-08 ✅ | ExternParseResult 同上 | 共用 OneFuncSidecar | `parser.sx` |
| P-DONE-09 ✅ | `current_func_empty_param_indices[16]` | DepCtxSidecar | `codegen.sx` + `pipeline_dep_ctx_empty_param_*` |
| P-DONE-10 ✅ | DepCtx lib_root / dep 槽 | DepCtxSidecar | `pipeline_dep_ctx_*`、`ast_pipeline_ctx_append_lib_root` |
| P-DONE-11 ✅ | `DriverSuEmitState.lib_root_bufs[16]` | DriverEmitSidecar | `main.sx` + `driver_emit_*` |
| P-DONE-12 ✅ | PipelineDepCtx 布局与 LSP sizeof | 去掉 `[16]i32` | `lsp_diag_pipeline_sizes.c` |

### 3.3 下游 pass 已去硬顶 / 池化

| ID | 原限制 | 现实现 | 文件 |
|----|--------|--------|------|
| P-DONE-13 ✅ | typeck expr_stmt 扫描 `idx < 32` | 仅 `idx < nes` | `typeck.sx` |
| P-DONE-14 ✅ | backend stmt_order `i < 96` | 仅 `i < ast_block_num_stmt_order(...)` | `backend.sx` |
| P-DONE-15 ✅ | backend/typeck import 限定符号层 `[16][64]` | 全局 scratch 池 | `asm_qual_sym_layer_*`（`ast_pool.c` + `backend.sx` / `typeck.sx`） |
| P-DONE-16 ✅ | preprocess `#if` 嵌套 `stack[32]` | grow 池 | `preprocess_if_stack_*`（`ast_pool.c` + `preprocess.sx`） |
| P-DONE-17 ✅ | `AsmFuncCtx.locals[24]` | sidecar 池 | `asm_ctx_local_*`（`ast_pool.c` + `backend.sx`） |
| P-DONE-18 ✅ | loop 标签栈 4 层 | 8 层（512 字节栈） | `backend.sx` |

### 3.4 阶段 A — 去循环硬顶 ✅

| ID | 文件 | 改法 | 状态 |
|----|------|------|------|
| P-A01 ✅ | `codegen.sx` | 去掉 `&& fi < 256` | 完成 |
| P-A02 ✅ | `backend.sx` | 去掉 `&& i < 256` | 完成 |
| P-A03 ✅ | `asm/arch/arm64.sx` | 去掉 `&& i < 256` | 完成 |
| P-A04 ✅ | `typeck.sx` | import 层 → `asm_qual_sym_layer_*` | 完成 |
| P-A05 ✅ | `backend.sx` | `imp_slot < module.num_imports` | 完成 |
| P-A06 ✅ | `typeck.sx` | struct_lit / import select 去 8 顶 | 完成 |
| P-A07 ✅ | `codegen.sx` | import select 去 `k < 8` | 完成 |

### 3.5 阶段 B — 新增 sidecar / scratch ✅

| ID | 文件 | 原限制 | 现实现 | 状态 |
|----|------|--------|--------|------|
| P-B01 ✅ | `parser.sx` | `arg_i < 64` | 仅 `arg_i < call_num_args` | 完成 |
| P-B02 ✅ | `parser.sx` | `ii < 32` callee 拷贝 | 仅 `ii < call_callee_len` | 完成 |
| P-B03 ✅ | `preprocess.sx` | `#if depth >= 32` | `preprocess_if_stack_*` grow 池 | 完成 |
| P-B04 ✅ | `backend.sx` | `locals[24]` | `asm_ctx_local_*` sidecar | 完成 |
| P-B05 ✅ | `backend.sx` | loop 栈 4 层 | 8 层 | 完成 |
...
| ID | 类别 | 典型位置 | 上限 | 原因 |
|----|------|----------|------|------|
| P-B06 🚫 | 调用约定 reg 临时 | `backend.sx` / `x86_64.sx` / `arm64.sx` 中 `k < 8` | 8 | **ABI** 最大通用寄存器实参数；非 AST 列表 |
| — | 标识符/字段名 | `ast.sx` Expr/Func/Type `name: [64]u8` | 64 | 语言与 C ABI 约定 |
| — | 形参名 | `Param.name: [32]u8` | 32 | 与 codegen `_pN`、C 声明一致 |
| — | goto/label | `Block.label/goto_target: [32]u8` | 32 | 标签名长度 |
| — | 路径 scratch | `main.sx` path `[512]`、`-L` copy `[256]` | 512/256 | OS 路径惯例 |
| — | 源码/预处理缓冲 | `PipelineDepCtx.loaded/preprocess_buf` | 4MiB | 文件大小顶，非 grow 列表 |
| — | codegen 符号拼接 | `codegen.sx` `buf: [96]u8` | 96 | 单符号 C 名 scratch |
| — | asm 指令行 | `x86_64.sx` / `arm64.sx` 等字面量 | 16–32 | 汇编文本模板 |
| — | Mach-O/ELF/COFF | `platform/*.sx` 文件头字节数组 | 固定布局 | 对象文件格式 |
| — | typeck 反射 | `ensure_array_type_ref_named_elem(..., 256)` | 256 | 类型描述常量，非 ast 池 |
| — | LSP I/O | `lsp.sx` line_buf `[256]` | 256 | 协议行缓冲 |

**parser.sx 中大量 `while (i < 64)`**：多数是 **拷贝到 Expr 的 [64]u8 字段** 的边界，属于语义长度，不是「第 65 个 func」问题。

---

## 五、C 路径 / 镜像同步 ✅

SU 自举与 C driver 镜像已对齐 **ast.sx 瘦布局 + ast_pool.c sidecar**。
...
**原则**：以 **ast.sx 瘦布局 + ast_pool.c** 为真源；C 镜像与 SU 共用 sidecar API（`ast_pipeline_*` / grow 池）。
...
| `DriverSuEmitState*`（`*u8`） | `-L` 库根 | `driver_emit_*` |
...
| 变更类型 | 同步更新 |
|----------|----------|
| 修改 `ast.sx` 中 PipelineDepCtx / Module / ASTArena 布局 | `lsp_diag_pipeline_sizes.c` |
