...
| **L5-fmt-style** | info | 未格式化 `.sx` | `driver_run_fmt` `--check`（TOOL-001） |
...
| case_id | 文件 | 期望 |
|---------|------|------|
| `case_clean` | `tests/lint/lint_clean_ok.sx` | `check` 静默 exit 0 |
| `case_error` | `tests/lint/lint_error_assign.sx` | `check` exit 1 + ` - error: ` |
| `case_warn_pad` | `tests/lint/lint_warn_pad.sx` | `SHUX_PAD_FIELDS=1 check` 含 `warning: -pad-fields`，exit 0 |
| `case_warn_reorder` | `tests/lint/lint_warn_reorder.sx` | `SHUX_HOT_REORDER=1 check` 含 `warning: -hot-reorder`，exit 0 |
| `case_info_unused` | `tests/lint/lint_unused_hint.sx` | `SHUX_UNUSED_HINT=1 check` 含 `unused binding`，exit 0 |
