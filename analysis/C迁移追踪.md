...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w352 read_file_x_view PREFER**（class B FileView）；残 GrowVec-LE／import_heap／Ubuntu check_expr ABI | 下一＝GrowVec-LE／import_heap／Ubuntu check_expr。禁盲翻；禁 Darwin mega `-E`（内存）。|
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w351 Cap A 双端**；下一＝var-module／GrowVec-LE／Ubuntu check_expr PREFER；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。
