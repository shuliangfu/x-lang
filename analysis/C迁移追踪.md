...
| leftover catalog no-Honesty portable-c／portable-suite soft→硬绿 | ✅ | 二过：拒 leftover catalog 无 Honesty＋leftover auto-make（`xlang_compiler_make all`）＋leftover prefer-c（`XLANG=xlang-c`＋`RUN_ALL_USE_C`＋无界 `run-all.sh`）＋leftover SKIP→OK（显式坏 XLANG 仍 OK）；G.7 补全 `dod_native_exe`；nested leftover `run-all.sh` skip=1；显式坏 XLANG 硬 die；keep `run-portable-c OK`／`run-portable-suite OK`；nested leftover runner 不重写；leftover prefer-c `XLANG_LINK_XLANG=xlang-c` 仍 leave（爆炸 ~336 nested）；archive leftover residual（无新建 Gate）；报告 `run=`／`obs=`／`skip=`；双端 L2；**leftover catalog no-Honesty portable-c／portable-suite FAIL 池空**；**leftover catalog no-Honesty tool-lsp-diag-perf 已收**；余 leftover prefer-c XLANG_LINK_XLANG portable-suite 仍 leave；余 leftover fossil DOC live 默认仍 `自举前必须清单.md` 四闸（bootstrap-min leave）；win32-*／F-* leftover XLANG fallthrough 仍 leave |
