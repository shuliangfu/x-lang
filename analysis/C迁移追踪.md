...
✅ **i32x4 `/` 后 lane AAPCS64 4B LDRSW** ✅ @ **`4ac03565b`**
  ...
  - 余（勿并本叶）：METHOD binop2 仍 CALL-only（本地 CALL `add(a,b)` 已 0）；nest>16 soft；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；INDEX 负 i32 `ldr w0` 零扩展比 64 位 −5（消费端比宽，非本刀 load）
