...
| STD soft SKIP 邻域续（其它闸） | 🟡 | prefer-c honesty 续（摸底 asm 已绿可续：STD-136 datetime-iana／STD-106 log-rotate-async／STD-092 net-context；aes-gcm nist2 RUN≠0 后置；compress-brotli ld 红跳）；仍跳过产品红／UNDEF（compress-stream／unified-stream ld／async-language／process-pipe XT001／mem-boundary UNDEF／encoding-hex-b64／xxhash／json-zc／unicode-nfc／map-vec／channel-unbounded／http-reqresp／async-context／mac／mac512／codec／atomic-widen／csv-stream／metrics／math-special／elf-write 等）。另残 asm ld `--export-dynamic`（backtrace 具名；非软）＋by-value `Set_i32`+MEMORY 邻参 ABI（非软） |
