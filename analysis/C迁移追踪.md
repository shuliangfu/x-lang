...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋w220–224 C | 含 wave224 typeck_active C thin（BSS 微胞收）。下一＝wave261 glue_statics 桥。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc…w219 `.x`；w220–224 **C thin** 具名 BSS（微胞收）。下一叶＝wave261 glue_statics 桥；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
