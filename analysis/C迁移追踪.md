...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋w220–291 C＋w292–w305 部分 `.x`（w268=.x） | w292 elf_fwd 全 `.x`；w293 typeck_orch shim＋layout C；w294–w305 `-E+$CC`（…／asm_locals／top_level_let）。下一＝wave306 C→.x。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc…w219 `.x`；w220–291 **C thin**＋w292–w305 部分 **C→.x**（w268＝既有 `.x` thin）。下一叶＝wave306 Cap residual C→.x；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
