...
| lexer-bounds／link-hardening soft prefer-c →硬绿 | ✅ | prefer asm＋`XLANG_LINK_XLANG`；退役默认 xlang-c＋soft auto-make＋check 绑闸（lexer）／Darwin soft SKIP 无 `run=`／`skip=`；显式坏 XLANG／缺 native 硬 die；long-ident／long-num 产品 `-o` exit0 硬绿；Linux PIE+NX＋exit42 硬绿／非 Linux skip；报告 `run=`／`obs=`／`skip=`；双端 L2；**…／signed＋layout-ovf／lexer-bounds＋link-hardening soft FAIL 池空**；zc3／zc4／zc5 host-c 后置 |
