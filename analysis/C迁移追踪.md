...
✅ **dest extra-arm extra wrap `{ { let t; dest } }` host-C**（asm leftover 偶绿；host-C `void` 赋结构体）
  ...
  - 余（勿并本叶）：dest-from-region dest-region-body defer（asm leftover 53）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off
