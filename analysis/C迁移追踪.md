...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋w220–224／w261–w267／w269–w274 C（w268=.x） | 含 wave274 asm WPO 域 C thin（Darwin L2＠7315416；Ubuntu 待）。下一＝wave275 sidecar 池管理域。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc…w219 `.x`；w220–224／w261–w267／w269–w274 **C thin**（w268＝既有 `.x` thin）。下一叶＝wave275 sidecar 池管理域；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
