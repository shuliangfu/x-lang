...
| STD soft SKIP 邻域续（其它闸） | 🟡 | STD-045 闭后仍有同型假权威：net-dns／thread-pool／io-context／backtrace-symbolicate 等 prefer-c 已探针 asm 绿；channel-unbounded／http-reqresp／async-context 仍 UNDEF／产品红；mac512 产品 UNDEF 非软。另残 by-value `Set_i32`+MEMORY 邻参 ABI（非软） |
