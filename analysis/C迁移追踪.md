...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w338 COMMON**；**w339 typeck_active** LINUX PREFER／DARWIN `-E`；**w335／w337 ban** 余 A/B/C | Cap A 首叶翻（金标 PREFER）。下一＝emit_ctx_* 或 GrowVec-LE。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w339 Cap A typeck_active**（LINUX PREFER／DARWIN `-E`；w338 COMMON）。下一＝emit_ctx_* 或 GrowVec-LE；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
