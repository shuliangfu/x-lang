...
| `pipeline_typeck_coerce_init.c` | **~916（thin）** | typeck coerce-init 域 Cap residual thin（wave229 ret_coerce／float_widen + wave227） | 🟡 **G.7 live coerce＝typeck.x**（`typeck_coerce_init_*`＋ret_coerce／float_widen）；residual 仅 int_binop_c／struct_lit_c + type_refs／integer_widen + int_lit + thin faces；仍 host-cc 入 `pipeline_x` |
