...
| Mega 去 pin M4（阶段 7） | 🟡 | 冷链关 pin；**7.2.1b** 200/1956；parser seed 物理删 ⬜ |
...
- 🟡 **7.2.1b** parser_asm suite audit B-minus — **221／1,956**  
  - [x] ABI＝B-minus（opaque＋lexer-step 桥；RFC §5）  
  - [x] 桥面／P9a／三契约／生成器 v1→v5.5／等价 harness  
  ...
  - [x] function_advance 展开（v5.5：诚实＋9；if_advance／混用原游标仍拒）  
...
2. 🟡 **7.2.1b** B-minus 续（221／1,956；if advance／after_imports／sel_lex／厚 buf；v5.5 function_advance 已收）＋ **BC + 8.3**  
