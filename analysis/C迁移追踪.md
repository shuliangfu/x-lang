...
| **终局：无 Makefile + 零 cc + v2==v3** | 🟡 | MG ✅ · BC 🟡 · PC 🟡（去 import→C／FORBID／ALLOW ✅；Darwin 产品 `-o` ld-only ✅；Linux hosted gcc-as-ld ⬜）；见 §0.1 三义；阶段 13 |
...
✅ **extra-.o has_obj 去重（invoke_cc／asm ld／sat mega）** ✅ @ **`f7424ae47`**
  ...
  - 残：sat 路由 **已开**（`try-r1` → `ensure_labi_prefer_one`；sat 形态 **427280** Darwin **5/5**）。mega **318600** 活。L0 `-E` `getcwd` **已闭**。emit_header `h[88]` 超额初始化 **已闭**（`u8[83]`）。dest extra-arm 末尾 `{ fields }` host-C **已闭**。dest extra-arm `unsafe {…}; dest` ASI **已闭**。dest extra-arm `region {…}; dest` ASI **已闭**。parse_into 函数体 `region {}; return` 同模式 ASI **已闭**。`push_stable` prefer 已调权威 `has_obj`。
...
🟡 **12.2.3 产品默认后端**（2026-08-12 · 去 import→C + seed APPLE 收敛后）
  ...
  - 下一刀候选：①Linux hosted gcc-as-ld（crt／pie；须点名）· ②sat 盖 prefer（须点名＋双端 L4）· ③pipeline_abi mega 仍硬禁 · ④ARM64 prologue 未存 x19（须点名）· ⑤ALLOW ensure 再瘦／Stage12 COMPILE residual 其它叶
