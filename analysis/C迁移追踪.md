...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-04（对照 tip residual：glue ~3.4k／ast_pool ~0.18k host 壳 + emit-heavy env／backend wrapper／scratch 已抽／ELF write + ELF ctx + type-to-c + skip/force + struct emit + residual + asm locals + slot bytes + block tree + ctx loop + WPO + selfhost + thin_delegate（五表全） + safe_helper（skip_heavy 全） + parser_emit_heavy 域抽出；**只改勾选与事实 LOC**，无波次流水）  
...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3 **进行中**（**2026-08-04 实测**）：`pipeline_glue.c` **~3.4k**（静态叶基本 fold 完，文件多为 domain `#include` + 薄 wrapper）；`ast_pool.c` **~0.18k**（几乎纯 `#include` 编排；域 thin 含 typedefs/ptr_at/wrapper/scratch/**emit-heavy env wave1280**） + **emit-heavy env → pipeline_asm_emit_heavy_env.c ~313** + **backend wrapper → pipeline_backend_asm_wrapper.c ~96** + **scratch → pipeline_scratch_bufs.c ~53** + **ELF write → pipeline_elf_write_o.c ~1.6k** + **ELF ctx → pipeline_elf_ctx.c ~1k** + **type-to-c → pipeline_codegen_type_to_c.c ~350** + **skip/force → pipeline_codegen_skip_force.c ~393** + **struct emit → pipeline_codegen_struct_emit.c ~243** + **residual → pipeline_codegen_residual.c ~188** + **asm locals → pipeline_asm_locals.c ~296** + **slot bytes → pipeline_asm_slot_bytes.c ~462** + **block tree → pipeline_asm_block_tree.c ~272** + **ctx loop → pipeline_asm_ctx_loop.c ~189** + **WPO → pipeline_asm_wpo.c ~1.5k** + **selfhost → pipeline_asm_selfhost.c ~280（9 谓词全）** + **thin_delegate → pipeline_asm_thin_delegate.c ~320（五表全）** + **safe_helper → pipeline_asm_emit_heavy_safe_helper.c ~674（skip_heavy 全）** + **parser_emit_heavy → pipeline_asm_parser_emit_heavy.c ~556** + **diag → pipeline_asm_diag.c ~96** + **skip_dispatch → pipeline_asm_skip_dispatch.c ~315（asm_empty_text_stub_label + asm_skip_heavy_module_func_body）** + **resolve_path → pipeline_resolve_path.c ~376（import 路径解析 + CodegenOutBuf 访问器 + fs extern）** + **emit_sidecar → pipeline_emit_sidecar.c ~203（driver_emit_lib_root + asm_qual_sym_layer）** + **preprocess_if → pipeline_preprocess_if.c ~74（#if 嵌套栈 cold fallback）** + **typeck_slots → pipeline_typeck_slots.c ~354（scratch64 + call_resolve/overload 槽 + integer_widen + binop_arith_infer + struct_layout_metrics）** + **codegen_dep → pipeline_codegen_dep.c ~504（run_x_pipeline_codegen_* dep 编排 + debug heap trace + dep 路径填充）** + **lsp_diag → pipeline_lsp_diag.c ~79（LSP 诊断 C glue + 256MiB pthread 深栈 typeck）** + **import_bind → pipeline_import_bind.c ~138（fs read + import bind/sync XLANG_WEAK）** + **parse_typeck_dispatch → pipeline_parse_typeck_dispatch.c ~465（parse scalars + typeck 分派 + import load）**；asm_ctx_fill_locals_block_tree 已折入 block_tree；run_x_pipeline 核心 · pipeline_loop_should_continue_* · 早期 struct 声明区仍 core）。**8.3.1 域 thin 子项大多 ✅**；**8.3.2 域 thin + write 域 ✅ 子项**；**8.3.3 field_access／soa 已抽出仍 host-cc 🟡**；**8.3.9 ✅**；**8.3.4–8.3.8／8.3.10 ⬜**。**BC 终局（离 host-cc）仍 ⬜** |
...
| `ast_pool.c` | **~179** | AST 池 host 壳（几乎纯 #include 编排） | 🟡 域 thin 基本收口（含 emit-heavy env）；仍 host-cc |
| `pipeline_asm_emit_heavy_env.c` | ~313 | EMIT_HEAVY 阈值／env／path／whitelist／prefix | 🟡 已抽出（wave1280）；仍 host-cc 入 `pipeline_x` |
| `pipeline_backend_asm_wrapper.c` | ~96 | M8-tail `asm_codegen_ast`／`to_elf` 薄包装 | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
| `pipeline_scratch_bufs.c` | ~53 | codegen path/prefix scratch 缓冲池 | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
...
| `pipeline_asm_selfhost.c` | **~280** | asm module self-host classification（num_defined_funcs/defined_func_ordinal + is_backend/typeck/pipeline/main_driver/driver_compile/parser/parser_emit_heavy/ast/compiler · 9 谓词全） | 🟡 **已抽出**（8.3.2）；is_ast/is_compiler 谓词后补入；仍 host-cc 入 `pipeline_x` |
| `pipeline_asm_thin_delegate.c` | **~320** | asm M8-tail thin delegate 查表（AsmBackendThinDelegateRow + k_asm_backend／k_asm_pipeline／k_asm_parser／k_asm_driver／k_asm_typeck_thin_delegate + backend／pipeline／driver／typeck_m8_tail_thin_delegate_c_name · 五表全） | 🟡 **已抽出**（8.3.2）；k_asm_parser 表迁出、asm_parser_m8_tail_thin_delegate_c_name 仍留 ast_pool parser_emit_heavy 域；driver/typeck 表后补入；仍 host-cc 入 `pipeline_x` |
| `pipeline_asm_emit_heavy_safe_helper.c` | **~674** | asm EMIT_HEAVY 第二遍 per-module 分类器（typeck／pipeline／driver_compile_safe_helper + skip_heavy_backend_m8/helper_keep + skip_heavy_backend_m8_tail_thin_keep／typeck_helper_keep／backend_mega_entry／typeck_mega_entry · 9 fn · skip_heavy 全集） | 🟡 **已抽出**（8.3.2）；共用 static pipeline_module_func_name_has_prefix_at 在 emit_heavy_env 域；4 skip_heavy 入口分类器后补入；仍 host-cc 入 `pipeline_x` |
| `pipeline_asm_parser_emit_heavy.c` | **~556** | asm parser EMIT_HEAVY 域（slot/bisect/debug + mega_skip + force_stub + safe_helper + x_body_keep + slot_fallback_ok + func_is_thin_delegate + m8_tail delegate + resolve_call_to_glue + callee_is_same_module_local · 13 fn） | 🟡 **已抽出**（8.3.2）；k_asm_parser 表经 thin_delegate include 可见；仍 host-cc 入 `pipeline_x` |
| `pipeline_asm_diag.c` | **~96** | asm 诊断域（asm_diag_start_func_skip + trace_func_body／body_ref／emit_phase／func_idx + trace_func wrapper · 6 fn） | 🟡 **已抽出**（8.3.2）；自 ast_pool 两处合抽（尾部 5 fn + Block B wrapper）；全公共符号；仍 host-cc 入 `pipeline_x` |
| `pipeline_asm_skip_dispatch.c` | **~315** | asm 桩化/skip dispatch 域（asm_empty_text_stub_label 空桩 FNV-1a 标签 + asm_skip_heavy_module_func_body 中央 dispatch · 2 fn） | 🟡 **已抽出**（8.3.2）；37 callee 均先于 include；调用方经 glue 前向声明；仍 host-cc 入 `pipeline_x` |
| `pipeline_resolve_path.c` | **~376** | import 路径解析/fs 读 glue 域（path_append_256/512 + resolve_path probe/flat_import + resolve_path_x dispatch + CodegenOutBuf len/set + std_fs extern · ~16 fn） | 🟡 **已抽出**（8.3.2）；依赖 dep_ctx 访问器（先于 include）；resolve_path_x/copy_lib_root 经 extern/前向声明；仍 host-cc 入 `pipeline_x` |
| `pipeline_emit_sidecar.c` | **~203** | emit sidecar 状态域（driver_emit_lib_root_reset/release/append/count/len/copy + AsmQualSymScratch + asm_qual_sym_layer_reset/push/count/len/copy） | 🟡 **已抽出**（8.3.2）；DriverEmitSidecar typedef 在 ast 早期 struct 区；仍 host-cc 入 `pipeline_x` |
| `pipeline_preprocess_if.c` | **~74** | preprocess #if/#else 嵌套栈 cold fallback（preprocess_if_stack_ensure/push/at/set_at · GrowVec + XLANG_WEAK） | 🟡 **已抽出**（8.3.2）；product pure 走 runtime_pipeline_abi 固定 i32[32]；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_slots.c` | **~354** | typeck scratch/推理/布局 slot 域（named/scratch64 + call_resolve/overload 槽 + integer_widen_ok + binop_arith_infer_type_c + struct_layout_metrics_try_packed + layout_metrics 槽） | 🟡 **已抽出**（8.3.2）；公共符号 + static 槽；仍 host-cc 入 `pipeline_x` |
...
| `pipeline_lsp_diag.c` | **~79** | LSP 诊断 C glue 域（lsp_diag_typeck_after_load + parse_entry_buf + parse_typeck_buf／_impl／_thread_fn + LspDiagParseTypeckArgs） | 🟡 **已抽出**（8.3.2）；256MiB pthread 深栈 typeck；driver_run_on_large_stack_pthread 本块内 extern；仍 host-cc 入 `pipeline_x` |
| `pipeline_import_bind.c` | **~138** | fs 读 + import bind/sync 域（read_file_x_impl_c／_c／read_fd_into_loaded_buf + preprocess_loaded_into_ctx + bind_import_dep_buffers + try_bind_seeded_import + sync_one_dep_slot · XLANG_WEAK cold twin） | 🟡 **已抽出**（8.3.2）；依赖 dep_ctx + driver_dep + xlang_read_file_into_path（先于 include）；仍 host-cc 入 `pipeline_x` |
...
| `pipeline_run_x_pipeline.c` | **~118** | run_x_pipeline 核心编排骨域（typecheck_entry_c + g_run_x_pipeline_last_rc/get/store + typeck_fail/null_return + load_deps_after_parse + typecheck_after_load + parse_entry_do_parse + typecheck_entry_emit） | 🟡 **已抽出**（8.3.2）；依赖 typeck/parse/load_and_sync（parse_typeck_dispatch，先于 include）；无 codegen_dep 前向引用；仍 host-cc 入 `pipeline_x` |
| `pipeline_loop_glue.c` | **~60** | 有界循环谓词 + dep prepare glue 域（loop_should_continue_ndep/imports/lib_root + index_at_or_beyond_* + load_and_sync_set_ndep + codegen_one_dep_prepare） | 🟡 **已抽出**（8.3.2）；依赖 dep_ctx_ndep + prepare_dep_codegen_path_c（codegen_dep，先于 include）；仍 host-cc 入 `pipeline_x` |
...
| `pipeline_lint_meta.c` | **~364** | pipeline lint + module metadata glue 域（XLANG_VISIBILITY + L7 unused-private-funcs lint + module_num_funcs／main_func_index／reset_parse_counters／strict_parse_into_init） | 🟡 **已抽出**（8.3.2）；藏于 module_func.c／block.c include 间隙；依赖 lifecycle reset（先于 include）；仍 host-cc 入 `pipeline_x` |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - ✅ codegen residual 域 thin：`pipeline_codegen_residual.c`（use_buf_wrapper + skip_emit_extern_io_batch_buf + should_skip_emit_func_by_name + emit_seed_mega_enabled + is_submit_batch_buf_call + should_skip_emit_func_core_read_ptr + asm_io_core_extern_callee_sym + io_driver_buf_call_sym + std_io_fixed_fd_emit_impl · ~172 LOC body）；g05 STALE／`PIPELINE_X_DEPS` 已收（wave1251）
  ...
  - ✅ asm EMIT_HEAVY env／path／whitelist 域 thin：`pipeline_asm_emit_heavy_env.c`（ASM_EMIT_HEAVY_* 阈值 + abort_lo/hi + dep path／import prefix + top_level const lit + env gates + SKIP_TYPECK whitelist + orchestration extern-only + skip_heavy ctx + func name prefix · ~313 LOC）；自 ast_pool residual 同 TU 抽出（#include 于 ctx_loop 后、selfhost 前）；g05 STALE／`PIPELINE_X_DEPS` 已收（wave1280）
  ...
  - ✅ asm parser EMIT_HEAVY 域 thin：`pipeline_asm_parser_emit_heavy.c`（ASM_EMIT_HEAVY_PARSER_SLOT_MAX + dbg_real/bisect_max_index/slot_max + mega_bisect_skip_stub + bootstrap_mega_emit_allowed + skip_heavy_parser_mega_entry + force_stub + safe_helper + x_body_keep + slot_fallback_ok + func_is_thin_delegate + m8_tail_thin_delegate_c_name + resolve_call_to_glue + callee_is_same_module_local · 13 fn · ~532 LOC body）；k_asm_parser_thin_delegate 经 thin_delegate include 可见；g05 STALE／`PIPELINE_X_DEPS` 已收
  ...
  - ✅ LSP 诊断 C glue 域 thin：`pipeline_lsp_diag.c`（lsp_diag_typeck_after_load_c + lsp_diag_parse_entry_buf_c + lsp_diag_parse_typeck_buf_c／_impl／_thread_fn + LspDiagParseTypeckArgs · ~79 LOC body）；自 ast_pool L2683-2761 同 TU 抽出；256MiB pthread 深栈 typeck；driver_run_on_large_stack_pthread／driver_is_large_stack_thread 本块内 extern；依赖 lsp_diag_enabled/add* + pipeline_load_and_sync_* + typeck_parsed_module_c + parse_set_main_from_buf_c（先于 include）；g05 STALE／`PIPELINE_X_DEPS` 已收
  ...
  - ✅ pipeline lint + module metadata 域 thin：`pipeline_lint_meta.c`（XLANG_VISIBILITY + visibility_allow_func + L7 unused-private-funcs lint + module metadata glue · ~364 LOC）；藏于 ast_pool.c module_func.c／block.c include 间隙（L1074-1437）同 TU 抽出；依赖 ast_pool_arena/module_reset（lifecycle.c，先于 include）；g05 STALE／`PIPELINE_X_DEPS` 已收
