...
> **刷新**：2026-09-22 · 状态债 · **三端 L2 硬闸已启用** ✅；BC present **0**；Class Y Win select／shuffle／fma3 try_inline 缺 **3→0** ✅ · 钉盘 **`ecdb5cc1e`**（不升）
...
| Win PE egg pin | ✅ | egg＠`99b008cc1`；默认 asm **5/5**；FIELD／INDEX／DEREF／`struct_let_init`／`copy_large_struct`／`simd_splat`／`vector_type_let_init`／select·shuffle·fma3 override **进 live**（Class S–Y leftover-safe LEGACY g05）；**三端 L2 硬闸已启用** |
