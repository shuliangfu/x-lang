...
1. **最优真减 host-cc**：P4ud 已开 parse_unary `.x` dest-buffer（P9a peek/step；primary 零算法 ptr shim；wrap 仍 P4uc）。P4bg IDENT 头仍开。suffix_loop BODIES 仍开（P4bf）。wrap 族已尽。下刀＝glue_tail 包装／type_ref parse／parse_block_return_end_tail／register_pending／dest-SLICE。pipeline_abi mega pure-asm 仍是已知地雷。  
