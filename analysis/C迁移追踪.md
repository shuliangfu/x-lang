...
✅ **IF dest 双臂 STRUCT_LIT（帧 dest `let r = if (c) { { h: { v: a } } } else { { h: { v: b } } }`）** ✅ @ **`37f873c8e`**
  ...
  - dest-in-rbx IF of STRUCT_LIT 发射仍 ⬜（CG002）

⬜ **dest-in-rbx IF of STRUCT_LIT `*p = if (c) { { h: { v: a } } } else { y }`**

  - 现象：CG002 `.Lf0_1` offset=-1（then dest-in-rbx STRUCT_LIT 后 else dest-in-rbx VAR 发射失败，done 标签未发出）
