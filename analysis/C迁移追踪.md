...
> **刷新**：2026-09-16 · tip **`89d5c2e25`**（P2d 双端绿）· 钉盘 **`ecdb5cc1e`**
...
1. **最优真减 host-cc**：P2d body_let_bracket dest-buffer **已开**（符号 `parser_asm_parse_body_let_bracket_compound_init_ref_x_into_c`；AUDIT／glue_tail 仍 C）。下刀优先 **register_pending**（`g_gp_pending_*` 仍 C）；其后 type_ref 余量／trait_check dest-SLICE 形状核／anonymous-struct alloc。**跳过 glue_tail**（by-value ABI 适配，迁了不减算法）。禁 P4b／pipeline mega／升钉／默 bstrict。pipeline_abi mega pure-asm 仍是已知地雷。  
