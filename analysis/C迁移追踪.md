...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3 **进行中**（**2026-08-08 实测**）：`pipeline_glue.c` 仍 host 编排壳 + domain `#include`；`ast_pool.c` 纯 `#include` 编排。**bc-inventory 诚实**：present residual product C rows **0**（ROWS=128；wave309 结构地板闭 · glue／typedefs／9×fwd／standalone **absent**）；`./xbuild bc-inventory --check` 绿。**pipeline.x residual**：resolve + run_x／thin + Cap-struct leave ✅；product pure-ld **无** pipeline mega。**8.3.1～8.3.7 结构／域 leave ✅**（含 wave309 壳地板）；**8.3.6 🟡** 仅全表 from_x 退役策略仍 ⬜；**8.3.8／8.3.10 ⬜**；**8.3.9 ✅**。日常 L2 矩阵 G.7 单权威 `./xbuild l2-matrix`。**BC 终局（离 host-cc）仍 ⬜** |
