...
1. **最优真减 host-cc**：P4bg 已开 P4b IDENT 头 `.x`（填序 zeros THEN `set_var_name`；arena zeros 清 `var_name_len`，C 本地 wipe 不碰 name 槽——P4be hello typeck-broke 根因）。suffix_loop BODIES 仍开（P4bf trampoline 写 `next_lex`）。wrap 族已尽。下刀＝glue_tail 包装／unary parse／parse_block_return_end_tail／register_pending／dest-SLICE。pipeline_abi mega pure-asm 仍是已知地雷。  
