...
1. **主刀 M2**（[`自举效率方法-M2主链.md`](自举效率方法-M2主链.md)）：已绿 thin 站住 PREFER_ASM。HARD BAN PREFER 名单仍跳（禁反复分类记账）。**Class F–K**：cfg_eval／lsp sizes／stubs／host_lit 纯 asm（减 host-cc）。下一刀＝下一产品梯（减 host-cc）。禁 leftover-first；禁 `-E` 当修；禁盲 FORCE mega；禁升钉。
