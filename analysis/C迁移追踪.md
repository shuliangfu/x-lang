...
1. **主刀 M2**（[`自举效率方法-M2主链.md`](自举效率方法-M2主链.md)）：已绿 thin 站住 PREFER_ASM。下一刀＝leftover ELF writer 按 leftover ctx 入链（w696 源已 offset-first；FORCE COMMON 仍 297×1 直到入链）。禁盲 FORCE mega 入产品；禁 `-E` 当修法；禁升钉。  
