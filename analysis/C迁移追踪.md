...
✅ **FIELD dest TYPE_NAMED let-init reuse（field_off==0）** ✅ @ **`4b2ee745e`**／typed 极性 **`006117bef`**／闸收口 **`7557ce640`**
  ...
  - 余（勿并本叶）：FIELD dest field_off≠0（x86 `effective_offset` stored −8，`Prefixed.s` dest=start−8）；嵌套 sret `id32(id32)`；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
