...
| `pipeline_typeck_method_call.c` | **~2242** | typeck method_call + generic UFCS mono（wave250：check_call_generic_type_args／try_infer／bounds → typeck.x；wave249 mono foundation；wave248 overload；wave247 call-resolve；残 generic UFCS／fixup／method_call_c／mono-map·subst） | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
