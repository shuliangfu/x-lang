...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋ALWAYS `.x`（`-E` 混成）＋**w331–w332 PREFER_ASM**（typeck_orch＋glue_statics） | ALWAYS C thin 清零；无 BSS leaf 继续翻纯 asm。下一＝wave333。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ALWAYS C thin 清零；**w331+ Cap leaf `-E`→PREFER_ASM**（typeck_orch＋glue_statics）。下一＝wave333；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
