...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w350 Cap A INDEX**＋**block_tree PREFER**；残 var-module／Ubuntu check_expr／GrowVec-LE；**w335／w337 ban** 余 | 下一＝var-module／GrowVec-LE／Ubuntu PREFER。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w350 Cap A INDEX + block_tree PREFER**；下一＝var-module／GrowVec-LE／Ubuntu PREFER；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。
