...
| 2026-08-07 | **feat(bc)** emit_ctx accessor-only BSS pure leave (wave221) | **代码** · **不升钉** · **双端 L2**。B′ pure emit_ctx func_index／arena／call_param_ty／call_arg_depth／elf_ctx／scope_block get+set＋host_is_arm64→runtime_pipeline_abi；Cap residual statics 保留 module／dep／sret／typeck 直写；seed cold twins −1；产品 `./xbuild` only。present **56**。tip **`5119edba5`**。下刀：residual module／dep／sret／typeck_active／typeck／elf／ast_pool residual／pipeline_x mega。PLATFORM: SHARED freestanding emit context. |
