...
| Mega 去 pin M4（阶段 7） | 🟡 | 冷链关 pin；**7.2.1b** 1961/1991（v5.41 leftover flatten lookahead 2）；PP002 heap entry ✅；parser seed 物理删 ⬜ |
...
- 🟡 **7.2.1b** parser_asm suite audit B-minus — **1961／1,991**（v5.41 leftover flatten lookahead 2；parse_suite leftover 0；PP002 heap entry 已抬；余 advance_to／from_at paren 走／peek_kind／import_path_post／validate_toplevel）  
...
2. 🟡 **7.2.1b** B-minus 续（1961／1,991；parse_suite leftover 0；PP002 heap entry ✅；v5.41 leftover flatten lookahead 2；续＝advance_to／from_at paren 走／peek_kind／import_path_post／validate_toplevel；**深链分批 smoke／deep_off=0**）＋ **BC + 8.3** 
