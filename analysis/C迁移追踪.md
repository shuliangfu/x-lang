...
1. **主刀 M2**（[`自举效率方法-M2主链.md`](自举效率方法-M2主链.md)）：已绿 thin 站住 PREFER_ASM。小文件 `x=call(); if` 绿。Darwin COMMON lea PAGE21 sidecar。Darwin F7 data_len 双 BSS sidecar。w745：`const_lit` 只认 `const`；load_operand 可变文件级 let 走 emit_expr；产品 PREFER `asm_wpo_thin`。w746：产品 PREFER `wpo_dump` helpers（leftover gcc weak/W 则再 PREFER；JSON v2 233B）。w747：产品 PREFER `asm_locals`（leftover gcc weak/W 则再 PREFER；get+set 同 TU；nest＝2）。w748：`assign_index` 非 leftover-wipe 活体 peel（活体 INDEX＝leftover gcc emit_assign 内联；thin 仍 smash；HARD BAN PREFER）。w749：`call_arg_lea` leftover gcc W 活体同符号（thin 仍 smash `sub $0xbc8`／`#0xbd0`；HARD BAN PREFER）。w750：`fnptr_arr_esz` leftover gcc W 活体三导出（thin 仍 smash `sub $0xb38`／`#0xb40`；HARD BAN PREFER）。`return_elf_impl_thin` 独立 `-c` T=1 U=7（HARD BAN PREFER）。8 EOF 别名＝WPO 帽 4096（pad≥2048 T=N+8）。下一刀＝余 HARD BAN（full `emit_expr_elf_c` tip／下一命名 CG002 墙／fnptr_as）。禁 leftover-first 当主刀；禁 `-E` 当修法；禁盲 FORCE mega；禁升钉；禁 PREFER assign_index／call_arg_lea／fnptr_as／fnptr_arr_esz／return_elf_impl_thin／full `emit_expr_elf_c` tip／wpo_dump orch／asm_locals get peer。  
