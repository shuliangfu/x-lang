...
> **刷新**：2026-09-16 · ttc→`.x` thin 纯 asm · 钉盘 **`ecdb5cc1e`**
...
| `pipeline_abi` mega pure-asm | 🟡 ttc 已 asm | tip 双端绿；ttc C-extract→`.x` thin。残＝blkpeel／assign。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc→`.x` thin。下刀＝blkpeel／assign；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
