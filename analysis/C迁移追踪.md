...
| `pipeline_asm_emit_index_helpers.c` | ~2629 | asm ELF INDEX residual helpers（slot+esz+try_index forest+lvalue_eff_addr elf+text）切片 | 🟡 已抽出；lvalue_text wave1013；wave178 pure：6× expr shape peel；wave179 pure：local-slot ptr/addr + field-slot deref；wave180 pure：type/stride；wave181 pure：8× simple assign-addr→rbx；余 base／nested add3·subadd3／eff_addr／lvalue 仍 host-cc |
| 2026-08-06 | **feat(bc)** try_index simple assign-addr forest pure leave (wave181) | **代码** · **不升钉** · **mac L2**（Ubuntu remote 待验）。B′ pure 8× lit/idx/plus_lit/plus_var/minus_lit/minus_var/mul_lit/mul_var addr_to_rbx；base_to_rbx residual export；seed cold twins；index_helpers ~2629；present **59**。下刀：base／nested／eff_addr／lvalue／call_args。PLATFORM: SHARED. |
