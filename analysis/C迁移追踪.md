...
✅ **FIELD dest VECTOR CALL let-init** ✅ @ **`4c4c82411`**
  ...
  - 余（勿并本叶）：INDEX dest VECTOR CALL（Darwin 139）；嵌套 `idv(add4)`（Darwin 20）；FIELD-as-receiver `h.v.sub4`（Darwin 22）；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
