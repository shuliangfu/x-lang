...
| `pipeline_asm_emit_index_helpers.c` | ~3092 | asm ELF INDEX residual helpers（slot+esz+try_index forest+lvalue_eff_addr elf+text）切片 | 🟡 已抽出；lvalue_text wave1013；wave178 pure：6× expr shape peel；wave179 pure：local-slot ptr/addr + field-slot deref；余 try_index emit／base／esz／lvalue 仍 host-cc |
| 2026-08-06 | **feat(bc)** INDEX local-slot＋field-deref pure leave (wave179) | **代码** · **不升钉** · **mac L2**。B′ pure 4× enc_local_slot + index_deref_ptr_field_slot；Cap residual bodies→extern；seed cold twins；index_helpers ~3092；present **59**。下刀：try_index emit／call_args／spill BSS。PLATFORM: SHARED. |
