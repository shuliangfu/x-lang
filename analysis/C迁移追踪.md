...
| `pipeline_asm_emit_index_helpers.c` | ~2371 | asm ELF INDEX residual helpers（slot+esz+try_index forest+lvalue_eff_addr elf+text）切片 | 🟡 已抽出；lvalue_text wave1013；wave178 pure：6× expr shape peel；wave179 pure：local-slot ptr/addr + field-slot deref；wave180 pure：type/stride；wave181 pure：8× simple assign-addr→rbx；wave182 pure：base_to_rax|rbx；余 nested add3·subadd3／eff_addr／lvalue 仍 host-cc |
