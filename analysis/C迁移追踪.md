...
| **Cap 能力解锁** | 🟡 | 4.2.1–4.2.3／dest-SLICE 族／dest-in-rbx 族／INDEX dest ARRAY_LIT／runtime-index dest ARRAY_LIT／INDEX dest ARRAY_LIT 非 VAR 基／FIELD dest ARRAY_LIT／dest-in-rbx ARRAY_LIT of ARRAY_LIT／dest-in-rbx ARRAY_LIT n>1／dest-in-rbx STRUCT_LIT 场 ARRAY_LIT／STRUCT_LIT 场 ARRAY_LIT 嵌套 STRUCT_LIT／dest-in-rbx CALL／dest-in-rbx ARRAY_LIT of CALL／dest-in-rbx IF／IF dest 双臂 STRUCT_LIT（帧 dest）／dest-in-rbx IF of STRUCT_LIT／dest-in-rbx IF of ARRAY_LIT／dest-in-rbx IF extra arm stmts／dest-in-rbx IF extra arm loops／dest-in-rbx IF extra arm region／dest-in-rbx MATCH／dest-in-rbx IF of MATCH／大 main 后段 let dest 名空／匿名 match／nest 17–20／SIMD dest／METHOD binop2／STRUCT_LIT dest **已闭**。 **开项**：nest>20（21 须加宽 `type_to_c_repr` 256）；TYPE_DYN 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19。 **禁** try_inline mega／`enc_add_rax_rbx` 64 位／`enc_store_rax_to_rbx_offset` /8／本叶抬 nest 21 |
