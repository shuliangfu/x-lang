...
| `pipeline_asm_emit_call_args.c` | ~639 | asm ELF CALL-arg emit（named_struct + resolve + f32 slot + lea + dual-GP + named layout + for_call_args）切片 | 🟡 已抽出（wave1003）+ type_named_struct（wave1017）+ resolve／f32 slot（wave1019）有则补全；wave190–216 pure leave（…／**expected_ret_ty BSS**／**for_call_args mega wave216**）；余 CALL／METHOD thin wrappers 等 shell～639；仍 host-cc 入 `pipeline_x` |
