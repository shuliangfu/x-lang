...
✅ **STRUCT_LIT FIELD 源 `Wrap { h: w.h }`** ✅ @ **`e7ad55d98`**
  ...
  - 余（勿并本叶）：dest-in-rbx ARRAY_LIT `*p = [w]`（Darwin 10）；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
