...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-17 · 钉盘 **`f7424ae47`** · nest 64 冻帽 · ARM64 序言存 x19 · param_home 栈参跳过 x19 槽 · sat `try-r1` 已走 `ensure_labi_prefer_one` · L0 `-E` getcwd 已闭 · emit_header `h[88]` 超额初始化已闭（`u8[83]`）· dest extra-arm 末尾 `{ fields }` host-C 已闭 · dest extra-arm `unsafe {…}; dest` 可选分号 ASI 已闭 · dest extra-arm `region {…}; dest` 可选分号 ASI 已闭 · parse_into 函数体 `region {}; return` 同模式 ASI 已闭 · 只改勾选与事实，无波次流水  
...
✅ **dest extra-arm `region { … }; dest` 可选分号 ASI**（parse_block TOKEN_REGION／with_arena／defer 后 `;` 当 expr → P001）
  ...
  - 余（勿并本叶）：parse_into 函数体 `region {}; return` 同模式 ASI **已闭**（下条）；with_arena extra-arm dest typeck 逃逸 **已闭**
...
✅ **parse_into 函数体 `region {}; return` 同模式 ASI**（onefunc TOKEN_REGION／with_arena／defer 后 `;` 当 expr → P001）
  ...
  - 余（勿并本叶）：with_arena extra-arm dest typeck 逃逸 **已闭**（下条）；onefunc 多 compound 叠容量
...
✅ **with_arena extra-arm dest typeck 逃逸**（AL-04 assign 把外层标量 `k = 1` 一律当 allocator region escape → XT001）
  ...
  - 闸：`tests/boundary/dest_wa_semi.x` 单 MATCH 臂 isolate／asm／xlang-c **0**；fn_wa_semi host-C＋asm＋xlang-c **1**；assign／return 负例仍拒
  - 余（勿并本叶）：同一 main 叠两条 with_arena dest Darwin 139（emit／kind=6 另层）；onefunc 多 compound 叠容量
