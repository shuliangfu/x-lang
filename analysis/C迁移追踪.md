...
| `pipeline_asm_emit_index_helpers.c` | ~782 | asm ELF INDEX residual helpers（slot+esz+try_index forest residual＋CAP BSS）切片 | 🟡 已抽出；wave178–186 pure：peel／slot／type／assign-addr／base／nested／eff_addr→rax／lvalue／soa；wave187 pure：field_type_ref；余 CAP BSS／stack_off 仍 host-cc |
