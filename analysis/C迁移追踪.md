...
✅ **import 模块 const FIELD emit（host-C INT_LIT＋dest-SLICE）** ✅ @ **`3b70c5327`**
  ...
  - 余（勿并本叶）：import 模块 const FIELD asm CG002；pipeline_abi `nf>0` 闸（consts-only dep 不共编）；nest>16 soft；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off
