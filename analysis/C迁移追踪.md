...
| `pipeline_asm_emit_call_args.c` | ~1859 | asm ELF CALL-arg emit（named_struct + resolve + f32 slot + lea + dual-GP + named layout + for_call_args）切片 | 🟡 已抽出（wave1003）+ type_named_struct（wave1017）+ resolve／f32 slot（wave1019）有则补全；wave190–202 pure leave（named_struct／layout／dual-GP／agg／call_return／value_size／return_kind／struct16／param_type_ref／resolve／f32 load／param home f32 xmm／**MEMORY push／store**）；余 for_call_args mega 等仍 residual；仍 host-cc 入 `pipeline_x` |
