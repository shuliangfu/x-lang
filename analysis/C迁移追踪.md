...
| STD soft SKIP 邻域续（其它闸） | 🟡 | prefer-c honesty 续（aes-gcm nist2 RUN≠0 后置；compress-brotli ld 红跳；其余 prefer-c 择优：**async-api／json** 等）；仍跳过产品红／UNDEF（compress-stream／unified-stream ld／async-language／async-future／process-pipe XT001／mem-boundary UNDEF／encoding-hex-b64／xxhash／unicode-nfc／map-vec／channel-unbounded／http-reqresp／http-server-pool／async-context／mac／mac512／codec／atomic-widen／csv-stream／metrics／math-special／elf-write／ed25519／chacha／hash-default／schema／fenv／queue-sync／http-context-get／json-typed-decode 等）。另残 asm ld `--export-dynamic`（backtrace 具名；非软）＋by-value `Set_i32`+MEMORY 邻参 ABI（非软） |
