...
🟡 **12.0.5 asm backend 模块覆盖（`.x`→`.o` 直出替 `-E`+`$CC -c`）**
  ...
  - 下一刀候选（本域）：ALLOW_TREE bisect／ALLOW ensure 再瘦（若仍有 path 无条件／双扫）／mega pure-asm 真开须 hang 根因＋授权
...
🟡 **12.2.3 产品默认后端**（2026-08-12 · 去 import→C + seed APPLE 收敛后）
  ...
  - 下一刀候选：①树级 PREFER 地图 bisect 仅 ALLOW_TREE · ②ALLOW ensure 再瘦（若仍有 path 无条件／双扫）· ③pipeline_abi mega pure-asm 真开（须 hang 根因＋授权）
