...
🟡 **12.2.3 产品默认后端**（2026-08-12 · 去 import→C + seed APPLE 收敛后）
  ...
  - 下一刀候选：①pipeline_abi mega 仍硬禁 · ②ALLOW ensure 再瘦／sat L4 身份（须点名）。leftover extra peels PARAM PTR-elem ndims≥1 `**[2][]T`／`**[2]*T` leftover mismatch T001 已闭；extra empty `[]`／extra STAR unused-slot ARRAY／SLICE／PTR 元 × 外层 PARAM 与 RET 已齐；leftover extra SLICE peels leftover eek＝PTR eand＝−2（`[]*[]T`／`[]*[][]T`）须点名；dest extras dest-PTR stamp ✅；asm 库 TU `.data` ✅（fmt pure-asm format 残另刀）
