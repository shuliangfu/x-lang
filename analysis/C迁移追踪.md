...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w216＋w217 asm | 含 wave217 CALL/METHOD wrappers thin。下一＝wave219 al_nc_seq。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc…w217 CALL/METHOD→`.x` thin。下一叶＝wave219 al_nc_seq；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
