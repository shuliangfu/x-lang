...
✅ **import 模块 const FIELD（typeck T001＋caller arena 盖印）** ✅ @ **`41bc131c7`**
  ...
  - 余（勿并本叶）：import 模块 const FIELD emit（host-C 用 caller arena 读 dep init_ref → `n=K` 未声明／dest-SLICE `dep.A`／asm CG002）；nest>16 soft；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off
