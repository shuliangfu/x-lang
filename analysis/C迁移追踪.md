...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋w220–224／w261–w267／w269–w291 C＋w292 elf_fwd `.x`（w268=.x） | Class E Cap residual ALWAYS C-thin 收＠w291；w292＝elf_fwd C→.x（双端 L2 tip `1a0cb33a3`）。下一＝wave293 bare alias 核验／下一 C→.x。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc…w219 `.x`；w220–224／w261–w267／w269–w291 **C thin**＋w292 elf_fwd **C→.x**（w268＝既有 `.x` thin）。下一叶＝wave293 bare alias 核验／下一 C→.x；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
