...
> **刷新**：2026-09-19 · 简写 · reent_deep_copy BAN · 钉盘 **`ecdb5cc1e`**
...
| `pipeline_abi` mega pure-asm | 🟡 | Cap residual thin 混成 `-E`+asm。下一＝余 PREFER inject（copy／resolve_call／spill／call_method 已 U-complete 不 BAN；reent_deep_copy 已 BAN）。禁盲 FORCE mega。 |
| nest 冻帽 | ✅ | **64** |
...
| `pipeline_abi` mega pure-asm | 🟡 | 下一＝余 PREFER inject（copy／resolve_call／spill／call_method 已 U-complete 不 BAN；reent_deep_copy 已 BAN）。禁盲 FORCE mega。 |
| nest 冻 64 | ✅ | — |
