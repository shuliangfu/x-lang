...
> **刷新**：2026-09-16 · tip **`da64d8a47`**（P3s alloc_pointee dest-buffer）· 钉盘 **`ecdb5cc1e`**
...
1. **最优真减 host-cc**：P3s alloc_pointee **已开**。P3r parse_type_ref_impl dispatcher **已开**。P3q IDENT vector spelling consume **已开**。P4br ident_pre_dispatch **已开**。P4bq lbrace lookahead **已开**。P4bp parse_unsafe **已开**。P4bo parse_asm_bang **已开**。P4bn finish_from_type_ident **已开**。P4bm STRING decode **已开**。P3p IDENT generic `<T,U>` **已开**。P3o IDENT named／dyn／impl peel **已开**。P4bl suffix_loop LT rewind **已开**（mega `-E` typeck 墙关）。P4bk BREAK/CONTINUE **已开**。P3n builtin scalar **已开**。P4bj anonymous-struct alloc 已开。下刀＝append_byte 仍 C（Expr by-value＋var_name[N]）。禁 dest-buffer parse_type_ref（P3f hello／fmt 红；ptr shim 仍 C）。**跳过 glue_tail**。禁升钉／默 bstrict。禁 FORCE 整 mega 产品默认。  
