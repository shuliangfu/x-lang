...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w364 block_domain** PREFER Darwin；**w363b module_func** Darwin PREFER／Ubuntu `-E`；**w361/362 BAN** asm_locals／struct_layout；**w360b module_enum** Darwin PREFER／Ubuntu `-E`；**w359b BAN** top_level_let；**w358 type_alias** PREFER 双端；**w357b type_pool** Darwin PREFER／Ubuntu `-E`；残 class C peers | 下一＝class C peers／Ubuntu Type LE。PREFER 后强制 type_alias -c 闸。禁盲翻。|
| nest 冻帽 | ✅ 纪律 | **64** |
