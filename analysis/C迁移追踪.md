...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-16 · 钉盘 **`f7424ae47`** · 只改勾选与事实，无波次流水  
...
| **Cap 能力解锁** | 🟡 | 4.2.1–4.2.3／dest-SLICE 族／dest-in-rbx 族／INDEX dest ARRAY_LIT／runtime-index dest ARRAY_LIT／INDEX dest ARRAY_LIT 非 VAR 基／FIELD dest ARRAY_LIT／dest-in-rbx ARRAY_LIT of ARRAY_LIT／dest-in-rbx ARRAY_LIT n>1／dest-in-rbx STRUCT_LIT 场 ARRAY_LIT／nest 17–20／SIMD dest／METHOD binop2／STRUCT_LIT dest **已闭**。 **开项**：大 main 后段 let dest 名空；nest>20（21 须加宽 `type_to_c_repr` 256）；TYPE_DYN 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19；匿名 match。 **禁** try_inline mega／`enc_add_rax_rbx` 64 位／`enc_store_rax_to_rbx_offset` /8／本叶抬 nest 21 |
...
⬜ **大 main 后段 let dest 名空**
  ...
  - 另层勿并：匿名 match；nest>20；sat／x19 序言；TYPE_DYN／vtable；4.2.4–5／4.2.7
