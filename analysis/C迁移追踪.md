...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3：`pipeline_glue` ~29.2k + `ast_pool` ~12.9k；8.3.1 二十四刀（+asm_emit_vector_let）+ 8.3.2 …+top_level residual 已切；8.3.9 ✅；其余 ⬜ |
...
| 消费方 | 如何引用 | 风险 |
|--------|----------|------|
| `compiler/mk/x_source_deps.mk` `PIPELINE_X_DEPS` | glue + 8.3.1 切片（含 struct_lit／vector_let）+ `ast_pool` + 11× ast_pool domain slices + bootstrap glue | 改源必重编 pipeline_x |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - 🟡 仍 host-cc 编入 `pipeline_x`（未 .x 化 / 未离 host-cc）
  - ⬜ 下一域候选：下一 asm 域（spill／SIMD vector lane／bulk_mem_copy…）或 8.3.3 field_access／soa
...
2. 🟡 **BC + 阶段 8.3**（库存 ✅ · 8.3.1 二十四刀（+vector_let）✅ · **8.3.2 …+top_level residual ✅** · 8.3.9 ✅ · **当前：下一 asm 域（spill／SIMD vector lane／bulk_mem_copy…）或 8.3.3 field_access／soa**）  
