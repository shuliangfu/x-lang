...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-08（对照 tip residual：glue／ast_pool host 壳 + 域叶 thin／leave；bc-inventory present residual **36**／ROWS=119 · wave273 ELF leave 后；**只改勾选与事实 LOC**，无波次流水）
...
| `pipeline_asm_wpo.c` | **~1,511** | asm WPO v0 DCE + PGO-Lite reach/emit order（AsmWpoReachState + reach BFS + PGO hot/depth + should_emit_func + emit_order） | 🟡 **已抽出**（8.3.2 wave1256）；仍 `#include` 入 `ast_pool`／`pipeline_x` · **wave274 下刀 pure leave 候选** |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - 🟡 asm WPO v0 DCE + PGO-Lite reach/emit order 域 thin（wave1256）：`pipeline_asm_wpo.c` 仍 host-cc `#include` 入 `ast_pool`／`pipeline_x`（~1511 LOC · public faces `pipeline_asm_wpo_reach_*`／`should_emit_func`／`pgo_emit_order_*`／`pgo_is_hot_func`）；g05 STALE／`PIPELINE_X_DEPS` 已收；**wave274 下刀 pure leave 候选**（authority → runtime_pipeline_abi pure + seed cold twin + dual-export ban）
