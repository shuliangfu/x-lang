...
| process／map-vec／core-cmp／fmt／i16 soft SKIP→OK →硬绿 | ✅ | prefer asm＋`XLANG_LINK_XLANG`；退役 soft SKIP→OK／prefer-c／soft auto-make／check 硬绑 CHK002／化石 TSV `vec_u64_append_slice`；显式坏 XLANG／缺 native 硬 die；map／vec／cmp／fmt／i16 产品 `-o` 硬绿（vec score=10）；process pipe tip `**u8`＝obs、win 非 Windows＝skip；DOC→archive＋`## Gate`；报告 `run=`／`obs=`／`skip=`；双端 L2；**…／type-borrow＋lint＋obs-phase＋zc-copy／process＋map-vec＋core-cmp＋fmt＋i16 soft FAIL 池空**；zc3／zc4／zc5 host-c 后置；process pipe tip＝obs |
