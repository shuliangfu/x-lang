...
| `pipeline_typeck_region_assign.c` | **~977** | typeck region/escape assign-site 切片（wave242：scan tree pure leave + wave241 region-label + wave240 with_arena + wave239 call_slice + … → typeck.x；残 stamp_let／read_ptr／call_struct／one_region／ptr_for_addr） | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
