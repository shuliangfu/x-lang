...
| **终局：无 Makefile + 零 cc + v2==v3** | 🟡 | MG ✅ · BC 🟡 · PC 🟡（去 import→C／FORBID／ALLOW ✅；Darwin 产品 `-o` ld-only ✅；Linux hosted 产品 `-o` ld-only ✅）；见 §0.1 三义；阶段 13 |
...
🟡 **12.0.5 asm backend 模块覆盖（`.x`→`.o` 直出替 `-E`+`$CC -c`）**
  ...
  - 下一刀候选（本域）：pipeline_abi mega 仍硬禁／ALLOW ensure 再瘦（若仍有 path 无条件／双扫）／Stage12 COMPILE residual 其它叶／sat L4 身份（须点名）
...
🟡 **12.2.3 产品默认后端**（2026-08-12 · 去 import→C + seed APPLE 收敛后）
  ...
  - **Darwin prefer hybrid merge**：✅ F7 双 `LC_SEGMENT` 使 Apple `ld -r` 拒；`pure_ld_partial_merge` Darwin `-r` 失败则 `libtool -static`（LINUX `$CC -r` 首成功）
  - 下一刀候选：①pipeline_abi mega 仍硬禁 · ②ALLOW ensure 再瘦／Stage12 COMPILE residual 其它叶 · ③sat L4 身份（须点名）
