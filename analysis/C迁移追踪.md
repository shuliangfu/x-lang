...
✅ **host-C 函数作用域 dest-SLICE 模块 VAR** ✅ @ **`ec28e1f0f`**
  ...
  - 余（已闭 函数作用域 const dest-SLICE 模块 VAR）：@ **`ff65af43c`**；另层：nest>16 soft；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；import 模块 const VAR
...
✅ **函数作用域 const dest-SLICE 模块 VAR（typeck T001）** ✅ @ **`ff65af43c`**
  ...
  - 余（勿并本叶）：nest>16 soft；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；import 模块 const VAR（白名单无 dep ctx）
