...
| `pipeline_abi` mega pure-asm | 🟡 第一刀 | P4bk 解 `while`+`break` 毒化下一函数（join_slash）；mega `-E` 下一 T001＝`asm_module_is_compiler_selfhost`。禁产品默认 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **最优真减 host-cc**：P4bk BREAK/CONTINUE primary **已开**（pabi mega 第一道 T001）。P3n builtin scalar **已开**。P4bj anonymous-struct alloc 已开。下刀＝mega 下一 T001（`asm_module_is_compiler_selfhost`）或 type_ref IDENT named。**跳过 glue_tail**。改 primary.x 须两遍 g05。禁 dest-buffer IDENT generic type-arg。禁升钉／默 bstrict。禁 FORCE 整 mega 产品默认。  
