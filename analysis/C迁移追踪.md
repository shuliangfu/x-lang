...
| `pipeline_typeck_check_expr.c` | **~902（thin）** | typeck check_expr 域 Cap residual thin（wave229 return + wave228） | 🟡 **G.7** panic／unary／addr／index／deref／var／return＝typeck.x thin；match／call／try＋mega region 仍 residual body；仍 host-cc 入 `pipeline_x` |
