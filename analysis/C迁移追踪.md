...
| `pipeline_typeck_coerce_init.c` | **~578（thin）** | typeck coerce-init 域 Cap residual thin（wave231 struct_lit + wave230 type_refs／integer_widen） | 🟡 **G.7 live coerce＝typeck.x**（`typeck_coerce_init_*`＋ret_coerce／float_widen＋type_refs／integer_widen／bool／expr_type_ref／struct_lit）；residual 仅 int_binop_c／resolve_alias／int_lit／float_bits + thin faces；仍 host-cc 入 `pipeline_x` |
