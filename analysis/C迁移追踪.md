...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3：`pipeline_glue` ~34k + `ast_pool` ~18k；8.3.1 十三刀 + 8.3.2 module_import 已切；8.3.9 ✅；其余 ⬜ |
...
| `ast_pool.c` | ~17,903 | AST 池 / MatchArm / sidecar | 🟡 module_import 域已切出；其余仍在 |
...
| 消费方 | 如何引用 | 风险 |
|--------|----------|------|
| `compiler/mk/x_source_deps.mk` `PIPELINE_X_DEPS` | glue + 8.3.1 切片 + `ast_pool` + `ast_pool_module_import` + bootstrap glue | 改源必重编 pipeline_x |
...
🟡 **8.3.1 `pipeline_glue.c` → .x（或按域切 thin + 唯一权威）**
  ...
  - ⬜ 下一域候选：8.3.2 下一 ast_pool 域 / 下一 asm 域

🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**

  - MatchArm / GrowVec / module sidecar 与 parser pin 同生命周期
  - ✅ module_import 域 thin：`ast_pool_module_import.c`（XLANG_WEAK ImportEntry cold twins ~208 LOC）自 `ast_pool.c` 同 TU `#include` 抽出；`PIPELINE_X_DEPS` COUNT 34→35 / g05 STALE / inventory 已收
  - 🟡 仍 host-cc 编入 `pipeline_x`（未 .x 化 / 未离 host-cc）
  - ⬜ 下一域候选：struct_layout / top_level / arena_block / expr_sidecar / …
...
2. 🟡 **BC + 阶段 8.3**（库存 ✅ · 8.3.1 十三刀 ✅ · **8.3.2 module_import ✅** · 8.3.9 ✅ · **当前：8.3.2 下一域 / 下一 asm 域**）  
