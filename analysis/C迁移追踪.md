...
✅ **METHOD 向量 assign let-init reuse** ✅ @ **`3f70bd34d`**
  ...
  - 余（已闭 host-C nest 17 fat）：见下；另层：import extras=2 CALL emit 值绿；nest>17 soft（`type_to_c_repr` scratch 256）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
