...
| leftover unused compiler-make（dual-chain）soft→硬绿 | ✅ | 二过：拒 leftover unused `compiler-make.sh` sourced unused（无 `xlang_compiler_make`）＝假权威旁路；G.7 补全既有 nested `resolve_shu`（不在父闸再开第三 resolver）；双链仍钉 seed 再 `xlang_asm`；显式坏 XLANG 硬 die；缺 native 仍 FAIL；nested struct／return-value／return-expr `-o` 硬绿；无 archive Gate／TSV（不新建）；报告 `run=`／`skip=`；双端 L2；**leftover unused compiler-make dual-chain FAIL 池空**；余 leftover unused compiler-make catalog 非 leave **池空**（leave-class SOURCE 仍 check／queue／compress／crypto／LANG-007）；win32-*／F-* leftover XLANG fallthrough 仍 leave |
