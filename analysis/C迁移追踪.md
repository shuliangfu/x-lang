...
| `pipeline_typeck_region_assign.c` | **~1470** | typeck region/escape assign-site 切片（wave237：allocator region + wave236 stack-escape／scope-borrow + wave235 M-3 assign／return pure leave → typeck.x；残 with_arena BSS／scan tree／call_slice） | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
