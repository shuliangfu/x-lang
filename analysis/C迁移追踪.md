...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w350 Cap A**（Darwin＋bt PREFER；Ubuntu 跳过／option）；残 var-module／Ubuntu CapA-option／GrowVec-LE | 下一＝Ubuntu CapA/option／var-module／GrowVec-LE。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w350 Cap A**（Darwin PREFER；Ubuntu 产品跳过）；下一＝Ubuntu CapA/option／var-module／GrowVec-LE；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。
