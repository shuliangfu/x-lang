...
| STD soft SKIP 邻域续（其它闸） | 🟡 | prefer-c／archaeology honesty 续（aes-gcm nist2 RUN≠0 后置；compress-brotli ld 红跳；其余择优：sync-lock-diag／test-runner 等已探针 asm 绿；encoding-extra／regex／net-tls 仍 UNDEF／产品红跳；跳 async-1m／language／future coop UNDEF）；仍跳过产品红／UNDEF（encoding-extra UNDEF／regex `_main`／net-tls XT001／compress-stream／unified-stream ld／async-language／async-future／process-pipe XT001／mem-boundary UNDEF／encoding-hex-b64／xxhash／unicode-nfc／map-vec／channel-unbounded／http-reqresp／http-server-pool／async-context／mac／mac512／codec／atomic-widen／csv-stream／metrics／math-special／elf-write／ed25519／chacha／hash-default／schema／fenv／queue-sync／http-context-get／json-typed-decode／Darwin json `needs_copy`／coop_pingpong 等）。另残 asm ld `--export-dynamic`（backtrace 具名；非软）＋by-value `Set_i32`+MEMORY 邻参 ABI（非软） |
