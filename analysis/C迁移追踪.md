...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-08（对照 tip residual：glue／ast_pool host 壳 + 域叶 thin／leave；bc-inventory present residual **14**／ROWS=128 · wave301 strict_minimal dual leave 后；**只改勾选与事实 LOC**，无波次流水）
...
| `pipeline_glue_strict_minimal`（seed → 产品链） | ~957（wave301） | 产品 weak 孪生 | 🟡 仍在链；**wave300**：28× typeck_x dual；**wave301**：runtime_pipeline_abi／driver_x dual + 死 helpers（~2546→~957）；残 parse_commit_pre／post + find_func_return overload 链；**8.3.6 退役未终** |
...
🟡 **8.3.6 `seeds/*.from_x.c` 全表退役策略**
  ...
  - ⬜ 残：strict_only WEAK／helpers／main_entry 桥／field_access 等；全表退役策略未终
