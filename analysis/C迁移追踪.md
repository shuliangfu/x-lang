...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w338 COMMON**；**w339–w343** Cap A（含 check_expr）LINUX PREFER／DARWIN `-E`；bootstrap→类 B；**w335／w337 ban** 余 | Cap A 主叶收。下一＝block_tree 或 GrowVec-LE。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w343 Cap A typeck_check_expr**（LINUX PREFER／DARWIN `-E`）；bootstrap_glue→类 B。下一＝block_tree 或 GrowVec-LE；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
