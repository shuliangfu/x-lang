...
✅ **dest extra-arm SIMD 叠 MATCH＋IF＋field-bind＋无分号 `with_arena` dest**（旧 Darwin 139；官方闸曾收成单条 MATCH）
  ...
  - 余（已闭 dest extra-arm `defer { k=1 }; dest`）：见下；另层：TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；dest extra-arm 嵌套 `{ let t; dest }` host-C void
...
✅ **dest extra-arm `defer { k=1 }; dest` emit**（asm leftover 71；host-C 0）
  ...
  - 余（勿并本叶）：dest extra-arm 嵌套 `{ let t; dest }` host-C void；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off
