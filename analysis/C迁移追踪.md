...
✅ **4.2.2 trait bounds / dyn Trait** dyn／impl 类型位 ✅ @ **`7bfff4e6b`**（impl 级 bound ✅ @ **`7e2e9aac9`** · struct 级 ✅ @ **`377bd8d74`** · 体方法 ✅ @ **`10bfd56f4`** · PLUS ✅ @ **`fe000ba3a`** · **pin-seed 体方法 resolve 闭** 2026-08-17）
  ...
  - ✅ F7 dyn stack extras（双端 L2）：产生点 `dep_idx==-2` `nargs>5` CG002，且 wrapper `prologue+call` 错位 impl `[rbp+16]`。G.7 补全既有 dyn extras＋wrapper（`glue_asm_call_reg_max`；溢出 extras x86 push／ARM64 `[sp]`；wrapper 拷入栈再 `call` impl）。wrapper rdi＝data 未改。验收：`dyn_add_stack.x` 双端 asm **7**。**仍开**：host-C nparams>6
