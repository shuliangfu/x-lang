...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋ALWAYS `.x`（`-E` 混成）＋**w331–w334 PREFER_ASM**；**w335 ban** onefunc／type_pool | ALWAYS C thin 清零；跳过 ban 叶继续翻。下一＝wave336。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ALWAYS C thin 清零；**w331–w334 PREFER_ASM**；**w335** onefunc／type_pool 禁翻。下一＝wave336；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
