...
| leftover fossil DOC／catalog no-Honesty OBS-002 soft→硬绿 | ✅ | 二过：拒 leftover 顶层 `analysis/obs-async-runtime-trace-v1.md` 已归档仍当 live DOC（缺文件 → portable OBS-002 红）＋leftover catalog 无 Honesty／缺 `run=`／`obs=`／`skip=`＋leftover `xlang_compiler_make` scheduler.o auto-make；live＝`analysis/archive/obs/`；顶层复活硬 die；无 XLANG 面故 G.7 不开 resolver；退役 leftover auto-make；host-C smoke UNDEF＝obs；keep `obs-async-runtime-trace gate OK`；archive leftover residual（无新建 Gate）；TSV cross_perf xref＝archive；报告 `run=`／`obs=`／`skip=`；双端 L2；**leftover fossil DOC OBS-002 FAIL 池空**；**leftover fossil DOC placeholder-inventory 已收**；余 leftover fossil DOC live 默认仍 `自举前必须清单.md` 四闸／stdlib-check-matrix（check postponed）；win32-*／F-* leftover XLANG fallthrough 仍 leave |
