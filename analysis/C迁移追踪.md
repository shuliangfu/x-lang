...
✅ **INDEX 负 i32 比宽 AAPCS64 LDRSW** ✅ @ **`0e42fa8c5`**
  ...
  - 余（勿并本叶）：METHOD binop2 仍 CALL-only（本地 CALL `add(a,b)` 已 0）；nest>16 soft；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；i64 大字面量比宽（2^32 vs `mov w0,#0`）另层
