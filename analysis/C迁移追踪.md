...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w347 lea**；**w348 for_call_args＋check_expr PREFER**；bootstrap→类 B；**w335／w337 ban** 余 | 下一＝block_tree／GrowVec-LE。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w348 for_call_args rvalue + check_expr PREFER**；下一＝block_tree／GrowVec-LE；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
