...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-16（钉盘仍 **`f7424ae47`** · dest-in-rbx 嵌套 STRUCT_LIT `*p = { h: { v: a } }` 双端 L2 闭 @ **`25092c1c2`** · dest-in-rbx ARRAY VAR `*p = src` 官方已绿（闸 @ **`25092c1c2`**）· INDEX dest ARRAY_LIT `rows[0] = [w]` 双端 L2 闭 @ **`3e53921df`** · dest-in-rbx ARRAY_LIT of FIELD `*p = [w.h]` 双端 L2 闭 @ **`fb6ad71f4`** · dest-in-rbx ARRAY_LIT `*p = [w]` 双端 L2 闭 @ **`2046f5a15`** · 值位置只认 `{ fields }`（禁 `Type { fields }`）双端 L2 闭 @ **`6c9773729`** · STRUCT_LIT FIELD 源 `Wrap { h: w.h }` 双端 L2 闭 @ **`e7ad55d98`** · dest-in-rbx DEREF 源 `*p = *q` 双端 L2 闭 @ **`97636e07d`** · dest-in-rbx INDEX 整元素 `*p = arr[i]` 双端 L2 闭 @ **`b96a7f9bf`** · INDEX 基 FIELD dest-in-rbx `*p = arr[i].h` 双端 L2 闭 @ **`1c14db3e7`** · ARRAY_LIT 16B named VAR `[w]` 双端 L2 闭 @ **`b7b46ae27`** · dest-in-rbx FIELD 源 `*p = w.h` 双端 L2 闭 @ **`ed9855203`** · STRUCT_LIT `Wrap { h: inner }` ARM64 16B field store 双端 L2 闭 @ **`7ddacf3a3`** · Holder 拷 `let h=w.h` 双端 L2 闭 @ **`d1a80c50a`** · typeck 不盖 SIMD FIELD ambient @ **`dbe877df1`** · dest `w.h.v[i]=` 已绿（rvalue 叶带过，本叶闸）· FIELD-chain SIMD INDEX `w.h.v[i]` 双端 L2 闭 @ **`b69a33a51`** · prefixed SIMD dest（`tag+Holder` field_off≠0）双端 L2 闭 @ **`149df550f`** · 第二份 SIMD ARRAY_LIT 盖 dest 双端 L2 闭 @ **`4b0fc890d`** · FIELD-chain `w.h.v.sub4` 双端 L2 闭 @ **`e6dd1a86b`** · 前刷新 `return idv(add4)[i]` emit_expr @ **`66e6e848d`** · 前刷新 FIELD-as-receiver `h.v.sub4` @ **`617b1f739`** · 前刷新嵌套 identity CALL `idv(add4)` @ **`24d7d4b2a`** · 前刷新嵌套 INDEX rvalue `arr[0][0]` @ **`345a51463`** · 前刷新 INDEX dest VECTOR CALL let-init @ **`6141c91e2`** · 前刷新 ARRAY_LIT SIMD VAR let-init @ **`7ed90f9e7`** · 前刷新 FIELD dest VECTOR CALL let-init @ **`4c4c82411`** · 前刷新 SIMD named `size_simple`＝lanes*esz @ **`b76bb2466`** · 前刷新 VECTOR CALL dest dest-in-rbx let-init＋x86 高位 home @ **`97548e68d`** · 前刷新 2026-08-15 钉盘升 **`f7424ae47`** · 双端 L4 真冷 + bstrict 129 · Stage 12.0.5 prefer 族／硬禁 mega 仍有效 · Cap 14–15 日叶闭：dest-SLICE 族／STRUCT_LIT bool／u8／FIELD overload／FIELD 16B／heap Arena64 FFI／div0 panic／ARM64 add_imm SUB／extra-.o has_obj／dest 影子 x19／dest-SLICE INDEX fat ARM64 dual-GP／ARM64 >16B INDEX MEMORY consume／Darwin import METHOD host-C f32 返回 s0→w0／Darwin 活 seed import METHOD f32 extras s0–s7／ARM64 INDEX 非 1/4/8 步长 64 位 scale1／ARM64 Vec8i shuffle 高半 slot+16／ARM64 Vec4f select fcmgt 后 BSL／METHOD binop2 UFCS let-init／VAR assign 16B CALL dual-GP／METHOD 向量 assign let-init reuse／host-C nest 20 fat／VAR assign >16B CALL sret／VAR copy >16B／FIELD dest TYPE_NAMED let-init reuse（field_off==0）／嵌套 MEMORY sret dest 保存／恢复／FIELD dest field_off≠0 frame-mag／FIELD-chain dest 走 VAR 根 let-init／指针／INDEX FIELD dest dest-in-rbx let-init／INDEX dest 16B CALL dest-in-rbx let-init／**DEREF dest TYPE_NAMED dest-in-rbx let-init**／**DEREF dest SLICE／ARRAY／VECTOR dest-in-rbx** · M4 冷链 **5/5** · Stage 8 **30/30** · **11.2.2 `./xbuild l4` ✅** · **只改勾选与事实**，无波次流水）  
...
✅ **INDEX dest ARRAY_LIT `rows[0] = [w]`** ✅ @ **`3e53921df`**
  ...
  - 余（已闭 dest-in-rbx ARRAY VAR／嵌套 STRUCT_LIT）：见下；另层：`return [w]`；大 main 后段 let dest 名空；匿名 match；nest>20；sat／x19 序言

✅ **dest-in-rbx ARRAY VAR `*p = src`** ✅ @ **`25092c1c2`**（emit 已绿 @ dest-in-rbx ARRAY_LIT 叶；本叶闸）

  - 现场：官方大 `main()` 已 0。小 isolate Darwin 12 是 Holder 尺寸 8 类（与小文件 `*p = w.h` 同类）；typeck dest 盖内层后 isolate 亦 0
  - 闸：`tests/boundary/vec_field_index_return.x` dest-in-rbx ARRAY VAR 102／103／104／105
  - 余（已闭 dest-in-rbx 嵌套 STRUCT_LIT）：见下

✅ **dest-in-rbx 嵌套 STRUCT_LIT `*p = { h: { v: a } }`** ✅ @ **`25092c1c2`**

  - 现场：isolate Darwin **12**（内层 `{ v: a }` 无 dest，field_store_sz=8）。官方大 `main()` 内联 108（后段 let dest 名空 `(struct )`，另层）。host-C 同 108。`{ h: inner }`／`{ h: w.h }` 已 0
  - **根修（G.7 有则补全）**：产生点＝`typeck_check_expr_struct_lit` 场 init expected=0，匿名路径回填 dest 名后不走 `field_inits_to_layout`。权威已是同一 dest 回填。补 `typeck_coerce_init_struct_lit_to_decl` 递归嵌套 STRUCT_LIT 场；匿名 check_expr 回填后走同一 field_inits。dest-in-rbx nest 直写 dest+foff（禁 bump next_offset）。官方闸用小 helper（大 main 后段 dest 名另层）
  - 权威 `typeck.x` assemble → `typeck_x.o`。pipeline_abi prefer dest-in-rbx nest rec。**禁**改 pin `typeck_gen.linux.x86_64.c`／try_inline／x19／nest 21
  - 闸：`tests/boundary/vec_field_index_return.x` dest-in-rbx 嵌套 STRUCT_LIT 106／107／108／109（`dest_nest_slit` helper）
  - 证：双端 vfir／xlang-c **0** · isolate nest／frame／arrvar **0** · smd／smdrv／vald **0** · fs16 **42** · named 拒 · 矩阵 **5/5**（try_inline 仍 seed 72264／57672；pipeline_abi prefer Darwin 1382384／Ubuntu 1639064；typeck_x Darwin 318112／Ubuntu 385888）· **日常 L2 不升钉**
  - 余（勿并本叶）：`return [w]`；大 main 后段 let dest 名空；匿名 match；nest>20；sat／x19 序言
