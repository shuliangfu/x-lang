...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-17 · 钉盘 **`f7424ae47`** · nest 64 冻帽 · ARM64 序言存 x19 · param_home 栈参跳过 x19 槽 · sat `try-r1` 已走 `ensure_labi_prefer_one` · L0 `-E` getcwd 已闭（emit_header 补 stdlib＋unistd）· 只改勾选与事实，无波次流水  
...
✅ **extra-.o has_obj 去重（invoke_cc／asm ld／sat mega）** ✅ @ **`f7424ae47`**
  ...
  - 残：sat 路由 **已开**（`try-r1` → `ensure_labi_prefer_one`；sat 形态 **427280** Darwin **5/5**）。mega **318600** 活。L0 `-E` `getcwd` **已闭**（`emit_header` 补 skip-decl 所需 `stdlib.h`＋`unistd.h`；LABI=0 `-E`＋cc 绿）。`push_stable` prefer 已调权威 `has_obj`。
