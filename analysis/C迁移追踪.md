...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w338 COMMON**；**w339–w342** Cap A OK；**w343 check_expr PREFER ban**；bootstrap→类 B；**w335／w337 ban** 余 | 下一＝GrowVec-LE／ordinal dual-home 或 block_tree。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w343 check_expr PREFER ban**（Ubuntu XT001／ordinal Lxml）；双端仍 `-E`。下一＝GrowVec-LE／dual-home 或 block_tree；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
