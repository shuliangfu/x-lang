...
| STD-002／001 net-api＋io-api soft→硬绿 | ✅ | DOC→`## 8/7. Gate`；闸 prefer asm＋`XLANG_LINK_XLANG`＋`dod_native_exe`；退役 soft XLANG fallthrough／soft auto-make／`check=` 报告；显式坏 XLANG／缺 native 硬 die；`run-net`／`run-io` exit0 硬失败；去化石 `addr_to_packed`∈main.x；check＝obs；报告 `run=`／`obs=`／`skip=`；双端 L2；**net-api soft fallthrough FAIL 池空** |
...
| STD-061 simd-prod soft→硬绿 | ✅ | DOC→`## 3. Gate`；对齐 `stub/Xlang`＋`r04_simd_shuffle_select`；闸 prefer asm＋`XLANG_LINK_XLANG`；check 观测；manifest hard；perf soft；报告 `check=`／`bench=`／`skip=`／`ratio=`；双端 L2（**仍 soft fallthrough 二过待收**） |
