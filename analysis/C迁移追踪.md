...
| STD soft SKIP 邻域续（其它闸） | 🟡 | prefer-c honesty 续（优先 **STD-006 crypto** 主闸 `tests/crypto/main.x` 已探针 asm 绿；net-api／io-api；aes-gcm nist2 RUN≠0 后置；compress-brotli ld 红跳）；仍跳过产品红／UNDEF（compress-stream／unified-stream ld／async-language／process-pipe XT001／mem-boundary UNDEF／encoding-hex-b64／xxhash／json-zc／unicode-nfc／map-vec／channel-unbounded／http-reqresp／async-context／mac512／codec／atomic-widen 等）。另残 asm ld `--export-dynamic`（backtrace 具名；非软）＋by-value `Set_i32`+MEMORY 邻参 ABI（非软） |
