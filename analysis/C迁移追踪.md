...
| `pipeline_abi` mega pure-asm | 🟡 | 已绿 PREFER 面同上。**Class AP** leftover 拼装已收；**Class AV–AX** seed／mega Cap（DEBUG／ASM_DEBUG／ASM_*_TRACE）已剥（产品 leftover 同尺寸，禁 FORCE）。真减 host-cc 续。HARD BAN／leftover-first／mega FORCE／`-E` 当修仍禁。 |
| nest 冻帽 | ✅ | **64** |
