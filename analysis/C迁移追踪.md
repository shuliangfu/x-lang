...
✅ **FIELD-as-receiver `h.v.sub4` lane stack-off** ✅ @ **`617b1f739`**
  ...
  - 余（勿并本叶）：`return idv(add4)[i]` emit_expr；第二份 SIMD ARRAY_LIT 盖 dest（x86）；FIELD-chain `w.h.v.sub4`；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
