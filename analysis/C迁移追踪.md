...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-17 · 钉盘 **`f7424ae47`** · nest 64 冻帽 · ARM64 序言存 x19 · param_home 栈参跳过 x19 槽 · sat `try-r1` 已走 `ensure_labi_prefer_one` · L0 `-E` getcwd 已闭 · emit_header `h[88]` 超额初始化已闭（`u8[83]`）· dest extra-arm 末尾 `{ fields }` host-C 已闭 · dest extra-arm `unsafe {…}; dest` 可选分号 ASI 已闭 · 只改勾选与事实，无波次流水  
...
✅ **extra-.o has_obj 去重（invoke_cc／asm ld／sat mega）** ✅ @ **`f7424ae47`**
  ...
  - 残：sat 路由 **已开**（`try-r1` → `ensure_labi_prefer_one`；sat 形态 **427280** Darwin **5/5**）。mega **318600** 活。L0 `-E` `getcwd` **已闭**。emit_header `h[88]` 超额初始化 **已闭**（`u8[83]`）。dest extra-arm 末尾 `{ fields }` host-C **已闭**。dest extra-arm `unsafe {…}; dest` ASI **已闭**。`push_stable` prefer 已调权威 `has_obj`。
