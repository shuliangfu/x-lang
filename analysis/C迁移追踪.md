...
| `pipeline_asm_emit_index_helpers.c` | ~3132 | asm ELF INDEX residual helpers（slot+esz+try_index forest+lvalue_eff_addr elf+text）切片 | 🟡 已抽出；lvalue_text wave1013；wave178 pure：6× expr shape peel；余 try_index emit／base／esz／lvalue 仍 host-cc |
| 2026-08-06 | **feat(bc)** INDEX expr shape peel forest pure leave (wave178) | **代码** · **不升钉** · **mac L2**。B′ pure 6× glue_index_expr_var_* peel；Cap residual try_index 调 pure；seed cold twins；index_helpers ~3132；present **59**。下刀：try_index emit／call_args／spill BSS。PLATFORM: SHARED. |
