...
| f07 soft→硬绿 | ✅ | DOC v1／v2→`## Gate`；闸 prefer asm＋`XLANG_LINK_XLANG`；static＋forbidden＋f06(FAIL=1)＋f03-core 硬失败；退役 soft `XLANG_F07_NO_CC_MIGRATED_FAIL`；报告 `static=`／`forbidden=`／`f06=`／`f03_core=`／`skip=`；双端 L2 |
| STD soft SKIP 邻域续（其它闸） | 🟡 | prefer-c／archaeology honesty 续（**f06**／f02 等；zc 若仍绑 host-c 链则后置；STD uuid／task／mem-safe／trace 等 asm UNDEF 跳；compress-brotli ld 红跳；test-executable／bench-fuzz 探针 ld UNDEF 跳；http-context／channel-unbounded／unicode-nfc 复探红跳；encoding-extra／regex／net-tls 仍跳；跳 async-1m／language／future coop UNDEF）；仍跳过产品红／UNDEF（aes-gcm nist2 RUN≠0／encoding-extra UNDEF／regex `_main`／net-tls XT001／compress-stream／unified-stream ld／async-language／async-future／process-pipe XT001／mem-boundary UNDEF／encoding-hex-b64／xxhash／map-vec／http-reqresp／http-server-pool／async-context／mac／mac512／codec／atomic-widen／csv-stream／metrics／math-special／elf-write／ed25519／chacha／hash-default／schema／fenv／queue-sync／http-context-get／json-typed-decode／Darwin json `needs_copy`／coop_pingpong／sqlite stub open／last_error 段错误／labi `std_sys_macos_write_*` needle 等）。另残 asm ld `--export-dynamic`（backtrace 具名；非软）＋by-value `Set_i32`+MEMORY 邻参 ABI（非软） |
