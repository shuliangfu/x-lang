...
✅ **4.2.2 trait bounds / dyn Trait** dyn／impl 类型位 ✅ @ **`7bfff4e6b`**（impl 级 bound ✅ @ **`7e2e9aac9`** · struct 级 ✅ @ **`377bd8d74`** · 体方法 ✅ @ **`10bfd56f4`** · PLUS ✅ @ **`fe000ba3a`** · **pin-seed 体方法 resolve 闭** 2026-08-17）
  ...
  - 🟡 F7 asm dyn dispatch：**ELF `.data`／`.rela.data` 已发**（`pipeline_elf_write_o_standard_to_buf_c`；shndx 4＝`.data`；r_type=200→`R_X86_64_64`；产品权威＝mega `.x` thin WEAK；seed rest `FROM_X` 不编此体，cold twin 同语义）。Ubuntu `readelf`：vtable `D`、`.rela.data` wrap 指针、`.text` 仍有 `movabs`＋`R_X86_64_64`（TEXTREL → dispatch 139）。**开项**：`backend_enc_lea_sym_to_reg_arch` x86 改 RIP 相对 LEA（禁 `.text` abs64）；wrapper 形参走 rdi 非 rax（邻层，勿并）
