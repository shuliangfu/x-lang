...
| EXC-004 error-chain soft→硬绿 | ✅ | DOC／TSV→`## 6. Gate`；闸 prefer asm＋`XLANG_LINK_XLANG`；check 观测；`error_chain_smoke.x` exit0 硬失败；报告 `check=`／`run=`／`skip=`；双端 L2；**2026-08-29 residual XLANG fallthrough 已收** |
| EXC-003 error-code-layer soft→硬绿 | ✅ | DOC／TSV→`## 7. Gate`；闸 prefer asm＋`XLANG_LINK_XLANG`；check 观测；`error_code_layer.x` exit0 硬失败；报告 `check=`／`run=`／`skip=`；双端 L2；**2026-08-29 residual XLANG fallthrough 已收** |
| EXC-006 error-recovery soft→硬绿 | ✅ | DOC／TSV→`## 3. Gate`；闸 prefer asm＋`XLANG_LINK_XLANG`；check 观测；recovery suite 30 case exit0 硬失败；报告 `check=`／`run=`／`skip=`；双端 L2；**2026-08-29 residual XLANG fallthrough 已收**（runner leftover auto-make 同收） |
