...
🟡 **值位置只认 `{ fields }`（禁 `Type { fields }`）** 🟡 Darwin L2（Ubuntu 本波）
  ...
  - 证：Darwin 矩阵 **5/5** · vfir／msd／da／fs16 **0／0／0／42** · named 拒 `expected { fields }, found Point` · **日常 L2 不升钉**
  - 余（勿并本叶）：dest-in-rbx ARRAY_LIT `*p = [w]`（Darwin 10）；匿名 match 模式；nest>20；sat／x19 序言
