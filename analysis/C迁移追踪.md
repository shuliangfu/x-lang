...
✅ **FIELD-chain dest 走 VAR 根 let-init** ✅ @ **`892da2d63`**
  ...
  - 余（勿并本叶）：指针／INDEX FIELD dest；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
...
🟡 **12.0.5 asm backend 模块覆盖（`.x`→`.o` 直出替 `-E`+`$CC -c`）**
  ...
  - 下一刀候选（本域）：sat 盖 prefer（须点名＋双端 L4）／pipeline_abi mega 仍硬禁／ARM64 prologue 未存 x19（须点名）／FIELD dest field_off≠0（须点名）／嵌套 sret（须点名）／ALLOW ensure 再瘦（若仍有 path 无条件／双扫）／Stage12 COMPILE residual 其它叶
