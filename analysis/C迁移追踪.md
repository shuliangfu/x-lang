...
| Mega 去 pin M4（阶段 7） | 🟡 | 冷链关 pin；**7.2.1b leftover flatten 完**；深链分批覆盖戏停；**已 T 缩壳**：P9 hybrid 跳过 suite 46k；**Route C 产品化**：P9b stretch lite＋P1b lex_skip＋P19b helpers＋P4b primary ident＋P3b type_ref＋P7b simd ident pack／callee 名＋P4ub unary TOKEN→ExprKind＋P4bb binop TOKEN→ExprKind＋P14b skip_if 走查＋P12b skip_tl struct／enum／extern 走查＋P18b body_tl scalar 表＋let／const／if skip 走查＋**P10b glue skip_one_function_full 走查**（glue 包装仍 C；pure-asm 过）；残＝产品 `.inc` 切片仍 host-cc（含 glue_tail 包装、primary parse、P19 rewind／run_len、P1 count、skip_tl trait-reg）、lite／suite／lex_skip／helpers／primary ident／type_ref／simd／unary／binop／skip_if walks／skip_tl struct／enum／extern／body_tl skip／glue skip_one_function_full portable 冷回退、`parser_asm_thin_c.from_x.c` 443KiB 仅冷 rest 回退；parser seed 物理删 ⬜ |
...
- 🟡 **8.3.6** from_x 全表退役策略终稿 — 残：有引用冷孪生三分处置（pthin suite 46k、lite 564、lex_skip portable、helpers kind／copy／pos／match-kw、primary ident、type_ref kind／dyn／builtin／vector IDENT、simd ident／callee 名、unary TOKEN→ExprKind、binop TOKEN→ExprKind、skip_if walks、skip_tl struct／enum／extern walks、body_tl scalar／skip walks、glue skip_one_function_full 已从 hybrid 预处理拿掉，文件仍留作冷回退；glue_tail 包装、primary parse、P19 rewind／run_len、P1 count、skip_tl trait-reg 仍 host-cc）  
...
2. 🟡 **7.2.1b leftover flatten 完**＋**8.3.6 有引用冷孪生三分**（P9 suite 46k＋lite 564＋P1b lex_skip portable＋P19b helpers kind／copy／pos／match-kw＋P4b primary ident＋P3b type_ref kind／dyn／builtin／vector IDENT＋P7b simd ident／callee 名＋P4ub unary TOKEN→ExprKind＋P4bb binop TOKEN→ExprKind＋P14b skip_if walks＋P12b skip_tl struct／enum／extern walks＋P18b body_tl scalar／skip walks＋P10b glue skip_one_function_full hybrid 已跳过；残＝产品 `.inc` 切片仍 host-cc含 glue_tail 包装、P19 rewind／run_len、P1 count、skip_tl trait-reg；**禁**再深链分批 eq；pipeline_x mega 须点名）＋ **BC + 8.3** 
