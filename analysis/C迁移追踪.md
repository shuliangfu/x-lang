...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w348 FCA**；**w349 block_tree T001**（PREFER 仍 ban／CG002）；bootstrap→类 B；**w335／w337 ban** 余 | 下一＝Cap A array／BSS 根／GrowVec-LE／Ubuntu check_expr PREFER。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w349 block_tree T001 unsafe**；PREFER 仍 ban（CG002 BSS）；下一＝Cap A array／BSS／GrowVec-LE／Ubuntu check_expr PREFER；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
