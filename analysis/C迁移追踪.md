...
✅ **VAR assign 16B CALL dual-GP** ✅
  ...
  - 余（勿并本叶）：METHOD 向量 assign `d=a.add4(b)` 仍 Darwin 12（CALL emit／callee lane0）；import extras=2 CALL emit 值绿；nest>16 soft；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
...
🟡 **12.0.5 asm backend 模块覆盖（`.x`→`.o` 直出替 `-E`+`$CC -c`）**
  ...
  - 下一刀候选（本域）：sat 盖 prefer（须点名＋双端 L4）／pipeline_abi mega 仍硬禁／ARM64 prologue 未存 x19（须点名）／METHOD 向量 assign 仍 CALL-only／ALLOW ensure 再瘦（若仍有 path 无条件／双扫）／Stage12 COMPILE residual 其它叶
