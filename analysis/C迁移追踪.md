...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w345 prepare**；**w346 check_expr const**（PREFER 仍 ban）；bootstrap→类 B；**w335／w337 ban** 余 | 下一＝check_expr PREFER body／block_tree／GrowVec-LE。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w346 check_expr ordinal let→const**；PREFER 仍 ban（body）；下一＝PREFER body／block_tree／GrowVec-LE；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
