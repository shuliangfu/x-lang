...
| `pipeline_asm_emit_index_helpers.c` | ~655 | asm ELF INDEX residual helpers（CAP cache＋stack_spill 表＋extern pure faces）切片 | 🟡 已抽出；wave178–189 pure：peel／slot／type／assign-addr／base／nested／eff_addr→rax／lvalue／soa／field_type_ref／stack_off／module_from_ctx／needs_ptr／array_slot／struct_slot；wave207 pure：rax_frame spill＋scratch depth；余 CAP cache structs／stack_spill 表仍 host-cc |
