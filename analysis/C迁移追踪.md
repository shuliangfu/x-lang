...
| `pipeline_asm_emit_call_args.c` | ~1052 | asm ELF CALL-arg emit（named_struct + resolve + f32 slot + lea + dual-GP + named layout + for_call_args）切片 | 🟡 已抽出（wave1003）+ type_named_struct（wave1017）+ resolve／f32 slot（wave1019）有则补全；wave190 pure leave named_struct＋lea gate（live＝runtime_pipeline_abi）；余 dual-GP／for_call_args 等仍 residual；仍 host-cc 入 `pipeline_x` |
