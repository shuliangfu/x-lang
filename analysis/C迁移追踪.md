...
| `pipeline_abi` mega pure-asm | 🟡 | 余 PREFER 已双端 U-complete **不 BAN**。add_defs／param_ptr_slot／unused_hints／asm_expr helpers 产品 PREFER_ASM。Darwin COMMON lea PAGE21 sidecar。Darwin F7 data_len 双 BSS sidecar。`return_elf_impl_thin` 独立 `-c` T=1 U=7；产品 leftover gcc W 未改。小文件 `x=call(); if` 绿。8 EOF 别名＝WPO 帽 2048 已活体 sidecar 4096（pad≥2048 T=N+8）。wpo_dump 活体仍 leftover gcc W。不 Soft-Cap `-E` 余叶。mega FORCE 禁。leftover-first 非主刀。禁 PREFER wpo_dump helpers／orch／asm_locals get／deref peel／grow_vec／deref scalar／rhs_to_rax／assign_var／emit_ctx BSS／final_expr／return_impl／fnptr_arr_esz／fnptr_as／assign_index／call_arg_lea／full `emit_expr_elf_c` tip／`asm_wpo_thin`（n_sect 已闭；墙＝活体 thin WPO CG002 `code_len=0`）。禁 LINUX `-E` emit_ctx BSS。禁 BAN 已齐余 PREFER。禁 un-BAN arrlit＋main。 |
| nest 冻 64 | ✅ | — |
...
1. **主刀 M2**（[`自举效率方法-M2主链.md`](自举效率方法-M2主链.md)）：已绿 thin 站住 PREFER_ASM。小文件 `x=call(); if` 绿。Darwin COMMON lea PAGE21 sidecar。Darwin F7 data_len 双 BSS sidecar（`g_aw_root_id=-1` `__DATA,__data`；n_sect 已闭）。`return_elf_impl_thin` 独立 `-c` T=1 U=7（HARD BAN PREFER）。8 EOF 别名＝WPO 帽 2048 已活体 sidecar 4096（pad≥2048 T=N+8）。下一刀＝PREFER `asm_wpo_thin`（墙＝活体 thin WPO CG002 `code_len=0`）。禁 leftover-first 当主刀；禁 `-E` 当修法；禁盲 FORCE mega；禁升钉；禁 PREFER wpo_dump／asm_locals／assign_index／return_elf_impl_thin／full `emit_expr_elf_c` tip／`asm_wpo_thin`。  
