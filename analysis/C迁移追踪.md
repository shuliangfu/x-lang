...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-11（Stage 12 零 cc：…／**12.0.5 labi full12 hybrid 双端 + COMPILE residual pure_asm 9/13 + L2-asm hybrid 3/3（user_asm merge residual 闭）· 默认仍 -E**；钉盘 **`e364f4a37`**；**只改勾选与事实**，无波次流水）
...
| **冷启动零 cc 链** | 🟡 | **LINK 全零 cc ✅** · **`.s` COMPILE 零 cc ✅** · **stub weak `.s` ✅** · **forbid_host_cc ✅** · **STRING_LIT ✅** · **module const binop ✅** · **empty `[]`／emit／lsp_diag CG002 ✅** · **`pure_asm_x_to_o` helper ✅** · **Darwin mangling ✅** · **`rt_*` pure_asm 23/23 ✅** · **hybrid pure-asm opt-in ✅** · **`PREFER_ASM_O_ONLY` 真 L2 地图 ✅** · **i32 VAR/call-ret/binop sxtw ✅** · **call-arg 栈序 ✅** · **INDEX `**T` 双剥皮闭 ✅** · **typeck_selfhost ndef 误判闭 ✅** · **`ONLY=` IN_NO_C 含 `rt_run_asm_backend` 真 L2 5/5 ✅** · **fixed-array bounds 无 U panic ✅** · **labi pure_asm 12/12 ✅** · **labi hybrid 单 slice RED×0 ✅** · **labi full12 hybrid 双端 L2 5/5 ✅** · **COMPILE residual pure_asm 9/13 双端 ✅** · **L2-asm hybrid pure-asm 3/3 装链 ✅**（user_asm merge residual 闭；**默认仍 -E**） · **SEED_SLICE 不并 no_c** · **产品默认 PREFER_ASM 仍禁** · 最小 seed ⬜ · COMPILE residual 仍需 `$CC`（pipeline_abi mega／FAIL residual×4） |
...
🟡 **12.0.5 asm backend 模块覆盖（`.x`→`.o` 直出替 `-E`+`$CC -c`）**
  ...
  - **labi hybrid 真 L2 地图 ✅**（`try-labi-prefer` + soft g05 + matrix + restore；矩阵须 hybrid 本波 `xlang`）：  
    ...
    - **RED residual 全闭 ✅**：path_pure · ensure_list · invoke_cc_list · ondemand_heavy（ABI 根修后 dual-end）  
  - **path_pure hybrid 闭**：`glue_emit_binop_add` is_64bit → scale1 64-bit ADD  
  - **ensure_list hybrid 闭**：`asm_ctx_local_find_offset_scoped` 限本块  
  - **invoke_cc_list hybrid 闭**：modlet `cell_size` bit30=TYPE_ARRAY LEA  
  - **ondemand_heavy hybrid 闭**：连带 residual 刀后 mac／Ubuntu hybrid **5/5**  
  - **full12 pure-asm hybrid 问题地图 ✅（2026-08-11 · tip `1a66374b1` · 钉盘 `e364f4a37`）**：  
  ...
  - **COMPILE residual prefer 薄叶 pure_asm 问题地图 ✅（2026-08-11 · tip `6a814b52c`）**：  
    ...
    - pure_asm **9/13 双端 OK**：l2asm 全 · tcpu 全 · ldpc · `async_asm_pool` · `seed_link_compat` · `strict_glue_thin`  
    - **FAIL×4**：`async_liveness`／`async_cps_codegen`／`runtime_lsp_glue` U `xlang_panic_` · `fmt_check_cmd_thin` CG002（code_len>0／num_labels=0）  
    - **L2-asm 真 hybrid 装链**（`XLANG_HOST_CC_SEED_FORCE=1`+`PREFER_ASM_O=1` try-l2-asm-prefer → unset → soft g05 → matrix）：  
      - pure-asm thin+rest **3/3 ✅**（含 `user_asm_seed_bridge` merge residual 闭）：  
        根因：FROM_X rest 去掉 debug／emit_trace 体却无 always-visible `extern` 面声明 → ISO C99 implicit-decl；补 `extern int seed_asm_{debug,emit_trace}_enabled(void)`（对标 `seed_elf_ctx_code_len`）  
      - 装链双端 L2 **5/5** · restore **5/5**  
    - 日志：mac `/tmp/xlang_compile_residual_pure_asm_map_*.log` · `/tmp/xlang_l2asm_hybrid_true_*.log` · `/tmp/xlang_uasb_hybrid_true_*.log`；Ubuntu `/tmp/ubu_compile_residual_pure_asm_map.log` · `/tmp/ubu_l2asm_hybrid_true.log` · `/tmp/ubu_uasb_hybrid_true.log`  
  - 下一步：FAIL residual 根修（async panic／fmt CG002／lsp_glue）· tcpu／ldpc hybrid 装链 · labi-only 默认化须明示授权 · pipeline_abi mega 仍禁 pure-asm 
