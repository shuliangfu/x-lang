...
| leftover fossil DOC／catalog no-Honesty zc-dashboard soft→硬绿 | ✅ | 二过：拒 leftover 顶层 `analysis/zc-dashboard-v1.md` 已归档仍当 live DOC（缺文件 → portable ZC-008 红）＋leftover catalog 无 Honesty／缺 `run=`／`obs=`／`skip=`；live＝`analysis/archive/zc/`；顶层复活硬 die；无 XLANG 面故 G.7 不开 resolver；keep `zc-dashboard gate OK`；nested leftover runner 不重写；zc3／zc4／zc5 host-c 仍 leave；archive leftover residual（无新建 Gate）；TSV cross_zc006 xref＝archive；报告 `run=`／`obs=`／`skip=`；双端 L2；**leftover fossil DOC zc-dashboard FAIL 池空**；**leftover catalog no-Honesty comprehensive-check 已收**；余 leftover fossil DOC live 默认仍 `自举前必须清单.md` 四闸／stdlib-check-matrix（check postponed）；win32-*／F-* leftover XLANG fallthrough 仍 leave |
