...
✅ **VAR assign >16B CALL sret（MEMORY dest-across-call）** ✅ @ **`34fdd01d7`**
  ...
  - 余（勿并本叶）：VAR copy >16B（`y=x` 32B Darwin 2）；nest>20（21 须加宽 `type_to_c_repr` 256／碰 pipeline_abi）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
