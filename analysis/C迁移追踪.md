...
✅ **SIMD named `size_simple`＝lanes*esz（idv 16B dual-GP）** ✅ @ **`b76bb2466`**
  ...
  - 余（勿并本叶）：FIELD dest VECTOR CALL（Darwin 32）；INDEX dest VECTOR CALL（Darwin 139）；嵌套 `idv(add4)`（Darwin 52）；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
