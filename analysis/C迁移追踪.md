...
1. **最优真减 host-cc**：P2c parse_cond_expr dest-buffer 已开（INT 后 `as` 回绕 INT 起点再 parse_expr；trampoline 写 next_lex）。P5k leftover parse_if_stmt C twin T-shrink 仍开。P2b parse_one_top_level_let＋type_alias `.x` dest-buffer 仍开。P6e parse_struct_record_layout 仍开。P5j parse_match_into 仍开。P4bi parse_struct_lit_fields 仍开。P4bh remaining parse_primary 仍开。P5i parse_match_struct_fields 仍开。P5h match subject parse 仍开。P7d parse_at_simd_builtin 仍开。P4te parse_assign 仍开。P4td parse_ternary 仍开。P4ad parse_as_suffix 仍开。P4bd parse_binop 仍开。P4ud parse_unary 仍开。P4bg IDENT 头仍开。suffix_loop BODIES 仍开（P4bf）。wrap 族已尽。下刀＝glue_tail 包装／type_ref parse／parse_block_return_end_tail／register_pending／dest-SLICE／anonymous-struct alloc／body_let_bracket。pipeline_abi mega pure-asm 仍是已知地雷。  
