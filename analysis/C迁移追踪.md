...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w214＋w216 asm | 含 wave216 for_call_args thin。下一＝wave217 CALL／METHOD wrappers。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc…w216 for_call_args→`.x` thin。下一叶＝wave217 CALL／METHOD wrappers；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
