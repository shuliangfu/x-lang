...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-17 · 钉盘 **`f7424ae47`** · nest 64 冻帽 · dest leftover 族 **已尽** · g05／ensure parser 冷路径 **pin-first** · **4.2.4 ambient 原语 expected bare 推断 半闭**（phantom 无 ambient 仍 T001）· **4.2.5 multi mono 闭** · **4.2.7 nested reent deep-copy 闭** · TYPE_DYN／vtable 后期 · 只改勾选与事实，无波次流水  
...
🟡 **4.2.4 bare 泛型 ret-only 推断** **半闭**（2026-08-17 ambient 原语 expected 补全）

  - **已闭（本波）**：`let a: i32 = mk_default()`／`return mk_default()`／`let a: i32 = as_t(42)`／`let a: i64 = mk()` → asm／host **绿**；host-C `mk_default__i32`
  - **根修（G.7 有则补全）**：`typeck_try_infer_generic_call_from_args` ret-only 接受 **任意 fully concrete** ambient expected（prim＋module NAMED；禁 free-T tree）；`typeck_generic_call_fixup_resolved_type` bare 同 stamp `resolved_type_ref`。权威 `typeck.x`＋`typeck_gen.c`＋`seeds/typeck_gen.linux.x86_64.c` 同 commit
  - **仍 leave-off**：无 ambient 的 phantom T（`unit_t()`／`forty_two()` ret 非 free T）→ 诚实 **T001** requires type arguments；须 turbofish
  - 值参路径回归：`id(42)` **42**；turbofish `mk_default<i32>()` **42**
