...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-07（对照 tip residual：glue／ast_pool host 壳 + 域叶 thin／leave；bc-inventory present residual **56**／ROWS=119；**只改勾选与事实 LOC**，无波次流水）
...
| `pipeline_typeck_method_call.c` | **~1262** | typeck method_call + generic UFCS mono | 🟡 **G.7** generic UFCS／CALL fixup／mono-map／type-args＝typeck.x；残 weak method_call_c／import／dep map；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_check_block.c` | **~484（thin）** | typeck check_block 域 Cap residual thin | 🟡 **G.7 live walker＝typeck.x**（`typeck_check_block*`）；residual 仅 ctx／depth／linear／has_implicit + thin faces；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_region_assign.c` | **~240（thin）** | typeck region/escape assign-site 切片 | 🟡 **G.7** region／escape／stack_local／return_slice＝typeck.x；残 thin `*_c` faces；仍 host-cc 入 `pipeline_x` |
