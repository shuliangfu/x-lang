...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w338 COMMON**；**w339–w342** Cap A；**w344 .data bake**；**w344b check_expr PREFER ban**；bootstrap→类 B；**w335／w337 ban** 余 | 下一＝check_expr dual-home／block_tree／GrowVec-LE。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w344 ordinal→.data bake**；**w344b check_expr PREFER ban**；下一＝dual-home／block_tree／GrowVec-LE；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
