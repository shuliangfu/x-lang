...
> **刷新**：2026-09-23 · 状态债 · **三端 L2 硬闸已启用** ✅；BC present **0**；AD–AO Cap／diagnostic 剥 ✅；**Class AP** Darwin pipeline_abi leftover 拼装半刀 ✅（Cap／真减续 🟡）· 钉盘 **`ecdb5cc1e`**（不升）
...
| 非 gen 产品 C／8.3 | 🟡 | wave309 壳物理退役（present 0）；产品 `.inc`／from_x／其它 host-cc seed 仍开；AP Darwin leftover 拼装半刀已收 |
...
| BC（编译层零 host-cc） | 🟡 | inventory present **0**（Class O）；余量＝map 外 from_x／seed／`.inc`／大户 Cap／真减（AP 拼装半刀后） |
| PC（产品默认 asm） | 🟡 | 门控已收；`labi_invoke_cc` 未删 |
| `pipeline_abi` mega pure-asm | 🟡 | 已绿 PREFER 面同上。**Class AP**：Darwin leftover 拼装半刀已收（tip 新链 L2 5/5；禁 FORCE）。Cap／真减 host-cc 续。HARD BAN／leftover-first／mega FORCE／`-E` 当修仍禁。 |
| nest 冻帽 | ✅ | **64** |
