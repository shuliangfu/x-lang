...
✅ **INDEX dest VECTOR CALL let-init** ✅ @ **`6141c91e2`**
  ...
  - 余（勿并本叶）：嵌套 `arr[0][0]`（Darwin 174）；嵌套 `idv(add4)`（Darwin 20）；FIELD-as-receiver `h.v.sub4`（Darwin 22）；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
