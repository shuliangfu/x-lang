...
✅ **Vec4f 逐 lane divss** ✅ @ **`11bc90275`**
  ...
  - 余（勿并本叶）：METHOD binop2 仍 CALL-only（本地 CALL `add(a,b)` 已 0）；i32x4 `/` 后 lane 仅 mac 红（Ubuntu 0；idiv／AAPCS64 另层）；nest>16 soft；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off
