...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋w220–224／w261–w266 C | 含 wave266 struct_layout 域 C thin（双端 L2 tip `a9ff3dfa7`）。下一＝wave267 asm_locals 域。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc…w219 `.x`；w220–224／w261–w266 **C thin**。下一叶＝wave267 asm_locals 域；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
