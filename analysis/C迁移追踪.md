...
| `pipeline_asm_emit_index_helpers.c` | ~946 | asm ELF INDEX residual helpers（slot+esz+try_index forest residual＋soa＋CAP BSS）切片 | 🟡 已抽出；wave178–184 pure：peel／slot／type／assign-addr／base／nested／eff_addr→rax；wave185 pure：lvalue_eff_addr elf+text；余 soa／field_type_ref／CAP BSS／stack_off 仍 host-cc |
