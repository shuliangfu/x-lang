...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w338 COMMON**；**w339–w340** typeck_active／emit_ctx_module_dep LINUX PREFER／DARWIN `-E`；**w335／w337 ban** 余 A/B/C | Cap A null-ptr 两叶。下一＝sret／bss 或 GrowVec-LE。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w340 Cap A emit_ctx_module_dep**（LINUX PREFER／DARWIN `-E`）。下一＝sret／bss 或 GrowVec-LE；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
