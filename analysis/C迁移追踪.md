...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-04（对照 tip residual：glue ~1.8k／ast_pool ~0.18k host 壳 + **early/mid/backend/typeck/typeck_mid/emit/emit_block_fwd 壳** + **AsmFuncCtxLayout 早域** + emit-heavy env／backend wrapper／scratch 已抽／ELF write + ELF ctx + type-to-c + skip/force + struct emit + residual + asm locals + slot bytes + block tree + ctx loop + WPO + selfhost + thin_delegate（五表全） + safe_helper（skip_heavy 全） + parser_emit_heavy pure leave；**bc-inventory 已诚实对齐**（glue/ast_pool 壳 min_loc 按后 thin 现实；域叶 ~110 present residual rows 入账）；**只改勾选与事实 LOC**，无波次流水）  
...
| `pipeline_glue_typeck_fwd.c` | **~325** | glue typeck pure fwd/extern（ast_forwarders 后、assign 前） | 🟡 **已抽出**；仍 host-cc |
| `pipeline_glue_typeck_mid_fwd.c` | **~286** | glue typeck mid pure fwd/extern（field_access 后、region_assign 前） | 🟡 **已抽出**；仍 host-cc |
| `pipeline_glue_emit_fwd.c` | **~113** | glue early emit inter-include pure fwd/static（logand 后、STRUCT_LIT 前） | 🟡 **已抽出**；仍 host-cc |
...
| `pipeline_typeck_assign.c` | **~108（thin）** | typeck assign 域 Cap residual thin（wave225 pure leave） | 🟡 **G.7 live＝typeck.x**（`typeck_check_expr_assign` + `typeck_diag_*`）；Cap residual 仅 C ABI face + `module_num_imports`；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_orch.c` | **~242（thin）** | typeck orch 域 Cap residual thin（wave228 pure leave） | 🟡 **G.7 live orch＝typeck.x**（`typeck_x_ast*`）；residual 仅 thin faces + weak soft-suppress／dep_ctx／dep_prerun + layout glue；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_check_expr.c` | **~547（thin）** | typeck check_expr 域 Cap residual thin（wave234 field_type／repr／extern_unsafe + wave232 call + wave231 match／try + wave229 return） | 🟡 **G.7** panic／unary／addr／index／deref／var／return／match／try／call／field_type／repr／extern_unsafe＝typeck.x thin；match subject BSS＋mega region＋method_call 仍 residual；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_coerce_init.c` | **~428（thin）** | typeck coerce-init 域 Cap residual thin（wave233 int_binop／resolve_alias／int_lit + wave231 struct_lit + wave230 type_refs） | 🟡 **G.7 live coerce＝typeck.x**（`typeck_coerce_init_*`＋ret_coerce／float_widen＋type_refs／integer_widen／bool／expr_type_ref／struct_lit／int_binop／resolve_alias／int_lit）；residual 仅 float_bits + thin faces；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_method_call.c` | **~1262** | typeck method_call + generic UFCS mono（wave252：generic UFCS／CALL fixup → typeck.x；wave251 mono-map／subst；wave250 type-args gate；残 weak method_call_c／import／dep map） | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_check_block.c` | **~484（thin）** | typeck check_block 域 Cap residual thin（wave226 pure leave） | 🟡 **G.7 live walker＝typeck.x**（`typeck_check_block*`）；residual 仅 ctx／depth／linear／has_implicit + thin faces；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_region_assign.c` | **~240（thin）** | typeck region/escape assign-site 切片（wave246：return_slice_in_scope pure leave + wave245 ptr_for_addr＋stack_local + … → typeck.x；残 thin *_c faces only） | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
...
| 2026-08-07 | **feat(bc)** typeck generic UFCS／CALL fixup pure leave (wave252) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。UFCS＋fixup＋subst_ret_from_formal_map→typeck.x；LANG-007 patch strong pure deref 破环；residual 删第二 body；method_call～1262；present **56**。下刀：method_call_c／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck generic UFCS＋CALL fixup. |
| 2026-08-07 | **feat(bc)** typeck mono-map／pattern-unify／subst pure leave (wave251) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。pattern_unify／subst／build_value_formal_mono_map→typeck.x flat stride-128；type-pool set_elem_array_size 有则补全；residual 删 mono 第二 body；method_call～1892；present **56**。下刀：method_call 残（UFCS／fixup）／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck generic mono map engine. |
| 2026-08-07 | **feat(bc)** typeck generic type-args／try_infer／bounds pure leave (wave250) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。check_call_generic_type_args／try_infer／bounds→typeck.x；residual 删第二 body；method_call～2242；present **56**。下刀：method_call 残（UFCS／fixup／mono-map）／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck generic call type-args gate. |
| 2026-08-07 | **feat(bc)** typeck mono foundation pure leave (wave249) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。named_is_module_type／call_arg_effective／type_tree_has_free_param→typeck.x；free-param＝inverted named 有则补全；residual 删第二 body；method_call～2660；present **56**。下刀：method_call 残（UFCS／fixup／try_infer）／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck generic mono foundation. |
| 2026-08-07 | **feat(bc)** typeck overload pick／resolve pure leave (wave248) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。for_emit／pick_for_call→typeck.x（score＝typeck_overload_arg_param_score）；residual 删第二 score 集群；method_call～2812；present **56**。下刀：method_call 残（generic UFCS／fixup）／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck CALL overload resolve. |
| 2026-08-07 | **feat(bc)** typeck call-resolve pure leave (wave247) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。resolve_call_callee_return_type_c→typeck.x；import faces thin→typeck_*；删 static equal helpers；method_call～3209；present **56**。下刀：method_call 残／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck CALL target resolve. |
| 2026-08-07 | **feat(bc)** typeck return_slice_in_scope pure leave (wave246) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。return_slice_region_in_scope→typeck.x；residual 删第二 body＋expr_diag_line_col；region_assign～240 thin；present **56**。下刀：method_call／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck region escape on return. |
| 2026-08-07 | **feat(bc)** typeck ptr_for_addr＋stack_local pure leave (wave245) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。ptr_for_addr_of→typeck.x；`pipeline_type_find_or_alloc_ptr` type-pool face；residual 删 stack_local／store-scan／第二 body；region_assign～286；present **56**。下刀：method_call／return_slice_in_scope／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck stack-local stamp. |
| 2026-08-07 | **feat(bc)** typeck one_region＋call_struct pure leave (wave244) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。one_region／call_struct_stack_escape→typeck.x；var_is_block_local 补全 block-tree walk；residual 删第二 body；region_assign～740；present **56**。下刀：stack_local／ptr_for_addr／method_call／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck region／CALL escape. |
| 2026-08-07 | **feat(bc)** typeck stamp_let＋read_ptr pure leave (wave243) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。is_read_ptr／read_ptr_return／stamp_block_let→typeck.x；residual 删第二 body；region_assign～901；present **56**。下刀：call_struct／stack_local／one_region／method_call／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck stamp／read_ptr. |
| 2026-08-07 | **feat(bc)** typeck post-scan tree pure leave (wave242) | **代码** · **不升钉** · **mac L2**（Ubuntu 金标待补）。scan_expr／scan_block／scan_module→typeck.x；residual 删 scan 体；call_struct_stack_escape 升 public residual；region_assign～977；present **56**。下刀：stamp_let／read_ptr／call_struct／method_call／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck post-scan. |
| 2026-08-07 | **feat(bc)** typeck region-label scope BSS pure leave (wave241) | **代码** · **不升钉** · **mac L2**（Ubuntu SSH 暂断待补）。region-label nest BSS＋push／pop／len／reset→typeck.x；residual region_assign thin U pure；region_assign～1257；present **56**。tip **`48517a824`**。下刀：scan tree／method_call／elf／ast_pool／pipeline_x mega／Ubuntu 补。PLATFORM: SHARED freestanding typeck region cells. |
| 2026-08-07 | **feat(bc)** typeck with_arena nest BSS pure leave (wave240) | **代码** · **不升钉** · **Ubuntu L2**。with_arena nest BSS＋push／pop／reset→typeck.x；residual region_assign thin U pure；region_assign～1324；present **56**。tip **`00db73543`**。下刀：scan tree／method_call／region-label／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck nest cells. |
| 2026-08-07 | **feat(bc)** typeck call_slice region pure leave (wave239) | **代码** · **不升钉** · **Ubuntu L2**。call_slice＋*Struct compat→typeck.x；method_call dual static 退役；region_assign～1347；present **56**。tip **`75bd48b38`**。下刀：with_arena BSS／scan tree／method_call／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck allocator region pure leave (wave237) | **代码** · **不升钉** · **Ubuntu L2**。allocator assign＋return→typeck.x；with_arena BSS faces residual export；suffix `heap.Allocator` 补全；region_assign～1470；present **56**。tip **`4d6eac944`**。下刀：ctfe／with_arena／call_slice／method_call／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck stack-escape＋scope-borrow pure leave (wave236) | **代码** · **不升钉** · **Ubuntu L2**。stack_escape／scope_borrow assign＋return→typeck.x；`pipeline_block_parent_block_ref_at`；residual thin；region_assign～1535；present **56**。tip **`578978083`**。下刀：ctfe／with_arena／allocator／call_slice／method_call／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck field_type＋call_arg_repr＋extern_unsafe pure leave (wave234) | **代码** · **不升钉** · **Ubuntu L2**。field_type／call_arg_repr／extern_unsafe→typeck.x；residual thin；match subject BSS 仍 residual；check_expr～547；present **56**。tip **`97341a69a`**。下刀：ctfe／region／method_call／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck int_binop＋resolve_alias＋int_lit pure leave (wave233) | **代码** · **不升钉** · **双端 L2**。int_binop／resolve_alias／int_lit→typeck.x；residual thin；type_refs_equal peel typeck；coerce～428；present **56**。tip **`c46dfddb1`**。下刀：ctfe／region／method_call／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck call generic fixup pure leave (wave232) | **代码** · **不升钉** · **双端 L2**。call 编排＋generic type-args／mono fixup→typeck.x；residual call_c thin；export generic faces；check_expr～705；present **56**。tip **`a39ce4af5`**。下刀：ctfe／region／method_call／int_binop／resolve_alias／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck match＋try＋struct_lit pure leave (wave231) | **代码** · **不升钉** · **双端 L2**。match subject＋iter／try Result_?／struct_lit 回填→typeck.x；residual thin；check_expr～792 · coerce～578；present **56**。tip **`54499d041`**。下刀：ctfe／region／method_call／call generic／int_binop／resolve_alias／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck type_refs＋integer_widen pure leave (wave230) | **代码** · **不升钉** · **双端 L2**。typeck.x type_refs_equal 破 wrap 环；residual type_refs／integer_widen／bool／expr_type_ref→typeck twins；coerce～612；present **56**。tip **`6e66ad3ea`**。下刀：ctfe／region／method_call／match／call／try／int_binop／struct_lit／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck return＋ret_coerce／float_widen pure leave (wave229) | **代码** · **不升钉** · **双端 L2**。return_c→typeck_check_expr_return；float_widen／return_operand／ret_coerce→typeck twins；check_expr～902 · coerce～916；present **56**。tip **`9fa488518`**。下刀：ctfe／region／method_call／match／call／try／type_refs／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck orch＋check_expr domain pure leave (wave228) | **代码** · **不升钉** · **双端 L2**。orch～242 thin→typeck_x_ast*；check_expr～1027 thin panic／unary／addr／index／deref／var；match／return／call／try 残；present **56**。tip **`69e737c60`**。下刀：ctfe／region／method_call／check_expr match-return-call-try／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck_coerce_init domain pure leave (wave227) | **代码** · **不升钉** · **双端 L2**。B′ G.7 coerce-init residual→typeck.x thin；Cap residual～1019（int_binop／struct_lit／type_refs 仍）；present **56**。tip **`b21779e69`**。下刀：ctfe／region／method_call／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck_check_block domain pure leave (wave226) | **代码** · **不升钉** · **双端 L2**。B′ G.7 check_block walker residual→typeck.x thin；Cap residual～484（linear／has_implicit 仍）；present **56**。tip **`f493f3434`**。下刀：coerce／ctfe／region／method_call／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck_assign domain pure leave (wave225) | **代码** · **不升钉** · **双端 L2**。B′ G.7 assign+diag-fmt residual→typeck.x thin；Cap residual～108；present **56**。tip **`3464ab3b4`**。下刀：check_block／coerce／ctfe／region／method_call／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** typeck_active module BSS pure leave (wave224) | **代码** · **不升钉** · **双端 L2**。B′ pure typeck_active_module get+set→runtime_pipeline_abi；Cap residual check_block／assign→pure setter；ctfe／coerce→pure getter；glue_statics 0 residual BSS；seed cold twin；产品 `./xbuild` only。present **56**。tip **`0e48f6630`**。**效率转向**停 micro-cell → 域 pure leave typeck／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
| 2026-08-07 | **feat(bc)** emit_ctx sret BSS pure leave (wave223) | **代码** · **不升钉** · **双端 L2**。B′ pure sret_active／home_off／ret_sz get+set→runtime_pipeline_abi；Cap residual mega_body→pure setters；typeck_active 仍 residual；seed cold twins −1；产品 `./xbuild` only。present **56**。tip **`6dde4f611`**。下刀：typeck_active／typeck／elf／ast_pool residual／pipeline_x mega。PLATFORM: SHARED freestanding sret cells. |
| 2026-08-07 | **feat(bc)** emit_ctx module／dep_pipe BSS pure leave (wave222) | **代码** · **不升钉** · **双端 L2**。B′ pure module／dep_pipe get+set→runtime_pipeline_abi；Cap residual bind＋struct_layout→pure faces；sret／typeck_active 仍 residual；seed cold twins −1；产品 `./xbuild` only。present **56**。tip **`5119edba5`**。下刀：sret／typeck_active／typeck／elf／ast_pool residual／pipeline_x mega。PLATFORM: SHARED freestanding emit module／dep. |
| 2026-08-07 | **feat(bc)** emit_ctx accessor-only BSS pure leave (wave221) | **代码** · **不升钉** · **双端 L2**。B′ pure emit_ctx func_index／arena／call_param_ty／call_arg_depth／elf_ctx／scope_block get+set＋host_is_arm64→runtime_pipeline_abi；Cap residual statics 保留 module／dep／sret／typeck 直写；seed cold twins −1；产品 `./xbuild` only。present **56**。tip **`70536e195`**。下刀：residual module／dep／sret／typeck_active／typeck／elf／ast_pool residual／pipeline_x mega。PLATFORM: SHARED freestanding emit context. |
| 2026-08-07 | **feat(bc)** if_expr_arm_emit_depth BSS pure leave (wave220) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_if_expr_arm_emit_depth_get/set＋BSS→runtime_pipeline_abi；Cap residual emit_fwd→extern；seed cold twin −1；产品 `./xbuild` only。present **56**。tip **`3eeb07bb0`**。下刀：typeck／elf／ast_pool residual／pipeline_x mega。PLATFORM: SHARED freestanding if/ternary arm nest depth. |
| 2026-08-07 | **feat(bc)** al_nc_seq COMMON label seq pure leave (wave219) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_pipeline_asm_al_nc_seq_take_c＋BSS→runtime_pipeline_abi；Cap residual glue body→extern；seed cold twin −1；产品 `./xbuild` only。present **56**。tip **`0da82a797`**。下刀：typeck／elf／ast_pool residual／pipeline_x mega。PLATFORM: SHARED freestanding emit label uniqueness. |
| 2026-08-07 | **feat(bc)** Cap residual emit shells host-cc leave (wave218) | **代码** · **不升钉** · **双端 L2**。删 call_args／spill／index_helpers prototype shells；early fwd→backend_fwd；present **56**。tip **`b0eb6991e`**。下刀：typeck／elf／ast_pool residual／pipeline_x mega。PLATFORM: SHARED freestanding Cap residual emit leave. |
| 2026-08-07 | **feat(bc)** CALL／METHOD text thin wrappers pure leave (wave217) | **代码** · **不升钉** · **双端 L2**。B′ pure pipeline_asm_emit_expr_call_c＋method_call_c→runtime_pipeline_abi；Cap residual call_args shell～623；seed cold twins −1；产品 `./xbuild` only。present **59**。tip **`2218c25a7`**。下刀：pipeline_x mega。PLATFORM: SHARED freestanding text M8-tail. |
| 2026-08-07 | **feat(bc)** for_call_args mega pure leave (wave216) | **代码** · **不升钉** · **双端 L2**。B′ pure pipeline_asm_emit_expr_elf_for_call_args→runtime_pipeline_abi；Cap residual call_args shell～639；seed cold twin −1；产品 `./xbuild` only。present **59**。tip **8cac954d0**。下刀：CALL thin wrappers／pipeline_x mega。PLATFORM: SHARED freestanding CALL-arg packing. |
| 2026-08-07 | **feat(bc)** live set arrays pure leave re-land (wave215) | **代码** · **不升钉** · **双端 L2**。B′ pure re-land wave214 after dual-GP packing 根修：live set BSS + loop stacks + opaque u8 thins + fill_live_end／set_from_expr_uses／peak_clear；Cap residual spill→shell ~434；seed cold twins；产品 `./xbuild` only。present **59**。tip **f908695e2**。下刀：for_call_args mega／CALL thin wrappers／pipeline_x mega。PLATFORM: SHARED freestanding 7.3. |
| 2026-08-07 | **feat(bc)** live control＋interf BSS pure leave (wave213) | **代码** · **不升钉** · **双端 L2**。B′ pure cfg_parent／active／emit_stmt／linear_max／pressure／interf graph＋push／pop_merge／prepare／linear_ctx + pure BSS；Cap residual→prototype；peak_clear residual；seed cold twins；产品 `./xbuild` only。spill ~1238；present **59**。下刀：for_call_args mega／CALL thin wrappers／spill live set arrays／pipeline_x mega。PLATFORM: SHARED freestanding 7.3. tip **2b749eb91**. |
| 2026-08-07 | **feat(bc)** Chaitin color／pin BSS pure leave (wave212) | **代码** · **不升钉** · **双端 L2**。B′ pure pin／color map／cfg_coloring／final_expr_use_n／stack_spill_enabled／var_prefers_stack_spill + pure BSS；Cap residual→prototype；seed cold twins；产品 `./xbuild` only。spill ~1525；present **59**。下刀：for_call_args mega／CALL thin wrappers／spill live_fwd＋interf BSS／pipeline_x mega。PLATFORM: SHARED freestanding 7.3. tip **9843f1351**. |
| 2026-08-07 | **feat(bc)** binop stack_spill try_reload enc pure leave (wave211) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_binop_stack_spill_try_reload_elf_c（stack_spill find_depth + CAP depth + VAR stamp + arm64 ldr_sp_slot）；Cap residual→prototype；seed cold twin；产品 `./xbuild` only。spill ~1616；present **59**。下刀：for_call_args mega／CALL thin wrappers／spill live_fwd＋color／interf BSS／pipeline_x mega。PLATFORM: SHARED freestanding 7.3. tip **5b779cbe7**. |
| 2026-08-07 | **feat(bc)** binop VAR slot cache BSS pure leave (wave210) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_binop_var_slot_cache_* + flattened BSS（rax/rbx + x10–x15）；Cap residual→prototype；try_reload pure setters；seed cold twins；产品 `./xbuild` only。spill ~1643；present **59**。下刀：for_call_args mega／CALL thin wrappers／spill live_fwd＋color／interf BSS／try_reload enc／pipeline_x mega。PLATFORM: SHARED freestanding 7.3. tip **2491a79fb**. |
| 2026-08-07 | **feat(bc)** CAP cache BSS pure leave (wave209) | **代码** · **不升钉** · **双端 L2**。B′ pure minus_pair／subadd3 CAP cache BSS + thin clear／valid／slot／ctx／keys／record + caches_hit_var；Cap residual→prototype；seed cold twins；产品 `./xbuild` only。spill ~1786；index_helpers ~659；present **59**。下刀：for_call_args mega／CALL thin wrappers／spill VAR＋color BSS／pipeline_x mega。PLATFORM: SHARED freestanding 7.3. tip **18df7efd8**. |
| 2026-08-07 | **feat(bc)** stack_spill table BSS pure leave (wave208) | **代码** · **不升钉** · **双端 L2**。B′ pure append／cap／clear／drop／find／n_get／off_at + pure BSS off[12]／at_depth／n；Cap residual→prototype；seed cold twins；产品 `./xbuild` only。spill ~1897；present **59**。下刀：for_call_args mega／CALL thin wrappers／spill VAR＋color BSS／CAP cache／pipeline_x mega。PLATFORM: SHARED freestanding 7.3. tip **2d9705840**. |
| 2026-08-07 | **feat(bc)** CAP BSS thin accessors pure leave (wave207) | **代码** · **不升钉** · **双端 L2**。B′ pure rax_frame_spill push/pop/depth/n_set + scratch_depth get/set + pure BSS；Cap residual→prototype；spill 改 getter；seed cold twins；产品 `./xbuild` only。index_helpers ~655；present **59**。下刀：for_call_args mega／CALL thin wrappers／spill VAR＋color BSS／CAP cache／pipeline_x mega。PLATFORM: SHARED freestanding 7.3. tip **e839773a0**. |
| 2026-08-07 | **feat(bc)** CALL expected_ret_ty BSS pure leave (wave206) | **代码** · **不升钉** · **双端 L2**。B′ pure set/call_expected_ret_ty_c + pure BSS；Cap residual→prototype；seed cold twin working BSS；产品 `./xbuild` only。call_args ~1176；present **59**。下刀：for_call_args mega／CALL thin wrappers／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED freestanding. tip **8a04644b7**. |
| 2026-08-07 | **feat(bc)** TYPE_SLICE reent deep_copy pure leave (wave205) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_slice_let_reent_deep_copy_after_dual_gp_elf_c（use_frame 0/1）；Cap residual→prototype；seed cold twin；产品 `./xbuild` only。call_args ~1198；present **59**。下刀：for_call_args mega／expected_ret_ty BSS／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED freestanding. tip **8f6defef0**. |
| 2026-08-07 | **feat(bc)** store_retval_pair + copy_large pure leave (wave204) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_store_retval_pair_to_rbp_elf_c + private copy_large_struct；deep_copy static→public Cap residual；seed cold twin；产品 `./xbuild` only。call_args ~1596；present **59**。下刀：for_call_args mega／deep_copy／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED freestanding · SysV/AAPCS64. tip **45ac55923**. |
| 2026-08-07 | **feat(bc)** CALL-arg VAR stack_off resolve pure leave (wave203) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_call_arg_resolve_var_stack_off_elf_c + private unusable／append_at_offset／anon body-let；Cap residual→prototype；seed cold twin；产品 `./xbuild` only。call_args ~1758；present **59**。下刀：for_call_args mega／store_retval／deep_copy／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED. tip **c4a6ef8c7**. |
| 2026-08-07 | **feat(bc)** MEMORY by-value CALL-arg pure leave (wave202) | **代码** · **不升钉** · **双端 L2**。B′ pure pipeline_asm_push_sysv_memory_by_value_elf_c + pipeline_asm_store_memory_by_value_to_sp_elf_c；Cap residual→prototype；seed cold twins；产品 `./xbuild` only。call_args ~1859；present **59**。下刀：for_call_args mega／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: LINUX+MACOS x86_64 SysV + MACOS\|ARM64 AAPCS64. tip **c7a5ca40d**. |
| 2026-08-07 | **feat(bc)** SysV f32 xmm param home pure leave (wave201) | **代码** · **不升钉** · **双端 L2**。B′ pure pipeline_asm_emit_param_home_elf_sysv_f32_xmm_c + private is_f32/is_f64；Cap residual→prototype；seed cold twin；产品 `./xbuild` only。call_args ~2070；present **59**。下刀：for_call_args mega／MEMORY push／store／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: LINUX+MACOS x86_64 SysV. tip **0c4da0ad2**. |
| 2026-08-07 | **feat(bc)** f32 VAR slot load pure leave (wave200) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_load_f32_var_slot_to_rax/rbx + private is_param；Cap residual→prototype；seed cold twins；产品 `./xbuild` only。call_args ~2242；present **59**。下刀：for_call_args mega／MEMORY／param home／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED. tip **501476728**. |
| 2026-08-06 | **feat(bc)** call_return byte size pure leave (wave194) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_call_return_byte_size_c；resolve static→public Cap residual；seed cold twin + 双 endif 回补；产品 `./xbuild` only。call_args ~2848；present **59**。下刀：for_call_args mega／resolve pure／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED. tip **7c286e11c**. |
| 2026-08-06 | **feat(bc)** call_args param agg＋VAR dual-GP load pure leave (wave193) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_func_param_agg_byte_size_c + glue_load_var_as_value_to_rax_rdx_elf_c；Cap residual→extern；seed cold twins；产品 `./xbuild` only。call_args ~2902；present **59**。下刀：for_call_args／call_return／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED. tip **7a4dd7e43**. |
| 2026-08-06 | **feat(bc)** call_args dual-GP＋home＋return size pure leave (wave192) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_sysv_dual_gp_byte_size_c + glue_func_param_home_width_c + glue_func_return_byte_size_c；Cap residual→extern；seed cold twins；产品 `./xbuild` only。call_args ~3057；present **59**。下刀：load_var dual-GP／for_call_args／agg／call_return／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** call_args named layout size＋pass_addr pure leave (wave191) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_type_named_layout_size_any_module_elf_c + glue_call_param_named_struct_pass_addr_elf_c；Cap residual→extern；seed cold twins；产品 `./xbuild` only。call_args ~3202；present **59**。下刀：dual-GP／for_call_args／return sizing／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED. tip **283c51d11**. |
| 2026-08-06 | **feat(bc)** call_args named_struct＋lea pure leave (wave190) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_type_ref_is_named_struct_layout_elf_c + glue_call_arg_var_use_lea_not_load_elf_c；Cap residual→extern；seed cold twins；同波 r3 prefer prologue→rt_prefer（562955c5c）；产品 `./xbuild` only。present **59**。下刀：call_args 余面／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED. tip **9d3c48dc9**. |
| 2026-08-06 | **feat(bc)** module_from_ctx＋needs_ptr pure leave (wave189) | **代码** · **不升钉** · **双端 L2**。B′ pure module_from_ctx + needs_ptr + array_slot（public）+ struct_slot（0）+ private param_ptr；Cap residual bodies→extern；seed cold twins；index_helpers ~685；present **59**。下刀：call_args／spill BSS／CAP BSS／pipeline_x mega。PLATFORM: SHARED. tip **8d6d38e31**. |
| 2026-08-06 | **feat(bc)** VAR stack_off pure leave (wave188) | **代码** · **不升钉** · **双端 L2**。B′ pure stack_off（reuse w148 scoped）；Cap residual body→extern；seed cold twin −1；index_helpers ~774；present **59**。下刀：call_args／spill BSS／local_slot／pipeline_x mega。PLATFORM: SHARED. tip **996ec4bcb**. |
| 2026-08-06 | **feat(bc)** field_type_ref pure leave (wave187) | **代码** · **不升钉** · **双端 L2**。B′ pure field_type_ref + layout name match（reuse name_eq + suffix）；Cap residual body→extern；seed cold twin 0；index_helpers ~782；present **59**。下刀：call_args／spill BSS／pipeline_x mega。PLATFORM: SHARED. tip **5ae5709c6**. |
| 2026-08-06 | **feat(bc)** lvalue_eff_addr ELF+text pure leave (wave185) | **代码** · **不升钉** · **双端 L2**。B′ pure lvalue_eff_addr_{elf,text}；Cap residual bodies→extern；seed cold twins；index_helpers ~946；present **59**。下刀：soa／call_args／spill BSS。PLATFORM: SHARED. tip **18aaa7561**. |
| 2026-08-06 | **feat(bc)** try_index eff_addr→rax forest pure leave (wave184) | **代码** · **不升钉** · **双端 L2**。B′ pure 16× eff_addr→rax；Cap residual bodies→extern；seed cold twins；同波根修 x86 IMUL dest；index_helpers ~1129；present **59**。下刀：lvalue／call_args／spill BSS。PLATFORM: SHARED. tip **62d33ed7c**. |
| 2026-08-06 | **feat(bc)** try_index nested add3/subadd3 addr pure leave (wave183) | **代码** · **不升钉** · **双端 L2**。B′ pure 10× nested assign-addr→rbx；Cap residual bodies→extern；seed cold twins；index_helpers ~1870；present **59**。下刀：eff_addr→rax／lvalue／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** try_index base_to_rax\|rbx pure leave (wave182) | **代码** · **不升钉** · **双端 L2**。B′ pure 2× base_to_rax|rbx；Cap residual bodies→extern；seed cold twins；index_helpers ~2371；present **59**。下刀：nested／eff_addr／lvalue／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** try_index simple assign-addr forest pure leave (wave181) | **代码** · **不升钉** · **双端 L2**。B′ pure 8× lit/idx/plus_lit/plus_var/minus_lit/minus_var/mul_lit/mul_var addr_to_rbx；base_to_rbx residual export；seed cold twins；index_helpers ~2629；present **59**。下刀：base／nested／eff_addr／lvalue／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** INDEX type/stride helpers pure leave (wave180) | **代码** · **不升钉** · **双端 L2**。B′ pure 3× fixed_array_total_bytes + index_elem_byte_sz_from_type_ref + var_expr_type_ref_with_decl_fallback；Cap residual bodies→extern；seed cold twins；index_helpers ~2907；present **59**。下刀：try_index emit／base／lvalue／call_args／spill BSS。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** INDEX local-slot＋field-deref pure leave (wave179) | **代码** · **不升钉** · **双端 L2**。B′ pure 4× enc_local_slot + index_deref_ptr_field_slot；Cap residual bodies→extern；seed cold twins；index_helpers ~3092；present **59**。下刀：try_index emit／call_args／spill BSS。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** INDEX expr shape peel forest pure leave (wave178) | **代码** · **不升钉** · **双端 L2**。B′ pure 6× glue_index_expr_var_* peel；Cap residual try_index 调 pure；seed cold twins；index_helpers ~3132；present **59**。下刀：try_index emit／call_args／spill BSS。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** color thin mov/try/farthest/pick pure leave (wave175) | **代码** · **不升钉** · **双端 L2**。B′ pure mov_reg_to_spill + try_colored + farthest + pick_evict；Cap residual VAR cache BSS + thin set_spill_slot／off_is_spill_pin／enabled／max_live／color which／next_use；seed cold twins；spill ~2130；present **59**。下刀：index_helpers／call_args／CFG phi 余面。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** spill_reg＋evict_rax|rbx pure leave (wave174) | **代码** · **不升钉** · **双端 L2**。B′ pure spill_reg_to_spill + evict_rax|rbx；Cap residual VAR cache BSS + thin mov/colored/pick/farthest/enabled/max_live；seed cold twins；spill ~2339；present **59**。下刀：Chaitin thin／index_helpers／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** left_assoc＋live_fwd before/apply pure leave (wave173) | **代码** · **不升钉** · **双端 L2**。B′ pure left_assoc + live_fwd_before_stmt + apply_top_stmt；Cap residual VAR cache／live BSS + thin accessors + spill_reg／evict thin；seed cold twins；spill ~2360；present **59**。下刀：spill_reg／evict／index_helpers／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** minus_pair／subadd3 cache spill pure leave (wave172) | **代码** · **不升钉** · **双端 L2**。B′ pure minus_pair spill/hit + subadd3 spill_store/hit/pop_top；Cap residual CAP cache BSS + key hash + thin valid/slot/ctx/keys/record；seed cold twins；spill ~2374；present **59**。下刀：spill 余面／index_helpers／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** index-scratch enc push/reload pure leave (wave171) | **代码** · **不升钉** · **双端 L2**。B′ pure enc push/reload/pop + reload_slot(_to_rbx)；Cap residual CAP depth + minus_pair／subadd3 helpers；seed cold twins；spill ~2358；present **59**。下刀：minus_pair／subadd3／index_helpers／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** binop try_reload pure leave (wave170) | **代码** · **不升钉** · **双端 L2**。B′ pure glue_binop_try_reload_spill_off_elf_c；Cap residual stack_try_reload thin + cache BSS；seed cold twin；spill ~2392；present **59**。下刀：enc/reload／cache spill helpers／index_helpers／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** index-scratch pure leave (wave169) | **代码** · **不升钉** · **双端 L2**。B′ pure cleanup_all + invalidate_var + binop_stack_spill_push；Cap residual CAP BSS + thin depth_get/pop/hit/append/cap；seed cold twins；spill ~2485；present **59**。下刀：spill 余面／index_helpers／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** cfg simulate walk pure leave (wave168) | **代码** · **不升钉** · **双端 L2**。B′ pure simulate_cfg_live + from_empty；Cap residual thin copy/gen_kill/apply/interf_push/pop/final_n_set；seed cold twins；spill ~2468；present **59**。下刀：index-scratch／index_helpers／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** cfg peak+color pure leave (wave167) | **代码** · **不升钉** · **双端 L2**。B′ pure note_cfg_live_peak + compute_cfg_peak_live_and_color；Cap residual thin prepare/simulate/bind/peak；seed cold twins；spill ~2547；present **59**。下刀：cfg simulate／index-scratch／index_helpers／call_args。PLATFORM: SHARED. |
| 2026-08-06 | **feat(bc)** pressure-evict pure leave (wave166) | **代码** · **不升钉** · **双端 L2**。B′ pure pressure eviction 三面；Cap residual thin thresh/find_depth/evict；seed cold twins；spill ~2470；present **59**。下刀：cfg interf／index-scratch／index_helpers／call_args。PLATFORM: SHARED. |
| `pipeline_asm_emit_spill.c` | ~434 | asm ELF 7.3 live／Chaitin Cap residual shell（wave156–177／207–215 pure 已抽） | 🟡 wave215 pure：live set arrays＋loop stacks＋u8 thins＋fill／peak；wave213 interf／control；wave212 color／pin；wave211 try_reload；wave210 VAR；wave207–209 CAP／stack_spill；余 cache_invalidate_at_cfg_merge shell + 仍 host-cc 入 pipeline_x |
...
| `ast_pool_lifecycle.c` | **~463** | ast 池生命周期/reset/release 域（sidecar entry 访问器 + block_on_alloc + module/arena reset/release + drop_bodies_for_check + onefunc reset/release） | 🟡 **已抽出**（8.3.2）；藏于 type.c／module_func.c include 间隙；依赖 sidecar 全局 + GrowVec（先于 include）；仍 host-cc 入 `pipeline_x` |
...
| `pipeline_grow_vec.c` | **~209** | GrowVec 可增长向量叶工具（typedef + MMAP_THRESH + alloc/dealloc/init/free/ensure/at/push/copy_append） | 🟡 **已抽出**（8.3.2）；纯叶工具仅依赖 mmap/malloc；先于所有 sidecar typedef；仍 host-cc 入 `pipeline_x` |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - ✅ asm locals + block slot sidecar 域 thin：`pipeline_asm_locals.c`（AsmLocalSlotEntry／AsmLocalsSidecar／AsmBlockSlotSidecar + sidecar_get + asm_ctx_local_reset/count/append/name_len/name_byte_at/name_copy64/offset_at/find_offset + pipeline_asm_local_offset_c + asm_ctx_block_slot_reset/set/get · ~280 LOC body）；g05 STALE／`PIPELINE_X_DEPS` 已收（wave1252）
  - ✅ asm slot bytes + ensure_block_locals 域 thin：`pipeline_asm_slot_bytes.c`（asm_slot_bytes_named_in_mod + asm_fixed_array_total_bytes_mod + asm_ctx_module_ref + asm_local_slot_bytes_mod + asm_local_slot_bytes + asm_ctx_ensure_block_locals · ~445 LOC body）；g05 STALE／`PIPELINE_X_DEPS` 已收（wave1253）
