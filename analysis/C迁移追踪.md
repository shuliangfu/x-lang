...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w338 COMMON**；**w339–w342** Cap A OK；**w343 ban**；**w344 ordinal→.data**；bootstrap→类 B；**w335／w337 ban** 余 | 下一＝check_expr PREFER 复试／block_tree／GrowVec-LE。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w344 ordinal→.data bake**（库 TU 非零标量）；check_expr PREFER 待 Ubuntu 复试；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
