...
| Mega 去 pin M4（阶段 7） | 🟡 | 冷链关 pin；**7.2.1b** 1977/2007（v5.49 leftover flatten validate_toplevel）；PP002 heap entry ✅；parser seed 物理删 ⬜ |
...
- 🟡 **7.2.1b** parser_asm suite audit B-minus — **1977／2,007**（v5.49 leftover flatten validate_toplevel；parse_suite leftover 0；PP002 heap entry 已抬；leftover-to-audit unique 0；classify／score 留 stretch.x）  
...
2. 🟡 **7.2.1b** B-minus 续（1977／2,007；parse_suite leftover 0；PP002 heap entry ✅；v5.49 leftover flatten validate_toplevel；leftover-to-audit unique 0；续＝import ctx 4MiB；**深链分批 smoke／deep_off=0**）＋ **BC + 8.3** 
