...
| `pipeline_typeck_region_assign.c` | **~1324** | typeck region/escape assign-site 切片（wave240：with_arena nest BSS pure leave + wave239 call_slice + wave237 allocator + wave236 stack-escape／scope-borrow + wave235 M-3 → typeck.x；残 scan tree／region-label scope） | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
