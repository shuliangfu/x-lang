...
- ✅ **SEED_LINK platform flags→mk**（wave924 · `mk/driver_seed_link_picks.mk` · LD_R_MULTIDEF_FLAGS/ASM_GLUE_DUP_LDFLAGS/SEED_LINK_PURE_OK/ENTRY/LD_TAIL · 从 Makefile inline `ifeq`（含 `else ifeq`）迁入 · 转嵌套 `ifeq/else/endif` · catalog_parse_mk 可解析 · `driver_seed_obj_catalog.sh --link-export phase1\|final` shell-primary 输出 9 SEED_LINK_* · 产品路径 0 make（`bootstrap_driver_seed_link.sh` + `gen_g06_phase1_backend_stub.sh`）· `XLANG_SEED_LINK_VIA_MAKE=1` 逃生口 · **非**物理删 · Makefile export-* targets 保留为 parity/escape）
- ✅ **PIPELINE_GEN_CFLAGS→mk**（wave925 · `mk/driver_seed_mode_objs.mk` · PIPELINE_GEN_CFLAGS_BASE/CLANG/PIPELINE_GEN_CFLAGS · 从 Makefile inline 迁入 · 转 `ifeq` 替代 `$(if)` · catalog_parse_mk 新增 `+=` append 支持 · CC_IS_CLANG 双端计算（Makefile `$(shell)` + catalog shell `$CC -v | grep clang`）· `driver_seed_obj_catalog.sh --cflags-export` shell-primary 输出 CFLAGS + PIPELINE_GEN_CFLAGS · `build_tool.sh` 0 make（`XLANG_CFLAGS_VIA_MAKE=1` 逃生口）· **非**物理删 · Makefile export-try-heat-cflags 保留为 parity/escape）
- ✅ **ARCHAEOLOGY link-objs/cflags→catalog**（wave926 · `driver_seed_obj_catalog.sh --link-objs-export <bag>` + `--link-cflags-export <bag>` · 8 OBJ bags（xnc/legacy-xlang-c/relink-product/xlang-x/bxf/bs/objs-core/bxc）+ 4 CFLAGS bags（relink-product/xnc/btc-typeck/bxf）· `legacy_xlang_c_link.sh` + `xlang_no_c_frontend.sh` 0 make（`XLANG_LEGACY_LINK_VIA_MAKE=1` / `XLANG_XNC_LINK_VIA_MAKE=1` 逃生口）· **非**物理删 · Makefile export-*-link-objs/cflags targets 保留为 parity/escape）
- ✅ **gen_g06 + bstrict ensure-seed make-call cleanup**（wave927 · `gen_g06_phase1_backend_stub.sh` 两处 make 改 shell cc：`make src/x_seed_bridge.o` → `cc -c seeds/x_seed_bridge.from_x.c`（g05 G-02f-11 cold fallback 模式；rebuild byte-identical 验证）+ SEED_LINK_OBJS probe loop `make $obj` → `cc -c ${obj%.o}.c`；`bootstrap_driver_bstrict.sh` `make bootstrap-driver-seed` → 直调 `bash scripts/bootstrap_driver_seed.sh`（Makefile target thin-call 同脚本）· 逃生口 `XLANG_SEED_LINK_VIA_MAKE=1` / `XLANG_BSTRICT_ENSURE_VIA_MAKE=1` · **非**物理删 · Makefile thin-call targets 保留为 parity/escape · macOS L2 绿）
- ✅ **build_xlang_asm experimental chain A-class make-call cleanup**（wave928 · `build_xlang_asm.sh` 3 处 make 改 shell：① `pipeline_bootstrap_orchestration.o`（2 处）统一所有平台 cc 直编 `seeds/pipeline_bootstrap_orchestration.from_x.c`（WINDOWS 已有模式，Linux/Darwin 统一）；② `src/lsp/lsp_diag_pipeline_ctx.o` → `ensure_host_cc_seed_o.sh try-ldpc-prefer`（wave767 权威）；③ `src/driver/fmt_check_cmd_driver.o` → `ensure_host_cc_seed_o.sh try-other-l2-prefer`（wave771/775 权威）· 逃生口 `XLANG_ASM_LINK_VIA_MAKE=1` · **Parity 验证**：try-ldpc-prefer + try-other-l2-prefer 输出与 make **字节级一致** · **非**物理删 · build_xlang_asm.sh 仍有 9 处 make 调用（B类 FORCE leaves + C类 复合 target；后续波次）· macOS L2 绿）
- ✅ **build_xlang_asm B-class FORCE leaves make-call cleanup**（wave929 · `build_xlang_asm.sh` 3 处 B类 make 改 shell：① `pipeline_x.o PIPELINE_X_FORCE_COMPILE=1` → `PIPELINE_X_FORCE_COMPILE=1 ensure_host_cc_seed_o.sh try-heat pipeline_x.o`（GEN_C_TO_O_SEED_OBJS body）；② `make -s parser_x.o` → `migrate_x_objs.sh parser_x.o`（MIGRATE_X_OBJS body）；③ `make typeck_x.o x_frontend_link_alias.o` → `migrate_x_objs.sh typeck_x.o` + `ensure_host_cc_seed_o.sh try-heat x_frontend_link_alias.o`（MIGRATE_X_OBJS + R1_ALIAS_STUBS_OBJS bodies）· 逃生口 `XLANG_ASM_LINK_VIA_MAKE=1` · **Parity 验证**：parser_x.o + typeck_x.o + x_frontend_link_alias.o 与 make **字节级一致** · **非**物理删 · build_xlang_asm.sh 仍有 6 处 make 调用（C类长串 companion list + C类复合 targets；后续波次）· macOS L2 绿）
- ✅ **build_xlang_asm C-class composite targets make-call cleanup**（wave930 · `build_xlang_asm.sh` 3 处 C类复合 target make 改 shell：① `make bootstrap-pipeline pipeline_x.o` → `ensure_lsp_pipeline_gen.sh pipeline` + `try-heat pipeline_x.o`；② `make -s lsp_io_gen.c lsp_gen.c lsp_io_std_heap_gen.c lsp_x.o lsp_io_x.o lsp_io_std_heap_x.o` → `ensure_lsp_pipeline_gen.sh lsp` + `ensure_archaeology_gen.sh lsp_io_std_heap` + `try-heat lsp_x.o` + `try-heat lsp_io_x.o` + cc -c lsp_io_std_heap_x.o；③ `make gen-x-driver-objs` → 3x try-heat（pipeline_x.o + driver_x.o + preprocess_x.o）· 逃生口 `XLANG_ASM_LINK_VIA_MAKE=1` · **Parity 验证**：pipeline_gen.c + driver_x.o + preprocess_x.o 与 make **字节级一致** · **非**物理删 · build_xlang_asm.sh 仍有 3 处 make 调用（line 3762 长串 companion list + line 3399 单叶子无 seed + escape 保留）· macOS L2 绿）
