...
✅ **4.2.2 trait bounds / dyn Trait** dyn／impl 类型位 ✅ @ **`7bfff4e6b`**（impl 级 bound ✅ @ **`7e2e9aac9`** · struct 级 ✅ @ **`377bd8d74`** · 体方法 ✅ @ **`10bfd56f4`** · PLUS ✅ @ **`fe000ba3a`** · **pin-seed 体方法 resolve 闭** 2026-08-17）
  ...
  - ✅ host-C f32 dyn 调用点类型化（双端 L2）：产生点 F3 调用点 `(*)(void*, …)` 默认提升 f32→f64；wrapper 已是 `(void* data, float a1)`。G.7 补 `codegen_emit_dyn_host_c_fn_ptr_suffix`（registry `param_kind` extra i→param i＋1；NAMED／PTR／SLICE 仍 `…`）。验收：`dyn_add_f32.x` 双端 host-C／asm **7** · `-E` `(void*, float)`。**仍开**：stack extras（nargs>5 CG002）
