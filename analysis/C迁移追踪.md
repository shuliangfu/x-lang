...
✅ **FIELD-chain SIMD INDEX `w.h.v[i]`** ✅ @ **`b69a33a51`**
  ...
  - 余（勿并本叶）：dest `w.h.v[i]=`；typeck T001 `return`／`let` INDEX；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
