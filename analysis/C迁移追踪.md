...
| Mega 去 pin M4（阶段 7） | 🟡 | 冷链关 pin；**7.2.1b** 1959/1989（v5.40 leftover flatten 4）；PP002 heap entry ✅；parser seed 物理删 ⬜ |
...
- 🟡 **7.2.1b** parser_asm suite audit B-minus — **1959／1,989**（v5.40 leftover flatten 4；parse_suite leftover 0；PP002 heap entry 已抬；余 advance_to／import_as·simd next_lex／peek_kind／import_path_post／validate_toplevel）  
...
2. 🟡 **7.2.1b** B-minus 续（1959／1,989；parse_suite leftover 0；PP002 heap entry ✅；v5.40 leftover flatten 4；续＝advance_to／import_as·simd next_lex／peek_kind／import_path_post／validate_toplevel；**深链分批 smoke／deep_off=0**）＋ **BC + 8.3** 
