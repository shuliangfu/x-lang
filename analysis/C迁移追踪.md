...
✅ **prefixed SIMD dest（`tag+Holder` field_off≠0）** ✅ @ **`149df550f`**
  ...
  - 余（勿并本叶）：FIELD-chain SIMD INDEX `w.h.v[i]`（Darwin 138）；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
