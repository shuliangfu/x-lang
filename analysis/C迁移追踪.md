...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3：`pipeline_glue` ~32.7k + `ast_pool` ~12.9k；8.3.1 十八刀（+asm_emit_panic）+ 8.3.2 …+top_level residual 已切；8.3.9 ✅；其余 ⬜ |
...
|-------------------|-----|------|------|
| `pipeline_glue.c` | ~32,760 | 产品 mega glue（typeck/codegen/asm…） | 🟡 CTFE+…+asm_emit_match+asm_emit_panic 域已切出；其余仍在 |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - 🟡 仍 host-cc 编入 `pipeline_x`（未 .x 化 / 未离 host-cc）
  - ⬜ 下一域候选：下一 asm 域（field_access_emit…）或 8.3.3 field_access／soa
...
2. 🟡 **BC + 阶段 8.3**（库存 ✅ · 8.3.1 十八刀（+panic）✅ · **8.3.2 …+top_level residual ✅** · 8.3.9 ✅ · **当前：下一 asm 域（field_access_emit…）或 8.3.3 field_access／soa**）  
