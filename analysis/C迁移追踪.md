...
| STD soft SKIP 邻域续（其它闸） | 🟡 | prefer-c／archaeology／**f-v2 soft** honesty 续（**余 f-v2 soft FAIL＝16**：async-future／backtrace／context／datetime／dynlib／elf／hash／json／regex／schema／socketio／task／test／trace／unicode／url；**f-* v1 soft FAIL 池空**；**zc 若仍绑 host-c 链则后置**；STD uuid／task／mem-safe／trace 等 asm UNDEF 跳；linux mmap smoke UNDEF 观测残；compress-stream／unified-stream 观测残；sqlite stub open／last_error 观测残；aes-gcm host-c／nist2 观测残；queue-sync 观测残；async future／iocps／ctx／lang／atomic-widen 观测残；channel-unbounded／csv-stream／elf-parse／ffi-struct／http-pool／reqresp／h2／context／json-typed／regex／schema／socketio／task／trace／unicode／backtrace-xplat／chacha／ed25519 观测残；compress-brotli ld 红跳；test-executable／bench-fuzz 探针 ld UNDEF 跳；http-context／channel-unbounded／unicode-nfc 复探红跳；encoding-extra／regex／net-tls 仍跳；跳 async-1m／language／future coop UNDEF）；仍跳过产品红／UNDEF（aes-gcm nist2 RUN≠0／encoding-extra UNDEF／regex `_main`／net-tls XT001／compress-stream／unified-stream ld／async-language／async-future／process-pipe XT001／mem-boundary UNDEF／encoding-hex-b64／xxhash／map-vec／http-reqresp／http-server-pool／async-context／mac／mac512／codec／atomic-widen／csv-stream／metrics／math-special／elf-write／ed25519／chacha／hash-default／schema／fenv／queue-sync／http-context-get／json-typed-decode／Darwin json `needs_copy`／coop_pingpong／labi `std_sys_macos_write_*` needle 等）。另残 asm ld `--export-dynamic`（backtrace 具名；非软）＋by-value `Set_i32`+MEMORY 邻参 ABI（非软） |
