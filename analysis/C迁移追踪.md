...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋blkpeel＋assign＋w157 asm | ttc／blkpeel／assign／w157→`.x` thin。COMMON＋reloc sidecar 已修。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc＋blkpeel＋assign＋w157→`.x` thin。下一叶＝残 Cap／C-extract；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
