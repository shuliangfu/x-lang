...
| `pipeline_typeck_method_call.c` | **~1892** | typeck method_call + generic UFCS mono（wave251：mono-map／pattern-unify／subst → typeck.x flat ABI；wave250 type-args gate；wave249 mono foundation；wave248 overload；wave247 call-resolve；残 generic UFCS／fixup／method_call_c） | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
