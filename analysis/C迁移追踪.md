...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3：`pipeline_glue` ~34k + `ast_pool` ~14.1k；8.3.1 十三刀 + 8.3.2 …+arena+block（含 loop/labeled/getters residual）已切；8.3.9 ✅；其余 ⬜ |
...
| `ast_pool.c` | ~14,053 | AST 池 / MatchArm / sidecar | 🟡 module_import+struct_layout+top_level+type_alias+expr_sidecar+module_enum+onefunc+dep_ctx+module_func+arena+block 已切；resolve/fill residual 仍 core |
...
| `ast_pool_block.c` | ~763 | block append/region/defer + loop/labeled/getters 切片 | 🟡 已抽出（wave988–989 有则补全）；仍 host-cc 入 `pipeline_x` |
...
🟡 **8.3.1 `pipeline_glue.c` → .x（或按域切 thin + 唯一权威）**
  ...
  - ⬜ 下一域候选：8.3.2 resolve residual／fill residual / 下一 asm 域
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - 🟡 仍 host-cc 编入 `pipeline_x`（未 .x 化 / 未离 host-cc）
  - ⬜ 下一域候选：resolve residual／fill residual / 下一 asm 域
