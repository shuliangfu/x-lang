...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w346 const**；**w347 call-arg i32 lea**（PREFER 仍 ban）；bootstrap→类 B；**w335／w337 ban** 余 | 下一＝for_call_args i32 VAR rvalue load→再试 check_expr PREFER。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w347 call-arg i32 lea 根因**；下一＝for_call_args rvalue load→check_expr PREFER；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
