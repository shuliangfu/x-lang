...
| STD soft SKIP 邻域续（其它闸） | 🟡 | prefer-c／archaeology honesty 续（compress-brotli ld 红跳；test-executable／bench-fuzz 探针 ld UNDEF 跳；其余择优续扫；http-context／channel-unbounded／unicode-nfc 复探 UNDEF／RUN≠0 跳；encoding-extra／regex／net-tls 仍 UNDEF／产品红跳；跳 async-1m／language／future coop UNDEF）；仍跳过产品红／UNDEF（aes-gcm nist2 RUN≠0／encoding-extra UNDEF／regex `_main`／net-tls XT001／compress-stream／unified-stream ld／async-language／async-future／process-pipe XT001／mem-boundary UNDEF／encoding-hex-b64／xxhash／map-vec／http-reqresp／http-server-pool／async-context／mac／mac512／codec／atomic-widen／csv-stream／metrics／math-special／elf-write／ed25519／chacha／hash-default／schema／fenv／queue-sync／http-context-get／json-typed-decode／Darwin json `needs_copy`／coop_pingpong／sqlite stub open／last_error 段错误／labi `std_sys_macos_write_*` needle 等）。另残 asm ld `--export-dynamic`（backtrace 具名；非软）＋by-value `Set_i32`+MEMORY 邻参 ABI（非软） |
