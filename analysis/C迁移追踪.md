...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w351 Cap A 双端**（Darwin PREFER／Ubuntu `-E`；bt PREFER）；残 var-module／GrowVec-LE／Ubuntu check_expr | 下一＝var-module／GrowVec-LE／Ubuntu PREFER。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
