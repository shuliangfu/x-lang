...
> **刷新**：2026-09-16 · tip **`c29200f48`**（P6f block_from_res dest-buffer）· 钉盘 **`ecdb5cc1e`**
...
| Mega 去 pin M4（阶段 7） | 🟡 | 冷链关 pin；**7.2.1b leftover flatten 完**；深链分批覆盖戏停；**已 T 缩壳**：P9 hybrid 跳过 suite 46k；**Route C 产品化**：P9b stretch lite＋P1b lex_skip＋P19b helpers＋**P19c run_len extra／lex_at_token／rewind**＋**P19d struct_field_name／ident_is_unsafe**＋**P19e align_lex（in-place _into_c；P9a 门同 P5d/P12b）**＋**P19f parse_block_return_end_tail（decide+flags；P1d sync+next_into trampoline）**＋P4b primary ident＋P3b type_ref＋**P3c type-inst mangle**＋**P3d consume_qualified／angle close**＋**P3e TYPE_DYN wrap dest-buffer**＋**P3g postfix array/slice dest-buffer**＋**P3h prefix `[N]T`／`[]T` dest-buffer**＋**P3i type-position `function(...): Ret` dest-buffer**＋**P3j prefix `*T` dest-buffer**＋**P3k IDENT `Linear(T)` dest-buffer**＋**P3l builtin vec token dest-buffer**＋**P3m alloc_vector_type_ref**＋**P3n builtin scalar token dest-buffer**＋**P3o IDENT named／dyn／impl peel dest-buffer**＋**P3p IDENT generic type-arg dest-buffer**＋**P3q IDENT vector spelling consume dest-buffer**＋**P3r parse_type_ref_impl dispatcher dest-buffer**＋**P3s alloc_pointee dest-buffer**＋**P4bj anonymous-struct alloc**＋**P4bm STRING decode dest-buffer**＋**P4bn finish_from_type_ident dest-buffer**＋**P4bo parse_asm_bang dest-buffer**＋**P4bp parse_unsafe dest-buffer**＋**P4bq lbrace lookahead dest-buffer**＋P7b simd ident pack／callee 名＋**P7c simd callee+CALL wrap dest-buffer**＋**P7d simd parse dest-buffer**＋P4ub unary TOKEN→ExprKind＋**P4uc unary wrap dest-buffer**＋**P4ud unary parse dest-buffer**＋P4bb binop TOKEN→ExprKind＋**P4bc binop wrap dest-buffer**＋**P4bd binop parse dest-buffer**＋**P4tb ternary wrap dest-buffer**＋**P4as as_suffix wrap dest-buffer**＋**P4ad as_suffix parse dest-buffer**＋**P4tc assign wrap dest-buffer**＋**P4td ternary parse dest-buffer**＋**P4te assign parse dest-buffer**＋**P4bh remaining parse_primary dest-buffer**＋**P4bi parse_struct_lit_fields dest-buffer**＋**P5g match wrap dest-buffer**＋**P5h match subject parse dest-buffer**＋**P5i match struct-fields parse dest-buffer**＋**P5j parse_match_into dest-buffer**＋**P6e parse_struct_record_layout dest-buffer**＋**P2b parse_one_top_level_let／type_alias dest-buffer**＋**P2c parse_cond_expr dest-buffer**＋**P2d body_let_bracket dest-buffer**＋**P5k leftover if_stmt C twin T-shrink**＋P14b skip_if 走查＋**P14c module_try_register_enum_name**＋**P6b struct_layout name-match**＋**P6c packed／soa modifiers**＋**P6d library wrap dest-buffer**＋P12b skip_tl struct／enum／extern 走查＋**P12c skip_one_impl dest-buffer**＋**P12d generic_bound_scan dest-buffer**＋**P12e skip_one_enum_register／append_variants**＋**P12i skip_name_is_self／self_matches_for**＋**P12j named_eq_self／rewrite_self**＋**P12k register_type_params／type_param_index**＋**P12l concrete_implements_trait**＋**P12m bound_check_type_args**＋**P12n impl-seen accessors**＋**P12o bound_check**＋**P12p F3 lookup**＋**P12q F3 simple getters**＋**P12r dest-extras elem_array_dim**＋**P12s method_on_param**＋**P12t skip_hoist_default_methods**＋**P12u trait_check_impls_complete**＋**P12v param_shape＋ret_shape**＋P18b body_tl scalar 表＋let／const／if skip 走查＋P10b glue skip_one_function_full 走查＋P13b try_skip_allow padding 走查＋P11b skip_imports 走查＋P5b if_stmt 注释感知花括号字节扫描／kw_at_pos＋P5c scan_sync pos＋**P5e match_dest_enum_tag**＋**P5f parse_if_expr**＋P15b library_scan＋P17b diag_late after_structs／fail＋**P11c consume_path／try_skip**（g05 `-E`+cc，pure-asm CG002）＋**P11d collect_imports dest-buffer**（copy_token_bytes 已 P1f；g05 `-E`+cc，pure-asm CG002）＋**P18c cfg_skip／diag_first_ident**＋**P18d P010–P014／dup**（msg 表仍 C＝STRING_LIT 64B；sticky 仍 C；g05 `-E`+cc）＋**P1c skip_generic_angle_list_count**＋**P1g register_pending**（权威 `xlang_generic_func_register_pending_type_params_x_into_c`；g_gp_pending_* 仍 C；g05 `-E`+cc，pure-asm CG002）＋**P1d ASI advance_past_stmt_semicolon／cond_rparen**（rewind／run_len／align／first_token 仍 C；g05 `-E`+cc，pure-asm CG002）＋**P1e parse_peek_function_name／first_token_kind**（rewind／run_len／align 仍 C；g05 `-E`+cc，pure-asm CG002）＋**P17c G.7 diag_skip_let_const_buf trampoline over P18b into**；残＝产品 `.inc` 切片仍 host-cc（含 glue_tail 包装、skip_tl skip_one_trait／parse_one_extern、try_skip_allow parse_into、if_stmt parse／realign；P12g 后 skip_one_trait 已纯 asm 默认）、lite／suite／lex_skip／helpers／primary ident／type_ref／simd／unary／binop／skip_if walks／skip_tl struct／enum／extern／impl header／body_tl skip／glue skip_one_function_full／try_skip_allow padding／skip_imports／consume_path／try_skip／collect_imports／ctrl brace-skip／scan_sync／library_scan／diag_late after_structs／fail／body_tl cfg_skip／diag_first_ident／lex_skip count portable 冷回退、`parser_asm_thin_c.from_x.c` 443KiB 仅冷 rest 回退；parser seed 物理删 ⬜ |
...
| `pipeline_abi` mega pure-asm | 🟡 `-E` typeck 墙关 | P4bk 解 `while`+`break`；P4bl 解 LT skip 吞后 `>`（parser_selfhost 静默丢失）。mega `-E` 双端 typeck OK（2554 funcs）。禁产品默认 FORCE 整 mega（assemble／CG002 未开）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
- 🟡 **7.2.1b** parser_asm suite audit B-minus — leftover flatten **完**（unique 0／helpers 0）；深链分批覆盖戏停；**已 T 缩壳**：P9 hybrid 跳过 suite 46k；**Route C 产品化**：P9b 编 `pthin_stretch.x` 跳过 lite；P1b 编 `pthin_lex_skip.x` 跳过 kind／copy／skip_walk；**P1c** 有则补全 skip_generic_angle_list_count（g_gp_pending_*／register_pending 仍 C；g05 `-E`+cc，pure-asm CG002）；**P1d** 有则补全 ASI advance_past_stmt_semicolon／cond_rparen（rewind／run_len／align／first_token／peek_function_name 仍 C；C trampoline 在 helpers.inc；g05 P19 带 P1 BODIES 宏；禁给 P19 加 P9a 硬闸；g05 `-E`+cc，pure-asm CG002）；**P1e** 有则补全 parse_peek_function_name／first_token_kind（rewind／run_len／align 仍 C；禁包 leftover peek-name AUDIT；g05 `-E`+cc，pure-asm CG002）；**P1f** 有则补全 copy_token_bytes 128 零填（slice trampoline 留 imports.inc；禁并进 param32／name64）；P19b 编 `pthin_helpers.x` 跳过 kind／copy／pos／match-kw；**P19c** 有则补全 run_len extra／lex_at_token pos／rewind kind（align／ident_is_unsafe_stmt／parse_block_return_end_tail 仍 C；禁并进 stretch run_len；禁给 P19 加 P9a 硬闸；g05 `-E`+cc，pure-asm CG002）；**P19d** 有则补全 struct_field_name／ident_is_unsafe（align／parse_block_return_end_tail 仍 C；禁并进 P4b；g05 `-E`+cc，pure-asm CG002）；P4b 编 `pthin_expr_primary.x` 跳过 ident 拼写／asm-option-bit；P3b 编 `pthin_type_ref.x` 跳过 token_starts_type／ident_is_dyn／builtin TypeKind／vector IDENT pack；P7b 编 `pthin_simd.x` 跳过 shuffle／select ident pack／callee 名填充；P4ub 编 `pthin_expr_unary.x` 跳过 TOKEN→ExprKind；P4bb 编 `pthin_expr_binop.x` 跳过 TOKEN→ExprKind；P14b 编 `pthin_skip_if.x` 跳过 trait／impl＋if-core／statement 走查；**P14c** 有则补全 module_try_register_enum_name（hybrid 跳过 C 孪生；P12e 仍 extern；sidecar＝pipeline_module_enum_*）；P12b 编 `pthin_skip_tl.x` 跳过 skip_one_struct／enum／extern 走查；**P12c** 有则补全 skip_one_impl dest-buffer（skip_one_trait／generic_bound／enum_register 仍 C；g05 `-E`+cc，pure-asm CG002）；**P12d** 有则补全 generic_bound_scan dest-buffer（skip_one_trait／enum_register／parse_one_extern 仍 C；g05 `-E`+cc，pure-asm CG002）；**P12e** 有则补全 skip_one_enum_register／append_variants（skip_one_trait／parse_one_extern 仍 C；g05 `-E`+cc，pure-asm CG002）；P18b 编 `pthin_body_tl.x` 跳过 is_fn_sig_scalar＋diag／body skip 走查＋skip_one_top_level let／const；**P18c** 有则补全 cfg_skip／diag_first_ident（P010–P014／onefunc_param_name_dup 当时仍 C；pure-asm 过）；**P18d** 有则补全 P010–P014／onefunc_param_name_dup（msg 表 parser_body_tl_p0xx_msg_c 仍 C＝bootstrap STRING_LIT 64B；sticky／allow_bare_self／cur_module 仍 C；g05 `-E`+cc）；**P10b** 编 `pthin_glue.x` 跳过 skip_one_function_full 走查（glue 包装仍 C；pure-asm 过）；**P13b** 编 `pthin_try_skip_allow.x` 跳过 padding 走查（write_result／parse_into 仍 C；pure-asm 过）；**P11b** 编 `pthin_imports.x` 跳过 skip_imports 走查；**P11c** 有则补全 consume_path／try_skip（g05 `-E`+cc，pure-asm CG002）；**P11d** 有则补全 collect_imports dest-buffer（copy_token_bytes 已 P1f；语言无 local u8[N]，C trampoline 持 128 字节 dest buf；module 写入走既有 pipeline_module_import_*；g05 `-E`+cc，pure-asm CG002）；**P5b** 编 `pthin_ctrl.x` 跳过注释感知花括号字节扫描／kw_at_pos；**P5c** 有则补全 scan_sync pos（parse／realign／match／if_expr 仍 C；parse AUDIT 前缀仅 audit 宏；g05 `-E`+cc，pure-asm CG002）；**P15b** 编 `pthin_library.x` 跳过 library_scan 走查（into／buf parse／lex_from 仍 C；into／buf AUDIT 前缀仅 audit 宏；pure-asm 过）；**P17b** 编 `pthin_diag_late.x` 跳过 after_structs／fail_at_token_kind 走查（buf AUDIT 前缀仅 audit 宏；pure-asm 过）；**P17c** G.7 `diag_skip_let_const_buf` trampoline over P18b into（body_skip_buf 已是 trampoline；禁新 .x 导出）；残＝产品 `.inc` 切片仍 host-cc、lite／suite／lex_skip／helpers／primary ident／type_ref／simd／unary／binop／skip_if walks／skip_tl struct／enum／extern／impl header／body_tl skip／glue skip_one_function_full／try_skip_allow padding／skip_imports／ctrl brace-skip／scan_sync／library_scan／diag_late after_structs／fail portable 冷回退、`parser_asm_thin_c.from_x.c` 443KiB 仅冷 rest 回退（G-02f-330 omit rest）；PP002 heap entry ✅；import ctx 4MiB heap orch ✅；resolve_read embed 诚实拒 ✅  
  ...
  - ✅ P1g Route C（`pthin_lex_skip.x` 有则补全 register_pending；权威 `xlang_generic_func_register_pending_type_params_x_into_c`；hybrid 跳过 C 孪生；`g_gp_pending_*` 静态表仍 C；禁并 P1c；禁 glue_tail／mega；冷 lane 仍 full .inc；禁再 eq 已 T combinator；禁开新 P 道）
  ...
  - ✅ P19f Route C（`pthin_helpers.x` 有则补全 parse_block_return_end_tail decide+flags；hybrid trampoline 持 sync-from-next_lex＋lexer_next_into／P1d 面；.x 不见 lexer_result；禁并 register_pending／type_ref／glue_tail；禁开新 P 道；g05 `-E`+cc，pure-asm CG002 同 P19e；冷 lane 仍 full .inc；禁再 eq 已 T combinator）
  ...
  - ✅ P3k B-minus（`pthin_type_ref.x` 有则补全 IDENT `Linear(T)` dest-buffer；hybrid 跳过 C 孪生；P9a peek/step 走 IDENT 拼写 `Linear`（76/105/110/101/97/114）然后 `(` T `)`；裸 `Linear` fail-closed（非 NAMED 回落）；inner T 走既有 primary parse_type_ref_ptr shim（G.7 一 shim；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红）；TYPE_LINEAR writer＝`pipeline_type_init_compound_kind_at`（kind 12 < 15）；不跟 postfix；C trampoline 发布 *out_lex；非 Linear 不前进 lex；parse_type_ref_impl 仍 C；禁抄 wrap 进 parse；禁并 wrap；禁 dest-buffer IDENT generic type-arg get/set；禁开新 P 道；禁 FORCE pabi mega；g05：LINEAR 独立门，缺 linear_x 则回落 C 孪生、不丢 P3j；冷 lane 仍 full .inc；禁再 eq 已 T combinator）
  - ✅ P3l B-minus（`pthin_type_ref.x` 有则补全 builtin vec token dest-buffer；hybrid 跳过 C 孪生；P9a peek/step 走 TOKEN_I32X4／I32X8／I32X16／U32X4／U32X8／U32X16／F32X4（lexer keyword-match `i32x4` 等，非 IDENT）；elem_ord／lanes 抄 C 孪生（F32X4 elem_ord=13 非 TYPE_F32=14——禁「顺手修」）；elem intern＝既有 `pipeline_type_ensure_by_kind_ord`（G.7）；TYPE_VECTOR writer＝`pipeline_type_init_compound_kind_at`（kind 13 < 15）；不跟 postfix；C trampoline 发布 *out_lex；非 vec 不前进 lex；parse_type_ref_impl 仍 C；禁抄 wrap 进 parse；禁并 wrap；禁 dest-buffer IDENT vector spelling consume（pack 已 .x；余 alloc wrap）；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁 dest-buffer IDENT generic type-arg get/set；禁开新 P 道；禁 FORCE pabi mega；g05：VEC 独立门，缺 vec_x 则回落 C 孪生、不丢 P3k；冷 lane 仍 full .inc；禁再 eq 已 T combinator）
  - ✅ P3m Route C（`pthin_type_ref.x` 有则补全 alloc_vector_type_ref wrap；权威 `parser_asm_alloc_vector_type_ref_x_into_c`；IDENT pack 已在 .x；禁整块 parse_type_ref_impl；禁 glue_tail／mega；冷 lane 仍 full .inc；禁开新 P 道）
  - ✅ P3n B-minus（`pthin_type_ref.x` 有则补全 builtin scalar/void token dest-buffer；hybrid 跳过 C 孪生；P9a peek/step 走 TOKEN_I32／BOOL／I64／U8／U32／U64／USIZE／ISIZE／VOID／F32／F64；kind_ord＝既有 builtin_kind_ord；writer＝init_primitive_kind_at 0..16；postfix 仍 P3g；C trampoline 发布 *out_lex；非 scalar 不前进 lex；parse_type_ref_impl 仍 C；禁 dest-buffer parse_type_ref；禁 dest-buffer IDENT generic type-arg；禁碰 primary.x T001；g05：SCALAR 独立门，缺 scalar_x 则回落 C 孪生、不丢 P3m；冷 lane 仍 full .inc；禁开新 P 道）
  - ✅ P3o B-minus（`pthin_type_ref.x` 有则补全 IDENT named／dyn／impl peel dest-buffer；hybrid 跳过 C 孪生；P9a peek/step 走 unconsumed cursor；`dyn`＋type＝P013 fail-closed（lex past `dyn`）；bare `dyn` restore 成 NAMED；`impl`＋type recurse 既有 primary parse_type_ref_ptr shim（G.7 一 shim；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红）；IDENT NAMED writer＝`pipeline_type_init_named_at`（G.7；同 P3j pointee）；C trampoline 持 name[256]＋qn_len（禁 local u8[N]、禁 &local i32）；generic `<T,U>` 是 P3p；wrap_dyn 仍 P3e（`<T>` 之后）；postfix 仍 P3g；parse_type_ref_impl 仍 C；禁抄 wrap 进 parse；禁开新 P 道；禁 FORCE pabi mega；g05：NAMED 独立门，缺 named_x／peel_x 则回落 C 孪生、不丢 P3n；冷 lane 仍 full .inc；禁再 eq 已 T combinator）
  - ✅ P3p B-minus（`pthin_type_ref.x` 有则补全 IDENT generic `<T,U>` dest-buffer；hybrid 跳过 C 孪生；P9a peek/step 走 unconsumed `<`；args recurse 既有 primary parse_type_ref_ptr shim（G.7 一 shim；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红）；append＝既有 `pipeline_type_append_type_arg`（G.7；同 P3i TYPE_FN params）；first-arg＋count＝既有 `pipeline_type_set_elem_array_size_at`（G.7；禁 Type by-value get/set；禁 FORCE pabi mega）；嵌套 `Name<Name<T>>` 复用 P3d RSHIFT 契约（step `>>` 再 rewind pos/col）；非 `<` 不前进 lex；wrap_dyn 仍 P3e（`<T>` 之后）；postfix 仍 P3g；parse_type_ref_impl 仍 C；禁抄 wrap 进 parse；禁开新 P 道；禁 FORCE pabi mega；g05：GENERIC 独立门，缺 generic_x 则回落 C 孪生、不丢 P3o；冷 lane 仍 full .inc；禁再 eq 已 T combinator）
  - ✅ P3q B-minus（`pthin_type_ref.x` 有则补全 IDENT vector spelling consume dest-buffer；hybrid 跳过 C 孪生；published face 仍 `from_ident_spelling_c`；C trampoline 抽 lexer_result 字段（data／length／token_start／ident_len／next_pos；语言无 lexer_result by-value）；start 恢复 `token_start==0` → `next_pos-ident_len`；pack＝既有 `vector_type_ident_pack_c`（G.7）；alloc＝既有 P3m `alloc_vector_type_ref_x`（G.7）；AUDIT_CALL 留 C；parse_type_ref_impl 仍 C；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁开新 P 道；禁 FORCE pabi mega；g05：IDENT_VEC 独立门，缺 ident_vec_x 则回落 C compositor、不丢 P3p；冷 lane 仍 full .inc；禁再 eq 已 T combinator）
  - ✅ P3r B-minus（`pthin_type_ref.x` 有则补全 parse_type_ref_impl dispatcher dest-buffer；hybrid 跳过 C compositor；published face 仍 `parse_type_ref_impl_c`／slice_c；C trampoline 持 name[256]＋qn_len＋label[64]（语言无 local u8[N]／&local i32）并留 AUDIT_CALL；IDENT 臂＝Linear／ident-vec／NAMED／generic／wrap_dyn／postfix；FUNCTION／STAR／LBRACKET 失败也 return（helper 可能已消耗）；vec／scalar 自拒不前进 lex；递归仍走既有 primary parse_type_ref_ptr shim（G.7 一 shim；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红）；g05：IMPL 独立门，缺 impl_x／ident_x 则回落 C compositor、不丢 P3q；冷 lane 仍 full .inc；禁开新 P 道；禁 FORCE pabi mega；禁再 eq 已 T combinator）
  - ✅ P3s B-minus（`pthin_type_ref.x` 有则补全 alloc_pointee dest-buffer；hybrid 跳过 C 孪生；published face 仍 `from_tok_c`；C trampoline 持 name[256]＋qn_len（语言无 local u8[N]／lexer_result by-value）；IDENT consume_qualified 仍 published C face（mutates *r）；writer＝既有 `pipeline_type_init_named_at`／`pipeline_type_init_primitive_kind_at`（G.7；未知标量→VOID 同 C else-arm）；STAR 仍 P3j 不并；glue_tail 仍一行 trampoline；g05：POINTEE 独立门，缺 pointee_x 则回落 C 孪生、不丢 P3r；冷 lane 仍 full .inc；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁 dest-buffer append_byte；禁开新 P 道；禁 FORCE pabi mega；禁再 eq 已 T combinator）
  - ✅ P4bj B-minus（`pthin_expr_primary.x` 有则补全 anonymous-struct alloc；权威 `parser_asm_parse_anonymous_struct_lit_x_into_c`；set_struct_lit_finish nlen=0＋既有 fields walk；ANON_STRUCT 独立门闩；finish_from_type_ident 是 P4bn；禁碰整块 parse_primary；禁开新 P 道）
  - ✅ P4bm B-minus（`pthin_expr_primary.x` 有则补全 STRING decode dest-buffer；hybrid 跳过 C 孪生；escape walk 走 `parser_asm_string_lit_decode_span_x_into_c`（`\n`／`\t`／`\r`／`\0`／`\\`／`\"`／`\xHH`）；append_byte chunk overflow 仍 C（Expr by-value＋var_name[N]；语言无 local u8[N]）；STRING concat 仍走 ptr_c trampoline；g05：STRING_DECODE 独立门，缺 decode_x 则回落 C 孪生、不丢 P4bh／P4bj；冷 lane 仍 full .inc；禁 dest-buffer append_byte；finish_from_type_ident 是 P4bn；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁开新 P 道；禁 FORCE pabi mega；禁再 eq 已 T combinator）
  - ✅ P4bn B-minus（`pthin_expr_primary.x` 有则补全 finish_struct_lit_from_type_ident dest-buffer；hybrid 跳过 C 孪生；IDENT VAR 占位名走既有 `pipeline_expr_var_name_len`／`pipeline_expr_var_name_into`；zeros＝`pipeline_expr_set_common_zeros_c`；writer＝既有 `pipeline_expr_set_struct_lit_finish_c`（G.7；同 suffix FIELD_ACCESS 与 P4bj nlen=0）；fields 仍 P4bi；C trampoline 持 name[256]（语言无 local u8[N]）；Cap 4.2.8 256B 面（C 孪生仍 128 截断）；g05：FINISH_TYPE_IDENT 独立门，缺 finish_x 则回落 C 孪生、不丢 P4bh／P4bj／P4bm；冷 lane 仍 full .inc；禁 dest-buffer append_byte；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁开新 P 道；禁 FORCE pabi mega；禁再 eq 已 T combinator）
  - ✅ P4bo B-minus（`pthin_expr_primary.x` 有则补全 parse_asm_bang dest-buffer；hybrid 跳过 C 孪生；template decode 写 C-owned tmpl[256]（cap 127＋overflow note；invalid `\x` 写 backslash，不复用 STRING decode_span）；regs pack 写 C-owned regs[128] 再 set_method_call_c（不改 kind）；option bits＝set_call_c(callee=0)；n_in＝set_int_val；GNU colon fail-closed；options／operand 拆 helper 避嵌套 while parse-drop；g05：ASM_BANG 独立门，缺 asm_x 则回落 C 孪生、不丢 P4bh／P4bj／P4bm／P4bn；冷 lane 仍 full .inc；parse_unsafe 是 P4bp；禁 dest-buffer append_byte；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁开新 P 道；禁 FORCE pabi mega；禁再 eq 已 T combinator）
  - ✅ P4bp B-minus（`pthin_expr_primary.x` 有则补全 parse_unsafe dest-buffer；hybrid 跳过 C 孪生；entry＝未消耗 TOKEN_LBRACE（caller 已匹配 IDENT `unsafe`）；inner block＝parse_block_ptr（lex after `{`，同 P4bh LBRACE block）；wrapper block 一枚 unsafe stmt（order kind=6）；wrap＝P5f wrap_block_ref（G.7；type_ref=0）；g05：UNSAFE 独立门，缺 unsafe_x 则回落 C 孪生、不丢 P4bh／P4bj／P4bm／P4bn／P4bo；冷 lane 仍 full .inc；ident_pre_dispatch 仍 C compositor；lbrace lookahead 是 P4bq；禁 dest-buffer append_byte；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁开新 P 道；禁 FORCE pabi mega；禁再 eq 已 T combinator）
  - ✅ P4bq B-minus（`pthin_expr_primary.x` 有则补全 lbrace_looks_like_block／empty_ident_braces dest-buffer；hybrid 跳过 C 孪生；published face 仍 ptr_c（copy-lexer；禁 step 调用方 cursor）；empty_x 的 field_depth＝C trampoline 读 `parser_asm_struct_field_value_depth`（G.7；不新开 getter）；ident_is_unsafe＝既有 buf_c；g05：LBRACE_LOOKAHEAD 独立门，缺双符号则回落 C 孪生、不丢 P4bp；冷 lane 仍 full .inc；ident_pre_dispatch 是 P4br；禁 dest-buffer append_byte；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁开新 P 道；禁 FORCE pabi mega；禁再 eq 已 T combinator）
  - ✅ P4br B-minus（`pthin_expr_primary.x` 有则补全 ident_pre_dispatch dest-buffer；hybrid 跳过 C 孪生；published face 仍 ptr_c（IDENT 头已调）；C trampoline 持 tmpl[256]＋regs[128]（语言无 local u8[N]；同 P4bo）；unsafe → parse_unsafe_x（IDENT 后未消耗 `{`）；asm＋`!` → parse_asm_bang_x（IDENT 后未消耗 `!`）；非 `!` restore pos/line/col（VAR 头仍见 IDENT）；g05：IDENT_PRE_DISPATCH 独立门，缺 dispatch_x 则回落 C compositor、不丢 P4bq；冷 lane 仍 full .inc；禁 dest-buffer append_byte；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁开新 P 道；禁 FORCE pabi mega；禁再 eq 已 T combinator）
  ...
  - ✅ P6f B-minus（`pthin_fn_block.x` 有则补全 fill_block_const_let_from_res／append_block_lets_from_res dest-buffer；hybrid 跳过 C 孪生；published face 仍 fill_c／append_c；C trampoline 持 name[256]＋pool extract＋写 res->num_consts／num_lets（语言无 local u8[N]／onefunc_result by-value）；writer＝既有 pipeline_expr_set_kind／set_int_val／set_common_zeros_c＋pipeline_block_append_const／append_let＋pipeline_onefunc_*（G.7；EXPR_LIT 净效果 resolved_type_ref=0 同 C twin zeros-after-write）；AUDIT 留 C；g05：BLOCK_FROM_RES 独立门，缺 fill_x 则回落 C 孪生、不丢 P6e；冷 lane 仍 full .inc；禁 dest-buffer parse_one_function／parse_one_function_library；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红；禁 dest-buffer append_byte；禁 mix range_for；禁开新 P 道；禁 FORCE pabi mega；禁再 eq 已 T combinator）
  - ✅ P2b B-minus（`pthin_let_alias.x` 有则补全空桩 parse_one_top_level_let＋parse_one_type_alias dest-buffer；hybrid 跳过 C 孪生；P9a peek/step 走 let／const 可选 mut／IDENT／可选 `: T`／`= expr ;` 与 `type Alias = T ;`；C trampoline 持 name[256] pack（语言无 local u8[N]）；type_ref／expr 走既有 primary ptr shim（G.7 各一 shim；禁 dest-buffer parse_type_ref——P3f 曾 hello／fmt 红）；P010／P012／P014 仍既有 C reporter；parse_cond_expr 当时仍 C；禁抄 wrap 进 parse；禁并 wrap；禁 dest-buffer parse_cond_expr 进本波；禁开新 P 道；禁 FORCE pabi mega；g05：P2 thin 缺两枚 parse_x 则回落 C parse 孪生；冷 lane 仍 full .inc；禁再 eq 已 T combinator）
  ...
  - ✅ P2d B-minus（`pthin_let_alias.x` 有则补全 body_let_bracket dest-buffer；hybrid 跳过 C 孪生；rewind 到 `[`（bracket_start）保 entry line/col 再 parse_expr（G.7 primary ptr shim）；C trampoline 发布 lex_out＋lexer_next_into（P1d 面；.x 不见 lexer_result）；AUDIT 仍冷孪生；禁迁 glue_tail 包装；禁开新 P 道；禁 FORCE pabi mega；g05：LET_ALIAS_BRACKET_FROM_X 独立门，缺 bracket_x 则回落 C 孪生、不丢 P2b/P2c；冷 lane 仍 full .inc；禁再 eq 已 T combinator）
  ...
  - ✅ P12v Route C（`pthin_skip_tl.x` 有则补全 trait_check dest-SLICE **param_shape**；权威 `xlang_skip_trait_check_param_shape_x_into_c`＋`xlang_skip_trait_check_ret_shape_x_into_c`；独立 SHAPE 门闩两片齐；gnm／fat-reg 仍 C trampoline；禁并 P12u 外层；禁当 extra；禁 glue_tail／mega）
...
1. **最优真减 host-cc**：P6f block_from_res **已开**。P3s alloc_pointee **已开**。P3r parse_type_ref_impl dispatcher **已开**。P3q IDENT vector spelling consume **已开**。P4br ident_pre_dispatch **已开**。P4bq lbrace lookahead **已开**。P4bp parse_unsafe **已开**。P4bo parse_asm_bang **已开**。P4bn finish_from_type_ident **已开**。P4bm STRING decode **已开**。P3p IDENT generic `<T,U>` **已开**。P3o IDENT named／dyn／impl peel **已开**。P4bl suffix_loop LT rewind **已开**（mega `-E` typeck 墙关）。P4bk BREAK/CONTINUE **已开**。P3n builtin scalar **已开**。P4bj anonymous-struct alloc 已开。下刀＝append_byte 仍 C（Expr by-value＋var_name[N]；无独立 var_name_len setter）。禁 dest-buffer parse_type_ref（P3f hello／fmt 红；ptr shim 仍 C）。**跳过 glue_tail**。禁升钉／默 bstrict。禁 FORCE 整 mega 产品默认。  
