...
> **刷新**：2026-09-19 · 简写 · slot_bytes helpers BAN · 钉盘 **`ecdb5cc1e`**
...
| `pipeline_abi` mega pure-asm | 🟡 | Cap residual thin 混成 `-E`+asm。下一＝余 PREFER inject（copy／resolve_call／spill／call_method／fnptr_arr_esz 已 U-complete 不 BAN；slot_bytes helpers 已 BAN）。禁盲 FORCE mega。 |
| nest 冻帽 | ✅ | **64** |
...
| `pipeline_abi` mega pure-asm | 🟡 | 下一＝余 PREFER inject（copy／resolve_call／spill／call_method／fnptr_arr_esz 已 U-complete 不 BAN；slot_bytes helpers 已 BAN）。禁盲 FORCE mega。 |
| nest 冻 64 | ✅ | — |
