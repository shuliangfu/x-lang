...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3：`pipeline_glue` ~34k + `ast_pool` ~14.5k；8.3.1 十三刀 + 8.3.2 module_import+struct_layout+top_level+type_alias+expr_sidecar+module_enum+onefunc+dep_ctx+module_func+arena+block 已切；8.3.9 ✅；其余 ⬜ |
...
|-------------------|-----|------|------|
| `pipeline_glue.c` | ~34,194 | 产品 mega glue（typeck/codegen/asm/match…） | 🟡 CTFE+assign+coerce_init+method_call+check_block+region_assign+asm_emit_unary+asm_emit_as+asm_emit_return+asm_emit_logand+asm_emit_block_body+asm_emit_block_if_stmt+asm_emit_block_inits 域已切出；其余仍在 |
...
| `pipeline_asm_emit_unary.c` | 259 | asm ELF unary emit（NEG/LOGNOT/BITNOT）切片 | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
| `pipeline_asm_emit_as.c` | 411 | asm ELF as/await/try/float-lit emit 切片 | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
...
| `ast_pool.c` | ~15,000 | AST 池 / MatchArm / sidecar | 🟡 module_import+struct_layout+top_level+type_alias+expr_sidecar+module_enum+onefunc+dep_ctx+module_func 已切；其余仍在 |
...
| `ast_pool_onefunc.c` | ~548 | OneFunc const/let/param/call/while/for 侧车切片 | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
...
| 消费方 | 如何引用 | 风险 |
|--------|----------|------|
| `compiler/mk/x_source_deps.mk` `PIPELINE_X_DEPS` | glue + 8.3.1 切片 + `ast_pool` + `ast_pool_module_import` + `ast_pool_struct_layout` + `ast_pool_top_level` + `ast_pool_type_alias` + `ast_pool_expr_sidecar` + `ast_pool_module_enum` + `ast_pool_onefunc` + `ast_pool_dep_ctx` + `ast_pool_module_func` + bootstrap glue | 改源必重编 pipeline_x |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - ✅ block 域 thin：`ast_pool_block.c`（block_pool_append_pos + append const/let/if/region/with_arena/unsafe/defer + region/defer accessors · ~297 LOC body；while/for/labeled/getters/resolve 仍 core）同 TU 抽出；COUNT 44→45 / g05 STALE / inventory 已收
  - 🟡 仍 host-cc 编入 `pipeline_x`（未 .x 化 / 未离 host-cc）
  - ⬜ 下一域候选：block residual（while/for/labeled/getters）/ resolve residual / 下一 asm 域
...
2. 🟡 **BC + 阶段 8.3**（库存 ✅ · 8.3.1 十三刀 ✅ · **8.3.2 …+module_func+arena+block ✅** · 8.3.9 ✅ · **当前：8.3.2 block residual／resolve residual / 下一 asm 域**）  
