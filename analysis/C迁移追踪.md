...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-16（钉盘仍 **`f7424ae47`** · 嵌套 INDEX rvalue `arr[0][0]` 双端 L2 闭 @ **`345a51463`** · 前刷新 INDEX dest VECTOR CALL let-init @ **`6141c91e2`** · 前刷新 ARRAY_LIT SIMD VAR let-init @ **`7ed90f9e7`** · 前刷新 FIELD dest VECTOR CALL let-init @ **`4c4c82411`** · 前刷新 SIMD named `size_simple`＝lanes*esz @ **`b76bb2466`** · 前刷新 VECTOR CALL dest dest-in-rbx let-init＋x86 高位 home @ **`97548e68d`** · 前刷新 2026-08-15 钉盘升 **`f7424ae47`** · 双端 L4 真冷 + bstrict 129 · Stage 12.0.5 prefer 族／硬禁 mega 仍有效 · Cap 14–15 日叶闭：dest-SLICE 族／STRUCT_LIT bool／u8／FIELD overload／FIELD 16B／heap Arena64 FFI／div0 panic／ARM64 add_imm SUB／extra-.o has_obj／dest 影子 x19／dest-SLICE INDEX fat ARM64 dual-GP／ARM64 >16B INDEX MEMORY consume／Darwin import METHOD host-C f32 返回 s0→w0／Darwin 活 seed import METHOD f32 extras s0–s7／ARM64 INDEX 非 1/4/8 步长 64 位 scale1／ARM64 Vec8i shuffle 高半 slot+16／ARM64 Vec4f select fcmgt 后 BSL／METHOD binop2 UFCS let-init／VAR assign 16B CALL dual-GP／METHOD 向量 assign let-init reuse／host-C nest 20 fat／VAR assign >16B CALL sret／VAR copy >16B／FIELD dest TYPE_NAMED let-init reuse（field_off==0）／嵌套 MEMORY sret dest 保存／恢复／FIELD dest field_off≠0 frame-mag／FIELD-chain dest 走 VAR 根 let-init／指针／INDEX FIELD dest dest-in-rbx let-init／INDEX dest 16B CALL dest-in-rbx let-init／**DEREF dest TYPE_NAMED dest-in-rbx let-init**／**DEREF dest SLICE／ARRAY／VECTOR dest-in-rbx** · M4 冷链 **5/5** · Stage 8 **30/30** · **11.2.2 `./xbuild l4` ✅** · **只改勾选与事实**，无波次流水）  
...
✅ **嵌套 INDEX rvalue `arr[0][0]`** ✅ @ **`345a51463`**
  ...
  - 余（勿并本叶）：嵌套 `idv(add4)`（Darwin 20）；FIELD-as-receiver `h.v.sub4`（Darwin 22）；第二份 SIMD ARRAY_LIT 盖 dest（x86 `t[0]=3`）；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
