...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-11（Stage 12 零 cc：…／**12.0.5 pure_asm hybrid：`ONLY=rt_content` 真 L2 绿（sxtw + >8 参 call-arg 栈序）· 默认 -E**；钉盘仍 **`d79a368b2`**；**只改勾选与事实**，无波次流水）
...
| **冷启动零 cc 链** | 🟡 | **LINK 全零 cc ✅** · **`.s` COMPILE 零 cc ✅** · **stub weak `.s` ✅** · **forbid_host_cc ✅** · **STRING_LIT ✅** · **module const binop ✅** · **empty `[]`／emit／lsp_diag CG002 ✅** · **`pure_asm_x_to_o` helper ✅** · **Darwin mangling ✅** · **`rt_*` pure_asm 23/23 ✅** · **hybrid pure-asm opt-in ✅** · **`PREFER_ASM_O_ONLY` 真 L2 地图 ✅** · **i32/u8 VAR load sxtw/zxt ✅** · **AAPCS64 >8 参 call-arg 栈先于 GP reload ✅**（`ONLY=rt_content` 真 L2 **5/5**）· **SEED_SLICE 不并 no_c** · **全量 PREFER_ASM 仍禁默认** · labi **8/12** · 最小 seed ⬜ · COMPILE residual 仍需 `$CC` |
...
🟡 **12.0.5 asm backend 模块覆盖（`.x`→`.o` 直出替 `-E`+`$CC -c`）**
  ...
  - **真 L2 地图（prefer + soft g05_relink + matrix）**：  
    - **IN_NO_C GREEN**：`rt_util`／`rt_run_x_emit`／**`rt_content`（call-arg 栈序修后）**（及早前若干 real thin）  
    - **RED_SEGV**：`rt_run_compiler_parsed`（及未再扫切片）  
  ...
  - **`rt_content` 根因钉（已闭）**：①i32 VAR load sxtw ✅ · ②AAPCS64 9 参 `rt_eq6`：stack place 须在 final GP load **之前**（seed+pure 同序；disasm `str [sp]`→`ldr x0 content`→`bl`）→ **`ONLY=rt_content` 真 L2 5/5**  
  - **再探 residual**：全量 PREFER_ASM_O 仍禁默认 · 其它 RED 切片待扩  
  - 下一步：扩 hybrid RED · labi 4× panic · SEED_SLICE permanent `.o`  
