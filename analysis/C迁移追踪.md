...
✅ **FIELD-chain SIMD INDEX `w.h.v[i]`** ✅ @ **`b69a33a51`**
  ...
  - 余（勿并本叶）：dest `w.h.v[i]=`（已绿，rvalue 叶带过）；typeck T001 `return`／`let` INDEX（已闭）；Holder 拷 `let h=w.h`（已闭）；STRUCT_LIT `Wrap { h: inner }` ARM64 16B field store；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
...
✅ **dest `w.h.v[i]=`** ✅（rvalue 叶 `b69a33a51` 带过；本叶闸 @ **`dbe877df1`**）
  ...
  - 余（勿并本叶）：typeck T001（已闭）；Holder 拷 `let h=w.h`（已闭）；STRUCT_LIT `Wrap { h: inner }`；nest>20；sat／x19 序言
...
✅ **typeck 不盖 SIMD FIELD ambient** ✅ @ **`dbe877df1`**
  ...
  - 余（勿并本叶）：Holder 拷 `let h=w.h`（已闭）；STRUCT_LIT `Wrap { h: inner }` ARM64 16B field store；dest-in-rbx FIELD 源；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19

✅ **Holder 拷 `let h=w.h`** ✅ @ **`d1a80c50a`**

  - 现场：dest 后 `let h: Holder = w.h`／`h = w.h` Darwin **30**（`h.v[2]` leftover）。VAR `let h2 = h` 已 0。`take(w.h)` 16B 实参已 0。`let t: i32x4 = w.h.v` 已 0。STRUCT_LIT `Wrap { h: inner }` 仍 30（ARM64 16B field store 兄弟，未折）
  ...
  - 余（勿并本叶）：STRUCT_LIT `Wrap { h: inner }` ARM64 16B field store（x86 已有 dual-GP）；dest-in-rbx FIELD 源；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
