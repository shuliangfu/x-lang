...
| signed／layout-overflow soft prefer-c →硬绿 | ✅ | prefer asm＋`XLANG_LINK_XLANG`；退役默认 xlang-c＋soft auto-make＋check 绑闸；显式坏 XLANG／缺 native 硬 die；unsigned_wrap_ok 产品 `-o` exit42 硬绿；layout 委托 repr-c FAIL=1 硬绿；报告 `run=`／`obs=`／`skip=`；双端 L2；**…／ub＋struct／signed＋layout-ovf soft FAIL 池空**；zc3／zc4／zc5 host-c 后置；lexer-bounds／link-hardening prefer-c 邻域残 |
