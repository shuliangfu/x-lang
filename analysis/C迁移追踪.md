...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w338 modlet COMMON**（NEG／null-ptr）；**w335／w337 ban** A/B/C | Ubuntu 金标探针绿。下一＝wave339 翻 Cap A（Darwin 待高内存 mega）或 GrowVec-LE。禁盲翻；禁 Darwin mega `-E`（内存）。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w338 modlet scalar COMMON 根修**（NEG-over-LIT／null-ptr；Ubuntu FORCE prefer）。下一＝wave339 翻 Cap A（Darwin 高内存 mega）或 GrowVec-LE；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。  
