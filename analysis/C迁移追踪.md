...
| leftover fossil DOC／catalog no-Honesty stdlib-check-matrix soft→硬绿 | ✅ | 二过：拒 leftover 顶层 `analysis/boot-stdlib-check-matrix-v1.md` 已归档仍当 live DOC（缺文件 → portable BOOT-013 红）＋leftover catalog 无 Honesty＋leftover auto-make xlang-c＋leftover prefer-c＋leftover SKIP→OK＋leftover 硬 `xlang check`；live＝`analysis/archive/boot/`；顶层复活硬 die；无 XLANG 面故 G.7 不开 resolver；nested check postponed skip=1；keep `stdlib-check-matrix gate OK`；nested leftover runner 不重写；archive leftover residual（无新建 Gate）；TSV read_path xref＝archive；报告 `run=`／`obs=`／`skip=`；双端 L2；**leftover fossil DOC stdlib-check-matrix FAIL 池空**；**leftover fossil DOC zc-dashboard 已收**；余 leftover fossil DOC live 默认仍 `自举前必须清单.md` 四闸（bootstrap-min leave）；win32-*／F-* leftover XLANG fallthrough 仍 leave |
