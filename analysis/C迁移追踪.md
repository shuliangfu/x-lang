...
| `pipeline_asm_emit_index_helpers.c` | ~685 | asm ELF INDEX residual helpers（CAP BSS thin＋extern pure faces）切片 | 🟡 已抽出；wave178–189 pure：peel／slot／type／assign-addr／base／nested／eff_addr→rax／lvalue／soa／field_type_ref／stack_off／module_from_ctx／needs_ptr／array_slot／struct_slot；余 CAP BSS thin accessors 仍 host-cc |
