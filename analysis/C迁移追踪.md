...
> **刷新**：2026-09-23 · 状态债 · **三端 L2 硬闸已启用** ✅；BC present **0**；AD–AP Cap／diagnostic／pipeline 拼装 ✅；**Class AQ** labi hello-stage1 Cap ✅；**Class AR** labi DEBUG_LD push Cap 剥 ✅（Cap／真减续 🟡）· 钉盘 **`ecdb5cc1e`**（不升）
...
| 非 gen 产品 C／8.3 | 🟡 | wave309 壳物理退役（present 0）；产品 `.inc`／from_x／其它 host-cc seed 仍开；AP leftover 拼装＋parse_commit Cap gather 剥已收 |
...
| BC（编译层零 host-cc） | 🟡 | inventory present **0**（Class O）；余量＝map 外 from_x／seed／`.inc`／大户 Cap／真减（AR labi Cap push 剥后） |
