...
| Mega 去 pin M4（阶段 7） | 🟡 | 冷链关 pin；**7.2.1b leftover flatten 完**；深链分批覆盖戏停；**已 T 缩壳**：P9 hybrid 跳过 suite 46k；**Route C 产品化**：P9b stretch lite＋P1b lex_skip＋P19b helpers＋P4b primary ident＋P3b type_ref＋P7b simd ident pack／callee 名＋P4ub unary TOKEN→ExprKind＋P4bb binop TOKEN→ExprKind＋**P14b skip_if 走查**（enum 仍 C；pure-asm 过）；残＝产品 `.inc` 切片仍 host-cc（含 primary parse、P19 rewind／run_len、P1 count、skip_tl 主体）、lite／suite／lex_skip／helpers／primary ident／type_ref／simd／unary／binop／skip_if walks portable 冷回退、`parser_asm_thin_c.from_x.c` 443KiB 仅冷 rest 回退；parser seed 物理删 ⬜ |
...
- 🟡 **7.2.1b** parser_asm suite audit B-minus — leftover flatten **完**（unique 0／helpers 0）；深链分批覆盖戏停；**已 T 缩壳**：P9 hybrid 跳过 suite 46k；**Route C 产品化**：P9b 编 `pthin_stretch.x` 跳过 lite；P1b 编 `pthin_lex_skip.x` 跳过 kind／copy／skip_walk；P19b 编 `pthin_helpers.x` 跳过 kind／copy／pos／match-kw；P4b 编 `pthin_expr_primary.x` 跳过 ident 拼写／asm-option-bit；P3b 编 `pthin_type_ref.x` 跳过 token_starts_type／ident_is_dyn／builtin TypeKind／vector IDENT pack；P7b 编 `pthin_simd.x` 跳过 shuffle／select ident pack／callee 名填充；P4ub 编 `pthin_expr_unary.x` 跳过 TOKEN→ExprKind；P4bb 编 `pthin_expr_binop.x` 跳过 TOKEN→ExprKind；**P14b** 编 `pthin_skip_if.x` 跳过 trait／impl＋if-core／statement 走查（enum register 仍 C；pure-asm 过）；残＝产品 `.inc` 切片仍 host-cc、lite／suite／lex_skip／helpers／primary ident／type_ref／simd／unary／binop／skip_if walks portable 冷回退、`parser_asm_thin_c.from_x.c` 443KiB 仅冷 rest 回退（G-02f-330 omit rest）；PP002 heap entry ✅；import ctx 4MiB heap orch ✅；resolve_read embed 诚实拒 ✅  
...
- 🟡 **8.3.6** from_x 全表退役策略终稿 — 残：有引用冷孪生三分处置（pthin suite 46k、lite 564、lex_skip portable、helpers kind／copy／pos／match-kw、primary ident、type_ref kind／dyn／builtin／vector IDENT、simd ident／callee 名、unary TOKEN→ExprKind、binop TOKEN→ExprKind、skip_if walks 已从 hybrid 预处理拿掉，文件仍留作冷回退；primary parse、P19 rewind／run_len、P1 count、skip_tl 主体仍 host-cc）  
...
2. 🟡 **7.2.1b leftover flatten 完**＋**8.3.6 有引用冷孪生三分**（P9 suite 46k＋lite 564＋P1b lex_skip portable＋P19b helpers kind／copy／pos／match-kw＋P4b primary ident＋P3b type_ref kind／dyn／builtin／vector IDENT＋P7b simd ident／callee 名＋P4ub unary TOKEN→ExprKind＋P4bb binop TOKEN→ExprKind＋P14b skip_if walks hybrid 已跳过；残＝产品 `.inc` 切片仍 host-cc含 P19 rewind／run_len、P1 count、skip_tl 主体；**禁**再深链分批 eq；pipeline_x mega 须点名）＋ **BC + 8.3** 
