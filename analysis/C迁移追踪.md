...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-04（对照 tip residual：glue ~3.4k／ast_pool ~5.2k／ELF write + ELF ctx + type-to-c + skip/force + struct emit + residual + asm locals + slot bytes + block tree + ctx loop + WPO + selfhost + thin_delegate（五表全） + safe_helper（skip_heavy 全） + parser_emit_heavy 域抽出；**只改勾选与事实 LOC**，无波次流水）  
...
| `ast_pool.c` | **~570** | AST 池 host 壳（#include 编排 + residual backend/scratch/emit-heavy） | 🟡 域 thin 已切（含 typedefs/ptr_at wave1278）；仍 host-cc |
