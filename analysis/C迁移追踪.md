...
| leftover XLANG seed/c fallthrough（b04）soft→硬绿 | ✅ | 二过：拒 leftover `if [ ! -x "$XLANG" ]; then XLANG=./compiler/xlang-c`（显式坏 XLANG 回落 xlang-c）＝假权威旁路；G.7 补全既有 nested `resolve_shu`（不在父闸再开第三 resolver）；Linux invoke 硬委托已诚实 linux-syscall-invoke；prefer asm＋`XLANG_LINK_XLANG`；显式坏 XLANG 硬 die；缺 native 仍 FAIL；Ubuntu invoke `-o` 硬绿；Darwin N/A skip=1；archive Gate leftover residual；TSV identity 不改；报告 `run=`／`skip=`；双端 L2；**leftover XLANG seed/c fallthrough b04 FAIL 池空**；win32-*／F-* leftover XLANG fallthrough 仍 leave |
