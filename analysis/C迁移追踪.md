...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋w220–291 曾 C＋**w292–w330 ALWAYS 全 `.x`**（w268=.x） | w292–w330 Cap residual ALWAYS 全 `-E+$CC`；**C thin 清零**。下一＝M2 收口／mega 残面／下一轨。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc…w219 `.x`；w220–291 曾 **C thin**＋**w292–w330 ALWAYS 全 C→.x**（w268＝既有 `.x` thin）；**C thin 清零**。下一＝M2 收口／下一轨；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
