...
> **刷新**：2026-09-19 · 简写 · w596 M1 掉尾＝parse_skip／`*p=` peel SEGV BAN 再注入 · 钉盘 **`ecdb5cc1e`**
...
| `pipeline_abi` mega pure-asm | 🟡 | Cap residual thin 混成 `-E`+asm。余 PREFER 已双端 U-complete **不 BAN**。LINUX `-E` 余叶均产品 PREFER 红不 Soft-Cap。M1：Ubuntu 掉尾＝parse_skip 假绿；`unsafe { *p=1 }` peel SEGV（PREFER／`-E` 均红，BAN 再注入）。mega FORCE 禁。禁 PREFER wpo_dump orch／asm_locals get／deref peel。禁 BAN 已齐余 PREFER。禁 un-BAN arrlit＋main。 |
| nest 冻帽 | ✅ | **64** |
...
| `pipeline_abi` mega pure-asm | 🟡 | 余 PREFER 已双端 U-complete **不 BAN**。下一＝M1 墙（parse_skip 掉尾／`*p=` peel SEGV／while hang）不 Soft-Cap `-E` 余叶。mega FORCE 禁。禁 PREFER wpo_dump orch／asm_locals get／deref peel。禁 BAN 已齐余 PREFER。禁 un-BAN arrlit＋main。 |
| nest 冻 64 | ✅ | — |
...
1. **主刀 M2**（[`自举效率方法-M2主链.md`](自举效率方法-M2主链.md)）：Cap residual 仍注入叶；M1 墙（parse_skip 掉尾／`*p=` peel SEGV）穿插；禁盲 FORCE mega；禁 Darwin mega `-E`；禁升钉。  
