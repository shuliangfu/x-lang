...
| `pipeline_asm_emit_call_args.c` | ~3057 | asm ELF CALL-arg emit（named_struct + resolve + f32 slot + lea + dual-GP + named layout + for_call_args）切片 | 🟡 已抽出（wave1003）+ type_named_struct（wave1017）+ resolve／f32 slot（wave1019）有则补全；wave190 pure leave named_struct＋lea gate；wave191 pure leave named_layout_size＋MEMORY pass_addr；wave192 pure leave dual_gp_byte_size＋param_home_width＋func_return_byte_size（live＝runtime_pipeline_abi）；余 load_var dual-GP／for_call_args／agg_byte_size／call_return 等仍 residual；仍 host-cc 入 `pipeline_x` |
