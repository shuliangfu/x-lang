...
| `pipeline_typeck_method_call.c` | **~2660** | typeck method_call + generic UFCS mono（wave249：mono foundation named／effective／free_param → typeck.x；wave248 overload；wave247 call-resolve；残 generic UFCS／fixup／method_call_c／try_infer） | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
