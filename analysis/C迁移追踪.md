...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋ALWAYS `.x`（`-E` 混成）＋**w331–w334＋w336 PREFER_ASM**；**w335／w337 ban** A/B/C | Cap PREFER 安全叶耗尽。下一＝wave338 根修（CG002 BSS／GrowVec-LE）。禁盲翻 PREFER；禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ALWAYS C thin 清零；**w331–w334＋w336 PREFER_ASM**；**w335／w337** 余叶 ban 分类（A BSS／B 数组／C GrowVec-LE）。下一＝wave338 根修；禁盲翻 PREFER；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
