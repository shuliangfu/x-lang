...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w376** bootstrap_glue T001＋BAN PREFER（BRANCH26）Darwin `-E`／Ubuntu hard-skip；**w375** for_call_args PREFER＋BAN parser_result；**w374b** parse_orch Darwin PREFER／Ubuntu hard-skip；**w374** T001；**w373** type_pool option T001 根因（仍 w372b）；**w372b** type_pool Darwin PREFER／Ubuntu `-E`；**w371b** mega_body Darwin PREFER／Ubuntu hard-skip；**w370b BAN** macho_write PREFER；**w369b BAN** asm_wpo PREFER；**w368b elf_ctx** Darwin PREFER／Ubuntu `-E`；**w367b BAN** dep_ctx PREFER；**w366 sidecar_pool** PREFER 双端；**w365 expr_sidecar** PREFER 双端；**w364 block_domain** PREFER 双端；**w363b module_func** Darwin PREFER／Ubuntu `-E`；**w361/362 BAN** asm_locals／struct_layout；**w360b module_enum** Darwin PREFER／Ubuntu `-E`；**w359b BAN** top_level_let；**w358 type_alias** PREFER 双端；残 BAN 叶／mega Ubuntu fn#116／Type LE asm | 下一＝module_import／value_abi／BAN 叶／mega Ubuntu。PREFER 后强制 type_alias -c 闸。禁盲翻。|
| nest 冻帽 | ✅ 纪律 | **64** |
