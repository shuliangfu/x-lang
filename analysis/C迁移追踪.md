...
| `pipeline_abi` mega pure-asm | 🟡 …＋**w358 type_alias** PREFER Darwin；**w357b type_pool** Darwin PREFER／Ubuntu `-E`；残 class C peers／Ubuntu Type LE ABI | 下一＝class C peers／Ubuntu Type LE。禁盲翻；禁 Darwin mega `-E`（内存）。|
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：**w358 type_alias** PREFER Darwin；下一＝class C peers／Ubuntu Type LE＋check_expr；禁盲翻；禁 Darwin mega `-E`（内存）；禁升钉。双端 L2＝mac push→Ubuntu pull。
