...
| `pipeline_abi` mega pure-asm | 🟡 | add_defs／param_ptr_slot／unused_hints／asm_expr helpers／`asm_wpo_thin`／`wpo_dump` helpers 产品 PREFER_ASM。Darwin COMMON lea PAGE21 sidecar。Darwin F7 data_len 双 BSS sidecar。w745：`const_lit` 只认 `const`；load_operand 可变文件级 let 走 emit_expr（at() 不再 imm 0）。w746：leftover gcc weak/W 则再 PREFER dump；JSON v2 233B。`return_elf_impl_thin` 独立 `-c` T=1 U=7（Class A unsafe）；产品 leftover gcc W 未改。HARD BAN PREFER。小文件 `x=call(); if` 绿。8 EOF 别名＝WPO 帽 4096（pad≥2048 T=N+8；活体 thin）。余 HARD BAN PREFER：asm_locals／assign_index／return_elf_impl_thin／full tip／wpo_dump orch。leftover-first 非主刀。mega FORCE 禁。禁 `-E` 当修。 |
| nest 冻帽 | ✅ | **64** |
...
| `pipeline_abi` mega pure-asm | 🟡 | 余 PREFER 已双端 U-complete **不 BAN**。add_defs／param_ptr_slot／unused_hints／asm_expr helpers／`asm_wpo_thin`／`wpo_dump` helpers 产品 PREFER_ASM。Darwin COMMON lea PAGE21 sidecar。Darwin F7 data_len 双 BSS sidecar。w745 const_lit is_const＋load_operand 文件级 let 回退 emit_expr。w746 dump leftover gcc weak/W 则再 PREFER；JSON v2 233B。`return_elf_impl_thin` 独立 `-c` T=1 U=7；产品 leftover gcc W 未改。小文件 `x=call(); if` 绿。8 EOF 别名＝WPO 帽 4096（pad≥2048 T=N+8；活体 thin）。不 Soft-Cap `-E` 余叶。mega FORCE 禁。leftover-first 非主刀。禁 PREFER wpo_dump orch／asm_locals get／deref peel／grow_vec／deref scalar／rhs_to_rax／assign_var／emit_ctx BSS／final_expr／return_impl／fnptr_arr_esz／fnptr_as／assign_index／call_arg_lea／full `emit_expr_elf_c` tip。禁 LINUX `-E` emit_ctx BSS。禁 BAN 已齐余 PREFER。禁 un-BAN arrlit＋main。 |
| nest 冻 64 | ✅ | — |
...
1. **主刀 M2**（[`自举效率方法-M2主链.md`](自举效率方法-M2主链.md)）：已绿 thin 站住 PREFER_ASM。小文件 `x=call(); if` 绿。Darwin COMMON lea PAGE21 sidecar。Darwin F7 data_len 双 BSS sidecar。w745：`const_lit` 只认 `const`；load_operand 可变文件级 let 走 emit_expr；产品 PREFER `asm_wpo_thin`。w746：产品 PREFER `wpo_dump` helpers（leftover gcc weak/W 则再 PREFER；JSON v2 233B）。`return_elf_impl_thin` 独立 `-c` T=1 U=7（HARD BAN PREFER）。8 EOF 别名＝WPO 帽 4096（pad≥2048 T=N+8）。下一刀＝余 HARD BAN LINUX `-E`（asm_locals）或下一命名 CG002 墙。禁 leftover-first 当主刀；禁 `-E` 当修法；禁盲 FORCE mega；禁升钉；禁 PREFER asm_locals／assign_index／return_elf_impl_thin／full `emit_expr_elf_c` tip／wpo_dump orch。  
