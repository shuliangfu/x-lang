...
| `pipeline_typeck_region_assign.c` | **~901** | typeck region/escape assign-site 切片（wave243：stamp_let＋read_ptr pure leave + wave242 scan tree + wave241 region-label + … → typeck.x；残 call_struct／stack_local／one_region／ptr_for_addr） | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
