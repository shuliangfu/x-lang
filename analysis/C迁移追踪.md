...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3：`pipeline_glue` ~24.7k + `ast_pool` ~12.9k；8.3.1 二十七刀（+asm_emit_index_helpers）+ 8.3.2 …+top_level residual 已切；8.3.9 ✅；其余 ⬜ |
...
| `pipeline_asm_emit_index_helpers.c` | ~3038 | asm ELF INDEX residual helpers（slot+esz+try_index forest+lvalue_eff_addr）切片 | 🟡 已抽出（wave1008）；仍 host-cc 入 `pipeline_x` |
...
| 消费方 | 如何引用 | 风险 |
|--------|----------|------|
| `compiler/mk/x_source_deps.mk` `PIPELINE_X_DEPS` | glue + 8.3.1 切片（含 struct_lit／vector_let／vector_simd／struct_let／index_helpers）+ `ast_pool` + 11× ast_pool domain slices + bootstrap glue | 改源必重编 pipeline_x |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - ⬜ 下一域候选：spill／bulk_mem_copy／index assign residual 或 8.3.3 field_access／soa
...
2. 🟡 **BC + 阶段 8.3**（库存 ✅ · 8.3.1 二十七刀（+index_helpers）✅ · **8.3.2 …+top_level residual ✅** · 8.3.9 ✅ · **当前：spill／bulk_mem_copy／index assign residual 或 8.3.3 field_access／soa**）  
