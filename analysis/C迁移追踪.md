...
| `pipeline_asm_emit_index_helpers.c` | ~2907 | asm ELF INDEX residual helpers（slot+esz+try_index forest+lvalue_eff_addr elf+text）切片 | 🟡 已抽出；lvalue_text wave1013；wave178 pure：6× expr shape peel；wave179 pure：local-slot ptr/addr + field-slot deref；wave180 pure：fixed_array_total_bytes + index_elem_byte_sz_from_type_ref + var_expr_type_ref_with_decl_fallback；余 try_index emit／base／lvalue 仍 host-cc |
| 2026-08-06 | **feat(bc)** INDEX type/stride helpers pure leave (wave180) | **代码** · **不升钉** · **mac L2**（Ubuntu 待）。B′ pure 3× fixed_array_total_bytes + index_elem_byte_sz_from_type_ref + var_expr_type_ref_with_decl_fallback；Cap residual bodies→extern；seed cold twins；index_helpers ~2907；present **59**。下刀：try_index emit／base／lvalue／call_args／spill BSS。PLATFORM: SHARED. |
