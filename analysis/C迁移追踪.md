...
| `pipeline_abi` mega pure-asm | 🟡 | add_defs／param_ptr_slot／unused_hints／asm_expr helpers／`asm_wpo_thin` 产品 PREFER_ASM。Darwin COMMON lea PAGE21 sidecar。Darwin F7 data_len 双 BSS sidecar。w745：`const_lit` 只认 `const`；load_operand 可变文件级 let 走 emit_expr（at() 不再 imm 0）。`return_elf_impl_thin` 独立 `-c` T=1 U=7（Class A unsafe）；产品 leftover gcc W 未改。HARD BAN PREFER。小文件 `x=call(); if` 绿。8 EOF 别名＝WPO 帽 4096（pad≥2048 T=N+8；活体 thin）。余 HARD BAN PREFER：wpo_dump／asm_locals／assign_index／return_elf_impl_thin／full tip。leftover-first 非主刀。mega FORCE 禁。禁 `-E` 当修。 |
| nest 冻帽 | ✅ | **64** |
