...
✅ **4.2.3 八层+ nested slice fat 布局 / 深 lit typeck** 布局 1..16 ✅ @ **`028959695`** · 深 lit ✅ @ **`89e611150`**
  ...
  - 余（已闭 host-C type_to_c_repr）：`[][2]i32` @ **`243c07be1`**；另层：host-C `[2][]i32` 行 brace；nest>16 soft
...
✅ **4.2.3 体 const nested slice parse** ✅ @ **`05069be34`**（parser.x **`adde4a343`**）
  ...
  - 余（已闭 parse＋asm fat＋const whitelist＋asm 聚合 emit＋host-C 多 const＋asm／host-C `[][2]i32`）：`[lit] as T` @ **`243c07be1`**；另层：host-C `[2][]i32` 行 brace
...
✅ **host-C `[][2]i32` type_to_c_repr** ✅ @ **`243c07be1`**
  ...
  - 余（勿并本叶）：host-C `[2][]i32` 行 brace（narr `{{1,2},{3,4}}` 当 slice 字段）；nslidx asm 偶发 134；nest>16 soft
