...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-04（对照 tip residual：glue ~3.4k／ast_pool ~0.46k host 壳 + backend wrapper／scratch 已抽／ELF write + ELF ctx + type-to-c + skip/force + struct emit + residual + asm locals + slot bytes + block tree + ctx loop + WPO + selfhost + thin_delegate（五表全） + safe_helper（skip_heavy 全） + parser_emit_heavy 域抽出；**只改勾选与事实 LOC**，无波次流水）  
...
> **规模（2026-08-04 刷新）**：`pipeline_glue.c` **~3.4k** + `ast_pool.c` **~0.46k** host 壳 + 已抽出 domain 叶合计仍大（同 TU `#include` 入 `pipeline_x`，**仍 host-cc**）；不离 host-cc 则 **BC 终局未达成**。  
...
| `ast_pool.c` | **~460** | AST 池 host 壳（#include 编排 + residual emit-heavy env／path／whitelist） | 🟡 域 thin 已切（含 typedefs/ptr_at + backend wrapper/scratch）；仍 host-cc |
