...
| `pipeline_typeck_check_expr.c` | **~1027（thin）** | typeck check_expr 域 Cap residual thin（wave228 pure leave） | 🟡 **G.7** panic／unary／addr／index／deref／var＝typeck.x thin；match／return／call／try＋mega region 仍 residual body；仍 host-cc 入 `pipeline_x` |
| `pipeline_typeck_coerce_init.c` | **~1019（thin）** | typeck coerce-init 域 Cap residual thin（wave227 pure leave） | 🟡 **G.7 live coerce＝typeck.x**（`typeck_coerce_init_*`）；residual 仅 int_binop_c／struct_lit_c + type_refs／widen／ret_coerce + thin faces；仍 host-cc 入 `pipeline_x` |
...
| 2026-08-07 | **feat(bc)** typeck orch＋check_expr domain pure leave (wave228) | **代码** · **不升钉** · **mac L2**（Ubuntu 同波）。orch～242 thin→typeck_x_ast*；check_expr～1027 thin panic／unary／addr／index／deref／var；match／return／call／try 残；present **56**。tip **`69e737c60`**。下刀：ctfe／region／method_call／check_expr match-return-call-try／elf／ast_pool／pipeline_x mega。PLATFORM: SHARED freestanding typeck. |
