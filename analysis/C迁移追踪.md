...
| Mega 去 pin M4（阶段 7） | 🟡 | 冷链关 pin；**7.2.1b** 1967/1997（v5.45 leftover flatten skip_* 链）；PP002 heap entry ✅；parser seed 物理删 ⬜ |
...
- 🟡 **7.2.1b** parser_asm suite audit B-minus — **1967／1,997**（v5.45 leftover flatten skip_balanced／skip_type_suffix／skip_one_param_type；parse_suite leftover 0；PP002 heap entry 已抬；余 advance_to／import_path_post／validate_toplevel）  
...
2. 🟡 **7.2.1b** B-minus 续（1967／1,997；parse_suite leftover 0；PP002 heap entry ✅；v5.45 leftover flatten skip_* 链；续＝advance_to／import_path_post／validate_toplevel；**深链分批 smoke／deep_off=0**）＋ **BC + 8.3** 
