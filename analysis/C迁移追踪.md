...
| `pipeline_typeck_check_expr.c` | **~705（thin）** | typeck check_expr 域 Cap residual thin（wave232 call + wave231 match／try + wave229 return） | 🟡 **G.7** panic／unary／addr／index／deref／var／return／match／try／call＝typeck.x thin；method_call＋mega region＋repr／unsafe helpers 仍 residual；仍 host-cc 入 `pipeline_x` |
