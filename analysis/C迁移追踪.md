...
| `pipeline_asm_emit_index_helpers.c` | ~657 | asm ELF INDEX residual helpers（CAP cache＋extern pure faces）切片 | 🟡 已抽出；wave178–189 pure：peel／slot／type／assign-addr／base／nested／eff_addr→rax／lvalue／soa／field_type_ref／stack_off／module_from_ctx／needs_ptr／array_slot／struct_slot；wave207 pure：rax_frame spill＋scratch depth；wave208 pure：stack_spill 表 BSS；余 CAP cache structs（minus_pair／subadd3）仍 host-cc |
