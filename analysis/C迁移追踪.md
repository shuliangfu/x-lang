...
- 🟡 lexer 结构返回尾与 `XLANG_WEAK` 别名簇仍 host-cc，`x_frontend_link_alias.from_x.c` 还不能整文件删除
- 🟡 不优先 `.x` 整包重编 `runtime_driver_no_c.o`。不在 694 条前缀对不上的编译器上重编 `x_frontend_link_alias.o`。不移动结构返回。不把 `XLANG_WEAK` 别名改成强符号
