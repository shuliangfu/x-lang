...
✅ **host-C 模块 dest-SLICE `[][]T` ARRAY_LIT** ✅ @ **`de5a9bcc4`**
  ...
  - 余（勿并本叶）：nest>16 soft；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；非 main 读模块 const 数组；dest-SLICE 可变 COMMON LEA；asm 模块 dest-SLICE `[][]T` INDEX（cmns asm **15**）
