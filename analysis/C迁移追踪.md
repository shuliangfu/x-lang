...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3：`pipeline_glue` ~21.2k + `ast_pool` ~12.9k；8.3.1 二十九刀（+expr_rec）+ index_eff base 有则补全 + 8.3.2 …+top_level residual 已切；8.3.9 ✅；其余 ⬜ |
...
| `pipeline_asm_emit_index_helpers.c` | ~3038 | asm ELF INDEX residual helpers（slot+esz+try_index forest+lvalue_eff_addr）切片 | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
| `pipeline_asm_emit_spill.c` | ~2532 | asm ELF 7.3 live／Chaitin spill／bulk_mem／index-assign residual 切片 | 🟡 已抽出；仍 host-cc 入 `pipeline_x` |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - ⬜ 下一域候选：lvalue_eff_addr_text／thin ELF return wrappers／binop residual／leaf residual 或 8.3.3 field_access／soa
...
2. 🟡 **BC + 阶段 8.3**（库存 ✅ · 8.3.1 二十九刀（+expr_rec）+ index_eff base 有则补全 ✅ · **8.3.2 …+top_level residual ✅** · 8.3.9 ✅ · **当前：lvalue_eff_addr_text／leaf residual 或 8.3.3 field_access／soa**）  
