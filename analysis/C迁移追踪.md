...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-04（对照 tip residual：glue ~3.4k／ast_pool ~8.4k／ELF write + ELF ctx + type-to-c + skip/force + struct emit + residual + asm locals + slot bytes + block tree + ctx loop 域抽出；**只改勾选与事实 LOC**，无波次流水）  
...
| `pipeline_asm_thin_delegate.c` | **~248** | asm M8-tail thin delegate 查表（AsmBackendThinDelegateRow + k_asm_backend／k_asm_pipeline／k_asm_parser_thin_delegate + backend／pipeline_m8_tail_thin_delegate_c_name） | 🟡 **已抽出**（8.3.2）；k_asm_parser 表迁出、asm_parser_m8_tail_thin_delegate_c_name 仍留 ast_pool parser_emit_heavy 域；仍 host-cc 入 `pipeline_x` |
| `pipeline_asm_emit_heavy_safe_helper.c` | **~513** | asm EMIT_HEAVY 第二遍 per-module 分类器（typeck／pipeline／driver_compile_safe_helper + skip_heavy_backend_m8/helper_keep · 5 fn） | 🟡 **已抽出**（8.3.2）；共用 static pipeline_module_func_name_has_prefix_at 留 ast_pool；仍 host-cc 入 `pipeline_x` |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - ✅ asm M8-tail thin delegate 域 thin：`pipeline_asm_thin_delegate.c`（AsmBackendThinDelegateRow + k_asm_backend／k_asm_pipeline／k_asm_parser_thin_delegate + asm_backend／asm_pipeline_m8_tail_thin_delegate_c_name · ~248 LOC body）；k_asm_parser 表迁出、asm_parser_m8_tail_thin_delegate_c_name 仍留 ast_pool parser_emit_heavy 域（同 TU static 可见 via #include L5231）；g05 STALE／`PIPELINE_X_DEPS` 已收
  - ✅ asm EMIT_HEAVY safe-helper 域 thin：`pipeline_asm_emit_heavy_safe_helper.c`（asm_typeck／pipeline／driver_compile_emit_heavy_safe_helper + asm_skip_heavy_backend_m8/helper_keep · 5 fn · ~492 LOC body）；共用 static pipeline_module_func_name_has_prefix_at 留 ast_pool（多域共用）；asm_module_is_*_selfhost 经 selfhost include 可见；g05 STALE／`PIPELINE_X_DEPS` 已收
