...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋…＋w219 asm＋w220–291 C＋w292–w324 部分 `.x`（w268=.x） | w292 elf_fwd 全 `.x`；w293/w318 typeck_orch 全 `.x`；w319 typeck_check_expr `.x`；w320 lifecycle `.x`；w321 bootstrap_glue `.x`；w322 ast_forwarders `.x`；w323 parse_orch `.x`；w324 module_func `.x`；w294–w324 `-E+$CC`。下一＝wave325 C→.x。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc…w219 `.x`；w220–291 **C thin**＋w292–w324 部分 **C→.x**（w268＝既有 `.x` thin）。下一叶＝wave325 Cap residual C→.x；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
