...
| **非 gen 产品 C（glue/ast 池）** | 🟡 | 阶段 8.3：`pipeline_glue` ~19.2k + `ast_pool` ~12.9k；8.3.1 三十一刀（+expr_rec／fast／**array_lit durable**）+ …／**durable+force_esz** 有则补全 + 8.3.2 …+top_level residual 已切；8.3.9 ✅；其余 ⬜ |
...
🟡 **8.3.2 `ast_pool.c` → .x / 已有 ast.x 权威收敛**
  ...
  - ⬜ 下一域候选：slice reent deep-copy／var_decl+lazy_append／slice_from_array residual 或 8.3.3 field_access／soa
...
2. 🟡 **BC + 阶段 8.3**（库存 ✅ · 8.3.1 三十一刀（+array_lit durable）+ index_eff／emit_expr fast／call_arg／try_binop 有则补全 ✅ · **8.3.2 …+top_level residual ✅** · 8.3.9 ✅ · **当前：leaf residual（slice reent／var_decl）或 8.3.3 field_access／soa**）  
