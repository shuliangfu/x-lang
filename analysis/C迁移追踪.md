...
| `pipeline_abi` mega pure-asm | 🟡 | **w739** asm_expr helpers LINUX PREFER_ASM 换 leftover gcc W rec（独立 `-c` T=2 U=35；活体 rec T `sub $0x19b8` 无 endbr64）。MACOS 保持 full overlay。add_defs／param_ptr_slot／unused_hints／asm_expr helpers 产品 PREFER。tip `emit_expr_elf_c` 仍 leftover gcc W。已绿未 BAN 的 LINUX `-E` 产品叶已收口。余 HARD BAN PREFER：wpo_dump／asm_locals／assign_index／return_elf_impl_thin／full tip。leftover-first 非主刀。mega FORCE 禁。禁 `-E` 当修。HARD BAN 产品 PREFER `return_elf_impl_thin`。 |
| nest 冻帽 | ✅ | **64** |
...
1. **主刀 M2**（[`自举效率方法-M2主链.md`](自举效率方法-M2主链.md)）：已绿 thin 站住 PREFER_ASM。**w739** asm_expr helpers LINUX PREFER_ASM 已入链。已绿未 BAN 的 LINUX `-E` 产品叶已收口。下一刀＝CG002 点名墙才插 thin M1。禁 leftover-first 当主刀；禁 `-E` 当修法；禁盲 FORCE mega；禁升钉；禁 PREFER wpo_dump／asm_locals／assign_index／return_elf_impl_thin／full `emit_expr_elf_c` tip。  
