...
| `pipeline_asm_emit_call_args.c` | ~623 | asm ELF CALL-arg emit（named_struct + resolve + f32 slot + lea + dual-GP + named layout + for_call_args）切片 | 🟡 已抽出（wave1003）+ type_named_struct（wave1017）+ resolve／f32 slot（wave1019）有则补全；wave190–217 pure leave（…／**for_call_args mega wave216**／**CALL／METHOD thin wrappers wave217**）；shell～623 prototype only；仍 host-cc 入 `pipeline_x` |
...
| `pipeline_asm_emit_index_helpers.c` | ~659 | asm ELF INDEX residual helpers（extern pure faces）切片 | 🟡 已抽出；wave178–189 pure：peel／slot／type／assign-addr／base／nested／eff_addr→rax／lvalue／soa／field_type_ref／stack_off／module_from_ctx／needs_ptr／array_slot／struct_slot；wave207 pure：rax_frame spill＋scratch depth；wave208 pure：stack_spill 表 BSS；wave209 pure：CAP cache structs（minus_pair／subadd3）＋thin＋hit_var；余 try_index 外壳仍 host-cc |
