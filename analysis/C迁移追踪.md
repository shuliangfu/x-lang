...
| `pipeline_abi` mega pure-asm | 🟡 ttc＋blkpeel＋assign asm | ttc／blkpeel／assign→`.x` thin。w157 挡于文件级 BSS 发射。禁 Darwin mega `-E`；禁盲 FORCE 整 mega。 |
| nest 冻帽 | ✅ 纪律 | **64** |
...
1. **主刀 M2**（[自举效率方法-M2主链.md](自举效率方法-M2主链.md)）：纯 asm／CG002。ttc＋blkpeel＋assign→`.x` thin。下刀＝文件级 BSS／`let` 发射（挡 w157 thin）；禁 Darwin mega `-E`；禁盲 FORCE mega；禁升钉。双端 L2＝mac push→Ubuntu pull。  
