...
| `pipeline_asm_emit_index_helpers.c` | ~905 | asm ELF INDEX residual helpers（slot+esz+try_index forest residual＋CAP BSS）切片 | 🟡 已抽出；wave178–185 pure：peel／slot／type／assign-addr／base／nested／eff_addr→rax／lvalue；wave186 pure：soa index.field addr；余 field_type_ref／CAP BSS／stack_off 仍 host-cc |
