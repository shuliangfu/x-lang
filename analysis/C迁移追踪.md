...
> **创建**：2026-07-29  
> **状态刷新**：2026-08-08（对照 tip residual：glue／ast_pool host 壳 + 域叶 thin／leave；bc-inventory present residual **14**／ROWS=128 · wave302 parse_commit leave 后；**只改勾选与事实 LOC**，无波次流水）
...
| `pipeline_glue_strict_minimal`（seed → 产品链） | ~900（wave302） | 产品 weak 孪生 | 🟡 仍在链；**wave300–301** dual WEAK 删；**wave302**：parse_commit_pre／post → runtime_driver_diagnostic thin（~957→~900）；残 find_func_return overload 链；**8.3.6 退役未终** |
...
🟡 **8.3.6 `seeds/*.from_x.c` 全表退役策略**
  ...
  - ⬜ 残：find_func_return overload 链／strict_minimal seed twin 仍链；全表退役策略未终
