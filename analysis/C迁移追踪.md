...
1. **最优真减 host-cc**：根修 P4b suffix_loop `.x` DOT＋METHOD RSS（`fmt.println`／`option.none_i32()`；tiny `f()`／`s.x` 已绿）再开 BODIES。P4be 已解 `-E` XT001（TOKEN／writer pin；num_funcs=61）；产品 hybrid 停放。wrap 族已尽。glue_tail 余量＝by-value 适配。pipeline_abi mega pure-asm 仍是已知地雷。  
