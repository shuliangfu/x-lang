...
| `pipeline_asm_emit_index_helpers.c` | ~1129 | asm ELF INDEX residual helpers（slot+esz+try_index forest+lvalue_eff_addr elf+text）切片 | 🟡 已抽出；lvalue_text wave1013；wave178–183 pure：peel／slot／type／assign-addr／base／nested addr；wave184 pure：16× eff_addr→rax；余 lvalue／CAP BSS 仍 host-cc |
