...
| `pipeline_asm_emit_index_helpers.c` | ~774 | asm ELF INDEX residual helpers（slot+esz+try_index forest residual＋CAP BSS）切片 | 🟡 已抽出；wave178–187 pure：peel／slot／type／assign-addr／base／nested／eff_addr→rax／lvalue／soa／field_type_ref；wave188 pure：stack_off；余 module_from_ctx／local_var_slot_needs_ptr／CAP BSS 仍 host-cc |
