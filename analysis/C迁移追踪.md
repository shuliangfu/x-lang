...
✅ **`return idv(add4)[i]` emit_expr 走 vector let-init** ✅ @ **`66e6e848d`**
  ...
  - 余（勿并本叶）：第二份 SIMD ARRAY_LIT 盖 dest（x86 `t[0]=3`）；FIELD-chain `w.h.v.sub4`；nest>20（21 须加宽 scratch）；TYPE_DYN／vtable 后期；4.2.4–5／4.2.7 leave-off；sat 盖 prefer；ARM64 prologue 未存 x19
