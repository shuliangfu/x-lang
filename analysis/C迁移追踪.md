...
| leftover catalog no-Honesty comprehensive-check soft→硬绿 | ✅ | 二过：拒 leftover catalog 无 Honesty＋缺 `run=`／`obs=`／`skip=`（STD-168 汇总宿主无诚实合同）；G.7 不开父闸 resolver（补全既有 nested `resolve_shu`）；keep `comprehensive-check gate OK`；nested 子闸失败仍硬；显式坏 XLANG 经 nested next-yellow 硬 die；子闸产品路径不重写；archive leftover residual（无新建 Gate）；报告 `run=`／`obs=`／`skip=`；双端 L2；**leftover catalog no-Honesty comprehensive-check FAIL 池空**；**leftover catalog no-Honesty g-ffi-5-std-wrap 已收**；余 portable catalog 无 Honesty 仍 stdlib-check-matrix／zc-dashboard／portable-c／portable-suite 宿主；win32-*／F-* leftover XLANG fallthrough 仍 leave |
