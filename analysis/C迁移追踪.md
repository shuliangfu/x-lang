...
| Mega 去 pin M4（阶段 7） | 🟡 | 冷链关 pin；**7.2.1b** 1964/1994（v5.44 leftover flatten expr_binop_kinds_probe）；PP002 heap entry ✅；parser seed 物理删 ⬜ |
...
- 🟡 **7.2.1b** parser_asm suite audit B-minus — **1964／1,994**（v5.44 leftover flatten expr_binop_kinds_probe；parse_suite leftover 0；PP002 heap entry 已抬；余 advance_to／import_path_post／validate_toplevel）  
...
2. 🟡 **7.2.1b** B-minus 续（1964／1,994；parse_suite leftover 0；PP002 heap entry ✅；v5.44 leftover flatten expr_binop_kinds_probe；续＝advance_to／import_path_post／validate_toplevel；**深链分批 smoke／deep_off=0**）＋ **BC + 8.3** 
