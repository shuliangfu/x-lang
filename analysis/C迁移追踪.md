...
| **Cap 能力解锁** | 🟡 | 4.2.1 untyped self ✅；4.2.2 PLUS `T: A + B` ✅；LANG-006 保留；下一硬叶 4.2.2 泛型体方法／dyn |
...
🟡 **4.2.2 trait bounds / dyn Trait** PLUS 子叶 ✅ @ **`fe000ba3a`**

  - 调用点 `function foo<T: Trait>` 已 T001（既有 scan+check）
  - **`T: Clone + Default` PLUS**：同 pos 多 trait 已硬查（A 缺第二／第三 trait → T001）
  - 余（勿并本叶）：①泛型体 `T: Clone` 下 `x.clone()` 仍 LANG-004（typeck method_call）②`dyn Trait` 无 KW／TYPE_DYN（`dyn Clone` 形参静默丢函数）③struct／impl 级 `<T: Trait>` 仍不扫
