...
| `pipeline_typeck_coerce_init.c` | **~612（thin）** | typeck coerce-init 域 Cap residual thin（wave230 type_refs／integer_widen + wave229） | 🟡 **G.7 live coerce＝typeck.x**（`typeck_coerce_init_*`＋ret_coerce／float_widen＋type_refs／integer_widen／bool／expr_type_ref）；residual 仅 int_binop_c／struct_lit_c／resolve_alias／int_lit／float_bits + thin faces；仍 host-cc 入 `pipeline_x` |
