...
| **T5-error-no-impl** | 无 impl 时 `no impl for type` | `method_no_impl.sx` | ✅ |
...
- `.sx` seed pipeline 不解析 trait（见 `runtime.c` `content_has_generic_or_trait`）→ 须 **shux / shux-c**
...
| case_id | 文件 | 期望 |
|---------|------|------|
| `case_method_call` | `tests/trait/main.sx` | 编译运行 exit **42** |
| `case_no_impl` | `tests/trait/method_no_impl.sx` | typeck `no impl for type` |
| `case_missing_method` | `tests/trait/impl_missing_method.sx` | typeck `missing method` |
