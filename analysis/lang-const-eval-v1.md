...
const A: i32 = 3;
const B: i32 = A + 2;
return B * 2;  // → exit 10（见 tests/let-const/const_expr.x）
```
...
| case_id | 文件 | 期望 |
|---------|------|------|
| `case_binop_mix` | `tests/lang-const/c_binop_mix.x` | exit **20** |
| `case_bitwise` | `tests/lang-const/c_bitwise.x` | exit **131** |
| `case_shift` | `tests/lang-const/c_shift.x` | exit **0** |
| `case_compare` | `tests/lang-const/c_compare.x` | exit **2** |
| `case_unary` | `tests/lang-const/c_unary.x` | exit **260** |
| `case_logical` | `tests/lang-const/c_logical.x` | exit **1** |
| `case_const_chain` | `tests/lang-const/c_const_chain.x` | exit **20** |
| `case_mod_div` | `tests/lang-const/c_mod_div.x` | exit **7** |
| `case_nested` | `tests/lang-const/c_nested.x` | exit **13** |
| `case_hex` | `tests/lang-const/c_hex.x` | exit **42** |
| `case_array_len` | `tests/lang-const/c_array_len.x` | exit **4** |
...
`tests/wpo/const_spec_fold.x`：`scale(1024,64)` 调用点特化折叠（**C6-codegen** 延伸）；由 `run-wpo-s2.sh` 覆盖，本 gate 仅登记 symbol 锚点。
