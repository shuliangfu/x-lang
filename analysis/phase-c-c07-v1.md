...
> **目标**：`.sx` 前端编译器（shux / shux_asm）与 C 前端（shux-c）对**同一输入**行为一致（v1 不测二进制像素级 / AST 字节级）。
