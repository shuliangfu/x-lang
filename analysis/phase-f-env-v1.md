...
> **F-env v1**：删除 **`env.c`**；KV 解析/烟测/args_iter 在 **`env.sx`**；OS getenv/setenv/iter 在 **`compiler/src/asm/runtime_env_os.c`**（F-ZC）。

## 变更

| 项 | 前 | 后 |
|----|----|-----|
| 实现 | `env.c`（424 行） | `env.sx` + `runtime_env_os.c` |
| `env.o` | `cc -c env.c` | 纯 `shux -backend asm env.sx` |
