...
| `bootstrap-driver-seed` | ~2995 | **shell 编排** + shell ensure_prereqs（wave744）+ 薄叶子 | `xbuild bootstrap` | 🟡 wave717–802：… → **B7A heat dep-thin FORCE 113**（wave791–797 · orch closed · `HEAT_RESIDUAL=0`）· **preflight wave798** · **execute-gate wave799** · **proof harness wave800** · **status-flip-preview wave801** · **status-flip-apply harness wave802**；residual **物理删**（Windows 绿 + STATUS flip 后） | L4 必经；列表 mk catalog shell 主路径；冷 rebuild residual_make=0；heat shell 可 `xbuild heat-o`；`xbuild phys-del-gate` 硬拒删 + proof + flip-preview/apply；11.3.1 物理删仍 ⬜ |
...
| `./xbuild compiler-all` | `scripts/compiler_all_ci.sh`（wave784） | CI；`xlang`=g05（wave786 B7D）+ `xlang-c` seed；冷 residual_make=0（wave787）；catalog shell 0-make（wave788）；heat try-heat（wave789）· thin-unify（wave790）· dep-thin FORCE 113（wave791–797）；preflight wave798；execute-gate wave799；proof harness wave800；status-flip-preview wave801；status-flip-apply harness wave802；physical delete residual | 物理删后 0-make（Windows 绿后） |
| `./xbuild heat-o OUT.o` | `ensure_host_cc_seed_o.sh try-heat`（wave789–802） | B7A heat shell 自动分发；recipes try-heat（wave790）；**113 FORCE** dep-thin（wave791–797 · orch closed · `HEAT_RESIDUAL=0`）；preflight/execute-gate/proof/flip-prep/apply wave798–802；非物理删 | 物理删 Makefile 后主热路径（Windows 绿后） |
| `./xbuild phys-del-gate` | `phys_del_makefile_gate.sh`（wave799–802） | 硬拒删 Makefile（Windows 未绿）；dry-run；MSYS min-gate + **proof stamp**（wave800）；`--verify-windows-proof`；**`--status-flip-preview`**（wave801 · 计划 only）；**`--status-flip-apply`**（wave802 · proof+confirm；树仍 not_reproven）；**非**物理删 | Windows 绿 + confirm apply + commit 后才允许删波 |
