...
- **build_xlang_asm.sh make-call cleanup**：A+B类 6处已清理（wave928/929；详见自举进度.md §6）；仍剩 6 处 make（C类长串 companion list + C类复合 targets）
- **条数**：4
- **权威（G.7）**：
...
| 3370 | `xlang-x-pipeline` | 🟢 wave845 shell · wave879 thin · wave946 bags · **wave947 ensure 0-make**（try-heat/migrate/asm-host） |
...
| ~2792 | `verify-selfhost-stage2-bstrict` | 🟢 wave893 shell `scripts/verify-selfhost-stage2-bstrict.sh`；**2026-08-24** tip：Darwin pipeline `__text=0B` 清＋Step2 真 `asm_only_strict` @ `881283463`；stubs filt UNDEF 清 @ `dc555af5b`；Ubuntu hash STRICT 仍失配；standalone seed skip @ `a6e8e05c0` |
...
| 3048 | `pipeline_bootstrap_orchestration.o` | 🟢 wave797 FORCE+try-heat（shell mtime orch twin） |
...
- **wave904** · B7B R1_MISC_BASENAME multi-target FORCE thin try-heat：9 leaves → `$(R1_MISC_BASENAME_OBJS)`（`mk/driver_seed_r_lists.mk` 既有 · G.7 有则补全）+ Makefile multi-target FORCE try-heat（收 link_abi_user_env / channel/scheduler/kv_mmap glue / enc_c ×2 / ldpc ctx / strict_minimal / runtime_asm_build）。**非**物理删。
- **wave905** · B7B R1_SEED_MAP multi-target FORCE thin try-heat：5 leaves → `$(R1_SEED_MAP_OBJS)`（`mk/driver_seed_r_lists.mk` 既有 · G.7 有则补全）+ Makefile multi-target FORCE try-heat（收 target_cpu / ast_seed / pipeline_bootstrap_orchestration / parser_asm_thin_glue / pipeline_glue_standalone）。**非**物理删。R1 multi-target family closed。
- **wave907** · B7B ASYNC_THREE multi-target FORCE thin try-heat：3 leaves → `$(ASYNC_THREE_SEED_OBJS)`（`mk/driver_seed_r_lists.mk` · G.7 有则补全 try-async-prefer）+ Makefile multi-target FORCE try-heat（收 async_liveness/cps/asm_pool）。**非**物理删。
- **wave906** · B7B R3_COLD multi-target FORCE thin try-heat：9 leaves → `$(R3_COLD_SEED_OBJS)`（`mk/driver_seed_r_lists.mk` 既有 · G.7 有则补全）+ Makefile multi-target FORCE try-heat（收 rio/rdabi/rdd/simd_*/backend_*_dispatch）。**非**物理删。
...
- **wave896** · B7B driver_leaf product edges list→mk multi-target thin：8 leaves → `mk/driver_leaf_product_objs.mk` + Makefile `$(DRIVER_LEAF_PRODUCT_OBJS)` multi-target FORCE ensure（收 8 per-leaf）。**非**物理删。
