...
| `bootstrap-driver-seed` | ~2995 | **shell 编排** + shell ensure_prereqs（wave744）+ 薄叶子 | `xbuild bootstrap` | 🟡 wave717–792：… → **B7A heat dep-thin FORCE 86**（wave791–794）；residual 物理删（Windows 绿后）· cfg_eval·asm·gen·stamp·std merge·gen_x | L4 必经；列表 mk catalog shell 主路径；冷 rebuild residual_make=0；heat shell 可 `xbuild heat-o`；11.3.1 物理删仍 ⬜ |
...
| `./xbuild heat-o OUT.o` | `ensure_host_cc_seed_o.sh try-heat`（wave789–793） | B7A heat shell 自动分发；recipes try-heat（wave790）；86 FORCE dep-thin（wave791–794）；`seed_project_hdrs_newer` + `force_thin_makefile_flags_newer`；非物理删 | 删其余 Makefile dep 边后主热路径 |
