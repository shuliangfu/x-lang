...
| `bootstrap-driver-seed` | ~2995 | **shell 编排** + shell ensure_prereqs（wave744）+ 薄叶子 | `xbuild bootstrap` | 🟡 wave717–753：编排+链接+§5b+prereq 边 shell；叶 pattern 库存（wave746）+ **R4 mode catalog**（wave747）+ **R1 rt-slice**（wave748）+ **R1 core-seed**（wave749）+ **R1 frontend-glue**（wave750）+ **R1 main-runtime**（wave751）+ **R1 alias-stubs**（wave752）+ **R1 extra-cflags**（wave753）；其它 pattern 体仍 make | L4 必经；列表 mk catalog；make 无 prereq 图；11.3.1 物理删仍 ⬜ |
