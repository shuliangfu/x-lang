...
| `bootstrap-driver-seed` | ~2995 | **shell 编排** + shell ensure_prereqs（wave744）+ 薄叶子 | `xbuild bootstrap` | 🟡 wave717–799：… → **B7A heat dep-thin FORCE 113**（wave791–797 · orch closed · `HEAT_RESIDUAL=0`）· **preflight wave798** · **execute-gate wave799**；residual **物理删**（Windows 绿后） | L4 必经；列表 mk catalog shell 主路径；冷 rebuild residual_make=0；heat shell 可 `xbuild heat-o`；`xbuild phys-del-gate` 硬拒删；11.3.1 物理删仍 ⬜ |
...
| `./xbuild heat-o OUT.o` | `ensure_host_cc_seed_o.sh try-heat`（wave789–799） | B7A heat shell 自动分发；recipes try-heat（wave790）；**113 FORCE** dep-thin（wave791–797 · orch closed · `HEAT_RESIDUAL=0`）；preflight/execute-gate wave798–799；非物理删 | 物理删 Makefile 后主热路径（Windows 绿后） |
| `./xbuild phys-del-gate` | `phys_del_makefile_gate.sh`（wave799） | 硬拒删 Makefile（Windows 未绿）；dry-run；MSYS min-gate 入口；**非**物理删 | Windows 绿后才允许删波 |
