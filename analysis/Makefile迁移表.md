...
| `bootstrap-driver-seed` | ~2995 | **shell 编排** + shell ensure_prereqs（wave744）+ 薄叶子 | `xbuild bootstrap` | 🟡 wave717–792：… → **B7A heat dep-thin pure seed+.x(+.h) 78 FORCE**（wave791–793）；residual 物理删（Windows 绿后）· twin·c multi·asm·gen dep 边 | L4 必经；列表 mk catalog shell 主路径；冷 rebuild residual_make=0；heat shell 可 `xbuild heat-o`；11.3.1 物理删仍 ⬜ |
...
| `./xbuild compiler-all` | `scripts/compiler_all_ci.sh`（wave784） | CI；`xlang`=g05（wave786 B7D）+ `xlang-c` seed；冷 residual_make=0（wave787）；catalog shell 0-make（wave788）；heat try-heat（wave789）· thin-unify（wave790）· dep-thin FORCE 78（wave791–793）；twin·c multi·asm·gen residual | 物理删后 0-make / 再缩 twin·c multi·asm·gen |
| `./xbuild heat-o OUT.o` | `ensure_host_cc_seed_o.sh try-heat`（wave789–793） | B7A heat shell 自动分发；recipes try-heat（wave790）；78 pure seed+.x(+.h) FORCE dep-thin（wave791–793）；`seed_project_hdrs_newer`；非物理删 | 删其余 Makefile dep 边后主热路径 |
