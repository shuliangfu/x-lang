...
| `bootstrap-driver-seed` | ~2995 | **shell 编排** + shell ensure_prereqs（wave744）+ 薄叶子 | `xbuild bootstrap` | 🟡 wave717–789：编排+链接+§5b+prereq 边 shell；叶 pattern 库存→R1–R6 prefer/pure-ld → phys-del prep（wave777）→ Windows/双端闸门（wave778）→ **B1–B6 吞体**（wave779–784）→ **B7 DAG 库存**（wave785）→ **B7D TARGET→g05**（wave786）→ **B7A 冷 residual_make=0**（wave787）→ **B7B shell catalog**（wave788）→ **B7A heat try-heat**（wave789）；residual 物理删（Windows 绿后）· Makefile thin-call 边 | L4 必经；列表 mk catalog shell 主路径；冷 rebuild residual_make=0；heat shell 可 `xbuild heat-o`；11.3.1 物理删仍 ⬜ |
...
| `./xbuild compiler-all` | `scripts/compiler_all_ci.sh`（wave784） | CI；`xlang`=g05（wave786 B7D）+ `xlang-c` seed；冷 residual_make=0（wave787）；catalog shell 0-make（wave788）；heat try-heat（wave789）；Makefile thin-call 边 residual | 物理删后 0-make / 缩 thin-call 边 |
| `./xbuild heat-o OUT.o` | `ensure_host_cc_seed_o.sh try-heat`（wave789） | B7A heat shell 自动分发（prefer→R1→R2→gen）；非物理删 | 删 Makefile thin-call 边后主热路径 |
