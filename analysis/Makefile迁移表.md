...
| `./xbuild phys-del-gate` | `phys_del_makefile_gate.sh`（wave799–809） | 硬拒删 Makefile（never-rm 体；wave808 后 ENDGAME=1 + TREE_ARMED=1 仍拒）；dry-run；MSYS min-gate + proof；STATUS flip（wave804）；tree arm（wave808）；delete-body-preview（wave809）；**非**物理删 | confirm 物理删体波才允许 `rm` |
