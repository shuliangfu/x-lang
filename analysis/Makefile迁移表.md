...
| `bootstrap-driver-seed` | ~2995 | **shell 编排** + shell ensure_prereqs（wave744）+ 薄叶子 | `xbuild bootstrap` | 🟡 wave717–760：编排+链接+§5b+prereq 边 shell；叶 pattern 库存（wave746）+ **R4 mode catalog**（wave747）+ **R1 八族**（wave748–755）+ **R4 pure-R1 try-r1**（wave756）+ **R3 cold-else try-r3-cold**（wave757）+ **thin_glue seed-map**（wave758）+ **glue-standalone seed-map**（wave759）+ **R2 panic cold try-r2**（wave760）；R4 remaining（gen/pipeline-x）体仍 make | L4 必经；列表 mk catalog；make 无 prereq 图；11.3.1 物理删仍 ⬜ |
