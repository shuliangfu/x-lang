...
| 2026-07-11 | **f-301 R10-lite**：`rt_entry.*`（explain CLI + smoke_diag + smoke_summary + fmt_report_no_files）；`SHUX_RT_ENTRY_FROM_X` |
| 2026-07-11 | **f-302 rest diag errno**：`rt_diag_errno.*`（code_for_kind + errno/path/pair + cli_usage_note）；`SHUX_RT_DIAG_ERRNO_FROM_X` |
| 2026-07-11 | **f-303 rest emit state**：`rt_emit_state.*`（-E path/lib/n/extern 状态 + setters）；`SHUX_RT_EMIT_STATE_FROM_X` |
| 2026-07-11 | **f-304 rest emit argv + elf diag**：`driver_argv_parse_x_emit_c` 并入 `rt_emit_state`；`rt_pipeline_elf_diag.*`（`runtime_pipeline_elf_ctx_diag_note`）；`SHUX_RT_PIPELINE_ELF_DIAG_FROM_X`；hybrid **十二切片+rest** |
| 2026-07-11 | **f-305 rest lib_root**：`rt_lib_root.*`（`ptr_usable` / `default` / `roots_from_key`）；`SHUX_RT_LIB_ROOT_FROM_X`；hybrid **十三切片+rest** |
| 2026-07-11 | **f-306 R2 扩**：`driver_source_has_generic/compound_assign_syntax` 并入 `rt_content`（path→peek→content_has）；仍 `SHUX_RT_CONTENT_FROM_X` |
| 2026-07-11 | **f-307 rest precise parse diag**：`rt_parse_diag.*`（`runtime_report_precise_parse_failure_if_known`）；`SHUX_RT_PARSE_DIAG_FROM_X`；hybrid **十四切片+rest** |
| 2026-07-11 | **f-308 rest fs open thin**：`rt_fs_open.*`（`driver_fs_open_read_path` / `write`）；`SHUX_RT_FS_OPEN_FROM_X`；hybrid **十五切片+rest**；🔒 open |
...
| 2026-07-11 | **f-317 R8-lite stack esc**：`rt_stack.*`（`driver_stack_esc_gate_thread_fn` / `large_stack`）；`SHUX_RT_STACK_FROM_X`；hybrid **二十三切片+rest**；`run-g02f-runtime-rest-empty-gate.sh`（product rest T=0 哨兵）；pthread 体仍 `driver_run_thread_on_large_stack` 🔒 |
