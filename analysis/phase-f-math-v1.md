...
> **F-math v1**：删除 **`math.c`**；**`math.sx`** + **`compiler/src/asm/runtime_math_libm.c`**（F-ZC）；链接 exe 仍须 **-lm**。

## 变更

| 项 | 前 | 后 |
|----|----|-----|
| 实现 | `math.c` | `math.sx` + `runtime_math_libm.c` |
| `math.o` | `ld -r` sx + glue | 纯 `shux -backend asm math.sx` |
