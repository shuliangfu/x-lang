...
| **S5** ✅ | **std.fs**（高性能文件） | core, 可选 std.io | 已实现：extern open/read/write/close + fs_open_read/fs_close/fs_read/fs_write 大块读写；可直接用于读 .sx、写目标文件。 |
...
> 与现有代码注释、`runtime/README.md`、`std/io/core.sx` / `std/io/driver.sx` 一致；实现「自己的高性能 shux io」。
...
- `std/io/core.sx`、`std/io/driver.sx`、`std/io/mod.sx` — 舒 IO 与高并发 IO 注释
- `std/fs/mod.sx` — 当前 fs 薄封装
- `std/string/README.md` — std.string API 与性能说明
- `std/net/mod.sx` — 网络与 driver 统一读写约定
- `std/mem/mod.sx` — Buffer ABI 与 shu_io_register
