# Shux Registry 包管理网站架构规划

> **文档目的**：规划 Shux 官方包管理网站（Shux Registry，域名 `registry.shuxlang.com`）的整体架构、实施路线与依赖格式。
>
> **配套文档**：[shux-toml-实现分析.md](shux-toml-实现分析.md)、[shux.toml](../shux.toml)、[自举方法.md](自举方法.md)。
>
> **域名状态**：`registry.shuxlang.com` 已注册（2026-07-21 确认）。
>
...
**自举完成（0.3）后，0.4-0.6 做本地包管理器 + shux.toml `[dependencies]` 格式定型；0.7 正式上线 `registry.shuxlang.com`（后端用 SHUX 自身编写 + PostgreSQL）；1.0+ 扩展私有仓库 / CI 集成 / 安全扫描。现在全力冲刺自举，包管理网站不阻塞当前主线。**
...
| 项 | 依赖自举？ | 说明 |
|----|-----------|------|
| shux.toml `[dependencies]` 格式 | 否 | 纯 schema 设计，0.4 即可定型 |
| 本地包管理器（shux add/publish） | **是** | 需 SHUX 编译器稳定 |
| Registry 后端用 SHUX 写 | **是** | 自举完成后练手，验证语言能力 |
...
| 域名 | 用途 | 优先级 | 状态 |
|------|------|--------|------|
| `shuxlang.com` | 主站 / 官网 | P0 | 已注册 |
| `registry.shuxlang.com` | 官方包仓库 | P0 | **已注册** |
| `docs.shuxlang.com` | 文档站 | P1 | 待注册 |
| `api.shuxlang.com` | Registry 后端 API | P1 | 待注册（或复用 registry 子域） |
| `play.shuxlang.com` | 在线 Playground | P2 | 待注册 |
| `cdn.shuxlang.com` | 静态资源 / 包下载加速 | P2 | 待注册 |

### 2.2 Registry 命名

候选名：
- **Shux Registry**（推荐，简洁专业）
- Shux Package Registry
- Shux Crates（类似 crates.io）
- Shux Hub

**采用：Shux Registry**，对外品牌名统一用此。

### 2.3 立即可做的准备

| 项 | 动作 | 时机 |
|----|------|------|
| DNS 解析 | `registry.shuxlang.com` 指向 placeholder 页 | 现在 |
...
│                   用户层（User-facing）                    │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────┐  │
│  │ Web UI      │  │ shux CLI    │  │ docs.shuxlang   │  │
│  │ (React/TS)  │  │ (shux add/  │  │ (自动生成)       │  │
│  │             │  │  publish)   │  │                 │  │
│  └──────┬──────┘  └──────┬──────┘  └────────┬────────┘  │
└─────────┼────────────────┼─────────────────┼────────────┘
          │                │                 │
...
┌──────────────────────────────────────────────────────────┐
│                   API 网关层（API Gateway）                │
│  registry.shuxlang.com  ·  api.shuxlang.com              │
│  ┌──────────────────────────────────────────────────┐    │
...
┌──────────────────┐ ┌──────────────┐ ┌──────────────────┐
│ PostgreSQL       │ │ 对象存储      │ │ CDN              │
│ (元数据/用户/版本) │ │ (包 tarball) │ │ cdn.shuxlang.com │
│                  │ │ S3/MinIO     │ │ (下载加速)        │
└──────────────────┘ └──────────────┘ └──────────────────┘
```
...
| **后端** | **SHUX 自身编写** | 自举完成后练手，验证语言能力（HTTP server + JSON + DB driver） |
...
**核心理由**：
1. **自举验证**：Registry 是真实生产负载，验证 SHUX 的 HTTP/JSON/DB/并发能力
2. **dogfooding**：用 Shux 写 Shux 生态基础设施，最强信任信号
3. **性能优势**：SHUX 无 GC / zero-copy，适合高并发 API server
...
**风险缓解**：若 SHUX 某项能力在 0.7 仍未就绪，后端可临时用 Go/Rust 写过渡层，SHUX 逐步替换。
...
CREATE TABLE package_versions (
  ...
  shux_version  VARCHAR(64),                   -- 所需 shux 版本
  size_bytes    BIGINT,
...
Authorization: Bearer <token>

  {
    ...
    "tarball": <binary .shuxpkg>
  }
```

**tarball 格式**：`.shuxpkg` = gzip tar，包含 `shux.toml` + `src/` + `README.md` + `LICENSE`。

### 5.3 鉴权

- Web UI：session cookie
- CLI：API token（`shux login` 生成，存 `~/.shux/credentials.toml`）
- 所有 publish 操作需 token + 2FA（管理员）

---

## 6. shux.toml `[dependencies]` 格式
...
[dev-dependencies]
"shux-test-harness" = "0.1"

[build-dependencies]
"shux-codegen-tool" = "0.2"
```
...
### 6.4 lockfile（shux.lock）

每次 `shux build` 生成 `shux.lock`，锁定具体版本：

```toml
# shux.lock — 自动生成，勿手动编辑
[[package]]
name = "std-extra"
version = "0.2.5"
checksum = "sha256:abc123..."
source = "registry+https://registry.shuxlang.com"
...
| 命令 | 用途 | 类比 |
|------|------|------|
| `shux init` | 初始化新项目（生成 shux.toml） | `cargo init` / `npm init` |
| `shux add <pkg>` | 添加依赖到 shux.toml | `npm install` |
| `shux add <pkg> --dev` | 添加 dev 依赖 | `npm install --save-dev` |
| `shux remove <pkg>` | 移除依赖 | `npm uninstall` |
| `shux update` | 更新所有依赖到 lockfile 允许的最新版 | `cargo update` |
| `shux update <pkg>` | 更新单个依赖 | — |
| `shux fetch` | 下载依赖到本地缓存（不构建） | `cargo fetch` |
| `shux build` | 构建（自动解析依赖） | `cargo build` |
| `shux run` | 构建并运行 | `cargo run` |
| `shux test` | 构建并测试 | `cargo test` |
| `shux check` | 类型检查（不生成 binary） | `cargo check` |
| `shux publish` | 发布到 Registry | `cargo publish` / `npm publish` |
| `shux login` | 登录 Registry（存 token） | `npm login` |
| `shux logout` | 登出 | `npm logout` |
| `shux search <query>` | 搜索包 | `npm search` |
| `shux info <pkg>` | 显示包详情 | `npm info` |
| `shux yank <pkg>@<ver>` | yank 版本 | `cargo yank` |

### 7.2 缓存目录

```
~/.shux/
├── credentials.toml          # API token（shux login）
├── registry/
│   └── cache/                # 下载的 tarball 缓存
│       ├── std-extra-0.2.5.shuxpkg
│       └── ...
├── git/                      # git 依赖克隆
│   └── db/
...
### 7.3 全局配置 `~/.shux/config.toml`

```toml
[registry]
default = "https://registry.shuxlang.com"
# 私有 registry（企业版）
...
2. **社区举报**：用户可举报恶意包
3. **管理员审核**：争议包人工审核
4. **Yank 机制**：问题版本可 yank（不删除，但 `shux build` 拒绝新用户）

### 8.3 保留名规则

| 前缀 | 保留给 | 规则 |
|------|--------|------|
| `std-*` | 官方 std 扩展 | 仅 `shuxlang` 组织可发布 |
| `shux-*` | 官方工具 | 仅 `shuxlang` 组织可发布 |
...
**目标**：本地包管理器可用，shux.toml `[dependencies]` 格式定型。

**范围**：
- shux.toml `[dependencies]` 格式定稿（§6）
- `shux add` / `remove` / `update` / `fetch` 命令
- git + path 依赖支持（无需 Registry）
- `shux.lock` lockfile 生成
- 本地缓存 `~/.shux/registry/cache/`
...
- 改：`shux.toml` schema 校验

### 阶段 2（0.7，正式上线）

**目标**：`registry.shuxlang.com` 上线，支持搜索/发布/下载。

**范围**：
- 后端（SHUX 编写）：HTTP server + PostgreSQL + 对象存储
- 前端（React/TS）：搜索/包详情/用户主页/文档
- `shux login` / `publish` / `search` / `info` / `yank` 命令
- semver 解析 + 依赖解析
- CDN 接入
...
**风险**：0.7 时 SHUX 的 HTTP server / PostgreSQL driver 可能不成熟。

**缓解**：
- 0.4-0.6 期间补齐 `std.http` server + DB driver
- 若 SHUX 后端不就绪，过渡期用 Go/Rust 写，SHUX 逐步替换
- TLS 可由 Nginx reverse proxy 卸载，SHUX 后端只处理明文 HTTP
...
| 决策 | 选择 | 理由 |
|------|------|------|
| 域名 | `registry.shuxlang.com` | 已注册；符合行业惯例 |
| 品牌名 | Shux Registry | 简洁专业 |
| 后端语言 | SHUX 自身 | dogfooding + 验证语言能力 |
| 前端 | React + TypeScript | 生态丰富、SSR 友好 |
| 数据库 | PostgreSQL | 成熟、JSON 支持、全文搜索 |
| 对象存储 | MinIO（自建）/ S3（云） | 灵活、成本可控 |
| 包格式 | `.shuxpkg`（gzip tar） | 与 crates.io 一致 |
| lockfile | `shux.lock`（TOML） | 人类可读、git diff 友好 |
| semver range | Cargo 风格（`^` / `~`） | 用户熟悉、规范成熟 |
| 上线时机 | 0.7（2027 Q3-Q4） | 语言稳定 + std 完善 + 社区形成 |
| 包名保留 | `std-*` / `shux-*` / `x-*` | 防抢注、保护官方生态 |
...
| 文档 | 关系 |
|------|------|
| [shux.toml](../shux.toml) | `[dependencies]` 占位，阶段 1 定型 |
| [shux-toml-实现分析.md](shux-toml-实现分析.md) | shux.toml 阶段 4 = 本文档阶段 1 |
...
| 项 | 动作 | 耗时 |
|----|------|------|
| DNS 解析 | `registry.shuxlang.com` 指向 placeholder | 10 分钟 |
| TLS 证书 | Let's Encrypt 申请 | 30 分钟 |
| Placeholder 页 | "Shux Registry · Coming 2027" | 1 小时 |
| 域名续费提醒 | 设置日历提醒 | 5 分钟 |
| shux.toml `[dependencies]` schema | 本文档 §6 已定稿 | 0（已完成） |
