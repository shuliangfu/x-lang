...
| **源码体量 + 调用树** | `typeck_su_ast` → `check_block` / `check_expr` → `check_expr_impl_mega` |
...
| `typeck_su_ast` | per-function loop + library 快路径 |
| `typeck_wpo_unify_soa_layouts` | WPO 跨模块 SoA layout 统一，消除 AoS↔SoA 转换 |
| `typeck_inline_u8_64_array_field_type_ref` | 常见 `[64]u8` 字段类型 inline，跳过通用 layout 查表 |
| `typeck_soa_array_storage_size_glue` | SoA 数组存储尺寸 C glue，减少 SU 递归 depth |
