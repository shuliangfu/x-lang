...
| 层级 | 模式 | 恢复手段 | 代表用例 |
|------|------|----------|----------|
| **R1-unwrap-or** | `Result` Err | `unwrap_or_i32` / `expect_i32` 默认值 | `recovery/r_double_default.x` |
| **R2-or-and** | `or_i32` / `and_i32` | 短路传播或 Err 后默认 | `recovery/r_or_fallback.x` |
| **R3-branch** | `is_ok` 三元 | 显式分支恢复 | `recovery/r_ternary_recover.x` |
| **R4-layer-c** | fs/io 语义码 | `fs_last_error` + 默认 | `layer_c_recoverable.x` |
| **R5-chain** | `ErrorChain` | `error_chain_root` + 分支映射 | `recovery/r_root_from_chain.x` |
...
3. **矩阵可扩展**：新增 `.x` 登记一行 `case_*` 即可纳入 gate。
...
| EXC-003 | 分层错误码 + `error_code_layer.x` |
