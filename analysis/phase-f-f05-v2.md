...
> **F-05 v2**：**`std/db/kv/kv.c`** → **`kv.sx`** + **`kv_mmap_glue.c`**（mmap 胶层）。

## 迁移范围

| 文件 | 说明 |
|------|------|
| `std/db/kv/kv.sx` | WAL + LSM + SST + compact + smoke 全量逻辑 |
...
- **F-05 v3 ✅**：`std/db/sqlite/sqlite.c` 已迁 `sqlite.sx` + `sqlite_glue.c`
