...
> **F-05 v1 v1**：**`std/db/arrow/arrow.c`** → **`arrow.sx`** + **`arrow_simd_glue.c`**（SIMD 归约胶层）。

## v1 完成（✅ manifest）

| 项 | 说明 |
|----|------|
| `arrow.sx` | 列/batch 生命周期、adopt、烟测 |
...
- **F-05 v2**：`std/db/kv/kv.c` 分批迁 `.sx`（mmap/WAL 可复用 std.sys.mmap）
