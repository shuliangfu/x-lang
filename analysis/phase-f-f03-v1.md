...
> **F-03 v1**：`heap_mem_set_c` / `heap_mem_compare_c` / `map_i32_i32_find_c` 从 **`heap.c` 迁至 `std/heap/heap_ops.sx`**（经 `core.mem`）；`heap.c` 仍保留 malloc/realloc/copy/arena/trace。

## v1 完成（✅ manifest）

| 项 | 说明 |
|----|------|
| `heap_ops.sx` | mem set/compare + map 线性探测 |
