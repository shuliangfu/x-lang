...
> **F-04 v13 v1**：从 **`net.c`** 迁出 **IPv4 TCP connect/listen/accept/batch** → **`tcp.sx`**。

## v13 v1 完成（✅ manifest）

| 项 | 说明 |
|----|------|
| `tcp.sx` | `net_tcp_connect_c` / `net_tcp_connect_blocking_c` / `net_tcp_listen_c` / `net_accept_c` / `net_accept_many_c` / `net_connect_many_c` |
| `net.c` | 移除 ~220 行 TCP 实现；保留 `net_run_accept_workers_c*` 与 UDP batch |
| `Makefile` | `net.o` 合并 **8** 个 `.sx` 模块（含 `net_tcp`） |
| 依赖 | `sock.sx`（`net_set_blocking_c` / `net_close_socket_c`）；Linux `io.o` io_uring |
...
| UDP batch | **F-04 v13b ✅** 见 `udp_batch.sx` |
