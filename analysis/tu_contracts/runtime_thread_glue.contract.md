...
- `xlang_cpu_zero`（IMPL，调用 `_impl`，Linux only）
- `xlang_cpu_set`（IMPL，调用 `_impl`，Linux only）

### 3.2 当前仍由 seed/rest 提供
- `xlang_cpu_zero_impl`（cpu_set_t 清零，Linux only）
- `xlang_cpu_set_impl`（cpu_set_t 置位，Linux only）
- 线程基础：`thread_self_c` / `thread_create_c` / `thread_create_with_stack_c` / `thread_join_c`
- 亲和性：`thread_set_affinity_self_c`（调用 `xlang_cpu_zero` + `xlang_cpu_set`）/ `thread_set_affinity_c`（调用 `xlang_cpu_zero` + `xlang_cpu_set`）
...
## 4. ABI Manifest
- _impl 残余列表：`xlang_cpu_zero_impl`、`xlang_cpu_set_impl`
- thin+rest 宏边界：`XLANG_RUNTIME_THREAD_GLUE_FROM_X`
- 前向声明：2 个 thin wrapper（`xlang_cpu_zero` / `xlang_cpu_set`），rest 模式下供 rest 函数调用
- 内部调用更新：`thread_set_affinity_self_c` / `thread_set_affinity_c` 调用 `xlang_cpu_zero` + `xlang_cpu_set`（在 `#elif defined(__linux__)` 块内）
- 类型擦除：.x 侧 `xlang_cpu_zero`/`xlang_cpu_set` 参数为 `*u8`（cpu_set_t 是 Linux 特有类型，.x 无法表达）；seed 侧前向声明与定义用 `cpu_set_t *`，C 链接器只看符号名不看类型，ABI 兼容（均为 64 位指针）
...
- 平台条件：`xlang_cpu_zero` / `xlang_cpu_set` 及其 `_impl` 都在 `#if defined(__linux__)` 块内（Linux only）；macOS 上不编译 `_impl`，merged.o 有 2 个 U 符号（预期行为，Linux 最终链接时解析）
