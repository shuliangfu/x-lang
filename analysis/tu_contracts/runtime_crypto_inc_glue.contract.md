...
- `xlang_sha256_block`（IMPL，调用 `_impl`）
- `xlang_sha256_rotr32`（DIRECT）
- `xlang_sha256_ch`（DIRECT）⚠️ .x 编译器未生成（已知限制）
- `xlang_sha256_maj`（DIRECT）
- `crypto_i32_sub_c`（DIRECT）
- `crypto_rotl32_c`（DIRECT）

### 3.2 当前仍由 seed/rest 提供
- `xlang_sha256_block_impl`（SHA-256 块处理，调用 thin helpers）
...
## 4. ABI Manifest
- _impl 残余列表：`xlang_sha256_block_impl`
- thin+rest 宏边界：`XLANG_RUNTIME_CRYPTO_INC_GLUE_FROM_X`
- 前向声明：`xlang_sha256_rotr32`, `xlang_sha256_ch`, `xlang_sha256_maj`（rest 模式下供 `_impl` 调用）
- 内部调用更新：`crypto_sha256_c` 中 3 处 `xlang_sha256_block` → `xlang_sha256_block_impl`
...
## 6. 备注
- 混合模式：1 IMPL wrapper（`xlang_sha256_block` 调用 `_impl`）+ 5 DIRECT wrapper（直接实现）
- `xlang_sha256_block_impl` 调用 3 个 DIRECT thin helpers（rotr32/ch/maj），rest 模式下为 U 符号由 thin.o 提供
- ⚠️ 已知限制：.x 编译器未生成 `xlang_sha256_ch`（可能内联），rest 模式下 merged.o 中 `xlang_sha256_ch` 为 U 符号。完整模式下 seed 提供 `xlang_sha256_ch`（#ifndef 块内），产品构建不受影响。Phase 3/4 需修复 .x 编译器
