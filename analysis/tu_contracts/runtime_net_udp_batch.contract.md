...
- `xlang_udp_batch_set_addr_port`（IMPL，调用 `_impl`）
- `xlang_udp_batch_poll_readable`（IMPL，调用 `_impl`）

### 3.2 当前仍由 seed/rest 提供
- `xlang_udp_batch_set_addr_port_impl`（填充 IPv4 sockaddr_in）
- `xlang_udp_batch_poll_readable_impl`（poll 可读检查）
- `xlang_net_udp_recvmmsg2_c`（weak，Linux recvmmsg 2 段）
- `xlang_net_udp_sendmmsg2_c`（weak，Linux sendmmsg 2 条）
- `xlang_net_udp_recvmmsg_buf_c`（weak，Linux recvmmsg Buffer 切片）
- `xlang_net_udp_sendmmsg_buf_c`（weak，Linux sendmmsg Buffer 切片）

## 4. ABI Manifest
- _impl 残余列表：`xlang_udp_batch_set_addr_port_impl`、`xlang_udp_batch_poll_readable_impl`
- thin+rest 宏边界：`XLANG_RUNTIME_NET_UDP_BATCH_FROM_X`
- 前向声明：`xlang_udp_batch_set_addr_port`、`xlang_udp_batch_poll_readable`（rest 模式下供 4 个 weak API 函数调用，符号由 thin.o 提供）
...
- 4 个 weak API 函数（`xlang_net_udp_recvmmsg2_c` 等）调用 thin wrapper（`xlang_udp_batch_set_addr_port` / `xlang_udp_batch_poll_readable`），rest 模式下 thin wrapper 为 U 符号由 thin.o 提供
