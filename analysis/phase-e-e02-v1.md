...
> **E-02 v1**：默认 bootstrap / shux-sx / build_shux_asm **不链** `lsp_diag.c`；用 `lsp_diag_stubs_no_c.o` + `lsp_diag_sx.o` + `lsp_diag_sx_alias.c`。文件保留。
...
- definition/hover/references 全迁 `.sx`，删 stubs 中的 null 桩
- `OBJS_CORE`（shux-c）改链 stubs 或纯 `.sx`
