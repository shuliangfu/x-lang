...
| 概念 | v1 映射 |
|------|---------|
| `Hasher::new()` | `hash_start_algo(algo)` |
| `write_bytes` | `hash_write_bytes_algo` |
| `finish` | `hash_finish_algo` |
| `drop` | `hash_free_algo` |
| 默认算法 | `default_hasher()` / `SHUX_HASH_ALGO` |

**Tier-S 稳定 API**（`hash_start` / `hash_bytes` 等）**始终走 SipHash**，与既有 HashMap / 回归兼容。
...
| 默认 | ✅ `hash_start` | 可选 `hash_start_algo(1)` | 同左 |

**结论**：生产默认保持 SipHash；需要吞吐的内存表可在预研阶段显式选 `HASHER_AHASH`，待真 aHash 落地后替换 `hash.c` 内实现而不改 `.sx` API。
