# BOOT-026：parser C4 全量 SU bootstrap v1

> 更新时间：2026-06-18  
> 状态：**定版（v1）**  
> 前置：BOOT-025 gen12 一致性、`run-parser-parse-bootstrap-su-emit-gate.sh`  
> 关联：`boot-mega7-gap.md` 阶段 C4、BOOT-024 SU emit 探测

---

## 1. 目标

在 BOOT-025 C3 gen1/gen2 基础上登记 **C4 SU bootstrap 波次**：无 C TU 切片的 `parser.sx` 全量 emit 探测 + MINIMAL 白名单轨道 + 父波次 gen12。
...
| ID | hook | 说明 |
|----|------|------|
| `c4_su_emit` | `run-parser-parse-bootstrap-su-emit-gate.sh` | SU 全量 emit 探测（无 C TU） |
...
复现矩阵：`tests/baseline/bootstrap-repro.tsv` `boot026_su_c4`。
...
```
shux: [SHUX_BOOT026] status=ok c4_minimal_ok=1 c4_su_probe=1 skip=1
```

- **c4_minimal_ok**：SU MINIMAL 白名单轨道通过（`MINIMAL OK`）
- **c4_su_probe**：SU emit 探测 gate 已执行且 PASS
...
- FULL SU emit `ec=0` 硬门禁（仍依赖 C TU 默认路径）
- Darwin SU bootstrap 硬门禁
- 跨平台 shux_asm2 二进制发布
