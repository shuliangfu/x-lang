...
> **F-log v1**：删除 **`log.c`**；格式化在 **`log.sx`**；sink/轮转/异步在 **`compiler/src/asm/runtime_log_os.c`**（F-ZC）。

## 变更

| 项 | 前 | 后 |
|----|----|-----|
| 实现 | `log.c`（386 行） | `log.sx` + `runtime_log_os.c` |
| `log.o` | `cc -c log.c` | 纯 `shux -backend asm log.sx` |
