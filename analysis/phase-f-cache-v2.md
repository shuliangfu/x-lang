# 阶段 F-cache v2（std.cache 逻辑 .sx 下沉）

> **F-cache v2**：**LRU/TTL + 对象池** 全量在 **`cache.sx`**；**删除 `cache_glue.c`**；`cache.o` 纯 `.sx` 编译（同 encoding）。

## 变更

| 项 | v1 | v2 |
|----|----|-----|
| LRU/池实现 | `cache_glue.c`（395 行） | **`cache.sx`** |
| `cache.o` | `ld -r` glue + sx | **纯 shux → cache.o** |
