...
> **E-03 v2 preprocess**：默认 bootstrap / shux-sx / bootstrap-driver-seed **不链** `preprocess_for_driver.o`；`runtime preprocess()` 在 `SHUX_USE_SX_PIPELINE` 下走 `preprocess.sx`（`su_preprocess_raw_to_malloc`）。`preprocess.c` 文件保留。

## v2 preprocess 完成（✅）

| 项 | 标准 | 产物 |
|----|------|------|
| runtime | `SHUX_USE_SX_PIPELINE` 默认走 SX；`SHUX_LEGACY_PREPROCESS_C` 走 C | `compiler/src/runtime.c` |
...
## 延后（E-03 v3+）
- `OBJS_CORE`（shux-c）改默认 SX preprocess
