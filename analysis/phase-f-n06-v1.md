...
> **NL-06 v1**：在 NL-01～05 之上，登记 freestanding `.sx` 替换模块。**F-03 后**：hosted `heap.c`/`fs.c`/`io.c` 已删；bootstrap 改链 **-lc**（track 见 `nolibc-n06-freestanding-replacements.tsv`）。
...
| 替换 manifest | freestanding `.sx` ↔ legacy `.c` 对照 | `tests/baseline/nolibc-n06-freestanding-replacements.tsv` |
...
**约定**：freestanding 程序应 `import std.heap.page_mmap`、`std.fs.freestanding_linux`、`std.net.freestanding_linux`；hosted 走 `heap_libc` / `fs_posix` / `io_backend` 纯 `.sx`。
...
- 编译器 bootstrap 改链 freestanding `.sx` 对象并启用 `-nostdlib`（NL-07 v2）
