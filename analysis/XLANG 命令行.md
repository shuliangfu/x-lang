...
> **文档目的**：评估将 `xlang` 命令行从「隐式推断」改为「强制子命令」的可行性、爆炸半径与迁移路径。
>
> **最后更新**：2026-07-20

---

## 1. 当前 CLI 形式（`xlang --help` 实测）

```
Usage:
  xlang [options] file.x          compile and run        ← 隐式（要删）
  xlang build [file.x] [-o exe]   compile (default a.out)
  xlang run file.x                compile and run
  xlang check [paths...]           parse + typeck only
  xlang fmt [--check] [paths...]   format .x sources
  xlang explain <CODE>             explain a diagnostic code
  xlang test [script.sh]           run test script
...
| 形式 | 语义 | 示例 |
|------|------|------|
| `xlang file.x` | 编译并运行（隐式） | `xlang main.x` |
| `xlang [options] file.x` | 编译（隐式，有 `-o` 时）或运行 | `xlang -backend asm -o out.o main.x` |
| `xlang <subcmd> ...` | 显式子命令 | `xlang build main.x -o out` |
...
- ❌ 禁止 `xlang file.x`（隐式运行）
- ❌ 禁止 `xlang --参数 file.x`（隐式编译）
- ✅ 编译必须用 `xlang build file.x [-o exe]`
- ✅ 运行 `.x` 源文件必须用 `xlang run file.x`
- ✅ 其他子命令（`check` / `fmt` / `explain` / `test`）保持不变

无子命令时输出错误并退出：
```
Error: missing subcommand. Use 'xlang build ...', 'xlang run ...', 'xlang check ...', 'xlang fmt ...', 'xlang explain ...', or 'xlang test ...'.
```
...
`xlang file.x` 当前歧义：到底是「编译」还是「解释执行」？强制子命令后，`build` 与 `run` 职责分离，CLI 成为纯路由层。

### 3.2 启动路径唯一化

`xlang run` 可建立独立的极简入口（零 libc 加载器），跳过 Linker 与 File I/O，直接 `Backend → PROT_EXEC 内存 → 执行`。`xlang build` 专注代码生成，不耦合运行时逻辑。

### 3.3 符合现代编译工具链惯例

`cargo build/run`、`go build/run`、`rustc`、`zig build/run` 均采用显式子命令。XLANG 对齐主流。

### 3.4 错误更早暴露

裸 `xlang file.x` 打错参数时仍可能「误打误撞」走隐式路径；强制子命令后参数错误立即报错。
...
| 位置 | 形式 | 数量 |
|------|------|------|
| `compiler/makefile` | `./xlang -backend asm ... file.x -o ...` | ~2 处实际编译（L250, L266），其余为 `xlang=./xlang` 变量赋值 |
| `compiler/scripts/xlang_compile_std_module.sh` | `"$XLANG_BIN" -x -E ...` / `"$XLANG_BIN" -backend c ...` | ~8 处（L82, L121, L124, L236, L251, L252, L259, L260） |
| `compiler/scripts/smoke_asm_peephole_metric.sh` | `eval "$XLANG -backend asm -o ..."` | 1 处（L18） |
| `compiler/scripts/build_xlang_asm.sh` | `$XLANG -backend asm ...` | 数处（含 echo/info，需筛实际调用） |
| `compiler/scripts/g05_*.sh` / `relink_*.sh` / `driver_leaf_x_to_o.sh` | `xlang -O3 ...` / `xlang -E ...` / `xlang -freestanding ...` | ~10-15 处 |
| `tests/run-*.sh` | `$XLANG ... file.x` 或 `$XLANG -backend ... file.x` | 需单独统计（gate 脚本） |

**估算总数**：约 **30-50 处**隐式调用需迁移为 `xlang build`。

### 4.3 bootstrap 链影响

`bootstrap_xlangc_create.sh` / `bootstrap_xlang_create.sh` 等核心 bootstrap 脚本**已使用 `xlang build`** 显式形式（Explore 确认）。bootstrap 链爆炸半径小，但需复核所有 `bootstrap_*.sh`。

### 4.4 自举完整性影响（关键）

XLANG 处于自举关键期。若 `xlang` 二进制改了 CLI，则：
- **seed bootstrap_xlangc**：如果 seed 代码内部用旧形式调用 `xlang`（通过 `system()` 或 exec），自举链会断
- **必须检查** `compiler/seeds/*.from_x.c` 中是否有 `system("xlang ...")` 或类似调用
...
把所有脚本中的隐式调用改为 `xlang build`：

```bash
# 旧：
xlang -backend asm -o out.o main.x
xlang -O3 file.x -o file.o
xlang -E -freestanding file.x -o file.o
"$XLANG_BIN" -x -E -lib-name "" file.x

# 新：
xlang build -backend asm -o out.o main.x
xlang build -O3 file.x -o file.o
xlang build -E -freestanding file.x -o file.o
xlang build -x -E -lib-name "" file.x
```

涉及文件：
- `compiler/makefile`（L250, L266 等）
- `compiler/scripts/xlang_compile_std_module.sh`
...
2. 改为报错退出：
   ...
   fprintf(stderr,
     "Error: missing subcommand. Use 'xlang build ...', 'xlang run ...', "
     "'xlang check ...', 'xlang fmt ...', 'xlang explain ...', or 'xlang test ...'.\n");
   return 2;
   ```
3. 同步修改 seed 镜像（`compiler/seeds/main.from_x.c` 等），保证 `.x` 与 seed C 同语义（AGENTS.md §4 禁止双权威）

**Phase 2 验证**：
- macOS + Ubuntu 双端冷编译（L4：删全部 .o 重编 xlang/xlang_asm/xlang-c/bootstrap_xlangc）
- `run-all-bstrict.sh` 全绿
- Windows gate（`XLANG_WIN_BSTRICT=1 ./tests/run-bootstrap-bstrict-windows-gate.sh`）全绿
- 手测 `xlang file.x` 报错退出码 2

**Phase 3：seed 内部调用复核**

grep `compiler/seeds/*.from_x.c` 中所有 `system("xlang ...")` 调用，确认无旧形式。若有，迁为 `system("xlang build ...")`。

### 6.2 `xlang run` 性能优化（可选，Phase 2 之后）

当前 `xlang run file.x` 可能仍走「编译到临时文件 → 执行」流程。可优化为：
...
- [ ] Phase 1：所有脚本迁移完成，`grep -rn 'xlang \$\|xlang -' compiler/scripts/*.sh compiler/makefile tests/*.sh` 无旧形式残留
...
- [ ] Phase 2：`xlang file.x` 输出错误并 exit 2
- [ ] Phase 2：`xlang --help` 更新 Usage（删除 `xlang [options] file.x` 行）
- [ ] Phase 2：L4 冷编译双端绿（macOS arm64 + Ubuntu x86_64）
- [ ] Phase 2：Windows gate 全绿
- [ ] Phase 3：`grep -rn 'system("xlang ' compiler/seeds/` 无旧形式
...
1. `refactor(scripts): migrate implicit xlang invocations to explicit build subcommand`
   - 仅改脚本 + Makefile + tests，不改 CLI 代码
   - 验证：bstrict 全绿

2. `feat(cli): require explicit subcommand; reject bare xlang file.x`
   - 改 `main.x` L838 + seed 镜像 + `--help` 文本
   - 验证：L4 双端冷编译 + 全量 bstrict + Windows gate

3.（可选）`perf(run): in-memory exec path for xlang run`
   - `xlang run` 跳过 File I/O，直接 mmap 执行
   - 独立优化，单独验证
...
曾考虑折中方案 A：**保留 `xlang file.x` 隐式运行，只禁止 `xlang --参数 file.x` 隐式编译**。经分析否决，选 B。

### 9.1 选项 A 的内部矛盾

若保留 `xlang file.x` 隐式运行，则 `xlang run file.x` 如何处置：

| 处置 | 后果 |
|------|------|
| 保留 `xlang run` | `xlang file.x` 与 `xlang run file.x` 做同一件事 = **双权威**，违反 AGENTS.md §3「禁止功能重复实现」 |
| 删掉 `xlang run` | Usage 不对称：`xlang file.x`（隐式）+ `xlang build`（显式）混用，违反一致性 |
...
| 扩展性 | `xlang file.x --args` 与 xlang flag 冲突 | `xlang run file.x -- --args` 用 `--` 分隔 |
| 错误暴露 | 打错参数可能误走隐式路径 | 立即报错 |
| 爆炸半径 | 迁移 ~30-50 处隐式编译 | 迁移 ~30-50 处 + 少量隐式运行 |

### 9.3 「手感」论不成立

「保留 `xlang file.x` 跑起来更快」这个直觉**只对脚本语言成立**。XLANG 是系统语言（no GC, zero-copy, 自举, 对标 C/Zig/Rust），对标的不该是 `python file.py` / `node file.js`，而是：

- `zig run file.zig`（Zig 强制子命令，无裸 `zig file.zig`）
- `cargo run` / `go run file.go`（Rust/Go 都强制子命令）
- `rustc file.rs`（只编译，运行靠 cargo）

`xlang run main.x` 比 `xlang main.x` 只多 4 字符，换来：单一权威（§3 合规）+ Usage 对称（build/run/check/fmt/test 全显式）+ 未来 `xlang run file.x -- --program-arg=1` 可分隔参数。

### 9.4 爆炸半径对比

脚本里「直接 `xlang file.x` 跑」**很少**——脚本通常要控制输出（编译成 .o / .exe），不会直接跑。大部分隐式调用是带 `-o` / `-backend` 的**编译**。全库 grep 确认 `xlang file.x` 裸运行形式在 scripts/ 和 makefile 里几乎没有。所以 A 和 B 爆炸半径差距很小，**多改几处换长期清晰值得**。
...
| bootstrap_xlangc seed 兼容性未知 | `bootstrap_xlangc` seed（6月30日 pin）可能用旧形式调用 xlang。改 CLI 后若 seed 内部 `system("xlang file.x")` 失败，**自举链断**，需先验证 seed 兼容性，分散冲刺精力 |
...
| 自举链风险 | bootstrap_xlangc 兼容性未知 | 已自举完成，无风险 |
| 基线 | 破坏 bstrict L4 基线 | 有完整测试基线可验证 |
| 精力 | 与 Windows 问题 + 自举冲刺冲突 | 专注工程优化 |
| `xlang run` mmap 优化 | 无法同期做 | 可一次性完成 |
...
xlang build main.x -o out
xlang run main.x

# 而非：
xlang main.x
xlang -backend asm main.x -o out
```
...
- **自举完成后**：按 §6 三阶段方案执行（Phase 1 脚本迁移 → Phase 2 CLI 改造 → Phase 3 `xlang run` mmap 优化）
- **过渡期**：新脚本用显式形式，旧脚本不动
...
- `xlang --print-target-cpu` → 打印 CPU 特性后退出
- `xlang --explain XP003` → 解释诊断码后退出

它们应该和 `build`/`run`/`check` 等**子命令并列**或单独列为「全局 action」。当前混在 options 里会让用户误以为要 `xlang build --print-target-cpu file.x`。

**问题 3：子命令自身 options 未展开**

`xlang fmt [--check]` 的 `--check` 是 fmt 专属 flag，只在 Usage 行带过，没在 options 区展开。`xlang build [-o exe]` 同理。每个子命令应支持 `xlang <subcmd> --help` 查看自己的 flag。

### 11.2 推荐 `--help` 结构

```
Usage:
  xlang <subcommand> [options] [args]

Subcommands:
  xlang build [options] file.x [-o exe]   Compile .x to binary/object
  xlang run   [options] file.x            Compile and run .x
  xlang check [paths...]                  Parse + typeck only
  xlang fmt   [--check] [paths...]        Format .x sources
  xlang explain <CODE>                    Explain a diagnostic code
  xlang test  [script.sh]                 Run test script
...
Global actions (no subcommand):
  xlang --print-target-cpu                Print host CPU features and exit
  xlang --explain <CODE>                  Print diagnostic code explanation and exit
  xlang -h, --help                        Show this help
...
Run 'xlang <subcommand> --help' for subcommand-specific details.
...
| 新增 `Run 'xlang <subcommand> --help'` 提示 | 引导用户查看子命令专属 flag |
| 删除 `xlang [options] file.x` 行 | 配合 §6 Phase 2 删除隐式入口 |
...
- Phase 2 删除隐式 fallback 后，Usage 第一行 `xlang [options] file.x` 要删
...
`xlang <subcommand> --help` 需要 dispatch 层支持：在 `main_cmd_build` / `main_cmd_run` / `driver_cmd_check` 等函数入口检测 `--help` flag，输出子命令专属 help 后退出。
...
XLANG 选 `--help, -h` + `--version, -v`（对齐 GNU gcc，简写小写 v）。
...
```
xlang 0.1.0-alpha (self-hosting, build <git-short-hash>)
```

**版本号来源**（推荐方案）：

- 编译时从 `git describe --tags --always` 自动生成，注入 `XLANG_VERSION` 宏
- Makefile 构建时通过 `-DXLANG_VERSION="..."` 传入
- main.x 读取宏并输出

```x
/* Build-time injected version string from git describe. */
/* PLATFORM: SHARED — same on all hosts. */
#ifndef XLANG_VERSION
#define XLANG_VERSION "0.1.0-dev"
#endif
const XLANG_VERSION_CSTR: *u8 = XLANG_VERSION;
```
...
```
Global actions (no subcommand):
  xlang --help, -h                Show this help
  xlang --version, -v             Show version and exit
  xlang --print-target-cpu        Print host CPU features and exit
  xlang --explain <CODE>          Print diagnostic code explanation and exit
```
...
`-v` 在很多工具里是 `--verbose`。XLANG 当前未用 `-v`，可给 `--version`。若未来需要 verbose 模式：

- 选项 A：`--verbose`（无简写，保留 `-v` 给 version）
- 选项 B：`-V`（大写，对齐 Rust clap，`-v` 保留给 version）

**推荐选项 A**——XLANG 是编译器，verbose 模式可用环境变量 `XLANG_VERBOSE=1` 替代，无需占用 `-v` flag。
...
2. **Makefile 注入版本宏**：构建 xlang 时 `XLANG_VERSION="$(shell git describe --tags --always --dirty)"` 传入 `-DXLANG_VERSION`
3. **--help 文本规范**：所有「全写+简写」组合统一为 `--long, -short` 顺序
4. **seed 镜像同步**：`compiler/seeds/main.from_x.c` 同 commit 加 `--version` 分支（§4 禁止双权威）
5. **bootstrap_xlangc seed 兼容性**：6月30日 pin 的 seed 不支持 `--version`，但 `--version` 是新增分支不影响旧调用（向后兼容）

### 12.8 验证

- [ ] `xlang --version` 和 `xlang -v` 输出版本号，exit 0
- [ ] `xlang --help` 和 `xlang -h` 输出 help，exit 0
- [ ] `--help` 文本中所有 flag 组合为 `--long, -short` 顺序
- [ ] `XLANG_VERSION` 宏在 Makefile 构建时正确注入
- [ ] seed 镜像同步（同 commit）
...
[U][B]Subcommands:[0]
  xlang [C][B]build[0] [Y][B][options] file.x [-o exe][0]   Compile .x to binary/object
  xlang [C][B]run[0]   [Y][B][options] file.x[0]            Compile and run .x
  xlang [C][B]check[0] [Y][B][paths...][0]                  Parse + typeck only
  xlang [C][B]fmt[0]   [Y][B][--check] [paths...][0]        Format .x sources
  xlang [C][B]explain[0] [Y][B]<CODE>[0]                    Explain a diagnostic code
  xlang [C][B]test[0]  [Y][B][script.sh][0]                 Run test script

[U][B]Global actions (no subcommand):[0]
  xlang [Y]--help, -h[0]                Show this help
  xlang [Y]--version, -v[0]             Show version and exit
  xlang [Y]--print-target-cpu[0]        Print host CPU features and exit
  xlang [Y]--explain[0] [Y][B]<CODE>[0]          Print diagnostic code explanation and exit
...
[U][B]Environment:[0]
  XLANG_ABI_F32_XMM=0   same as -legacy-f32-abi [G](x86_64 SysV)[0]
  XLANG_OPT             default -O level if omitted

Run 'xlang <subcommand> --help' for subcommand-specific details.

Release default: xlang_asm -backend asm -O2 [G](f32 xmm ABI on unless legacy)[0].
...
function cli_should_use_color(): i32 {
  ...
  /* 3. XLANG_FORCE_COLOR=1 explicitly enables (CI / log capture). */
  if (env_get_eq("XLANG_FORCE_COLOR", "1")) return 1;
  /* 4. TTY check. */
...
| **XLANG 方案** | **加粗+下划线** | **青色加粗** | **黄色** | **默认** | **灰色** |

XLANG 方案与 `cargo`/`bat` 接近，符合 Rust 现代工具链惯例（XLANG 对标系统语言）。

### 13.6 风险与权衡

| 风险 | 说明 | 缓解 |
|------|------|------|
| Windows 兼容性 | 旧 Windows 不支持 ANSI VT | XLANG 已要求 Win11，假定 VT 可用；检测失败时 fallback 纯文本 |
| seed 镜像同步 | main.x 加颜色逻辑 → seed 同步（§4 禁止双权威） | 同 commit 改 seed main.from_x.c |
| 测试基线 | bstrict 脚本可能 grep help 输出做断言，颜色码干扰 | 脚本用 `NO_COLOR=1 xlang --help` 强制纯文本 |
...
- [ ] `xlang --help` 终端输出彩色，管道 `xlang --help | cat` 输出纯文本
- [ ] `NO_COLOR=1 xlang --help` 输出纯文本
- [ ] `XLANG_FORCE_COLOR=1 xlang --help | cat` 输出彩色（强制）
