...
示例（`region_assign_escape.x` 第 12 行，docblock 之后）：
...
| case_id | 文件 | 子串 | 期望行 |
|---------|------|------|--------|
| `diag_escape_assign` | `region_assign_escape.x` | `slice region escape` | **12** |
| `diag_mismatch` | `region_mismatch.x` | `slice region mismatch` | **10** |
| `diag_return_escape` | `region_return_escape.x` | `slice region escape` | **11** |
| `diag_call_escape` | `region_call_escape.x` | `slice region escape` | **16** |
