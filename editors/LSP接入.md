...
> 权威来源：[compiler/src/lsp/lsp.x](file:///home/shuliangfu/worker/xlang/x-lang/compiler/src/lsp/lsp.x)、[lsp_io.x](file:///home/shuliangfu/worker/xlang/x-lang/compiler/src/lsp/lsp_io.x)、[lsp_diag.x](file:///home/shuliangfu/worker/xlang/x-lang/compiler/src/lsp/lsp_diag.x)、[editors/vscode/src/lspClient.ts](file:///home/shuliangfu/worker/xlang/x-lang/editors/vscode/src/lspClient.ts)。如本文与代码冲突，以代码为准。
...
- **单条 body 上限**：1 MiB（[lsp.x](file:///home/shuliangfu/worker/xlang/x-lang/compiler/src/lsp/lsp.x) 中 `LSP_BODY_CAP = 1048576`）。超长消息会被丢弃，循环读下一条。
...
下表是 [lsp.x#lsp_main_impl](file:///home/shuliangfu/worker/xlang/x-lang/compiler/src/lsp/lsp.x) 实际 dispatch 的全部 method。未列出者（如 `codeAction`、`signatureHelp`、`inlayHint`）**未实现**，客户端不要声明依赖。
...
| `textDocument/didOpen` | notification | 从 body 提取 `"text"` 并存为当前文档（[lsp_io.x#extract_document_text](file:///home/shuliangfu/worker/xlang/x-lang/compiler/src/lsp/lsp_io.x)） |
...
- `--lsp`（必需）：进入 LSP 主循环。`xlang-c` 不识别此 flag，[lspClient.ts#validateXlangLanguageServer](file:///home/shuliangfu/worker/xlang/x-lang/editors/vscode/src/lspClient.ts) 在启动前用 2 秒探针检测，发现 `unknown option --lsp` 即拒绝启动。
...
VSCode 扩展 [editors/vscode](file:///home/shuliangfu/worker/xlang/x-lang/editors/vscode) 已内置 LSP 客户端，无需额外配置即可使用。
...
[xlangPath.ts#resolveServerCommand](file:///home/shuliangfu/worker/xlang/x-lang/editors/vscode/src/xlangPath.ts) 的优先级：
...
[extension.ts#loadHeavyFeatures](file:///home/shuliangfu/worker/xlang/x-lang/editors/vscode/src/extension.ts) 按 `xlang.features.*` 注册 Provider。LSP 自身的能力（诊断/定义/引用/悬停/补全/大纲/语义高亮/重命名/格式化）直接走 `LanguageClient`，无需扩展侧 Provider；以下为 **扩展侧本地实现** 的轻量增强，与 LSP 并行：
...
[lspClient.ts#startXlangLanguageClient](file:///home/shuliangfu/worker/xlang/x-lang/editors/vscode/src/lspClient.ts) 监听 `onDidChangeState`：`Running → Stopped` 且非用户主动 stop 时，自动 `client.start()`。`xlang.server.restartOnCrash: false` 关闭。
...
- `filetypes = { 'x' }`：需先在 `runtimepath` 中加载 XLANG 的 `ftdetect/xlang.vim`（[editors/vim/ftdetect/xlang.vim](file:///home/shuliangfu/worker/xlang/x-lang/editors/vim/ftdetect/xlang.vim)），否则 Neovim 不识别 `.x`。
...
注意：vim-lsp 同样以 Push 诊断为主，Pull 支持请查阅其最新文档。本仓库的 [vim/autoload/xlang.vim](file:///home/shuliangfu/worker/xlang/x-lang/editors/vim/autoload/xlang.vim) 已在 `omnifunc` 中预留 "LSP 接管后自动降级" 的注释，但实际并未集成任何 LSP 客户端——Vim 用户需自行选用上述之一。
...
- VSCode：`vscode-languageclient` ≥ 8.1，已在 [package.json#dependencies](file:///home/shuliangfu/worker/xlang/x-lang/editors/vscode/package.json) 中固定。
- Neovim：≥ 0.10。
...
- `definition` / `references` 仅在 **当前文档** 内扫描（[lsp_diag.x#lsp_diag_definition_at](file:///home/shuliangfu/worker/xlang/x-lang/compiler/src/lsp/lsp_diag.x)），不支持跨文件索引。
...
- VSCode 扩展在 [extension.ts#buildServerEnv](file:///home/shuliangfu/worker/xlang/x-lang/editors/vscode/src/extension.ts) 自动从 `xlang.compiler.libRoots` 拼接注入；其他客户端需手动 join。
...
历史 bug：栈上缓冲 + 大 JSON 输出在 ARM64 上溢出。已在 [lsp.x#lsp_main_impl](file:///home/shuliangfu/worker/xlang/x-lang/compiler/src/lsp/lsp.x) 中将 `initialize` 响应改用堆 `diag_ptr`，`formatting` / `completion` / `documentSymbol` / `semanticTokens` / `rename` 共用 `format_ptr`（堆 256 KiB）。若再次出现响应截断，检查这两个缓冲是否仍为堆分配。
