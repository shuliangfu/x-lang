" autoload/xlang.vim — XLANG 插件核心函数库
"
...
" O(n) 扫描当前文件，缓存到 b:xlang_symbols 直到 buffer 修改。
...
" xlang#Complete — omnifunc 入口
" 【Why】Vim 补全过程：findstart 阶段返回补全起点 col，base 阶段返回候选列表
function! xlang#Complete(findstart, base) abort
  if a:findstart
    " 向左查找非单词字符，确定补全起点
    let line = getline('.')
    let start = col('.') - 1
...
" xlang#ScanSymbols — 扫描当前 buffer 的符号定义
" 【Why】提供函数名/类型名/变量名补全，无需编译器
" 【Asm/Perf】O(n) 扫描，结果缓存到 b:xlang_symbols，b:changedtick 变化时重扫
function! xlang#ScanSymbols() abort
  if exists('b:xlang_symbols') && exists('b:xlang_symbols_tick') && b:xlang_symbols_tick == b:changedtick
    return b:xlang_symbols
  endif
  ...
  let b:xlang_symbols = symbols
  let b:xlang_symbols_tick = b:changedtick
  return symbols
endfunction

" ==================== 文本对象 ====================

" xlang#TextObject — function/block 文本对象
" 【Why】af（含 function 声明行）、if（仅 body）、ab（含括号）、ib（仅括号内）
" 用法：nnoremap <silent> af :<C-U>call xlang#TextObject('af')<CR>
function! xlang#TextObject(type) abort
  if a:type ==# 'af' || a:type ==# 'if'
    " 向上查找 function 声明行
    let start = search('^\s*\(export\s\+\)\?\(extern\s\+\)\?function\s\+\w\+', 'bnW')
    if start == 0
      " 没有 function，退化为 block 文本对象
      call xlang#TextObject(a:type ==# 'af' ? 'ab' : 'ib')
      return
    endif
...
" xlang#CurrentFunction — 状态栏显示当前所在函数名
" 【Why】配合 lightline/airline，状态栏显示 fn:main 等上下文信息
function! xlang#CurrentFunction() abort
  let pos = getpos('.')
  " 向上查找最近的 function 声明
...
" xlang#CurrentContext — 状态栏显示函数 + struct 上下文
function! xlang#CurrentContext() abort
  let fn = xlang#CurrentFunction()
  if !empty(fn)
    return fn
  endif
...
" xlang#FoldText — 自定义折叠显示文本
" 【Why】默认折叠显示第一行，这里显示声明 + 行数，更清晰
function! xlang#FoldText() abort
...
" xlang#ToggleComment — 切换当前行/选区的 // 注释
" 【Why】不依赖 vim-commentary，提供内置注释切换
" 用法：nnoremap <buffer> gcc :call xlang#ToggleComment()<CR>
"       vnoremap <buffer> gc :call xlang#ToggleComment('v')<CR>
function! xlang#ToggleComment(...) abort
  if a:0 > 0 && a:1 ==# 'v'
    let start = line("'<")
    let end = line("'>")
  else
...
" xlang#JumpFunction — 跳转到下一个/上一个 function 声明
" 【Why】]f/[f 快速在函数间跳转
" 用法：nnoremap <buffer> ]f :call xlang#JumpFunction('next')<CR>
"       nnoremap <buffer> [f :call xlang#JumpFunction('prev')<CR>
function! xlang#JumpFunction(direction) abort
  if a:direction ==# 'next'
...
" xlang#JumpStruct — 跳转到下一个/上一个 struct 声明
function! xlang#JumpStruct(direction) abort
  if a:direction ==# 'next'
