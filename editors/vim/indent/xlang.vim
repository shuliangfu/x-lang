" indent/xlang.vim — XLANG 缩进规则（indentexpr）
"
" 【Why 逻辑根源】基于 cindent 扩展，处理 XLANG 特有的 struct 字面量、
" match arm =>、block 尾表达式等结构。cindent 对 C 风格 block 缩进良好，
" 但不理解 XLANG 的 match/struct literal 语义，需要 indentexpr 覆盖。
"
...
setlocal indentexpr=XlangIndent()
