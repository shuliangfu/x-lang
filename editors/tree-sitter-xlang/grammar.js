...
module.exports = grammar({
  ...
  rules: {
    ...
    // struct 体：字段分隔符可选（逗号/分号/换行均可），XLANG 真实代码存在无分隔符仅换行的情况
    struct_body: $ => seq(
      '{',
    ...
    // 数组类型：XLANG 同时支持 T[N]（C 风格）和 [N]T（Go 风格）
    array_type: $ => choice(
    ...
    // let 后 ; 可选：XLANG parser 在 let 初值后若遇 return/if/while/for/} 即视为语句结束
    // （parser.x parser_rewind_lex_for_following_stmt）；struct_literal 末尾 } 后常省略 ;
    // = expr 可选：XLANG 支持 `let x: Type;`（仅声明未初始化）
    let_declaration: $ => seq(
      'let',
      field('name', $.identifier),
      optional(seq(':', $._type)),
    ...
    // 表达式语句 ; 可选：block 尾表达式无分号（XLANG 尾表达式语义）
    expression_statement: $ => seq($._expression, optional(';')),

    // return 的 ; 可选：XLANG 允许 block 内 return 后直接 }（如 `if c { return E }`）
    return_statement: $ => seq('return', optional($._expression), optional(';')),

    // if 既是语句也是表达式（XLANG 支持 `return if cond { 0 } else { 1 };`）
    if_expression: $ => seq(
      'if',
