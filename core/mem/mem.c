 ...
 * 【所属模块】core.mem；与 core/mem/mod.sx 同属一模块。依赖 C11 stdatomic.h（或 GCC __atomic_*）。
...
#define SHUX_MEM_COMPILER_FENCE() __asm__ __volatile__("" ::: "memory")
#elif defined(_MSC_VER)
#include <intrin.h>
#define SHUX_MEM_COMPILER_FENCE() _ReadWriteBarrier()
#else
#define SHUX_MEM_COMPILER_FENCE() ((void)0)
#endif

#if defined(__STDC_NO_ATOMICS__) || (defined(__GNUC__) && !defined(__clang__) && __GNUC__ < 4)
/* 极旧工具链回退：仅 compiler fence，线程栅栏为空操作 */
#define SHUX_MEM_THREAD_FENCE_ACQ() SHUX_MEM_COMPILER_FENCE()
#define SHUX_MEM_THREAD_FENCE_REL() SHUX_MEM_COMPILER_FENCE()
#define SHUX_MEM_THREAD_FENCE_SEQ() SHUX_MEM_COMPILER_FENCE()
#else
#include <stdatomic.h>
#define SHUX_MEM_THREAD_FENCE_ACQ() atomic_thread_fence(memory_order_acquire)
#define SHUX_MEM_THREAD_FENCE_REL() atomic_thread_fence(memory_order_release)
#define SHUX_MEM_THREAD_FENCE_SEQ() atomic_thread_fence(memory_order_seq_cst)
#endif
