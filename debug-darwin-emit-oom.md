...
- **Status**: [OPEN]
- **Issue**: macOS 上运行 `asm_seed_full.sx -E` 时资源异常膨胀，导致系统卡死/重启
- **Debug Server**: pending
- **Log File**: .dbg/trae-debug-log-darwin-emit-oom.ndjson

## Reproduction Steps
1. 在 macOS 上触发 `asm_seed_full.sx -E` 路径
...
|----|------------|------------|--------|----------|
| A | `asm_seed_full.sx -E` 错误走 transitive closure，导致依赖图重复展开 | High | Med | Pending |
...
- `asm_seed_full.sx -E 失败，沿用已有 build_asm/seed_host/asm_backend_partial.o`
- `shux-sx` 旧行为日志仍出现 `collect parse dep=...` / `dep prerun begin ...`，说明它会展开 transitive closure
- 以新源码重建的 `shux-c` 运行同一路径时不再出现 closure 展开日志，但当前直接解析 `asm_seed_full.sx` 会在 `#[cfg]` 处失败

## Hypotheses & Verification
| ID | Hypothesis | Likelihood | Effort | Evidence |
|----|------------|------------|--------|----------|
| A | `asm_seed_full.sx -E` 错误走 transitive closure，导致依赖图重复展开 | High | Med | Confirmed on old `shux-sx` |
...
- 已确认这不是机器性能问题，而是 `asm_seed_full.sx -E` 路径的资源放大 bug。
- 已加入两类修复：
  1. 源码侧：`asm_seed_full.sx` 纳入 direct-import-only；large-stack 默认值修正为 256MiB。
  2. 脚本侧：`build_seed_asm_host.sh` 内建 RSS watchdog，Darwin 默认可限制高风险 `-E` 进程的 RSS，并在超限后自动 kill + fallback。
- 当前仍有一个独立问题：`shux-sx` 重新构建受 `_pipeline_typeck_after_parse_ok` 缺口阻塞，导致新源码尚未完全体现在 `shux-sx` 二进制上。
...
- 二进制反汇编已确认 `PARSER_ASM_STRETCH_AUDIT_CALL(...)` 在当前 `shux-sx` 中未实际执行，`SHUX_PARSER_STRETCH_AUDIT` 不是运行时 runaway 根因。
...
- 当前验证状态：
  ...
  - 已继续修复 `shux-sx` 在 Darwin 上错误复用未过滤 `pipeline_sx.o` / `USER_ASM_LINK` 的链接输入，现已改为复用现成 filtered 对象；
  - `make shux-sx` 已成功，带 parser 修复的新二进制已重链完成。

## Latest Runtime Evidence
- 新 `shux-sx` 在 watchdog 下复跑：
  - `python3 ../.dbg/watch_shuxsx_emit.py ./shux-sx ... -E src/asm/asm_seed_full.sx`
