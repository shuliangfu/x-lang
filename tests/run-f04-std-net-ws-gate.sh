...
# run-std-net-ws-gate; refuse leftover ignore of explicit-bad). leftover
# nested product path (inventory / observational STD-031 ws) stay.
...
# nested leftover inventory / leftover observational STD-031 (refuse
...
# (do not rewrite leftover inventory / observational run-std-net-ws-gate).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
