#!/usr/bin/env bash
# F-04 v4：std.compress zlib 去 C 门禁（libz.sx + 无 zlib.c）。
#
...
ZLIB_LIBZ="std/compress/zlib/libz.sx"
...
[ -f "$ZLIB_LIBZ" ] || die "missing libz.sx"
...
grep -q 'import("std.compress.zlib.libz")' "$ZLIB_MOD" || die "mod.sx missing libz import"
