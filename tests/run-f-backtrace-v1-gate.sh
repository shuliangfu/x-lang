#!/usr/bin/env bash
# F-backtrace v1：std.backtrace 去 C（backtrace.c → backtrace.sx；胶层 v2 已拆，见 run-f-backtrace-v2-gate.sh）。
set -e
cd "$(dirname "$0")/.."
...
echo "=== F-backtrace v1: backtrace.c → backtrace.sx (glue superseded by v2) ==="
...
grep -q 'backtrace.sx' compiler/Makefile || die "Makefile missing backtrace.sx"
