#!/usr/bin/env bash
# CI 全平台统一测试入口：平台支持的能力必须实跑，禁止 silent SKIP（XLANG_CI_NO_SKIP=1）。
# 用法（仓库根）：CI=1 ./tests/run-ci-full-suite.sh
# 环境：XLANG_CI_SKIP_TIER_A=1 或 XLANG_CI_SKIP_HEAVY=1 — 跳过 Tier A（build_xlang_asm/bstrict），仍跑 Tier P/B。
...
export XLANG_CI_NO_SKIP="${XLANG_CI_NO_SKIP:-1}"
...
# Linux ARM64 CI 精简路径：全量 run-all-x + build_xlang_asm 易超 170min；Neon DOD/SIMD 烟测保留，完整回归由 x86_64 承担。
...
# 是否为 Tier A 重门禁可跳过（lite 平台或 XLANG_CI_SKIP_TIER_A / 旧名 XLANG_CI_SKIP_HEAVY）。
ci_skip_tier_a() {
  if [ -n "${XLANG_CI_SKIP_TIER_A:-}" ] || [ -n "${XLANG_CI_SKIP_HEAVY:-}" ]; then
    return 0
  fi
...
ci_install_zig_tarball() {
  local ver="${XLANG_CI_ZIG_VERSION:-0.13.0}"
...
# 要求 xlang_asm 已产出。
ci_require_xlang_asm() {
  if [ ! -x compiler/xlang_asm ]; then
    echo "ci-full-suite FAIL: compiler/xlang_asm missing (required on this platform)" >&2
    exit 1
  fi
}
...
if ci_is_linux_arm64_ci_lite; then
  ...
  echo "ci-full-suite: Docker — -o 链接强制 xlang-c（避免 seed xlang asm 在 musl 上 cc failed）"
  export XLANG_LINK_XLANG=./compiler/xlang-c
fi

ulimit -s 65532 2>/dev/null || ulimit -s 16384 2>/dev/null || ulimit -s hard 2>/dev/null || true

# ── Tier P：全平台统一便携测试（同一套 .x / xlang-c，平台能力自动 N/A） ───
...
if ci_is_linux; then
  IO_UNIFIED_ENV="XLANG_PERF_FAIL_ON_IO_REGRESSION=1"
  # L1 ≥ Zig + ZC-1 -10%：GHA 原生 ubuntu 须实锤；Docker Desktop 无 io_uring 时不硬门禁。
  if ci_is_linux_x64 && ! ci_is_docker; then
    IO_UNIFIED_ENV="${IO_UNIFIED_ENV} XLANG_PERF_FAIL_ON_IO_ZIG=1 XLANG_CI_REQUIRE_ZC1=1"
  fi
  # shellcheck disable=SC2086
...
if ci_is_linux && ! ci_is_docker; then
  ...
  XLANG_PERF_FAIL_ON_COMPILE_REGRESSION=0 ./tests/run-perf-compile-dogfood-gate.sh | tee /tmp/compile_dogfood.log
fi
...
if ci_is_windows_msys_ci_lite; then
  ...
  cp -f compiler/xlang compiler/xlang_asm
  {
    ...
    echo "refresh xlang asm gate OK (Windows lite: xlang_asm <- seed xlang)"
    echo "region typeck OK"
    echo "linear typeck OK"
  } | tee /tmp/refresh_asm_gate.log
else
  ./tests/run-refresh-xlang-asm-gate.sh | tee /tmp/refresh_asm_gate.log
fi
grep -q 'refresh xlang asm gate OK' /tmp/refresh_asm_gate.log
...
if ci_is_linux; then
  ...
  if ci_is_linux_x64 && ! ci_is_docker; then
    export XLANG_ZC5_REQUIRE_STRACE=1
    export XLANG_CI_REQUIRE_ZC1=1
    XLANG=./compiler/xlang_asm ./tests/run-zc-gates.sh --perf | tee /tmp/zc_gates.log
  else
    XLANG=./compiler/xlang_asm ./tests/run-zc-gates.sh | tee /tmp/zc_gates.log
  fi
...
if ci_is_linux; then
  ...
  XLANG_PERF_FAIL_ON_ASYNC_REGRESSION=1 ./tests/run-perf-async.sh --bench | tee /tmp/async_perf.log
  grep -q 'async perf OK' /tmp/async_perf.log
elif ci_is_darwin; then
  XLANG=./compiler/xlang-c XLANG_PERF_FAIL_ON_ASYNC_REGRESSION=1 ./tests/run-perf-async.sh --bench | tee /tmp/async_perf.log
  grep -q 'async perf OK' /tmp/async_perf.log
fi

echo "── B-BOOT coldstart ──"
chmod +x tests/run-perf-coldstart.sh
XLANG_COLDSTART_STD_ONLY=1 XLANG_PERF_FAIL_ON_COLDSTART_REGRESSION=1 \
...
if ci_is_windows_msys_ci_lite; then
  echo "ci-full-suite: bootstrap seed gate N/A on Windows MSYS2 (C test suite smoke already ran bootstrap-xlang-gate)"
else
  ./tests/run-bootstrap-xlang-gate.sh
fi
...
if ci_skip_tier_a; then
  echo "ci-full-suite: Tier A build_xlang_asm/bstrict deferred ($(ci_host_summary); ubuntu x86_64 covers)"
  ci_require_xlang_asm
else
  echo "── build_xlang_asm (Goal 2) ──"
  cd compiler && XLANG=./xlang ./scripts/build_xlang_asm.sh
  cd ..
  [ -x compiler/xlang_asm ] || { echo "ci-full-suite FAIL: xlang_asm missing after build_xlang_asm" >&2; exit 1; }

  echo "── asm .o quality ──"
  cd compiler && XLANG=./xlang ./scripts/check_asm_o_quality.sh
  cd ..

  echo "── B-SIZE xlang_asm stripped (advisory, ENG-002) ──"
  chmod +x tests/run-size-xlang-asm-gate.sh
  ./tests/run-size-xlang-asm-gate.sh | tee /tmp/size_xlang_asm.log
  grep -qE 'size gate OK|size gate WARN|size gate SKIP' /tmp/size_xlang_asm.log
fi
...
if ci_skip_tier_a; then
  echo "ci-full-suite: xlang_asm smoke N/A (Tier A skipped; stub xlang_asm lacks asm backend on $(ci_host_summary))"
elif ci_is_linux_arm64_ci_lite; then
  echo "ci-full-suite: xlang_asm smoke on Linux ARM64 uses -backend c (full asm smoke: ubuntu x86_64)"
  cd compiler && ./scripts/run_xlang_asm_smoke.sh
  cd ..
else
  cd compiler && ./scripts/run_xlang_asm_smoke.sh
  cd ..
...
if ci_is_darwin && ! ci_skip_tier_a; then
  echo "ci-full-suite: xlang_asm smoke on Darwin uses -backend c for run (asm user exe N/A on gen_driver hybrid)"
fi
...
ci_run_dod_arm64_subset() {
  ...
  XLANG=./compiler/xlang_asm ./tests/run-dod-s1-gate.sh | tee /tmp/dod_s1.log
  grep -q 'dod-s1 gate OK' /tmp/dod_s1.log
  XLANG=./compiler/xlang_asm ./tests/run-dod-s3-gate.sh | tee /tmp/dod_s3.log
  grep -q 'dod-s3 gate OK' /tmp/dod_s3.log
  XLANG=./compiler/xlang_asm ./tests/run-dod-cl-s1-gate.sh | tee /tmp/dod_cl_s1.log
  grep -q 'dod-cl-s1 gate OK' /tmp/dod_cl_s1.log
  XLANG=./compiler/xlang_asm ./tests/run-dod-cl-s2-gate.sh | tee /tmp/dod_cl_s2.log
  grep -q 'dod-cl-s2 gate OK' /tmp/dod_cl_s2.log
  XLANG=./compiler/xlang_asm ./tests/run-simd-s3-gate.sh | tee /tmp/simd_s3.log
...
# x86_64 / arm64：objdump/otool 校验 shuffle/select/add 硬件指令（XLANG_SIMD_HW_STRICT=1）。
ci_run_simd_hw_strict_gates() {
  chmod +x tests/run-simd-s3-gate.sh tests/run-simd-s4-gate.sh
  echo "── SIMD HW strict (XLANG_SIMD_HW_STRICT=1) ──"
  XLANG=./compiler/xlang_asm XLANG_SIMD_HW_STRICT=1 ./tests/run-simd-s3-gate.sh | tee /tmp/simd_s3_strict.log
  grep -q 'simd-s3 gate OK' /tmp/simd_s3_strict.log
  XLANG=./compiler/xlang_asm XLANG_SIMD_HW_STRICT=1 ./tests/run-simd-s4-gate.sh | tee /tmp/simd_s4_strict.log
...
ci_run_dod_x86_correctness() {
  ...
  if ci_is_windows_msys; then
    ...
    XLANG=./compiler/xlang_asm ./tests/run-f32-xmm-gates.sh | tee /tmp/f32_xmm_gates.log
...
if ci_is_linux && ci_is_x86_64_host; then
  ...
  XLANG=./compiler/xlang_asm XLANG_DOD_SOA_FAIL=1 XLANG_DOD_SOA_REQUIRE_L1="$DOD_SOA_REQUIRE_L1" ./tests/run-perf-dod-soa.sh | tee /tmp/dod_soa_perf.log
  ...
  echo "── DOD/SIMD (Linux ARM64, xlang-c -o link; refresh xlang_asm typeck) ──"
  ci_run_dod_arm64_subset
elif ci_is_darwin && ci_is_arm64_host; then
...
if ci_is_linux && ci_is_x86_64_host; then
  ...
  chmod +x tests/run-perf-coldstart.sh
  XLANG=./compiler/xlang_asm XLANG_COLDSTART_FREESTANDING_ONLY=1 \
    XLANG_PERF_FAIL_ON_COLDSTART_REGRESSION=1 ./tests/run-perf-coldstart.sh --bench | tee /tmp/coldstart_fs.log
...
chmod +x tests/run-wpo-s2.sh tests/run-perf-wpo-s2.sh tests/run-wpo-dce-asm.sh \
  ...
  tests/run-perf-wpo-dce-xlang-asm-text.sh tests/run-wpo-build-asm-chain-gate.sh \
...
XLANG=./compiler/xlang_asm XLANG_PERF_FAIL_ON_WPO_DCE_TEXT=1 ./tests/run-perf-wpo-dce-text.sh | tee /tmp/wpo_dce_text.log
grep -q 'wpo dce text OK' /tmp/wpo_dce_text.log
if ci_skip_tier_a; then
  echo "ci-full-suite: wpo compiler-self / xlang_asm text / build_asm chain / strict-link N/A (Tier A skipped)"
  echo "wpo compiler self text OK (Linux ARM64 N/A)" | tee /tmp/wpo_compiler_self_text.log
  echo "wpo xlang_asm text OK (Linux ARM64 N/A)" | tee /tmp/wpo_xlang_asm_text.log
  ...
  # refresh 路径 proxy 仅 ~0.84%；须先 ensure 五模块 WPO .o 再测 xlang_asm text（~4.65% stretch）。
  XLANG_WPO_ENSURE_FAIL=1 XLANG_WPO_ENSURE_COMPILER=./compiler/xlang_asm ./tests/ensure-wpo-build-asm-artifacts.sh | tee /tmp/wpo_ensure.log
  grep -q 'ensure-wpo-build-asm-artifacts OK' /tmp/wpo_ensure.log
  XLANG=./compiler/xlang_asm XLANG_PERF_FAIL_ON_WPO_COMPILER_SELF_TEXT=1 ./tests/run-perf-wpo-dce-compiler-self-text.sh | tee /tmp/wpo_compiler_self_text.log
  grep -q 'wpo compiler self text OK' /tmp/wpo_compiler_self_text.log
  XLANG=./compiler/xlang_asm XLANG_PERF_FAIL_ON_WPO_XLANG_ASM_TEXT=1 ./tests/run-perf-wpo-dce-xlang-asm-text.sh | tee /tmp/wpo_xlang_asm_text.log
  grep -q 'wpo xlang_asm text OK' /tmp/wpo_xlang_asm_text.log
  # stretch -3%：strict bstrict 全链（非 CI-fast）时可选
  if [ "${XLANG_WPO_STRETCH_3PCT:-0}" = "1" ]; then
    XLANG=./compiler/xlang_asm XLANG_WPO_STRETCH_3PCT=1 XLANG_PERF_FAIL_ON_WPO_XLANG_ASM_TEXT=1 \
      ./tests/run-perf-wpo-dce-xlang-asm-text.sh | tee /tmp/wpo_xlang_asm_text_3pct.log
    grep -q 'wpo xlang_asm text OK' /tmp/wpo_xlang_asm_text_3pct.log
  fi
  ...
  XLANG_WPO_STRICT_LINK_FAIL=1 ./tests/run-wpo-strict-link-gate.sh | tee /tmp/wpo_strict_link_gate.log
...
if ci_is_docker; then
  ...
  XLANG=./compiler/xlang_asm XLANG_PERF_FAIL_ON_WPO_S2_REGRESSION=1 XLANG_WPO_S2_RUNS=1 XLANG_WPO_S2_LIMIT=1000000 XLANG_WPO_S2_COMPILE_ONLY=1 ./tests/run-perf-wpo-s2.sh --bench | tee /tmp/wpo_s2_perf.log
