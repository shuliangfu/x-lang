...
fail=0
note() { echo "run-no-legacy-xlang-gate: $*"; }
...
# 2) 旧 CLI 二进制名（Shulang 时代 shu/shu-c；勿与 compiler/xlang 混淆）
...
if rg -q 'XLANGX_' --glob '!tests/run-no-legacy-xlang-gate.sh' compiler tests scripts tools editors 2>/dev/null; then
  bad "XLANGX typo remains (use XLANG)"
  rg -n 'XLANGX_' --glob '!tests/run-no-legacy-xlang-gate.sh' compiler tests scripts tools editors 2>/dev/null | head -10 | sed 's/^/  /'
fi

# 4) 硬切禁止：Shulang / shu-c / 旧路径 / boot-*-shu-（合法 xlang / xlang-c 不在此列）
scan_dirs=(compiler core std tests scripts tools editors)
if rg -q 'SHULANG_|\bshu-c\b|/compiler/shu\b|boot-[0-9]+-shu-' \
  --glob '!tests/run-no-legacy-xlang-gate.sh' --glob '!editors/vscode/package-lock.json' \
...
# 5) xlang-c / xlang 为合法名；禁止 xlangxx 级联
if rg -q 'xlang{3,}' --glob '!tests/run-no-legacy-xlang-gate.sh' compiler tests scripts 2>/dev/null; then
  bad "xlang cascade typo (xlangxx+)"
...
# 7) VS Code 扩展：禁止旧 grammar su.tmLanguage（vscode-xlang 包名为合法 Xlang 扩展）
if rg -q 'su\.tmLanguage|snippets/su\.json' \
  --glob '!tests/run-no-legacy-xlang-gate.sh' \
...
if [ "$fail" -ne 0 ]; then
  echo "run-no-legacy-xlang-gate: FAILED" >&2
  exit 1
fi
