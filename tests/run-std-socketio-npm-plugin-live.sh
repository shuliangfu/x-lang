...
PORT="${XLANG_SOCKETIO_NPM_PLUGIN_PORT:-13006}"

if [ "${XLANG_SOCKETIO_NPM_PLUGIN:-0}" != "1" ]; then
  echo "std-socketio npm plugin live SKIP (set XLANG_SOCKETIO_NPM_PLUGIN=1 to enable)"
  exit 0
fi
...
if XLANG_BIN="$(stdlib_cm_native_shu ./compiler/xlang-c && echo ./compiler/xlang-c || true)"; then
  ...
  echo "std-socketio npm plugin live SKIP (no native xlang)" >&2
  exit 0
fi
...
if ! (cd "$NPM_DIR" && npm install --no-audit --no-fund >/tmp/xlang_socketio_npm_plugin_install_$$.log 2>&1); then
  ...
  tail -15 /tmp/xlang_socketio_npm_plugin_install_$$.log >&2 || true
  exit 1
fi
...
PORT="$PORT" node "$NPM_DIR/server.mjs" >/tmp/xlang_socketio_npm_plugin_svr_$$.log 2>&1 &
SVR_PID=$!
sleep 1.5
...
exe="/tmp/xlang_std_socketio_npm_plugin_$$"
if ! "$XLANG_BIN" -L . "$LIVE_X" -o "$exe" 2>/tmp/xlang_socketio_npm_plugin_link_$$.log; then
  echo "std-socketio npm plugin live FAIL: link" >&2
  tail -8 /tmp/xlang_socketio_npm_plugin_link_$$.log >&2 || true
  exit 1
fi
...
if [ "$run_ec" -ne 0 ]; then
  ...
  tail -8 /tmp/xlang_socketio_npm_plugin_svr_$$.log >&2 || true
  exit 1
fi
