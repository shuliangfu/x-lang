...
$LINK_SHUX -L . tests/string/main.sx -o "$STRING_OUT" 2>&1
exitcode=0; "$STRING_OUT" >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 0 ] && { echo "expected exit 0 (string_empty), got $exitcode"; exit 1; }

$LINK_SHUX -L . tests/string/from_slice_eq.sx -o "${TMPDIR:-/tmp}/shux_string_fs" 2>&1
...
$LINK_SHUX -L . tests/string/compare_append_find.sx -o "${TMPDIR:-/tmp}/shux_string_caf" 2>&1
...
$LINK_SHUX -L . tests/string/contains_trim_replace.sx -o "${TMPDIR:-/tmp}/shux_string_ctr" 2>&1
...
$LINK_SHUX -L . tests/string/view_zerocopy.sx -o "${TMPDIR:-/tmp}/shux_string_view" 2>&1
