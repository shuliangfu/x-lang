#!/usr/bin/env bash
# S5：build_asm/typeck_wpo.o WPO 生产链硬门禁（WPO 压缩 typeck.su dogfood）。
...
#   SHU_WPO_TYPECK_O_FAIL=1 ./tests/run-wpo-typeck-o-gate.sh
set -e
cd "$(dirname "$0")/.."
...
BASELINE="${SHU_WPO_TYPECK_O_BASELINE:-tests/baseline/wpo-typeck-o.tsv}"
...
if [ -x "$(dirname "$0")/run-wpo-typeck-reach-gate.sh" ]; then
  SHU_WPO_TYPECK_REACH_FAIL="${SHU_WPO_TYPECK_REACH_FAIL:-0}" \
