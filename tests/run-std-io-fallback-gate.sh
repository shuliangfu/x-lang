...
for f in "$DOC" "$MANIFEST" "$LIB" "$IO_C" "$MOD_X" "$README" "$SMOKE" "$RUNNER"; do
  if [ ! -f "$f" ]; then
...
split="$(std_io_fallback_manifest_ok "$DOC" "$README" "$IO_C" "$MOD_X" "$MANIFEST" || true)"
matrix_miss="${split%% *}"
code_miss="${split#* }"
...
if XLANG_BIN="$(resolve_shu 2>/dev/null)"; then
  echo "=== STD-026: typeck (XLANG=$XLANG_BIN) ==="
  ...
  echo "std-io-fallback gate SKIP typeck (no native xlang)" >&2
fi
