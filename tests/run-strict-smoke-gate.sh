...
STRICT_FAILED="$BUILD_DIR/xlang_asm.strict_failed"
BUILD_LOG="${BUILD_LOG:-}"
...
if [ -n "$BUILD_LOG" ] && [ -f "$BUILD_LOG" ]; then
  if grep -q 'installed xlang_asm.experimental as xlang_asm (fallback OK)' "$BUILD_LOG"; then
    echo "strict-smoke gate FAIL: build log shows experimental fallback ($BUILD_LOG)" >&2
    exit 1
  fi
  if ! grep -qE 'strict xlang_asm smoke passed|XLANG_ASM_SKIP_STRICT_SMOKE' "$BUILD_LOG"; then
...
# Linux x86_64：编排入口须为 C glue（非 pipeline_wpo X 入口）；smoke 仅 compile+run（完整 gate 由 run-xlang-asm-gate 覆盖）。
case "$(uname -s)-$(uname -m 2>/dev/null)" in
  Linux-x86_64|Linux-amd64)
    if command -v nm >/dev/null 2>&1; then
      if ! nm "$XLANG_ASM" 2>/dev/null | grep -q 'run_x_pipeline_parse_entry_do_parse_c'; then
        echo "strict-smoke gate FAIL: xlang_asm missing run_x_pipeline_parse_entry_do_parse_c (C orchestration glue)" >&2
        exit 1
      fi
    fi
    ;;
...
echo "strict-smoke gate: run_xlang_asm_smoke (compile+run return-value) ..."
(cd compiler && XLANG_ASM_SMOKE_SKIP_GATE=1 ./scripts/run_xlang_asm_smoke.sh)

echo "strict-smoke gate OK (strict xlang_asm smoke, no experimental fallback)"
