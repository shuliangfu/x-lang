...
JSON_SX="tests/json/object_array_parse.sx"
ASYNC_SX="tests/async/await_scheduler_mod.sx"
CORE_MEM_SX="tests/core-mem/volatile_fence.sx"
MIN_ALWAYS=31
MIN_ON_DEMAND=2
...
for f in "$DOC" "$MANIFEST" "$LIB" "$RUNTIME" "$MAKEFILE" "$JSON_SX" "$ASYNC_SX" "$CORE_MEM_SX"; do
  if [ ! -f "$f" ]; then
...
if SHUX_BIN="$(resolve_shu 2>/dev/null)"; then
  ...
  make -C compiler -q shux-c 2>/dev/null || make -C compiler shux-c 2>/dev/null || true
  if boot_link_contract_run_smoke "$SHUX_BIN" "$JSON_SX" "/tmp/shux_boot_link_json"; then
    SMOKE_OK=1
  else
    boot_link_contract_emit_report "fail" "$ALWAYS_OK" 0 0 0
    exit 1
  fi
  if boot_link_contract_run_smoke "$SHUX_BIN" "$ASYNC_SX" "/tmp/shux_boot_link_async"; then
    ON_DEMAND_OK=1
    SMOKE_OK=2
  else
  ...
  if boot_link_contract_run_smoke "$SHUX_BIN" "$CORE_MEM_SX" "/tmp/shux_boot_link_core_mem"; then
    ON_DEMAND_OK=2
    SMOKE_OK=3
  else
