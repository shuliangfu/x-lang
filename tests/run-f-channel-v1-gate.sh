...
# std-channel-unbounded; refuse leftover ignore of explicit-bad). leftover
...
# leftover nested ensure / leftover nested std-channel-select /
# leftover nested observational std-channel-unbounded (refuse leftover
# SKIP→OK / leftover ignore of explicit-bad / leftover XLANG
# fallthrough). leftover nested product path stays when XLANG is unset
# (do not rewrite leftover xlang_compiler_make / std-channel-select /
# observational std-channel-unbounded).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
