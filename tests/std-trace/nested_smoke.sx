...
function main(): i32 {
  ...
  tr2 = trace.from_ctx(ctx);
  if (tr2.handle != tr.handle) { return 3; }

  root = trace.start(&tr, zero, &n_root[0], 4);
  if (root.id == zero) { return 4; }
  child = trace.start_child(&tr, &n_child[0], 5);
  if (child.id == zero || child.id == root.id) { return 5; }
  if (trace.end(&tr, child) != trace.err_ok()) { return 6; }
  if (trace.end(&tr, root) != trace.err_ok()) { return 7; }
  cnt = trace.count(&tr);
  if (cnt != 2) { return 8; }
  n = trace.export_text(&tr, &out[0], 256);
  if (n <= 0) { return 9; }
  if (out[0] != 116) { return 10; }
  trace.free(&tr);
  context.free(ctx);
  return 0;
}
