#!/usr/bin/env bash
# asm 7.3: arr[i] = a + b emits RHS before INDEX addr→rbx (arm64 x1 not clobbered).
#
...
#   - hard: tests/asm/assign_index_binop_fast.x product -o run exit 42
#   - hard (Darwin+otool): main has no `sub sp, sp, #0x10` spill
...
PREFIX="${XLANG_ASM_ASSIGN_INDEX_BINOP_PREFIX:-xlang: [XLANG_ASM_ASSIGN_INDEX_BINOP]}"
...
die() {
  echo "asm-assign-index-binop FAIL: $*" >&2
...
echo "=== asm-assign-index-binop gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

SRC="tests/asm/assign_index_binop_fast.x"
[ -f "$SRC" ] || die "missing $SRC"
exe="/tmp/xlang_asm_assign_index_binop_$$"
log="/tmp/xlang_asm_assign_index_binop_$$.log"
rm -f "$exe" "$log"
...
if [ "$(uname -s)" = Darwin ] && command -v otool >/dev/null 2>&1; then
  if otool -tv "$exe" 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'sub.*sp, sp, #0x10'; then
    rm -f "$exe"
    die "main still uses stack push for index assign+binop"
  fi
  RUN_OK=$((RUN_OK + 1))
else
  SKIP=$((SKIP + 1))
