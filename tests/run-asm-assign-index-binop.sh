...
$SHU tests/asm/assign_index_binop_fast.su -o /tmp/shu_asm_assign_index_binop 2>&1
exitcode=0
/tmp/shu_asm_assign_index_binop >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 42 ] && { echo "run-asm-assign-index-binop FAIL: expected exit 42, got $exitcode"; exit 1; }

if otool -tv /tmp/shu_asm_assign_index_binop 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'sub.*sp, sp, #0x10'; then
