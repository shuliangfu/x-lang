...
neg=$("$TYPECK_SHUX" check tests/typeck/linear/double_move.x 2>&1) || true
...
neg_call=$("$TYPECK_SHUX" check tests/typeck/linear/call_double.x 2>&1) || true
...
neg_addr=$("$TYPECK_SHUX" check tests/typeck/linear/addr_of.x 2>&1) || true
...
neg_ret=$("$TYPECK_SHUX" check tests/typeck/linear/return_branch.x 2>&1) || true
...
pos=$("$TYPECK_SHUX" check tests/typeck/linear/move_ok.x 2>&1) || {
