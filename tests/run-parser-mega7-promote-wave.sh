...
#
# 对 7 个 mega 函数各跑一次 XLANG_ASM_PARSER_MEGA_BISECT，记录 ec/text/delta/status。
...
if [ -x "./compiler/xlang_asm.experimental" ]; then
  COMP_IN="./xlang_asm.experimental"
elif [ ! -x "./compiler/xlang_asm" ]; then
  echo "parser-mega7-promote-wave SKIP (no compiler/xlang_asm)"
  exit 0
fi
...
compile_one() {
  local name="$1"
  local out="/tmp/xlang_mega_promote_${name}.$$.o"
  local ec text
