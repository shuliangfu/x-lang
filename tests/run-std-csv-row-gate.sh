#!/usr/bin/env bash
# STD-036：std.csv parse_row / write_row 门禁
#
...
for f in "$DOC" "$MANIFEST" "$LIB" "$CSV_SU" "$CSV_C" "$RT_SU" "$MAIN_SU"; do
  if [ ! -f "$f" ]; then
...
sym_miss="$(std_csv_row_symbols_ok "$CSV_SU" "$CSV_C" "$MANIFEST" || true)"
