...
DOC="${SHU_STD_UUID_DOC:-analysis/std-uuid-v1.md}"
MANIFEST="${SHU_STD_UUID_MANIFEST:-tests/baseline/std-uuid-manifest.tsv}"
VECTORS="${SHU_STD_UUID_VECTORS:-tests/baseline/std-uuid-vectors.tsv}"
MOD_SU="std/uuid/mod.su"
UUID_C="std/uuid/uuid.c"
LIB="tests/lib/std-uuid.sh"
SMOKE_SU="tests/std-uuid/roundtrip.su"
...
if [ "${SHU_STD_UUID_MANIFEST_ONLY:-0}" = "1" ]; then
...
if cc -std=c11 -O1 -o /tmp/shu_uuid_smoke \
  "$SMOKE_C" std/uuid/uuid.o std/random/random.o std/time/time.o 2>/dev/null; then
  if /tmp/shu_uuid_smoke >/dev/null 2>&1; then
    C_OK=1
  fi
  rm -f /tmp/shu_uuid_smoke
fi
if [ "$C_OK" -eq 0 ]; then
  std_uuid_emit_report "fail" 0 0 0
...
if [ -n "$SHU_BIN" ]; then
  echo "=== STD-075: .su smoke (SHU=$SHU_BIN) ==="
  ...
  if std_uuid_run_smoke "$SHU_BIN" "$SMOKE_SU" "roundtrip"; then
    SU_OK=1
  else
    std_uuid_emit_report "fail" "$C_OK" 0 0
    exit 1
  ...
  echo "std-uuid gate SKIP .su smoke (no shu)" >&2
  SKIP=1
fi
