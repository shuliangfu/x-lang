...
struct_link_o() {
  ...
  if [ "$ec" -ne 0 ] && [ -z "${SHUX_BOOTSTRAP_MIN:-}" ] \
    ...
    ./compiler/shux-c -L . "$x" -o "$out" 2>&1
    ec=$?
  fi
  # shux-c -E + cc 回退（无 -o 能力时）
  if [ "$ec" -ne 0 ] && [ -x ./compiler/shux-c ]; then
    ./compiler/shux-c -E -L . "$x" > /tmp/shux_struct_fallback.c 2>&1
    ${CC:-cc} -O2 -o "$out" /tmp/shux_struct_fallback.c 2>&1
    ec=$?
  fi
  return "$ec"
}

struct_link_o tests/struct/simple.x /tmp/shux_struct_simple
...
struct_link_o tests/struct/padding_allow.x /tmp/shux_struct_pad
exitcode=0; /tmp/shux_struct_pad >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 2 ] && { echo "expected 2 (padding_allow x.b), got $exitcode"; exit 1; }

if ${TYPECK_SHUX:-$SHUX} check tests/struct/padding_no_allow.x 2>&1 | grep -q "implicit padding"; then
  : # 预期 typeck 报错
else
...
struct_link_o tests/memory-contract/packed_struct.x /tmp/shux_struct_packed
...
struct_link_o tests/memory-contract/arena_align.x /tmp/shux_struct_arena_align
...
struct_link_o tests/boundary/struct_add_pair_inline.x /tmp/shux_struct_add_pair_inline
...
struct_link_o tests/boundary/struct_get_field_inline.x /tmp/shux_struct_get_field_inline
...
struct_link_o tests/boundary/struct_mk_field_inline.x /tmp/shux_struct_mk_field_inline
...
struct_link_o tests/boundary/struct_mk_pair_sum_inline.x /tmp/shux_struct_mk_pair_sum_inline
...
struct_link_o tests/boundary/struct_mk_let_inline.x /tmp/shux_struct_mk_let_inline
...
struct_link_o tests/boundary/struct_mk_while_let_inline.x /tmp/shux_struct_mk_while_let_inline
