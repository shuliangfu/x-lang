#!/usr/bin/env bash
# std.heap leftover runner: tests/heap/main.x (alloc_size_zero) product -o exit 0.
#
# Honesty: leftover soft auto-make (`xlang_compiler_make … heap.o || make`) +
...
# Product `-o` tests/heap/main.x must exit 0. Report: run=/obs=/skip=
# PLATFORM: SHARED archaeology — Ubuntu gold still required.
# Usage: ./tests/run-heap.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
PREFIX="${XLANG_HEAP_PREFIX:-xlang: [HEAP]}"
SMOKE="tests/heap/main.x"
RUN_OK=0
OBS=0
SKIP=0
...
echo "=== heap leftover (prefer asm; hard; refuse leftover auto-make / gcc fallback) ==="
if [ -n "${XLANG:-}" ]; then
...
set +e
"$XLANG_BIN" check -L . "$SMOKE" >/tmp/xlang_heap_check.log 2>&1
chk_ec=$?
set -e
if [ "$chk_ec" -ne 0 ]; then
  echo "heap OBS check (paused / CHK residual ec=$chk_ec; refuse soft SKIP→OK)" >&2
  OBS=$((OBS + 1))
fi
...
set +e
"$XLANG_BIN" -L . "$SMOKE" -o "$exe" >/tmp/xlang_heap_o.log 2>&1
o_ec=$?
set -e
if [ "$o_ec" -ne 0 ] || [ ! -x "$exe" ]; then
  tail -n 12 /tmp/xlang_heap_o.log 2>/dev/null || true
  rm -f "$exe"
...
set -e
rm -f "$exe"
[ "$run_ec" -eq 0 ] || die "runnable exit=$run_ec (expected 0; alloc_size_zero)"
RUN_OK=$((RUN_OK + 1))
