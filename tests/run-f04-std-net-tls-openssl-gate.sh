#!/usr/bin/env bash
# F-04 v8：std.net OpenSSL TLS 去 C 门禁（tls_openssl.sx + 无 tls_openssl.inc.c）。
#
...
[ -f "$TLS_SX" ] || die "missing tls_openssl.sx"
[ ! -f std/net/tls_openssl.inc.c ] || die "tls_openssl.inc.c should be deleted"
grep -q 'shu_net_tls_openssl_marker' "$TLS_SX" || die "tls_openssl.sx missing marker"
grep -q 'net_tls_connect_client_c' "$TLS_SX" || die "tls_openssl.sx missing connect"
grep -q 'net_tls_openssl_smoke_c' "$TLS_SX" || die "tls_openssl.sx missing smoke"
...
grep -q 'tls_openssl.sx' compiler/Makefile || die "Makefile missing tls_openssl.sx build"
grep -q 'invoke_cc_append_net_tls_ld.*repo_root' compiler/src/runtime_link_abi.c \
