#!/usr/bin/env bash
# 全量回归：仅用 .x 流水线编译器（shux_sx）跑所有 run-*.sh，验证 .x 前端/流水线行为。
...
# seed 由 Makefile test_sx / run-all.sh 负责；CI 内 skip 全量 bootstrap-driver（asm 编译 typeck.x 极慢 → test_sx 25min 超时）
...
echo "run-all-sx: running full test suite with SHUX=compiler/shux_sx (.x pipeline compiler)"
