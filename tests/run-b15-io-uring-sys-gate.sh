...
grep -q 'xlang_io_uring_is_available_c' std/sys/linux_io_uring.x || { echo "b15 gate FAIL: facade" >&2; exit 1; }
grep -q 'io_uring' std/io/mod.x || grep -q 'xlang_io_uring' std/io/core.x || grep -q 'xlang_io_uring' compiler/src/io/io.c || {
