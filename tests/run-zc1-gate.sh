...
#
# 环境变量（--perf 时）：
#   SHUX_PERF_FAIL_ON_NET_REGRESSION=1 — 默认随 --perf 开启
#   SHUX_PERF_FAIL_ON_ZC1=1            — provided echo 须比 batch 快 ≥10%
#   SHUX_NET_RUNS=1                    — CI=1 时 run-perf-net 默认已为 1
set -e
cd "$(dirname "$0")/.."
...
if grep -q "provided buffers smoke N/A" /tmp/zc1_smoke.log; then
  if [ -n "${SHUX_CI_REQUIRE_ZC1:-}" ] && [ "$(uname -s)" = "Linux" ]; then
    echo "ZC-1 gate FAIL: provided buffers N/A on Linux (SHUX_CI_REQUIRE_ZC1=1)" >&2
    exit 1
  fi
...
if grep -q "provided buffers smoke SKIP" /tmp/zc1_smoke.log; then
  if [ -n "${SHUX_CI_NO_SKIP:-}" ] && [ "$(uname -s)" = "Linux" ]; then
    echo "ZC-1 gate FAIL: provided buffers skipped on Linux (SHUX_CI_NO_SKIP=1)" >&2
    exit 1
  fi
...
SHUX_PERF_FAIL_ON_NET_REGRESSION=1 SHUX_PERF_FAIL_ON_ZC1=1 SHUX_NET_RUNS="${SHUX_NET_RUNS:-1}" \
