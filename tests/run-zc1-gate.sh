...
#
# 环境变量（--perf 时）：
#   XLANG_PERF_FAIL_ON_NET_REGRESSION=1 — 默认随 --perf 开启
#   XLANG_PERF_FAIL_ON_ZC1=1            — provided echo 须比 batch 快 ≥10%
...
if grep -q "provided buffers smoke N/A" /tmp/zc1_smoke.log; then
  if [ -n "${XLANG_CI_REQUIRE_ZC1:-}" ] && [ "$(uname -s)" = "Linux" ]; then
    echo "ZC-1 gate FAIL: provided buffers N/A on Linux (XLANG_CI_REQUIRE_ZC1=1)" >&2
    exit 1
  fi
...
if grep -q "provided buffers smoke SKIP" /tmp/zc1_smoke.log; then
  if [ -n "${XLANG_CI_NO_SKIP:-}" ] && [ "$(uname -s)" = "Linux" ]; then
    echo "ZC-1 gate FAIL: provided buffers skipped on Linux (XLANG_CI_NO_SKIP=1)" >&2
    exit 1
  fi
...
XLANG_PERF_FAIL_ON_NET_REGRESSION=1 XLANG_PERF_FAIL_ON_ZC1=1 XLANG_NET_RUNS="${XLANG_NET_RUNS:-1}" \
