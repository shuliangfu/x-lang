...
# std-random-rng; refuse leftover ignore of explicit-bad). leftover
...
# leftover nested ensure / leftover nested std-random-rng (refuse
...
# (do not rewrite leftover xlang_compiler_make / std-random-rng).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
