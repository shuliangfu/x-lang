#!/usr/bin/env bash
# F-04 v10：std.net DNS/ALPN 去 C 门禁（net_dns.sx + net_alpn.sx，net.c 无 resolve/alpn）。
#
...
ALPN_SX="std/net/net_alpn.sx"
DNS_SX="std/net/net_dns.sx"
NET_C="std/net/net.c"
...
[ -f "$ALPN_SX" ] || die "missing net_alpn.sx"
[ -f "$DNS_SX" ] || die "missing net_dns.sx"
grep -q 'net_tls_alpn_h2_http1_wire_c' "$ALPN_SX" || die "net_alpn.sx missing alpn wire"
grep -q 'net_resolve_ipv4_ex_c' "$DNS_SX" || die "net_dns.sx missing resolve_ex"
grep -q 'net_resolve_ipv6_ex_c' "$DNS_SX" || die "net_dns.sx missing resolve_ipv6"
...
grep -q 'net_alpn.sx' compiler/Makefile || die "Makefile missing net_alpn.sx build"
grep -q 'net_dns.sx' compiler/Makefile || die "Makefile missing net_dns.sx build"
