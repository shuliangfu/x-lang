...
function main(): i32 {
  ...
  if (set.contains_key(su, 2000 as u64) != 0) { return 9; }
  if (set.remove(&su, 9999 as u64) != 0) { return 10; }
  set.deinit(&su);

  // —— Set_str ——
  let ss: Set_str = set.str_new();
  if (set.str_with_capacity(&ss, 4) != 0) { return 11; }
  ...
  if (set.str_insert(&ss, long_key, 33) != -1) { return 16; }
  if (set.str_remove(&ss, k2, 3) != 1) { return 17; }
  if (set.str_contains(ss, k2, 3) != 0) { return 18; }
  if (set.str_len(ss) != 1) { return 19; }
  set.str_deinit(&ss);
  return 0;
}
