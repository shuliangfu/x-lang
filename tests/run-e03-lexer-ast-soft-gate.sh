...
# 环境：
#   XLANG_E03_LEXER_AST_FAIL=1           — 失败时硬退出
#   XLANG_E03_LEXER_AST_MANIFEST_ONLY=1  — 仅 manifest
set -e
cd "$(dirname "$0")/.."

FAIL=${XLANG_E03_LEXER_AST_FAIL:-0}
...
grep -q 'XLANG_LEGACY_SEED_LEXER_AST' "$MF" || die "Makefile missing XLANG_LEGACY_SEED_LEXER_AST"
...
if ! awk '/^ifeq \(\$\(XLANG_LEGACY_C_FRONTEND\),1\)/,/^endif/' "$MF" | grep -q '^LEXER_LINK_O = src/lexer/lexer.o'; then
...
if grep -E 'bootstrap-driver-seed:|^xlang-x:' "$MF" | grep -qE 'src/lexer/lexer\.o|src/ast/ast_seed\.o'; then
  die "Makefile bootstrap link still hardcodes lexer.o or ast_seed.o"
fi

echo "e03 track: OBJS_CORE still lists lexer.o / ast.o (xlang-c cold start; defer E-03 v3)"
echo "e03 track: build_xlang_asm SEED still cc -c lexer/ast (strict relink archaeology; defer)"
...
echo "e03 lexer-ast soft-retire gate OK (default LEXER/AST_LINK_O=empty; LEGACY=XLANG_LEGACY_SEED_LEXER_AST=1)"
