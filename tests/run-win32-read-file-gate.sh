...
# 环境：XLANG_WIN32_READ_FILE_FAIL=1 失败时硬退出
set -e
cd "$(dirname "$0")/.."

FAIL=${XLANG_WIN32_READ_FILE_FAIL:-0}
...
if [ ! -x "$XLANG" ]; then
  echo "win32-read-file-gate: SKIP (no xlang/xlang-c)"
  exit 0
fi
...
if ! "$XLANG" build -L . -o "$OUT" "$X" 2>/tmp/xlang_win32_read_file.log; then
