...
DOC="${SHU_STD068_DOC:-analysis/std-sqlite-row-col-text-v1.md}"
MANIFEST="${SHU_STD068_TSV:-tests/baseline/std-sqlite-row-col-text.tsv}"
VECTORS="${SHU_STD068_VECTORS:-tests/baseline/std-sqlite-row-col-text-vectors.tsv}"
MOD_SU="std/sqlite/mod.su"
DB_C="std/sqlite/sqlite.c"
LIB="tests/lib/std-sqlite-row-col-text.sh"
SMOKE_SU="tests/std-sqlite/row_col_text_roundtrip.su"
...
SHU_STD067_MANIFEST_ONLY=1 ./tests/run-std-sqlite-next-row-gate.sh

if [ "${SHU_STD068_MANIFEST_ONLY:-0}" = "1" ]; then
...
if std_sqlite_probe_libs; then
  ...
  if [ -n "$SHU_BIN" ]; then
    echo "=== STD-068: .su row_col_text smoke (SHU=$SHU_BIN) ==="
    if ! "$SHU_BIN" check -L . "$SMOKE_SU" >/dev/null 2>&1; then
      echo "std-sqlite-row-col-text gate SKIP .su smoke (typeck fail)" >&2
      SKIP=1
    elif std_sqlite_run_smoke "$SHU_BIN" "$SMOKE_SU" "text"; then
      TEXT_SU=1
      SKIP=0
    else
      echo "std-sqlite-row-col-text gate SKIP .su smoke (link/compile)" >&2
      SKIP=1
    fi
  else
    echo "std-sqlite-row-col-text gate SKIP .su smoke (no native shu)" >&2
    SKIP=1
  fi
  std_sqlite_restore_default_o
else
