...
# 环境：
#   XLANG_NOLIBC_FAIL=1                  — 失败时硬退出
#   XLANG_NOLIBC_MANIFEST_ONLY=1         — 仅 NL-01 manifest（同 N01_MANIFEST_ONLY）
#   XLANG_NOLIBC_N01_MANIFEST_ONLY=1     — 仅 NL-01
#   XLANG=./compiler/xlang                — freestanding 编译器
set -e
cd "$(dirname "$0")/.."

FAIL=${XLANG_NOLIBC_FAIL:-0}
...
XLANG_NOLIBC_N01_FAIL="$FAIL" ./tests/run-nolibc-n01-preparation-gate.sh || die "NL-01 preparation failed"
...
run_sub tests/run-nolibc-n06-std-track-gate.sh XLANG_NOLIBC_N06_FAIL
...
run_sub tests/run-nolibc-n07-bootstrap-prep-gate.sh XLANG_NOLIBC_N07_FAIL
...
run_sub tests/run-nolibc-n07-v2-prep-gate.sh XLANG_NOLIBC_N07_V2_FAIL
...
run_sub tests/run-nolibc-n07-v3-link-gate.sh XLANG_NOLIBC_N07_V3_FAIL
...
run_sub tests/run-nolibc-n07-v4-build-gate.sh XLANG_NOLIBC_N07_V4_FAIL
...
run_sub tests/run-nolibc-n07-v5-gate.sh XLANG_NOLIBC_N07_V5_FAIL
...
export XLANG="${XLANG:-./compiler/xlang}"
...
if [ -x tests/run-std-sys-gate.sh ]; then
  ...
  run_sub tests/run-std-sys-gate.sh XLANG_STD_SYS_FAIL
fi

echo "=== NL-02: delegate run-no-libc-socket-gate ==="
run_sub tests/run-no-libc-socket-gate.sh XLANG_NOLIBC_SOCKET_FAIL

echo "=== NL-03: delegate run-no-libc-heap-gate ==="
run_sub tests/run-no-libc-heap-gate.sh XLANG_NOLIBC_HEAP_FAIL

echo "=== NL-04: delegate run-no-libc-fs-gate ==="
run_sub tests/run-no-libc-fs-gate.sh XLANG_NOLIBC_FS_FAIL

echo "=== NL-05: delegate run-no-libc-link-gate (runtime audit; smokes skipped) ==="
XLANG_NOLIBC_LINK_SKIP_SMOKE=1 run_sub tests/run-no-libc-link-gate.sh XLANG_NOLIBC_LINK_FAIL
