...
# std-security; refuse leftover ignore of explicit-bad). leftover nested
# product path stay.
...
# leftover nested ensure / leftover nested std-security (refuse leftover
...
# rewrite leftover xlang_compiler_make / std-security).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
