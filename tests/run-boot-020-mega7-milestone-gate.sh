...
if [ "$IS_DARWIN" -eq 1 ]; then
  ...
  if XLANG_PARSER_THIN_GLUE_SYMBOL_INTEGRITY_FAIL=1 "$HOOK_SYMBOL" >/tmp/boot020_symbol.log 2>&1; then
...
if [ "$IS_DARWIN" -eq 0 ] && [ -f "$HOOK_SECOND" ]; then
  ...
  if XLANG_PARSER_SECOND_PASS_FAIL=1 "$HOOK_SECOND" >/tmp/boot020_second.log 2>&1; then
