...
#
# 用法（source 后）：
#   std_simd_ss_symbols_ok MOD_SU TSV
#   std_simd_ss_run_smoke SHU_BIN SU TAG
#   std_simd_ss_emit_report status shuffle_ok select_ok s4_ok skip

STD_SIMD_SS_PREFIX="${SHU_STD_SIMD_SHUFFLE_SELECT_PREFIX:-shu: [SHU_STD_SIMD_SHUFFLE_SELECT]}"
    ...
    case "$kind" in
      api)
        if ! grep -qE "function ${anchor}\\(" "$mod_su" 2>/dev/null; then
          echo "std-simd-shuffle-select FAIL: missing api '$anchor' in $mod_su" >&2
          miss=$((miss + 1))
        fi
        ;;
      smoke)
...
std_simd_ss_run_smoke() {
  ...
  local src="$2"
  local tag="${3:-smoke}"
  local obj="/tmp/shu_std_simd_ss_${tag}_$$.o"
  local exe="/tmp/shu_std_simd_ss_${tag}_$$"
  if [ ! -f "$src" ]; then
    echo "std-simd-shuffle-select FAIL: missing $src" >&2
    return 1
  fi
  # 先编译 .o（避免链 task.o 等待未解析 async 符号导致误报）
  if ! "$shu" -L . "$src" -o "$obj" >/dev/null 2>&1; then
    echo "std-simd-shuffle-select FAIL: compile $src" >&2
    "$shu" -L . "$src" -o "$obj" 2>&1 | tail -10 >&2 || true
    rm -f "$obj" "$exe"
    return 1
  fi
  if [ ! -f "$obj" ]; then
