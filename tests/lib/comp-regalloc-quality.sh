...
#
# 用法（source 后）：
#   comp_regalloc_quality_run_metric XLANG script_path
#   comp_regalloc_quality_emit_report status metrics_ok metrics_skip skip

COMP_REGALLOC_QUALITY_PREFIX="${XLANG_COMP013_PREFIX:-xlang: [XLANG_COMP013_REGALLOC_QUALITY]}"

# 执行单条 quality 指标脚本；成功返回 0，SKIP（无 xlang_asm）返回 2。
