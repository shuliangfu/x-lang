...
#
# 环境：
#   SHU_ZIG_BASELINE_TSV — 默认 tests/baseline/zig-perf.tsv
#   SHU_ZIG_BASELINE_STRICT=1 — zig 版本须匹配 meta zig_version_pin
#   SHU_ZIG_BASELINE_VERIFY=1 — 实测 median 须在 recorded median × tolerance 内

# 基线 TSV 路径（相对 repo 根）。
ZIG_BASELINE_TSV="${SHU_ZIG_BASELINE_TSV:-tests/baseline/zig-perf.tsv}"
...
zig_baseline_check_version() {
  ...
  if ! command -v zig >/dev/null 2>&1; then
    ...
    [ "${SHU_ZIG_BASELINE_STRICT:-0}" = "1" ] && return 1
    return 0
  fi
  ...
  echo "zig-baseline WARN: zig $cur != pin $pin (use SHU_CI_ZIG_VERSION=$pin or tarball)" >&2
  [ "${SHU_ZIG_BASELINE_STRICT:-0}" = "1" ] && return 1
  return 0
}
