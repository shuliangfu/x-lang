...
# 返回 f32/SIMD 在须走 C 后端时的参数（Darwin + Linux ARM64 + Windows MSYS2 lite shu_asm）。
...
# 返回用于 -o 可执行链接的编译器；Darwin/ARM64/Win lite 用 shu-c，避免 seed asm 与 -backend asm 不可用。
dod_host_exe_shu() {
...
# SIMD-S3 hw vec peel：refresh shu_asm 默认 asm 在 Darwin/Linux ARM64/Win 上不可用；x86_64 Linux 承担。
