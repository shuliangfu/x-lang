#!/usr/bin/env bash
# std-sqlite-row-col-blob.sh — STD-069 col_blob helpers (honesty prefer-asm).
#
# Usage (after source):
#   std_sqlite_row_col_blob_source_sqlite
#   std_sqlite_row_col_blob_symbols_ok MOD_X SQLITE_X TSV
#   std_sqlite_row_col_blob_run_c_smoke SQLITE_X   # prebuilt sqlite.o only
#   std_sqlite_row_col_blob_emit_report status run obs skip
  ...
  while IFS=$'\t' read -r item_id kind anchor mod_path _notes; do
    ...
    case "$kind" in
      ...
      symbol)
        ...
        ;;
      section)
        local doc="${XLANG_STD069_DOC:-analysis/archive/std/std-sqlite-row-col-blob-v1.md}"
        if [ ! -f "$doc" ] || ! grep -qF "$anchor" "$doc" 2>/dev/null; then
          echo "std-sqlite-row-col-blob FAIL: missing section '$anchor' in $doc" >&2
          miss=$((miss + 1))
        fi
        ;;
      ...
      script)
        if [ -n "$anchor" ] && [ -f "$anchor" ]; then
          ...
          echo "std-sqlite-row-col-blob FAIL: missing script '$anchor'" >&2
          miss=$((miss + 1))
        fi
        ;;
    esac
...
std_sqlite_row_col_blob_run_c_smoke() {
  ...
  if ! std_sqlite_probe_libs; then
    echo "std-sqlite-row-col-blob OBS c smoke (no libsqlite3; refuse soft SKIP→OK)" >&2
    return 2
  fi
  if [ ! -f "$sqlite_o" ]; then
    echo "std-sqlite-row-col-blob OBS c smoke (missing prebuilt $sqlite_o; refuse soft auto-make)" >&2
    return 2
  fi
  if ! std_sqlite_o_has_x_symbols "$sqlite_o"; then
    echo "std-sqlite-row-col-blob OBS c smoke (sqlite.o missing .x symbols; refuse soft rebuild)" >&2
    return 2
  fi
  if ! nm "$sqlite_o" 2>/dev/null | grep -q ' db_sqlite_row_col_blob_smoke_c'; then
    echo "std-sqlite-row-col-blob OBS c smoke (sqlite.o missing blob smoke symbol; refuse soft rebuild)" >&2
    return 2
  fi
  if ! cc -std=c11 -O1 -o "$out" "$src" "$sqlite_o" -lsqlite3 2>/tmp/std_sqlite_row_col_blob_c_$$.log; then
    echo "std-sqlite-row-col-blob OBS c smoke link (refuse soft ensure)" >&2
    return 1
  fi
  ...
  rm -f "$out"
  if [ "$ec" -ne 0 ]; then
    echo "std-sqlite-row-col-blob OBS c smoke run exit=$ec" >&2
    return 1
  fi
  return 0
}

# Structured report line (honesty: run=/obs=/skip=; retired blob_c=/blob_x=).
std_sqlite_row_col_blob_emit_report() {
  local status="$1"
  local run_ok="$2"
  local obs="$3"
  local skip="$4"
  echo "${STD_DB_ROW_COL_BLOB_PREFIX} status=${status} run=${run_ok} obs=${obs} skip=${skip}"
}
