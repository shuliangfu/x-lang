...
#
# 环境：
#   XLANG_PERF_BASELINE_REGISTRY — 默认 tests/baseline/perf-baseline-registry.tsv

PERF_BASELINE_REGISTRY="${XLANG_PERF_BASELINE_REGISTRY:-tests/baseline/perf-baseline-registry.tsv}"
...
# XLANG_ENG_BASELINE_BASE_REF 默认 origin/main，回退 HEAD~1 / HEAD。
perf_baseline_diff_requires_version_bump() {
  local base_ref="${XLANG_ENG_BASELINE_BASE_REF:-}"
  if [ -z "$base_ref" ]; then
