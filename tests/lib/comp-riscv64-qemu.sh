...
#   comp_riscv64_qemu_run_case XLANG_BIN SAMPLE EXPECT_EXIT
#   comp_riscv64_qemu_count MATRIX
#   comp_riscv64_qemu_emit_report status qemu_ok qemu_skip skip

COMP018_PREFIX="${XLANG_COMP018_PREFIX:-xlang: [XLANG_COMP018_RISCV_QEMU]}"
...
comp_riscv64_qemu_run_case() {
  ...
  o="$(mktemp /tmp/xlang_riscv_qemu.XXXXXX.o)"
  bin="$(mktemp /tmp/xlang_riscv_qemu.XXXXXX)"
  rm -f "$o" "$bin" 2>/dev/null || true
  if ! "$xlang" -backend asm -target "$RISCV_TARGET" -o "$o" "$x" 2>/dev/null; then
    rm -f "$o" "$bin"
    return 1
  fi
