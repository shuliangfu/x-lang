...
#
# native shu 探测、.s 文本检查、ELF .o 校验、可选 riscv ld。
set -euo pipefail
...
comp_riscv64_check_asm_text() {
  ...
  local su="$2"
  local out
  out="$(mktemp /tmp/shu_riscv_asm.XXXXXX)"
  if ! "$shu" -backend asm -target "$RISCV_TARGET" "$su" >"$out" 2>/dev/null; then
    rm -f "$out"
    return 1
  fi
...
comp_riscv64_emit_elf_o() {
  ...
  if ! "$shu" -backend asm -target "$RISCV_TARGET" -o "$out" "$su" 2>/dev/null; then
    return 1
  fi
