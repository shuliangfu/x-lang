#!/usr/bin/env bash
# p0-gate-xlang.sh — 自举前 P0 gate 编译器选择（typeck 优先 gen1 asm，seed xlang-c 作回退）
#
# xlang_asm_stage1 在 Docker 低内存下 check 偶发 SIGSEGV/OOM(137)；对 137/139 重试，
# 再回退 xlang_asm2 / xlang-c（contextual typing 已覆盖 usize 字面量）。
#
# 用法（source 后）：
...
# typeck：先 seed（xlang-c），失败再按候选列表回退。
...
# 尝试 -o 编译并运行；xlang-c 走 -backend c；失败时默认 WARN 返回 2。
# 参数：$1=编译器 $2=.x $3=exe；XLANG_P0_GATE_RUN_STRICT=1 时失败即 exit 1。
p0_gate_try_run_o() {
  local bin="$1"
  local x="$2"
  local exe="$3"
  local ec run_ec logf backend_args=""
  logf="/tmp/xlang_p0_gate_o_$$.log"
  rm -f "$exe"
  case "$(basename "$bin")" in
    xlang-c) backend_args="" ;;  # xlang-c 不支持 -backend（runtime.c:1219 BLD001），走默认 C 后端
  esac
  if [ "${XLANG_MINIMAL_CC_LINK:-0}" = "1" ]; then
    export XLANG_MINIMAL_CC_LINK=1
  fi
  local o_timeout="${XLANG_P0_GATE_O_TIMEOUT:-300}"
  local hb="${XLANG_P0_GATE_O_HEARTBEAT:-15}"
  ...
  if [ "$ec" -ne 0 ] || [ ! -x "$exe" ]; then
    ...
    min_wrap="$root/tests/lib/xlang-min-link.sh"
    if [ -x "$min_wrap" ] && [ -x "$bin" ]; then
      export XLANG_MIN_LINK_REAL="$bin"
      rm -f "$exe"
      if declare -F gate_progress_run_heartbeat >/dev/null 2>&1; then
  ...
  if [ "$ec" -ne 0 ] || [ ! -x "$exe" ]; then
    if [ "${XLANG_P0_GATE_RUN_STRICT:-0}" = "1" ]; then
      cat "$logf" >&2
      echo "p0-gate: -o FAIL ec=$ec (XLANG_P0_GATE_RUN_STRICT=1)" >&2
      rm -f "$logf"
      return 1
    fi
