...
#   std_base64_stream_run_c_smoke B64_X   # existing .o only; no soft rebuild
#   std_base64_stream_emit_report status run obs skip
...
# Host-C archaeology: stream_smoke_ok.c + existing base64.o.
...
# Hard-green signal is product -o stream.x; check/host-C = obs.
std_base64_stream_emit_report() {
  local status="$1"
  local run_ok="$2"
  local obs="$3"
  local skip="$4"
  echo "${STD_BASE64_STREAM_PREFIX} status=${status} run=${run_ok} obs=${obs} skip=${skip}"
}
