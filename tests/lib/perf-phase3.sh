...
# 从 time 输出提取 real 秒数。
perf_phase3_extract_real_sec() {
  sed -n 's/^real[[:space:]]*\([0-9]*\)m\([0-9.]*\)s.*/\1 \2/p; s/^real[[:space:]]*\([0-9.]*\)s.*/0 \1/p' \
    | awk 'NF==2 { print $1*60+$2; next } NF==1 { print $1 }'
}

# 多次运行取 real 中位数。
perf_phase3_median_real() {
  local exe="$1"
  local runs="${2:-3}"
  local i vals med
  vals=""
  for i in $(seq 1 "$runs"); do
    vals=$( ( time "$exe" >/dev/null ) 2>&1 | perf_phase3_extract_real_sec; printf '\n%s' "$vals" )
  done
...
# 输出结构化报告行。
perf_phase3_emit_report() {
  local status="$1"
  local check_ok="$2"
  local run_ok="$3"
  local skip="$4"
  echo "${PERF_PHASE3_PREFIX} status=${status} check=${check_ok} run=${run_ok} skip=${skip}"
}
