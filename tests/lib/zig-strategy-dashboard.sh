...
#   zsd_report_emit case_id xlang zig ahead target status trend
#   zsd_append_history month case_id xlang zig ahead [history_tsv]
#
# 环境：
#   XLANG_ZIG_STRATEGY_PREFIX — 默认 xlang: [XLANG_ZIG_STRATEGY]

# 计算领先 Zig 的百分点；xlang/zig 无效时输出 nan。
zsd_ahead_pct() {
  local xlang="$1"
  local zig="$2"
  awk -v s="$xlang" -v z="$zig" 'BEGIN {
...
zsd_append_history() {
  ...
  if awk -F'\t' -v m="$month" -v c="$case_id" \
    ...
    awk -F'\t' -v m="$month" -v c="$case_id" -v s="$xlang" -v z="$zig" -v a="$ahead" '
      $1 == m && $2 == c && $1 !~ /^#/ { print m "\t" c "\t" s "\t" z "\t" a; next }
      { print }
    ' OFS='\t' "$hist" > "${hist}.new"
    mv "${hist}.new" "$hist"
  else
    printf '%s\t%s\t%s\t%s\t%s\n' "$month" "$case_id" "$xlang" "$zig" "$ahead" >> "$hist"
  fi
}
...
zsd_print_dashboard_header() {
  printf '\n| case | xlang (s) | zig (s) | ahead%% | trend | status |\n'
  printf '|------|---------|---------|--------|-------|--------|\n'
}
...
zsd_print_dashboard_row() {
  ...
  local status="$6"
  printf '| %s | %s | %s | %s | %s | %s |\n' \
    "$case_id" "$xlang" "$zig" "$ahead" "$trend" "$status"
}
