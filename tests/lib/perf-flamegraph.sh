...
#
# 环境：
#   XLANG_PERF_FLAMEGRAPH_TOPN — Top 行数（默认 20）
#   XLANG_PERF_FLAMEGRAPH_FREQ  — 采样频率 Hz（默认 997）
#   XLANG_PERF_FLAMEGRAPH_PREFIX — stderr 前缀（默认 xlang: [XLANG_PERF_FLAMEGRAPH]）
...
perf_fg_probe_ok() {
  ...
  tmpdata="$(mktemp -t xlang-perf-fg-probe.XXXXXX.data)"
...
perf_fg_top_symbols() {
  ...
  freq="${XLANG_PERF_FLAMEGRAPH_FREQ:-997}"
  ...
  sysctl -w kernel.perf_event_paranoid=-1 2>/dev/null || true
  tmpdata="$(mktemp -t "xlang-perf-fg-${case_id}.XXXXXX.data")"
  tmpreport="$(mktemp -t "xlang-perf-fg-${case_id}.XXXXXX.report")"
  ...
  rows=0
  # perf report 符号行形如：  12.34%  xlang-c  [.] symbol_name
