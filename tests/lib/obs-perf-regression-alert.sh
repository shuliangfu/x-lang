...
obs_perf_alert_enabled() {
  case "${SHU_PERF_ALERT_EMIT:-1}" in
    0|false|FALSE|no|NO) return 1 ;;
    *) return 0 ;;
  esac
}

# 输出一条 SHU_PERF_ALERT 行到 stderr（OBS-003 bracket 格式）。
obs_perf_alert_emit() {
  local baseline_id="$1"
  local metric="$2"
  local measured="$3"
  ...
  printf 'shu: [SHU_PERF_ALERT] severity=%s baseline_id=%s metric=%s measured=%s cap=%s gate=%s\n' \
...
obs_perf_alert_line_valid() {
  local line="$1"
  echo "$line" | grep -qE '^shu: \[SHU_PERF_ALERT\] severity=(critical|warn) baseline_id=[a-z0-9_-]+ metric=[^ ]+ measured=[0-9.]+ cap=[0-9.]+ gate=[^ ]+'
}
