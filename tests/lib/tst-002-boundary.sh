...
#   tst002_emit_report status run obs skip
...
tst002_count_cases() {
  ...
  n="${n:-0}"
  if [ "$n" -lt "$min" ]; then
    echo "TST-002 FAIL: $x has ${n} unique nonzero returns, want >= ${min}" >&2
    return 1
  fi
  echo "$n"
  return 0
...
tst002_emit_report() {
  ...
  local obs="$3"
  local skip="$4"
  echo "${TST002_PREFIX} status=${status} run=${run_ok} obs=${obs} skip=${skip}"
}
