...
#
# 用法（source 后）：
#   tst002_count_cases SU MIN
#   tst002_verify_manifest TSV
#   tst002_run_boundary SHU_BIN SU OUT
#   tst002_emit_report status heap_ok vec_ok map_ok proc_ok skip

TST002_PREFIX="${SHU_TST002_BOUNDARY_PREFIX:-shu: [SHU_TST002_BOUNDARY]}"
...
tst002_count_cases() {
  ...
  local min="$2"
  local n
  n="$(grep -cE '// case [0-9]+' "$su" 2>/dev/null || echo 0)"
  if [ "$n" -lt "$min" ]; then
    echo "TST-002 FAIL: $su has ${n} cases, want >= ${min}" >&2
    return 1
  fi
  echo "$n"
  return 0
...
tst002_run_boundary() {
  ...
  local out="$3"
  rm -f "$out"
  if ! "$shu" -L . "$su" -o "$out" >/tmp/tst002_smoke.log 2>&1; then
  ...
  "$out" >/dev/null 2>&1 || ec=$?
  if [ "$ec" -ne 0 ]; then
    echo "TST-002 FAIL: $su exit=$ec" >&2
    return 1
  fi
  return 0
}
