...
#
# 环境：
#   XLANG_SYSCALL_BATCH_PREFIX — 默认 xlang: [XLANG_SYSCALL_BATCH]
...
perf_sb_strace_probe_ok() {
  ...
  probe_out="$(mktemp /tmp/xlang_sb_strace_probe.XXXXXX)"
...
perf_sb_strace_io_counts() {
  ...
  strace_out="$(mktemp /tmp/xlang_sb_strace.XXXXXX)"
  rc=0
...
perf_sb_report_emit() {
  ...
  local ref_total="${10:--}"
  local ok_flag="${11:-0}"
  local prefix="${XLANG_SYSCALL_BATCH_PREFIX:-xlang: [XLANG_SYSCALL_BATCH]}"
