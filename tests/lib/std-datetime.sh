    ...
    case "$kind" in
      ...
      script|gate)
        if [ -n "$anchor" ] && [ -f "$anchor" ]; then
          ...
          echo "std-datetime FAIL: missing script '$anchor'" >&2
          miss=$((miss + 1))
        fi
        ;;
      section)
...
# Host-C archaeology: prebuilt datetime.o + time.o + runtime_time_os.o only.
# Refuse soft xlang_compiler_make / soft ensure_std_c_o.
# C harness tests/std-datetime/datetime_smoke_ok.c calls datetime_smoke_c.
# Do not invent a second time ABI; do not rebuild missing objects.
...
std_datetime_host_c_obs() {
  local smoke_c="${1:-tests/std-datetime/datetime_smoke_ok.c}"
  local dt_o="std/datetime/datetime.o"
  local time_o="std/time/time.o"
  local rt_o="compiler/runtime_time_os.o"
  local o
  for o in "$dt_o" "$time_o" "$rt_o"; do
    if [ ! -f "$o" ]; then
      echo "std-datetime OBS host-C (missing prebuilt $o; refuse soft auto-make)" >&2
      return 2
    fi
  done
  if [ ! -f "$smoke_c" ]; then
    echo "std-datetime OBS host-C (missing $smoke_c)" >&2
    return 2
  fi
  local out="/tmp/xlang_datetime_smoke_$$"
  local log="/tmp/xlang_datetime_smoke_link_$$.err"
  if ! cc -std=c11 -O1 -o "$out" "$smoke_c" "$dt_o" "$time_o" "$rt_o" 2>"$log"; then
    echo "std-datetime OBS host-C link (existing .o only; refuse soft auto-make)" >&2
  ...
  rm -f "$out" "$log"
  if [ "$ec" -ne 0 ]; then
    echo "std-datetime OBS host-C run exit=$ec" >&2
    return 1
  fi
  return 0
}
...
std_datetime_run_smoke() {
  ...
  local log="/tmp/xlang_std_datetime_${tag}_build_$$.log"
...
std_datetime_emit_report() {
  ...
  local obs="$3"
  local skip="$4"
  echo "${STD_DATETIME_PREFIX} status=${status} run=${run_ok} obs=${obs} skip=${skip}"
}
