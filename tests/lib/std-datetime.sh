...
STD_DATETIME_PREFIX="${SHU_STD_DATETIME_PREFIX:-shu: [SHU_STD_DATETIME]}"
...
std_datetime_symbols_ok() {
  ...
  while IFS=$'\t' read -r item_id kind anchor mod_path _notes; do
    ...
    case "$kind" in
      ...
      symbol)
        local path="$mod_path"
        if [ "$path" = "std/datetime/datetime.c" ]; then path="$dt_c"; fi
...
std_datetime_run_smoke() {
  ...
  local src="$2"
  local tag="${3:-dt}"
  local exe="/tmp/shu_std_datetime_${tag}_$$"
...
std_datetime_emit_report() {
  ...
  local su_ok="$3"
  local skip="$4"
  echo "${STD_DATETIME_PREFIX} status=${status} c_smoke=${c_ok} su=${su_ok} skip=${skip}"
}
