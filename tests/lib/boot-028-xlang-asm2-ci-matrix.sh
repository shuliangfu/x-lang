#!/usr/bin/env bash
# boot-028-xlang-asm2-ci-matrix.sh — BOOT-028 C6 xlang_asm2 CI 矩阵辅助
#
# 用法（source 后）：
...
# shellcheck source=tests/lib/boot-027-xlang-asm2-cross.sh
_BOOT028_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
. "$_BOOT028_LIB_DIR/boot-027-xlang-asm2-cross.sh"

BOOT028_PREFIX="${XLANG_BOOT028_PREFIX:-xlang: [XLANG_BOOT028]}"
...
boot028_parse_smoke_log() {
  local log="$1"
  if grep -qE 'xlang-asm2-cross-smoke OK|xlang-asm2-cross-smoke SKIP' "$log" 2>/dev/null; then
    echo 1
  else
    echo 0
  fi
