...
#
# 环境：
#   XLANG_ALLOC_HOTSPOT_PREFIX — 默认 xlang: [XLANG_ALLOC_HOTSPOT]
...
perf_ah_strace_probe_ok() {
  ...
  probe_out="$(mktemp /tmp/xlang_ah_strace_probe.XXXXXX)"
...
perf_ah_strace_heap_counts() {
  ...
  strace_out="$(mktemp /tmp/xlang_ah_strace.XXXXXX)"
  rc=0
...
perf_ah_report_emit() {
  ...
  local cap_r="$7"
  local ok_flag="$8"
  local prefix="${XLANG_ALLOC_HOTSPOT_PREFIX:-xlang: [XLANG_ALLOC_HOTSPOT]}"
