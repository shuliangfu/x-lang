#!/usr/bin/env bash
# boot-027-shu-asm2-cross.sh — BOOT-027 C5 shu_asm2 跨平台波次辅助
#
# 用法（source 后）：
...
BOOT027_PREFIX="${SHU_BOOT027_PREFIX:-shu: [SHU_BOOT027]}"

# 探测 Linux 上可烟测的 shu_asm2 候选路径。
boot027_linux_asm2_candidate() {
...
boot027_parse_smoke_log() {
  ...
  local ok=0
  local smoke=0
  if grep -qF 'shu-asm2-cross-smoke OK' "$log" 2>/dev/null; then
    smoke=1
    ok=1
  elif grep -qF 'shu-asm2-cross-smoke SKIP' "$log" 2>/dev/null; then
    smoke=1
  fi
  echo "$ok $smoke"
}
