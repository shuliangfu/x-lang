...
# C 烟测：blob_stream_roundtrip_ok.c + sqlite.o + -lsqlite3。
std_sqlite_blob_stream_run_c_smoke() {
  ...
  if [ ! -f "$sqlite_o" ]; then
    echo "std-sqlite-blob-stream FAIL: missing $sqlite_o" >&2
    return 1
  fi
  if ! cc -std=c11 -O1 -o "$out" "$src" "$sqlite_o" -lsqlite3 2>/dev/null; then
    echo "std-sqlite-blob-stream FAIL: compile $src" >&2
    return 1
  fi
  set +e
  "$out" >/dev/null 2>&1
  ...
  rm -f "$out"
  if [ "$ec" -ne 0 ]; then
    echo "std-sqlite-blob-stream FAIL: c smoke exit=$ec" >&2
    return 1
  fi
  return 0
}
...
std_sqlite_blob_stream_emit_report() {
  ...
  local stream_su="$3"
  local skip="$4"
  echo "${STD_DB_BLOB_STREAM_PREFIX} status=${status} stream_c=${stream_c} stream_su=${stream_su} skip=${skip}"
}
