#!/usr/bin/env bash
# std-sort-key-cmp.sh — STD-150: sort key comparator manifest helpers.
#
# Usage (after source):
#   std_sort_key_cmp_symbols_ok MOD_X SORT_X TSV [DOC]
#   std_sort_key_cmp_vectors_ok TSV [min]
#   std_sort_key_cmp_run_c_smoke SORT_X   # observational host-C only
#   std_sort_key_cmp_emit_report status run obs skip
...
# Observational C smoke: existing sort.o + key_cmp_ok.c (refuse soft ensure).
...
std_sort_key_cmp_emit_report() {
  ...
  local obs="$3"
  local skip="$4"
  echo "${STD150_PREFIX} status=${status} run=${run_ok} obs=${obs} skip=${skip}"
}
