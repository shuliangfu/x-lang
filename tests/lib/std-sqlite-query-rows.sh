...
STD_DB_QUERY_ROWS_PREFIX="${SHU_STD066_PREFIX:-shu: [SHU_STD066_DB_ROWS]}"
...
std_sqlite_query_rows_run_c_smoke() {
  ...
  local out="/tmp/shu_std_sqlite_query_rows_$$"
  local db_o
...
std_sqlite_query_rows_emit_report() {
  ...
  local rows_su="$3"
  local skip="$4"
  echo "${STD_DB_QUERY_ROWS_PREFIX} status=${status} rows_c=${rows_c} rows_su=${rows_su} skip=${skip}"
}
