...
#   std_unicode_nfc_run_smoke SHU_BIN SU TAG
#   std_unicode_nfc_emit_report status nfc_ok main_ok skip

STD_UNICODE_NFC_PREFIX="${SHU_STD_UNICODE_NFC_PREFIX:-shu: [SHU_STD_UNICODE_NFC]}"
...
# 编译并运行烟测 .su（须已 ensure unicode.o）。
std_unicode_nfc_run_smoke() {
  local shu="$1"
  local src="$2"
  local tag="${3:-smoke}"
  local exe="/tmp/shu_std_unicode_nfc_${tag}_$$"
  if [ ! -f "$src" ]; then
