#!/usr/bin/env bash
# boot-027-xlang-asm2-cross.sh — BOOT-027 C5 xlang_asm2 跨平台波次辅助
#
# 用法（source 后）：
...
BOOT027_PREFIX="${XLANG_BOOT027_PREFIX:-xlang: [XLANG_BOOT027]}"
_LIB_DIR="$(dirname "${BASH_SOURCE[0]:-$0}")"
# shellcheck source=tests/lib/ci-host.sh
. "$_LIB_DIR/ci-host.sh"

# 探测 Linux 上可烟测的 xlang_asm2 候选路径（Docker portable 无 xlang_asm 链则 SKIP）。
boot027_linux_asm2_candidate() {
...
boot027_parse_smoke_log() {
  ...
  local ok=0
  local smoke=0
  if grep -qF 'xlang-asm2-cross-smoke OK' "$log" 2>/dev/null; then
    smoke=1
    ok=1
  elif grep -qF 'xlang-asm2-cross-smoke SKIP' "$log" 2>/dev/null; then
    smoke=1
  fi
  echo "$ok $smoke"
}
