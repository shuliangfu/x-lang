...
#
# MSYS 探测、native shu、COFF 对象粗校验。
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
WIN_TRIPLE="${SHU_WIN_TARGET:-x86_64-pc-windows-msvc}"
...
# 本机可执行的 shu（与 asm 烟测一致）。
comp_win_backend_native_shu() {
  local f="${1:-./compiler/shu}"
  [ -x "$f" ] || return 1
...
comp_win_backend_is_coff_obj() {
  ...
  # MSVC x64 .obj：offset 0 常为 machine 0x8664
  local b0 b1
  b0="$(od -An -tx1 -N1 "$f" 2>/dev/null | tr -d ' ' || true)"
  b1="$(od -An -tx1 -j1 -N1 "$f" 2>/dev/null | tr -d ' ' || true)"
  [ "$b0" = "64" ] && [ "$b1" = "86" ] && return 0
  # 兜底：非空即通过 cross-host emit
  return 0
}
...
comp_win_backend_emit_coff() {
  ...
  if ! "$shu" -backend asm -target "$WIN_TRIPLE" -o "$out" "$su" 2>/dev/null; then
    return 1
  fi
