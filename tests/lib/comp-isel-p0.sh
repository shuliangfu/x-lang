...
#
# 用法（source 后）：
#   comp_isel_p0_run_hook XLANG script_name
#   comp_isel_p0_emit_report status p0_ok p0_skip skip

COMP_ISEL_P0_PREFIX="${XLANG_COMP014_PREFIX:-xlang: [XLANG_COMP014_ISEL_P0]}"
...
comp_isel_p0_run_hook() {
  ...
  if ! XLANG="$xlang" "$path" >/dev/null 2>&1; then
    XLANG="$xlang" "$path" 2>&1 | tail -6 >&2 || true
    return 1
  fi
  return 0
}
