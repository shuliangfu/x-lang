...
#
# 用法（source 后）：
#   std_pps_symbols_ok PROC_SU TSV
#   std_pps_emit_report status pipe_ok win_ok skip

STD_PPS_PREFIX="${SHU_STD_PROCESS_PIPE_SPAWN_PREFIX:-shu: [SHU_STD_PROCESS_PIPE_SPAWN]}"
...
std_pps_symbols_ok() {
  ...
  while IFS=$'\t' read -r item_id kind anchor mod_path _notes; do
    ...
    case "$kind" in
      symbol)
        if ! grep -qF "$anchor" "$proc_su" 2>/dev/null; then
          echo "std-process-pipe-spawn FAIL: missing '$anchor' in $proc_su" >&2
          miss=$((miss + 1))
        fi
        ;;
      file)
