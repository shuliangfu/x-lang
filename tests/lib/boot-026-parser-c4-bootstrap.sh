#!/usr/bin/env bash
# boot-026-parser-c4-bootstrap.sh — BOOT-026 C4 X bootstrap 波次辅助
#
# 用法（source 后）：
...
boot026_parse_x_emit_log() {
  local log="$1"
  local minimal=0
  local probe=0
  if grep -qE 'parser-parse-bootstrap-x-emit-gate PASS' "$log" 2>/dev/null; then
    probe=1
...
boot026_emit_report() {
  ...
  local c4_x_probe="$3"
  local skip="$4"
  echo "${BOOT026_PREFIX} status=${status} c4_minimal_ok=${c4_minimal_ok} c4_x_probe=${c4_x_probe} skip=${skip}"
}
