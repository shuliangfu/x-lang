...
STBL_TIER_S_PREFIX="${SHU_STBL_TIER_S_PREFIX:-shu: [SHU_STBL_TIER_S]}"

# api 型 manifest：每行一个符号，须在 mod.su 存在 function 定义。
...
stbl_tier_s_validate_feature_manifest() {
  ...
  # std-error-unify 专用：module 行 base_fn 在 std/error/mod.su。
