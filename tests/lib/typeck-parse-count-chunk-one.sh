#!/usr/bin/env bash
# typeck-parse-count-chunk-one.sh — 单块 typeck.x parse 指标（独立进程，避免父 shell 连跑 ~90 次后异常）
#
# 用法（仓库根）：
#   ./tests/lib/typeck-parse-count-chunk-one.sh <compiler> <chunk.x> <out.o> [log]
#
