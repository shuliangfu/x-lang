#!/usr/bin/env bash
# net-asm-gcc-link.sh — xlang_asm -o 失败时用 gcc 手动链 net 测试可执行文件
#
# 背景：旧 xlang_asm 内嵌 runtime_link_abi 可能未按需链 net.o；本脚本 emit user.o 后
# 按 boot-std-link-contract 补齐 net/thread/runtime glue，供 run-net.sh 回退。
#
# 用法（仓库根）：
#   . tests/lib/net-asm-gcc-link.sh
#   net_asm_gcc_link tests/net/main.x /tmp/xlang_net
...
net_asm_gcc_link() {
  ...
  local x="$2"
  local exe="$3"
  local user_o="${4:-/tmp/xlang_net_user.$$.o}"
  local gcc_bin="${XLANG_NET_GCC:-gcc}"
  ensure_runtime_thread_glue_o
  net_asm_emit_user_o "$link_xlang" "$x" "$user_o"
