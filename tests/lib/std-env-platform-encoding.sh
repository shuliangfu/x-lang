...
STD_ENV_PLATFORM_ENCODING_PREFIX="${SHU_STD132_ENV_PLATFORM_ENCODING_PREFIX:-shu: [SHU_STD132_ENV_PLATFORM_ENCODING]}"
...
std_env_platform_encoding_run_c_smoke() {
  ...
  local out="/tmp/shu_std_env_pe_c_$$"
  if [ ! -f "$src" ]; then
    printf '%s\n' \
      '#include <stdint.h>' \
