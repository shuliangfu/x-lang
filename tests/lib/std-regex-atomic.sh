...
STD_REGEX_ATOMIC_PREFIX="${SHU_STD124_REGEX_ATOMIC_PREFIX:-shu: [SHU_STD124_REGEX_ATOMIC]}"
...
std_regex_atomic_run_c_smoke() {
  local regex_c="$1"
  local out="/tmp/shu_std_regex_atomic_c_$$"
...
std_regex_atomic_run_su_smoke() {
  local shu="$1"
  local src="$2"
  local exe="/tmp/shu_std_regex_atomic_su_$$"
...
std_regex_atomic_emit_report() {
  echo "${STD_REGEX_ATOMIC_PREFIX} status=$1 c=$2 su=$3 skip=$4"
}
