...
#   d04_outcome_check XLANG src
#   d04_outcome_link_run XLANG src expect_exit case_id
#   d04_emit_report status cases_ok cases_fail skip

D04_PREFIX="${XLANG_D04_PREFIX:-xlang: [XLANG_D04]}"
...
# 链接回退 xlang-c（与 BOOT-019/015 runner 一致）。
...
d04_stage_binaries_ready() {
  local s1="${XLANG_D04_STAGE1:-compiler/xlang_asm_stage1}"
