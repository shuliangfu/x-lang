...
tool_lint_expect_check_silent() {
  ...
  local src="$2"
  local out
  out=$("$xlang" check "$src" 2>&1) && ec=0 || ec=$?
  if [ "$ec" -ne 0 ]; then
...
# XLANG_PAD_FIELDS=1：期望含 -pad-fields warning 且 exit 0（默认 profile）。
tool_lint_expect_warn_pad() {
  local xlang="$1"
  local src="$2"
  local out
  out=$(XLANG_PAD_FIELDS=1 "$xlang" check "$src" 2>&1) && ec=0 || ec=$?
...
# XLANG_HOT_REORDER=1：期望含 -hot-reorder warning 且 exit 0。
tool_lint_expect_warn_reorder() {
  local xlang="$1"
  local src="$2"
  local out
  out=$(XLANG_HOT_REORDER=1 "$xlang" check "$src" 2>&1) && ec=0 || ec=$?
