#!/usr/bin/env bash
# std-compress-stream.sh — STD-039 gzip stream helpers (honesty prefer-asm).
#
# Usage (after source):
#   std_compress_stream_symbols_ok MOD_X COMPRESS_C TSV
#   std_compress_stream_run_smoke XLANG_BIN SRC [TAG]
#   std_compress_stream_emit_report status run obs skip
    ...
    case "$kind" in
      api)
        if ! grep -qE "function ${anchor}\\(" "$mod_x" 2>/dev/null; then
          echo "std-compress-stream FAIL: missing api '$anchor' in $mod_x" >&2
          miss=$((miss + 1))
        fi
        ;;
      symbol)
        case "$mod_path" in
          ...
          std/compress/gzip/libz.x) mod_path="$compress_c" ;;
          std/compress/gzip/gzip.c) mod_path="$compress_c" ;;
          std/compress/brotli/lib.x|std/compress/brotli/brotli.c) mod_path="$compress_c" ;;
          *) mod_path="$mod_x" ;;
        esac
        ...
        ;;
      section)
        local doc="${XLANG_STD_COMPRESS_STREAM_DOC:-analysis/archive/std/std-compress-stream-v1.md}"
        if [ ! -f "$doc" ] || ! grep -qF "$anchor" "$doc" 2>/dev/null; then
          echo "std-compress-stream FAIL: missing section '$anchor' in $doc" >&2
          miss=$((miss + 1))
        fi
        ;;
      file|smoke)
      ...
      script)
        if [ -n "$anchor" ] && [ -f "$anchor" ]; then
          ...
          echo "std-compress-stream FAIL: missing script '$anchor'" >&2
          miss=$((miss + 1))
        fi
        ;;
    esac
...
std_compress_stream_run_smoke() {
  ...
  local log="/tmp/xlang_std_compress_stream_${tag}_$$.log"
  if [ ! -f "$src" ]; then
  ...
  if [ "$o_ec" -ne 0 ] || [ ! -x "$exe" ]; then
    echo "std-compress-stream OBS tip product -o (ec=$o_ec)" >&2
  ...
  rm -f "$exe" "$log"
  if [ "$ec" -ne 0 ]; then
    echo "std-compress-stream OBS tip run exit=$ec" >&2
    return 1
  fi
  return 0
}
...
std_compress_stream_emit_report() {
  ...
  local obs="$3"
  local skip="$4"
  echo "${STD_COMPRESS_STREAM_PREFIX} status=${status} run=${run_ok} obs=${obs} skip=${skip}"
}
