...
std_sync_lock_diag_symbols_ok() {
  ...
  while IFS=$'\t' read -r item_id kind anchor mod_path _notes; do
    ...
    case "$kind" in
      ...
      symbol)
        local path="$mod_path"
        if [ "$path" = "std/sync/sync.c" ]; then path="$sync_c"; fi
...
# C 烟测：lock_diag_smoke_ok.c + sync.o。
std_sync_lock_diag_run_c_smoke() {
  ...
  if [ ! -f "$sync_o" ]; then
    echo "std-sync-lock-diag FAIL: missing $sync_o" >&2
    return 1
  fi
  if ! cc -std=c11 -O1 -o "$out" "$src" "$sync_o" -lpthread 2>/dev/null; then
    if ! cc -std=c11 -O1 -o "$out" "$src" "$sync_o" 2>/dev/null; then
...
std_sync_lock_diag_emit_report() {
  ...
  local su_ok="$3"
  local skip="$4"
  echo "${STD_SYNC_LOCK_DIAG_PREFIX} status=${status} c=${c_ok} su=${su_ok} skip=${skip}"
}
