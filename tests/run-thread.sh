#!/usr/bin/env bash
# std.thread self/create/join smoke
#
...
PREFIX="${XLANG_THREAD_PREFIX:-xlang: [THREAD]}"
...
echo "=== thread gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

exe="/tmp/xlang_thread_$$"
log="/tmp/xlang_thread_$$.log"
rm -f "$exe" "$log"
[ -f "tests/thread/main.x" ] || die "missing tests/thread/main.x"
set +e
gate_run_timeout "$XLANG_CASE_TIMEOUT" "$XLANG_BIN" -L . "tests/thread/main.x" -o "$exe" >"$log" 2>&1
o_ec=$?
set -e
if [ "$o_ec" -eq 124 ]; then
