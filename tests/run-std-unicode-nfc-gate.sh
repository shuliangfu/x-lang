...
for f in "$DOC" "$MANIFEST" "$LIB" "$UNI_X" "$UNI_X" "$UNI_IMPL" "$NFC_X" "$MAIN_X"; do
  if [ ! -f "$f" ]; then
...
while IFS=$'\t' read -r item_id kind anchor _rest; do
  ...
  case "$kind" in
    api)
      API_N=$((API_N + 1))
      if ! grep -qE "function ${anchor}\\(" "$UNI_X" 2>/dev/null; then
...
sym_miss="$(std_unicode_nfc_symbols_ok "$UNI_X" "$UNI_IMPL" "$MANIFEST" || true)"
