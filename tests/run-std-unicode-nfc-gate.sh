...
DOC="${SHU_STD_UNICODE_NFC_DOC:-analysis/std-unicode-nfc-v1.md}"
MANIFEST="${SHU_STD_UNICODE_NFC_TSV:-tests/baseline/std-unicode-nfc.tsv}"
UNI_SU="std/unicode/mod.su"
UNI_C="std/unicode/unicode.c"
LIB="tests/lib/std-unicode-nfc.sh"
NFC_SU="tests/unicode/nfc_gold.su"
MAIN_SU="tests/unicode/main.su"
MIN_APIS=3
...
if [ -n "$SHU_BIN" ]; then
  echo "=== STD-037: typeck + smoke (SHU=$SHU_BIN) ==="
  ...
  if ! "$SHU_BIN" check -L . "$NFC_SU" >/dev/null 2>&1; then
    echo "std-unicode-nfc gate FAIL: typeck $NFC_SU" >&2
    "$SHU_BIN" check -L . "$NFC_SU" 2>&1 | tail -10 >&2 || true
    std_unicode_nfc_emit_report "fail" 0 0 0
    exit 1
  fi
  if std_unicode_nfc_run_smoke "$SHU_BIN" "$NFC_SU" "nfc_gold"; then
    NFC_OK=1
  else
    std_unicode_nfc_emit_report "fail" 0 0 0
    exit 1
  fi
  if std_unicode_nfc_run_smoke "$SHU_BIN" "$MAIN_SU" "main"; then
    MAIN_OK=1
  else
  ...
  echo "std-unicode-nfc gate SKIP smoke (no native shu-c)" >&2
fi
