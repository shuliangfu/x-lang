...
#
# 启用：XLANG_SOCKETIO_NPM_ROOM=1 ./tests/run-std-socketio-npm-room-live.sh
set -e
cd "$(dirname "$0")/.."

NPM_DIR="tests/socketio/npm_live"
LIVE_X="tests/socketio/npm_live_room.x"
PORT="${XLANG_SOCKETIO_NPM_ROOM_PORT:-13004}"

if [ "${XLANG_SOCKETIO_NPM_ROOM:-0}" != "1" ]; then
  echo "std-socketio npm room live SKIP (set XLANG_SOCKETIO_NPM_ROOM=1 to enable)"
  exit 0
fi
...
if XLANG_BIN="$(stdlib_cm_native_shu ./compiler/xlang-c && echo ./compiler/xlang-c || true)"; then
  ...
  echo "std-socketio npm room live SKIP (no native xlang)" >&2
  exit 0
fi
...
if ! (cd "$NPM_DIR" && npm install --no-audit --no-fund >/tmp/xlang_socketio_npm_room_install_$$.log 2>&1); then
  echo "std-socketio npm room live FAIL: npm install" >&2
  tail -15 /tmp/xlang_socketio_npm_room_install_$$.log >&2 || true
  exit 1
fi
...
PORT="$PORT" node "$NPM_DIR/server.mjs" >/tmp/xlang_socketio_npm_room_svr_$$.log 2>&1 &
SVR_PID=$!
sleep 1.5
...
exe="/tmp/xlang_std_socketio_npm_room_$$"
if ! "$XLANG_BIN" -L . "$LIVE_X" -o "$exe" 2>/tmp/xlang_socketio_npm_room_link_$$.log; then
  echo "std-socketio npm room live FAIL: link" >&2
  tail -8 /tmp/xlang_socketio_npm_room_link_$$.log >&2 || true
  exit 1
fi
...
if [ "$run_ec" -ne 0 ]; then
  ...
  tail -8 /tmp/xlang_socketio_npm_room_svr_$$.log >&2 || true
  exit 1
fi
