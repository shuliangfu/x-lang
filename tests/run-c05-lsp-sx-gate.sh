...
if [ "${SHUX_C05_MANIFEST_ONLY:-0}" = "1" ]; then
  echo "c05 lsp-sx gate OK (manifest only)"
  exit 0
fi

if ! native_shu "$SHUX_BIN"; then
  for cand in ./compiler/shux ./compiler/shux-sx ./compiler/shux-c; do
...
if ! native_shu "$SHUX_BIN"; then
  echo "c05 lsp-sx gate: SKIP smoke (no native shux; manifest OK)"
  exit 0
fi
