...
if [ "${XLANG_BOOT025_MANIFEST_ONLY:-0}" = "1" ]; then
...
XLANG_BOOT024_MANIFEST_ONLY=1 ./tests/run-boot-024-parser-bootstrap-emit-gate.sh
...
if boot025_gen12_linux_asm; then
  echo "=== BOOT-025: stage2 bstrict (Linux xlang_asm) ==="
  ...
  if [ -x "./compiler/xlang" ]; then
    ...
    if XLANG=./compiler/xlang ./tests/run-bootstrap-stage2-dogfood-parser-typeck.sh >/dev/null 2>&1; then
      DOGFOOD_OK=1
  ...
  echo "boot-025-parser-gen12-consistency gate SKIP wave (Darwin or no xlang_asm)" >&2
fi
