...
if [ -n "${XLANG_S3_EMIT_HEAVY_COMPILER:-}" ] && [ -x "${XLANG_S3_EMIT_HEAVY_COMPILER}" ]; then
  if XLANG_S3_EMIT_HEAVY_COMPILER="${XLANG_S3_EMIT_HEAVY_COMPILER}" ./tests/run-s3-driver-emit-heavy.sh; then
    emit_heavy_ok=1
  fi
fi
if [ "$emit_heavy_ok" -eq 0 ]; then
  for cand in ./compiler/xlang_asm.strict_glue ./compiler/xlang_asm.experimental ./compiler/xlang_asm; do
    ...
    if XLANG_S3_EMIT_HEAVY_COMPILER="$cand" ./tests/run-s3-driver-emit-heavy.sh; then
...
WPO_OK=0
COMP="${XLANG_S3_EMIT_HEAVY_COMPILER:-./compiler/xlang_asm.experimental}"
