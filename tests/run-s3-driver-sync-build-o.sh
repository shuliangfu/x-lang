...
if [ -n "${SHU_S3_EMIT_HEAVY_COMPILER:-}" ] && [ -x "${SHU_S3_EMIT_HEAVY_COMPILER}" ]; then
  if SHU_S3_EMIT_HEAVY_COMPILER="${SHU_S3_EMIT_HEAVY_COMPILER}" ./tests/run-s3-driver-emit-heavy.sh; then
    emit_heavy_ok=1
  fi
fi
if [ "$emit_heavy_ok" -eq 0 ]; then
  for cand in ./compiler/shu_asm.strict_glue ./compiler/shu_asm.experimental ./compiler/shu_asm; do
    ...
    if SHU_S3_EMIT_HEAVY_COMPILER="$cand" ./tests/run-s3-driver-emit-heavy.sh; then
...
SRC="/tmp/shu_s3_driver_emit_heavy.o"
[ -f "$SRC" ] || {
...
WPO_OK=0
COMP="${SHU_S3_EMIT_HEAVY_COMPILER:-./compiler/shu_asm.experimental}"
...
WPO_TMP="/tmp/shu_s3_driver_wpo.o"
...
if [ -x "$COMP" ]; then
  ...
  if env -u SHU_ASM_START_FUNC SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=1 \
    SHU_ASM_WPO_DCE=1 \
    "$COMP" -backend asm -o "$WPO_TMP" $LIBROOT compiler/src/driver/compile.su 2>/dev/null; then
    wpo_sz=$(text_section_size "$WPO_TMP")
    if [ "$wpo_sz" -gt 0 ] && [ "$wpo_sz" -le 768 ] 2>/dev/null && \
       nm "$WPO_TMP" 2>/dev/null | grep -qE ' T (compile_dispatch_asm_backend|run_compiler_full_su|entry)$'; then
      cp -f "$WPO_TMP" "$DEST"
...
if [ -f "$LINK_ALIAS_SRC" ]; then
  ...
  if nm "$LINK_O" 2>/dev/null | grep -q ' T _driver_run_compiler_full_su$'; then
    echo "s3 driver sync-build-o: wrote $LINK_O (driver_run_compiler_full_su alias OK)"
  else
    echo "s3 driver sync-build-o: warn: $LINK_O missing driver_run_compiler_full_su" >&2
  fi
fi
