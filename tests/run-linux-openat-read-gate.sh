...
SU="tests/sys/linux_openat_read_smoke.sx"
...
if ! "$SHUX" -freestanding -backend asm -o "$OUT" "$SU" 2>/tmp/shux_linux_openat_read.log; then
  echo "linux-openat-read-gate FAIL: compile $SU" >&2
