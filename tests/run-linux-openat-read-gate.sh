...
# 环境：XLANG_LINUX_OPENAT_READ_FAIL=1 失败时硬退出
set -e
cd "$(dirname "$0")/.."

FAIL=${XLANG_LINUX_OPENAT_READ_FAIL:-0}
...
if [ ! -x "$XLANG" ]; then
  echo "linux-openat-read-gate: SKIP (no xlang/xlang-c)"
  exit 0
fi
...
if ! "$XLANG" -freestanding -backend asm -o "$OUT" "$X" 2>/tmp/xlang_linux_openat_read.log; then
  echo "linux-openat-read-gate FAIL: compile $X" >&2
...
echo "linux-openat-read-gate OK (Linux freestanding xlang_sys_openat + read)"
exit 0
