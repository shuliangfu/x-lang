...
DOC="${XLANG_PERF_SYSCALL_BATCH_DOC:-analysis/perf-syscall-batch-v1.md}"
...
if perf_sb_strace_probe_ok; then
  ...
  chmod +x "$RUNNER"
  if XLANG_SYSCALL_BATCH_FAIL=0 ./"$RUNNER" 2>&1 | tee /tmp/perf_syscall_batch_smoke.log | tail -10; then
