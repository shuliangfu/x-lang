...
if [ -n "$XLANG_BIN" ]; then
  echo "=== LANG-009: smoke (XLANG=$XLANG_BIN; check observational) ==="
  ...
  for x in "$SMOKE1" "$SMOKE2"; do
    ...
    if [ "$link_ec" -ne 0 ]; then
      echo "lang-option-generic gate SKIP runnable link ($x)" >&2
    ...
    rm -f "$exe"
    if [ "$run_ec" -ne 0 ]; then
      echo "lang-option-generic gate SKIP run $x exit=$run_ec" >&2
      SKIP=1
      break
    fi
