#!/usr/bin/env bash
# toplevel let smoke: single global + two lets with dependency
#
...
PREFIX="${XLANG_TOPLEVEL_LET_PREFIX:-xlang: [TOPLEVEL_LET]}"
...
run_case() {
  ...
  local exe="/tmp/xlang_toplevel_${tag}_$$"
  local log="/tmp/xlang_toplevel_${tag}_$$.log"
  local o_ec r_ec
...
echo "=== toplevel-let gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

run_case main tests/toplevel_let/main.x 42
run_case two_lets tests/toplevel_let/two_lets.x 12
ok_report
