...
for f in "$DOC" "$OPT_MANIFEST" "$RES_MANIFEST" "$LIB" "$OPT_SU" "$RES_SU" "$SMOKE_SU" std/option/README.md std/result/README.md; do
  if [ ! -f "$f" ]; then
...
std_option_result_check_manifest "$OPT_SU" "$OPT_MANIFEST" "$MIN_OPT" "std.option"
std_option_result_check_manifest "$RES_SU" "$RES_MANIFEST" "$MIN_RES" "std.result"
...
std_option_result_emit_report "ok" "$SU_OK" "$SKIP"
