...
XLANG="$RUN_XLANG" ./tests/run-io-read-ptr-slice.sh

echo "=== IO unified: std.io smoke (run-io.sh) ==="
chmod +x tests/run-io.sh
XLANG="$RUN_XLANG" ./tests/run-io.sh
...
if ci_is_linux; then
  # 默认 regression；CI 原生 Linux x86_64 可经 env 追加 XLANG_PERF_FAIL_ON_IO_ZIG=1（L1 ≥ Zig）
...
if ci_is_linux && ci_io_uring_available; then
  ...
  grep -q 'ZC-1 provided vs batch OK' /tmp/io_unified_zc1_perf.log || {
    if [ -n "${XLANG_CI_REQUIRE_ZC1:-}" ]; then
      echo "run-io-unified-gate FAIL: ZC-1 -10% required (XLANG_CI_REQUIRE_ZC1=1)" >&2
      exit 1
    fi
