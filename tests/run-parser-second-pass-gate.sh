...
# strict 链仍靠 parser_su.o；本门禁保证 experimental/second pass 不回归空 parser.o。
# 用法：
...
# su_len 须与符号名等长；偏差时 safe_helper 不命中，combined 指标回归无感知。
if [ "${SHU_PARSER_SAFE_HELPER_LEN_GATE:-1}" = "1" ]; then
...
if [ "${SHU_PARSER_THIN_GLUE_SYMBOL_INTEGRITY:-1}" = "1" ]; then
  chmod +x tests/run-parser-thin-glue-symbol-integrity-gate.sh 2>/dev/null || true
  SYM_FAIL=${SHU_PARSER_THIN_GLUE_SYMBOL_INTEGRITY_FAIL:-${SHU_PARSER_SECOND_PASS_FAIL:-0}}
  SHU_PARSER_THIN_GLUE_SYMBOL_INTEGRITY_FAIL="$SYM_FAIL" \
    ./tests/run-parser-thin-glue-symbol-integrity-gate.sh
fi

FAIL=${SHU_PARSER_SECOND_PASS_FAIL:-1}
EMIT_HEAVY=${SHU_PARSER_SECOND_PASS_EMIT_HEAVY:-0}
WPO_DCE=${SHU_PARSER_SECOND_PASS_WPO_DCE:-0}
if [ "$EMIT_HEAVY" = "1" ]; then
  # parser.o：slice 委托 + safe_helper 真 emit 回归下限。
  # combined：parser.o + thin_glue（深循环 C glue）；全量 parser_su 链入后 thin_glue 不再含 seed parse_into_buf C（~9KB），故默认 125KB。
  MIN_TEXT="${SHU_PARSER_SECOND_PASS_MIN_TEXT:-10000}"
  MIN_COMBINED="${SHU_PARSER_SECOND_PASS_MIN_COMBINED:-125000}"
  # stretch：含 parser_su.o 侧 parse_into_buf C 体积的审计指标（9434B ≈ seed slice 差值；非链接对象）。
  STRETCH_COMBINED="${SHU_PARSER_SECOND_PASS_STRETCH_COMBINED:-150000}"
  SEED_PARSE_METRIC_BYTES="${SHU_PARSER_SECOND_PASS_SEED_METRIC_BYTES:-9434}"
else
  MIN_TEXT="${SHU_PARSER_SECOND_PASS_MIN_TEXT:-16}"
  MIN_COMBINED=0
  STRETCH_COMBINED=0
...
# 与 build_shu_asm compile_su 一致（须在 compiler/ 目录下执行）。
...
COMP_IN="./shu_asm"
if [ -n "${SHU_PARSER_SECOND_PASS_COMPILER:-}" ]; then
  case "${SHU_PARSER_SECOND_PASS_COMPILER}" in
    compiler/*) COMP_IN="./${SHU_PARSER_SECOND_PASS_COMPILER#compiler/}" ;;
    *) COMP_IN="${SHU_PARSER_SECOND_PASS_COMPILER}" ;;
  esac
fi
...
TMP="/tmp/shu_parser_second_pass_gate.$$.o"
rm -f "$TMP" 2>/dev/null || true

echo "parser-second-pass-gate: compile parser.su (ENTRY_MODULE_ONLY + SKIP_TYPECK${EH_SUFFIX}) with compiler/$COMP_IN ..."
PASS_ENV="env -u SHU_ASM_START_FUNC SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1"
if [ "$EMIT_HEAVY" = "1" ]; then
  PASS_ENV="$PASS_ENV SHU_ASM_ENTRY_EMIT_HEAVY=1"
  if [ "$WPO_DCE" = "1" ]; then
    PASS_ENV="$PASS_ENV SHU_ASM_WPO_DCE=1"
  else
    PASS_ENV="$PASS_ENV SHU_ASM_WPO_DCE=0"
  fi
fi
if ! (
  cd compiler
  $PASS_ENV \
    "$COMP_IN" -backend asm -o "$TMP" $LIBROOT src/parser/parser.su
) > /tmp/shu_parser_sp_gate.log 2>&1; then
  echo "parser-second-pass-gate FAIL: compile command failed" >&2
  tail -n 12 /tmp/shu_parser_sp_gate.log 2>/dev/null || true
  rm -f "$TMP" /tmp/shu_parser_sp_gate.log 2>/dev/null || true
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi

if grep -q 'asm_codegen_elf_o failed' /tmp/shu_parser_sp_gate.log 2>/dev/null; then
  echo "parser-second-pass-gate FAIL: asm_codegen_elf_o failed in log" >&2
  tail -n 8 /tmp/shu_parser_sp_gate.log 2>/dev/null || true
  rm -f "$TMP" /tmp/shu_parser_sp_gate.log 2>/dev/null || true
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi
...
# audit：动态度量 seed parse_into_buf C 体积（thin_glue 含/不含 NO_SEED 差值；bootstrap 侧由 parser_su.o 提供）。
if [ "$EMIT_HEAVY" = "1" ] && [ "${SHU_PARSER_SECOND_PASS_SEED_METRIC_DYNAMIC:-1}" = "1" ]; then
  SEED_METRIC_TMP="/tmp/shu_parser_seed_metric.$$.o"
  SEED_METRIC_TMP2="/tmp/shu_parser_seed_metric_noseed.$$.o"
...
STRETCH_FAIL=${SHU_PARSER_SECOND_PASS_STRETCH_FAIL:-0}
...
BASELINE_FAIL=${SHU_PARSER_SECOND_PASS_BASELINE_FAIL:-0}
BASELINE_FILE="${SHU_PARSER_SECOND_PASS_BASELINE:-tests/baseline/parser-second-pass.tsv}"
