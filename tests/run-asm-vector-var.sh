#!/usr/bin/env bash
# asm 7.3: VAR+VAR vector per-lane binop direct ldr (no push/pop); based on vec_add_check.x.
#
...
#   - hard: tests/vector/vec_add_check.x product -o run exit 0
#   - hard (Darwin+otool): main has no `sub sp, sp, #0x10` lane spill
...
PREFIX="${XLANG_ASM_VECTOR_VAR_PREFIX:-xlang: [XLANG_ASM_VECTOR_VAR]}"
...
echo "=== asm-vector-var gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

SRC="tests/vector/vec_add_check.x"
[ -f "$SRC" ] || die "missing $SRC"
exe="/tmp/xlang_asm_vector_var_$$"
log="/tmp/xlang_asm_vector_var_$$.log"
rm -f "$exe" "$log"
...
if [ "$(uname -s)" = Darwin ] && command -v otool >/dev/null 2>&1; then
  if otool -tv "$exe" 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'sub.*sp, sp, #0x10'; then
    rm -f "$exe"
    die "main still uses stack push for vector lane binop"
  fi
  RUN_OK=$((RUN_OK + 1))
else
  SKIP=$((SKIP + 1))
