...
for f in "$DOC" "$MANIFEST" "$VECTORS" "$CRYPTO_MOD" "$RAND_MOD" \
  std/crypto/core.x compiler/src/asm/runtime_crypto_inc_glue.inc std/random/random.x compiler/src/asm/runtime_random_fill.inc tests/lib/std-crypto.sh; do
  if [ ! -f "$f" ]; then
