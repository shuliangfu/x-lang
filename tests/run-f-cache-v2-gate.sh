#!/usr/bin/env bash
# F-cache v2：std.cache 逻辑全量 .sx（删除 cache_glue.c）。
set -e
cd "$(dirname "$0")/.."
...
echo "=== F-cache v2: LRU/pool logic → cache.sx (zero glue) ==="
...
grep -q 'cache_lru_create_c' std/cache/cache.sx || die "cache.sx missing lru create"
grep -q 'cache_pool_create_c' std/cache/cache.sx || die "cache.sx missing pool create"
grep -q 'cache_smoke_c' std/cache/cache.sx || die "cache.sx missing smoke"
grep -q 'cache_f_cache_v2_marker_c' std/cache/cache.sx || die "cache.sx missing v2 marker"
