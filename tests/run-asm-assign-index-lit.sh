...
$SHU tests/asm/assign_index_lit_fast.su -o /tmp/shu_asm_assign_index_lit 2>&1
exitcode=0
/tmp/shu_asm_assign_index_lit >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 99 ] && { echo "run-asm-assign-index-lit FAIL: expected exit 99, got $exitcode"; exit 1; }

if otool -tv /tmp/shu_asm_assign_index_lit 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'mov x2'; then
