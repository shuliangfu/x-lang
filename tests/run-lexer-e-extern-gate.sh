#!/usr/bin/env bash
# A-11/C-04：lexer.sx parse 修复烟测 + pipeline -E-extern 可生成（解锁瘦 TU）。
...
if ! "$SHUX" check src/lexer/lexer.sx 2>/tmp/shux_lexer_check.log; then
  echo "lexer-e-extern-gate FAIL: check lexer.sx" >&2
...
if ! "$SHUX" $DIRS src/pipeline/pipeline.sx -E -E-extern >"$GEN" 2>/tmp/shux_pipeline_e.log; then
...
rm -f "$GEN" 2>/dev/null || true
echo "lexer-e-extern-gate OK (lexer.sx parse + pipeline -E-extern)"
exit 0
