#!/usr/bin/env bash
# B-strict enum smoke: minimal enum + Color.Green variant (no return match).
#
# Honesty: soft default xlang_asm + soft bootstrap-link (false authority) retired.
...
# bootstrap-link). Report: run=/obs=/skip=
...
PREFIX="${XLANG_ENUM_ASM_PREFIX:-xlang: [ENUM-ASM]}"
...
run_exit() {
  ...
  local exe="/tmp/xlang_enum_${tag}_$$"
  local log="/tmp/xlang_enum_${tag}_$$.log"
  local o_ec r_ec
...
echo "=== enum-asm gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

run_exit minimal tests/enum/minimal.x 0
run_exit simple tests/enum/simple.x 1
