#!/usr/bin/env bash
# parser/typeck/codegen/ast .x 变更后：migrate-x gen 门禁 + relink shux_asm + 烟测 import/hex + M-3 region typeck（X 路径）。
...
SHUX_FORCE_REFRESH_ASM_GATE="${SHUX_FORCE_REFRESH_ASM_GATE:-0}" ./tests/run-migrate-x-gen-gate.sh
...
echo "refresh shux asm gate: 0x const typeck (X path via relinked shux) ..."
./compiler/shux check -L . tests/parser/const_hex.x 2>&1 || {
  echo "refresh shux asm gate FAIL: shux check const_hex.x" >&2
  exit 1
}

echo "refresh shux asm gate: 0x const C emit (shux-c) ..."
out_hex=$(./compiler/shux-c -L . -E tests/parser/const_hex.x 2>&1) || {
  echo "refresh shux asm gate FAIL: shux-c -E const_hex.x" >&2
  exit 1
}
...
echo "refresh shux asm gate: M-3 region typeck (X path via relinked shux) ..."
...
echo "refresh shux asm gate: M-4 linear typeck (X path via relinked shux) ..."
...
grep -q 'linear typeck OK' /tmp/refresh_asm_linear.log || {
  echo "refresh shux asm gate FAIL: X path linear typeck" >&2
  exit 1
}
