...
# bootstrap run-all 下 $SHUX 为 seed（验 .sx 流水线）；-o 链接走 $RUN_SHUX（Darwin/ARM64 seed asm ld 会 __TEXT r-x 失败）。
set -e
cd "$(dirname "$0")/.."
...
# 无 -D：应走 #else，返回 22
"$RUN_SHUX" tests/preprocess/main.sx -o /tmp/shux_pp_no_d 2>&1
...
"$RUN_SHUX" -D FOO tests/preprocess/main.sx -o /tmp/shux_pp_d_foo 2>&1
exitcode=0; /tmp/shux_pp_d_foo >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 11 ] && { echo "expected 11 (-D FOO), got $exitcode"; exit 1; }

# -DFOO 无空格形式
"$RUN_SHUX" -DFOO tests/preprocess/main.sx -o /tmp/shux_pp_dfoo 2>&1
...
err=$("$RUN_SHUX" tests/preprocess/elseif_after_else.sx -o /tmp/shux_pp_bad 2>&1) || true
