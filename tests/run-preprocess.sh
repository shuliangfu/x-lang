...
# bootstrap run-all 下 $SHUX build 为 seed（验 .x 流水线）；-o 链接走 $RUN_SHUX（Darwin/ARM64 seed asm ld 会 __TEXT r-x 失败）。
set -e
cd "$(dirname "$0")/.."
...
# 无 -D：应走 #else，返回 22
"$RUN_SHUX" build tests/preprocess/main.x -o /tmp/shux_pp_no_d 2>&1
...
"$RUN_SHUX" build -D FOO tests/preprocess/main.x -o /tmp/shux_pp_d_foo 2>&1
exitcode=0; /tmp/shux_pp_d_foo >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 11 ] && { echo "expected 11 (-D FOO), got $exitcode"; exit 1; }

# -DFOO 无空格形式
"$RUN_SHUX" build -DFOO tests/preprocess/main.x -o /tmp/shux_pp_dfoo 2>&1
...
err=$("$RUN_SHUX" build tests/preprocess/elseif_after_else.x -o /tmp/shux_pp_bad 2>&1) || true
echo "$err" | grep -q "#elseif after #else" || { echo "expected '#elseif after #else' in: $err"; exit 1; }

# 边界：同一块内两个 #else → 应报 duplicate #else
err=$("$RUN_SHUX" build tests/preprocess/duplicate_else.x -o /tmp/shux_pp_bad 2>&1) || true
...
"$RUN_SHUX" build tests/preprocess/target_os_if.x -o /tmp/shux_pp_target_os 2>&1
...
err=$("$RUN_SHUX" build tests/preprocess/extra_endif.x -o /tmp/shux_pp_bad 2>&1) || true
echo "$err" | grep -q "#endif without #if" || { echo "expected '#endif without #if' in: $err"; exit 1; }
