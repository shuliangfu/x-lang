#!/usr/bin/env bash
# S3 driver 同链烟测：build_asm/driver_compile_link.o 须导出 driver_run_compiler_full_x 且含 X parse_argv。
# 用法：./tests/run-s3-driver-o-parity.sh
# 前置：./tests/run-s3-driver-sync-build-o.sh
# 可选：XLANG_S3_FAIL_ON_PARITY=1 — 任一检查失败时 exit 1
set -e
cd "$(dirname "$0")/.."
...
MIN_REAL=${MIN_REAL:-0}
FAIL=${XLANG_S3_FAIL_ON_PARITY:-0}
...
for sym in driver_run_compiler_full_x driver_compile_dispatch_asm_backend driver_compile_dispatch_emit_c_path; do
  if ! nm "$DRIVER_LINK_O" 2>/dev/null | grep -q " T _${sym}$"; then
    parity_fail "$DRIVER_LINK_O missing symbol $sym"
  fi
done
echo "s3 driver parity: link.o exports driver_run_compiler_full_x + dispatch_*"
