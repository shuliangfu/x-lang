...
for kw in STD-076 parse build query_encode resolve IPv6; do
  if ! grep -qF -- "$kw" "$DOC" 2>/dev/null && ! grep -qF -- "$kw" "$MANIFEST" 2>/dev/null; then
    echo "std-url gate FAIL: doc/manifest missing '$kw'" >&2
    exit 1
  fi
done

if ! grep -qF '## 3. Gate' "$DOC" 2>/dev/null; then
  echo "std-url gate FAIL: doc missing '## 3. Gate'" >&2
  exit 1
fi
...
if [ "${sym_miss:-0}" -gt 0 ]; then
  ...
  echo "std-url gate FAIL: symbol_miss=${sym_miss}" >&2
  exit 1
fi
...
if [ -x ./compiler/xlang-c ] || [ -x ./compiler/xlang ] || [ -x ./compiler/xlang_asm ]; then
  ...
  if cc -std=c11 -O1 -o /tmp/xlang_url_smoke \
    ...
    echo "std-url gate SKIP c smoke (observational; link failed)" >&2
    tail -n 10 /tmp/xlang_url_smoke_link.err 2>/dev/null || true
  fi
else
  echo "std-url gate SKIP c smoke (observational; no compiler)" >&2
fi
echo "std-url c_smoke_note=${C_NOTE}"

if XLANG_BIN="$(resolve_shu 2>/dev/null)"; then
  echo "=== STD-076: smoke (XLANG=$XLANG_BIN; check observational; runnable hard) ==="
  if "$XLANG_BIN" check -L . "$SMOKE_X" >/dev/null 2>&1; then
    CHECK_OK=1
  else
    echo "std-url gate SKIP check smoke (paused 2026-08-05)" >&2
  fi
  ...
  if $RUN_XLANG build -L . "$SMOKE_X" -o "$OUT" 2>"$LOG"; then
    ...
    if [ "$exitcode" -eq "$SMOKE_EXPECT" ]; then
      ...
      echo "std-url gate FAIL runnable exit=$exitcode (expect $SMOKE_EXPECT)" >&2
      std_url_emit_report "fail" "$CHECK_OK" 0 0
      exit 1
    fi
  else
    echo "std-url gate FAIL runnable link" >&2
    tail -20 "$LOG" 2>/dev/null >&2 || true
    std_url_emit_report "fail" "$CHECK_OK" 0 0
    exit 1
  fi
else
  echo "std-url gate FAIL: no native xlang" >&2
  std_url_emit_report "fail" 0 0 0
  exit 1
fi

# check stays observational; hard-green signal is run= (runnable).
echo "std-url check_ok=${CHECK_OK} (observational)"
std_url_emit_report "ok" "$CHECK_OK" "$RUN_OK" "$SKIP"
