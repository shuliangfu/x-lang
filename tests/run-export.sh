#!/usr/bin/env bash
# run-export.sh — 模块 export / SHUX_VISIBILITY 金样
set -euo pipefail
...
function main(): i32 { return foo(); }' > /tmp/shux_export_syn.x
out="$($SHUX check /tmp/shux_export_syn.x 2>&1 || true)"
echo "$out" | grep -q 'type check failed' && fail "export syntax should typeck"
pass "export syntax"

# 2) 默认（strict）：未 export 不可跨模块
out="$($SHUX check tests/export/user_private_should_fail.x 2>&1 || true)"
...
out="$(SHUX_VISIBILITY=compat $SHUX check tests/export/user_private_should_fail.x 2>&1 || true)"
echo "$out" | grep -q 'type check failed' && fail "compat should allow private_mul"
pass "compat allows non-export"

# 4) warn：告警仍放行
out="$(SHUX_VISIBILITY=warn $SHUX check tests/export/user_private_should_fail.x 2>&1 || true)"
...
out="$(SHUX_VISIBILITY=strict $SHUX check tests/export/user_export.x 2>&1 || true)"
echo "$out" | grep -q 'type check failed' && fail "strict must allow export API"
pass "strict allows export API"

out="$($SHUX check tests/export/user_export.x 2>&1 || true)"
...
set +e
out="$($SHUX check tests/export/lint_unused_private.x 2>&1)"
ec=$?
set -e
...
set +e
out="$($SHUX check tests/export/lint_used_private.x 2>&1)"
ec=$?
set -e
...
set +e
out="$(SHUX_UNUSED_PRIVATE=0 $SHUX check tests/export/lint_unused_private.x 2>&1)"
ec=$?
set -e
[[ "$ec" -eq 0 ]] || fail "L7-off check should exit 0"
echo "$out" | grep -q "unused private function" && fail "SHUX_UNUSED_PRIVATE=0 should silence L7"
