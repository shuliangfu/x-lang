...
#   SHUX=./compiler/shux_asm ./tests/run-perf-dod-soa.sh
#   SHUX_DOD_SOA_FAIL=1 ./tests/run-perf-dod-soa.sh
set -e
cd "$(dirname "$0")/.."
...
SOA_SRC="tests/bench/dod_soa_sum_x.sx"
AOS_SRC="tests/bench/dod_aos_sum_x.sx"
F32_SOA_SRC="tests/bench/dod_f32_soa_sum_x.sx"
F32_AOS_SRC="tests/bench/dod_f32_aos_sum_x.sx"
SOA_O="/tmp/shux_dod_soa_bench.o"
AOS_O="/tmp/shux_dod_aos_bench.o"
F32_SOA_O="/tmp/shux_dod_f32_soa_bench.o"
F32_AOS_O="/tmp/shux_dod_f32_aos_bench.o"
SOA_EXE="/tmp/shux_dod_soa_bench"
AOS_EXE="/tmp/shux_dod_aos_bench"
F32_SOA_EXE="/tmp/shux_dod_f32_soa_bench"
F32_AOS_EXE="/tmp/shux_dod_f32_aos_bench"
CRT0="compiler/src/runtime/crt0_linux_x86_64.o"
N="${SHUX_DOD_BENCH_N:-4096}"
MAX_SOA_MISS_PCT="${SHUX_DOD_SOA_MAX_L1_MISS_PCT:-1.0}"
...
if ! dod_native_exe "$SHUX_BIN"; then
  if [ -n "${SHUX_CI_NO_SKIP:-}" ]; then
    echo "dod-soa FAIL: ${SHUX_BIN} not native on $(uname -s)-$(uname -m)" >&2
    exit 1
  fi
  echo "dod-soa SKIP: ${SHUX_BIN} not native (rebuild shux_asm on Linux)"
  exit 0
fi

rm -f "$SOA_O" "$AOS_O" "$F32_SOA_O" "$F32_AOS_O" "$SOA_EXE" "$AOS_EXE" "$F32_SOA_EXE" "$F32_AOS_EXE"

# 优先 shux_asm 全量链接（不依赖 crt0）；失败再回落 .o + ld。
if ! SHUX="$SHUX_BIN" "$SHUX_BIN" "$SOA_SRC" -o "$SOA_EXE" 2>/dev/null; then
  SOA_EXE=""
  if ! SHUX="$SHUX_BIN" "$SHUX_BIN" "$SOA_SRC" -o "$SOA_O"; then
    echo "dod-soa FAIL: compile $SOA_SRC" >&2
    exit 1
  fi
fi
if ! SHUX="$SHUX_BIN" "$SHUX_BIN" "$AOS_SRC" -o "$AOS_EXE" 2>/dev/null; then
  AOS_EXE=""
  if ! SHUX="$SHUX_BIN" "$SHUX_BIN" "$AOS_SRC" -o "$AOS_O"; then
...
# f32 列扫描（addss 热路径）
if ! SHUX="$SHUX_BIN" "$SHUX_BIN" "$F32_SOA_SRC" -o "$F32_SOA_EXE" 2>/dev/null; then
  F32_SOA_EXE=""
  if ! SHUX="$SHUX_BIN" "$SHUX_BIN" "$F32_SOA_SRC" -o "$F32_SOA_O"; then
    echo "dod-soa FAIL: compile $F32_SOA_SRC" >&2
    exit 1
  fi
fi
if ! SHUX="$SHUX_BIN" "$SHUX_BIN" "$F32_AOS_SRC" -o "$F32_AOS_EXE" 2>/dev/null; then
  F32_AOS_EXE=""
  if ! SHUX="$SHUX_BIN" "$SHUX_BIN" "$F32_AOS_SRC" -o "$F32_AOS_O"; then
...
# GHA / CI：SHUX_DOD_SOA_REQUIRE_L1=1 时 perf 必须可用，禁止 SKIP。
if [ "${SHUX_DOD_SOA_REQUIRE_L1:-0}" = "1" ]; then
  if [ "${SOA_MISS:-nan}" = "nan" ] || [ "${AOS_MISS:-nan}" = "nan" ]; then
    echo "dod-soa FAIL: SHUX_DOD_SOA_REQUIRE_L1=1 but perf miss rate unavailable" >&2
    exit 1
  fi
fi
