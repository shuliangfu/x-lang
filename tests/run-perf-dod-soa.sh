...
#   XLANG_DOD_SOA_FAIL=1 ./tests/run-perf-dod-soa.sh
set -e
cd "$(dirname "$0")/.."
...
if ! dod_native_exe "$XLANG_BIN"; then
  if [ -n "${XLANG_CI_NO_SKIP:-}" ]; then
    echo "dod-soa FAIL: ${XLANG_BIN} not native on $(uname -s)-$(uname -m)" >&2
    exit 1
  fi
  echo "dod-soa SKIP: ${XLANG_BIN} not native (rebuild xlang_asm on Linux)"
  exit 0
fi

rm -f "$SOA_O" "$AOS_O" "$F32_SOA_O" "$F32_AOS_O" "$SOA_EXE" "$AOS_EXE" "$F32_SOA_EXE" "$F32_AOS_EXE"

# 优先 xlang_asm 全量链接（不依赖 crt0）；失败再回落 .o + ld。
...
if ! XLANG="$XLANG_BIN" "$XLANG_BIN" "$F32_SOA_SRC" -o "$F32_SOA_EXE" 2>/dev/null; then
  F32_SOA_EXE=""
  if ! XLANG="$XLANG_BIN" "$XLANG_BIN" "$F32_SOA_SRC" -o "$F32_SOA_O"; then
...
if ! XLANG="$XLANG_BIN" "$XLANG_BIN" "$F32_AOS_SRC" -o "$F32_AOS_EXE" 2>/dev/null; then
  F32_AOS_EXE=""
  if ! XLANG="$XLANG_BIN" "$XLANG_BIN" "$F32_AOS_SRC" -o "$F32_AOS_O"; then
...
if [ -x "$F32_SOA_EXE" ] && [ -x "$F32_AOS_EXE" ]; then
  ...
  if ! dod_rc_ok "$F32_SOA_RC" || ! dod_rc_ok "$F32_AOS_RC"; then
    ...
    if [ "${XLANG_DOD_SOA_FAIL:-0}" = "1" ]; then
      exit 1
    fi
  fi
...
# GHA / CI：XLANG_DOD_SOA_REQUIRE_L1=1 时 perf 必须可用，禁止 SKIP。
if [ "${XLANG_DOD_SOA_REQUIRE_L1:-0}" = "1" ]; then
  if [ "${SOA_MISS:-nan}" = "nan" ] || [ "${AOS_MISS:-nan}" = "nan" ]; then
    echo "dod-soa FAIL: XLANG_DOD_SOA_REQUIRE_L1=1 but perf miss rate unavailable" >&2
    exit 1
  fi
fi
