...
if ! nm "$TYPECK_O" 2>/dev/null | grep -qE ' T (_)?typeck_su_ast'; then
  echo "run-wpo-typeck-reach-gate FAIL: $TYPECK_O missing typeck_su_ast export" >&2
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi
...
echo "run-wpo-typeck-reach-gate OK (typeck_su_ast+check_block/check_expr defined, exports=${EXPORTS})"
