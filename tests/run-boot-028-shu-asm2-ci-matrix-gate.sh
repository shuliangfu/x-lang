#!/usr/bin/env bash
# BOOT-028：parser C6 shu_asm2 CI 矩阵门禁
#
# 用法：./tests/run-boot-028-shu-asm2-ci-matrix-gate.sh
set -e
cd "$(dirname "$0")/.."

DOC="${SHU_BOOT028_DOC:-analysis/boot-028-shu-asm2-ci-matrix-v1.md}"
MANIFEST="${SHU_BOOT028_TSV:-tests/baseline/boot-028-shu-asm2-ci-matrix.tsv}"
WAVE="${SHU_BOOT028_WAVE_TSV:-tests/baseline/shu-asm2-ci-matrix-wave.tsv}"
PLAT_MATRIX="${SHU_BOOT028_PLAT_TSV:-tests/baseline/shu-asm2-ci-platform-matrix.tsv}"
...
LIB="tests/lib/boot-028-shu-asm2-ci-matrix.sh"
MIN_HOOKS=4
MIN_ROWS=4
MIN_LINUX_MUST=1
MIN_MACOS_MUST=1

# shellcheck source=tests/lib/boot-028-shu-asm2-ci-matrix.sh
. "$LIB"

echo "=== BOOT-028: shu_asm2 CI matrix manifest ==="
for f in "$DOC" "$MANIFEST" "$WAVE" "$PLAT_MATRIX" "$LIB" "$MATRIX" "$REPRO" "$CI_MATRIX" \
  ...
  if [ ! -f "$f" ]; then
    echo "boot-028-shu-asm2-ci-matrix gate FAIL: missing $f" >&2
    exit 1
  fi
done
...
for kw in c6_matrix_ok c6_smoke boot028_shu_asm2_ci_matrix shu_asm2_ci_target; do
  if ! grep -qF "$kw" "$DOC" 2>/dev/null; then
    echo "boot-028-shu-asm2-ci-matrix gate FAIL: doc missing '$kw'" >&2
    exit 1
  fi
done

if ! grep -qF 'boot028_shu_asm2_ci_matrix' "$REPRO" 2>/dev/null; then
  echo "boot-028-shu-asm2-ci-matrix gate FAIL: bootstrap-repro missing boot028" >&2
  exit 1
fi
...
if [ "$ROW_N" -lt "$MIN_ROWS" ]; then
  echo "boot-028-shu-asm2-ci-matrix gate FAIL: rows=${ROW_N} < min ${MIN_ROWS}" >&2
  exit 1
fi
...
if [ "$HOOK_N" -lt "$MIN_HOOKS" ]; then
  echo "boot-028-shu-asm2-ci-matrix gate FAIL: hooks=${HOOK_N} < min ${MIN_HOOKS}" >&2
  exit 1
fi
...
if ! grep -F 'C7_shu_asm2_ci_matrix' "$MATRIX" 2>/dev/null | grep -qF 'run-boot-028-shu-asm2-ci-matrix-gate.sh'; then
  echo "boot-028 FAIL: C7_shu_asm2_ci_matrix not wired to boot-028 gate" >&2
  MISS=$((MISS + 1))
fi
...
echo "boot-028-shu-asm2-ci-matrix manifest OK (rows=${ROW_N} hooks=${HOOK_N} linux_must=${LINUX_M} macos_must=${MACOS_M})"

echo "=== BOOT-028: parent BOOT-027 manifest ==="
chmod +x tests/run-boot-027-shu-asm2-cross-gate.sh
SHU_BOOT027_MANIFEST_ONLY=1 ./tests/run-boot-027-shu-asm2-cross-gate.sh

if [ "${SHU_BOOT028_MANIFEST_ONLY:-0}" = "1" ]; then
  boot028_emit_report "ok" 0 0 1
  echo "boot-028-shu-asm2-ci-matrix gate OK (manifest only)"
  exit 0
fi
...
while IFS=$'\t' read -r case_id script linux_p macos_p win_p ok_pat _notes; do
  ...
  if [ "$case_id" = "boot027_parent" ]; then
    extra_env=(SHU_BOOT027_MANIFEST_ONLY=1)
  fi
  set +e
  ...
  if [ "$case_id" = "shu_asm2_cross_smoke" ]; then
    SMOKE_LOG="$log"
...
SKIP=0
if [ -n "$SMOKE_LOG" ] && grep -qF 'shu-asm2-cross-smoke SKIP' "$SMOKE_LOG" 2>/dev/null; then
  SKIP=1
fi

boot028_emit_report "ok" "$C6_MATRIX_OK" "$C6_SMOKE" "$SKIP"
echo "boot-028-shu-asm2-ci-matrix gate OK"
