#!/usr/bin/env bash
# S3 EMIT_HEAVY 烟测：用 shu_asm 第二遍重编 pipeline.o，统计非 ret0 桩的真机码函数数。
...
OUT="/tmp/shu_s3_pipeline_emit_heavy.o"
...
if [ "${SHU_S3_FAIL_ON_EMIT_HEAVY:-0}" = "1" ]; then
  if [ "${real:-0}" -lt "${MIN_REAL}" ] 2>/dev/null; then
    ...
    echo "s3 emit-heavy hint: ast_pool.c 变更后须重编 pipeline_su.o + relink shu_asm" >&2
    exit 1
  fi
