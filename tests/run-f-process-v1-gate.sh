...
# std-process-xplat; refuse leftover ignore of explicit-bad). leftover
...
# leftover nested ensure / leftover nested std-process-xplat (refuse
...
# (do not rewrite leftover xlang_compiler_make / std-process-xplat).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
