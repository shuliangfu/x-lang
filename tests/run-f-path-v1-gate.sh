...
# path-extreme / leftover nested path-fs-windows; refuse leftover ignore
...
# leftover nested ensure / leftover nested path-extreme / leftover nested
# path-fs-windows (refuse leftover SKIP→OK / leftover ignore of
# explicit-bad / leftover XLANG fallthrough). leftover nested product
# path stays when XLANG is unset (do not rewrite leftover
# xlang_compiler_make / path-extreme / path-fs-windows).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
