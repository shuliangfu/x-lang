#!/usr/bin/env bash
# run-portable-c.sh — Tier P：全平台可移植 C 编译器回归（shu-c，无 asm/自举）。
#
# 在 ARM64/Windows 等 lite CI 上替代「仅 bootstrap-shu-gate 烟测」，覆盖 std/语言/IO 等
...
if [ ! -x ./compiler/shu-c ]; then
  echo "run-portable-c FAIL: compiler/shu-c missing" >&2
  exit 1
fi
