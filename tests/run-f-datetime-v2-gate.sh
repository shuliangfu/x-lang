...
# std-datetime / leftover nested std-datetime-timezone / leftover nested
# std-datetime-iana; refuse leftover ignore of explicit-bad). leftover
...
# leftover nested ensure / leftover nested std-datetime family (refuse leftover
# SKIP→OK / leftover ignore of explicit-bad / leftover XLANG
# fallthrough). leftover nested product path stays when XLANG is unset
# (do not rewrite leftover xlang_compiler_make / std-datetime /
# std-datetime-timezone / std-datetime-iana).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
