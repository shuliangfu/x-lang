...
# 负例：f32 初值为 none 应 typeck 失败（typeck.sx 允许 let x: f32 = 1 整型隐式转换）
if $SHUX check tests/float/init_invalid.sx 2>&1 | grep -qE "typeck|pipeline failed|cannot|mismatch|invalid"; then
  : # 预期报错，通过
else
...
# 回归：f32 = 1 整型字面量在 typeck.sx 可绿（旧负例 init_non_zero_int.sx 已弃用）
$SHUX check tests/float/init_non_zero_int.sx >/dev/null 2>&1 || {
