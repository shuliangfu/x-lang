...
# static / leftover nested std-socketio; refuse leftover ignore of
# explicit-bad). leftover auto-make of socketio.o (`xlang_compiler_make`
...
# leftover nested std-socketio (refuse leftover SKIP→OK / leftover ignore
# of explicit-bad / leftover XLANG fallthrough). leftover auto-make of
# socketio.o retired; leftover nested std-socketio stay.
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
...
if [ ! -f std/socketio/socketio.o ]; then
  die "missing std/socketio/socketio.o (refuse leftover auto-make)"
fi
ENSURE_OK=1
