...
SMOKE_SX="tests/std-elf/parse_sym_rela.sx"
...
sym_miss="$(std_elf_sym_rela_symbols_ok "$MOD_SX" "$ELF_SX" "$MANIFEST" || true)"
...
if [ -x ./compiler/shux-c ] || [ -x ./compiler/shux ]; then
  ...
  echo "std-elf-sym-rela gate SKIP c/sx smoke (need shux-c for elf.sx merge)" >&2
...
if [ -n "$SHUX_BIN" ]; then
  ...
  if ! "$SHUX_BIN" check -L . "$SMOKE_SX" >/dev/null 2>&1; then
    echo "std-elf-sym-rela gate FAIL: typeck $SMOKE_SX" >&2
    "$SHUX_BIN" check -L . "$SMOKE_SX" 2>&1 | tail -10 >&2 || true
    std_elf_sym_rela_emit_report "fail" "$SYM_C" 0 0
    exit 1
  fi
  if std_elf_sym_rela_run_sx_smoke "$SHUX_BIN" "$SMOKE_SX" "symrela"; then
    SYM_SX=1
  else
    std_elf_sym_rela_emit_report "fail" "$SYM_C" 0 0
    exit 1
...
std_elf_sym_rela_emit_report "ok" "$SYM_C" "$SYM_SX" "$SKIP"
