#!/usr/bin/env bash
# A-12：B-strict xlang_asm strict link 跨模块符号门禁（arch_* / backend 导出无 U）。
# 用法：./tests/run-a12-cross-module-symbols-gate.sh
# 环境：XLANG_A12_CROSS_MODULE_FAIL=1 存在 baseline 外 undefined 时硬失败
set -e
cd "$(dirname "$0")/.."

FAIL=${XLANG_A12_CROSS_MODULE_FAIL:-0}
...
# baseline 允许已知过渡态；默认 track-only，CI 可 XLANG_A12_CROSS_MODULE_FAIL=1 收紧。
if [ -f "$BASELINE" ]; then
