...
#   ./tests/run-g-fast-track.sh --w2-d03-only # 仅 D-03 哈希（~5s，须已有 xlang_asm_stage1/2）
#   ./tests/run-g-fast-track.sh --w3         # + W3（~1h+，Docker，慎用）
#   ./tests/run-g-fast-track.sh --w0-only    # 仅本地秒级清单
#
# 环境：
#   XLANG_FAST_TRACK_FAIL=1     任一步失败即 exit 1（默认 1）
#   XLANG_DOCKER_PERSIST=1      Docker 常驻容器（W1+ 推荐）
#   XLANG_G06_E_TIMEOUT=120     preflight --asm-e 超时（默认 W1 不跑 --asm-e）
...
LOG_DIR="${XLANG_FAST_TRACK_LOG_DIR:-/tmp/xlang-fast-track-$$}"
FAIL="${XLANG_FAST_TRACK_FAIL:-1}"

W0_ONLY=0
RUN_W2=0
RUN_W2_D03_ONLY=0
RUN_W3=0
RUN_P0="${XLANG_FAST_TRACK_P0:-0}"
...
run_step "W0 d01 manifest" "$LOG_DIR/w0-d01-manifest.log" \
  env XLANG_D01_MANIFEST_ONLY=1 ./tests/run-d01-stage0-to-stage1-gate.sh

run_step "W0 f08 core inventory" "$LOG_DIR/w0-f08.log" \
  env XLANG_F08_CORE_INVENTORY_FAIL=1 ./tests/run-f08-core-inventory-gate.sh

# E-soft 全量 manifest 在 compiler 仍含 LEGACY C 时可能红；W0 仅跑 track（不 FAIL）
run_step "W0 e-soft track" "$LOG_DIR/w0-e-soft.log" \
  env XLANG_E_SOFT_FAIL=0 XLANG_E_SOFT_MANIFEST_ONLY=1 ./tests/run-e-soft-retire-gate.sh
...
export XLANG_G06_PREFLIGHT_LOG="$LOG_DIR/w1-g06-preflight.log"
export XLANG_G06_BOOTSTRAP_LOG="$LOG_DIR/w1-g06-bootstrap.log"
...
# P0 安全 gate 依赖完整 xlang_asm/typeck 链；冷启动 stub partial 下默认跳过（--p0 或 W2+）
if [ "$RUN_P0" = "1" ]; then
  ...
  progress "SKIP W1 P0 security gates (set XLANG_FAST_TRACK_P0=1 or --p0 after bstrict)"
fi
...
run_step "W2 d01 stage0→1" "$LOG_DIR/w2-d01.log" \
  env XLANG_D01_FAIL=1 "$DOCKER" tests './run-d01-stage0-to-stage1-gate.sh'
