...
$SHU tests/asm/assign_lval_fast.su -o /tmp/shu_asm_assign_var 2>&1
exitcode=0
/tmp/shu_asm_assign_var >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 52 ] && { echo "run-asm-assign-var FAIL: expected exit 52, got $exitcode"; exit 1; }

if otool -tv /tmp/shu_asm_assign_var 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'sub.*sp, sp, #0x10'; then
