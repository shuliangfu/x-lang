...
# Report static=/ensure=/iter=/plat=/skip=.
# Honesty: leftover XLANG fallthrough (`for cand in "${XLANG:-}" …`)
# retired. Explicit-bad XLANG / missing native = hard die FIRST (before
# static / leftover nested std-env-iter / leftover nested
# std-env-platform-encoding; refuse leftover ignore of explicit-bad).
# leftover auto-make of env.o (`xlang_compiler_make` even when the leaf
# is present — try-heat/g05 raced L2) retired. leftover unused
# compiler-make.sh sourced unused after leftover auto-make retired.
# Missing leaf .o = hard die. leftover nested std-env-iter /
# leftover nested std-env-platform-encoding stay.
...
# leftover nested std-env-iter / leftover nested
# std-env-platform-encoding (refuse leftover SKIP→OK / leftover ignore
# of explicit-bad / leftover XLANG fallthrough). leftover auto-make of
# env.o retired; leftover nested std-env-iter / leftover nested
# std-env-platform-encoding stay.
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
...
if [ ! -f std/env/env.o ]; then
  die "missing std/env/env.o (refuse leftover auto-make)"
fi
ENSURE_OK=1
