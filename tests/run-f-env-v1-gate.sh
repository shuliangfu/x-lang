...
# std-env-iter / leftover nested std-env-platform-encoding; refuse
...
# leftover nested ensure / leftover nested std-env-iter / leftover
# nested std-env-platform-encoding (refuse leftover SKIP→OK / leftover
...
# xlang_compiler_make / std-env-iter / std-env-platform-encoding).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
