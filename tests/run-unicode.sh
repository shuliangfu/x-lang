#!/usr/bin/env bash
# std.unicode smoke: product -o exit 0
#
...
PREFIX="${XLANG_UNICODE_PREFIX:-xlang: [UNICODE]}"
...
echo "=== unicode gate (prefer asm; hard) ==="
...
log="/tmp/xlang_unicode_$$.log"
rm -f "$exe" "$log"
[ -f "tests/unicode/main.x" ] || die "missing tests/unicode/main.x"
set +e
gate_run_timeout "$XLANG_CASE_TIMEOUT" "$XLANG_BIN" -L . "tests/unicode/main.x" -o "$exe" >"$log" 2>&1
o_ec=$?
set -e
if [ "$o_ec" -eq 124 ]; then
