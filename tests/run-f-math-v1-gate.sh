...
# static=/ensure=/special=/fenv=/skip=.
# Honesty: leftover XLANG fallthrough (`for cand in "${XLANG:-}" …`)
# retired. Explicit-bad XLANG / missing native = hard die FIRST (before
# static / leftover nested observational std-math-special / leftover nested
# observational std-math-fenv; refuse leftover ignore of explicit-bad).
# leftover auto-make of math.o (`xlang_compiler_make` even when the leaf
# is present — try-heat/g05 raced L2) retired. leftover unused
# compiler-make.sh sourced unused after leftover auto-make retired.
# Missing leaf .o = hard die. leftover nested observational
# std-math-special / leftover nested observational std-math-fenv stay.
...
# leftover nested observational std-math-special / leftover nested
# observational std-math-fenv (refuse leftover SKIP→OK / leftover
# ignore of explicit-bad / leftover XLANG fallthrough). leftover
# auto-make of math.o retired; leftover nested observational
# std-math-special / leftover nested observational std-math-fenv stay.
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
...
if [ ! -f std/math/math.o ]; then
  die "missing std/math/math.o (refuse leftover auto-make)"
fi
ENSURE_OK=1
