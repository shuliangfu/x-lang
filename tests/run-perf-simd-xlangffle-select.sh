...
X_SRC="bench/simd_shuffle_select.x"
STUB_SRC="bench/simd_shuffle_select_stub.c"
...
if ! simd_ss_native_exe "$XLANG_BIN"; then
  echo "simd-shuffle-select-perf SKIP: ${XLANG_BIN} not native"
  exit 0
fi
...
if ! XLANG="$XLANG_BIN" "$XLANG_BIN" build -L . "$X_SRC" -o "$X_O"; then
  echo "simd-shuffle-select-perf FAIL: compile $X_SRC" >&2
  exit 1
fi
if ! cc -O2 -o "$X_EXE" "$X_O" -lm 2>/dev/null; then
  if ! XLANG="$XLANG_BIN" "$XLANG_BIN" build -L . "$X_SRC" -o "$X_EXE"; then
...
if awk -v r="$RATIO" -v m="$MIN_RATIO" 'BEGIN { exit (r + 0.000001 >= m) ? 0 : 1 }'; then
  ...
  if [ "${XLANG_SIMD_SS_FAIL:-0}" = "1" ]; then
    exit 1
  fi
fi
