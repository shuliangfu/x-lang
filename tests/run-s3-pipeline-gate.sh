...
# 可选：SHUX_S3_UPDATE_BASELINE=1 — 更新 tests/baseline/s3-pipeline-o.tsv
set -e
cd "$(dirname "$0")/.."
...
if [ ! -f "$PIPELINE_O" ]; then
  if [ "${SHUX_S3_REQUIRE_PIPELINE_O:-0}" = "1" ]; then
...
# build_shux_asm CI-fast 可能留下 stub pipeline.o（__text≈1KiB）；回归门禁前同步 EMIT_HEAVY 产物。
STUB_TEXT_MAX=8192
if { [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] || [ "${SHUX_S3_REQUIRE_PIPELINE_O:-0}" = "1" ]; } \
...
if [ "${SHUX_S3_UPDATE_BASELINE:-0}" = "1" ]; then
  {
    echo "# S3 pipeline.o：build_asm/pipeline.x 的 asm 产物 baseline"
    echo "# 更新：SHUX_S3_UPDATE_BASELINE=1 ./tests/run-s3-pipeline-gate.sh"
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] || [ "${SHUX_S3_REQUIRE_PIPELINE_O:-0}" = "1" ]; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${path256_insns:-0}" -lt 15 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${path512_insns:-0}" -lt 15 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${path_import_insns:-0}" -lt 15 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${has_dot_insns:-0}" -lt 15 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${load_insns:-0}" -lt 80 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${load_one_insns:-0}" -lt 30 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${load_sync_insns:-0}" -lt 60 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${sync_slots_insns:-0}" -lt 25 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${fill_path_insns:-0}" -lt 25 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${parse_buf_insns:-0}" -lt 25 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${parse_init_buf_insns:-0}" -lt 25 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${parse_entry_insns:-0}" -lt 20 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${typecheck_entry_insns:-0}" -lt 8 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${typeck_entry_mod_insns:-0}" -lt 8 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${typeck_after_insns:-0}" -lt 8 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${typeck_parsed_insns:-0}" -lt 8 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${lsp_typeck_load_insns:-0}" -lt 25 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${lsp_diag_insns:-0}" -lt 8 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${impl_insns:-0}" -lt 60 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${codegen_one_insns:-0}" -lt 40 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${codegen_deps_insns:-0}" -lt 30 ] 2>/dev/null; then
...
if [ "${SHUX_S3_FAIL_ON_REGRESSION:-0}" = "1" ] && [ "${codegen_entry_insns:-0}" -lt 25 ] 2>/dev/null; then
