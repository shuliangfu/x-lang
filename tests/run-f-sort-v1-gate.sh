...
# std-sort-stable-cmp / leftover nested std-sort-key-cmp; refuse leftover
...
# leftover nested ensure / leftover nested std-sort-stable-cmp /
# leftover nested std-sort-key-cmp (refuse leftover SKIP→OK /
# leftover ignore of explicit-bad / leftover XLANG fallthrough).
# leftover nested product path stays when XLANG is unset (do not
# rewrite leftover xlang_compiler_make / std-sort-stable-cmp /
# std-sort-key-cmp).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
