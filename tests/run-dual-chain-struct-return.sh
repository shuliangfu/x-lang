...
# 要求：xlang_compiler_make bootstrap-driver-seed && xlang_compiler_make bootstrap-driver-bstrict
...
for chain in seed xlang_asm; do
  if [ "$chain" = "seed" ]; then
    if [ ! -x ./compiler/xlang ]; then
      echo "run-dual-chain-struct-return: ./compiler/xlang missing (xlang_compiler_make bootstrap-driver-seed)" >&2
      exit 127
    fi
    ...
    if [ ! -x ./compiler/xlang_asm ]; then
      echo "run-dual-chain-struct-return: ./compiler/xlang_asm missing (xlang_compiler_make bootstrap-driver-bstrict)" >&2
      exit 127
    fi
