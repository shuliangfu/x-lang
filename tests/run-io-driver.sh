#!/usr/bin/env bash
# std.io.driver placeholder smoke
#
...
PREFIX="${XLANG_IO_DRIVER_PREFIX:-xlang: [IO_DRIVER]}"
...
run_case() {
  ...
  local exe="/tmp/xlang_io_driver_${tag}_$$"
  local log="/tmp/xlang_io_driver_${tag}_$$.log"
  local o_ec r_ec
...
echo "=== io_driver gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

run_case main tests/io-driver/main.x 0
ok_report
