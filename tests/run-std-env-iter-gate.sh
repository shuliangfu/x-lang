...
DOC="${SHU_STD_ENV_ITER_DOC:-analysis/std-env-iter-v1.md}"
MANIFEST="${SHU_STD_ENV_ITER_TSV:-tests/baseline/std-env-iter.tsv}"
ENV_SU="std/env/mod.su"
ENV_C="std/env/env.c"
LIB="tests/lib/std-env-iter.sh"
SMOKE="tests/env/env_iter.su"
...
if SHU_BIN="$(resolve_shu 2>/dev/null)"; then
  echo "=== STD-025: typeck (SHU=$SHU_BIN) ==="
  ...
  if $RUN_SHU -L . "$SMOKE" -o /tmp/shu_std_env_iter 2>/tmp/shu_std_env_iter_build.log; then
    exitcode=0
    /tmp/shu_std_env_iter >/dev/null 2>&1 || exitcode=$?
    ...
    tail -5 /tmp/shu_std_env_iter_build.log 2>/dev/null >&2 || true
    SKIP=1
  fi
else
  echo "std-env-iter gate SKIP typeck (no native shu)" >&2
fi
