...
XLANG_STD066_MANIFEST_ONLY=1 ./tests/run-std-sqlite-query-rows-gate.sh
...
if std_sqlite_probe_libs; then
  ...
  if [ -n "$XLANG_BIN" ]; then
    echo "=== STD-067: .x next_row smoke (XLANG=$XLANG_BIN) ==="
    if ! "$XLANG_BIN" check -L . "$SMOKE_X" >/dev/null 2>&1; then
      echo "std-sqlite-next-row gate SKIP .x smoke (typeck fail)" >&2
      SKIP=1
    elif std_sqlite_run_smoke "$XLANG_BIN" "$SMOKE_X" "cursor"; then
      CURSOR_X=1
      SKIP=0
    else
      echo "std-sqlite-next-row gate SKIP .x smoke (link/compile)" >&2
      SKIP=1
    fi
  else
    echo "std-sqlite-next-row gate SKIP .x smoke (no native xlang)" >&2
    SKIP=1
  fi
  std_sqlite_restore_default_o
else
...
std_sqlite_next_row_emit_report "ok" "$CURSOR_C" "$CURSOR_X" "$SKIP"
