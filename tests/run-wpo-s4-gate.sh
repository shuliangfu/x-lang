#!/usr/bin/env bash
# WPO-S4 PGO-Lite 门禁：S0 binary __text 代理不回归；S1 XLANG_WPO_PGO_HOT=1 时 .text.hot + .text.unlikely；S2 call-depth emit 序。
# 用法：
...
if wpo_host_asm_run_na; then
  echo "wpo-s4: asm/PGO N/A on $(uname -s)-$(uname -m) (refresh xlang_asm asm stub; x86_64 covers)"
  echo "wpo-s4 gate OK"
  exit 0
fi
...
if [ ! -x "$XLANG_ASM_ABS" ]; then
  echo "wpo-s4 gate SKIP (no xlang_asm: $XLANG_ASM_ABS)"
  exit 0
fi

chmod +x tests/run-perf-wpo-dce-xlang-asm-text.sh tests/lib/wpo-ab-proxy.sh
XLANG="$XLANG_ASM" ./tests/run-perf-wpo-dce-xlang-asm-text.sh

# S4-S1：编译用户程序并断言 .text.hot 段（非 xlang_asm 自身）
if [ -n "${XLANG_WPO_PGO_HOT:-}" ]; then
  ...
  if ! command -v readelf >/dev/null 2>&1; then
    ...
    rm -f "$PGO_O"
    if ! XLANG_WPO_PGO_HOT=1 XLANG="$XLANG_ASM" "$XLANG_ASM_ABS" "$PGO_SRC" -o "$PGO_O"; then
      echo "wpo-s4 FAIL: compile $PGO_SRC with XLANG_WPO_PGO_HOT=1" >&2
      exit 1
    fi
    if ! readelf -S "$PGO_O" 2>/dev/null | grep -q '\.text\.hot'; then
      echo "wpo-s4 FAIL: no .text.hot in $PGO_O (XLANG_WPO_PGO_HOT=1)" >&2
      exit 1
    fi
    UNLIKELY_LINE="$(readelf -S "$PGO_O" 2>/dev/null | grep '\.text\.unlikely' || true)"
    if [ -z "$UNLIKELY_LINE" ]; then
      echo "wpo-s4 FAIL: no .text.unlikely in $PGO_O (XLANG_WPO_PGO_HOT=1)" >&2
      exit 1
    fi
