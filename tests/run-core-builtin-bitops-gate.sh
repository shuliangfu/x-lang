#!/usr/bin/env bash
# CORE-009：core.builtin clz/ctz/popcount 内建门禁
#
...
for f in "$DOC" "$MANIFEST" "$LIB" "$CODEGEN" "$BUILTIN_C" "$EMIT_SU"; do
  if [ ! -f "$f" ]; then
...
c_miss="$(core_builtin_c_impl_ok "$BUILTIN_C" || true)"
if [ "${map_miss:-0}" -gt 0 ] || [ "${c_miss:-0}" -gt 0 ]; then
  core_builtin_emit_report "fail" 0 "$MIN_MAP"
  echo "core-builtin-bitops gate FAIL: mapping_miss=${map_miss:-0} c_miss=${c_miss:-0}" >&2
  exit 1
fi
