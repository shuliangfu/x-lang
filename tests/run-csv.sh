#!/usr/bin/env bash
# std.csv leftover runner: tests/csv/main.x product -o exit 0.
#
# Honesty: leftover soft auto-make (`xlang_compiler_make -q || xlang_compiler_make`
# + ensure_std_c_o csv.o) + bootstrap-link wrap (prefer-c) retired.
...
# Product `-o` tests/csv/main.x must exit 0. Report: run=/obs=/skip=
# PLATFORM: SHARED archaeology — Ubuntu gold still required.
# Usage: ./tests/run-csv.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
PREFIX="${XLANG_CSV_PREFIX:-xlang: [CSV]}"
SMOKE="tests/csv/main.x"
RUN_OK=0
OBS=0
SKIP=0
...
echo "=== csv leftover (prefer asm; hard; refuse leftover auto-make) ==="
if [ -n "${XLANG:-}" ]; then
...
set +e
"$XLANG_BIN" check -L . "$SMOKE" >/tmp/xlang_csv_check.log 2>&1
chk_ec=$?
set -e
if [ "$chk_ec" -ne 0 ]; then
  echo "csv OBS check (paused / CHK residual ec=$chk_ec; refuse soft SKIP→OK)" >&2
  OBS=$((OBS + 1))
fi
...
set +e
"$XLANG_BIN" -L . "$SMOKE" -o "$exe" >/tmp/xlang_csv_o.log 2>&1
o_ec=$?
set -e
if [ "$o_ec" -ne 0 ] || [ ! -x "$exe" ]; then
  tail -n 12 /tmp/xlang_csv_o.log 2>/dev/null || true
  rm -f "$exe"
