...
# std-url / leftover nested std-url-ipv6-host; refuse leftover ignore
...
# leftover nested ensure / leftover nested std-url family (refuse leftover
# SKIP→OK / leftover ignore of explicit-bad / leftover XLANG
# fallthrough). leftover nested product path stays when XLANG is unset
# (do not rewrite leftover xlang_compiler_make / std-url /
# std-url-ipv6-host).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
