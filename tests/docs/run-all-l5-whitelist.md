...
| run-io.sh | ✅ | ✅ | — | ✅ | F1：bstrict + run-all 白名单（M7 shu 下含 read_ptr 全绿） |
...
| run-fmt-std.sh | ✅ | ✅ | — | ✅ | F6：std.fmt 全量；修复 main.su 注释折行 |
| run-ub.sh | ✅ | ✅ | — | ✅ | F6：asm 整数除/模除零 → shulang_panic_(1,0) |
| run-pool-limits.sh | ✅ | ✅ | — | ✅ | F6：grow 池边界（嵌套 while / many_locals / many_funcs 等） |

**L5 本轮**：run-all 白名单 **100 项**（与 `run-all-bstrict.sh` 对齐）；`run-all-bstrict.sh` **100 项**。
...
验收：`SHU=./compiler/shu_asm ./tests/run-asm-73-gate.sh`；改 `pipeline_glue.c` / `ast_pool_bootstrap_glue.c` 后须 `touch ast_pool.c && make -C compiler relink-shu` 并 `cp shu shu_asm`。

### F6：永久 shu-c 脚本

`SHULANG_BSTRICT_RUN_ALL=1` 下 **无** 永久 shu-c 脚本（`run-pool-limits.sh` 已于 2026-05 迁入 shu_asm 白名单）。

验收：`SHULANG_BSTRICT_RUN_ALL=1 SHU=./compiler/shu_asm CI=1 ./tests/run-all.sh` — 白名单全绿。
