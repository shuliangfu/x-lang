...
#
# 启用：SHUX_SOCKETIO_NPM=1 ./tests/run-std-socketio-npm-live.sh
set -e
cd "$(dirname "$0")/.."
...
if [ "${SHUX_SOCKETIO_NPM:-0}" != "1" ]; then
  echo "std-socketio npm live SKIP (set SHUX_SOCKETIO_NPM=1 to enable)"
  exit 0
fi
...
if SHUX_BIN="$(stdlib_cm_native_shu ./compiler/shux-c && echo ./compiler/shux-c || true)"; then
  ...
  echo "std-socketio npm live SKIP (no native shux)" >&2
  exit 0
fi
...
if ! (cd "$NPM_DIR" && npm install --no-audit --no-fund >/tmp/shux_socketio_npm_install_$$.log 2>&1); then
  echo "std-socketio npm live FAIL: npm install" >&2
  tail -15 /tmp/shux_socketio_npm_install_$$.log >&2 || true
  exit 1
fi
...
PORT="$PORT" node "$NPM_DIR/server.mjs" >/tmp/shux_socketio_npm_svr_$$.log 2>&1 &
SVR_PID=$!
sleep 1.5
...
if ! "$SHUX_BIN" -L . "$LIVE_X" -o "$exe" 2>/tmp/shux_socketio_npm_link_$$.log; then
...
if ! "$exe"; then
  ...
  tail -5 /tmp/shux_socketio_npm_svr_$$.log >&2 || true
  exit 1
fi
