...
TYPECK_TO="${XLANG_S7_TYPECK_TIMEOUT:-90}"
...
for m in "${REQUIRED[@]}"; do
  ...
  LAST_BIN="$bin"
  gate_progress "OK typeck $m (XLANG=$bin)"
done

# 可选模块：优先 xlang-c typeck（C 解析器已修 ?/: 与 MAX_TOP_LEVEL_LETS）；失败再 stage1。
if [ "${XLANG_S7_OPTIONAL_SKIP:-0}" != "1" ]; then
for m in "${OPTIONAL[@]}"; do
  [ -f "$m" ] || { gate_progress "WARN: missing $m"; WARN=$((WARN + 1)); continue; }
  gate_progress "S7 可选: typeck $m (stage1/xlang 回退, ${TYPECK_TO}s) ..."
  set +e
  bin="$(p0_gate_run_typeck "$m")"
  ec=$?
  set -e
...
gate_progress "std-harddeps gate OK (4 modules, XLANG=$LAST_BIN)"
