#!/usr/bin/env bash
# B-16 v1：macOS std.sys 文件 MAP_SHARED mmap 烟测（Darwin 常规链接，无 mmap.inc.c）。
# 用法：./tests/run-macos-mmap-file-gate.sh
# 环境：SHUX_MACOS_MMAP_FILE_FAIL=1 失败时硬退出
set -e
cd "$(dirname "$0")/.."

FAIL=${SHUX_MACOS_MMAP_FILE_FAIL:-0}
SU="tests/sys/macos_mmap_file_smoke.sx"
OUT="/tmp/shux_macos_mmap_file.$$.out"
GATE_FILE="/tmp/shux_macos_mmap_file_gate.dat"
SHUX="${SHUX:-./compiler/shux-c}"

if [ "$(uname -s 2>/dev/null)" != "Darwin" ]; then
  echo "macos-mmap-file-gate: N/A (Darwin only)"
  exit 0
fi

if [ ! -x "$SHUX" ]; then
  SHUX="./compiler/shux"
fi
if [ ! -x "$SHUX" ]; then
  echo "macos-mmap-file-gate: SKIP (no shux/shux-c)"
  exit 0
fi

: >"$GATE_FILE"
rm -f "$OUT" 2>/dev/null || true

if ! "$SHUX" -o "$OUT" "$SU" 2>/tmp/shux_macos_mmap_file.log; then
  echo "macos-mmap-file-gate FAIL: compile $SU" >&2
  tail -n 10 /tmp/shux_macos_mmap_file.log 2>/dev/null || true
  rm -f "$OUT" "$GATE_FILE" 2>/dev/null || true
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi

if [ ! -x "$OUT" ]; then
  echo "macos-mmap-file-gate FAIL: no executable $OUT" >&2
  rm -f "$GATE_FILE" 2>/dev/null || true
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi

rc=0
"$OUT" || rc=$?
rm -f "$OUT" "$GATE_FILE" 2>/dev/null || true

if [ "$rc" -ne 0 ]; then
  echo "macos-mmap-file-gate FAIL: expected exit 0, got $rc" >&2
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi

echo "macos-mmap-file-gate OK (macOS MAP_SHARED os_mmap_rw via std.sys.macos)"
exit 0
