#!/usr/bin/env bash
# enum minimal/simple/match smoke
#
...
PREFIX="${XLANG_ENUM_PREFIX:-xlang: [ENUM]}"
...
echo "=== enum gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

run_case minimal tests/enum/minimal.x 0
run_case simple tests/enum/simple.x 1
run_case match tests/enum/match.x 1
ok_report
echo "enum test OK"
