#!/usr/bin/env bash
# SysV f32 xmm 实/形参 ABI 门禁（默认开启；SHU_ABI_F32_XMM=0 时 SKIP legacy）。
# 用法：
#   SHU=./compiler/shu_asm ./tests/run-abi-f32-xmm-gate.sh
#   SHU_ABI_F32_XMM=0 …  # SKIP
set -e
cd "$(dirname "$0")/.."
...
if [ "${SHU_ABI_F32_XMM:-1}" = "0" ]; then
  echo "abi-f32-xmm SKIP (SHU_ABI_F32_XMM=0 legacy path)"
  exit 0
fi

if ! dod_native_exe "$SHU_BIN"; then
  echo "abi-f32-xmm SKIP (no native shu/shu_asm)"
  exit 0
fi
...
abi_f32_xmm_run_smoke() {
  ...
  local tag="$2"
  local want_rc="$3"
  local obj="/tmp/shu_abi_f32_xmm_${tag}.o"
  local bin="/tmp/shu_abi_f32_xmm_${tag}"
  local rc=0

  rm -f "$obj" "$bin"
  if ! SHU="$SHU_BIN" "$SHU_BIN" "$src" -o "$bin" 2>/dev/null; then
    if ! SHU="$SHU_BIN" "$SHU_BIN" "$src" -o "$obj"; then
...
abi_f32_xmm_run_smoke "tests/abi/f32_call_xmm_smoke.su" "pure_f32" 6
abi_f32_xmm_run_smoke "tests/abi/f32_xmm_mixed_call_smoke.su" "mixed_ptr_f32" 6
abi_f32_xmm_run_smoke "tests/abi/f32_xmm_mixed_field_read_smoke.su" "mixed_field_read" 6
abi_f32_xmm_run_smoke "tests/abi/f32_tri_field_read_smoke.su" "tri_field_read" 6

if command -v objdump >/dev/null 2>&1; then
  for tag in mixed_ptr_f32 mixed_field_read; do
    DIS="$(objdump -d "/tmp/shu_abi_f32_xmm_${tag}" 2>/dev/null || true)"
    ...
    if echo "$DIS" | grep -q 'cvtsd2ss'; then
      echo "abi-f32-xmm FAIL: ${tag} disasm still has cvtsd2ss (legacy widen under SHU_ABI_F32_XMM=1)" >&2
      exit 1
    fi
  done
fi

# CLI -legacy-f32-abi 须等价 SHU_ABI_F32_XMM=0（功能仍 exit=6；disasm 含 cvtsd2ss）。
echo "=== ABI f32 xmm: CLI -legacy-f32-abi ==="
CLI_BIN="/tmp/shu_abi_f32_xmm_cli_legacy"
CLI_OBJ="/tmp/shu_abi_f32_xmm_cli_legacy.o"
rm -f "$CLI_BIN" "$CLI_OBJ"
if ! SHU="$SHU_BIN" "$SHU_BIN" -backend asm -L . -legacy-f32-abi tests/abi/f32_call_xmm_smoke.su -o "$CLI_BIN" 2>/dev/null; then
  if ! SHU="$SHU_BIN" "$SHU_BIN" -backend asm -L . -legacy-f32-abi tests/abi/f32_call_xmm_smoke.su -o "$CLI_OBJ"; then
