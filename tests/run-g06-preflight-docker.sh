...
# 另开终端看日志：tail -f /tmp/xlang-g06-preflight.log
# 高频：XLANG_DOCKER_PERSIST=1 ./tests/run-g06-preflight-docker.sh --asm-e
...
TIMEOUT="${XLANG_G06_E_TIMEOUT:-300}"
LOG="${XLANG_G06_PREFLIGHT_LOG:-/tmp/xlang-g06-preflight.log}"
INNER="cp -f seeds/bootstrap_xlangc.linux.x86_64 bootstrap_xlangc && chmod +x bootstrap_xlangc
ulimit -s 65532 2>/dev/null || true
XLANG_G06_E_TIMEOUT=${TIMEOUT} XLANG_E=./bootstrap_xlangc ./scripts/preflight_g06_coldstart.sh ${ASM_FLAG}"

progress "run-g06-preflight-docker: START timeout=${TIMEOUT}s persist=${XLANG_DOCKER_PERSIST:-0}"
