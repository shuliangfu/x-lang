...
#   XLANG_WPO_DRIVER_O_FAIL=1 ./tests/run-wpo-driver-o-gate.sh
set -e
cd "$(dirname "$0")/.."
...
BASELINE="${XLANG_WPO_DRIVER_O_BASELINE:-tests/baseline/wpo-driver-o.tsv}"
MAX_TEXT=$(awk -F'\t' '$1=="driver_o_max_text_bytes" && $1 !~ /^#/ { print $2; exit }' "$BASELINE")
MAX_TEXT=${MAX_TEXT:-768}
FAIL=${XLANG_WPO_DRIVER_O_FAIL:-1}
