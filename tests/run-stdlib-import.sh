...
# 依赖：compiler/shux 或 shux-c、tests/stdlib-import/main.x、core/*.x、std/*.x（-L .）。
...
# bootstrap seed：typeck 走 .x pipeline（check）；多模块可执行链接仍用 shux-c（seed -o 常 cc failed）。
if [ -n "${SHUX_RUN_ALL_BOOTSTRAP_SHUX:-}" ]; then
  # deno check 语义：成功时静默（exit 0、无输出）；与 tests/run-check.sh 一致。
  chk_out=$($SHUX check -L . tests/stdlib-import/main.x 2>&1) || chk_rc=$?
  chk_rc=${chk_rc:-0}
  if [ "$chk_rc" -ne 0 ]; then
  ...
  out=$($CHECK_SHUX -L . tests/stdlib-import/main.x 2>&1)
