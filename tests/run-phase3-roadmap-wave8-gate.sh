...
DOC="${XLANG_PLAN008_DOC:-analysis/phase3-roadmap-wave8-v1.md}"
...
if ! grep -qF "### 2.19" "$NEXT_MD" 2>/dev/null; then
  echo "phase3-roadmap-wave8 gate FAIL: NEXT.md missing §2.19" >&2
  phase3_roadmap_wave8_emit_report "fail" 0 0 0
  exit 1
fi

sym_miss="$(phase3_roadmap_wave8_check "$MANIFEST" "$NEXT_MD" || true)"
task_n="$(phase3_roadmap_wave8_task_count "$MANIFEST")"
done_n="$(phase3_roadmap_wave8_done_count "$NEXT_MD")"
