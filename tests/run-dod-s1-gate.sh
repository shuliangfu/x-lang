...
#   XLANG=./compiler/xlang_asm ./tests/run-dod-s1-gate.sh
set -e
cd "$(dirname "$0")/.."
...
if [ -z "$XLANG_ABS" ] || ! dod_native_exe "$XLANG_ABS"; then
  echo "dod-s1 gate SKIP (no native xlang/xlang_asm; rebuild on Linux for run test)"
  exit 0
fi
...
if ! XLANG="$XLANG_ABS" "$XLANG_ABS" "$ATTR_SRC" -o "$ATTR_O"; then
  echo "dod-s1 FAIL: compile $ATTR_SRC" >&2
  exit 1
fi

# f32：Darwin / Linux ARM64 上 xlang_asm 默认 asm ELF 不可用；-backend c f32 仍 WIP，仅跳过 .o 烟测。
if [ -n "$DOD_F32_BACKEND_ARGS" ]; then
  echo "dod-s1: skip f32 .o compile (asm f32 ELF N/A on $(uname -s)-$(uname -m); f32 exe run N/A below)"
else
  if ! XLANG="$XLANG_ABS" "$XLANG_ABS" "$F32_SRC" -o "$F32_O"; then
    echo "dod-s1 FAIL: compile $F32_SRC" >&2
    exit 1
  fi

  if ! XLANG="$XLANG_ABS" "$XLANG_ABS" "$F32_AOS_LIT_SRC" -o "$F32_AOS_LIT_O"; then
...
# 链接并运行：Darwin/ARM64 lite 用 xlang-c；Linux x86_64 用 xlang_asm -backend asm。
