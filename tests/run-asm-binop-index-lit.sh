...
$SHU tests/asm/binop_index_lit_fast.su -o /tmp/shu_asm_binop_index_lit 2>&1
exitcode=0
/tmp/shu_asm_binop_index_lit >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 75 ] && { echo "run-asm-binop-index-lit FAIL: expected exit 75, got $exitcode"; exit 1; }

if otool -tv /tmp/shu_asm_binop_index_lit 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'mov x2'; then
