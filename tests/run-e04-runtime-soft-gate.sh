...
#   XLANG_E04_MANIFEST_ONLY=1     — 仅 manifest
set -e
cd "$(dirname "$0")/.."
...
grep -q 'xlang_collect_deps_transitive' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_collect_deps_transitive"
grep -q 'xlang_merge_direct_then_transitive_deps' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_merge_direct_then_transitive_deps"
grep -q 'xlang_load_direct_imports_for_asm_layout' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_load_direct_imports_for_asm_layout"
grep -q 'xlang_collect_deps_transitive' "$PIPELINE_ABI_H" || die "runtime_pipeline_abi.h missing xlang_collect_deps_transitive"
...
grep -q 'xlang_collect_deps_transitive' "$RUNTIME" || die "runtime.c should call xlang_collect_deps_transitive"
...
grep -q 'xlang_preprocess' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_preprocess"
grep -q 'xlang_preprocess' "$PIPELINE_ABI_H" || die "runtime_pipeline_abi.h missing xlang_preprocess"
...
grep -q 'XLANG_RUNTIME_PREPROCESS' "$RUNTIME" || die "runtime.c should use XLANG_RUNTIME_PREPROCESS for preprocess calls"
...
grep -q 'xlang_import_dep_dir_from_path' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_import_dep_dir_from_path"
...
grep -q 'xlang_pipeline_run_x_pipeline_large_stack' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_pipeline_run_x_pipeline_large_stack"
grep -q 'xlang_pipeline_dep_prerun_parse_only' "$PIPELINE_ABI_H" || die "runtime_pipeline_abi.h missing xlang_pipeline_dep_prerun_parse_only"
...
grep -q 'xlang_pipeline_run_x_pipeline_large_stack' "$RUNTIME" || die "runtime.c should call xlang_pipeline_run_x_pipeline_large_stack"
...
grep -q 'xlang_preprocess_raw_to_malloc' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_preprocess_raw_to_malloc"
...
grep -q 'xlang_preprocess_raw_to_malloc' "$RUNTIME" || die "runtime.c should call xlang_preprocess_raw_to_malloc"
...
grep -q 'xlang_pipeline_fill_ctx_path_buffers' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_pipeline_fill_ctx_path_buffers"
grep -q 'xlang_pipeline_pctx_seed_dep_slots' "$PIPELINE_ABI_H" || die "runtime_pipeline_abi.h missing xlang_pipeline_pctx_seed_dep_slots"
grep -q 'xlang_asm_user_std_dep_skip_x_typeck' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_asm_user_std_dep_skip_x_typeck"
...
grep -q 'xlang_pipeline_fill_ctx_path_buffers' "$RUNTIME" || die "runtime.c should call xlang_pipeline_fill_ctx_path_buffers"
...
grep -q 'xlang_find_loaded_import_index' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_find_loaded_import_index"
grep -q 'xlang_dep_prerun_entry_dir' "$PIPELINE_ABI_H" || die "runtime_pipeline_abi.h missing xlang_dep_prerun_entry_dir"
grep -q 'xlang_asm_out_buf_is_object' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_asm_out_buf_is_object"
...
grep -q 'xlang_find_loaded_import_index' "$RUNTIME" || die "runtime.c should call xlang_find_loaded_import_index"
...
grep -q 'XLANG_DRIVER_DEP_SLOT_MAX' "$PIPELINE_ABI_H" || die "runtime_pipeline_abi.h missing XLANG_DRIVER_DEP_SLOT_MAX"
...
grep -q 'xlang_resolve_import_file_path_multi' "$PIPELINE_ABI_C" || die "runtime_pipeline_abi.from_x.c missing xlang_resolve_import_file_path_multi"
grep -q 'xlang_get_entry_dir' "$PIPELINE_ABI_H" || die "runtime_pipeline_abi.h missing xlang_get_entry_dir"
...
grep -q 'xlang_resolve_import_file_path_multi' "$RUNTIME" || die "runtime.c should call xlang_resolve_import_file_path_multi"
grep -q 'xlang_get_entry_dir' "$RUNTIME" || die "runtime.c should call xlang_get_entry_dir"
grep -q 'ensure_runtime_pipeline_abi_obj' "$BUILD" || die "build_xlang_asm.sh missing ensure_runtime_pipeline_abi_obj"
...
grep -q 'xlang_invoke_ld_for_exe' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_invoke_ld_for_exe"
grep -q 'xlang_output_want_exe' "$LINK_ABI_H" || die "runtime_link_abi.h missing xlang_output_want_exe"
grep -q 'xlang_output_is_elf_o' "$LINK_ABI_H" || die "runtime_link_abi.h missing xlang_output_is_elf_o"
...
grep -q 'xlang_invoke_ld_for_exe' "$RUNTIME" || die "runtime.c should call xlang_invoke_ld_for_exe"
grep -q 'xlang_output_want_exe' "$RUNTIME" || die "runtime.c should call xlang_output_want_exe"
...
grep -q 'XLANG_IS_WIN_HOST' "$MF" || die "Makefile missing XLANG_IS_WIN_HOST"
...
grep -q 'XLANG_LEGACY_MAIN_C' "$MF" || die "Makefile missing XLANG_LEGACY_MAIN_C"
...
grep -q 'xlang_forward_main_to_main_entry' "$ABI_C" || die "runtime_abi.c missing xlang_forward_main_to_main_entry"
grep -q 'xlang_forward_main_to_main_entry' "$MAIN" || die "main.c should call xlang_forward_main_to_main_entry"
...
grep -q 'xlang_invoke_cc' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_invoke_cc"
grep -q 'xlang_generated_c_needs_async_scheduler' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_generated_c_needs_async_scheduler"
grep -q 'XLANG_INVOKE_CC_MAX_C_FILES' "$LINK_ABI_H" || die "runtime_link_abi.h missing XLANG_INVOKE_CC_MAX_C_FILES"
if grep -qE '^static int invoke_cc\(' "$RUNTIME" 2>/dev/null; then
  die "runtime.c still defines invoke_cc"
fi
grep -q 'xlang_invoke_cc(' "$RUNTIME" || die "runtime.c should call xlang_invoke_cc"

echo "=== E-04 v16: retire get_std_*_o_path; invoke_cc uses xlang_rel_o_path_from_argv0 ==="
...
grep -q 'xlang_asm_invoke_ld_platform' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_asm_invoke_ld_platform"
...
grep -q 'XLANG_LD_ARGV_CAP' "$LINK_ABI_H" || die "runtime_link_abi.h missing XLANG_LD_ARGV_CAP"
grep -q 'xlang_asm_ld_prepare_for_exe_link' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_asm_ld_prepare_for_exe_link"
grep -q 'xlang_asm_user_o_has_undef_syms' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_asm_user_o_has_undef_syms"
grep -q 'xlang_asm_ld_append_mach_tail_libs' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_asm_ld_append_mach_tail_libs"
grep -q 'xlang_asm_ld_append_unix_gcc_tail_libs' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_asm_ld_append_unix_gcc_tail_libs"
...
if grep -q '#define XLANG_LD_ARGV_CAP' "$RUNTIME" 2>/dev/null; then
  die "runtime.c still defines XLANG_LD_ARGV_CAP (expected runtime_link_abi.h)"
fi
...
grep -q 'xlang_rel_o_path_from_argv0' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_rel_o_path_from_argv0"
grep -q 'xlang_asm_ld_append_std_objs' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_asm_ld_append_std_objs"
grep -q 'xlang_asm_ld_append_on_demand_user_objs' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_asm_ld_append_on_demand_user_objs"
...
grep -q 'xlang_asm_ld_bank_push' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_asm_ld_bank_push"
grep -q 'xlang_asm_ld_try_under_lib_roots' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_asm_ld_try_under_lib_roots"
grep -q 'xlang_std_io_o_path' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_std_io_o_path"
grep -q 'xlang_repo_root_from_argv0' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_repo_root_from_argv0"
...
if grep -qE '^static const char \*xlang_asm_ld_(bank_push|try_under_lib_roots)\(' "$RUNTIME" 2>/dev/null; then
  die "runtime.c still defines xlang_asm_ld_bank_push or try_under_lib_roots"
fi
...
grep -q 'xlang_link_freestanding_enabled' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_link_freestanding_enabled"
grep -q 'xlang_ensure_runtime_asm_io_stubs_o' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_ensure_runtime_asm_io_stubs_o"
grep -q 'xlang_ensure_crt0_user_o' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_ensure_crt0_user_o"
grep -q 'xlang_ensure_freestanding_io_o' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_ensure_freestanding_io_o"
...
grep -q 'xlang_crt0_user_o_path' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_crt0_user_o_path"
grep -q 'xlang_freestanding_io_o_path' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_freestanding_io_o_path"
grep -q 'xlang_runtime_asm_io_stubs_o_path' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_runtime_asm_io_stubs_o_path"
grep -q 'xlang_ensure_runtime_panic_o' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_ensure_runtime_panic_o"
...
grep -q 'xlang_runtime_panic_o_path' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_runtime_panic_o_path"
grep -q 'xlang_std_async_scheduler_o_path' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_std_async_scheduler_o_path"
...
grep -q 'xlang_append_linux_link_harden' "$LINK_ABI_C" || die "runtime_link_abi.from_x.c missing xlang_append_linux_link_harden"
if grep -q '^static void xlang_append_linux_link_harden(' "$RUNTIME" 2>/dev/null; then
  die "runtime.c still defines xlang_append_linux_link_harden (expected runtime_link_abi.inc)"
fi
...
if grep -q '^static void asm_ld_append_compress_libs(' "$RUNTIME" 2>/dev/null; then
  die "runtime.c still defines asm_ld_append_compress_libs (expected runtime_link_abi.inc)"
fi
...
grep -q 'ensure_runtime_link_abi_obj' "$BUILD" || die "build_xlang_asm missing ensure_runtime_link_abi_obj"
...
if grep -q '^static int invoke_cc_argv_push_existing(' "$RUNTIME" 2>/dev/null; then
  die "runtime.c still defines invoke_cc_argv_push_existing (expected runtime_link_abi.inc)"
fi
...
echo "e04 track: main.c only XLANG_LEGACY_MAIN_C or xlang-c OBJS_CORE (by design; not E blocker)"
echo "e04 track: runtime.c active as driver glue + xlang-c (E-04 CLOSED at v35; optional split defer)"
echo "e04 track: OBJS_CORE C frontend for xlang-c cold start only (E-03 track; defer to F)"
