...
function main(): i32 {
  ...
  let blen: i32 = path.basename(&win_path[0], 16, &base[0], 16);
  if (blen != 8) { return 4; }
  if (base[0] != 102 || base[1] != 105 || base[2] != 108 || base[3] != 101) { return 5; }
  ...
  let jlen: i32 = path.join(&out[0], 16, &a[0], 7, &b[0], 5);
  if (jlen != 13) { return 7; }
  if (path.is_sep(out[2]) == 0) { return 8; }
  return 0;
}
