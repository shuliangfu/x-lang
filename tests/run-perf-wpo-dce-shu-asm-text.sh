#!/usr/bin/env bash
# S5：shu_asm 全二进制 .text dogfood（实测 + wpo-eligible 模块代理反推 WPO-off 体积）。
...
#   SHU_PERF_UPDATE_BASELINE=1 ./tests/run-perf-wpo-dce-shu-asm-text.sh
set -e
cd "$(dirname "$0")/.."
...
BASELINE="${SHU_WPO_SHU_ASM_TEXT_BASELINE:-tests/baseline/wpo-dce-shu-asm-text.tsv}"
MAIN_TIMEOUT="${SHU_WPO_MAIN_ASM_TIMEOUT:-180}"
FAIL_REGRESS=0
UPDATE_BASELINE=0
[ "${SHU_PERF_FAIL_ON_WPO_SHU_ASM_TEXT:-0}" = "1" ] && FAIL_REGRESS=1
...
TEXT_ON=$(text_bytes "$SHU_ASM_ABS") || {
  echo "wpo shu_asm text FAIL: cannot read .text from $SHU_ASM_ABS" >&2
  exit 1
}
echo "shu_asm binary: .text=${TEXT_ON}B ($SHU_ASM_ABS)"
...
if [ -n "$main_fast" ]; then
  MOFF="${main_fast%% *}"
  MON="${main_fast#* }"
  echo "wpo shu_asm text: main A/B fast-path (__text=${MON}B proxy off=${MOFF}B)"
fi
drv_fast=$(wpo_ab_try_driver_fast "$BUILD_ASM/driver_compile.o" "$BASELINE_PROXY" 768) || drv_fast=""
if [ -n "$drv_fast" ]; then
  DOFF="${drv_fast%% *}"
  DON="${drv_fast#* }"
  echo "wpo shu_asm text: driver A/B fast-path (__text=${DON}B proxy off=${DOFF}B)"
fi
...
if [ -n "$pipe_fast" ]; then
  POFF="${pipe_fast%% *}"
  PON="${pipe_fast#* }"
  echo "wpo shu_asm text: pipeline A/B fast-path ($pipe_tree __text=${PON}B proxy off=${POFF}B)"
fi
...
if [ -n "$tck_fast" ]; then
  TOFF="${tck_fast%% *}"
  TON="${tck_fast#* }"
  echo "wpo shu_asm text: typeck A/B fast-path ($tck_tree __text=${TON}B proxy off=${TOFF}B)"
fi
...
if [ -n "$be_fast" ]; then
  BOFF="${be_fast%% *}"
  BON="${be_fast#* }"
  echo "wpo shu_asm text: backend A/B fast-path ($be_tree __text=${BON}B proxy off=${BOFF}B)"
fi

MAIN_OFF="/tmp/shu_wpo_shu_asm_main_off.o"
MAIN_ON="/tmp/shu_wpo_shu_asm_main_on.o"
DRIVER_OFF="/tmp/shu_wpo_shu_asm_driver_off.o"
DRIVER_ON="/tmp/shu_wpo_shu_asm_driver_on.o"
PIPE_OFF="/tmp/shu_wpo_shu_asm_pipe_off.o"
PIPE_ON="/tmp/shu_wpo_shu_asm_pipe_on.o"
TCK_OFF="/tmp/shu_wpo_shu_asm_tck_off.o"
TCK_ON="/tmp/shu_wpo_shu_asm_tck_on.o"
BE_OFF="/tmp/shu_wpo_shu_asm_be_off.o"
BE_ON="/tmp/shu_wpo_shu_asm_be_on.o"

compile_main_ab() {
  rm -f "$MAIN_OFF" "$MAIN_ON"
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=0 SHU_ASM_WPO_DCE=0 \
      "$SHU_ASM_ABS" -backend asm -o "$MAIN_OFF" src/main.su >/dev/null 2>&1 ) || return 1
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=0 SHU_ASM_WPO_DCE=1 \
      "$SHU_ASM_ABS" -backend asm -o "$MAIN_ON" src/main.su >/dev/null 2>&1 ) || return 1
...
compile_driver_ab() {
  ...
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=1 SHU_ASM_WPO_DCE=0 \
      "$SHU_ASM_ABS" -backend asm -o "$DRIVER_OFF" src/driver/compile.su >/dev/null 2>&1 ) || return 1
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=1 SHU_ASM_WPO_DCE=1 \
      "$SHU_ASM_ABS" -backend asm -o "$DRIVER_ON" src/driver/compile.su >/dev/null 2>&1 ) || return 1
...
compile_pipeline_ab() {
  ...
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=1 SHU_ASM_WPO_DCE=0 \
      "$SHU_ASM_ABS" -backend asm -o "$PIPE_OFF" src/pipeline/pipeline.su >/dev/null 2>&1 ) || return 1
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=1 SHU_ASM_WPO_DCE=1 \
      "$SHU_ASM_ABS" -backend asm -o "$PIPE_ON" src/pipeline/pipeline.su >/dev/null 2>&1 ) || return 1
...
compile_typeck_ab() {
  ...
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=1 SHU_ASM_WPO_DCE=0 \
      "$SHU_ASM_ABS" -backend asm -o "$TCK_OFF" src/typeck/typeck.su >/dev/null 2>&1 ) || return 1
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=1 SHU_ASM_WPO_DCE=1 \
      "$SHU_ASM_ABS" -backend asm -o "$TCK_ON" src/typeck/typeck.su >/dev/null 2>&1 ) || return 1
  [ -s "$TCK_OFF" ] && [ -s "$TCK_ON" ]
}
...
compile_backend_ab() {
  ...
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=1 SHU_ASM_WPO_DCE=0 \
      "$SHU_ASM_ABS" -backend asm -o "$BE_OFF" src/asm/backend.su >/dev/null 2>&1 ) || return 1
  ( cd compiler && \
    timeout "$MAIN_TIMEOUT" env SHU_ASM_ENTRY_MODULE_ONLY=1 SHU_ASM_BUILD_SKIP_TYPECK=1 SHU_ASM_ENTRY_EMIT_HEAVY=1 SHU_ASM_WPO_DCE=1 \
      "$SHU_ASM_ABS" -backend asm -o "$BE_ON" src/asm/backend.su >/dev/null 2>&1 ) || return 1
  [ -s "$BE_OFF" ] && [ -s "$BE_ON" ]
}
...
echo "| shu_asm .text | $TEXT_ON | $TEXT_OFF_PROXY | $BINARY_SAVE | ${BINARY_PCT}% |"
echo "| wpo-eligible TU | $ELIGIBLE_ON | $ELIGIBLE_OFF | $ELIGIBLE_SAVE | — |"
echo "| main.su TU | $MON | $MOFF | $((MOFF - MON)) | — |"
echo "| driver/compile.su TU | $DON | $DOFF | $((DOFF - DON)) | — |"
echo "| pipeline/pipeline.su TU | $PON | $POFF | $((POFF - PON)) | — |"
echo "| typeck/typeck.su TU | $TON | $TOFF | $((TOFF - TON)) | — |"
echo "| asm/backend.su TU | $BON | $BOFF | $((BOFF - BON)) | — |"

fail=0
if [ "$ELIGIBLE_SAVE" -lt "$MIN_ELIGIBLE_BYTES" ]; then
  echo "WPO shu_asm text FAIL: wpo-eligible save ${ELIGIBLE_SAVE}B < min ${MIN_ELIGIBLE_BYTES}B" >&2
  fail=1
fi
if [ "$BINARY_SAVE" -lt "$MIN_BINARY_BYTES" ]; then
  echo "WPO shu_asm text FAIL: binary proxy save ${BINARY_SAVE}B < min ${MIN_BINARY_BYTES}B" >&2
  fail=1
fi
if awk -v p="$BINARY_PCT" -v m="$MIN_BINARY_PCT" 'BEGIN { exit (p + 0 >= m + 0) ? 0 : 1 }'; then
  :
else
  echo "WPO shu_asm text FAIL: binary proxy save ${BINARY_PCT}% < min ${MIN_BINARY_PCT}%" >&2
  fail=1
fi
...
if [ "$UPDATE_BASELINE" = "1" ] && [ "$ELIGIBLE_SAVE" -gt 0 ]; then
  cat > "$BASELINE" <<EOF
# shu_asm 全二进制 .text dogfood：wpo-eligible(main+driver+pipeline) 代理反推 WPO-off
# 更新：SHU_PERF_UPDATE_BASELINE=1 ./tests/run-perf-wpo-dce-shu-asm-text.sh
...
echo "wpo shu_asm text OK (binary .text=${TEXT_ON}B; eligible save=${ELIGIBLE_SAVE}B; proxy save=${BINARY_SAVE}B/${BINARY_PCT}%)"
