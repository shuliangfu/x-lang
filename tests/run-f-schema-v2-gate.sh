#!/usr/bin/env bash
# F-schema v2：std.schema 逻辑全量 .sx（删除 schema_glue.c）。
set -e
cd "$(dirname "$0")/.."
...
echo "=== F-schema v2: JSON/CSV decode → schema.sx (zero glue) ==="
...
grep -q 'schema_create_c' std/schema/schema.sx || die "schema.sx missing create"
grep -q 'schema_decode_json_c' std/schema/schema.sx || die "schema.sx missing decode_json"
grep -q 'schema_smoke_c' std/schema/schema.sx || die "schema.sx missing smoke"
grep -q 'schema_f_schema_v2_marker_c' std/schema/schema.sx || die "schema.sx missing v2 marker"
