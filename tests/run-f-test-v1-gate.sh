#!/usr/bin/env bash
# F-test v1：std.test 去 C（test.c → test.sx；F-ZC 删 test_glue.c）。
set -e
cd "$(dirname "$0")/.."
FAIL=${SHUX_F_TEST_V1_FAIL:-0}
die() { echo "f-test-v1 gate FAIL: $*" >&2; [ "$FAIL" = "1" ] && exit 1; exit 0; }
echo "=== F-test v1: std.test test.c → test.sx (F-ZC) ==="
...
grep -q 'test.sx' compiler/Makefile || die "Makefile missing test.sx"
