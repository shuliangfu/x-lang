...
# 前置：compiler/shu_asm 已 bootstrap。
set -e
cd "$(dirname "$0")/.."
...
if [ ! -x compiler/shu_asm ]; then
  echo "typeck-wpo-optin: SKIP (no shu_asm)"
  exit 0
fi
...
(
  ...
  SHU_ASM_EXPERIMENTAL_SKIP_GEN=1 SHU=./shu ./scripts/build_shu_asm.sh > /tmp/typeck_wpo_smoke.log 2>&1
)
...
RC=0
compiler/shu_asm "$SMK" -o /tmp/typeck_wpo_smki 2>/dev/null || RC=$?
if [ "$RC" -ne 0 ]; then
...
if nm compiler/shu_asm 2>/dev/null | grep -qE ' T check_block$'; then
  cb_src=$(nm compiler/shu_asm 2>/dev/null | grep ' T check_block$' | awk '{print $1}')
