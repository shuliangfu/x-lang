...
// Observe via `if (w.h.v[i] != …)` — typeck of `return w.h.v[i]` /
// `let x: i32 = w.h.v[i]` is a leftover T001 (subscript base must be
// array/slice/pointer) and is not folded here.
...
// Does not fold pointer *Holder, nest 21, dest `w.h.v[i]=`, or typeck
// T001 on return/let INDEX.
// Expected exit 0.
