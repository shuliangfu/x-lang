#!/usr/bin/env bash
# F-03 v1：std.heap 算法层去 C 门禁（heap_ops.sx + heap.c 无 mem/map 符号）。
#
...
[ -f std/heap/heap_ops.sx ] || die "missing heap_ops.sx"
grep -q 'heap_mem_set_c' std/heap/heap_ops.sx || die "heap_ops missing heap_mem_set_c"
grep -q 'map_i32_i32_find_c' std/heap/heap_ops.sx || die "heap_ops missing map find"
if grep -q 'heap_mem_set_c' std/heap/heap.c 2>/dev/null; then
  die "heap.c still defines heap_mem_set_c (should be in heap_ops.sx)"
fi
if grep -q 'map_i32_i32_find_c' std/heap/heap.c 2>/dev/null; then
  die "heap.c still defines map_i32_i32_find_c"
fi
grep -q 'import("std.heap.heap_ops")' std/heap/mod.sx || die "mod.sx missing heap_ops import"
