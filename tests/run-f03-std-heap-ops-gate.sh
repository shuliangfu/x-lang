...
# inventory / nested run-heap; refuse leftover ignore of explicit-bad).
# leftover nested product path (inventory / run-heap) stay. G.7: complete
...
# nested run-heap (refuse leftover SKIP→OK / leftover ignore of
# explicit-bad / leftover XLANG fallthrough). leftover nested product path
# stays when XLANG is unset (do not rewrite leftover inventory / run-heap).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
