...
SMOKE_X="tests/std-compress/brotli_roundtrip.x"
...
for f in "$DOC" "$MANIFEST" "$LIB" "$MOD_X" "$COMPRESS_C" "$SMOKE_X"; do
...
sym_miss="$(std_compress_brotli_symbols_ok "$MOD_X" "$COMPRESS_C" "$MANIFEST" || true)"
