...
# 环境：XLANG_LINUX_MMAP_FILE_FAIL=1 失败时硬退出
set -e
cd "$(dirname "$0")/.."

FAIL=${XLANG_LINUX_MMAP_FILE_FAIL:-0}
...
if [ ! -x "$XLANG" ]; then
  echo "linux-mmap-file-gate: SKIP (no xlang/xlang-c)"
  exit 0
fi
...
if ! "$XLANG" build -o "$OUT" "$X" 2>/tmp/xlang_linux_mmap_file.log; then
