...
NET_RUNTIME="compiler/src/asm/runtime_net_workers.c"
...
[ -f std/net/net_workers.sx ] || die "missing net_workers.sx"
[ -f "$NET_RUNTIME" ] || die "missing runtime_net_workers.c"
[ ! -f std/net/net_workers_glue.c ] || die "net_workers_glue.c should be deleted"
grep -q 'net_run_accept_workers_c' std/net/net_workers.sx || die "workers.sx missing API"
grep -q 'net_run_accept_workers_c_real' std/net/net_workers.sx || die "workers.sx missing _real alias"
grep -q 'shu_net_worker_accept_entry_ptr_c' "$NET_RUNTIME" || die "runtime missing entry ptr"
grep -q 'net_workers.sx' compiler/Makefile || die "Makefile missing net_workers.sx"
