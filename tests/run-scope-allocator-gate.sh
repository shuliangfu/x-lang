...
OUT="/tmp/xlang_with_arena_smoke_$$"

check_emit() {
  local gen
  gen="$(mktemp /tmp/xlang_scope_alloc_gate_XXXXXX.c)"
  if ! "$XLANG" build -E "$SRC" >"$gen" 2>/tmp/xlang_scope_alloc_gate_build.log; then
...
if ! "$XLANG" build "$SRC" -o "$OUT" >/tmp/xlang_scope_alloc_run.log 2>&1; then
...
# MEM-C1：scope_alloc 展开（XLANG_KEEP_C 保留生成 C 供 emit 断言；带 import 时 -E 会 DCE 掉入口 main）
SCOPE_SRC="tests/mem/scope_alloc.x"
SCOPE_OUT="/tmp/xlang_scope_alloc_$$"
if ! XLANG_KEEP_C=1 "$XLANG" build "$SCOPE_SRC" -o "$SCOPE_OUT" >/tmp/xlang_scope_alloc_scope_run.log 2>&1; then
  echo "scope-allocator-gate FAIL: build $SCOPE_SRC" >&2
  tail -8 /tmp/xlang_scope_alloc_scope_run.log 2>/dev/null || true
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi
gen2="$(grep -oE '/tmp/xlang_[A-Za-z0-9]+\.c' /tmp/xlang_scope_alloc_scope_run.log | tail -1)"
if [ -z "$gen2" ] || [ ! -f "$gen2" ] || ! grep -q '__xlang_scope_al_' "$gen2"; then
  echo "scope-allocator-gate FAIL: missing __xlang_scope_al_ emit" >&2
  [ -n "$gen2" ] && rm -f "$gen2"
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi
