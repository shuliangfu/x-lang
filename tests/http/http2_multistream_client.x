...
function main(): i32 {
  if (http.settings_smoke() != 0) { return 1; }
  ...
  http.peer_settings_apply_entry(&cli.peer, http.setting_max_concurrent_streams(), 2);
  let s1: i32 = http.client_open(&cli);
  let s2: i32 = http.client_open(&cli);
  let s3: i32 = http.client_open(&cli);
  if (s1 != 1 || s2 != 3) { return 5; }
  if (s3 != http.err_max_streams()) { return 6; }

  let frame: u8[512] = [];
  let host: u8[12] = [101, 120, 97, 109, 112, 108, 101, 46, 99, 111, 109, 0];
  let path: u8[3] = [47, 97, 0];
  if (http.client_get(&cli, 1, &host[0], 11, &path[0], 2, &frame[0], 512) <= 0) { return 7; }
