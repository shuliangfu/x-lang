...
# static / leftover nested std-unicode-nfc / leftover nested
# std-unicode-grapheme-case; refuse leftover ignore of explicit-bad).
# leftover auto-make of unicode.o (`xlang_compiler_make` even when the
...
# Missing leaf .o = hard die. leftover nested std-unicode-nfc /
# std-unicode-grapheme-case stay.
...
# leftover nested std-unicode-nfc / leftover nested
# std-unicode-grapheme-case (refuse leftover SKIP→OK / leftover ignore
# of explicit-bad / leftover XLANG fallthrough). leftover auto-make of
# unicode.o retired; leftover nested std-unicode-nfc /
# std-unicode-grapheme-case stay.
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
...
if [ ! -f std/unicode/unicode.o ]; then
  die "missing std/unicode/unicode.o (refuse leftover auto-make)"
fi
ENSURE_OK=1
