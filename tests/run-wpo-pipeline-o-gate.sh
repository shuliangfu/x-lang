#!/usr/bin/env bash
# S5：build_asm/pipeline_wpo.o WPO 生产链硬门禁（WPO 压缩 pipeline.su dogfood）。
...
#   SHU_WPO_PIPELINE_O_FAIL=1 ./tests/run-wpo-pipeline-o-gate.sh
set -e
cd "$(dirname "$0")/.."
...
BASELINE="${SHU_WPO_PIPELINE_O_BASELINE:-tests/baseline/wpo-pipeline-o.tsv}"
...
FAIL=${SHU_WPO_PIPELINE_O_FAIL:-1}
...
if ! nm "$PIPE_O" 2>/dev/null | grep -q 'run_su_pipeline_impl'; then
  echo "run-wpo-pipeline-o-gate FAIL: $PIPE_O missing run_su_pipeline_impl" >&2
  exit 1
fi
...
# 编排链 reach：run_su_pipeline_impl 不应 U 其 direct callee（须 ast_pool fixpoint 后重编 pipeline_wpo.o）。
if [ -x "$(dirname "$0")/run-wpo-pipeline-reach-gate.sh" ]; then
  SHU_WPO_PIPELINE_REACH_FAIL="${SHU_WPO_PIPELINE_REACH_FAIL:-0}" \
    "$(dirname "$0")/run-wpo-pipeline-reach-gate.sh" "$PIPE_O" || {
    [ "$FAIL" = "1" ] && exit 1
  }
fi

echo "wpo pipeline_wpo.o gate OK (__text=${TXT}B <= ${MAX_TEXT}B, save=${SAVE}B, run_su_pipeline_impl present)"
