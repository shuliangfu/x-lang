#!/usr/bin/env bash
# 脱离对 C 的依赖测试：用 asm 路径构建 xlang_asm（.x → asm → .o + 最少 C 桩链接），
...
# 2) 当前 xlang 必须支持 -backend asm（bootstrap-driver 产出）；否则先构建 bootstrap-driver
# 使用含 main 的用例（../tests/return-value/main.x），否则 typeck 会因 main_func_index < 0 失败
XLANG_ASM_CHECK_INPUT="../tests/return-value/main.x"
XLANG_ASM_CHECK=$(mktemp)
...
# 说明：run-all 中 run-asm 在 CI 下会直接 SKIP，不会执行 bootstrap-driver，故此时 compiler/xlang 多为「仅 C 版」；
...
# 3) 用 asm 路径构建 xlang_asm（不编任何 .x 为 C，只编为 asm .o + 最少 C 桩链接）
echo "run-without-c: building xlang_asm (asm backend, minimal C link) ..."
(cd "$COMPILER_DIR" && ./build_tool ./xlang asm)
if [ ! -x "$XLANG_ASM" ]; then
  echo "run-without-c: $XLANG_ASM build not produced; asm build failed. Fix build_xlang_asm.sh / -backend asm first."
  exit 1
fi

# 4) 用 xlang_asm 跑全量测试（不执行 make，不覆盖 compiler/xlang）
echo "run-without-c: running full test suite with XLANG=$XLANG_ASM build ..."
XLANG="$XLANG_ASM" ./tests/run-all.sh

echo "run-without-c: all tests passed with xlang_asm (no-C-runtime path)."
