...
out2=$(./compiler/xlang-c -E tests/wpo/dead_user.x 2>/dev/null)
...
# -o 链路
./compiler/xlang-c -o /tmp/xlang_wpo_dead_user tests/wpo/dead_user.x >/dev/null
rc=$(/tmp/xlang_wpo_dead_user; echo $?)
[ "$rc" = "7" ] || { echo "dead_user exit=$rc want 7"; exit 1; }

# if 分支 AST_EXPR_BLOCK 内 call 须计入 WPO 可达性（否则 core.option some/none 被 DCE 误删）
GRAPH="/tmp/xlang_wpo_if_block_reach.json"
rm -f "$GRAPH"
XLANG_WPO_DUMP_CALLGRAPH="$GRAPH" ./compiler/xlang-c check -L . tests/wpo/if_block_reach.x >/dev/null
...
out3=$(./compiler/xlang-c -L . tests/wpo/if_block_reach.x -E 2>/dev/null)
