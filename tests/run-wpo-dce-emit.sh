...
# 单文件 intra-module
out=$(./compiler/shux-c -E tests/wpo/dead_fn.sx 2>/dev/null)
...
out2=$(./compiler/shux-c -E tests/wpo/dead_user.sx 2>/dev/null)
...
# -o 链路
./compiler/shux-c -o /tmp/shux_wpo_dead_user tests/wpo/dead_user.sx >/dev/null
...
rm -f "$GRAPH"
SHUX_WPO_DUMP_CALLGRAPH="$GRAPH" ./compiler/shux-c check -L . tests/wpo/if_block_reach.sx >/dev/null
...
out3=$(./compiler/shux-c -L . tests/wpo/if_block_reach.sx -E 2>/dev/null)
...
./compiler/shux-c -L . tests/wpo/if_block_reach.sx -o /tmp/shux_wpo_if_block_reach >/dev/null
