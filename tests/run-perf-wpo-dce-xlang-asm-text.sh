...
BASELINE="${XLANG_WPO_XLANG_ASM_TEXT_BASELINE:-tests/baseline/wpo-dce-xlang-asm-text.tsv}"
MAIN_TIMEOUT="${XLANG_WPO_MAIN_ASM_TIMEOUT:-180}"
FAIL_REGRESS=0
UPDATE_BASELINE=0
[ "${XLANG_PERF_FAIL_ON_WPO_XLANG_ASM_TEXT:-0}" = "1" ] && FAIL_REGRESS=1
...
if [ -n "$main_fast" ]; then
  MOFF="${main_fast%% *}"
  MON="${main_fast#* }"
  echo "wpo xlang_asm text: main A/B fast-path (__text=${MON}B proxy off=${MOFF}B)"
fi
drv_fast=$(wpo_ab_try_driver_fast "$BUILD_ASM/driver_compile.o" "$BASELINE_PROXY" 768) || drv_fast=""
if [ -n "$drv_fast" ]; then
  DOFF="${drv_fast%% *}"
  DON="${drv_fast#* }"
  echo "wpo xlang_asm text: driver A/B fast-path (__text=${DON}B proxy off=${DOFF}B)"
fi
...
if [ -n "$pipe_fast" ]; then
  POFF="${pipe_fast%% *}"
  PON="${pipe_fast#* }"
  echo "wpo xlang_asm text: pipeline A/B fast-path ($pipe_tree __text=${PON}B proxy off=${POFF}B)"
fi
...
if [ -n "$tck_fast" ]; then
  TOFF="${tck_fast%% *}"
  TON="${tck_fast#* }"
  echo "wpo xlang_asm text: typeck A/B fast-path ($tck_tree __text=${TON}B proxy off=${TOFF}B)"
fi
...
if [ -n "$be_fast" ]; then
  BOFF="${be_fast%% *}"
  BON="${be_fast#* }"
  echo "wpo xlang_asm text: backend A/B fast-path ($be_tree __text=${BON}B proxy off=${BOFF}B)"
fi

MAIN_OFF="/tmp/xlang_wpo_xlang_asm_main_off.o"
MAIN_ON="/tmp/xlang_wpo_xlang_asm_main_on.o"
DRIVER_OFF="/tmp/xlang_wpo_xlang_asm_driver_off.o"
DRIVER_ON="/tmp/xlang_wpo_xlang_asm_driver_on.o"
PIPE_OFF="/tmp/xlang_wpo_xlang_asm_pipe_off.o"
PIPE_ON="/tmp/xlang_wpo_xlang_asm_pipe_on.o"
TCK_OFF="/tmp/xlang_wpo_xlang_asm_tck_off.o"
TCK_ON="/tmp/xlang_wpo_xlang_asm_tck_on.o"
BE_OFF="/tmp/xlang_wpo_xlang_asm_be_off.o"
BE_ON="/tmp/xlang_wpo_xlang_asm_be_on.o"

# G.7: A/B recompile via wpo_ab_compile_entry_pair（须 $LIBROOT，对齐 build_xlang_asm）。
compile_main_ab() {
  wpo_ab_compile_entry_pair "$XLANG_ASM_ABS" "$MAIN_OFF" "$MAIN_ON" "src/main.x" 0 "$MAIN_TIMEOUT"
}

compile_driver_ab() {
  wpo_ab_compile_entry_pair "$XLANG_ASM_ABS" "$DRIVER_OFF" "$DRIVER_ON" "src/driver/compile.x" 1 "$MAIN_TIMEOUT"
}

compile_pipeline_ab() {
  wpo_ab_compile_entry_pair "$XLANG_ASM_ABS" "$PIPE_OFF" "$PIPE_ON" "src/pipeline/pipeline.x" 1 "$MAIN_TIMEOUT"
}

compile_typeck_ab() {
  wpo_ab_compile_entry_pair "$XLANG_ASM_ABS" "$TCK_OFF" "$TCK_ON" "src/typeck/typeck.x" 1 "$MAIN_TIMEOUT"
}

compile_backend_ab() {
  wpo_ab_compile_entry_pair "$XLANG_ASM_ABS" "$BE_OFF" "$BE_ON" "src/asm/backend.x" 1 "$MAIN_TIMEOUT"
}
...
echo "| xlang_asm .text | $TEXT_ON | $TEXT_OFF_PROXY | $BINARY_SAVE | ${BINARY_PCT}% |"
