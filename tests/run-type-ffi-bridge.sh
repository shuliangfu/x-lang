...
# i32 → putchar（标量桥接）
exe="/tmp/xlang_ffi_bridge_putchar"
if ! XLANG="$XLANG_BIN" "$XLANG_BIN" -L . tests/ffi/putchar.x -o "$exe" 2>/tmp/xlang_ffi_bridge_putchar.log; then
  cat /tmp/xlang_ffi_bridge_putchar.log >&2
