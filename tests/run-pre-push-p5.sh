#!/usr/bin/env bash
# P5 push 前自检：M-3/M-4 typeck + M-5 IO + ZC-3 slice/option + WPO DCE + ZC-1/IO-A6 SKIP 烟测（跨平台 xlang-c）。
# Linux 额外跑 refresh xlang_asm gate（X 路径 region/linear）。
...
./tests/run-bcmp-gate.sh | tee /tmp/xlang_bcmp_p5.log
grep -q 'bcmp gate OK' /tmp/xlang_bcmp_p5.log
...
echo "=== P5: WPO-S4 PGO-Lite (.text.hot/unlikely + call-depth emit; xlang_asm 时全量) ==="
chmod +x tests/run-wpo-s4-gate.sh tests/run-perf-wpo-dce-xlang-asm-text.sh tests/lib/wpo-ab-proxy.sh
if [ -x ./compiler/xlang_asm ] && { [ "$(uname -s)" = "Linux" ] || file ./compiler/xlang_asm 2>/dev/null | grep -q Mach-O; }; then
  XLANG_WPO_PGO_HOT=1 XLANG=./compiler/xlang_asm ./tests/run-wpo-s4-gate.sh | tee /tmp/xlang_wpo_s4_p5.log
  grep -q 'wpo-s4 gate OK' /tmp/xlang_wpo_s4_p5.log
  grep -q 'wpo-s4: .text.hot order OK' /tmp/xlang_wpo_s4_p5.log
else
...
if [ -x ./compiler/xlang_asm ] && { [ "$(uname -s)" = "Linux" ] || file ./compiler/xlang_asm 2>/dev/null | grep -q Mach-O; }; then
  XLANG=./compiler/xlang_asm ./tests/run-simd-s1-gate.sh | tee /tmp/xlang_simd_s1_p5.log
  grep -q 'simd-s1 gate OK' /tmp/xlang_simd_s1_p5.log
else
...
if [ -x ./compiler/xlang_asm ] && { [ "$(uname -s)" = "Linux" ] || file ./compiler/xlang_asm 2>/dev/null | grep -q Mach-O; }; then
  XLANG=./compiler/xlang_asm ./tests/run-simd-s2-gate.sh | tee /tmp/xlang_simd_s2_p5.log
  grep -q 'simd-s2 gate OK' /tmp/xlang_simd_s2_p5.log
else
...
if [ -x ./compiler/xlang_asm ] && { [ "$(uname -s)" = "Linux" ] || file ./compiler/xlang_asm 2>/dev/null | grep -q Mach-O; }; then
  XLANG=./compiler/xlang_asm ./tests/run-simd-s3-gate.sh | tee /tmp/xlang_simd_s3_p5.log
  grep -q 'simd-s3 gate OK' /tmp/xlang_simd_s3_p5.log
else
...
if [ -x ./compiler/xlang_asm ] && { [ "$(uname -s)" = "Linux" ] || file ./compiler/xlang_asm 2>/dev/null | grep -q Mach-O; }; then
  XLANG=./compiler/xlang_asm ./tests/run-simd-s4-gate.sh | tee /tmp/xlang_simd_s4_p5.log
  grep -q 'simd-s4 gate OK' /tmp/xlang_simd_s4_p5.log
else
  ./tests/run-simd-s4-gate.sh
fi

echo "=== P5: SIMD dot perf (optional, Linux xlang_asm) ==="
chmod +x tests/run-perf-simd-dot.sh
./tests/run-perf-simd-dot.sh

# xlang_asm 可用时补 WPO-S2 perf compile/disasm/exit 门禁（跳过 median，Mac 友好）
if [ -x ./compiler/xlang_asm ]; then
  echo "=== P5: WPO-S2 perf compile-only (xlang_asm) ==="
  XLANG=./compiler/xlang_asm XLANG_WPO_S2_COMPILE_ONLY=1 XLANG_WPO_S2_LIMIT=100000 ./tests/run-perf-wpo-s2.sh --bench
fi
...
if [ -x ./compiler/xlang_asm ] && [ "$(uname -s)" = "Linux" ]; then
...
if [ "$(uname -s)" = "Linux" ]; then
  ...
  if [ -x ./compiler/xlang_asm ]; then
    echo "=== P5: B-SIZE xlang_asm stripped (advisory, ENG-002) ==="
    ...
    XLANG_WPO_ENSURE_FAIL=1 XLANG_WPO_ENSURE_COMPILER=./compiler/xlang_asm ./tests/ensure-wpo-build-asm-artifacts.sh
    XLANG=./compiler/xlang_asm XLANG_PERF_FAIL_ON_WPO_XLANG_ASM_TEXT=1 ./tests/run-perf-wpo-dce-xlang-asm-text.sh
    # B-CMP-ASM：跟踪 xlang_asm vs C-O3（计算 loop 仍劣于 C，勿硬门禁）
    echo "=== P5: B-CMP xlang_asm 1.0× (track-only, non-fatal) ==="
    XLANG_PERF_BCMP_ASM=1 ./tests/run-bcmp-gate.sh | tee /tmp/xlang_bcmp_asm_p5.log || true
    grep -q 'B-CMP-ASM' /tmp/xlang_bcmp_asm_p5.log || true
    # stretch -3%：须 run-bootstrap-bstrict-ci 全链后 ./tests/run-wpo-stretch-gate.sh
  fi
else
  echo "=== P5: refresh xlang_asm gate SKIP (non-Linux) ==="
...
if command -v git >/dev/null 2>&1 && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  if [ "$(uname -s)" = "Darwin" ] && [ "$dirty" != "0" ]; then
    echo "note: after ./scripts/docker-ci-local.sh ubuntu-gates, rebuild native xlang-c (make clean && make xlang-c)"
  fi
fi
