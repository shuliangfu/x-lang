...
SU="tests/sys/sys_platform_write_unified.sx"
...
if ! "$SHUX" "${EXTRA[@]}" -o "$OUT" "$SU" 2>/tmp/shux_sys_platform_write.log; then
  echo "sys-platform-write-gate FAIL: compile $SU on $OS" >&2
