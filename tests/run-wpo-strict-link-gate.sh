#!/usr/bin/env bash
# S5：strict 链 WPO link 门禁（pipeline_wpo helpers + C orchestration 链入 shux_asm.strict_glue）。
# 用法：
...
FAIL=${SHUX_WPO_STRICT_LINK_FAIL:-1}
COMPILER="${SHUX_WPO_STRICT_LINK_COMPILER:-compiler/shux_asm.strict_glue}"
PIPE_WPO="${SHUX_WPO_PIPELINE_O:-compiler/build_asm/pipeline_wpo.o}"

chmod +x compiler/scripts/relink_shux_asm_strict_glue.sh tests/run-wpo-pipeline-reach-gate.sh 2>/dev/null || true
...
if [ -f compiler/build_asm/typeck_wpo.o ]; then
  SHUX_WPO_TYPECK_REACH_FAIL="$FAIL" ./tests/run-wpo-typeck-reach-gate.sh compiler/build_asm/typeck_wpo.o || exit 1
fi
if [ -f compiler/build_asm/backend_wpo.o ]; then
  SHUX_WPO_BACKEND_REACH_FAIL="$FAIL" ./tests/run-wpo-backend-reach-gate.sh compiler/build_asm/backend_wpo.o || exit 1
fi
...
if [ "${SHUX_WPO_STRICT_LINK_SMOKE_COMPILE:-0}" = "1" ]; then
  ...
  if ! "$COMPILER" -backend asm -o "$TMP_O" $LIBROOT "$TEST_X" 2>/dev/null; then
    echo "run-wpo-strict-link-gate FAIL: $COMPILER cannot compile $TEST_X" >&2
    [ "$FAIL" = "1" ] && exit 1
    exit 0
  fi
  if [ ! -s "$TMP_O" ]; then
    echo "run-wpo-strict-link-gate FAIL: empty output from $TEST_X" >&2
    [ "$FAIL" = "1" ] && exit 1
    exit 0
  fi
  rm -f "$TMP_O" 2>/dev/null || true
