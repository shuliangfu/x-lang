#!/usr/bin/env bash
# S5：strict 链 WPO link 门禁（pipeline_wpo helpers + C orchestration 链入 xlang_asm.strict_glue）。
# 用法：
...
# 小测试文件可编（strict_glue 全路径 asm 编译已知不稳定；默认仅验链+符号，设 XLANG_WPO_STRICT_LINK_SMOKE_COMPILE=1 启用）。
