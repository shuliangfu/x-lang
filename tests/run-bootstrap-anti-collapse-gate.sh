...
#
# 在 gen1/gen2 与 A-09 之后运行，确保未静默复制 pinned seed / 旧 xlang 冒充 strict 产物。
#
# 用法（仓库根）：
#   ./tests/run-bootstrap-anti-collapse-gate.sh
#   XLANG_BOOTSTRAP_ANTI_COLLAPSE_FAIL=1 ./tests/run-bootstrap-anti-collapse-gate.sh
#
# 环境：
#   XLANG_BOOTSTRAP_ANTI_COLLAPSE_SKIP=1 — 完全跳过

set -euo pipefail
cd "$(dirname "$0")/.."

if [ "${XLANG_BOOTSTRAP_ANTI_COLLAPSE_SKIP:-0}" = "1" ]; then
