...
# std-tar-ustar / leftover nested std-tar-extended; refuse leftover
...
# leftover nested ensure / leftover nested std-tar-ustar / leftover
# nested std-tar-extended (refuse leftover SKIP→OK / leftover ignore
# of explicit-bad / leftover XLANG fallthrough). leftover nested
# product path stays when XLANG is unset (do not rewrite leftover
# xlang_compiler_make / std-tar-ustar / std-tar-extended).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
