#!/usr/bin/env bash
# core.option API smoke: Option_i32 / Option_u8 product -o exit 102
#
...
PREFIX="${XLANG_OPTION_PREFIX:-xlang: [OPTION]}"
...
# Alpine/Docker small stacks: typeck/codegen on option may overflow; raise soft limit.
ulimit -s 16384 2>/dev/null || true

echo "=== option gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

src="tests/option/main.x"
[ -f "$src" ] || die "missing $src"
exe="/tmp/xlang_option_$$"
log="/tmp/xlang_option_$$.log"
rm -f "$exe" "$log"
set +e
...
if [ "$r_ec" -eq 124 ]; then
  die "run timeout"
elif [ "$r_ec" -ne 102 ]; then
  # 10+42+7 + unwrap_or_u8 extras + map/and_then/generic/ptr → 102
  die "expected exit 102, got $r_ec"
fi
RUN_OK=$((RUN_OK + 1))
ok_report
echo "option test OK"
