...
SMOKE_SU="tests/std-hash/default_strategy.su"
...
for kw in STD-148 recommend_hasher_map SHU_HASH_ALGO HASHER_XXHASH; do
...
if [ -n "$SHU_BIN" ]; then
  ...
  if std_hash_default_strategy_run_su_smoke "$SHU_BIN" "$SMOKE_SU" "$HASH_O"; then
    SU_OK=1
  else
    std_hash_default_strategy_emit_report "fail" "$C_OK" 0 0
    exit 1
