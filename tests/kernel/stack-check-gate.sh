...
MAX_STACK_COUNT=$(grep -c 'xlang_max_stack' "$C_FILE" 2>/dev/null || echo 0)
...
VAL1=$(grep -o 'xlang_max_stack:64' "$C_FILE" | head -1)
VAL2=$(grep -o 'xlang_max_stack:32' "$C_FILE" | head -1)
