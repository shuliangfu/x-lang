#!/usr/bin/env bash
# asm 7.3: VAR+VAR binop loads stack slots directly (no push/pop).
#
...
#   - hard: tests/asm/binop_var_fast.x product -o run exit 143
...
PREFIX="${XLANG_ASM_BINOP_VAR_PREFIX:-xlang: [XLANG_ASM_BINOP_VAR]}"
...
echo "=== asm-binop-var gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

SRC="tests/asm/binop_var_fast.x"
[ -f "$SRC" ] || die "missing $SRC"
exe="/tmp/xlang_asm_binop_var_$$"
log="/tmp/xlang_asm_binop_var_$$.log"
rm -f "$exe" "$log"
...
if [ "$r_ec" -eq 124 ]; then
  rm -f "$exe"
  die "run timeout"
elif [ "$r_ec" -ne 143 ]; then
  rm -f "$exe"
  die "expected exit 143, got $r_ec"
fi
RUN_OK=$((RUN_OK + 1))
...
if [ "$(uname -s)" = Darwin ] && command -v otool >/dev/null 2>&1; then
  if otool -tv "$exe" 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'sub.*sp, sp, #0x10'; then
    rm -f "$exe"
    die "main still uses stack push for binop (expected direct ldr)"
  fi
  RUN_OK=$((RUN_OK + 1))
else
  SKIP=$((SKIP + 1))
