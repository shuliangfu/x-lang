...
if [ -z "$SHU_ABS" ] || ! simd_s4_native_exe "$SHU_ABS"; then
  echo "simd-s4 gate SKIP (no native shu/shu_asm)"
  exit 0
fi

VEC4F_SRC="tests/simd/vec4f_shuffle_smoke.su"
VEC8I_SRC="tests/simd/vec8i_shuffle_smoke.su"
VEC8I_SEL_SRC="tests/simd/vec8i_select_smoke.su"
VEC4F_SEL_SRC="tests/simd/vec4f_select_smoke.su"
AT_SRC="tests/simd/at_builtin_smoke.su"
DOT_SRC="tests/bench/simd_dot.su"
VEC4F_O="/tmp/shu_simd_s4_vec4f.o"
VEC8I_O="/tmp/shu_simd_s4_vec8i.o"
VEC8I_SEL_O="/tmp/shu_simd_s4_vec8i_sel.o"
VEC4F_SEL_O="/tmp/shu_simd_s4_vec4f_sel.o"
AT_O="/tmp/shu_simd_s4_at.o"
DOT_O="/tmp/shu_simd_s4_dot.o"
rm -f "$VEC4F_O" "$VEC8I_O" "$VEC8I_SEL_O" "$VEC4F_SEL_O" "$AT_O" "$DOT_O"

if ! SHU="$SHU_ABS" "$SHU_ABS" "$VEC4F_SRC" -o "$VEC4F_O"; then
  echo "simd-s4 FAIL: compile $VEC4F_SRC" >&2
  exit 1
fi

if ! SHU="$SHU_ABS" "$SHU_ABS" "$VEC8I_SRC" -o "$VEC8I_O"; then
  echo "simd-s4 FAIL: compile $VEC8I_SRC" >&2
  exit 1
fi

if ! SHU="$SHU_ABS" "$SHU_ABS" "$VEC8I_SEL_SRC" -o "$VEC8I_SEL_O"; then
  echo "simd-s4 FAIL: compile $VEC8I_SEL_SRC" >&2
  exit 1
fi

if ! SHU="$SHU_ABS" "$SHU_ABS" "$VEC4F_SEL_SRC" -o "$VEC4F_SEL_O"; then
  echo "simd-s4 FAIL: compile $VEC4F_SEL_SRC" >&2
  exit 1
fi

if ! SHU="$SHU_ABS" "$SHU_ABS" "$AT_SRC" -o "$AT_O"; then
  echo "simd-s4 FAIL: compile $AT_SRC" >&2
  exit 1
fi

if ! SHU="$SHU_ABS" "$SHU_ABS" "$DOT_SRC" -o "$DOT_O"; then
...
if [ "$ARCH" = "x86_64" ] || [ "$ARCH" = "amd64" ]; then
  if command -v objdump >/dev/null 2>&1; then
    ...
    if echo "$V4_DISAS" | grep -qE 'pshufd'; then
      ...
      echo "simd-s4 WARN: no pshufd in vec4f shuffle smoke (rebuild shu_asm with simd_enc.o)"
    fi
