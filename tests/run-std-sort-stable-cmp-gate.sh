#!/usr/bin/env bash
# STD-060: std.sort stable + custom cmp gate — honesty soft auto-make →硬绿.
#
...
# auto-make / prefer-c / soft ensure rebuild). Product stable_i32.x + cmp_desc.x
# -o exit0 = hard run (run=2). check / host-C archaeology = obs.
# Report: run=/obs=/skip=.
