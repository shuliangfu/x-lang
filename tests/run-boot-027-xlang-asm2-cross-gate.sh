...
LIB="tests/lib/boot-027-xlang-asm2-cross.sh"
MIN_HOOKS=4
MIN_ROWS=4
...
echo "=== BOOT-027: xlang_asm2 cross manifest ==="
for f in "$DOC" "$MANIFEST" "$WAVE" "$LIB" "$MATRIX" "$REPRO" \
  ...
  if [ ! -f "$f" ]; then
      echo "boot-027-xlang-asm2-cross gate FAIL: missing $f" >&2
      exit 1
  fi
done
...
for kw in c5_ok c5_smoke boot027_xlang_asm2_cross xlang_asm2_cross_target; do
  if ! grep -qF "$kw" "$DOC" 2>/dev/null; then
    echo "boot-027-xlang-asm2-cross gate FAIL: doc missing '$kw'" >&2
    exit 1
  fi
done

if ! grep -qF 'boot027_xlang_asm2_cross' "$REPRO" 2>/dev/null; then
  echo "boot-027-xlang-asm2-cross gate FAIL: bootstrap-repro missing boot027" >&2
  exit 1
fi
...
if [ "$ROW_N" -lt "$MIN_ROWS" ]; then
  echo "boot-027-xlang-asm2-cross gate FAIL: rows=${ROW_N} < min ${MIN_ROWS}" >&2
  exit 1
fi
...
if [ "$HOOK_N" -lt "$MIN_HOOKS" ]; then
  echo "boot-027-xlang-asm2-cross gate FAIL: hooks=${HOOK_N} < min ${MIN_HOOKS}" >&2
  exit 1
fi

if ! grep -F 'C6_xlang_asm2_cross' "$MATRIX" 2>/dev/null | grep -qF 'run-boot-027-xlang-asm2-cross-gate.sh'; then
  echo "boot-027 FAIL: C6_xlang_asm2_cross not wired to boot-027 gate" >&2
  MISS=$((MISS + 1))
fi
...
echo "boot-027-xlang-asm2-cross manifest OK (rows=${ROW_N} hooks=${HOOK_N})"

echo "=== BOOT-027: parent BOOT-026 manifest ==="
chmod +x tests/run-boot-026-parser-c4-bootstrap-gate.sh
XLANG_BOOT026_MANIFEST_ONLY=1 ./tests/run-boot-026-parser-c4-bootstrap-gate.sh

if [ "${XLANG_BOOT027_MANIFEST_ONLY:-0}" = "1" ]; then
  boot027_emit_report "ok" 0 0 1
  echo "boot-027-xlang-asm2-cross gate OK (manifest only)"
  exit 0
fi
...
if boot027_linux_asm2_candidate; then
  echo "=== BOOT-027: xlang_asm2 cross smoke (Linux) ==="
  chmod +x tests/run-xlang-asm2-cross-smoke.sh
  SMOKE_LOG="/tmp/boot027_smoke_$$.log"
  if ./tests/run-xlang-asm2-cross-smoke.sh 2>&1 | tee "$SMOKE_LOG"; then
  ...
  echo "boot-027-xlang-asm2-cross gate SKIP wave (Darwin or no native xlang_asm2)" >&2
fi

if [ "$SKIP" -eq 0 ] && [ "$C5_OK" -lt 1 ]; then
  echo "boot-027-xlang-asm2-cross gate FAIL: c5_ok=0" >&2
  boot027_emit_report "fail" 0 "$C5_SMOKE" 0
  exit 1
fi

boot027_emit_report "ok" "$C5_OK" "$C5_SMOKE" "$SKIP"
echo "boot-027-xlang-asm2-cross gate OK"
