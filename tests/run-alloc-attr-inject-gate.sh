...
rm -f "$OUT"
if ! XLANG_KEEP_C=1 "$XLANG" "$SRC" -o "$OUT" >/tmp/xlang_alloc_attr_run.log 2>&1; then
  echo "alloc-attr-inject-gate FAIL: compile/run failed" >&2
  tail -8 /tmp/xlang_alloc_attr_run.log 2>/dev/null || true
  exit 1
fi
gen="$(grep -oE '/tmp/xlang_[A-Za-z0-9]+\.c' /tmp/xlang_alloc_attr_run.log | tail -1)"
if [ -z "$gen" ] || [ ! -f "$gen" ]; then
  echo "alloc-attr-inject-gate FAIL: XLANG_KEEP_C did not retain generated C" >&2
  exit 1
fi
