...
# 3) ci-full-suite 不含 XLANG_SIZE_FAIL=1（体积 advisory）
...
for f in \
  ...
  tests/run-size-xlang-asm-gate.sh; do
  if [ ! -f "$f" ]; then
...
if grep -q 'XLANG_SIZE_FAIL=1' tests/run-ci-full-suite.sh; then
  echo "eng-quality-gate gate FAIL: run-ci-full-suite.sh still uses XLANG_SIZE_FAIL=1" >&2
  exit 1
fi
...
# size gate 默认 advisory：不得 CI+Linux 自动 XLANG_SIZE_FAIL
if grep -q '\[ -n "\${CI:-}" \]' tests/run-size-xlang-asm-gate.sh 2>/dev/null; then
  echo "eng-quality-gate gate FAIL: run-size-xlang-asm-gate.sh still auto-fails on CI" >&2
  exit 1
fi
