...
if [ "${XLANG_COMP020_MANIFEST_ONLY:-0}" = "1" ]; then
...
XLANG_COMP_INCR_COMPILE_MANIFEST_ONLY=1 ./tests/run-comp-incr-compile-gate.sh
...
while IFS=$'\t' read -r item_id kind anchor src tier _notes; do
  ...
  if ! comp_incr_wave_hook_runnable "$anchor"; then
    echo "comp-incr-compile-wave SKIP $item_id ($anchor no native xlang)"
