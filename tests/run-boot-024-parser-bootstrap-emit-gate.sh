...
for f in "$DOC" "$MANIFEST" "$WAVE" "$LIB" "$MATRIX" \
  compiler/seeds/parser_asm/parser_asm_parse_bootstrap_obj.inc \
...
for kw in bootstrap_minimal_ok bootstrap_full_emit XLANG_ASM_PARSER_PARSE_BOOTSTRAP_EMIT boot_bisect; do
...
if [ "${XLANG_BOOT024_MANIFEST_ONLY:-0}" = "1" ]; then
...
if boot024_parser_linux_shu; then
  echo "=== BOOT-024: bootstrap bisect (Linux xlang) ==="
  ...
  echo "boot-024-parser-bootstrap-emit gate SKIP wave (Darwin or no compiler/xlang)" >&2
fi
