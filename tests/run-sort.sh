#!/usr/bin/env bash
# std.sort smoke: product -o exit 0
#
...
PREFIX="${XLANG_SORT_PREFIX:-xlang: [SORT]}"
...
echo "=== sort gate (prefer asm; hard) ==="
...
exe="/tmp/xlang_sort_$$"
log="/tmp/xlang_sort_$$.log"
rm -f "$exe" "$log"
[ -f "tests/sort/main.x" ] || die "missing tests/sort/main.x"
set +e
gate_run_timeout "$XLANG_CASE_TIMEOUT" "$XLANG_BIN" -L . "tests/sort/main.x" -o "$exe" >"$log" 2>&1
o_ec=$?
set -e
if [ "$o_ec" -eq 124 ]; then
