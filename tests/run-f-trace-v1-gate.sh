#!/usr/bin/env bash
# F-trace v1：std.trace 去 C（trace.c → trace.sx；v2 后逻辑全在 trace.sx）。
set -e
cd "$(dirname "$0")/.."
...
echo "=== F-trace v1: trace.c → trace.sx ==="
...
grep -q 'trace.sx' compiler/Makefile || die "Makefile missing trace.sx"
