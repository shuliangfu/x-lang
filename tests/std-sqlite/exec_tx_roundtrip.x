// See implementation.
const sqlite = import("std.db.sqlite");

/** Internal function `main`.
 * Program/test entry point.
 * @return i32
 */
function main(): i32 {
  let mem: u8[9] = [58, 109, 101, 109, 111, 114, 121, 58, 0];
  let sql_create: u8[27] = [
    67, 82, 69, 65, 84, 69, 32, 84, 65, 66, 76, 69, 32, 116, 40, 107,
    32, 73, 78, 84, 69, 71, 69, 82, 41, 59, 0
  ];
  let sql_insert: u8[30] = [
    73, 78, 83, 69, 82, 84, 32, 73, 78, 84, 79, 32, 116, 40, 107, 41,
    32, 86, 65, 76, 85, 69, 83, 32, 40, 55, 41, 59, 0
  ];
  let sql_insert2: u8[30] = [
    73, 78, 83, 69, 82, 84, 32, 73, 78, 84, 79, 32, 116, 40, 107, 41,
    32, 86, 65, 76, 85, 69, 83, 32, 40, 57, 57, 41, 59, 0
  ];
  let c: DbConn = sqlite.open(&mem[0]);
  if (c.handle == 0) {
    return 1;
  }
  if (sqlite.exec(c, &sql_create[0]) != 0) {
    sqlite.close(c);
    return 2;
  }
  if (sqlite.begin_tx(c) != 0) {
    sqlite.close(c);
    return 3;
  }
  if (sqlite.exec(c, &sql_insert[0]) != 0) {
    sqlite.rollback(c);
    sqlite.close(c);
    return 4;
  }
  if (sqlite.changes(c) != 1) {
    sqlite.rollback(c);
    sqlite.close(c);
    return 5;
  }
  if (sqlite.commit(c) != 0) {
    sqlite.close(c);
    return 6;
  }
  if (sqlite.begin_tx(c) != 0) {
    sqlite.close(c);
    return 7;
  }
  if (sqlite.exec(c, &sql_insert2[0]) != 0) {
    sqlite.rollback(c);
    sqlite.close(c);
    return 8;
  }
  if (sqlite.rollback(c) != 0) {
    sqlite.close(c);
    return 9;
  }
  if (sqlite.close(c) != 0) {
    return 10;
  }
  return 0;
}
