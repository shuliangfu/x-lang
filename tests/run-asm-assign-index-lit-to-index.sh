...
$XLANG build tests/asm/assign_index_lit_to_index.x -o /tmp/xlang_asm_assign_index_lit_to_index 2>&1
exitcode=0
/tmp/xlang_asm_assign_index_lit_to_index >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 15 ] && { echo "run-asm-assign-index-lit-to-index FAIL: expected exit 15, got $exitcode"; exit 1; }

if otool -tv /tmp/xlang_asm_assign_index_lit_to_index 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'mov x2'; then
