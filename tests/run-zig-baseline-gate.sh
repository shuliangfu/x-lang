...
#   XLANG_ZIG_BASELINE_STRICT=1 ...                 # zig 版本须匹配 0.13.x
#   XLANG_ZIG_BASELINE_VERIFY=1 ...                 # 实测须在 recorded×(1+tolerance%) 内
set -e
cd "$(dirname "$0")/.."
...
if [ "$DO_RECORD" -eq 0 ] && [ "${XLANG_ZIG_BASELINE_VERIFY:-0}" != "1" ]; then
...
while IFS=$'\t' read -r typ case_id category src recorded tol notes; do
  ...
  if [ "${XLANG_ZIG_BASELINE_VERIFY:-0}" = "1" ] && [ -n "$recorded" ] && [ "$recorded" != "nan" ]; then
    pct="${tol:-15}"
