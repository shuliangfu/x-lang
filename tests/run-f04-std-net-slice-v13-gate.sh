...
[ -f std/net/net_tcp.sx ] || die "missing net_tcp.sx"
grep -q 'net_tcp_connect_c' std/net/net_tcp.sx || die "net_tcp missing connect"
grep -q 'net_tcp_listen_c' std/net/net_tcp.sx || die "net_tcp missing listen"
grep -q 'net_accept_c' std/net/net_tcp.sx || die "net_tcp missing accept"
grep -q 'net_accept_many_c' std/net/net_tcp.sx || die "net_tcp missing accept_many"
grep -q 'net_connect_many_c' std/net/net_tcp.sx || die "net_tcp missing connect_many"
...
if [ -f "$NET_C" ]; then
  grep -q 'net_tcp.sx' "$NET_C" || die "net.c should reference net_tcp.sx"
...
if [ -f std/net/net_workers.sx ]; then
  grep -q 'net_run_accept_workers_c' std/net/net_workers.sx || die "net_workers.sx missing run_accept_workers"
fi
if grep -q 'net_close_socket_c_real' "$NET_C" 2>/dev/null; then
  die "net.c still references net_close_socket_c_real"
fi
grep -q 'net_tcp.sx' compiler/Makefile || die "Makefile missing net_tcp.sx"
