#!/usr/bin/env bash
# raw pointer *T: type + zero-init smoke
#
...
PREFIX="${XLANG_POINTER_PREFIX:-xlang: [POINTER]}"
...
echo "=== pointer gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

exe="/tmp/xlang_pointer_$$"
log="/tmp/xlang_pointer_$$.log"
rm -f "$exe" "$log"
[ -f "tests/pointer/main.x" ] || die "missing tests/pointer/main.x"
set +e
gate_run_timeout "$XLANG_CASE_TIMEOUT" "$XLANG_BIN" -L . "tests/pointer/main.x" -o "$exe" >"$log" 2>&1
o_ec=$?
set -e
if [ "$o_ec" -eq 124 ]; then
