#!/usr/bin/env bash
# Shux 硬切验收：禁止 .su / shu / shulang / SHU_ 等遗留品牌与后缀（§12、§13）。
...
# 2) 旧 CLI 二进制名（生产路径）
for legacy_bin in compiler/shu compiler/shu-c compiler/shulang; do
  if [ -e "$legacy_bin" ]; then
    bad "legacy binary still exists: $legacy_bin"
  fi
done

# 3) 级联误替换 / 旧 env 名（排除本 gate 与迁移工具）
if rg -q 'SHUXX_' --glob '!tests/run-no-legacy-shux-gate.sh' --glob '!tools/shux_migrate*.py' compiler tests scripts tools editors 2>/dev/null; then
  bad "SHUXX typo remains (use SHUX)"
  rg -n 'SHUXX_' --glob '!tests/run-no-legacy-shux-gate.sh' --glob '!tools/shux_migrate*.py' compiler tests scripts tools editors 2>/dev/null | head -10 | sed 's/^/  /'
fi

# 4) 硬切禁止：路径/脚本中的 shu-asm、vscode-shulang 等
scan_dirs=(compiler core std tests scripts tools editors)
if rg -q '\bshulang\b|SHULANG_|\bshu-c\b|/compiler/shu\b|boot-[0-9]+-shu-' \
  --glob '!tools/shux_migrate*.py' --glob '!tests/run-no-legacy-shux-gate.sh' --glob '!editors/vscode/package-lock.json' \
  "${scan_dirs[@]}" 2>/dev/null; then
  bad "legacy shu/shulang references in scan tree"
  rg -n '\bshulang\b|SHULANG_|\bshu-c\b|/compiler/shu\b|boot-[0-9]+-shu-' \
    --glob '!tools/shux_migrate*.py' --glob '!tests/run-no-legacy-shux-gate.sh' --glob '!editors/vscode/package-lock.json' \
...
# 6) 旧 IO/async ABI（shu_io_ / __shu_frame / shu_async_）；排除生成 C、baseline 与迁移工具
if rg -q '\bshu_io_|\b__shu_frame\b|\bshu_async_' \
  --glob '!*_gen*.c' --glob '!*.base' --glob '!tests/baseline/*' --glob '!tools/shux_migrate*.py' --glob '!tests/run-no-legacy-shux-gate.sh' \
...
# 7) VS Code 扩展：禁止 vscode-shulang / 旧 grammar su.tmLanguage
if rg -q 'vscode-shulang|su\.tmLanguage|snippets/su\.json' \
  --glob '!tests/run-no-legacy-shux-gate.sh' --glob '!tools/shux_migrate*.py' \
  editors 2>/dev/null; then
  bad "legacy vscode-shulang / su.tmLanguage in editors/"
  rg -n 'vscode-shulang|su\.tmLanguage|snippets/su\.json' editors 2>/dev/null | head -10 | sed 's/^/  /'
fi
...
# 8) 测试脚本/目录名：禁止 run-*-su-*、tests/su-pipeline（迁移扫尾）
legacy_test_paths=(
  tests/run-all-su.sh
