...
XLANG_COMP020_MANIFEST_ONLY=1 ./tests/run-comp-incr-compile-wave-gate.sh

if [ "${XLANG_COMP021_MANIFEST_ONLY:-0}" = "1" ]; then
...
while IFS=$'\t' read -r item_id kind anchor src tier _notes; do
  ...
  if ! comp_incr_prod_hook_runnable "$anchor"; then
    echo "comp-incr-compile-prod SKIP $item_id ($anchor no native xlang)"
    PROD_SKIP=$((PROD_SKIP + 1))
    continue
  fi

  if [ "$anchor" = "run-comp-incr-compile-gate.sh" ]; then
    if XLANG_COMP_INCR_COMPILE_MANIFEST_ONLY=1 "$hook"; then
      PROD_RUN=$((PROD_RUN + 1))
  ...
  if [ "$anchor" = "run-comp-incr-compile-wave-gate.sh" ]; then
    if XLANG_COMP020_MANIFEST_ONLY=1 "$hook"; then
      PROD_RUN=$((PROD_RUN + 1))
      echo "comp-incr-compile-prod runnable OK $item_id"
    else
