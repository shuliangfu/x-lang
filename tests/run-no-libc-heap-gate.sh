...
# 环境：
#   XLANG_NOLIBC_HEAP_FAIL=1  — 失败时硬退出
#   XLANG=./compiler/xlang      — freestanding 编译器
set -e
cd "$(dirname "$0")/.."

FAIL=${XLANG_NOLIBC_HEAP_FAIL:-0}
...
if [ ! -x "$XLANG" ]; then
  echo "nolibc-heap gate: SKIP (no xlang/xlang_asm)"
  exit 0
fi

rm -f "$OUT" 2>/dev/null || true
if ! "$XLANG" -freestanding -backend asm -o "$OUT" "$X" 2>/tmp/xlang_nolibc_heap.log; then
