...
$SHUX tests/if-expr/simple.sx -o /tmp/shux_if_simple 2>&1
...
$SHUX tests/if-expr/with_else.sx -o /tmp/shux_if_else 2>&1
exitcode=0; /tmp/shux_if_else >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 2 ] && { echo "expected 2 (with_else), got $exitcode"; exit 1; }

# if 1 { 42 } (无 else) -> 42
$SHUX tests/if-expr/no_else.sx -o /tmp/shux_if_no_else 2>&1
...
$SHUX tests/if-expr/else_if.sx -o /tmp/shux_if_else_if 2>&1
