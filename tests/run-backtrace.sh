#!/usr/bin/env bash
# backtrace smoke: product -o exit 0
#
...
PREFIX="${XLANG_BACKTRACE_PREFIX:-xlang: [BACKTRACE]}"
...
echo "=== backtrace gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

exe="/tmp/xlang_backtrace_$$"
log="/tmp/xlang_backtrace_$$.log"
rm -f "$exe" "$log"
[ -f "tests/backtrace/main.x" ] || die "missing tests/backtrace/main.x"
set +e
gate_run_timeout "$XLANG_CASE_TIMEOUT" "$XLANG_BIN" -L . "tests/backtrace/main.x" -o "$exe" >"$log" 2>&1
o_ec=$?
set -e
if [ "$o_ec" -eq 124 ]; then
