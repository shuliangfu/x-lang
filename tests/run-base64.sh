#!/usr/bin/env bash
# std.base64 leftover runner: tests/base64/main.x product -o exit 0.
#
# Honesty: leftover soft auto-make (`xlang_compiler_make -q || xlang_compiler_make`
# + ensure_std_c_o base64.o) + bootstrap-link wrap (prefer-c) retired.
...
# Product `-o` tests/base64/main.x must exit 0. Report: run=/obs=/skip=
# PLATFORM: SHARED archaeology — Ubuntu gold still required.
# Usage: ./tests/run-base64.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
PREFIX="${XLANG_BASE64_PREFIX:-xlang: [BASE64]}"
SMOKE="tests/base64/main.x"
RUN_OK=0
OBS=0
SKIP=0
...
echo "=== base64 leftover (prefer asm; hard; refuse leftover auto-make) ==="
if [ -n "${XLANG:-}" ]; then
...
set +e
"$XLANG_BIN" check -L . "$SMOKE" >/tmp/xlang_base64_check.log 2>&1
chk_ec=$?
set -e
if [ "$chk_ec" -ne 0 ]; then
  echo "base64 OBS check (paused / CHK residual ec=$chk_ec; refuse soft SKIP→OK)" >&2
  OBS=$((OBS + 1))
fi

exe="/tmp/xlang_base64_$$"
rm -f "$exe" 2>/dev/null || true
set +e
"$XLANG_BIN" -L . "$SMOKE" -o "$exe" >/tmp/xlang_base64_o.log 2>&1
o_ec=$?
set -e
if [ "$o_ec" -ne 0 ] || [ ! -x "$exe" ]; then
  tail -n 12 /tmp/xlang_base64_o.log 2>/dev/null || true
  rm -f "$exe"
