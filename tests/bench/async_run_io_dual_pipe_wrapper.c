/**
 * tests/bench/async_run_io_dual_pipe_wrapper.c — 双 pipe 注入 fd 3/4 后 exec .su 二进制
 *
 * 为 async_run_io_dual_pipe.su 提供可读 pipe 端：fd 3 ← "ab"，fd 4 ← "xyz"。
 *
 * 编译：cc -std=c11 -Wall -Wextra -o async_run_io_dual_pipe_wrapper \
 *   tests/bench/async_run_io_dual_pipe_wrapper.c
 * 用法：SHU_ASYNC_YIELD=1 ./async_run_io_dual_pipe_wrapper /tmp/shu_async_run_io_dual_pipe
 */
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
...
/**
 * 入口：建双 pipe → dup2 到 fd 3/4 → exec 用户 .su 可执行文件。
 */
