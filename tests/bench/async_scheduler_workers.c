 ...
 *
 * XLANG_ASYNC_WORKERS=2：两线程各 drain worker 0/1，验证隔离与并行消费。
 *
...
extern int xlang_async_task_submit_to(uint32_t worker_id, int32_t (*fn)(void));
extern int32_t xlang_async_worker_drain(uint32_t worker_id);
extern void xlang_async_queue_reset(void);
extern uint32_t xlang_async_worker_pending(uint32_t worker_id);
...
static void *drain_thread(void *arg) {
    ...
    a->got = xlang_async_worker_drain(a->worker_id);
    return NULL;
}
...
int main(void) {
    ...
    setenv("XLANG_ASYNC_WORKERS", "2", 1);
    xlang_async_queue_reset();

    if (xlang_async_task_submit_to(0, task_w0) != 0) {
        fprintf(stderr, "async_scheduler_workers: submit w0 failed\n");
        return 1;
    }
    if (xlang_async_task_submit_to(1, task_w1) != 0) {
        fprintf(stderr, "async_scheduler_workers: submit w1 failed\n");
        return 2;
    }
    if (xlang_async_worker_pending(0) != 1 || xlang_async_worker_pending(1) != 1) {
        fprintf(stderr, "async_scheduler_workers: pending w0=%u w1=%u want 1/1\n",
            (unsigned)xlang_async_worker_pending(0), (unsigned)xlang_async_worker_pending(1));
        return 3;
    }
    ...
    if (xlang_async_worker_pending(0) != 0 || xlang_async_worker_pending(1) != 0) {
