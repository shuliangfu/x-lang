 ...
 * 在 liburing + 内核 6.0+ 下 xlang_io_uring_multishot_accept_hits() 应 > 0。
 *
...
extern unsigned xlang_io_uring_multishot_accept_hits(void);
extern void xlang_io_uring_multishot_accept_stats_reset(void);
...
int main(void) {
    ...
    setenv("XLANG_IO_URING_MULTISHOT_ACCEPT", "1", 1);
    xlang_io_uring_multishot_accept_stats_reset();
    ...
    hits = xlang_io_uring_multishot_accept_hits();
    if (hits == 0) {
