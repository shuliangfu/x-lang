/**
 * tests/bench/async_runtime_trace_smoke.c — OBS-002 烟测：SHUX_ASYNC_RUNTIME_TRACE Top-N 输出
 *
 ...
 * 运行：SHUX_ASYNC_RUNTIME_TRACE=1 ./async_runtime_trace_smoke
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
extern void shux_async_runtime_trace_flush(void);
...
int main(void) {
    ...
    setenv("SHUX_ASYNC_RUNTIME_TRACE", "1", 1);
    setenv("SHUX_ASYNC_RUNTIME_TRACE_TOPN", "5", 1);
    setenv("SHUX_ASYNC_RUNTIME_TRACE_SAMPLE", "1", 1);

    g_step = 0;
    if (shux_async_task_submit(trace_smoke_task) != 0) {
    ...
    shux_async_runtime_trace_flush();
    return 0;
}
