/**
 * tests/bench/async_run_i32_io_stdin.c — shu_async_run_i32 + IO await stdin 烟测
 *
 ...
 * 运行：printf abcd | SHU_ASYNC_YIELD=1 ./async_run_i32_io_stdin
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
static int32_t read_four_task(void) {
    ...
    if (g_step == 0) {
        g_step = 1;
        g_phase = 0;
        g_slot = shu_io_submit_read_async(g_buf, 4, 0);
        if (g_slot < 0)
            return -1;
...
int main(void) {
    ...
    r = shu_async_run_i32(read_four_task);
    if (r != 4) {
