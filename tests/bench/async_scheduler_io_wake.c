 ...
 *
 * SHUX_ASYNC_YIELD=1 + SHUX_ASYNC_IO_WAIT=1：suspend 任务进 IO 等待队列；
 * shux_async_io_wake_all 后 drain 完成。
 *
...
int main(void) {
    ...
    setenv("SHUX_ASYNC_IO_WAIT", "1", 1);

    shux_async_queue_reset();
    g_step = 0;

    if (shux_async_task_submit(io_task) != 0) {
