...
# _main 反汇编不得含对用户函数的 bl（CALL 内联）；允许 bl _xlang_panic_（向量除法逐 lane 除零检查）。
check_no_bl_in_main() {
  local out="$1"
  local tag="$2"
  local main_asm
