...
MODULES=(
  ...
  # labi_freestanding_list R2 full：.x 吃满 env/io_sym/panic/ensure + wave117 heap/nostdlib + wave136 gen needs + wave137 compress + wave138 core_slice/db + wave139 provides_core_mem/std_heap + wave141 needs_win32/wsa + wave142 needs_core_builtin/mem stub0 + wave143 needs_async_scheduler + wave144 freestanding needs_io/panic + wave159 freestanding_enabled + wave167 ensure_crt0_user_o + wave168 ensure_freestanding_io_o + wave175 contains_substr + wave176 contains_substr_use_line + wave177 any_substr_use_line 纯表+orch；
  # 产品 rest 在 FROM_X 下业务 H=0（仅 marker）；Cap residual：buf scan/use_line + undef_sym + resolve/access/cc/stat（ensure 叶）
