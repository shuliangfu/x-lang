...
MODULES=(
  ...
  # lsp_io_std_heap：Track L 退役叶子；prove 锁 nm vs 入库 seed pin（禁工作区 gitignore 的 *_gen.c）
  # PLATFORM: SHARED — seed 不可再声明 uint8_t* malloc 与 stdlib void* 冲突（Ubuntu seed cc）
  ...
  # labi_path_pure R2 full：.x 吃满 21 公共门闩 + count（wave116 try_under + wave146–151 + wave160 compiler_o_path_copy + wave162 repo_root + wave163 panic_o_path + wave164 crt0_user_o_path + wave165 freestanding_io_o_path）；
  # 产品 rest 在 FROM_X 下业务 H=0（仅 marker）；Cap residual：Windows #if sep 在 mega 冷路径；bank_push/rel/skip Cap；resolve Cap（wave160）；resolve+rel Cap（wave162）；realpath+getcwd+skip Cap（wave163）
  ...
  # + wave158 invoke_cc_append_net_tls_ld pure orch（exports_marker/realpath/rel_o/push_existing Cap）；
  # 产品 rest 在 FROM_X 下业务 H=0（仅 marker）；Cap residual：host_is_apple + needs+ensure+path + push_existing + exports_marker/realpath/rel_o + spawn/ld/cc IO mega
  ...
  # labi_freestanding_list R2 full：.x 吃满 env/io_sym/panic/ensure + wave117 heap/nostdlib + wave136 gen needs + wave137 compress + wave138 core_slice/db + wave139 provides_core_mem/std_heap + wave141 needs_win32/wsa + wave142 needs_core_builtin/mem stub0 + wave143 needs_async_scheduler + wave144 freestanding needs_io/panic + wave159 freestanding_enabled + wave167 ensure_crt0_user_o + wave168 ensure_freestanding_io_o 纯表+orch；
  # 产品 rest 在 FROM_X 下业务 H=0（仅 marker）；Cap residual：contains_substr/undef_sym + resolve/access/cc/stat（ensure 叶）
  ...
  # labi_ensure_list R2 full：.x 吃满 ensure catalog 纯表（26 条目 stem/out/seed/flags/step_at）+ wave169 ensure_runtime_panic_o pure orch；
  # 产品 rest 在 FROM_X 下业务 H=0（仅 marker）；Cap residual：spawn/cc IO 在 mega ensure_from_catalog；panic ensure resolve/access/cc/stat + host linux_x86_64/posix_aarch64
