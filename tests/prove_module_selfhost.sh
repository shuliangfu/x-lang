...
MODULES=(
  ...
  # labi_path_pure R2 full：.x 吃满 18 公共门闩 + count（wave116 try_under + wave146–151 + wave160 compiler_o_path_copy + wave162 repo_root）；
  # 产品 rest 在 FROM_X 下业务 H=0（仅 marker）；Cap residual：Windows #if sep 在 mega 冷路径；bank_push/rel/skip Cap；resolve Cap（wave160）；resolve+rel Cap（wave162）
