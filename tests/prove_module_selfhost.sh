...
MODULES=(
  ...
  #   rest FROM_X 无 pure-dup flag_slot/path-len/format/set_at/os_lit/begin/end/flush _impl；
  # Cap residual：uname/setrlimit/pthread/path-read IO/debug_pipe/gettimeofday
  ...
  #  lib path slots + full try_append）：
  # thin.x 吃满 lit/entry + pure 真体（path_should_ignore / .x 后缀 / lint / file_list_push /
  #  process_child / collect_paths / default_dirs / check_one_file 门闩 / try_append 全 pure /
  #  parse_ignore 全量 / invoke_compile·dep_clear / set_current_file / print_collected /
  #  cwd_fallback / try_walk / path_resolve_abs / append_repo / missing_diag / collect_mode /
  #  user_passed_L / init / file_list_n / user_ignore_count / lib_bufs_n / user_ignore_at /
  #  lib_buf_at/store）；
  # rest FROM_X 无 pure-dup _impl；Cap residual：walk/stat/argv/file_list ptrs/one_file_body
