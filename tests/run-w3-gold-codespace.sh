...
#   - 可观测：tee 写 log + 心跳；非 TTY 下 xlang -o 不挂死
#
# 用法（Codespace 仓库根）：
#   ./tests/run-w3-gold-codespace.sh              # 全链
#   XLANG_W3_RESUME_FROM=l5 ./tests/run-w3-gold-codespace.sh   # 仅 L5+P1
#   XLANG_W3_RESUME_FROM=p1 ./tests/run-w3-gold-codespace.sh   # 仅 P1（L5 已过）
#   ./tests/run-w3-gold-codespace.sh status     # 查看是否在跑
#   ./tests/run-w3-gold-codespace.sh stop       # 停止 gate + 清理 xlang

set -euo pipefail
cd "$(dirname "$0")/.."

LOG="${XLANG_W3_GOLD_LOG:-logs/gold-inner.log}"
PIDFILE="${XLANG_W3_GOLD_PIDFILE:-logs/gold-inner.pid}"
WALL_SEC="${XLANG_W3_GOLD_WALL_SEC:-2700}"

mkdir -p logs

# 清理孤儿 xlang 进程（Codespace fork 上限易触顶）。
_w3_cleanup_xlang() {
...
echo "w3-gold: starting wall=${WALL_SEC}s log=$LOG resume=${XLANG_W3_RESUME_FROM:-full}"
rm -f "$LOG"
...
nohup bash -c "
  ...
  if command -v timeout >/dev/null 2>&1; then
    timeout -k 60 ${WALL_SEC} env XLANG_W3_RESUME_FROM=\"${XLANG_W3_RESUME_FROM:-}\" \
      ./tests/run-linux-a09-a11-gate-inner.sh 2>&1 | tee -a \"${LOG}\" | cat >/dev/null
  else
    env XLANG_W3_RESUME_FROM=\"${XLANG_W3_RESUME_FROM:-}\" \
      ./tests/run-linux-a09-a11-gate-inner.sh 2>&1 | tee -a \"${LOG}\" | cat >/dev/null
  fi
  echo EXIT=\$? >> \"${LOG}\"
