...
for f in "$DOC" "$WAVE" "$XPLAT" "$LIB" "$PARENT_DOC" "$MIN_INC" "$MOD_SU" \
...
if ! grep -qF 'STD-062' "$MOD_SU" 2>/dev/null; then
  echo "std-regex-perf gate FAIL: missing STD-062 anchor in $MOD_SU" >&2
  exit 1
fi
