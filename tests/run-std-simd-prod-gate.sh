#!/usr/bin/env bash
# STD-061: std.simd shuffle/select production bench gate — honesty soft fallthrough →硬绿.
#
# Honesty: soft XLANG fallthrough (explicit-bad still picks another binary) +
# soft auto-make + check=/bench=/skip=/ratio= retired. Prefer product xlang_asm;
# pin XLANG_LINK_XLANG. Explicit bad XLANG / missing native asm = hard die
# (refuse soft SKIP→OK / soft auto-make / prefer-c). Product
