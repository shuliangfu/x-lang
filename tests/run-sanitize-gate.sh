...
#   XLANG=./compiler/xlang-c ./tests/run-sanitize-gate.sh
set -e
cd "$(dirname "$0")/.."
...
mkdir -p "$OUT_DIR"
NORM_OUT="$OUT_DIR/xlang_sanitize_norm.c"
SAN_OUT="$OUT_DIR/xlang_sanitize_asan.c"
...
"$XLANG_BIN" -E "$SRC" >"$NORM_OUT" 2>/dev/null
"$XLANG_BIN" -fsanitize=address -E "$SRC" >"$SAN_OUT" 2>/dev/null
...
UB_OUT="$OUT_DIR/xlang_sanitize_oob"
rm -f "$UB_OUT"
if ! "$XLANG_BIN" -fsanitize=address tests/ub/bounds_array.x -o "$UB_OUT" 2>/dev/null; then
