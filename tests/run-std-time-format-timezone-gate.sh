#!/usr/bin/env bash
# STD-137：std.time 墙钟格式化与时区偏移门禁（假权威诚实）。
#
# 用法：./tests/run-std-time-format-timezone-gate.sh
# wave honesty (2026-08-24): DOC defaults under analysis/archive/ when archived;
# live roadmap = analysis/自举进度.md (NEXT.md left; refuse resurrect).
# 2026-08-26: Prefer xlang_asm; pin XLANG_LINK_XLANG; check observational SKIP
# (check gate paused 2026-08-05); format_timezone.x exit 0 hard-fail (no soft
...
if [ -f analysis/std-time-format-timezone-v1.md ]; then
  echo "std-time-format-timezone gate FAIL: top-level DOC resurrected (live = archive/std/)" >&2
  exit 1
fi

for f in "$DOC" "$MANIFEST" "$LIB" "$MOD_X" "$TIME_X" "$TIME_RUNTIME" "$SMOKE_X"; do
  if [ ! -f "$f" ]; then
    echo "std-time-format-timezone gate FAIL: missing $f" >&2
    exit 1
  fi
done
...
for kw in STD-137 format_timezone wall_local_offset_min format_wall_rfc3339; do
  if ! grep -qF -- "$kw" "$DOC" 2>/dev/null; then
    echo "std-time-format-timezone gate FAIL: doc missing '$kw'" >&2
    exit 1
  fi
done

if ! grep -qF '## 3. Gate' "$DOC" 2>/dev/null; then
  echo "std-time-format-timezone gate FAIL: doc missing '## 3. Gate'" >&2
  exit 1
fi

sym_miss="$(std_time_format_tz_symbols_ok "$MOD_X" "$TIME_X" "$MANIFEST" || true)"
if [ "${sym_miss:-0}" -gt 0 ]; then
  std_time_format_tz_emit_report "fail" 0 0 1
  echo "std-time-format-timezone gate FAIL: symbol_miss=${sym_miss}" >&2
  exit 1
fi
echo "std-time-format-timezone manifest OK"

if [ "${XLANG_STD137_TIME_FORMAT_TZ_MANIFEST_ONLY:-0}" = "1" ]; then
  std_time_format_tz_emit_report "ok" 0 0 1
...
echo "=== STD-137: time c smoke (observational) ==="
C_NOTE=0
xlang_compiler_make ../std/time/time.o ../std/datetime/datetime.o runtime_time_os.o >/dev/null 2>&1 || true
...
if [ -f "$TIME_O" ] && [ -f "$DT_O" ] && nm "$TIME_O" 2>/dev/null | grep -qF 'time_format_timezone_smoke_c'; then
  if std_time_format_tz_run_c_smoke "$TIME_O" "$DT_O"; then
    ...
    echo "std-time-format-timezone gate SKIP c smoke (observational; link)" >&2
  fi
else
  echo "std-time-format-timezone gate SKIP c smoke (observational; .o / symbol)" >&2
fi
echo "std-time-format-timezone c_smoke_note=${C_NOTE}"

if XLANG_BIN="$(resolve_shu 2>/dev/null)"; then
  echo "=== STD-137: smoke (XLANG=$XLANG_BIN; check observational; runnable hard) ==="
  if "$XLANG_BIN" check -L . "$SMOKE_X" >/dev/null 2>&1; then
    CHECK_OK=1
  else
    echo "std-time-format-timezone gate SKIP check smoke (paused 2026-08-05)" >&2
  fi
  ...
  xlang_compiler_make -q runtime_time_os.o 2>/dev/null || xlang_compiler_make runtime_time_os.o 2>/dev/null || true
  ...
  if $RUN_XLANG build -L . "$SMOKE_X" -o "$OUT" 2>"$LOG"; then
    ...
    if [ "$exitcode" -eq "$SMOKE_EXPECT" ]; then
      ...
      echo "std-time-format-timezone gate FAIL runnable exit=$exitcode (expect $SMOKE_EXPECT)" >&2
      std_time_format_tz_emit_report "fail" "$CHECK_OK" 0 0
      exit 1
    fi
  else
    echo "std-time-format-timezone gate FAIL runnable link" >&2
    tail -20 "$LOG" 2>/dev/null >&2 || true
    std_time_format_tz_emit_report "fail" "$CHECK_OK" 0 0
    exit 1
  fi
else
  echo "std-time-format-timezone gate FAIL: no native xlang" >&2
  std_time_format_tz_emit_report "fail" 0 0 0
  exit 1
fi

# check stays observational; hard-green signal is run= (runnable).
echo "std-time-format-timezone check_ok=${CHECK_OK} (observational)"
std_time_format_tz_emit_report "ok" "$CHECK_OK" "$RUN_OK" "$SKIP"
