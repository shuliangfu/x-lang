...
set -e
cd "$(dirname "$0")/.."
DOC="analysis/archive/std/std-time-format-timezone-v1.md"
...
if [ -x ./compiler/xlang-c ] || [ -x ./compiler/xlang ]; then
  ...
  echo "std-time-format-timezone gate SKIP c smoke (no xlang-c; manifest OK)" >&2
  SKIP=1
fi
if [ -x ./compiler/xlang-c ]; then
  ./compiler/xlang-c check -L . "$SMOKE_X" >/dev/null
  std_time_format_tz_run_smoke ./compiler/xlang-c "$SMOKE_X" && X_OK=1 || exit 1
else
  SKIP=1
fi
