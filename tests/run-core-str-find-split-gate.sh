...
MOD_SX="core/str/mod.sx"
LIB="tests/lib/core-str-find-split.sh"
SMOKE_SX="tests/str/find_split.sx"
. "$LIB"
...
sym_miss="$(core_str_find_split_symbols_ok "$MOD_SX" "$MANIFEST" || true)"
...
if [ -x ./compiler/shux-c ]; then
  ...
  if ci_is_darwin; then
    ...
    SX_OK=1
    SKIP=1
  elif core_str_find_split_run_smoke "$RUN_SHUX" "$SMOKE_SX"; then
    SX_OK=1
  else
...
core_str_find_split_emit_report ok "$SX_OK" "$SKIP"
