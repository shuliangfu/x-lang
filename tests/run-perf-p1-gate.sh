...
XLANG_PERF_FAIL_ON_IO_ZIG=1 XLANG_PERF_FAIL_ON_IO_REGRESSION=1 ./tests/run-perf-io.sh --bench | tee /tmp/perf_p1_io.log
...
if [ "$(uname -s)" = "Linux" ]; then
  ...
  if io_uring_available; then
    ...
    XLANG_PERF_FAIL_ON_NET_REGRESSION=1 ./tests/run-perf-net.sh --bench | tee /tmp/perf_p1_net.log
