#!/usr/bin/env bash
# F-04 v1：std.net TLS stub 去 C 门禁（tls_stub.sx + 无 tls_stub.inc.c）。
#
...
[ -f "$TLS_STUB" ] || die "missing tls_stub.sx"
...
grep -q 'import("std.net.tls_stub")' "$NET_MOD" || die "mod.sx missing tls_stub import"
if grep -q 'extern function net_tls_is_available_c' "$NET_MOD" 2>/dev/null; then
  die "mod.sx still extern net_tls_is_available_c"
fi
