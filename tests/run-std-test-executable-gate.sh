...
SMOKE_SU="tests/std-test/bench_fuzz_exec.su"
...
for kw in STD-143 bench_run_noop fuzz_run_noop SHU_BENCH; do
...
sym_miss="$(std_test_executable_symbols_ok "$MOD_SU" "$TEST_C" "$MANIFEST" || true)"
...
if [ -n "$SHU_BIN" ]; then
  ...
  if std_test_executable_run_smoke "$SHU_BIN" "$SMOKE_SU" "$TEST_O"; then
    EXEC_OK=1
  else
    std_test_executable_emit_report "fail" 0 0
    exit 1
