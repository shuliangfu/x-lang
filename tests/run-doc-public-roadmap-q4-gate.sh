...
DOC="${XLANG_DOC010_DOC:-analysis/archive/doc/doc-public-roadmap-q4-v1.md}"
...
case "$q_manifest" in
  2026-Q4|2027-Q1|2027-Q2|2027-Q3|2027-Q4)
    ;;
  *)
    echo "doc-public-roadmap-q4 gate FAIL: doc005 manifest quarter=$q_manifest want 2026-Q4+ after refresh (live may roll forward)" >&2
    doc_roadmap_q4_emit_report "fail" "$QUARTER" 0
    exit 1
    ;;
esac
...
if ! doc_roadmap_quarter_ok "$DOC" "$QUARTER"; then
  echo "doc-public-roadmap-q4 gate FAIL: quarter DOC missing $QUARTER" >&2
  doc_roadmap_q4_emit_report "fail" "$QUARTER" 0
  exit 1
fi
...
if [ ! -f "$TEMPLATE" ]; then
  echo "run-doc-public-roadmap-q4-gate.sh FAIL: missing template" >&2
  doc_roadmap_q4_emit_report "fail" "$QUARTER" 0
  exit 1
fi
