...
if [ ! -f compiler/docs/BUILD_DAG.md ]; then
  ...
  bad "missing compiler/scripts/ensure_host_cc_seed_o.sh (wave748–752 R1 families)"
elif ! grep -q 'ensure_host_cc_seed_o\.sh' compiler/Makefile; then
  bad "Makefile must thin-call ensure_host_cc_seed_o.sh (wave748–752)"
...
elif ! grep -qE 'host-cc-seed|rt-seed-slice|core-seed|frontend-glue|main-runtime|alias-stubs' xlang-build.sh \
  || ! grep -q 'ensure_host_cc_seed_o\.sh' xlang-build.sh; then
  bad "xlang-build missing host-cc-seed / families (wave748–752)"
  ...
  note "BUILD_DAG + ensure_prereqs + product-dag + PLATFORM_LINKER + LEAF_PATTERN + R1 families (wave744–752)"
  ...
  if ! ./xbuild leaf-patterns --check >/tmp/xbuild_leaf_patterns_check.out 2>/tmp/xbuild_leaf_patterns_check.err; then
    bad "xbuild leaf-patterns --check failed (wave752)"
  elif ! grep -q 'CHECK OK' /tmp/xbuild_leaf_patterns_check.out /tmp/xbuild_leaf_patterns_check.err; then
    bad "xbuild leaf-patterns --check missing CHECK OK (wave752)"
  else
    note "xbuild leaf-patterns --check OK (wave747 R4 + wave748–752 R1)"
  fi
  ...
  if ! printf '%s\n' "$_leaf_out" | grep -q 'RESIDUAL_CLASS_R1=host_cc_seed_from_x_to_o'; then
    ...
    note "xbuild leaf-patterns inventory OK (wave747 R4 + wave748–752 R1 families)"
  fi
  if ! ./xbuild host-cc-seed --check >/tmp/xbuild_host_cc_seed_check.out 2>/tmp/xbuild_host_cc_seed_check.err; then
    bad "xbuild host-cc-seed --check failed (wave752)"
  elif ! grep -q 'CHECK OK' /tmp/xbuild_host_cc_seed_check.out /tmp/xbuild_host_cc_seed_check.err; then
    bad "xbuild host-cc-seed --check missing CHECK OK (wave752)"
  else
    note "xbuild host-cc-seed --check OK (wave748–752 R1 families)"
  fi
  if ! bash compiler/scripts/driver_seed_obj_catalog.sh --check >/tmp/xbuild_catalog_rt.out 2>/tmp/xbuild_catalog_rt.err; then
    bad "driver_seed_obj_catalog --check failed (wave748–752 R1 family keys)"
  else
    note "driver_seed_obj_catalog --check OK (includes RT_SEED_SLICE + R1_CORE_SEED + R1_FRONTEND_GLUE + R1_MAIN_RUNTIME + R1_ALIAS_STUBS)"
  fi
