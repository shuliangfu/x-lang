#!/usr/bin/env bash
# 文件职责：阶段 1 词法测试——对 tests/lexer/main.x 运行 xlang，将输出与 tests/lexer/expected.txt 逐行比对。
# 与其它文件的关系：由 compiler/Makefile 的 test 目标调用；依赖 compiler/xlang、tests/lexer/main.x、tests/lexer/expected.txt。
# 使用 xlang-c（C 版编译器）以保证无 -o 时输出 token 列表 + parse/typeck OK；若当前 xlang 为 pipeline 版则无该行为。
# 当 XLANG 已设置（test_x/run-all-x）时仅用 compiler/xlang（xlang_x），不构建 xlang-c，并跳过依赖 token 输出的比对。

set -e
cd "$(dirname "$0")/.."
if [ -n "$XLANG" ]; then
  # test_c 传 XLANG=./compiler/xlang-c 时用其跑负例；test_x 传 XLANG=./compiler/xlang 时用 xlang_x
  LEXER_XLANG="$XLANG"
  out=$(mktemp)
  # bootstrap-driver-seed / stage 的 xlang 走 .x 单文件 -o 时可能不跑完整 C 前端，lexer 负例会误成功；有 xlang-c 时非法字符负例仍用 xlang-c，与 Makefile test_c 一致。
  NEG_XLANG="$XLANG"
  if [ -x ./compiler/xlang-c ] && [ "${XLANG##*/}" != "xlang-c" ]; then
    NEG_XLANG=./compiler/xlang-c
  fi
  "$NEG_XLANG" build tests/lexer/invalid_char.x -o /tmp/xlang_lexer_fail 2>/dev/null && { echo "lexer: expected compile failure for invalid char"; exit 1; }
  rm -f "$out"
else
  make -C compiler xlang-c 2>/dev/null || true
  if [ -f ./compiler/xlang-c ]; then
    LEXER_XLANG=./compiler/xlang-c
  else
    make -C compiler
    LEXER_XLANG=./compiler/xlang
  fi
  out=$(mktemp)
  "$LEXER_XLANG" tests/lexer/main.x > "$out"
  diff -u --strip-trailing-cr tests/lexer/expected.txt "$out" || exit 1
  "$LEXER_XLANG" tests/lexer/comments.x > "$out"
  diff -u --strip-trailing-cr tests/lexer/expected-comments.txt "$out" || exit 1
  rm -f "$out"
  "$LEXER_XLANG" build tests/lexer/invalid_char.x -o /tmp/xlang_lexer_fail 2>/dev/null && { echo "lexer: expected compile failure for invalid char"; exit 1; }
fi
echo "lexer test OK"
