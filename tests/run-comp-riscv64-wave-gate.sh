...
if [ "${XLANG_COMP016_MANIFEST_ONLY:-0}" = "1" ]; then
...
XLANG_COMP_RISCV64_MANIFEST_ONLY=1 ./tests/run-comp-riscv64-gate.sh
...
if [ -n "$XLANG_BIN" ]; then
  echo "=== COMP-016: wave asm_text (XLANG=$XLANG_BIN) ==="
  HOST_SKIP=0
  while IFS=$'\t' read -r case_id sample _check _expect policy _notes; do
    [ -z "${case_id:-}" ] && continue
    case "$case_id" in \#*|min_*) continue ;; esac
    [ "${policy:-}" = "wave" ] || continue
    if comp_riscv64_wave_run_case "$XLANG_BIN" "$sample"; then
      WAVE_OK=$((WAVE_OK + 1))
  ...
  echo "comp-riscv64-wave gate SKIP runnable (no native xlang)" >&2
  WAVE_SKIP=$MIN_WAVE
fi
