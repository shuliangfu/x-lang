...
# 3) Linux + perf + native xlang：烟测 check_typeck 须输出 ≥ min_top_n 行
#
# 用法：./tests/run-perf-flamegraph-gate.sh
set -e
cd "$(dirname "$0")/.."

DOC="${XLANG_PERF_FLAMEGRAPH_DOC:-analysis/perf-flamegraph-v1.md}"
...
if ! perf_fg_probe_ok || [ -z "$XLANG_BIN" ]; then
  echo "perf-flamegraph gate SKIP hooks (perf or native xlang unavailable; host=$(ci_host_summary))" >&2
