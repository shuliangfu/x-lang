...
SMOKE_SX="tests/std-config/layer_smoke.sx"
...
for f in "$DOC" "$MANIFEST" "$LIB" "$MOD_SX" "$CFG_SX" "$SMOKE_SX" "$SMOKE_C" std/config/README.md; do
  if [ ! -f "$f" ]; then
...
sym_miss="$(std_config_symbols_ok "$MOD_SX" "$CFG_SX" "$MANIFEST" || true)"
...
if [ -x ./compiler/shux-c ] || [ -x ./compiler/shux ]; then
  ...
  echo "std-config gate SKIP c smoke (need shux-c for config.sx merge)" >&2
  SKIP=1
fi
...
if [ -n "$SHUX_BIN" ]; then
  ...
  if std_config_run_smoke "$SHUX_BIN" "$SMOKE_SX" "layer"; then SX_OK=1; else
    std_config_emit_report "fail" "$C_OK" 0 0
    exit 1
  fi
else
...
std_config_emit_report "ok" "$C_OK" "$SX_OK" "$SKIP"
