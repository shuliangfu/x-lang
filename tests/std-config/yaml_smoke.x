...
function main(): i32 {
  ...
  let raw: u8[88] = [
    ...
    10,
    100, 98, 58, 10,
    32, 32, 117, 114, 108, 58, 32, 115, 113, 108, 105, 116, 101, 58, 47, 47, 109, 101, 109, 10
  ];
  ...
  if (config.get_bool(&cfg, &k_dbg[0], 5, &dbg) != config.err_ok() || dbg != 0) { return 7; }
  if (config.yaml_smoke() != 0) { return 8; }
  config.free(&cfg);
  return 0;
}
