...
C_OUT="/tmp/xlang_autovec_dot_loop.c"
rm -f "$OUT" "$C_OUT"
if ! "$XLANG" build -E "$SRC" >"$C_OUT" 2>/tmp/xlang_autovec_emit.log; then
  echo "autovec-loop-gate FAIL: emit failed" >&2
  tail -8 /tmp/xlang_autovec_emit.log 2>/dev/null || true
  exit 1
fi
if ! grep -q 'xlang_autovec_dot_f32' "$C_OUT"; then
  echo "autovec-loop-gate FAIL: missing xlang_autovec_dot_f32 in generated C" >&2
  exit 1
fi
...
if XLANG_NO_AUTovec=1 "$XLANG" build -E "$SRC" >"${C_OUT}.scalar" 2>/dev/null; then
  if ! grep -qE 'for \(.*ap\)\[i\]|ap\)\[i\].*\*.*bp\)\[i\]' "${C_OUT}.scalar"; then
    echo "autovec-loop-gate WARN: XLANG_NO_AUTovec=1 did not preserve scalar loop" >&2
  fi
fi
XLANG_KEEP_C=1 "$XLANG" "$SRC" -o "$OUT" 2>/tmp/xlang_autovec_run.log || {
  echo "autovec-loop-gate FAIL: compile/run failed" >&2
  tail -8 /tmp/xlang_autovec_run.log 2>/dev/null || true
  exit 1
}
rc=0
"$OUT" >/dev/null 2>&1 || rc=$?
