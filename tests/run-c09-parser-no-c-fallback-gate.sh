...
#
# v1 不要求 mega7 七函数全部 SX emit；生产仍 C glue 见 analysis/boot-mega7-gap.md。
#
...
echo "=== C-09: delegate C-06 sx frontend default ==="
chmod +x tests/run-c06-sx-frontend-default-gate.sh
SHUX_C06_FAIL="$FAIL" ./tests/run-c06-sx-frontend-default-gate.sh || die "C-06 sub-gate failed"

# ── strict 链：build_shux_asm 强制 parser_sx.o 覆盖 seed parser.o ──
echo "=== C-09: strict link ensure_parser_sx_o ==="
grep -q 'ensure_parser_sx_o_for_strict_link()' "$BUILD_ASM" || die "build_shux_asm missing ensure_parser_sx_o_for_strict_link"
grep -q 'ensure_parser_sx_o_for_strict_link' "$BUILD_ASM" || die "build_shux_asm never calls ensure_parser_sx_o_for_strict_link"

# ── pipeline_sx：LINK_OBJS 以 parser_sx.o 为 parser TU ──
echo "=== C-09: PIPELINE_SX_LINK_OBJS uses parser_sx.o ==="
LINK_LINE=$(grep -E '^PIPELINE_SX_LINK_OBJS\s*=' "$MF" | head -1)
[ -n "$LINK_LINE" ] || die "missing PIPELINE_SX_LINK_OBJS"
echo "$LINK_LINE" | grep -q 'parser_sx\.o' || die "PIPELINE_SX_LINK_OBJS missing parser_sx.o"
echo "$LINK_LINE" | grep -qE 'src/parser/parser\.o' && die "PIPELINE_SX_LINK_OBJS still embeds src/parser/parser.o"
...
# ── parser.sx：mega7 七函数名存在（登记，不要求真 emit）──
echo "=== C-09: parser.sx mega7 symbol anchors ==="
PARSER_SX="compiler/src/parser/parser.sx"
[ -f "$PARSER_SX" ] || die "missing $PARSER_SX"
for sym in parse_into_buf parse_into parse parse_one_function_impl parse_expr_into parse_block_into parse_body_lets_into; do
  grep -q "function ${sym}" "$PARSER_SX" || grep -q "${sym}(" "$PARSER_SX" || die "parser.sx missing mega7 anchor $sym"
done
...
echo "c09 parser-no-c-fallback gate OK (v1: default parser_sx.o + strict override + force_stub/mega7 manifest)"
