...
# 依次尝试 strict_glue / shux_asm / experimental，取 __text 最大且编译成功的产物。
...
try_emit_heavy() {
  local cand="$1"
  [ -x "$cand" ] || return 1
  if ! SHUX_S3_EMIT_HEAVY_COMPILER="$cand" ./tests/run-s3-pipeline-emit-heavy.sh >/tmp/s3_eh_try.log 2>&1; then
    return 1
  fi
  [ -f /tmp/shux_s3_pipeline_emit_heavy.o ] || return 1
  text_section_size /tmp/shux_s3_pipeline_emit_heavy.o
}
...
if [ -n "${SHUX_S3_EMIT_HEAVY_COMPILER:-}" ] && [ -x "${SHUX_S3_EMIT_HEAVY_COMPILER}" ]; then
  if sz=$(try_emit_heavy "${SHUX_S3_EMIT_HEAVY_COMPILER}"); then
    best_sz=$sz
    best_comp="${SHUX_S3_EMIT_HEAVY_COMPILER}"
    emit_heavy_ok=1
  fi
fi
...
export SHUX_S3_EMIT_HEAVY_COMPILER="$best_comp"
SHUX_S3_EMIT_HEAVY_COMPILER="$best_comp" ./tests/run-s3-pipeline-emit-heavy.sh

SRC="/tmp/shux_s3_pipeline_emit_heavy.o"
[ -f "$SRC" ] || {
