...
FAIL=${SHU_PARSER_PARSE_BOOTSTRAP_FAIL:-0}
CC=${CC:-cc}
...
BOOT_O="/tmp/shu_parser_parse_bootstrap_gate.$$.o"
rm -f "$BOOT_O" 2>/dev/null || true

echo "parser-parse-bootstrap-gate: cc parser_asm_parse_bootstrap_obj.c ..."
if ! $CC $CFLAGS -c -o "$BOOT_O" "$BOOT_SRC" > /tmp/shu_parser_boot_gate.log 2>&1; then
  echo "parser-parse-bootstrap-gate FAIL: cc compile" >&2
  tail -n 12 /tmp/shu_parser_boot_gate.log 2>/dev/null || true
  rm -f "$BOOT_O" /tmp/shu_parser_boot_gate.log 2>/dev/null || true
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi
...
MIN_TEXT=${SHU_PARSER_PARSE_BOOTSTRAP_MIN_TEXT:-512}
