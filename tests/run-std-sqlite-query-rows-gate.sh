...
DOC="${SHU_STD066_DOC:-analysis/std-sqlite-query-rows-v1.md}"
MANIFEST="${SHU_STD066_TSV:-tests/baseline/std-sqlite-query-rows.tsv}"
VECTORS="${SHU_STD066_VECTORS:-tests/baseline/std-sqlite-query-rows-vectors.tsv}"
MOD_SU="std/sqlite/mod.su"
DB_C="std/sqlite/sqlite.c"
LIB="tests/lib/std-sqlite-query-rows.sh"
SMOKE_SU="tests/std-sqlite/query_rows_roundtrip.su"
...
sym_miss="$(std_sqlite_query_rows_symbols_ok "$MOD_SU" "$DB_C" "$MANIFEST" || true)"
...
SHU_STD065_MANIFEST_ONLY=1 ./tests/run-std-sqlite-exec-deep-gate.sh

if [ "${SHU_STD066_MANIFEST_ONLY:-0}" = "1" ]; then
...
if std_sqlite_probe_libs; then
  ...
  if [ -n "$SHU_BIN" ]; then
    echo "=== STD-066: .su query_rows smoke (SHU=$SHU_BIN) ==="
    if ! "$SHU_BIN" check -L . "$SMOKE_SU" >/dev/null 2>&1; then
      echo "std-sqlite-query-rows gate SKIP .su smoke (typeck fail)" >&2
      SKIP=1
    elif std_sqlite_run_smoke "$SHU_BIN" "$SMOKE_SU" "rows"; then
      ROWS_SU=1
      SKIP=0
    else
      echo "std-sqlite-query-rows gate SKIP .su smoke (link/compile)" >&2
      SKIP=1
    fi
  else
    echo "std-sqlite-query-rows gate SKIP .su smoke (no native shu)" >&2
    SKIP=1
  fi
  std_sqlite_restore_default_o
else
...
std_sqlite_query_rows_emit_report "ok" "$ROWS_C" "$ROWS_SU" "$SKIP"
