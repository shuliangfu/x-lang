#!/usr/bin/env bash
# D-01 v1：Stage 0（seed）→ Stage 1（xlang_asm B-strict）门禁。
#
...
echo "=== D-01: Stage0 seed → Stage1 xlang_asm (v1) ==="
...
grep -q 'bootstrap-driver-seed' "$BUILD_ASM" || die "build_xlang_asm missing seed reference"
...
if ! d01_native_exe "$XLANG_ASM"; then
  echo "d01 stage0-to-stage1 gate: SKIP (no native $XLANG_ASM; manifest OK)"
  exit 0
fi

# Stage 1 须可执行且具备 asm 后端（区别于纯 xlang-c）
