#!/usr/bin/env bash
# STD-137：std.db.sqlite 大 BLOB 分块读门禁
#
# 用法：./tests/run-std-sqlite-blob-stream-gate.sh
set -e
cd "$(dirname "$0")/.."

DOC="${SHUX_STD137_DOC:-analysis/std-sqlite-blob-stream-v1.md}"
MANIFEST="${SHUX_STD137_TSV:-tests/baseline/std-sqlite-blob-stream.tsv}"
VECTORS="${SHUX_STD137_VECTORS:-tests/baseline/std-sqlite-blob-stream-vectors.tsv}"
MOD_X="std/db/sqlite/mod.x"
DB_C="std/db/sqlite/sqlite.x"
LIB="tests/lib/std-sqlite-blob-stream.sh"
SMOKE_X="tests/std-sqlite/blob_stream_roundtrip.x"
SMOKE_C="tests/std-sqlite/blob_stream_roundtrip_ok.c"
MIN_STREAM=2

# shellcheck source=tests/lib/std-sqlite-blob-stream.sh
. "$LIB"
std_sqlite_blob_stream_source_sqlite

echo "=== STD-137: db blob stream manifest ==="
for f in "$DOC" "$MANIFEST" "$VECTORS" "$LIB" "$MOD_X" "$DB_C" "$SMOKE_X" "$SMOKE_C" \
  analysis/std-sqlite-row-col-blob-v1.md tests/run-std-sqlite-row-col-blob-gate.sh; do
  if [ ! -f "$f" ]; then
    echo "std-sqlite-blob-stream gate FAIL: missing $f" >&2
    exit 1
  fi
done

for kw in STD-137 row_col_blob_len db_sqlite_blob_stream_smoke_c; do
  if ! grep -qF -- "$kw" "$DOC" 2>/dev/null; then
    echo "std-sqlite-blob-stream gate FAIL: doc missing '$kw'" >&2
    exit 1
  fi
done

while IFS=$'\t' read -r c1 c2 _rest; do
  c1="${c1#\# }"
  case "$c1" in
    min_stream_apis) MIN_STREAM="$c2" ;;
  esac
done < "$MANIFEST"

if ! grep -qF 'blob_stream_done' "$VECTORS" 2>/dev/null; then
  echo "std-sqlite-blob-stream gate FAIL: vectors missing blob_stream_done" >&2
  exit 1
fi

API_N=0
while IFS=$'\t' read -r item_id kind anchor _rest; do
  [ -z "${item_id:-}" ] && continue
  case "$item_id" in \#*|min_*) continue ;; esac
  [ "$kind" = "api" ] || continue
  API_N=$((API_N + 1))
  if ! grep -qF "$anchor" "$DOC" 2>/dev/null; then
    echo "std-sqlite-blob-stream FAIL: doc missing api $anchor" >&2
    exit 1
  fi
  echo "std-sqlite-blob-stream OK api $anchor"
done < "$MANIFEST"

if [ "$API_N" -lt "$MIN_STREAM" ]; then
  echo "std-sqlite-blob-stream gate FAIL: api count $API_N < min $MIN_STREAM" >&2
  exit 1
fi

sym_miss="$(std_sqlite_blob_stream_symbols_ok "$MOD_X" "$DB_C" "$MANIFEST" || true)"
if [ "${sym_miss:-0}" -gt 0 ]; then
  std_sqlite_blob_stream_emit_report "fail" 0 0 0
  exit 1
fi
echo "std-sqlite-blob-stream manifest OK"

echo "=== STD-137: parent STD-069 manifest ==="
chmod +x tests/run-std-sqlite-row-col-blob-gate.sh
SHUX_STD069_MANIFEST_ONLY=1 ./tests/run-std-sqlite-row-col-blob-gate.sh

if [ "${SHUX_STD137_MANIFEST_ONLY:-0}" = "1" ]; then
  std_sqlite_blob_stream_emit_report "ok" 0 0 1
  echo "std-sqlite-blob-stream gate OK (manifest only)"
  exit 0
fi

STREAM_C=0
STREAM_X=0
SKIP=1

if std_sqlite_probe_libs; then
  echo "=== STD-137: blob stream smoke ==="
  if ! std_sqlite_build_o; then
    std_sqlite_blob_stream_emit_report "fail" 0 0 0
    exit 1
  fi
  if std_sqlite_blob_stream_run_c_smoke "$DB_C"; then
    STREAM_C=1
  else
    std_sqlite_restore_default_o
    std_sqlite_blob_stream_emit_report "fail" 0 0 0
    exit 1
  fi

  SHUX_BIN=""
  stdlib_cm_native_shu() {
    local f="$1"
    [ -n "$f" ] && [ -x "$f" ] || return 1
    case "$(uname -s)-$(uname -m 2>/dev/null)" in
      Darwin-arm64) file "$f" 2>/dev/null | grep -qE 'Mach-O.*arm64' ;;
      Darwin-x86_64) file "$f" 2>/dev/null | grep -qE 'Mach-O.*x86_64' ;;
      Linux-x86_64|Linux-amd64) file "$f" 2>/dev/null | grep -qE 'ELF.*x86-64' ;;
      Linux-aarch64|Linux-arm64) file "$f" 2>/dev/null | grep -qE 'ELF.*aarch64|ELF.*ARM' ;;
      *) return 0 ;;
    esac
  }
  if stdlib_cm_native_shu ./compiler/shux-c; then
    SHUX_BIN=./compiler/shux-c
  elif stdlib_cm_native_shu ./compiler/shux; then
    SHUX_BIN=./compiler/shux
  fi

  if [ -n "$SHUX_BIN" ]; then
    echo "=== STD-137: .x blob stream smoke (SHUX=$SHUX_BIN) ==="
    if ! "$SHUX_BIN" check -L . "$SMOKE_X" >/dev/null 2>&1; then
      echo "std-sqlite-blob-stream gate SKIP .x smoke (typeck fail)" >&2
      SKIP=1
    elif std_sqlite_run_smoke "$SHUX_BIN" "$SMOKE_X" "blobstream"; then
      STREAM_X=1
      SKIP=0
    else
      echo "std-sqlite-blob-stream gate SKIP .x smoke (link/compile)" >&2
      SKIP=1
    fi
  else
    echo "std-sqlite-blob-stream gate SKIP .x smoke (no native shux)" >&2
    SKIP=1
  fi
  std_sqlite_restore_default_o
else
  echo "std-sqlite-blob-stream gate SKIP stream smoke (no libsqlite3)" >&2
  # shellcheck source=tests/lib/build-std-c-o.sh
  . tests/lib/build-std-c-o.sh
  ensure_std_c_o ../std/db/sqlite/sqlite.o
fi

std_sqlite_blob_stream_emit_report "ok" "$STREAM_C" "$STREAM_X" "$SKIP"
echo "std-sqlite-blob-stream gate OK"
