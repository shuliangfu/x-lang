#!/usr/bin/env bash
#
# 【文件职责】std.process 模块的全量回归测试脚本；按顺序编译并运行 tests/process/*.sx，校验退出码。
...
# 【覆盖用例】
#   1. main.sx     — exit(99)，校验进程退出码 99
#   2. args.sx     — args_count、arg(0)、getenv("PATH")
#   3. setenv_unsetenv.sx — setenv、getenv、unsetenv 全流程
#   4. getpid.sx   — getpid()>0
#   5. getppid.sx  — getppid() 可调用
#   6. getcwd.sx   — getcwd 成功且 buf 有效
#   7. chdir.sx    — chdir(".")、getcwd 前后一致
#   8. self_exe_path.sx — self_exe_path 成功且 buf 有效
#   8b. zerocopy.sx — getcwd_ptr/getcwd_cached_len、self_exe_path_ptr/self_exe_path_cached_len 零拷贝
#   9. spawn_wait.sx — spawn_simple("/bin/true")、waitpid(pid)==0（Windows 无此路径则 SKIP；Alpine 仅有 /bin/true）
#  10. exec_fail.sx — exec_simple("/nonexistent") 返回 -1
...
run_one "args" "tests/process/args.sx" || exit 1

# 3. setenv、getenv、unsetenv
run_one "setenv_unsetenv" "tests/process/setenv_unsetenv.sx" || exit 1

# 4. getpid
run_one "getpid" "tests/process/getpid.sx" || exit 1

# 5. getppid
run_one "getppid" "tests/process/getppid.sx" || exit 1

# 6. getcwd
run_one "getcwd" "tests/process/getcwd.sx" || exit 1

# 7. chdir
run_one "chdir" "tests/process/chdir.sx" || exit 1

# 8. self_exe_path
run_one "self_exe_path" "tests/process/self_exe_path.sx" || exit 1

# 8b. 零拷贝 getcwd_ptr / self_exe_path_ptr
run_one "zerocopy" "tests/process/zerocopy.sx" || exit 1

# 9. spawn_simple + waitpid（/bin/true 或 /usr/bin/true；Windows 无此路径则跳过）
if run_one "spawn_wait" "tests/process/spawn_wait.sx"; then
  :
else
...
# 10. exec_simple 失败路径
run_one "exec_fail" "tests/process/exec_fail.sx" || exit 1

# 11. STD-142 跨平台聚合烟测
run_one "xplat_behavior" "tests/process/xplat_behavior.sx" || exit 1
