...
# static / leftover nested xlang_compiler_make; refuse leftover ignore of
...
# leftover nested xlang_compiler_make (refuse leftover SKIP→OK /
# leftover ignore of explicit-bad / leftover XLANG fallthrough). leftover
# nested product path stays when XLANG is unset (do not rewrite leftover
# xlang_compiler_make).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
