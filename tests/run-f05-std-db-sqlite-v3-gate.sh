...
[ -f std/db/sqlite/sqlite.sx ] || die "missing sqlite.sx"
[ -f compiler/src/asm/runtime_sqlite_glue.c ] || die "missing sqlite_glue.c"
grep -q 'db_open_c' std/db/sqlite/sqlite.sx || die "sqlite.sx missing db_open_c"
grep -q 'db_sqlite_exec_smoke_c' std/db/sqlite/sqlite.sx || die "sqlite.sx missing smoke"
...
grep -q 'sqlite.sx' compiler/Makefile || die "Makefile missing sqlite.sx build"
...
EOF
  ...
  echo "f05 sqlite smoke SKIP (sqlite.o missing .sx symbols; need shux-c)"
fi
