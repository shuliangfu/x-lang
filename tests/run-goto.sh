#!/usr/bin/env bash
# label/goto parse+codegen smoke
#
...
#   - hard: product -o + expected exit 42
# Report: run=/obs=/skip=
...
PREFIX="${XLANG_GOTO_PREFIX:-xlang: [GOTO]}"
...
echo "=== goto gate (prefer asm; hard) ==="
...
exe="/tmp/xlang_goto_$$"
log="/tmp/xlang_goto_$$.log"
rm -f "$exe" "$log"
[ -f "tests/goto/main.x" ] || die "missing tests/goto/main.x"
set +e
gate_run_timeout "$XLANG_CASE_TIMEOUT" "$XLANG_BIN" -L . "tests/goto/main.x" -o "$exe" >"$log" 2>&1
o_ec=$?
set -e
if [ "$o_ec" -eq 124 ]; then
