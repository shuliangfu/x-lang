...
# std-task; refuse leftover ignore of explicit-bad). leftover nested
# product path stay.
...
# leftover nested ensure / leftover nested std-task (refuse leftover
...
# leftover xlang_compiler_make / std-task).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
