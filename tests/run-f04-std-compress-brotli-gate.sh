#!/usr/bin/env bash
# F-04 v6：std.compress brotli 去 C 门禁（lib.sx + 无 brotli.c）。
#
...
BROTLI_LIB="std/compress/brotli/lib.sx"
...
[ -f "$BROTLI_LIB" ] || die "missing lib.sx"
...
grep -q 'import("std.compress.brotli.lib")' "$BROTLI_MOD" || die "mod.sx missing lib import"
