#!/usr/bin/env bash
# F-04 v6：std.compress brotli 去 C 门禁（brotli_lib.sx + 无 brotli.c）。
#
...
BROTLI_LIB="std/compress/brotli/brotli_lib.sx"
BROTLI_MOD="std/compress/brotli/mod.sx"
...
[ -f "$BROTLI_LIB" ] || die "missing brotli_lib.sx"
[ ! -f std/compress/brotli/brotli.c ] || die "brotli.c should be deleted"
grep -q 'compress_brotli_compress_c' "$BROTLI_LIB" || die "brotli_lib missing compress_brotli_compress_c"
grep -q 'compress_brotli_stream_compress_c' "$BROTLI_LIB" || die "brotli_lib missing stream compress"
grep -q 'shu_compress_brotli_marker' "$BROTLI_LIB" || die "brotli_lib missing marker"
grep -q 'import("std.compress.brotli.brotli_lib")' "$BROTLI_MOD" || die "mod.sx missing brotli_lib import"
if grep -q 'extern function compress_brotli_compress_c' "$BROTLI_MOD" 2>/dev/null; then
  die "mod.sx still extern compress_brotli_compress_c"
fi
  ...
  while IFS=$'\t' read -r item_id kind anchor mod_path _notes; do
    ...
    case "$kind" in
      symbol)
        target="$BROTLI_LIB"
        case "$mod_path" in
          std/compress/brotli/mod.sx) target="$BROTLI_MOD" ;;
        esac
