#!/usr/bin/env bash
# STD-001: std.io stable API + run-io smoke — honesty soft fallthrough →硬绿.
#
...
# SKIP→OK / soft auto-make / prefer-c). Product run-io.sh exit0 = hard run
# (run=1). check = obs. Report: run=/obs=/skip=.
