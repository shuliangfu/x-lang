#!/usr/bin/env bash
# std.error leftover runner: tests/error/main.x product -o exit 0 (ok).
#
# Honesty: leftover soft auto-make (`xlang_compiler_make -q || make` +
# error.o) + fossil `$XLANG build` retired.
...
# Product `-o` tests/error/main.x must exit 0. Report: run=/obs=/skip=
# PLATFORM: SHARED archaeology — Ubuntu gold still required.
# Usage: ./tests/run-error.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
PREFIX="${XLANG_ERROR_PREFIX:-xlang: [ERROR]}"
SMOKE="tests/error/main.x"
RUN_OK=0
OBS=0
SKIP=0
...
echo "=== error leftover (prefer asm; hard; refuse leftover auto-make) ==="
if [ -n "${XLANG:-}" ]; then
...
set +e
"$XLANG_BIN" check -L . "$SMOKE" >/tmp/xlang_error_check.log 2>&1
chk_ec=$?
set -e
if [ "$chk_ec" -ne 0 ]; then
  echo "error OBS check (paused / CHK residual ec=$chk_ec; refuse soft SKIP→OK)" >&2
  OBS=$((OBS + 1))
fi

exe="/tmp/xlang_error_$$"
rm -f "$exe" 2>/dev/null || true
set +e
"$XLANG_BIN" -L . "$SMOKE" -o "$exe" >/tmp/xlang_error_o.log 2>&1
o_ec=$?
set -e
if [ "$o_ec" -ne 0 ] || [ ! -x "$exe" ]; then
  tail -n 12 /tmp/xlang_error_o.log 2>/dev/null || true
  rm -f "$exe"
  die "product -o failed (ec=$o_ec; refuse leftover auto-make / fossil XLANG build)"
fi
set +e
"$exe" >/dev/null 2>&1
run_ec=$?
