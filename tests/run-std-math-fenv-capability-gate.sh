...
SMOKE_SX="tests/std-math/fenv_capability.sx"
...
for f in "$DOC" "$MANIFEST" "$VECTORS" "$LIB" "$MOD_SX" "$MATH_RUNTIME" "$SMOKE_SX" "$SMOKE_C" std/math/README.md; do
  if [ ! -f "$f" ]; then
...
sym_miss="$(std_math_fenv_cap_symbols_ok "$MOD_SX" "$MATH_RUNTIME" "$MANIFEST" || true)"
...
if [ -n "$SHUX_BIN" ]; then
  ...
  if std_math_fenv_cap_run_sx_smoke "$SHUX_BIN" "$SMOKE_SX"; then
    SX_OK=1
  else
    std_math_fenv_cap_emit_report "fail" "$C_OK" 0 0 "$(ci_host_summary)"
    exit 1
...
std_math_fenv_cap_emit_report "ok" "$C_OK" "$SX_OK" "$SKIP" "$(ci_host_summary)"
