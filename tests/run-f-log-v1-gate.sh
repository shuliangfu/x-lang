...
# static / leftover nested std-log-multi-sink / leftover nested
# std-log-rotate-async; refuse leftover ignore of explicit-bad).
# leftover auto-make of log.o (`xlang_compiler_make` even when the leaf
# is present — try-heat/g05 raced L2) retired. leftover unused
# compiler-make.sh sourced unused after leftover auto-make retired.
# Missing leaf .o = hard die. leftover nested std-log-multi-sink /
# leftover nested std-log-rotate-async stay.
...
# leftover nested std-log-multi-sink / leftover nested
# std-log-rotate-async (refuse leftover SKIP→OK / leftover ignore
# of explicit-bad / leftover XLANG fallthrough). leftover auto-make of
# log.o retired; leftover nested std-log-multi-sink / leftover nested
# std-log-rotate-async stay.
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
...
if [ ! -f std/log/log.o ]; then
  die "missing std/log/log.o (refuse leftover auto-make)"
fi
ENSURE_OK=1
