#!/usr/bin/env bash
# B-14 v2: Linux freestanding open+read (os_read_file_into / xlang_sys_open) smoke.
#
# Honesty: soft XLANG_LINUX_OPEN_READ_FAIL retired — compile/run failure was
...
#
# Usage: ./tests/run-linux-open-read-gate.sh
# Report: run=/skip=
...
PREFIX="xlang: [XLANG_LINUX_OPEN_READ]"
RUN_OK=0
SKIP=1
...
die() {
  echo "linux-open-read-gate FAIL: $*" >&2
  echo "${PREFIX} status=fail run=${RUN_OK:-0} skip=${SKIP:-0} host=$(ci_host_summary)"
  exit 1
}

if ! ci_is_linux; then
  echo "linux-open-read-gate: N/A (Linux freestanding only)"
...
echo "=== linux-open-read (XLANG=$XLANG_BIN; hard) ==="
printf 'OK' >"$GATE_FILE"
rm -f "$OUT" 2>/dev/null || true

if ! "$XLANG_BIN" -freestanding -backend asm -o "$OUT" "$X" 2>/tmp/xlang_linux_open_read.log; then
...
RUN_OK=1
SKIP=0
echo "linux-open-read-gate OK (Linux freestanding xlang_sys_open + read loop; honesty)"
