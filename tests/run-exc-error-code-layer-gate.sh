...
echo "=== EXC-003: smoke (XLANG=$XLANG_BIN; check observational; runnable hard) ==="
  ...
  # Product path authority = existing exc_error_code_layer_run_smoke (`-L . -o`).
  # PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
  export XLANG="$XLANG_BIN"
  export XLANG_LINK_XLANG="$XLANG_BIN"

  # Hard-fail runnable (no soft SKIP→OK / leftover wrap).
  # PLATFORM: SHARED
  if exc_error_code_layer_run_smoke "$XLANG_BIN" "$SMOKE"; then
    RUN_OK=1
    SKIP=0
  else
    echo "exc-error-code-layer gate FAIL runnable (refuse leftover wrap / fossil RUN_XLANG build)" >&2
