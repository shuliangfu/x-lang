#!/usr/bin/env bash
# STD-069: std.db.sqlite col_blob gate — honesty soft prefer-c /
# soft SKIP→OK / soft auto-make / soft std_sqlite_build_o / soft ensure /
# blob_c=/blob_x= report →硬绿.
#
...
# `blob_c=`/`blob_x=` retired. Prefer product xlang_asm; pin
...
# Usage: ./tests/run-std-sqlite-row-col-blob-gate.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
die() {
  echo "std-sqlite-row-col-blob gate FAIL: $*" >&2
  std_sqlite_row_col_blob_emit_report "fail" "$RUN_OK" "$OBS" "$SKIP"
  exit 1
}
...
echo "=== STD-069: db col_blob manifest ==="
...
[ ! -f analysis/std-sqlite-row-col-blob-v1.md ] || die "dual-authority fossil analysis/std-sqlite-row-col-blob-v1.md (archive live)"

for kw in STD-069 col_blob db_sqlite_row_col_blob_smoke_c; do
  grep -qF -- "$kw" "$DOC" 2>/dev/null || die "doc missing '$kw'"
done
grep -qE '^## Gate' "$DOC" || die "doc missing ## Gate section"
grep -qF 'blob_done' "$VECTORS" 2>/dev/null || die "vectors missing blob_done"

while IFS=$'\t' read -r c1 c2 _rest; do
  c1="${c1#\# }"
  case "$c1" in min_blob_apis) MIN_BLOB="$c2" ;; esac
done < "$MANIFEST"
...
[ "$API_N" -ge "$MIN_BLOB" ] || die "api count $API_N < min $MIN_BLOB"

sym_miss="$(std_sqlite_row_col_blob_symbols_ok "$MOD_X" "$DB_C" "$MANIFEST" || true)"
[ "${sym_miss:-0}" -eq 0 ] || die "symbol_miss=${sym_miss}"
echo "std-sqlite-row-col-blob manifest OK"
# Parent STD-068 is honesty this wave — file presence of DOC/gate only
# (aligned with STD-137; refuse re-entering soft parent MANIFEST_ONLY chain).
# PLATFORM: SHARED archaeology.

if [ "${XLANG_STD069_MANIFEST_ONLY:-0}" = "1" ]; then
  SKIP=1
  std_sqlite_row_col_blob_emit_report "ok" "$RUN_OK" "$OBS" "$SKIP"
...
echo "=== STD-069: smoke (XLANG=$XLANG_BIN; host-C=obs; check=obs; tip product=obs) ==="
...
set +e
std_sqlite_row_col_blob_run_c_smoke "$DB_C"
c_rc=$?
set -e
case "$c_rc" in
  0)
    RUN_OK=$((RUN_OK + 1))
    echo "std-sqlite-row-col-blob OK: c smoke"
    ;;
  *)
    echo "std-sqlite-row-col-blob OBS c smoke (rc=$c_rc)" >&2
    OBS=$((OBS + 1))
    ;;
esac

set +e
"$XLANG_BIN" check -L . "$SMOKE_X" >/tmp/xlang_std_sqlite_row_col_blob_check_$$.log 2>&1
chk=$?
set -e
if [ "$chk" -ne 0 ]; then
  echo "std-sqlite-row-col-blob OBS check (paused / CHK residual ec=$chk; refuse soft SKIP→OK)" >&2
  OBS=$((OBS + 1))
fi

# tip product SEGV/UNDEF residual = obs (leave product debt).
# PLATFORM: SHARED — refuse soft SKIP→OK / soft silence.
if std_sqlite_run_smoke "$XLANG_BIN" "$SMOKE_X" "blob"; then
  RUN_OK=$((RUN_OK + 1))
  echo "std-sqlite-row-col-blob OK: product row_col_blob_roundtrip"
else
  echo "std-sqlite-row-col-blob OBS tip product row_col_blob_roundtrip (SEGV/UNDEF residual)" >&2
  OBS=$((OBS + 1))
fi
