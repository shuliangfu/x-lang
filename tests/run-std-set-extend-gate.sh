...
DOC="${XLANG_STD_SET_EXTEND_DOC:-analysis/std-set-extend-v1.md}"
...
if XLANG_BIN="$(resolve_shu 2>/dev/null)"; then
  echo "=== STD-015: typeck (XLANG=$XLANG_BIN) ==="
  ...
  if $RUN_XLANG build -L . "$SMOKE" -o /tmp/xlang_std_set_extend 2>/tmp/xlang_std_set_extend_build.log; then
    exitcode=0
    /tmp/xlang_std_set_extend >/dev/null 2>&1 || exitcode=$?
    ...
    tail -5 /tmp/xlang_std_set_extend_build.log 2>/dev/null >&2 || true
    SKIP=1
  fi
else
  echo "std-set-extend gate SKIP typeck (no native xlang)" >&2
fi
