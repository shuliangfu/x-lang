...
if SHUX_BIN="$(resolve_shu 2>/dev/null)"; then
  echo "=== STD-015: typeck (SHUX=$SHUX_BIN) ==="
  ...
  if $RUN_SHUX -L . "$SMOKE" -o /tmp/shux_std_set_extend 2>/tmp/shux_std_set_extend_build.log; then
    exitcode=0
    /tmp/shux_std_set_extend >/dev/null 2>&1 || exitcode=$?
    ...
    tail -5 /tmp/shux_std_set_extend_build.log 2>/dev/null >&2 || true
    SKIP=1
  fi
else
  echo "std-set-extend gate SKIP typeck (no native shux)" >&2
fi
