...
# std-base64-stream; refuse leftover ignore of explicit-bad). leftover
...
# leftover nested ensure / leftover nested std-base64-stream (refuse
...
# (do not rewrite leftover xlang_compiler_make / std-base64-stream).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
