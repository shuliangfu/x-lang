...
grep -q 'build_use_asm_only' build.x || { echo "c08 build-sx FAIL: build.x missing build_use_asm_only" >&2; exit 1; }
grep -q 'build_tool' build.x || { echo "c08 build-sx FAIL: build.x missing build_tool ref" >&2; exit 1; }
