...
XLANG_D05_FAIL="$FAIL" tests/run-d05-single-xlang-release-gate.sh || die "d05 failed"
...
if [ -f tests/run-no-handwritten-c-gate.sh ]; then
  ...
  XLANG_NO_HANDWRITTEN_C_FAIL="$FAIL" XLANG_NO_HANDWRITTEN_C_STRICT=1 \
    XLANG_NO_HANDWRITTEN_C_MANIFEST_ONLY=1 \
...
XLANG_D03_FAIL=0 tests/run-d03-stage2-hash-gate.sh || die "d03 failed"
...
# 可选：历史 F-std 大批量（默认关；XLANG_F11_RUN_F_STD_BATCH=1 开启）
...
if [ -f tests/run-g-ffi-5-release-ci-gate.sh ]; then
  ...
  XLANG_G_FFI5_FAIL="$FAIL" tests/run-g-ffi-5-release-ci-gate.sh || die "g-ffi-5 release-ci failed"
fi
