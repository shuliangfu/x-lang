...
# XLANG = hard die. Missing native = hard die. Native present but Windows
# asm not available = skip= (capability N/A, not soft SKIP→OK). COFF emit
# miss = hard fail when capable. Report run=/skip=.
#
