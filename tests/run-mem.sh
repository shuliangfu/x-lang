#!/usr/bin/env bash
# core.mem smoke: align_of_i32 == 4 (exit 0)
#
...
PREFIX="${XLANG_MEM_PREFIX:-xlang: [MEM]}"
...
echo "=== mem gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

src="tests/mem/main.x"
[ -f "$src" ] || die "missing $src"
exe="/tmp/xlang_mem_$$"
log="/tmp/xlang_mem_$$.log"
rm -f "$exe" "$log"
set +e
...
if [ "$r_ec" -eq 124 ]; then
  ...
  die "expected exit 0 (align_of_i32 == 4), got $r_ec"
fi
RUN_OK=$((RUN_OK + 1))
ok_report
echo "mem test OK"
