#!/usr/bin/env bash
# core.fmt smoke: fmt_i32 / *_to_buf → exit 42
#
...
PREFIX="${XLANG_FMT_PREFIX:-xlang: [FMT]}"
...
echo "=== fmt gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

src="tests/fmt/main.x"
[ -f "$src" ] || die "missing $src"
exe="/tmp/xlang_fmt_$$"
log="/tmp/xlang_fmt_$$.log"
rm -f "$exe" "$log"
set +e
...
if [ "$r_ec" -eq 124 ]; then
  ...
  die "main expected exit 42 (fmt_i32(42)), got $r_ec"
fi
RUN_OK=$((RUN_OK + 1))
