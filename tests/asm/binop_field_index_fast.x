// 7.3：FIELD/INDEX 与 VAR 混合 binop 应直 load 到 rax/rbx，免 push/pop（run-asm-binop-field-index.sh）。
struct Point {
  x: i32
  y: i32
}
...
function main(): i32 {
  ...
  /** arr[i] + p.y = 24 */
  let e: i32 = b + p.y;
  /** arr[i] + c = 50（INDEX+VAR，arm64 x2 暂存 rbx、x9 暂存 rax 免 push） */
  let f: i32 = arr[i] as i32 + c;
  /** arr[i] + arr[j] = 50（INDEX+INDEX，同上） */
