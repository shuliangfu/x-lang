...
DOC="${SHU_BOOT023_DOC:-analysis/boot-023-mega7-full-emit-v1.md}"
MANIFEST="${SHU_BOOT023_TSV:-tests/baseline/boot-023-mega7-full-emit.tsv}"
WAVE="${SHU_BOOT023_WAVE_TSV:-tests/baseline/parser-mega7-emit-wave.tsv}"
...
while IFS=$'\t' read -r emit_id mega_fn role hook _notes; do
  ...
  if ! grep -qE "function ${mega_fn}\\(" compiler/src/parser/parser.su 2>/dev/null; then
    echo "boot-023 FAIL: parser.su missing $mega_fn" >&2
    MISS=$((MISS + 1))
  fi
...
if boot023_mega7_linux_asm; then
  echo "=== BOOT-023: full emit wave (Linux shu_asm) ==="
  ...
  echo "boot-023-mega7-full-emit gate SKIP wave (Darwin or no shu_asm)" >&2
fi
