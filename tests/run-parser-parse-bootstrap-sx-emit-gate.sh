...
#   SHUX_PARSER_PARSE_BOOTSTRAP_SX_EMIT_FAIL=1 ./tests/run-parser-parse-bootstrap-sx-emit-gate.sh
#   SHUX_PARSER_PARSE_BOOTSTRAP_SX_EMIT_EXPECT_OK=1 SHUX_PARSER_PARSE_BOOTSTRAP_SX_EMIT_FAIL=1 ...  # 修通 sx emit 后硬门禁
set -e
cd "$(dirname "$0")/.."
...
if [ "$(uname -s 2>/dev/null)" = "Darwin" ]; then
  echo "parser-parse-bootstrap-sx-emit-gate: N/A on Darwin"
  exit 0
fi

SHUX_SEED="compiler/shux"
if [ ! -x "$SHUX_SEED" ]; then
  echo "parser-parse-bootstrap-sx-emit-gate: SKIP (no $SHUX_SEED)"
  exit 0
fi

OUT="/tmp/shux_parser_boot_sx_emit.$$.o"
LOG="/tmp/shux_parser_boot_sx_emit.log"
rm -f "$OUT" "$LOG" 2>/dev/null || true

ulimit -s 65532 2>/dev/null || ulimit -s hard 2>/dev/null || true

echo "parser-parse-bootstrap-sx-emit-gate: probe SHUX_ASM_PARSER_PARSE_BOOTSTRAP_EMIT on seed ./shux ..."
set +e
(
  cd compiler
...
if [ "$EXPECT_OK" = "1" ]; then
  if [ "$EC" -ne 0 ] || [ "$HAS_SYM" != "1" ]; then
    echo "parser-parse-bootstrap-sx-emit-gate FAIL: expected sx bootstrap OK (ec=$EC has_sym=$HAS_SYM)" >&2
    tail -n 10 "$LOG" 2>/dev/null || true
    rm -f "$LOG" 2>/dev/null || true
    [ "$FAIL" = "1" ] && exit 1
    exit 0
  fi
  echo "parser-parse-bootstrap-sx-emit-gate PASS: sx bootstrap emit OK (unexpected — prefer C TU unless intentional)"
  rm -f "$LOG" 2>/dev/null || true
  exit 0
fi
...
if [ "$EC" -eq 0 ] && [ "$HAS_SYM" = "1" ] && [ "$HAS_SYM_SZ" -gt 4096 ]; then
  echo "parser-parse-bootstrap-sx-emit-gate FAIL: sx bootstrap unexpectedly succeeded (use C TU or set EXPECT_OK)" >&2
  rm -f "$LOG" 2>/dev/null || true
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi
...
MIN_OUT="/tmp/shux_parser_boot_sx_emit_min.$$.o"
MIN_LOG="/tmp/shux_parser_boot_sx_emit_min.log"
...
if [ "$MIN_EC" -ne 0 ] || [ "$MIN_HAS_INIT" != "1" ] || [ "$MIN_HAS_BUF_SZ" -gt 128 ]; then
  echo "parser-parse-bootstrap-sx-emit-gate FAIL: MINIMAL bootstrap (ec=$MIN_EC init=$MIN_HAS_INIT buf_sz=$MIN_HAS_BUF_SZ)" >&2
  [ "$FAIL" = "1" ] && exit 1
  exit 0
fi

if [ "$EC" -eq 139 ] || [ "$EC" -eq 134 ] || [ "$EC" -ne 0 ]; then
  echo "parser-parse-bootstrap-sx-emit-gate PASS (known sx bootstrap fail ec=$EC; MINIMAL OK; use parser_asm_parse_bootstrap_obj.c)"
  rm -f "$LOG" 2>/dev/null || true
  exit 0
fi

echo "parser-parse-bootstrap-sx-emit-gate PASS (ec=$EC has_o=$HAS_O; sx emit path inactive — C TU default)"
rm -f "$LOG" 2>/dev/null || true
exit 0
