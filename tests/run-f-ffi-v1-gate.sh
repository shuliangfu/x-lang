...
# Report static=/ensure=/cstr=/struct=/skip=.
# Honesty: leftover XLANG fallthrough (`for cand in "${XLANG:-}" …`)
# retired. Explicit-bad XLANG / missing native = hard die FIRST (before
# static / leftover nested std-ffi-cstring-lifecycle / leftover nested
# std-ffi-struct-callback; refuse leftover ignore of explicit-bad).
# leftover auto-make of ffi.o (`xlang_compiler_make` even when the leaf
# is present — try-heat/g05 raced L2) retired. leftover unused
# compiler-make.sh sourced unused after leftover auto-make retired.
# Missing leaf .o = hard die. leftover nested std-ffi-cstring-lifecycle /
# leftover nested std-ffi-struct-callback stay. G.7: complete existing
...
# leftover nested std-ffi-cstring-lifecycle / leftover nested
# std-ffi-struct-callback (refuse leftover SKIP→OK / leftover ignore of
# explicit-bad / leftover XLANG fallthrough). leftover auto-make of
# ffi.o retired; leftover nested std-ffi-cstring-lifecycle /
# leftover nested std-ffi-struct-callback stay.
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
...
if [ ! -f std/ffi/ffi.o ]; then
  die "missing std/ffi/ffi.o (refuse leftover auto-make)"
fi
ENSURE_OK=1
