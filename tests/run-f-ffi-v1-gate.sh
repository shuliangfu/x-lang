...
# std-ffi-cstring-lifecycle / leftover nested std-ffi-struct-callback;
# refuse leftover ignore of explicit-bad). leftover nested product path stay.
...
# leftover nested ensure / leftover nested std-ffi-cstring-lifecycle /
# leftover nested std-ffi-struct-callback (refuse leftover SKIP→OK /
# leftover ignore of explicit-bad / leftover XLANG fallthrough).
# leftover nested product path stays when XLANG is unset (do not
# rewrite leftover xlang_compiler_make / std-ffi-cstring-lifecycle /
# std-ffi-struct-callback).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
