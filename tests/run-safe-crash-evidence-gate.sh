...
for f in "$DOC" "$MANIFEST" std/backtrace/mod.x compiler/src/asm/runtime_backtrace_platform.c \
  compiler/src/asm/runtime_panic.c tests/crash/evidence_manual.x tests/ub/div_zero.x; do
  if [ ! -f "$f" ]; then
