#!/usr/bin/env bash
# F-url v2：std.url 逻辑下沉（parse/build/query/resolve → url.sx；F-ZC 纯 .sx 无 inet glue）。
set -e
cd "$(dirname "$0")/.."
...
echo "=== F-url v2: URL logic → url.sx (F-ZC zero C) ==="
...
grep -q 'url_parse_c' std/url/url.sx || die "url.sx missing parse"
grep -q 'url_resolve_c' std/url/url.sx || die "url.sx missing resolve"
grep -q 'url_smoke_c' std/url/url.sx || die "url.sx missing smoke"
grep -q 'url_f_url_v2_marker_c' std/url/url.sx || die "url.sx missing v2 marker"
grep -q 'url_f_zero_c_marker_c' std/url/url.sx || die "url.sx missing zero-c marker"
grep -q 'url_inet_pton6_c' std/url/url.sx || die "url.sx missing inet_pton"
grep -q 'inet_ntop' std/url/url.sx || die "url.sx missing inet_ntop extern"
