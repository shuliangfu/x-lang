...
SMOKE_SU="tests/std-unicode/grapheme_case.sx"
. "$LIB"
...
C_OK=0
cc -std=c11 -O1 -o /tmp/shux_unicode_gc_c "$UNI_O" -x c - <<'EOF' && C_OK=1 || exit 1
#include <stdint.h>
extern int32_t unicode_grapheme_case_smoke_c(void);
int main(void){ return unicode_grapheme_case_smoke_c()!=0; }
EOF
/tmp/shux_unicode_gc_c >/dev/null
SU_OK=0
SKIP=0
if [ -x ./compiler/shux-c ]; then
  ./compiler/shux-c check -L . "$SMOKE_SU" >/dev/null
  std_unicode_gc_run_smoke ./compiler/shux-c "$SMOKE_SU" && SU_OK=1 || exit 1
else
  SKIP=1
fi
