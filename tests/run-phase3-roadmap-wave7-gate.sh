...
DOC="${XLANG_PLAN007_DOC:-analysis/archive/other-tickets/phase3-roadmap-wave7-v1.md}"
MANIFEST="${XLANG_PLAN007_TSV:-tests/baseline/phase3-roadmap-wave7.tsv}"
...
for f in "$DOC" "$MANIFEST" "$ROADMAP" "$LIB" \
  analysis/archive/other-tickets/phase3-roadmap-wave6-v1.md tests/run-phase3-roadmap-wave6-gate.sh \
...
for kw in Phase 3 BOOT-026 COMP-020 STD-068 XLANG_PLAN007; do
...
if ! grep -qF "§2.18" "$TASK_DOC" 2>/dev/null; then
  echo "phase3-roadmap-wave7 gate FAIL: DOC missing §2.18" >&2
  phase3_roadmap_wave7_emit_report "fail" 0 0 0
  exit 1
fi

sym_miss="$(phase3_roadmap_wave7_check "$MANIFEST" "$TASK_DOC" || true)"
task_n="$(phase3_roadmap_wave7_task_count "$MANIFEST")"
done_n="$(phase3_roadmap_wave7_done_count "$TASK_DOC")"
