#!/usr/bin/env bash
# S3 driver EMIT_HEAVY 烟测：用 shu_asm 第二遍重编 compile.su，统计非 ret0 桩的真机码函数数。
# 依赖：compiler/shu_asm.experimental 或 strict_glue 已重链含最新 ast_pool.c。
# 用法：./tests/run-s3-driver-emit-heavy.sh
# 门禁：SHU_S3_FAIL_ON_EMIT_HEAVY=1 — real_funcs / __text 低于 baseline 时失败
set -e
cd "$(dirname "$0")/.."
...
COMPILE_SU="compiler/src/driver/compile.su"
OUT="/tmp/shu_s3_driver_emit_heavy.o"
...
if [ ! -x "$COMP" ]; then
  echo "s3 driver emit-heavy: no executable compiler (set SHU_S3_EMIT_HEAVY_COMPILER=)" >&2
  exit 127
fi
...
if [ "${SHU_S3_FAIL_ON_EMIT_HEAVY:-0}" = "1" ]; then
  if [ "${real:-0}" -lt "${MIN_REAL}" ] 2>/dev/null; then
    ...
    echo "s3 driver emit-heavy hint: ast_pool.c 变更后须 relink shu_asm" >&2
    exit 1
  fi
