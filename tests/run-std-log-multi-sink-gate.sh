...
if [ -x ./compiler/xlang-c ] || [ -x ./compiler/xlang ]; then
  ...
  echo "std-log-multi-sink gate SKIP c smoke (no xlang-c)" >&2
fi
...
if [ -n "$XLANG_BIN" ]; then
  ...
  if std_log_multi_sink_run_smoke "$XLANG_BIN" "$SMOKE_X" "level"; then
    X_OK=1
  else
    std_log_multi_sink_emit_report "fail" "$C_OK" 0 0
    exit 1
  ...
  echo "std-log-multi-sink gate SKIP .x smoke (no native xlang)" >&2
  SKIP=1
fi
