...
DOC="${SHU_STD_TASK_DOC:-analysis/std-task-v1.md}"
MANIFEST="${SHU_STD_TASK_MANIFEST:-tests/baseline/std-task-manifest.tsv}"
MOD_SU="std/task/mod.su"
TASK_C="std/task/task.c"
LIB="tests/lib/std-task.sh"
SMOKE_SU="tests/std-task/group_smoke.su"
...
if cc -std=c11 -O1 -o /tmp/shu_task_smoke \
  "$SMOKE_C" std/task/task.o std/async/scheduler.o std/context/context.o std/time/time.o 2>/dev/null; then
  if /tmp/shu_task_smoke >/dev/null 2>&1; then C_OK=1; fi
  rm -f /tmp/shu_task_smoke
fi
if [ "$C_OK" -eq 0 ]; then
  std_task_emit_report "fail" 0 0 0
...
if [ -n "$SHU_BIN" ]; then
  echo "=== STD-089: .su smoke (SHU=$SHU_BIN) ==="
  ...
  if std_task_run_smoke "$SHU_BIN" "$SMOKE_SU" "group"; then SU_OK=1; else
    std_task_emit_report "fail" "$C_OK" 0 0
    exit 1
  fi
else
