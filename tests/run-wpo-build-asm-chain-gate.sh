...
#   SHUX_WPO_CHAIN_FAIL=1 ./tests/run-wpo-build-asm-chain-gate.sh
set -e
cd "$(dirname "$0")/.."
...
FAIL=${SHUX_WPO_CHAIN_FAIL:-1}
BASELINE="${SHUX_WPO_CHAIN_BASELINE:-tests/baseline/wpo-dce-compiler-self-text.tsv}"
...
if [ "${SHUX_WPO_ENSURE_ARTIFACTS:-1}" = "1" ]; then
  SHUX_WPO_ENSURE_FAIL="$FAIL" ./tests/ensure-wpo-build-asm-artifacts.sh
fi
...
SHUX_WPO_MAIN_O_FAIL="$FAIL" ./tests/run-wpo-main-o-gate.sh "$BUILD_ASM/main.o"
SHUX_WPO_DRIVER_O_FAIL="$FAIL" ./tests/run-wpo-driver-o-gate.sh "$BUILD_ASM/driver_compile.o"
SHUX_WPO_PIPELINE_O_FAIL="$FAIL" ./tests/run-wpo-pipeline-o-gate.sh "$BUILD_ASM/pipeline_wpo.o"
SHUX_WPO_TYPECK_O_FAIL="$FAIL" ./tests/run-wpo-typeck-o-gate.sh "$BUILD_ASM/typeck_wpo.o"
SHUX_WPO_BACKEND_O_FAIL="$FAIL" ./tests/run-wpo-backend-o-gate.sh "$BUILD_ASM/backend_wpo.o"
