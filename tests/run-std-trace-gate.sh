...
DOC="${SHU_STD_TRACE_DOC:-analysis/std-trace-v1.md}"
MANIFEST="${SHU_STD_TRACE_MANIFEST:-tests/baseline/std-trace-manifest.tsv}"
MOD_SU="std/trace/mod.su"
TRACE_C="std/trace/trace.c"
LIB="tests/lib/std-trace.sh"
SMOKE_SU="tests/std-trace/nested_smoke.su"
...
if cc -std=c11 -O1 -o /tmp/shu_trace_smoke \
  "$SMOKE_C" std/trace/trace.o std/time/time.o std/random/random.o 2>/dev/null; then
  if /tmp/shu_trace_smoke >/dev/null 2>&1; then C_OK=1; fi
  rm -f /tmp/shu_trace_smoke
fi
if [ "$C_OK" -eq 0 ]; then
  std_trace_emit_report "fail" 0 0 0
...
if [ -n "$SHU_BIN" ]; then
  echo "=== STD-088: .su smoke (SHU=$SHU_BIN) ==="
  ...
  if std_trace_run_smoke "$SHU_BIN" "$SMOKE_SU" "nested"; then SU_OK=1; else
    std_trace_emit_report "fail" "$C_OK" 0 0
    exit 1
  fi
else
