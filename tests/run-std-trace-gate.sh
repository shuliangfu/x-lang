#!/usr/bin/env bash
# STD-088：std.trace 门禁（F-trace v2：纯 trace.sx）
set -e
cd "$(dirname "$0")/.."
...
SMOKE_SX="tests/std-trace/nested_smoke.sx"
...
for f in "$DOC" "$MANIFEST" "$LIB" "$MOD_SX" "$TRACE_SX" "$SMOKE_SX" "$SMOKE_C" std/trace/README.md; do
  if [ ! -f "$f" ]; then
...
sym_miss="$(std_trace_symbols_ok "$MOD_SX" "$TRACE_SX" "$MANIFEST" || true)"
...
if [ -x ./compiler/shux-c ] || [ -x ./compiler/shux ]; then
  ...
  if ensure_std_c_o ../std/trace/trace.o 2>/dev/null && std_trace_run_c_smoke "$TRACE_SX"; then
    C_OK=1
  else
...
if [ -n "$SHUX_BIN" ]; then
  ...
  if std_trace_run_smoke "$SHUX_BIN" "$SMOKE_SX" "nested"; then SX_OK=1; else
    std_trace_emit_report "fail" "$C_OK" 0 0
    exit 1
  fi
else
...
std_trace_emit_report "ok" "$C_OK" "$SX_OK" "$SKIP"
