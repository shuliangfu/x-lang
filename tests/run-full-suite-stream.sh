...
#   XLANG_FULL_SUITE_SKIP_CI=1 ./tests/run-full-suite-stream.sh   # 跳过 Tier 5（数小时）
#
# 环境：
#   XLANG_FULL_SUITE_SKIP_CI=1 — 不跑 run-ci-full-suite.sh
#   XLANG_FULL_SUITE_SKIP_BSTRICT_CI=1 — 不跑 run-bootstrap-bstrict-ci.sh（Tier 4）
#   XLANG_PROGRESS_TIMEOUT_SEC — 传给 progress-run（单 Tier 超时）
#   XLANG_BUILD_ASM_TIMEOUT — build_xlang_asm 外层超时（默认 3600）
...
export XLANG_BUILD_ASM_TIMEOUT="${XLANG_BUILD_ASM_TIMEOUT:-3600}"
...
run_tier "T3 e-soft retire" "$LOG_DIR/t3-e-soft.log" \
  env XLANG_E_SOFT_FAIL=1 ./tests/run-e-soft-retire-gate.sh

# Tier 4：B-strict CI 聚合（含 run-all-bstrict 等）
if [ -z "${XLANG_FULL_SUITE_SKIP_BSTRICT_CI:-}" ]; then
  run_tier "T4 bootstrap-bstrict-ci" "$LOG_DIR/t4-bstrict-ci.log" \
    ./tests/run-bootstrap-bstrict-ci.sh
else
  progress_suite "SKIP Tier 4 (XLANG_FULL_SUITE_SKIP_BSTRICT_CI=1)"
fi

# Tier 5：CI 全量（Linux x86_64 Tier A）
if [ -z "${XLANG_FULL_SUITE_SKIP_CI:-}" ]; then
  run_tier "T5 ci-full-suite" "$LOG_DIR/t5-ci-full.log" \
    env CI=1 XLANG_CI_NO_SKIP=1 ./tests/run-ci-full-suite.sh
else
  progress_suite "SKIP Tier 5 (XLANG_FULL_SUITE_SKIP_CI=1)"
fi
