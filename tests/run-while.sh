...
$LINK_SHUX build tests/while/never.x -o /tmp/shux_while_never 2>&1
exitcode=0
...
$LINK_SHUX build tests/while/break_out.x -o /tmp/shux_while_break 2>&1
exitcode=0
...
$LINK_SHUX build tests/while/continue_never.x -o /tmp/shux_while_cont 2>&1
exitcode=0
...
$LINK_SHUX build tests/while/loop_break.x -o /tmp/shux_loop_break 2>&1
exitcode=0
...
if $TYPECK_SHUX build tests/while/break_outside.x -o /tmp/shux_break_out 2>&1 | grep -q "only allowed inside a loop"; then
...
$LINK_SHUX build tests/while/index_as_cast.x -o /tmp/shux_while_index_as 2>&1
exitcode=0
