...
# -o 链接走 RUN_SHU（bootstrap 下非 x86_64 为 shu-c）；typeck 负例仍用 seed SHU。
LINK_SHU="$RUN_SHU"
TYPECK_SHU="$SHU"

# while 0 { 1 }; 42 -> 42（循环不执行）
$LINK_SHU tests/while/never.su -o /tmp/shu_while_never 2>&1
exitcode=0
...
$LINK_SHU tests/while/break_out.su -o /tmp/shu_while_break 2>&1
exitcode=0
...
$LINK_SHU tests/while/continue_never.su -o /tmp/shu_while_cont 2>&1
exitcode=0
...
$LINK_SHU tests/while/loop_break.su -o /tmp/shu_loop_break 2>&1
exitcode=0
...
if $TYPECK_SHU tests/while/break_outside.su -o /tmp/shu_break_out 2>&1 | grep -q "only allowed inside a loop"; then
...
$LINK_SHU tests/while/index_as_cast.su -o /tmp/shu_while_index_as 2>&1
exitcode=0
