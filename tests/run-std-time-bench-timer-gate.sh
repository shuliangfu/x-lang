...
SMOKE_SX="tests/time/bench_timer.sx"
. "$LIB"
...
sym_miss="$(std_time_bench_timer_symbols_ok "$MOD_SX" "$MANIFEST" || true)"
...
if [ -x ./compiler/shux-c ]; then
  ...
  std_time_bench_timer_run_smoke ./compiler/shux-c "$SMOKE_SX" && SX_OK=1 || exit 1
else
  SKIP=1
fi
std_time_bench_timer_emit_report ok "$SX_OK" "$SKIP"
