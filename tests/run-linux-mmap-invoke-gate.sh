#!/usr/bin/env bash
# B-14 v3: Linux freestanding mmap/munmap smoke.
#
# Honesty: soft XLANG_LINUX_MMAP_INVOKE_FAIL retired — compile/run failure was
...
#
# Usage: ./tests/run-linux-mmap-invoke-gate.sh
# Report: run=/skip=
...
PREFIX="xlang: [XLANG_LINUX_MMAP_INVOKE]"
RUN_OK=0
SKIP=1
...
die() {
  echo "linux-mmap-invoke-gate FAIL: $*" >&2
  echo "${PREFIX} status=fail run=${RUN_OK:-0} skip=${SKIP:-0} host=$(ci_host_summary)"
  exit 1
}

if ! ci_is_linux; then
  echo "linux-mmap-invoke-gate: N/A (Linux freestanding only)"
...
echo "=== linux-mmap-invoke (XLANG=$XLANG_BIN; hard) ==="
rm -f "$OUT" 2>/dev/null || true

if ! "$XLANG_BIN" -freestanding -backend asm -o "$OUT" "$X" 2>/tmp/xlang_linux_mmap_invoke.log; then
...
RUN_OK=1
SKIP=0
echo "linux-mmap-invoke-gate OK (Linux freestanding xlang_sys_mmap/munmap; honesty)"
