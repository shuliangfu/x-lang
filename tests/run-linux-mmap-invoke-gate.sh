...
SU="tests/sys/linux_mmap_invoke_smoke.sx"
...
if ! "$SHUX" -freestanding -backend asm -o "$OUT" "$SU" 2>/tmp/shux_linux_mmap_invoke.log; then
  echo "linux-mmap-invoke-gate FAIL: compile $SU" >&2
