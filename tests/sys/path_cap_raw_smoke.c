/*
 * path_cap_raw_smoke.c — Stage 9 (9.1.2) Linux raw Cap residual probe.
 *
 * Host-cc smoke for xlang_path_cap.h on Linux:
 *   - access via raw syscall faccessat (no libc access)
 *   - stat via raw syscall newfstatat / fstatat (no libc stat)
 *   - fstat via raw syscall fstat (no libc fstat)
 *   - realpath via raw open(O_PATH) + readlink(/proc/self/fd/N) + close (no libc realpath)
 *
...
int main(void) {
  fprintf(stderr, "path_cap_raw_smoke: Linux only\n");
  return 0;
}
#else
...
int main(void) {
  ...
  fd = (int)xlang_linux_syscall3(2, (long)target, O_RDONLY, 0);
#elif defined(__aarch64__)
  fd = (int)xlang_linux_syscall4(56, (long)AT_FDCWD, (long)target, O_RDONLY, 0);
#else
...
#if defined(__x86_64__)
    (void)xlang_linux_syscall2(3, fd, 0);
#elif defined(__aarch64__)
    (void)xlang_linux_syscall2(57, fd, 0);
#else
    close(fd);
#endif
    return 5;
