/**
 * tests/sys/time_cap_raw_smoke.c — Linux raw syscall time Cap probe (9.1.5).
 *
 * Verifies xlang_time_cap.h primitives without libc clock_gettime/nanosleep/gmtime_r:
 * 1. REALTIME clock via raw syscall 228/113 returns plausible epoch (> 2020)
 * 2. MONOTONIC clock via raw syscall returns valid time
 * 3. nanosleep via raw syscall 35/101 sleeps 2ms successfully
 ...
 *
 * PLATFORM: LINUX raw syscall Cap (9.1.5).
 */
#include <stdint.h>
#include <stddef.h>
#include <string.h>
...
int main(void) {
  ...
  /* Step 3: nanosleep 2ms via raw syscall */
  req.tv_sec = 0;
