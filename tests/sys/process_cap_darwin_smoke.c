/*
 * process_cap_darwin_smoke.c — Stage 9 (9.1.3 / 9.1.4) Darwin Cap residual probe.
 *
 * Host-cc smoke for xlang_process_cap.h on Darwin:
 *   - getpid via raw syscall 0x2000014 / 20 (no libc getpid)
 *   - getppid via raw syscall 0x2000027 / 39 (no libc getppid)
 *   - chdir via raw syscall 0x200000c / 12 (no libc chdir)
 *   - getcwd via raw open(".") + fcntl(F_GETPATH) + close (no libc getcwd)
 *   - pipe via raw syscall 0x200002a / 42 (no libc pipe)
 *   - dup2 via raw syscall 0x200005a / 90 (no libc dup2)
 *   - fork via raw syscall 0x2000002 / 2 (no libc fork)
 *   - wait4 via raw syscall 0x2000007 / 7 (no libc waitpid)
 *   - exit via raw syscall 0x2000001 / 1 (no libc exit)
...
#ifndef __APPLE__
int main(void) {
  fprintf(stderr, "process_cap_darwin_smoke: Darwin only\n");
  return 0;
}
#else
