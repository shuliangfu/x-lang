/**
 * tests/sys/time_cap_darwin_smoke.c — Darwin Cap time probe (9.1.5).
 *
 * Verifies xlang_time_cap.h primitives without libc clock_gettime/nanosleep/gmtime_r:
 * 1. REALTIME clock via SYS_gettimeofday raw syscall returns plausible epoch (> 2020)
 * 2. MONOTONIC clock via ARM64 cntvct_el0 / raw syscall returns valid time
 * 3. nanosleep via SYS_pselect raw syscall sleeps 2ms successfully
 ...
 *
 * PLATFORM: MACOS|DARWIN Cap (9.1.5).
 */
#include <stdint.h>
#include <stddef.h>
#include <string.h>
