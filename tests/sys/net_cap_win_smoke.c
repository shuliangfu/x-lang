/**
 * tests/sys/net_cap_win_smoke.c — Windows Winsock network Cap probe (9.1.7).
 *
 * Verifies xlang_net_cap.h and xlang_dns_cap.h primitives on Windows via Winsock Cap:
 * 1. xlang_net_socket creates TCP socket via Winsock Cap (auto WSAStartup)
 * 2. xlang_net_setsockopt sets SO_REUSEADDR
 * 3. xlang_net_bind binds socket to loopback
 * 4. xlang_net_listen listens
 * 5. xlang_net_poll polls listener with timeout=0
 * 6. xlang_net_fcntl sets O_NONBLOCK via ioctlsocket FIONBIO
 * 7. xlang_net_close closes socket via closesocket
 * 8. xlang_dns_resolve_ipv4 resolves literal "127.0.0.1"
 * 9. xlang_dns_resolve_ipv4 resolves "localhost"
 * 10. UDP batch loopback test via xlang_net_sendmmsg and xlang_net_recvmmsg
 *
 * PLATFORM: WINDOWS Winsock Cap (9.1.7).
 */
#include <stdint.h>
#include <stddef.h>
#include <string.h>
...
int main(void) {
  ...
  int one = 1;
  if (xlang_net_setsockopt(s, SOL_SOCKET, SO_REUSEADDR,
