/*
 * env_cap_darwin_smoke.c — Stage 9 (9.1.1) Darwin Cap residual probe.
 *
 * Host-cc smoke for xlang_environ_cap.h on Darwin:
 *   - getenv via (*_NSGetEnviron()) walk (no libc getenv)
 *   - setenv via (*_NSGetEnviron()) replace/append (no libc setenv)
 *   - unsetenv via (*_NSGetEnviron()) slot shift (no libc unsetenv)
 *
...
#ifndef __APPLE__
int main(void) {
  fprintf(stderr, "env_cap_darwin_smoke: Darwin only\n");
  return 0;
}
#else
...
int main(void) {
  ...
  r = xlang_environ_setenv("XLANG_DARWIN_ENV_TEST", "val_alpha", 1);
  if (r != 0) {
  ...
  val = xlang_environ_getenv("XLANG_DARWIN_ENV_TEST");
  ...
  r = xlang_environ_setenv("XLANG_DARWIN_ENV_TEST", "val_beta", 0);
  if (r != 0) {
  ...
  r = xlang_environ_setenv("XLANG_DARWIN_ENV_TEST", "val_beta", 1);
  if (r != 0) {
  ...
  r = xlang_environ_unsetenv("XLANG_DARWIN_ENV_TEST");
  if (r != 0) {
