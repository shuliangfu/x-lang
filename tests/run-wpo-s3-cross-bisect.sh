#!/usr/bin/env bash
# WPO-S3 跨模块 asm 二分：隔离 import 路径 fault（需 Linux xlang_asm）
# 用法：XLANG=./compiler/xlang_asm ./tests/run-wpo-s3-cross-bisect.sh
# 含 import 的 .x 须在 xlang_asm -backend asm -o 下产出非空 .o（修复前 typeck_entry SIGSEGV）。
set -e
cd "$(dirname "$0")/.."

XLANG_BIN="${XLANG:-./compiler/xlang_asm}"
if [ ! -x "$XLANG_BIN" ]; then
  echo "wpo-s3 cross bisect SKIP (need $XLANG_BIN)"
  exit 0
fi
...
wpo_s3_try_asm_o() {
  local label="$1"
  local x="$2"
  local out="/tmp/xlang_wpo_s3_${label}.o"
  rm -f "$out"
  set +e
  "$XLANG_BIN" -backend asm -o "$out" "$x" 2>/tmp/xlang_wpo_s3_${label}.log
  local brc=$?
  set -e
  if [ "$brc" -ne 0 ] || [ ! -f "$out" ]; then
    echo "wpo-s3 cross bisect FAIL: $label .o compile (rc=$brc)" >&2
    tail -5 "/tmp/xlang_wpo_s3_${label}.log" 2>/dev/null || true
    return 1
  fi
  echo "wpo-s3 cross bisect OK: $label .o"
}

echo "=== WPO-S3 cross-module asm bisect ($XLANG_BIN) ==="
