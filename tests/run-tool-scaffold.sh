...
EXPECT_EXIT="${SHU_SCAFFOLD_EXPECT_EXIT:-42}"
WORKDIR="/tmp/shu_scaffold_test_$$"
EXE="$WORKDIR/app"
...
if ! "$SHU" -L . "$WORKDIR/main.su" -o "$EXE" 2>&1; then
  echo "run-tool-scaffold FAIL: compile template main.su" >&2
  exit 1
fi
