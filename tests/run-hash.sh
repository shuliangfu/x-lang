#!/usr/bin/env bash
# std.hash smoke: product -o exit 0
#
...
PREFIX="${XLANG_HASH_PREFIX:-xlang: [HASH]}"
...
echo "=== hash gate (prefer asm; hard) ==="
...
exe="/tmp/xlang_hash_$$"
log="/tmp/xlang_hash_$$.log"
rm -f "$exe" "$log"
[ -f "tests/hash/main.x" ] || die "missing tests/hash/main.x"
set +e
gate_run_timeout "$XLANG_CASE_TIMEOUT" "$XLANG_BIN" -L . "tests/hash/main.x" -o "$exe" >"$log" 2>&1
o_ec=$?
set -e
if [ "$o_ec" -eq 124 ]; then
