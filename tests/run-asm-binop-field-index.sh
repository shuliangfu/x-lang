#!/usr/bin/env bash
# asm 7.3: FIELD/INDEX mixed with VAR binop loads directly (no push/pop).
# Fixture has struct; product path is prefer-asm (no soft prefer-c).
#
...
#   - hard: tests/asm/binop_field_index_fast.x product -o run exit 205
#   - hard (Darwin+otool): main has no `sub sp, sp, #0x10` binop spill
...
PREFIX="${XLANG_ASM_BINOP_FIELD_INDEX_PREFIX:-xlang: [XLANG_ASM_BINOP_FIELD_INDEX]}"
...
die() {
  echo "asm-binop-field-index FAIL: $*" >&2
...
echo "=== asm-binop-field-index gate (prefer asm; hard) ==="
XLANG_BIN="$(resolve_shu)" || die "no native xlang/xlang_asm/xlang-c (refuse soft SKIP→OK / soft auto-make)"
export XLANG="$XLANG_BIN"
export XLANG_LINK_XLANG="$XLANG_BIN"
echo "XLANG=$XLANG_BIN"

SRC="tests/asm/binop_field_index_fast.x"
[ -f "$SRC" ] || die "missing $SRC"
exe="/tmp/xlang_asm_binop_field_index_$$"
log="/tmp/xlang_asm_binop_field_index_$$.log"
rm -f "$exe" "$log"
...
if [ "$r_ec" -eq 124 ]; then
  rm -f "$exe"
  die "run timeout"
elif [ "$r_ec" -ne 205 ]; then
  rm -f "$exe"
  die "expected exit 205, got $r_ec"
fi
RUN_OK=$((RUN_OK + 1))

# PLATFORM: DARWIN — otool arm64 main disasm. Non-Darwin = skip= honesty.
if [ "$(uname -s)" = Darwin ] && command -v otool >/dev/null 2>&1; then
  if otool -tv "$exe" 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'sub.*sp, sp, #0x10'; then
    rm -f "$exe"
    die "main still uses stack push for binop"
  fi
  RUN_OK=$((RUN_OK + 1))
else
  SKIP=$((SKIP + 1))
