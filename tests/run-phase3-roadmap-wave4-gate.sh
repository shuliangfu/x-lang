...
DOC="${XLANG_PLAN004_DOC:-analysis/archive/other-tickets/phase3-roadmap-wave4-v1.md}"
MANIFEST="${XLANG_PLAN004_TSV:-tests/baseline/phase3-roadmap-wave4.tsv}"
...
for f in "$DOC" "$MANIFEST" "$ROADMAP" "$LIB" \
  analysis/archive/other-tickets/phase3-roadmap-wave3-v1.md tests/run-phase3-roadmap-wave3-gate.sh \
...
for kw in Phase 3 BOOT-023 COMP-017 STD-065 XLANG_PLAN004; do
...
if ! grep -qF "§2.15" "$TASK_DOC" 2>/dev/null; then
  echo "phase3-roadmap-wave4 gate FAIL: DOC missing §2.15" >&2
  phase3_roadmap_wave4_emit_report "fail" 0 0 0
  exit 1
fi

sym_miss="$(phase3_roadmap_wave4_check "$MANIFEST" "$TASK_DOC" || true)"
task_n="$(phase3_roadmap_wave4_task_count "$MANIFEST")"
done_n="$(phase3_roadmap_wave4_done_count "$TASK_DOC")"
