#!/usr/bin/env bash
# STD-135: std.datetime fixed-offset timezone gate — honesty leftover unused compiler-make →硬绿.
#
...
# timezone.x -o exit0 = hard run (run=1). check / host-C archaeology = obs.
# Report: run=/obs=/skip=. G.7: complete existing resolve_shu; drop unused
# compiler-make.sh.
# PLATFORM: SHARED archaeology — Ubuntu gold still required.
# Usage: ./tests/run-std-datetime-timezone-gate.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
die() {
  echo "std-datetime-timezone gate FAIL: $*" >&2
  std_datetime_timezone_emit_report "fail" "$RUN_OK" "$OBS" "$SKIP"
  exit 1
}
...
for f in "$DOC" "$MANIFEST" "$LIB" "$MOD_X" "$DT_X" "$SMOKE_X" "$SMOKE_C"; do
  [ -f "$f" ] || die "missing $f"
done
...
if [ "${XLANG_STD135_DATETIME_TIMEZONE_MANIFEST_ONLY:-0}" = "1" ]; then
  SKIP=1
  std_datetime_timezone_emit_report "ok" "$RUN_OK" "$OBS" "$SKIP"
...
echo "=== STD-135: smoke (XLANG=$XLANG_BIN; check/host-C obs; product -o hard) ==="
...
if std_datetime_timezone_run_c_smoke; then
  echo "std-datetime-timezone c smoke OK (observational)"
else
  echo "std-datetime-timezone OBS c smoke (host-C archaeology; refuse soft ensure/auto-make)" >&2
  OBS=$((OBS + 1))
fi

set +e
"$XLANG_BIN" check -L . "$SMOKE_X" >/tmp/xlang_std135_tz_check.log 2>&1
chk=$?
set -e
if [ "$chk" -ne 0 ]; then
  echo "std-datetime-timezone OBS check (paused / CHK residual ec=$chk; refuse soft SKIP→OK)" >&2
  OBS=$((OBS + 1))
fi
...
RUN_OK=$((RUN_OK + 1))
echo "std-datetime-timezone OK: product -o"
