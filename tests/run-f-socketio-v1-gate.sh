#!/usr/bin/env bash
# F-socketio v1：std.socketio 去 C（socketio.c → socketio.sx；胶层 v2 已删，见 run-f-socketio-v2-gate.sh）。
set -e
cd "$(dirname "$0")/.."
...
echo "=== F-socketio v1: socketio.c → socketio.sx (glue superseded by v2) ==="
