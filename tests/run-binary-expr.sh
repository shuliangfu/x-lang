...
# 测试 1+2 -> 退出码 3
$LINK_SHUX build tests/binary-expr/main.x -o /tmp/shux_binary_expr 2>&1
exitcode=0
...
# 测试 42-10 -> 退出码 32
$LINK_SHUX build tests/binary-expr/sub.x -o /tmp/shux_binary_sub 2>&1
exitcode=0
...
# 测试 6*7 -> 退出码 42
$LINK_SHUX build tests/binary-expr/mul.x -o /tmp/shux_binary_mul 2>&1
exitcode=0
...
# 测试 20/4 -> 退出码 5
$LINK_SHUX build tests/binary-expr/div.x -o /tmp/shux_binary_div 2>&1
exitcode=0
...
$LINK_SHUX build tests/binary-expr/precedence.x -o /tmp/shux_binary_prec 2>&1
exitcode=0
...
# 测试括号：(1+2)*3 = 9
$LINK_SHUX build tests/binary-expr/paren.x -o /tmp/shux_binary_paren 2>&1
exitcode=0
...
# 测试一元负号：10 + -4 = 6
$LINK_SHUX build tests/binary-expr/unary_neg.x -o /tmp/shux_binary_neg 2>&1
exitcode=0
