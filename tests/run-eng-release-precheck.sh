...
# 全量：XLANG_RELEASE_FULL=1 追加 run-portable-suite.sh
#
# 用法：
#   ./tests/run-eng-release-precheck.sh
#   XLANG_RELEASE_TAG=v0.2.0 ./tests/run-eng-release-precheck.sh
#   XLANG_RELEASE_REQUIRE_CLEAN=1 ./tests/run-eng-release-precheck.sh
set -e
cd "$(dirname "$0")/.."

MANIFEST="${XLANG_ENG_RELEASE_MANIFEST:-tests/baseline/eng-branch-release-gate.tsv}"
PREFIX="xlang: [XLANG_RELEASE_PRECHECK]"
...
if [ -n "${XLANG_RELEASE_TAG:-}" ]; then
  if ! eng_release_tag_valid "$XLANG_RELEASE_TAG"; then
    echo "${PREFIX} FAIL invalid tag '${XLANG_RELEASE_TAG}' (want vX.Y.Z)" >&2
    exit 1
  fi
  echo "${PREFIX} tag OK ${XLANG_RELEASE_TAG}" >&2
fi

if [ "${XLANG_RELEASE_REQUIRE_CLEAN:-0}" = "1" ]; then
  if ! eng_release_worktree_clean; then
    echo "${PREFIX} FAIL working tree not clean (XLANG_RELEASE_REQUIRE_CLEAN=1)" >&2
...
while IFS=$'\t' read -r item_id kind anchor notes; do
  ...
  case "$kind" in
    release_gate)
      ...
      if ! "./$script" >/tmp/xlang_release_precheck_"${item_id}".log 2>&1; then
        tail -20 /tmp/xlang_release_precheck_"${item_id}".log >&2 || true
      ...
      ;;
    release_hook)
      if [ "${XLANG_RELEASE_FULL:-0}" != "1" ]; then
        echo "${PREFIX} skip $anchor (set XLANG_RELEASE_FULL=1 for full portable suite)" >&2
        continue
      fi
      script="tests/$anchor"
      if [ ! -f "$script" ]; then
      ...
      if ! "./$script" >/tmp/xlang_release_precheck_full.log 2>&1; then
        tail -30 /tmp/xlang_release_precheck_full.log >&2 || true
