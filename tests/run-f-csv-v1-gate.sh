...
# Report static=/ensure=/row=/stream=/skip=.
# Honesty: leftover XLANG fallthrough (`for cand in "${XLANG:-}" …`)
# retired. Explicit-bad XLANG / missing native = hard die FIRST (before
# static / leftover nested std-csv-row / leftover nested
# std-csv-stream; refuse leftover ignore of explicit-bad). leftover
# auto-make of csv.o (`xlang_compiler_make` even when the leaf is
# present — try-heat/g05 raced L2) retired. leftover unused
# compiler-make.sh sourced unused after leftover auto-make retired.
# Missing leaf .o = hard die. leftover nested std-csv-row /
# std-csv-stream stay.
...
# leftover nested std-csv-row / leftover nested std-csv-stream
# (refuse leftover SKIP→OK / leftover ignore of explicit-bad /
# leftover XLANG fallthrough). leftover auto-make of csv.o retired;
# leftover nested std-csv-row / std-csv-stream stay.
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
...
if [ ! -f std/csv/csv.o ]; then
  die "missing std/csv/csv.o (refuse leftover auto-make)"
fi
ENSURE_OK=1
