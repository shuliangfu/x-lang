#!/usr/bin/env bash
# F-csv v1：std.csv 去 C（csv.c → csv.sx）。
set -e
cd "$(dirname "$0")/.."
...
echo "=== F-csv v1: csv.c → csv.sx ==="
[ -f "$DOC" ] || die "missing $DOC"
grep -q 'F-csv v1' "$DOC" || die "doc marker"
[ -f std/csv/csv.sx ] || die "missing csv.sx"
...
grep -q 'csv.sx' compiler/Makefile || die "Makefile missing csv.sx"
