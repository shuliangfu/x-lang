...
#
# Linux x86_64：xlang_asm -backend asm -o return-value 须 exit=42。
# 无 xlang_asm 时 SKIP（bootstrap-min 仍可用 min-link 回退）。
#
# 用法：./tests/run-bootstrap-c6-asm-o-gate.sh
# 环境：XLANG_BOOTSTRAP_C6_SKIP=1 跳过
set -euo pipefail
cd "$(dirname "$0")/.."
...
rm -f "$EXE"
gate_progress "C6: xlang_asm -backend asm -o return-value ..."
set +e
"$ASM" -backend asm -L . "$RV" -o "$EXE" >/tmp/xlang_c6_asm_o.log 2>&1
ec=$?
set -e
if [ "$ec" -ne 0 ] || [ ! -x "$EXE" ]; then
  tail -8 /tmp/xlang_c6_asm_o.log >&2
