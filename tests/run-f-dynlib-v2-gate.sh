...
# static / leftover nested std-dynlib-windows / leftover nested
# std-dynlib-last-error; refuse leftover ignore of explicit-bad). leftover
# auto-make of dynlib.o (`xlang_compiler_make` even when the leaf is
# present — try-heat/g05 raced L2) retired. leftover unused
# compiler-make.sh sourced unused after leftover auto-make retired.
# Missing leaf .o = hard die. leftover nested std-dynlib-windows
# / leftover nested std-dynlib-last-error stay.
...
# leftover nested std-dynlib-windows / leftover nested
# std-dynlib-last-error (refuse leftover SKIP→OK / leftover ignore of
# explicit-bad / leftover XLANG fallthrough). leftover nested product
# path stays when XLANG is unset (do not rewrite leftover nested
# std-dynlib-windows / leftover nested std-dynlib-last-error).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
...
if [ ! -f std/dynlib/dynlib.o ]; then
  die "missing std/dynlib/dynlib.o (refuse leftover auto-make)"
fi
ENSURE_OK=1
