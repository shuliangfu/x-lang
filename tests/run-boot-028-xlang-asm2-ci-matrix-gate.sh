...
LIB="tests/lib/boot-028-xlang-asm2-ci-matrix.sh"
MIN_HOOKS=4
MIN_ROWS=4
MIN_LINUX_MUST=1
MIN_MACOS_MUST=1

# shellcheck source=tests/lib/boot-028-xlang-asm2-ci-matrix.sh
. "$LIB"

echo "=== BOOT-028: xlang_asm2 CI matrix manifest ==="
for f in "$DOC" "$MANIFEST" "$WAVE" "$PLAT_MATRIX" "$LIB" "$MATRIX" "$REPRO" "$CI_MATRIX" \
  tests/run-xlang-asm2-cross-smoke.sh \
  ...
  if [ ! -f "$f" ]; then
    echo "boot-028-xlang-asm2-ci-matrix gate FAIL: missing $f" >&2
    exit 1
  fi
done
...
for kw in c6_matrix_ok c6_smoke boot028_xlang_asm2_ci_matrix xlang_asm2_ci_target; do
  if ! grep -qF "$kw" "$DOC" 2>/dev/null; then
    echo "boot-028-xlang-asm2-ci-matrix gate FAIL: doc missing '$kw'" >&2
    exit 1
  fi
done

if ! grep -qF 'boot028_xlang_asm2_ci_matrix' "$REPRO" 2>/dev/null; then
  echo "boot-028-xlang-asm2-ci-matrix gate FAIL: bootstrap-repro missing boot028" >&2
  exit 1
fi
...
if [ "$ROW_N" -lt "$MIN_ROWS" ]; then
  echo "boot-028-xlang-asm2-ci-matrix gate FAIL: rows=${ROW_N} < min ${MIN_ROWS}" >&2
  exit 1
fi
...
if [ "$HOOK_N" -lt "$MIN_HOOKS" ]; then
  echo "boot-028-xlang-asm2-ci-matrix gate FAIL: hooks=${HOOK_N} < min ${MIN_HOOKS}" >&2
  exit 1
fi
...
if ! grep -F 'C7_xlang_asm2_ci_matrix' "$MATRIX" 2>/dev/null | grep -qF 'run-boot-028-xlang-asm2-ci-matrix-gate.sh'; then
  echo "boot-028 FAIL: C7_xlang_asm2_ci_matrix not wired to boot-028 gate" >&2
  MISS=$((MISS + 1))
fi
...
echo "boot-028-xlang-asm2-ci-matrix manifest OK (rows=${ROW_N} hooks=${HOOK_N} linux_must=${LINUX_M} macos_must=${MACOS_M})"

echo "=== BOOT-028: parent BOOT-027 manifest ==="
chmod +x tests/run-boot-027-xlang-asm2-cross-gate.sh
XLANG_BOOT027_MANIFEST_ONLY=1 ./tests/run-boot-027-xlang-asm2-cross-gate.sh

if [ "${XLANG_BOOT028_MANIFEST_ONLY:-0}" = "1" ]; then
  boot028_emit_report "ok" 0 0 1
  echo "boot-028-xlang-asm2-ci-matrix gate OK (manifest only)"
  exit 0
fi
...
while IFS=$'\t' read -r case_id script linux_p macos_p win_p ok_pat _notes; do
  ...
  if [ "$case_id" = "boot027_parent" ]; then
    extra_env=(XLANG_BOOT027_MANIFEST_ONLY=1)
  fi
  set +e
  ...
  if [ "$case_id" = "xlang_asm2_cross_smoke" ]; then
    SMOKE_LOG="$log"
...
SKIP=0
if [ -n "$SMOKE_LOG" ] && grep -qF 'xlang-asm2-cross-smoke SKIP' "$SMOKE_LOG" 2>/dev/null; then
  SKIP=1
fi

boot028_emit_report "ok" "$C6_MATRIX_OK" "$C6_SMOKE" "$SKIP"
echo "boot-028-xlang-asm2-ci-matrix gate OK"
