...
exitcode=0
/tmp/xlang_asm_div_index_binop >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 2 ] && { echo "run-asm-binop-div-index FAIL: div index/binop expected 2, got $exitcode"; exit 1; }

$XLANG build tests/asm/binop_div_binop_index.x -o /tmp/xlang_asm_div_binop_index 2>&1
exitcode=0
/tmp/xlang_asm_div_binop_index >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 10 ] && { echo "run-asm-binop-div-index FAIL: div binop/index expected 10, got $exitcode"; exit 1; }

$XLANG build tests/asm/assign_index_block_seq.x -o /tmp/xlang_asm_assign_index_block 2>&1
exitcode=0
/tmp/xlang_asm_assign_index_block >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 45 ] && { echo "run-asm-binop-div-index FAIL: block seq expected 45, got $exitcode"; exit 1; }
if otool -tv /tmp/xlang_asm_assign_index_block 2>/dev/null | sed -n '/^_main:/,/^_[a-z]/p' | grep -q 'mov x2'; then
