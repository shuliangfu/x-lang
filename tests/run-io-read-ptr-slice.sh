...
$RUN_SHUX build -L . tests/io/read_ptr_slice.x -o /tmp/shux_io_read_ptr_slice 2>&1
echo -n "AB" | /tmp/shux_io_read_ptr_slice
ec=$?
[ "$ec" -ne 0 ] && { echo "expected exit 0 (read_stdin_ptr_slice), got $ec"; exit 1; }

$RUN_SHUX build -L . tests/io/read_ptr_slice_param.x -o /tmp/shux_io_read_ptr_slice_param 2>&1
