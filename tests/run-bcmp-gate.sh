...
#   XLANG_PERF_C_O3_RATIO=1.0 ./tests/run-bcmp-gate.sh
set -e
cd "$(dirname "$0")/.."
chmod +x tests/run-perf-baseline.sh
XLANG_PERF_FAIL_ON_C_O3=1 XLANG_PERF_FAIL_ON_ZIG=0 XLANG_PERF_C_O3_RATIO="${XLANG_PERF_C_O3_RATIO:-1.0}" \
  ./tests/run-perf-baseline.sh --bench | tee /tmp/bcmp_gate.log
grep -q 'perf baseline OK' /tmp/bcmp_gate.log
echo "bcmp gate OK (≤${XLANG_PERF_C_O3_RATIO:-1.0}× C -O3 on loop_i32/mem_copy/struct_param/call_boundary)"
