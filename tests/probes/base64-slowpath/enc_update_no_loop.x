...
/** Internal function `base64_stream_enc_update_no_loop_c`.
 * Implements `base64_stream_enc_update_no_loop_c`.
 * @param state *u8
 * @param state_cap i32
 * @param inp *u8
 * @param in_len i32
 ...
 * @return i32
 */
function base64_stream_enc_update_no_loop_c(state: *u8, state_cap: i32, inp: *u8, in_len: i32, out: *u8, out_cap: i32, is_last: i32, in_consumed: *i32): i32 {
  ...
  if (chunk > 3) { chunk = 3; }
  if (chunk <= 0) { return 0; }
  n = emit_stub(s, inp, chunk, is_last, out, out_cap);
  if (n < 0) { return -1; }
...
function main(): i32 {
  ...
  return base64_stream_enc_update_no_loop_c(&st[0], 32, &inp[0], 5, &out[0], 16, 1, &consumed);
}
