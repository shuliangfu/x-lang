...
/** Internal function `generic_update_no_loop_c`.
 * Implements `generic_update_no_loop_c`.
 * @param state *u8
 * @param state_cap i32
 * @param inp *u8
 * @param in_len i32
 ...
 * @return i32
 */
function generic_update_no_loop_c(state: *u8, state_cap: i32, inp: *u8, in_len: i32, out: *u8, out_cap: i32, is_last: i32, in_consumed: *i32): i32 {
  ...
  if (chunk > 4) { chunk = 4; }
  if (chunk <= 0) { return 0; }
  n = update_emit_stub(s, inp, chunk, is_last, out, out_cap);
  if (n < 0) { return -1; }
...
function main(): i32 {
  ...
  return generic_update_no_loop_c(&st[0], 16, &inp[0], 8, &out[0], 16, 1, &consumed);
}
