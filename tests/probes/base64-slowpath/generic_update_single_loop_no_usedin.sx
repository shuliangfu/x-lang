...
function generic_update_single_loop_no_usedin_c(state: *u8, state_cap: i32, inp: *u8, in_len: i32, out: *u8, out_cap: i32, is_last: i32, in_consumed: *i32): i32 {
  ...
  while (rem > 0) {
    ...
    if (rem < 4) { chunk = rem; }
    if (chunk <= 0) { break; }
    n = update_emit_stub(s, cursor, chunk, (rem <= chunk && is_last != 0) as i32, out, out_cap);
    if (n < 0) {
      if (in_consumed != 0) { in_consumed[0] = in_len - rem; }
      return -1;
    }
    cursor = cursor + chunk;
    rem = rem - chunk;
...
function main(): i32 {
  ...
  return generic_update_single_loop_no_usedin_c(&st[0], 16, &inp[0], 8, &out[0], 16, 1, &consumed);
}
