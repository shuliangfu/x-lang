...
/** Internal function `generic_update_single_loop_no_chunk_no_islast_c`.
 * Implements `generic_update_single_loop_no_chunk_no_islast_c`.
 * @param state *u8
 * @param state_cap i32
 * @param inp *u8
 * @param in_len i32
 ...
 * @return i32
 */
function generic_update_single_loop_no_chunk_no_islast_c(state: *u8, state_cap: i32, inp: *u8, in_len: i32, out: *u8, out_cap: i32, in_consumed: *i32): i32 {
  ...
  while (used_in < in_len) {
    n = update_emit_stub(s, cursor, 1, 0, out, out_cap);
    if (n < 0) {
...
function main(): i32 {
  ...
  return generic_update_single_loop_no_chunk_no_islast_c(&st[0], 16, &inp[0], 8, &out[0], 16, &consumed);
}
