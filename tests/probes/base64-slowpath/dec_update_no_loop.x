allow(padding) struct B64Stream {
  magic: u32
  is_url: i32
  is_enc: i32
  pending_len: i32
  pending: u8[3]
  dec_len: i32
  dec_pending: u8[4]
}

const SHU_B64_STREAM_MAGIC: u32 = 0x42345354;
const B64_STREAM_STATE_BYTES: i32 = 28;

extern function memset(s: *u8, c: i32, n: usize): *u8;

/** Internal function `b64_stream_cast`.
 * Implements `b64_stream_cast`.
 * @param state *u8
 * @param state_cap i32
 * @return *B64Stream
 */
function b64_stream_cast(state: *u8, state_cap: i32): *B64Stream {
  let need: i32 = B64_STREAM_STATE_BYTES;
  if (state == 0 || state_cap < need) { return 0 as *B64Stream; }
  let s: *B64Stream = state as *B64Stream;
  if (s.magic != SHU_B64_STREAM_MAGIC) { return 0 as *B64Stream; }
  return s;
}

/** Internal function `base64_stream_dec_init_c`.
 * Implements `base64_stream_dec_init_c`.
 * @param state *u8
 * @param state_cap i32
 * @param url i32
 * @return i32
 */
function base64_stream_dec_init_c(state: *u8, state_cap: i32, url: i32): i32 {
  let need: i32 = B64_STREAM_STATE_BYTES;
  let s: *B64Stream = 0 as *B64Stream;
  if (state == 0 || state_cap < need) { return -1; }
  unsafe { memset(state, 0, need); }
  s = state as *B64Stream;
  s.magic = SHU_B64_STREAM_MAGIC;
  s.is_url = (url != 0) ? 1 : 0;
  s.is_enc = 0;
  return 0;
}

/** Internal function `decode_stub`.
 * Implements `decode_stub`.
 * @param s *B64Stream
 * @param quad *u8
 * @param n i32
 * @param is_last i32
 * @param out *u8
 * @param out_cap i32
 * @return i32
 */
function decode_stub(s: *B64Stream, quad: *u8, n: i32, is_last: i32, out: *u8, out_cap: i32): i32 {
  if (s == 0 || quad == 0 || out == 0 || out_cap <= 0 || n <= 0) { return -1; }
  out[0] = quad[0];
  if (is_last != 0 && n > 1 && out_cap > 1) {
    out[1] = quad[1];
    return 2;
  }
  return 1;
}

/** Internal function `base64_stream_dec_update_no_loop_c`.
 * Implements `base64_stream_dec_update_no_loop_c`.
 * @param state *u8
 * @param state_cap i32
 * @param inp *u8
 * @param in_len i32
 * @param out *u8
 * @param out_cap i32
 * @param is_last i32
 * @param in_consumed *i32
 * @return i32
 */
function base64_stream_dec_update_no_loop_c(state: *u8, state_cap: i32, inp: *u8, in_len: i32, out: *u8, out_cap: i32, is_last: i32, in_consumed: *i32): i32 {
  let s: *B64Stream = b64_stream_cast(state, state_cap);
  let chunk: i32 = 0;
  let n: i32 = 0;
  if (s == 0) { return -1; }
  if (s.is_enc != 0) { return -1; }
  if (out == 0) { return -1; }
  if (out_cap <= 0) { return -1; }
  if (in_len > 0 && inp == 0) { return -1; }
  if (in_consumed != 0) { in_consumed[0] = 0; }
  if (in_len <= 0) { return 0; }
  chunk = in_len;
  if (chunk > 4) { chunk = 4; }
  if (chunk <= 0) { return 0; }
  n = decode_stub(s, inp, chunk, is_last, out, out_cap);
  if (n < 0) { return -1; }
  if (in_consumed != 0) { in_consumed[0] = chunk; }
  return n;
}

/** Internal function `main`.
 * Program/test entry point.
 * @return i32
 */
function main(): i32 {
  let st: u8[32] = [];
  let out: u8[16] = [];
  let inp: u8[8] = [81, 85, 74, 68, 82, 69, 86, 71];
  let consumed: i32 = 0;
  if (base64_stream_dec_init_c(&st[0], 32, 0) != 0) { return -1; }
  return base64_stream_dec_update_no_loop_c(&st[0], 32, &inp[0], 8, &out[0], 16, 1, &consumed);
}
