...
/** Internal function `generic_update_single_loop_no_chunk_inline_c`.
 * Implements `generic_update_single_loop_no_chunk_inline_c`.
 * @param state *u8
 * @param state_cap i32
 * @param inp *u8
 * @param in_len i32
 ...
 * @return i32
 */
function generic_update_single_loop_no_chunk_inline_c(state: *u8, state_cap: i32, inp: *u8, in_len: i32, out: *u8, out_cap: i32, is_last: i32, in_consumed: *i32): i32 {
  ...
  while (used_in < in_len) {
    out[0] = cursor[0];
    if ((used_in + 1) >= in_len && is_last != 0 && out_cap > 1) {
      out[1] = 33;
    }
    cursor = cursor + 1;
    used_in = used_in + 1;
...
function main(): i32 {
  ...
  return generic_update_single_loop_no_chunk_inline_c(&st[0], 16, &inp[0], 8, &out[0], 16, 1, &consumed);
}
