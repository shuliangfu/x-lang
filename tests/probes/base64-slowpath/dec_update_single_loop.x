...
/** Internal function `base64_stream_dec_update_single_loop_c`.
 * Implements `base64_stream_dec_update_single_loop_c`.
 * @param state *u8
 * @param state_cap i32
 * @param inp *u8
 * @param in_len i32
 ...
 * @return i32
 */
function base64_stream_dec_update_single_loop_c(state: *u8, state_cap: i32, inp: *u8, in_len: i32, out: *u8, out_cap: i32, is_last: i32, in_consumed: *i32): i32 {
  ...
  while (used_in < in_len) {
    ...
    if (chunk <= 0) { break; }
    n = decode_stub(s, inp + used_in, chunk, ((used_in + chunk) >= in_len && is_last != 0) as i32, out, out_cap);
    if (n < 0) {
...
function main(): i32 {
  ...
  return base64_stream_dec_update_single_loop_c(&st[0], 32, &inp[0], 8, &out[0], 16, 1, &consumed);
}
