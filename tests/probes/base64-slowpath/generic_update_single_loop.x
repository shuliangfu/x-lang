...
/** Internal function `update_emit_stub`.
 * Implements `update_emit_stub`.
 * @param s *UpdateState
 * @param inp *u8
 * @param n i32
 * @param is_last i32
...
function update_emit_stub(s: *UpdateState, inp: *u8, n: i32, is_last: i32, out: *u8, out_cap: i32): i32 {
  ...
  out[0] = inp[0];
  if (n > 1 && out_cap > 1) { out[1] = inp[1]; }
  if (is_last != 0 && out_cap > 2) { out[2] = 33; }
  return 1;
}

/** Internal function `generic_update_single_loop_c`.
 * Implements `generic_update_single_loop_c`.
 * @param state *u8
 * @param state_cap i32
 * @param inp *u8
 * @param in_len i32
 ...
 * @return i32
 */
function generic_update_single_loop_c(state: *u8, state_cap: i32, inp: *u8, in_len: i32, out: *u8, out_cap: i32, is_last: i32, in_consumed: *i32): i32 {
  ...
  while (used_in < in_len) {
    chunk = 4;
    if (in_len - used_in < 4) { chunk = in_len - used_in; }
    if (chunk <= 0) { break; }
    n = update_emit_stub(s, inp + used_in, chunk, ((used_in + chunk) >= in_len && is_last != 0) as i32, out, out_cap);
    if (n < 0) {
...
function main(): i32 {
  ...
  return generic_update_single_loop_c(&st[0], 16, &inp[0], 8, &out[0], 16, 1, &consumed);
}
