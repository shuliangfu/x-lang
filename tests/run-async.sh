...
if [ "$(uname -s)" = "Linux" ]; then
  XLANG_IO_CC_LIBS="-luring -lpthread"
fi

# musl/Alpine/Docker：async C harness 中 setenv 须 POSIX 宏。
XLANG_ASYNC_CC=(cc -std=gnu11 -Wall -Wextra -D_POSIX_C_SOURCE=200809L)
...
# Windows MSYS2：seed xlang -backend c 编译会挂起，须走 xlang-c。
...
# 非 Linux x86_64 的 -o 烟测：优先 xlang-c（DCE 已保留副作用 let；Win 避免 seed -backend c 挂起）。
async_host_compile_o() {
  local src="$1"
  local out="$2"
  shift 2
  if async_is_linux_x64_asm; then
    "$XLANG" build -L . "$@" "$src" -o "$out"
...
if [ -z "${XLANG:-}" ]; then
  if [ -x ./compiler/xlang ]; then
    ...
    echo "async smoke FAIL: no compiler/xlang or compiler/xlang-c" >&2
    exit 1
  fi
fi

# relink 后 seed xlang 的 X codegen 在 run/spawn -o 链路上可能 SIGSEGV；C 前端 -o 烟测与 EMIT_XLANG 对齐用 xlang-c。
COMPILE_XLANG="$XLANG"
if [ -x ./compiler/xlang-c ]; then
  COMPILE_XLANG=./compiler/xlang-c
fi
...
if async_is_linux_x64_asm; then
  ...
  echo "async_switch: compile+run (xlang-c) ..."
elif [ -x ./compiler/xlang ]; then
  echo "async_switch: compile+run (seed xlang -backend c) ..."
else
  echo "async_switch: compile+run (${COMPILE_XLANG##*/} fallback) ..."
fi
async_switch_compile_o
[ -x /tmp/xlang_async_switch ] || { echo "async_switch FAIL: binary not built"; exit 1; }
rc=$(/tmp/xlang_async_switch; echo $?)
...
if async_is_linux_x64_asm; then
  ...
  if "$XLANG" build -L . tests/bench/async_switch_sched.x -backend asm -o /tmp/xlang_async_sched >"$sched_log" 2>&1; then
    ...
    echo "async_switch_sched N/A (asm backend not in $XLANG build yet)"
  else
    cat "$sched_log" >&2
...
# import std.async + coop_pingpong_jmp：非 Linux x86_64 优先 xlang-c，避免 seed -o 挂起。
XLANG_IMPORT=${XLANG_IMPORT:-$XLANG} build if ! async_is_linux_x64_asm && [ -x ./compiler/xlang-c ]; then
...
rc=$(/tmp/xlang_async_mod_import; echo $?)
[ "$rc" = "0" ] || { echo "async_mod_import failed exit=$rc"; exit 1; }

echo "async smoke OK"

echo "async syntax: async function parse + compile ..."
async_host_compile_o tests/parser/async_function.x /tmp/xlang_async_fn_syntax 2>&1 || {
  echo "async syntax FAIL: async_function.x should compile"
  exit 1
}
rc=$(/tmp/xlang_async_fn_syntax; echo $?)
[ "$rc" = "42" ] || { echo "async syntax FAIL: expected exit 42, got $rc"; exit 1; }
echo "async syntax OK"

echo "async import std.async: parse (-E) + dep codegen ($XLANG_IMPORT) ..."
out=$("$XLANG_IMPORT" -L . -E tests/parser/import_std_async.x 2>&1) || {
  echo "async import std.async FAIL: -E import_std_async.x"
  exit 1
}
echo "$out" | grep -q 'xlang_async_queue_reset' || {
  echo "async import std.async FAIL: missing xlang_async_queue_reset from std.async"
  exit 1
}
echo "async import std.async OK"

echo "const hex: top-level 0x literal parse (-E) ($XLANG_IMPORT) ..."
# relink 后 xlang/xlang_asm 的 -E 仅 parse/typeck 摘要；MAGIC 数值须 xlang-c -E 佐证。
EMIT_HEX="$XLANG_IMPORT"
case "$(basename "$XLANG_IMPORT")" in
  xlang|xlang_asm)
    if [ -x ./compiler/xlang-c ]; then
      EMIT_HEX=./compiler/xlang-c
    fi
    ;;
esac
...
async_host_compile_o tests/parser/await_async.x /tmp/xlang_await_async 2>&1 || {
  echo "await syntax FAIL: await_async.x should compile"
  exit 1
}
rc=$(/tmp/xlang_await_async; echo $?)
[ "$rc" = "42" ] || { echo "await syntax FAIL: expected exit 42, got $rc"; exit 1; }

if "$COMPILE_XLANG" -L . tests/parser/await_sync_err.x -o /tmp/xlang_await_sync_err 2>&1 | grep -q "await.*only allowed inside async function"; then
  : # 预期 typeck 报错
else
...
echo "async liveness A3: XLANG_ASYNC_FRAME in -E output ..."
LIVENESS_EMIT="$EMIT_XLANG"
out=$("$LIVENESS_EMIT" -E tests/parser/async_await_liveness.x 2>&1) || {
  echo "async liveness FAIL: async_await_liveness.x -E failed"
  exit 1
}
echo "$out" | grep -qE 'XLANG_ASYNC_FRAME func=liveness_demo.*slots=1.*vars=keep' || {
  echo "async liveness FAIL: expected slots=1 vars=keep"
  echo "$out" | grep XLANG_ASYNC_FRAME || true
  exit 1
}
echo "$out" | grep -q 'typedef struct __xlang_async_frame_liveness_demo' || {
...
echo "$out" | grep 'XLANG_ASYNC_FRAME func=liveness_demo' | grep -q 'dead' && {
...
echo "$out2" | grep -qE 'XLANG_ASYNC_FRAME func=two_await.*slots=2.*vars=a,b' || {
  echo "async liveness FAIL: expected two_await slots=2 vars=a,b"
  echo "$out2" | grep XLANG_ASYNC_FRAME || true
  exit 1
}
echo "$out2" | grep -q 'typedef struct __xlang_async_frame_two_await' || {
...
echo "$out" | grep -q 'XLANG_ASYNC_CPS switch=1' || {
  echo "async CPS FAIL: missing XLANG_ASYNC_CPS marker"
  exit 1
}
echo "$out" | grep -q 'switch (__xlang_frame.__phase)' || {
...
echo "$out" | grep -q '__xlang_frame.keep = keep' || {
...
echo "$out" | grep -q 'xlang_async_cps_suspend' || {
  echo "async CPS FAIL: missing xlang_async_cps_suspend at await boundary"
  exit 1
}
echo "$out" | grep -q 'static __xlang_async_frame_liveness_demo __xlang_frame' || {
  echo "async CPS FAIL: frame must be static for suspend/resume"
  exit 1
}

echo "async liveness WPO-S3: struct Pair across await (-E) ..."
out_await=$("$EMIT_XLANG" -E tests/wpo/stack_promote_await.x 2>&1) || {
...
echo "$out_await" | grep -qE 'XLANG_ASYNC_FRAME func=struct_across_await.*slots=1.*vars=p' || {
  echo "async struct await FAIL: expected slots=1 vars=p"
  echo "$out_await" | grep XLANG_ASYNC_FRAME || true
  exit 1
}
...
echo "$out_await" | grep -q '__xlang_frame.p = p' || {
  echo "async struct await FAIL: missing frame save __xlang_frame.p = p at await boundary"
  exit 1
}
...
if [ -f compiler/../std/async/scheduler.o ] || [ -f std/async/scheduler.o ]; then
  ...
  if file /tmp/xlang_async_cps_yield 2>/dev/null | grep -q "executable"; then
    rc=$(/tmp/xlang_async_cps_yield; echo $?)
  ...
  XLANG_ASYNC_YIELD=1 async_host_compile_o tests/wpo/stack_promote_await_yield.x /tmp/xlang_wpo_await_yield 2>&1 || {
    echo "async struct await yield FAIL: compile stack_promote_await_yield.x"
    exit 1
  }
  if file /tmp/xlang_wpo_await_yield 2>/dev/null | grep -q "executable"; then
    rc=$(XLANG_ASYNC_YIELD=1 /tmp/xlang_wpo_await_yield; echo $?)
...
rc=$(/tmp/xlang_async_scheduler_queue; echo $?)
...
rc=$(/tmp/xlang_async_scheduler_io_wake; echo $?)
...
rc=$(/tmp/xlang_async_scheduler_io_await_flag; echo $?)
[ "$rc" = "0" ] || { echo "async scheduler IO-A5 await flag FAIL: exit=$rc"; exit 1; }
echo "async scheduler IO-A5 await flag OK"

echo "async await IO: await read() codegen (xlang_async_cps_suspend_io) ..."
out=$("$EMIT_XLANG" -E tests/parser/async_await_io.x 2>&1) || {
  echo "async await IO FAIL: -E async_await_io.x"
  exit 1
}
echo "$out" | grep -q '__xlang_frame.__io_rd_slot = xlang_io_submit_read_async' || {
  echo "async await IO FAIL: missing __io_rd_slot submit at await read boundary"
  exit 1
}
echo "$out" | grep -q 'xlang_io_complete_read_async_slot(__xlang_frame.__io_rd_slot)' || {
...
echo "$out" | grep -q 'xlang_async_cps_suspend_io' || {
  echo "async await IO FAIL: missing xlang_async_cps_suspend_io at await read boundary"
  exit 1
}
echo "$out" | grep -q 'XLANG_ASYNC_CPS switch=1' || {
...
out=$("$EMIT_XLANG" -E tests/parser/async_await_io_write.x 2>&1) || {
  echo "async await IO write FAIL: -E async_await_io_write.x"
  exit 1
}
echo "$out" | grep -q 'xlang_io_submit_write_async' || {
  echo "async await IO write FAIL: missing xlang_io_submit_write_async"
  exit 1
}
echo "$out" | grep -q 'xlang_io_complete_write_async' || {
  echo "async await IO write FAIL: missing xlang_io_complete_write_async"
  exit 1
}
echo "async await IO write OK"

echo "async await IO read_fd: await read_fd() codegen ..."
out=$("$EMIT_XLANG" -E tests/parser/async_await_read_fd.x 2>&1) || {
  echo "async await IO read_fd FAIL: -E async_await_read_fd.x"
  exit 1
}
echo "$out" | grep -q 'xlang_io_submit_read_async' || {
  echo "async await IO read_fd FAIL: missing xlang_io_submit_read_async"
  exit 1
}
...
out=$("$EMIT_XLANG" -E tests/parser/async_await_write_fd.x 2>&1) || {
  echo "async await IO write_fd FAIL: -E async_await_write_fd.x"
  exit 1
}
echo "$out" | grep -q 'xlang_io_submit_write_async' || {
  echo "async await IO write_fd FAIL: missing xlang_io_submit_write_async"
  exit 1
}
echo "$out" | grep -q '(size_t)(unsigned)(' || {
...
out=$("$EMIT_XLANG" -E tests/parser/async_await_io_dual.x 2>&1) || {
...
echo "$out" | grep -E 'xlang_async_run_seed_push_i32\([0-9]' | wc -l | tr -d ' ' | grep -q '^2$' || {
  echo "async await IO dual FAIL: expected two run seed push"
  exit 1
}
echo "$out" | grep -E 'xlang_async_sched_read_chunk\(\)' | wc -l | tr -d ' ' | grep -q '^2$' || {
  echo "async await IO dual FAIL: expected two sched_read_chunk calls"
  exit 1
}
echo "$out" | grep -q 'complete_read_async_slot(__xlang_frame.__io_rd_slot)' || {
...
echo "async scheduler drain: run + XLANG_ASYNC_YIELD=1 ..."
out=$("$EMIT_XLANG" -E tests/parser/async_scheduler_drain.x 2>&1) || {
  echo "async scheduler drain FAIL: -E async_scheduler_drain.x"
  exit 1
}
echo "$out" | grep -q 'xlang_async_sched_yield_demo' || {
  echo "async scheduler drain FAIL: missing xlang_async_sched_yield_demo wrapper"
  exit 1
}
echo "$out" | grep -q '__xlang_frame.__phase = 0' || {
...
if make -C compiler ../std/async/scheduler.o -q 2>/dev/null; then
  ...
  if XLANG_ASYNC_YIELD=1 "$COMPILE_XLANG" -L . tests/parser/async_scheduler_drain.x -o /tmp/xlang_async_sched_drain 2>/tmp/xlang_async_sched_drain.log; then
    if file /tmp/xlang_async_sched_drain 2>/dev/null | grep -q "executable"; then
      rc=$(XLANG_ASYNC_YIELD=1 /tmp/xlang_async_sched_drain; echo $?)
    ...
    echo "async scheduler drain: skip run (compile: $(head -1 /tmp/xlang_async_sched_drain.log 2>/dev/null || echo xlang failed))"
  fi
else
...
out=$("$EMIT_XLANG" -E tests/parser/run_async.x 2>&1) || {
  echo "run async FAIL: -E run_async.x"
  exit 1
}
echo "$out" | grep -q 'xlang_async_sched_yield_demo()' || {
  echo "run async FAIL: expected xlang_async_sched_yield_demo() in -E"
  exit 1
}
if "$COMPILE_XLANG" -L . tests/parser/run_async_sync_err.x -o /tmp/xlang_run_async_err 2>&1 | grep -q "run.*not allowed inside async"; then
  : # 预期 typeck 报错
else
...
out=$("$EMIT_XLANG" -E tests/parser/run_async_arg.x 2>&1) || {
  echo "run async v1 FAIL: -E run_async_arg.x"
  exit 1
}
echo "$out" | grep -q 'xlang_async_run_seed_push_i32' || {
  echo "run async v1 FAIL: missing xlang_async_run_seed_push_i32"
  exit 1
}
echo "$out" | grep -q 'xlang_async_run_seed_take_i32' || {
...
if make -C compiler ../std/async/scheduler.o -q 2>/dev/null; then
  ...
  if file /tmp/xlang_run_async_arg 2>/dev/null | grep -q "executable"; then
    rc=$(/tmp/xlang_run_async_arg; echo $?)
...
out=$("$EMIT_XLANG" -E tests/parser/run_async_arg2.x 2>&1) || {
  echo "run async v2 FAIL: -E run_async_arg2.x"
  exit 1
}
echo "$out" | grep -q 'xlang_async_run_seed_reset' || {
  echo "run async v2 FAIL: missing xlang_async_run_seed_reset"
  exit 1
}
# 两处 run add_two(…) 各 push 两个 i32；grep -c 数行不准（comma-expr 同一行含多次 push）
n_push=$(echo "$out" | grep -oE 'xlang_async_run_seed_push_i32\([0-9]+' | wc -l | tr -d ' ')
[ "$n_push" = "4" ] || {
  echo "run async v2 FAIL: expected 4 push_i32 calls (2 runs x 2 args), got $n_push"
  exit 1
}
echo "$out" | grep -q 'a = xlang_async_run_seed_take_i32' || {
  echo "run async v2 FAIL: missing seed take for param a"
  exit 1
}
echo "$out" | grep -q 'b = xlang_async_run_seed_take_i32' || {
...
if make -C compiler ../std/async/scheduler.o -q 2>/dev/null; then
  ...
  if file /tmp/xlang_run_async_arg2 2>/dev/null | grep -q "executable"; then
    rc=$(/tmp/xlang_run_async_arg2; echo $?)
...
out=$("$EMIT_XLANG" -E tests/parser/run_async_u32.x 2>&1) || {
  echo "run async v3 FAIL: -E run_async_u32.x"
  exit 1
}
echo "$out" | grep -q 'xlang_async_run_seed_push_u32' || {
  echo "run async v3 FAIL: missing push_u32"
  exit 1
}
echo "$out" | grep -q 'xlang_async_run_seed_take_u32' || {
  echo "run async v3 FAIL: missing take_u32 in async entry"
  exit 1
}
out2=$("$EMIT_XLANG" -E tests/parser/run_async_i64.x 2>&1) || {
  echo "run async v3 FAIL: -E run_async_i64.x"
  exit 1
}
echo "$out2" | grep -q 'xlang_async_run_seed_push_i64' || {
  echo "run async v3 FAIL: missing push_i64"
  exit 1
}
echo "$out2" | grep -q 'xlang_async_run_seed_take_i64' || {
  echo "run async v3 FAIL: missing take_i64"
  exit 1
}
if "$COMPILE_XLANG" -L . tests/parser/run_async_type_err.x -o /tmp/xlang_run_async_type_err 2>&1 | grep -q "type mismatch"; then
  : # 预期 typeck 报错
else
  echo "run async v3 FAIL: i32 arg to u32 param should be rejected"
  exit 1
fi
if [ -x "$COMPILE_XLANG" ]; then
  "$COMPILE_XLANG" -L . tests/parser/run_async_u32.x -o /tmp/xlang_run_async_u32 2>&1 || {
    echo "run async v3 FAIL: compile run_async_u32.x"
    exit 1
  }
  if file /tmp/xlang_run_async_u32 2>/dev/null | grep -q "executable"; then
    rc=$(/tmp/xlang_run_async_u32; echo $?)
    [ "$rc" = "0" ] || { echo "run async v3 FAIL: run_async_u32 exit=$rc"; exit 1; }
    echo "run async v3 run_async_u32 OK"
  fi
  "$COMPILE_XLANG" -L . tests/parser/run_async_i64.x -o /tmp/xlang_run_async_i64 2>&1 || {
    echo "run async v3 FAIL: compile run_async_i64.x"
    exit 1
  }
  if file /tmp/xlang_run_async_i64 2>/dev/null | grep -q "executable"; then
    rc=$(/tmp/xlang_run_async_i64; echo $?)
...
out=$("$EMIT_XLANG" -E tests/parser/run_async_usize.x 2>&1) || {
  echo "run async v4 FAIL: -E run_async_usize.x"
  exit 1
}
echo "$out" | grep -q 'xlang_async_run_seed_push_usize' || {
  echo "run async v4 FAIL: missing push_usize"
  exit 1
}
echo "$out" | grep -q 'xlang_async_run_seed_take_usize' || {
  echo "run async v4 FAIL: missing take_usize in async entry"
  exit 1
}
if [ -x "$COMPILE_XLANG" ]; then
  "$COMPILE_XLANG" -L . tests/parser/run_async_usize.x -o /tmp/xlang_run_async_usize 2>&1 || {
    echo "run async v4 FAIL: compile run_async_usize.x"
    exit 1
  }
  if file /tmp/xlang_run_async_usize 2>/dev/null | grep -q "executable"; then
    rc=$(/tmp/xlang_run_async_usize; echo $?)
...
if [ -x ./compiler/xlang-c ]; then
  ...
  if file /tmp/xlang_run_async_arg_full 2>/dev/null | grep -q "executable"; then
    rc=$(/tmp/xlang_run_async_arg_full; echo $?)
  ...
  ./compiler/xlang-c -L . tests/parser/run_async_arg2.x -o /tmp/xlang_run_async_arg2_full 2>&1 || {
    echo "async linux full FAIL: compile run_async_arg2.x"
    exit 1
  }
  if file /tmp/xlang_run_async_arg2_full 2>/dev/null | grep -q "executable"; then
    rc=$(/tmp/xlang_run_async_arg2_full; echo $?)
  ...
  if [ "$ASYNC_LINUX_FULL_IO" = "1" ]; then
    XLANG_ASYNC_YIELD=1 ./compiler/xlang-c -L . tests/parser/async_run_io_stdin.x -o /tmp/xlang_async_run_io_stdin 2>&1 || {
      echo "async linux full FAIL: compile async_run_io_stdin.x"
      exit 1
    }
    if file /tmp/xlang_async_run_io_stdin 2>/dev/null | grep -q "executable"; then
      rc=$(printf 'abcd' | XLANG_ASYNC_YIELD=1 /tmp/xlang_async_run_io_stdin; echo $?)
    ...
    XLANG_ASYNC_YIELD=1 ./compiler/xlang-c -L . tests/parser/async_run_io_dual_pipe.x -o /tmp/xlang_async_run_io_dual_pipe 2>&1 || {
    ...
    XLANG_ASYNC_YIELD=1 ./compiler/xlang-c -L . tests/parser/async_run_io_dual_parallel.x -o /tmp/xlang_async_run_io_dual_parallel 2>&1 || {
  ...
  echo "async linux full: skip (no compiler/xlang-c)"
fi
...
out=$("$EMIT_XLANG" -L . -E tests/parser/async_spawn_v4_smoke.x 2>&1) || {
  echo "async spawn v4 FAIL: -E async_spawn_v4_smoke.x"
  exit 1
}
echo "$out" | grep -q 'xlang_async_queue_reset' || {
  echo "async spawn v4 FAIL: missing xlang_async_queue_reset (import std.async)"
  exit 1
}
echo "$out" | grep -q 'xlang_async_task_submit' || {
  echo "async spawn v4 FAIL: missing xlang_async_task_submit"
  exit 1
}
echo "$out" | grep -q 'xlang_async_run_drain_until_idle' || {
  echo "async spawn v4 FAIL: missing xlang_async_run_drain_until_idle"
  exit 1
}
# 两处 spawn 各一次 submit；按调用点计数（extern 声明也占 grep -c 行）
n_submit=$(echo "$out" | grep -oE 'xlang_async_task_submit\s*\(' | wc -l | tr -d ' ')
