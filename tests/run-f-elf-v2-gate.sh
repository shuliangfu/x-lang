#!/usr/bin/env bash
# F-elf v2：std.elf 逻辑下沉（解析/写入/烟测 → elf.sx；F-ZC 纯 .sx 无 io glue）。
set -e
cd "$(dirname "$0")/.."
...
echo "=== F-elf v2: ELF logic → elf.sx (F-ZC zero C) ==="
...
grep -q 'elf64_parse_hdr_c' std/elf/elf.sx || die "elf.sx missing parse_hdr"
grep -q 'elf64_write_min_reloc_c' std/elf/elf.sx || die "elf.sx missing write"
grep -q 'elf64_parse_smoke_c' std/elf/elf.sx || die "elf.sx missing smoke"
grep -q 'elf_f_elf_v2_marker_c' std/elf/elf.sx || die "elf.sx missing v2 marker"
grep -q 'elf_f_zero_c_marker_c' std/elf/elf.sx || die "elf.sx missing zero-c marker"
grep -q 'elf_read_fixture_c' std/elf/elf.sx || die "elf.sx missing read_fixture"
grep -q 'fs_open_read_c' std/elf/elf.sx || die "elf.sx missing fs fixture IO"
