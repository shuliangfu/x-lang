...
# Restore xlang.bak (if any) then always print wall time — single EXIT trap so later
# paths do not overwrite duration reporting.
_run_all_on_exit() {
  if [ -f compiler/xlang.bak ]; then
    mv compiler/xlang.bak compiler/xlang 2>/dev/null || true
  fi
  _run_all_print_elapsed
}
...
# "C:\Users\...\AppData\Loc"，导致 xlang-c.exe 内部 gcc 调用失败 → std .o 构建失败 → 82/90 测试回归。
...
# （run-import/run-std/run-stdlib-import 等用 `out=$($XLANG build file.x)` 检查 parse/typeck 输出全挂）。
# 修复：Windows 下 export XLANG_PIPELINE_NO_LARGE_STACK=1，跳过 pthread，直接在当前栈上跑。
case "$(uname -s 2>/dev/null)" in
  MINGW*|MSYS*|CYGWIN*)
    export TEMP=C:/xlang_tmp
    export TMP=C:/xlang_tmp
    mkdir -p "$TEMP" 2>/dev/null || true
    export XLANG_PIPELINE_NO_LARGE_STACK=1
...
# RUN_ALL_PORTABLE=1：全平台可移植子集（xlang-c、无 asm/自举）；供 run-portable-c.sh / lite CI 使用。
...
if [ -n "$XLANG" ]; then
    ...
    # B-strict run-all（tests/run-all-bstrict.sh）：勿 bootstrap-driver-seed 覆盖 xlang_asm。
    if [ -n "${XLANG_BSTRICT_RUN_ALL:-}" ]; then
        if [ ! -x "$XLANG" ]; then
            echo "run-all.sh: XLANG_BSTRICT_RUN_ALL but $XLANG build not executable" >&2
            exit 127
        fi
    ...
    # 修复：Windows 下跳过 bootstrap-driver-seed，所有测试用 xlang-c（C 前端）。
    case "$(uname -s 2>/dev/null)" in
      MINGW*|MSYS*|CYGWIN*)
      ...
      *)
    export XLANG_RUN_ALL_BOOTSTRAP_XLANG=1
    # 【Why 根源治理 xlang-c 被覆盖】bootstrap-driver-seed 依赖 $(XLANG_C)=xlang-c，
    # 而 xlang-c 默认 target 是 `cp -f bootstrap_xlangc xlang-c`（Makefile line 708），
    ...
    # 【Why 根源治理 .o 污染】xlang_compile_std_x.sh auto 优先选 xlang_asm/xlang（seed），
    # 而 seed 在 macOS -backend asm 产出错误 .o 且 exit=0 不回退。
    # 修复：强制 std .o 用 xlang-c 编译（与测试程序同源）。
    ...
    if [ -f compiler/xlang ] && [ compiler/xlang -ef "$XLANG" ]; then
        ...
        mv compiler/xlang compiler/xlang.bak
        cp "$XLANG" compiler/xlang
    ...
    # 阻止子脚本再次 `make all` / `make xlang-c`：默认 xlang-c target 是 `cp -f bootstrap_xlangc xlang-c`，
    # 会覆盖上面刚刚构建好的真正 C 前端（XLANG_LEGACY_C_FRONTEND=1），导致回归全部失败。
    export XLANG_SKIP_SUBSCRIPT_MAKE=1
...
run_xlang_for_script() {
    ...
    case "$(uname -m 2>/dev/null)" in
      x86_64|amd64) ;;
      *)
        if [ -x ./compiler/xlang-c ] && { [ -n "${XLANG_RUN_ALL_BOOTSTRAP_XLANG:-}" ] || [ -n "${XLANG_BSTRICT_RUN_ALL:-}" ]; }; then
            case "$script" in
              run-typeck.sh|run-check.sh|run-lexer.sh|run-preprocess.sh) ;;
              *)
                echo "./compiler/xlang-c"
...
run() {
    ...
    # CI Linux：run-asm-* 依赖 macOS otool 验 arm64 spill；asm 由 asm-smoke / build_xlang_asm job 覆盖。
    if { [ -n "${GITHUB_ACTIONS:-}" ] || [ -n "${CI:-}" ]; } && [ -z "${XLANG_CI_FORCE_ASM:-}" ]; then
        case "$script" in
          run-asm*.sh|run-enum-asm.sh)
            echo "run-all SKIP (CI: $script; asm otool gates need macOS or XLANG_CI_FORCE_ASM=1)"
            return 0
            ;;
        esac
    fi
    ...
    echo "[$(date +%H:%M:%S)] run-all START tests/$script (XLANG=$run_shu)"
    if [ -n "${GITHUB_ACTIONS:-}" ] || [ -n "${CI:-}" ]; then
        echo "run-all: → $script"
        s=0; XLANG="$run_shu" ./tests/$script || s=$?
        if [ "$s" -ne 0 ]; then
            # B-strict 全量：非白名单走 xlang-c，失败仅记 SKIP（L5 只门禁 xlang_asm 白名单）。
            if [ -n "${XLANG_BSTRICT_RUN_ALL:-}" ] && ! run_all_l5_whitelist_case "$script"; then
                echo "run-all SKIP (non-whitelist xlang-c): $script (exit $s)"
...
# 词法 / 类型检查 / 最小可运行
run_required run-no-legacy-xlang-gate.sh
run run-lexer.sh
run run-typeck.sh
run run-hello.sh
run run-check.sh
...
run run-vector.sh
# asm 反汇编门禁须 seed/asm 后端；test_c（RUN_ALL_USE_C + xlang-c）仅验 C 流水线语义，见 test_x / linux-asm-smoke
...
# async 门禁进主套件（平台守卫：非目标平台 exit 0，不红 run-all）
# 见 analysis/async-platform-matrix.md §3.1
run run-async.sh
run run-no-libc-gate.sh
run run-iocp-gate.sh
run run-ub.sh
...
if [ -n "${XLANG_BSTRICT_RUN_ALL:-}" ] && [ "${RUN_BSTRICT_SKIP_COUNT:-0}" -gt 0 ]; then
    echo "run-all-bstrict: L5 whitelist OK; skipped $RUN_BSTRICT_SKIP_COUNT non-whitelist xlang-c script(s)"
fi
echo "all tests OK"
