#!/usr/bin/env bash
# F-simd v1：std.simd 去 C（simd.c → simd.sx；F-ZC 纯 .sx 无 os glue）。
set -e
cd "$(dirname "$0")/.."
...
echo "=== F-simd v1: simd.sx (F-ZC zero C) ==="
[ -f "$DOC" ] || die "missing $DOC"
grep -q 'F-simd v1' "$DOC" || die "doc marker"
[ -f std/simd/simd.sx ] || die "missing simd.sx"
...
grep -q 'simd_autovec_smoke_c' std/simd/simd.sx || die "simd.sx missing smoke"
grep -q 'simd_f_zero_c_marker_c' std/simd/simd.sx || die "simd.sx missing zero-c marker"
grep -q 'simd.sx' compiler/Makefile || die "Makefile missing simd.sx"
