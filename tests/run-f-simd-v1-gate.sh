...
# std-simd-autovec-strategy / leftover nested std-simd-prod /
# leftover nested std-simd-intrinsic / leftover nested
...
# leftover nested ensure / leftover nested std-simd-autovec-strategy /
# leftover nested std-simd-prod / leftover nested std-simd-intrinsic /
# leftover nested std-simd-shuffle-select (refuse leftover SKIP→OK /
# leftover ignore of explicit-bad / leftover XLANG fallthrough).
# leftover nested product path stays when XLANG is unset (do not
# rewrite leftover xlang_compiler_make / std-simd-autovec-strategy /
# std-simd-prod / std-simd-intrinsic / std-simd-shuffle-select).
# PLATFORM: SHARED — product path honesty; Ubuntu gold still required.
if [ -n "${XLANG:-}" ]; then
