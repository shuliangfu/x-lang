...
DOC="${SHU_STD_NET_TLS_DOC:-analysis/std-net-tls-v1.md}"
MANIFEST="${SHU_STD_NET_TLS_TSV:-tests/baseline/std-net-tls.tsv}"
NET_SU="std/net/mod.su"
NET_C="std/net/net.c"
LIB="tests/lib/std-net-tls.sh"
STUB_SU="tests/net/tls_stub.su"
RUNTIME_SU="tests/net/tls_runtime_link_smoke.su"
...
if [ -n "$SHU_BIN" ]; then
  echo "=== STD-030: typeck + stub smoke (SHU=$SHU_BIN) ==="
  ...
  if ! "$SHU_BIN" check -L . "$STUB_SU" >/dev/null 2>&1; then
    echo "std-net-tls gate FAIL: typeck $STUB_SU" >&2
    "$SHU_BIN" check -L . "$STUB_SU" 2>&1 | tail -10 >&2 || true
    std_net_tls_emit_report "fail" 0 0 0 0 0
    exit 1
  fi
  TYPECK_OK=1
  exe="/tmp/shu_std_net_tls_stub_$$"
  set +e
  link_log=$(SHU_NET_TLS=stub "$SHU_BIN" -L . "$STUB_SU" -o "$exe" 2>&1)
  link_ec=$?
  set -e
  ...
  echo "std-net-tls gate SKIP smoke (no native shu-c)" >&2
fi

OPENSSL_OK=0
if std_net_tls_probe_openssl; then
  echo "=== STD-083: OpenSSL TLS handshake smoke ==="
  TLS_PORT="${SHU_TLS_SMOKE_PORT:-9876}"
  TLS_CERT="/tmp/shu_tls_cert_$$.pem"
  TLS_KEY="/tmp/shu_tls_key_$$.pem"
  TLS_PID="/tmp/shu_tls_spid_$$"
  if openssl req -x509 -newkey rsa:2048 -keyout "$TLS_KEY" -out "$TLS_CERT" -days 1 -nodes \
    ...
    if std_net_tls_build_openssl_o && std_net_tls_start_s_server "$TLS_PID" "$TLS_PORT" "$TLS_CERT" "$TLS_KEY"; then
      ...
      export SHU_TLS_SMOKE_PORT="$TLS_PORT"
...
if std_net_tls_probe_mbedtls; then
  ...
  TLS_PORT_MB="${SHU_TLS_SMOKE_PORT_MB:-9877}"
  TLS_CERT_MB="/tmp/shu_tls_cert_mb_$$.pem"
  TLS_KEY_MB="/tmp/shu_tls_key_mb_$$.pem"
  TLS_PID_MB="/tmp/shu_tls_spid_mb_$$"
  if openssl req -x509 -newkey rsa:2048 -keyout "$TLS_KEY_MB" -out "$TLS_CERT_MB" -days 1 -nodes \
    ...
    if std_net_tls_build_mbedtls_o && std_net_tls_start_s_server "$TLS_PID_MB" "$TLS_PORT_MB" "$TLS_CERT_MB" "$TLS_KEY_MB"; then
      make -C compiler ../std/io/io.o >/dev/null 2>&1 || true
      export SHU_TLS_SMOKE_PORT="$TLS_PORT_MB"
...
RUNTIME_LINK_OK=0
if [ -n "$SHU_BIN" ] && std_net_tls_probe_openssl; then
  echo "=== STD-083: runtime auto TLS link (shu-c + net-o-openssl) ==="
  if std_net_tls_build_openssl_o && std_net_tls_runtime_link_smoke "$SHU_BIN"; then
    RUNTIME_LINK_OK=1
  fi
  std_net_tls_restore_stub_o
else
  echo "std-net-tls gate SKIP runtime link smoke (no shu or libssl)" >&2
fi
