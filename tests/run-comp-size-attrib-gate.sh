#!/usr/bin/env bash
# COMP-010: size attribution manifest gate (false-authority honesty).
#
# Honesty: soft SKIP→OK in run-comp-size-attrib.sh retired (2026-08-27). Prefer
...
die() {
  echo "comp-size-attrib gate FAIL: $*" >&2
  echo "${PREFIX} status=fail host=$(ci_host_summary)"
  exit 1
}

# Refuse resurrecting top-level DOC (archive is authority).
if [ -f analysis/comp-size-attrib-v1.md ]; then
  die "refuse top-level analysis/comp-size-attrib-v1.md (use archive/comp)"
fi
...
for f in "$DOC" "$MANIFEST" "$MATRIX" \
  ...
  tests/run-size-xlang-asm-gate.sh tests/baseline/xlang-asm-size.tsv \
