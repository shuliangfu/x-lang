...
- **模块**：`import("std.io")` / `import("core.mem")`；目录即模块，入口为 `mod.sx`
...
```bash
./compiler/shux examples/hello.sx          # 编译并运行（需已构建 shux）
./compiler/shux build examples/hello.sx -o hello && ./hello
./compiler/shux check examples/hello.sx      # 仅 parse + typeck
```
...
构建配置入口为根目录 [`build.sx`](build.sx)：对标 Zig 的 `build.zig`，描述「怎么编、编什么」；顶层 Makefile 仍作委托与 bootstrap，长期由 `build.sx` 完全替代（阶段 G-05）。
...
├── docs/                     # 语言语法文档（面向用户）
├── compiler/                 # 编译器（.sx 主链 + C seed / glue，向物理零 C）
│   ├── src/
...
| 路径 | 说明 |
|------|------|
| **用户编译** | `.sx` → driver → asm/c 后端 → ld → 可执行文件 |
...
| 前端运行时 `.sx` | ✅ lexer / parser / ast / typeck / codegen / pipeline 默认 `*_sx.o` |
...
| M5 | 自举（`.sx` 编译器可编译自身） | ✅ D+E+F v1；🟡 阶段 G 终局 |
...
| 语言服务器 | `compiler/src/lsp/` | `lsp.sx`、`lsp_diag.sx` 等；需 `compiler/shux` 支持 `--lsp` |
...
2. 修改 `.sx` 源码；改后运行 `./tests/run-all.sh` 或相关 gate。
