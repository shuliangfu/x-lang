...
#
# 用法：
#   ./bench/diff/run_diff.sh              # 跑全部 D1-D6
#   ./bench/diff/run_diff.sh d1 d2        # 跑指定用例
#   XLANG=./compiler/xlang_asm ./bench/diff/run_diff.sh
#
...
if [ "$os" = "Darwin" ] && [ "$arch" = "arm64" ]; then
  ...
  echo "      请在 Linux x86_64 上运行：ssh ubuntu-server && cd ~/worker/xlang/x-lang && ./bench/diff/run_diff.sh"
  exit 0
fi
