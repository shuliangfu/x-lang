/**
 * bench/i07_io_read_async.c — IO-A5 v1 submit_read_async + complete_read_async 烟测
 *
 * 用 pipe 写入 3 字节；submit 后 complete 读回。非 Linux 走 io_read 回退同步 complete。
 *
 * 编译：cc -std=c11 -o io_read_async bench/i07_io_read_async.c std/io/io.o
 */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    if (xlang_io_submit_read_async(buf, sizeof(buf), (size_t)(unsigned)fds[0]) != 0) {
