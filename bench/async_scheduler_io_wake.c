/**
 * bench/async_scheduler_io_wake.c — IO-A5 IO 等待队列 + completion 唤醒烟测
 *
 * XLANG_ASYNC_YIELD=1 + XLANG_ASYNC_IO_WAIT=1：suspend 任务进 IO 等待队列；
 * xlang_async_io_wake_all 后 drain 完成。
 *
 * 编译：cc -std=c11 -o async_scheduler_io_wake bench/async_scheduler_io_wake.c std/async/scheduler.o
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
