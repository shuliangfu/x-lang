/**
 * bench/i07_io_write_async_multi.c — IO-A5 v2 多 write in-flight 烟测
 *
 * 双 pipe 同时 submit 两个 write async，再按 slot 分别 complete。
 *
 * 编译：cc -std=c11 -o io_write_async_multi bench/i07_io_write_async_multi.c std/io/io.o
 */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    slot_a = xlang_io_submit_write_async(pay_a, 2, (size_t)(unsigned)fds_a[1]);
    slot_b = xlang_io_submit_write_async(pay_b, 3, (size_t)(unsigned)fds_b[1]);
