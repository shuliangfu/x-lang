/**
 * bench/async_scheduler_mpsc.c — A4 MPSC 就绪队列烟测（双线程 submit + 主线程 drain）
 *
 * 两生产者线程各 submit 一次-suspend 任务，主线程 drain 至全部完成。
 * 编译：cc -std=c11 -pthread -o async_scheduler_mpsc bench/async_scheduler_mpsc.c std/async/scheduler.o
 */
#include <pthread.h>
#include <stdint.h>
#include <stdio.h>
