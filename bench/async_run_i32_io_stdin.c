/**
 * bench/async_run_i32_io_stdin.c — xlang_async_run_i32 + IO await stdin 烟测
 *
 * 验证 run_i32 在 IO 等待队列非空时 wake/drain 直至完成。
 *
 * 编译：cc -std=c11 -o async_run_i32_io_stdin \
 *   bench/async_run_i32_io_stdin.c std/io/io.o std/async/scheduler.o
 * 运行：printf abcd | XLANG_ASYNC_YIELD=1 ./async_run_i32_io_stdin
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
static int32_t read_four_task(void) {
    ...
    if (g_step == 0) {
        g_step = 1;
        g_phase = 0;
        g_slot = xlang_io_submit_read_async(g_buf, 4, 0);
        if (g_slot < 0)
            return -1;
...
int main(void) {
    ...
    r = xlang_async_run_i32(read_four_task);
    if (r != 4) {
