/**
 * bench/i06_async_scheduler_io_read_multi_e2e.c — 双协程并行 read async + scheduler IO wake
 *
 ...
 *
 * 编译：cc -std=c11 -o async_scheduler_io_read_multi_e2e bench/i06_async_scheduler_io_read_multi_e2e.c \
...
int main(void) {
    ...
    if (xlang_async_task_submit(io_read_task_a) != 0
        || xlang_async_task_submit(io_read_task_b) != 0) {
