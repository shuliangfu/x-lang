/**
 * bench/i06_async_run_io_spawn_workers.c — A4 v5 spawn 并行 IO + XLANG_ASYNC_WORKERS=2
 *
 * 双 pipe + 双 task submit（round-robin 至 worker 0/1）+ drain_until_idle + poll。
 *
 * 编译：cc -std=c11 -pthread -o async_run_io_spawn_workers \
 *   bench/i06_async_run_io_spawn_workers.c std/io/io.o std/async/scheduler.o
 * 运行：XLANG_ASYNC_YIELD=1 XLANG_ASYNC_WORKERS=2 ./async_run_io_spawn_workers
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
extern uint32_t xlang_async_worker_count(void);
...
/**
 * 入口：XLANG_ASYNC_WORKERS=2 + spawn 并行 IO drain。
 */
int main(void) {
    int fds_a[2];
    int fds_b[2];
