/**
 * bench/i06_async_runtime_trace_smoke.c — OBS-002 烟测：XLANG_ASYNC_RUNTIME_TRACE Top-N 输出
 *
 * 提交带 suspend 的任务并 drain，验证 stderr 含 trace summary 与 rank 行。
 *
 * 编译：cc -std=gnu11 -o async_runtime_trace_smoke \
 *         bench/i06_async_runtime_trace_smoke.c std/async/scheduler.o
 * 运行：XLANG_ASYNC_RUNTIME_TRACE=1 ./async_runtime_trace_smoke
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
extern void xlang_async_runtime_trace_flush(void);
...
int main(void) {
    ...
    setenv("XLANG_ASYNC_RUNTIME_TRACE", "1", 1);
    setenv("XLANG_ASYNC_RUNTIME_TRACE_TOPN", "5", 1);
    setenv("XLANG_ASYNC_RUNTIME_TRACE_SAMPLE", "1", 1);

    g_step = 0;
    if (xlang_async_task_submit(trace_smoke_task) != 0) {
