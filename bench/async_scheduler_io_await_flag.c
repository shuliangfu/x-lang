/**
 * bench/async_scheduler_io_await_flag.c — IO-A5 xlang_async_cps_suspend_io 烟测
 *
 * 仅 XLANG_ASYNC_YIELD=1（无 XLANG_ASYNC_IO_WAIT）：IO await suspend 仍进 IO 等待队列。
 *
 * 编译：cc -std=c11 -o async_scheduler_io_await_flag bench/async_scheduler_io_await_flag.c std/async/scheduler.o
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
