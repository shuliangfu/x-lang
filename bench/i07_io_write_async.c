/**
 * bench/i07_io_write_async.c — IO-A5 v1 submit_write_async + complete_write_async 烟测
 *
 * pipe 写端 submit/complete 写入 3 字节；读端校验 payload。
 *
 * 编译：cc -std=c11 -o io_write_async bench/i07_io_write_async.c std/io/io.o
 */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    if (xlang_io_submit_write_async(payload, 3, (size_t)(unsigned)fds[1]) < 0) {
