/**
 * bench/i06_async_scheduler_io_read_e2e.c — IO-A5 scheduler + read async 端到端烟测
 *
 * 模拟 codegen 路径：submit_read_async → suspend_io → IO 等待队列 → wake → complete。
 * 父进程写 pipe；子任务 drain/wake/drain 读回 3 字节。
 *
 * 编译：cc -std=c11 -o async_scheduler_io_read_e2e bench/i06_async_scheduler_io_read_e2e.c \
