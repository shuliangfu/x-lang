/**
 * bench/async_scheduler_queue.c — A4 就绪队列烟测
 *
 * 验证 xlang_async_task_submit + xlang_async_scheduler_drain：
 *   1) 单任务一次 suspend 后 resume 完成；
 *   2) 双任务穿插（先 suspend 的任务让出，immediate 任务先完成）。
 *
 * 编译：cc -o async_scheduler_queue bench/async_scheduler_queue.c std/async/scheduler.o
 */
#include <stdint.h>
#include <stdio.h>
...
int main(void) {
    ...
    g_step_a = 0;
    if (xlang_async_task_submit(task_suspend_once) != 0) {
    ...
    if (xlang_async_task_submit(task_immediate) != 0) {
